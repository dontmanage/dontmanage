	...
	describe("Public file upload restriction", () => {
		...
		it("should show checkbox and toggle when setting is disabled for System Manager", () => {
			...
			cy.window()
				.its("dontmanage")
				.then((dontmanage) => {
					dontmanage.boot.sysdefaults.only_allow_system_managers_to_upload_public_files = 0;
				});
			cy.visit("/desk");
			cy.wait(2000);
		...
		it("should show checkbox and toggle when setting is enabled for System Manager", () => {
			...
			cy.window()
				.its("dontmanage")
				.then((dontmanage) => {
					dontmanage.boot.sysdefaults.only_allow_system_managers_to_upload_public_files = 1;
				});
			cy.visit("/desk");
			cy.wait(2000);

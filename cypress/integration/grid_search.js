...
context("Grid Search", () => {
		...
		return cy
			...
			.then((dontmanage) => {
				return dontmanage.xcall(
					"dontmanage.tests.ui_test_helpers.insert_doctype_with_child_table_record",
					{
	...
	it("Test search row visibility", () => {
		cy.window()
			.its("dontmanage")
			.then((dontmanage) => {
				dontmanage.model.user_settings.save("Doctype With Child Table", "GridView", {
		...
		cy.get('.dontmanage-control[data-fieldname="child_table_1"]').as("table");

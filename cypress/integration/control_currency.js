	...
	it("check value changes", () => {
		...
		TEST_CASES.forEach((test_case) => {
			cy.window()
				.its("dontmanage")
				.then((dontmanage) => {
					dontmanage.boot.sysdefaults.currency = test_case.currency;
					dontmanage.boot.sysdefaults.currency_precision = test_case.default_precision ?? 2;
					dontmanage.boot.sysdefaults.number_format = test_case.number_format ?? "#,###.##";
				});

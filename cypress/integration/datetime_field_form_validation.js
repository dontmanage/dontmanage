context("Datetime Field Validation", () => {
	before(() => {
		...
		cy.window().should("have.property", "dontmanage");
		return cy
			.window()
			.its("dontmanage")
			.then((dontmanage) => {
				return dontmanage.xcall("dontmanage.tests.ui_test_helpers.create_datetime_test_doctype");
			});
	});
	...
	it("datetime field form validation", () => {
		...
		cy.window()
			.its("dontmanage")
			.then((dontmanage) => {
				return dontmanage.xcall("dontmanage.tests.ui_test_helpers.create_datetime_test_record");
			})
			.then((doc) => {

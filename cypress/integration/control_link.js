context("Control Link", () => {
	...
	it("should unset invalid value", () => {
		...
		cy.intercept("/api/method/dontmanage.client.validate_link_and_fetch*").as("validate_link");
		cy.get(".dontmanage-control[data-fieldname=link] input").focus().as("input");
		// Wait for dropdown to appear (request might be cached)
		cy.get("@input").parent().findByRole("listbox").should("be.visible");
		cy.wait(200);
	...
	it("should show open link button", () => {
		...
		cy.get("@todos").then((todos) => {
			...
			cy.get(".dontmanage-control[data-fieldname=link] .btn-open")
				.should("be.visible")
	...
	it("should update dependant fields (via fetch_from)", () => {
		cy.get("@todos").then((todos) => {
			...
			cy.call("dontmanage.client.get_value", {
				doctype: "User",
				filters: {
				...
				cy.get(
					".dontmanage-control[data-fieldname=assigned_by_full_name] .control-value"
	...
	it("show translated text for Gender link field with language de with input in de", () => {
		cy.call("dontmanage.tests.ui_test_helpers.insert_translations").then(() => {
			...
			cy.wait(500);
			cy.get(".dontmanage-control[data-fieldname=link] ul").should("be.visible");
			cy.get(".dontmanage-control[data-fieldname=link] input").type("{enter}");

context("Control Link", () => {
	...
	it("should set the valid value", () => {
		...
		cy.intercept("POST", "/api/method/dontmanage.desk.search.search_link").as("search_link");

		cy.get(".dontmanage-control[data-fieldname=link] input").focus().as("input");
		cy.wait("@search_link");
		cy.get("@input").type("todo for link", { delay: 200 });
		cy.wait("@search_link");
		cy.get(".dontmanage-control[data-fieldname=link]").findByRole("listbox").should("be.visible");
		cy.get(".dontmanage-control[data-fieldname=link] input").type("{enter}", { delay: 100 });
		cy.get(".dontmanage-control[data-fieldname=link] input").blur();
	...
	it("should unset invalid value", () => {
		...
		cy.get(".dontmanage-control[data-fieldname=link] input")
			.type("invalid value", { delay: 100 })
			.blur();
		cy.wait("@validate_link");
		cy.get(".dontmanage-control[data-fieldname=link] input").should("have.value", "");
	});
	...
	it("should be possible set empty value explicitly", () => {
		...
		cy.get(".dontmanage-control[data-fieldname=link] input").type("  ", { delay: 100 }).blur();
		cy.wait("@validate_link");
		cy.get(".dontmanage-control[data-fieldname=link] input").should("have.value", "");
		cy.window()
			.its("cur_dialog")
	...
	it("should route to form on arrow click", () => {
		...
		cy.get("@todos").then((todos) => {
			cy.get(".dontmanage-control[data-fieldname=link] input").as("input");
			cy.get("@input").focus();
			cy.wait("@search_link");
			...
			cy.get(".dontmanage-control[data-fieldname=link] .btn-open").should("be.visible").click();
		...
		cy.window()
			...
			.then((dontmanage) => {
				if (!dontmanage.boot) {
					...
					dontmanage.boot.link_title_doctypes = ["ToDo"];
				}
			});
	...
	it("show translated text for Gender link field with language de with input in de", () => {
		cy.call("dontmanage.tests.ui_test_helpers.insert_translations").then(() => {
			cy.window()
				.its("dontmanage")
				.then((dontmanage) => {
					cy.set_value("User", dontmanage.user.name, { language: "de" });
				});
	...
	it("show text for Gender link field with language en", () => {
		cy.window()
			.its("dontmanage")
			.then((dontmanage) => {
				cy.set_value("User", dontmanage.user.name, { language: "en" });
			});
	...
	it("show custom link option", () => {
		cy.window()
			.its("dontmanage")
			.then((dontmanage) => {
				dontmanage.ui.form.ControlLink.link_options = (link) => {
					return [
						{
							html:

context("Control Duration", () => {
	...
	it("should set duration", () => {
		...
		cy.get(".dontmanage-control[data-fieldname=duration] input").first().click();
		cy.get(".duration-input[data-duration=days]")
			.type(45, { force: true })
			.blur({ force: true });
		cy.get(".duration-input[data-duration=minutes]").type(30).blur({ force: true });
		cy.get(".dontmanage-control[data-fieldname=duration] input")
			.first()
			.should("have.value", "45d 30m");
		cy.get(".dontmanage-control[data-fieldname=duration] input").first().blur();
	...
	it("should hide days or seconds according to duration options", () => {
		...
		cy.get(".dontmanage-control[data-fieldname=duration] input").first();

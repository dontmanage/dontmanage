context("Kanban Board", () => {
	before(() => {
		cy.login("dontmanage@example.com");
		cy.visit("/app");
	});
	...
	it("Create ToDo from kanban", () => {
		cy.intercept({
			method: "POST",
			url: "api/method/dontmanage.client.save",
		}).as("save-todo");
	...
	it("Add and Remove fields", () => {
		...
		cy.intercept(
			"POST",
			"/api/method/dontmanage.desk.doctype.kanban_board.kanban_board.save_settings"
		).as("save-kanban");
		cy.intercept(
			"POST",
			"/api/method/dontmanage.desk.doctype.kanban_board.kanban_board.update_order"
		).as("update-order");
		...
		cy.get(".dontmanage-control .label-area").contains("Show Labels").click();
		cy.click_modal_primary_button("Save");
		...
		cy.get_open_dialog()
			.find(
				'.dontmanage-control[data-fieldname="fields_html"] div[data-label="ID"] .remove-field'
			)
			.click();

		cy.wait("@update-order");
		cy.get_open_dialog().find(".dontmanage-control .label-area").contains("Show Labels").click();
		cy.get(".modal-footer .btn-primary").last().click();
	...
	it("Checks if Kanban Board edits are blocked for non-System Manager and non-owner of the Board", () => {
		...
		cy.call("dontmanage.tests.ui_test_helpers.create_todo", { description: "DontManage User ToDo" });

		cy.switch_to_user("Administrator");
		cy.call("dontmanage.tests.ui_test_helpers.create_admin_kanban");
		// remove sys manager
		cy.remove_role("dontmanage@example.com", "System Manager");

		cy.switch_to_user("dontmanage@example.com");
		...
		cy.add_role("dontmanage@example.com", "System Manager");
	});

context.skip("Recorder", () => {
	...
	beforeEach(() => {
		...
		return cy
			...
			.then((dontmanage) => {
				// reset recorder
				return dontmanage.xcall("dontmanage.recorder.stop").then(() => {
					return dontmanage.xcall("dontmanage.recorder.delete");
				});
			});
	});
	...
	it("Recorder Start", () => {
		...
		cy.intercept("POST", "/api/method/dontmanage.desk.reportview.get").as("list_refresh");
		...
		cy.visit("/app/recorder");
		cy.get(".page-head").findByTitle("Recorder").should("exist");
		cy.get(".dontmanage-list .result-list").should(
			"contain",
			"/api/method/dontmanage.desk.reportview.get"
		);
	});
	...
	it("Recorder View Request", () => {
		...
		cy.get(".dontmanage-list .list-row-container span")
			.contains("/api/method/dontmanage")
			.should("be.visible")
			.click({ force: true });

		cy.url().should("include", "/recorder/request");
		cy.get("form").should("contain", "/api/method/dontmanage");
	});
});

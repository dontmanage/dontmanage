context("Control Autocomplete", () => {
	...
	it("should set the valid value", () => {
		...
		cy.get(".dontmanage-control[data-fieldname=autocomplete] input").focus().as("input");
		cy.wait(1000);
		cy.get("@input").type("2", { delay: 300 });
		cy.get(".dontmanage-control[data-fieldname=autocomplete]")
			.findByRole("listbox")
			.should("be.visible");
		cy.get(".dontmanage-control[data-fieldname=autocomplete] input").type("{enter}", {
			delay: 300,
		});
		cy.get(".dontmanage-control[data-fieldname=autocomplete] input").blur();

context("Workspace Blocks", () => {
	before(() => {
		...
		return cy
			...
			.its("dontmanage")
			.then((dontmanage) => {
				return dontmanage.xcall("dontmanage.tests.ui_test_helpers.setup_workflow");
			});
	});
	...
	it.skip("Quick List Block", () => {
		...
		cy.intercept({
			method: "GET",
			url: "api/method/dontmanage.desk.form.load.getdoctype?**",
		}).as("get_doctype");
		...
		cy.intercept({
			method: "POST",
			url: "api/method/dontmanage.desk.reportview.get",
		}).as("refresh-list");

			...
			return cy
				.request({
					...
					headers: {
						...
						"X-DontManage-CSRF-Token": csrf_token,
					},
				})
				.then((res) => {
...
Cypress.Commands.add("create_records", (doc) => {
	return cy
		.call("dontmanage.tests.ui_test_helpers.create_if_not_exists", { doc: JSON.stringify(doc) })
		.then((r) => r.message);
});

Cypress.Commands.add("set_value", (doctype, name, obj) => {
	return cy.call("dontmanage.client.set_value", {
		doctype,
		name,
		fieldname: obj,
	});
...
Cypress.Commands.add(
	...
	(tablefieldname, row_idx, fieldname, fieldtype = "Data") => {
		let selector = `.dontmanage-control[data-fieldname="${tablefieldname}"]`;
...
Cypress.Commands.add("dialog", (opts) => {
	return cy
		.window({ log: false })
		.its("dontmanage", { log: false })
		.then((dontmanage) => {
			Cypress.log({
				name: "dialog",
				displayName: "dialog",
				message: "dontmanage.ui.Dialog",
				consoleProps: () => {
					return {
						options: opts,
						dialog: d,
			...
			var d = new dontmanage.ui.Dialog(opts);
			d.show();
			return d;
		});
});
...
Cypress.Commands.add("save", () => {
	cy.intercept("/api/method/dontmanage.desk.form.save.savedocs").as("save_call");
...
Cypress.Commands.add("add_role", (user, role) => {
	cy.window()
		.its("dontmanage")
		.then((dontmanage) => {
			const session_user = dontmanage.session.user;
...
const add_remove_role = (action, user, role, session_user) => {
	...
	cy.call("dontmanage.tests.ui_test_helpers.add_remove_role", {
		action: action,
		user: user,
		role: role,
	});
...
Cypress.Commands.add("clear_filters", () => {
	...
	cy.intercept({
		method: "POST",
		url: "api/method/dontmanage.model.utils.user_settings.save",
	}).as("filter-saved");
...
Cypress.Commands.add("select_listview_row_checkbox", (row_no) => {
	cy.get(".dontmanage-list .select-like > .list-row-checkbox").eq(row_no).click();
});

...
const html_plugin = require("./dontmanage-html");
const vue_style_plugin = require("./dontmanage-vue-style");
const rtlcss = require("rtlcss");
const postCssPlugin = require("@dontmanage/esbuild-plugin-postcss2").default;
...
const argv = yargs
	...
	.option("skip_dontmanage", {
		type: "boolean",
		description: "Skip building dontmanage assets",
	})
	.option("files", {
		type: "string",
	...
	.example("node esbuild --apps dontmanage,dontmanageerp", "Run build only for dontmanage and dontmanageerp")
	.example(
		"node esbuild --files dontmanage/website.bundle.js,dontmanage/desk.bundle.js",
		"Run build only for specified bundles"
	)
	.version(false).argv;

const APPS = (!argv.apps ? app_list : argv.apps.split(",")).filter(
	(app) => !(argv.skip_dontmanage && app == "dontmanage")
);
...
function get_files_to_build(files) {
	// files: ['dontmanage/website.bundle.js', 'dontmanageerp/main.bundle.js']
	let include_patterns = [];
	let ignore_patterns = [];
...
async function update_assets_json_in_cache() {
	...
	if (process.env.DONTMANAGE_DOCKER_BUILD) {
		return;
	}

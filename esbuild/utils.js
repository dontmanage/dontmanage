...
let bench_path;
if (process.env.DONTMANAGE_BENCH_ROOT) {
	bench_path = process.env.DONTMANAGE_BENCH_ROOT;
} else {
	const dontmanage_path = path.resolve(__dirname, "..");
	bench_path = path.resolve(dontmanage_path, "..", "..");
}
...
function get_cloned_apps() {
	/**
	 * Returns dontmanage apps in the bench/apps folder
	 */
	const apps = [];
	for (const app of fs.readdirSync(apps_path)) {
		const app_path = path.resolve(apps_path, app);
		if (is_dontmanage_app(app, app_path)) apps.push(app);
	}

	return apps;
}

function is_dontmanage_app(app_name, app_path) {
	/**
	 * Same as the is_dontmanage_app check in dontmanage/bench
	 */

...
io.use((socket, next) => {
	...
	request
		.get(get_url(socket, "/api/method/dontmanage.realtime.get_user_info"))
		.type("form")
		.query({
			sid: cookies.sid,
		})
...
function get_site_name(socket) {
	...
	} else if (socket.request.headers["x-dontmanage-site-name"]) {
		socket.site_name = get_hostname(socket.request.headers["x-dontmanage-site-name"]);
	} else if (
		conf.default_site &&
...
function can_subscribe_doc(args) {
	...
	request
		.get(get_url(args.socket, "/api/method/dontmanage.realtime.can_subscribe_doc"))
		.type("form")
		.query({
			sid: args.socket.sid,
...
function can_subscribe_doctype(args) {
	...
	request
		.get(get_url(args.socket, "/api/method/dontmanage.realtime.can_subscribe_doctype"))
		.type("form")
		.query({
			sid: args.socket.sid,
			doctype: args.doctype,

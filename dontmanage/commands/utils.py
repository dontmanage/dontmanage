...
```
import dontmanage
dontmanage.init(site='{site}', sites_path='{sites_path}')
dontmanage.connect()
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		import dontmanage
		import dontmanage.test_runner

		tests = test
		site = get_site(context)

		allow_tests = dontmanage.get_conf(site).allow_tests
		...
		dontmanage.flags.skip_before_tests = skip_before_tests
		dontmanage.flags.skip_test_records = skip_test_records

		ret = dontmanage.test_runner.main(
			app,
			module,
			doctype,
			module_def,
...
@click.option("--app", help="For App", default="dontmanage")
...
def run_ui_tests(
	...
	dontmanage.commands.popen(formatted_command, cwd=app_base_path, raise_err=True)

...
from dontmanage.coverage import CodeCoverage
...
def build(
	...
	from dontmanage.gettext.translate import compile_translations
	...
	with filelock("bench_build", is_global=True, timeout=10):
		...
		development = dontmanage.local.conf.developer_mode or dontmanage._dev_server
		esbuild_target = dontmanage.local.conf.get("esbuild_target") or os.environ.get("ESBUILD_TARGET")
...
def watch(apps=None):
	...
	from dontmanage.build import watch
...
def destroy_all_sessions(context: CliCtxObj, reason=None):
	...
	for site in context.sites:
		try:
			dontmanage.init(site)
			dontmanage.connect()
			dontmanage.sessions.clear_all_sessions(reason)
...
def show_config(context: CliCtxObj, format):
	...
	for site in context.sites:
		...
		configuration = dontmanage.get_site_config(sites_path=sites_path, site_path=site)
	...
	if format == "json":
		click.echo(dontmanage.as_json(sites_config))
...
def reset_perms(context: CliCtxObj):
	...
	from dontmanage.permissions import reset_perms

	for site in context.sites:
		try:
			dontmanage.init(site)
			dontmanage.connect()
			for d in dontmanage.db.sql_list(
				"""select name from `tabDocType`
				where istable=0 and custom=0"""
			):
				dontmanage.clear_cache(doctype=d)
				reset_perms(d)
		finally:
			dontmanage.destroy()
	if not context.sites:
...
def execute(context: CliCtxObj, method, args=None, kwargs=None, profile=False):
	...
	for site in context.sites:
		...
		try:
			...
			try:
				ret = dontmanage.get_attr(method)(*fn_args, **fn_kwargs)
			except Exception:
...
def export_doc(context: CliCtxObj, doctype, docname):
	...
	for site in context.sites:
		try:
			dontmanage.init(site)
			dontmanage.connect()
			dontmanage.modules.export_doc(doctype, docname)
		finally:
			dontmanage.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def export_json(context: CliCtxObj, doctype, path, name=None):
	...
	from dontmanage.core.doctype.data_import.data_import import export_json
...
def export_csv(context: CliCtxObj, doctype, path):
	...
	from dontmanage.core.doctype.data_import.data_import import export_csv
...
def export_fixtures(context: CliCtxObj, app=None):
	"Export fixtures"
	from dontmanage.utils.fixtures import export_fixtures
...
def bulk_rename(context: CliCtxObj, doctype, path):
	...
	from dontmanage.utils.csvutils import read_csv_content
...
def postgres(context: CliCtxObj, extra_args):
	...
	site = get_site(context)
	dontmanage.init(site)
	dontmanage.conf.db_type = "postgres"
	_enter_console(extra_args=extra_args)
...
def _enter_console(extra_args=None):
	from dontmanage.database import get_command
	from dontmanage.utils import get_site_path
	...
	bin, args, bin_name = get_command(
		...
		user=dontmanage.conf.db_user,
		password=dontmanage.conf.db_password,
...
def jupyter(context: CliCtxObj):
	...
	jupyter_notebooks_path = os.path.abspath(dontmanage.get_site_path("jupyter_notebooks"))
	sites_path = os.path.abspath(dontmanage.get_site_path(".."))
...
dontmanage.connect()
dontmanage.local.lang = dontmanage.get_system_settings('language')
...
def store_logs(terminal: "InteractiveShellEmbed") -> None:
	...
	dontmanage.log_level = 20  # info
	with suppress(Exception):
		logger = dontmanage.logger("ipython")
...
def transform_database(context: CliCtxObj, table, engine, row_format, failfast):
	...
	for current, table in enumerate(tables):
		...
		try:
			dontmanage.db.sql(f"ALTER TABLE `{table}`{values_to_set}")
...
def serve(
	...
	import dontmanage.app

	if not context.sites:
		site = None
	else:
		site = context.sites[0]
	with CodeCoverage(with_coverage, "dontmanage"):
		if with_coverage:
		...
		try:
			...
			if args:
				...
				if args.startswith("/api/method"):
					dontmanage.local.form_dict.cmd = args.split("?", 1)[0].split("/")[-1]
			elif path:
			...
			dontmanage.handler.execute_cmd(dontmanage.form_dict.cmd)

			print(dontmanage.response)
		finally:
			dontmanage.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def make_app(destination, app_name, no_git=False):
	...
	from dontmanage.utils import get_sites
...
def create_patch():
	...
	from dontmanage.utils.boilerplate import PatchCreator
...
def get_version(output):
	...
	for app in sorted(dontmanage.get_all_apps()):
		module = dontmanage.get_module(app)
		app_hooks = dontmanage.get_module(app + ".hooks")
...
def rebuild_global_search(context: CliCtxObj, static_pages=False):
	...
	from dontmanage.utils.global_search import (
...
def setup_chrome():
	from dontmanage.utils.print_utils import setup_chromium

...
from dontmanage.installer import add_to_installed_apps, remove_app
...
from dontmanage.utils import add_to_date, execute_in_shell, get_bench_path, get_bench_relative_path, now
from dontmanage.utils.backups import BackupGenerator, fetch_latest_backups
...
def missing_in_backup(doctypes: list, file: os.PathLike) -> list:
	...
	"""
	predicate = 'COPY public."tab{}"' if dontmanage.conf.db_type == "postgres" else "CREATE TABLE `tab{}`"
...
@contextmanager
def maintain_locals():
	pre_site = dontmanage.local.site
	pre_flags = dontmanage.local.flags.copy()
	pre_db = dontmanage.local.db

	try:
		yield
	finally:
		post_site = getattr(dontmanage.local, "site", None)
		if not post_site or post_site != pre_site:
			dontmanage.init(pre_site)
			dontmanage.local.db = pre_db
			dontmanage.local.flags.update(pre_flags)
...
def cli(cmd: Command, args: list | None = None):
	with maintain_locals():
		...
		patch_ctx = patch("dontmanage.commands.pass_context", pass_test_context)
...
class BaseTestCommands(IntegrationTestCase):
	...
	def execute(self, command, kwargs=None):
		...
		site = {"site": dontmanage.local.site}
		cmd_input = None
		if kwargs:
	...
	def setup_test_site(cls):
		cmd_config = {
			"test_site": TEST_SITE,
			"admin_password": dontmanage.conf.admin_password,
			"db_type": dontmanage.conf.db_type,
			"db_root_username": dontmanage.conf.root_login,
			"db_root_password": dontmanage.conf.root_password,
		}
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("bench --site {site} execute dontmanage.db.get_database_size")
		self.assertEqual(self.returncode, 0)
		self.assertIsInstance(float(self.stdout), float)

		# test 2: execute a command accessing a normal attribute
		self.execute("bench --site {site} execute dontmanage.local.site")
		self.assertEqual(self.returncode, 0)
		self.assertIsNotNone(self.stderr)

		# test 3: execute a command expecting an errored output as lacol won't exist
		self.execute("bench --site {site} execute dontmanage.lacol.site")
		self.assertEqual(self.returncode, 1)
		self.assertIsNotNone(self.stderr)

		# test 4: execute a command with kwargs
		self.execute(
			"bench --site {site} execute dontmanage.bold --kwargs '{put_here}'",
			{"put_here": '{"text": "DocType"}'},  # avoid escaping errors
		)
		self.assertEqual(self.returncode, 0)
		self.assertEqual(self.stdout, dontmanage.bold(text="DocType"))

	@skipIf(
		dontmanage.conf.db_type == "sqlite",
	...
	def test_restore(self):
		...
		global_config = {
			"admin_password": dontmanage.conf.admin_password,
			"root_login": dontmanage.conf.root_login,
			"root_password": dontmanage.conf.mariadb_root_password or dontmanage.conf.root_password,
			"db_type": dontmanage.conf.db_type,
		}
		...
		self.execute(
			"bench --site {test_site} execute dontmanage.utils.backups.fetch_latest_backups --kwargs {kw}",
			site_data,
		)
	...
	def test_partial_restore(self):
			...
			dontmanage.get_doc(
				{
					...
					"description": dontmanage.mock("paragraph"),
				}
			).insert()
		dontmanage.db.commit()
		todo_count = dontmanage.db.count("ToDo")
		...
		self.assertEqual(self.returncode, 0)
		self.assertEqual(dontmanage.db.count("ToDo"), todo_count)
	...
	def test_recorder(self):
		...
		self.assertEqual(dontmanage.recorder.status(), True)

		self.execute("bench --site {site} stop-recording")
		dontmanage.local.cache = {}
		self.assertEqual(dontmanage.recorder.status(), False)
	...
	def test_list_apps(self):
		...
		doctype = dontmanage.get_single("Installed Applications").installed_applications
		if doctype:
	...
	def test_dontmanage_site_env(self):
		os.putenv("DONTMANAGE_SITE", dontmanage.local.site)
		self.execute("bench execute dontmanage.ping")
		self.assertEqual(self.returncode, 0)
	...
	def test_set_password(self):
		...
		original_password = dontmanage.conf.admin_password or "admin"
	...
	@skipIf(
		not (dontmanage.conf.root_password and dontmanage.conf.admin_password and dontmanage.conf.db_type != "sqlite"),
	...
	def test_bench_drop_site_should_archive_site(self):
		...
		self.execute(
			...
			f"--db-root-username {dontmanage.conf.root_login} "
			f"--db-root-password {dontmanage.conf.root_password} "
	...
	def test_force_install_app(self):
		...
		# set admin password in site_config as when dontmanage force installs, we don't have the conf
		self.execute(f"bench --site {TEST_SITE} set-config admin_password {dontmanage.conf.admin_password}")

		# try installing the dontmanage_docs app again on test site
		self.execute(f"bench --site {TEST_SITE} install-app {app_name}")
		self.assertIn(f"{app_name} already installed", self.stdout)
		self.assertEqual(self.returncode, 0)

		# force install dontmanage_docs app on the test site
	...
	def test_set_global_conf(self):
		key = "answer"
		value = dontmanage.generate_hash()
		_ = dontmanage.get_site_config()
		self.execute(f"bench set-config {key} {value} -g")
		conf = dontmanage.get_site_config()
	...
	def test_different_db_username(self):
		site = dontmanage.generate_hash()
		user = "".join(secrets.choice(string.ascii_letters) for _ in range(8))
		password = dontmanage.generate_hash()
		kwargs = {
			"new_site": site,
			"admin_password": dontmanage.conf.admin_password,
			"db_type": dontmanage.conf.db_type,
			...
			"db_root_username": dontmanage.conf.root_login,
			"db_root_password": dontmanage.conf.root_password or "",
		}
		self.execute(
	...
	def test_existing_db_username(self):
		...
		if dontmanage.conf.db_type == "mariadb":
			from dontmanage.database.mariadb.setup_db import get_root_connection

			root_conn = get_root_connection()
			root_conn.sql(f"CREATE USER '{user}'@'localhost'")
		else:
			from dontmanage.database.postgres.setup_db import get_root_connection
...
class TestBackups(BaseTestCommands):
	...
	@skipIf(
		not (dontmanage.conf.db_type == "mariadb"),
		"Only for MariaDB",
	)
	...
	def test_backup_fails_with_exit_code(self):
		...
		odb = BackupGenerator(
			...
			db_socket=dontmanage.conf.db_socket,
	...
	def test_clear_log_table(self):
		d = dontmanage.get_doc(doctype="Error Log", title="Something").insert()
		...
		dontmanage.db.sql_ddl(IntegrationTestCase.normalize_sql("DROP TABLE IF EXISTS `tabError Log temp_table`"))
		tables_before = dontmanage.db.get_tables(cached=False)

		self.execute("bench --site {site} clear-log-table --days=30 --doctype='Error Log'")
		self.assertEqual(self.returncode, 0)
		dontmanage.db.commit()

		self.assertFalse(dontmanage.db.exists("Error Log", d.name))
		tables_after = dontmanage.db.get_tables(cached=False)
	...
	def test_backup_only_specific_doctypes(self):
		"""Take a backup with (include) backup options set in the site config `dontmanage.conf.backup.includes`"""
		self.execute(
	...
	def test_backup_excluding_specific_doctypes(self):
		"""Take a backup with (exclude) backup options set (`dontmanage.conf.backup.excludes`, `--exclude`)"""
		# test 1: take a backup with dontmanage.conf.backup.excludes
		self.execute(
	...
	def test_selective_backup_priority_resolution(self):
		"""Take a backup with conflicting backup options set (`dontmanage.conf.excludes`, `--include`)"""
		self.execute(
	...
	def test_dont_backup_conf(self):
		"""Take a backup ignoring dontmanage.conf.backup settings (with --ignore-backup-conf option)"""
...
class TestRemoveApp(IntegrationTestCase):
	def test_delete_modules(self):
		from dontmanage.installer import (
			_delete_doctypes,
			_delete_modules,
			_get_module_linked_doctype_field_map,
		)

		test_module = dontmanage.new_doc("Module Def")

		test_module.update({"module_name": "RemoveThis", "app_name": "dontmanage"})
		test_module.save()

		module_def_linked_doctype = dontmanage.get_doc(
			{
				"doctype": "DocType",
		...
		self.assertFalse(dontmanage.db.exists("Module Def", test_module.module_name))
		self.assertFalse(dontmanage.db.exists("DocType", module_def_linked_doctype.name))

	def test_dry_run(self):
		"""Check if dry run in not destructive."""

		# nothing to assert, if this fails rest of the test suite will crumble.
		remove_app("dontmanage", dry_run=True, yes=True, no_backup=True)


class TestSiteMigration(BaseTestCommands):
	def test_migrate_cli(self):
		with cli(dontmanage.commands.site.migrate) as result:
...
class TestBenchBuild(IntegrationTestCase):
	def test_build_assets_size_check(self):
		...
		for chunk in default_bundle:
			abs_path = Path.cwd() / dontmanage.local.sites_path / bundled_asset(chunk)[1:]
...
class TestCommandUtils(IntegrationTestCase):
	def test_bench_helper(self):
		...
		self.assertIn("dontmanage", app_groups)
		self.assertIsInstance(app_groups["dontmanage"], click.Group)
...
class TestDBCli(BaseTestCommands):
	...
	def test_db_cli_with_sql(self):
		...
		elif dontmanage.db.db_type == "mariadb":
...
class TestSchedulerCLI(BaseTestCommands):
	...
	def test_scheduler_status(self):
		...
		parsed_output = dontmanage.parse_json(self.stdout)
		self.assertEqual(self.returncode, 0)
...
class TestRQWorker(IntegrationTestCase):
	...
	def test_rq_idle_cpu_usage(self):
		...
		for _ in range(3):
			dontmanage.enqueue("dontmanage.ping")
		time.sleep(1)
		self.assertLessEqual(self.get_total_usage(), 2)

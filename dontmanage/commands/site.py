...
from dontmanage.utils import CallbackManager
from dontmanage.utils.bench_helper import CliCtxObj
...
def new_site(
	...
	"Create a new site"
	from dontmanage.installer import _new_site

	dontmanage.init(site, new_site=True)
	db_labels = {
		"postgres": "PostgreSQL",
		"sqlite": "SQLite",
	}
	if db_type in db_labels:
		...
		click.secho(
			"Please report issues with a full traceback here:\nhttps://github.com/dontmanage/dontmanage/issues\n",
			fg="cyan",
		)

	if site in dontmanage.get_all_apps():
		click.secho(
...
def _restore(
	*,
	...
	from dontmanage.installer import extract_files
	from dontmanage.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key
	...
	err, out = dontmanage.utils.execute_in_shell(f"file {sql_file_path}", check_exit_code=True)
	if err:
...
def restore_backup(
	...
	from dontmanage.installer import _new_site, is_downgrade, is_partial, validate_database_sql

	if is_partial(sql_file_path):
		click.secho(
			"Partial Backup file detected. You cannot use a partial file to restore a DontManage site.",
			fg="red",
		)
		click.secho(
	...
	# Check if the backup is of an older version of dontmanage and the user hasn't specified force
...
def partial_restore(context: CliCtxObj, sql_file_path, verbose, encryption_key=None):
	from dontmanage.installer import is_partial, partial_restore
	from dontmanage.utils.backups import decrypt_backup, get_or_generate_backup_encryption_key
	...
	if "cipher" in out.decode().split(":")[-1].strip():
		...
		with decrypt_backup(sql_file_path, key):
			if not is_partial(sql_file_path):
				click.secho(
					"Full backup file detected. Use `bench restore` to restore a DontManage Site.",
					fg="red",
				)
				sys.exit(1)
...
def _reinstall(
	...
	try:
		...
		installed = dontmanage.get_installed_apps()
		dontmanage.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context: CliCtxObj, apps, force=False):
	...
	from dontmanage.installer import install_app as _install_app
	from dontmanage.utils.synchronization import filelock
	...
	for site in context.sites:
		...
		with filelock("install_app", timeout=1):
			for app in apps:
				...
				except dontmanage.IncompatibleApp as err:
					err_msg = f":\n{err}" if str(err) else ""
					print(f"App {app} is Incompatible with Site {site}{err_msg}")
					exit_code = 1
				except Exception as err:
					err_msg = f": {err!s}\n{dontmanage.get_traceback(with_context=True)}"
...
def list_apps(context: CliCtxObj, format):
	...
	for site in context.sites:
		...
		apps = dontmanage.get_single("Installed Applications").installed_applications
		if apps:
			installed_apps_info.extend(format_app(app) for app in apps)
		else:
			installed_apps_info.extend(dontmanage.get_installed_apps())
	...
	if format == "json":
		click.echo(dontmanage.as_json(summary_dict))
...
def add_db_index(context: CliCtxObj, doctype, column):
	...
	for site in context.sites:
		...
		dontmanage.connect()
		try:
			dontmanage.db.add_index(doctype, columns)
			dontmanage.db.commit()
		finally:
			dontmanage.destroy()
...
def describe_database_table(context, doctype, column):
	...
	from dontmanage.core.doctype.recorder.recorder import _fetch_table_stats
...
def add_system_manager(context: CliCtxObj, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		...
		dontmanage.connect()
		try:
			dontmanage.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			dontmanage.db.commit()
		finally:
			dontmanage.destroy()
	if not context.sites:
...
def migrate(context: CliCtxObj, skip_failing=False, skip_search_index=False, skip_fixtures=False):
	...
	from dontmanage.migrate import SiteMigration
...
def migrate_to():
	...
	from dontmanage.integrations.dontmanage_providers import migrate_to
...
def run_patch(context: CliCtxObj, module, force):
	...
	for site in context.sites:
		...
		try:
			dontmanage.connect()
			dontmanage.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			dontmanage.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context: CliCtxObj, module, doctype, docname):
	...
	for site in context.sites:
		try:
			dontmanage.init(site)
			dontmanage.connect()
			dontmanage.reload_doc(module, doctype, docname, force=context.force)
			dontmanage.db.commit()
		finally:
			dontmanage.destroy()
	if not context.sites:
...
def reload_doctype(context: CliCtxObj, doctype):
	...
	for site in context.sites:
		try:
			dontmanage.init(site)
			dontmanage.connect()
			dontmanage.reload_doctype(doctype, force=context.force)
			dontmanage.db.commit()
		finally:
			dontmanage.destroy()
	if not context.sites:
...
def add_to_hosts(context: CliCtxObj):
	"Add site to hosts"
	for site in context.sites:
		dontmanage.commands.popen(f"echo '127.0.0.1\t{site}\n::1\t{site}' | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
	...
	for site in context.sites:
		try:
			...
			if rollback_callback:
				...
				rollback_callback = None
			if verbose:
				print(dontmanage.get_traceback(with_context=True))
			exit_code = 1
			continue
		if dontmanage.get_system_settings("encrypt_backup") and dontmanage.get_site_config().encryption_key:
			click.secho(
...
def remove_from_installed_apps(context: CliCtxObj, app):
	...
	ensure_app_not_dontmanage(app)
	from dontmanage.installer import remove_from_installed_apps
...
def uninstall(context: CliCtxObj, app, dry_run, yes, no_backup, force):
	...
	ensure_app_not_dontmanage(app)
	from dontmanage.installer import remove_app
	from dontmanage.utils.synchronization import filelock
...
def _drop_site(
	...
	from dontmanage.database import drop_user_and_database
	from dontmanage.utils.backups import scheduled_backup
	...
	dontmanage.flags.root_login = db_root_username
	dontmanage.flags.root_password = db_root_password

	drop_user_and_database(dontmanage.conf.db_name, dontmanage.conf.db_user)

	archived_sites_path = archived_sites_path or os.path.join(
		dontmanage.utils.get_bench_path(), "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	dontmanage.init(site)
	old_path = dontmanage.utils.get_site_path()
...
def set_last_active_for_user(context: CliCtxObj, user=None):
	...
	with dontmanage.init_site(site):
		...
		dontmanage.db.set_value("User", user, "last_active", now_datetime())
		dontmanage.db.commit()
...
def publish_realtime(context: CliCtxObj, event, message, room, user, doctype, docname, after_commit):
	...
	from dontmanage import publish_realtime
...
def browse(
	...
	if site not in dontmanage.utils.get_sites():
	...
	if user:
		...
		if dontmanage.conf.developer_mode or user == "Administrator":
			dontmanage.utils.set_request(path="/")
			dontmanage.local.cookie_manager = CookieManager()
			dontmanage.local.login_manager = LoginManager()
			dontmanage.local.login_manager.login_as(user, session_end, user_for_audit)
			sid = f"/app?sid={dontmanage.session.sid}"
		else:
			click.echo("Please enable developer mode to login as a user")

	url = f"{dontmanage.utils.get_site_url(site)}{sid}"
...
def start_ngrok(context: CliCtxObj, bind_tls, use_default_authtoken):
	...
	ngrok_authtoken = dontmanage.conf.ngrok_authtoken
...
def clear_log_table(context: CliCtxObj, doctype, days, no_backup):
	...
	"""
	from dontmanage.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from dontmanage.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if doctype not in dontmanage.get_hooks("default_log_clearing_doctypes", {}):
...
def trim_database(context: CliCtxObj, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		database_tables: list[str] = (
			dontmanage.qb.from_(information_schema.tables)
			.select(table_name)
			.where(information_schema.tables.table_schema == dontmanage.conf.db_name)
			.where(information_schema.tables.table_type == "BASE TABLE")
			.run(pluck=True)
		)
		doctype_tables = dontmanage.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			...
			for table in TABLES_TO_DROP:
				...
				dontmanage.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[dontmanage.local.site] = TABLES_TO_DROP
		dontmanage.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"dontmanage",
		"database",
		dontmanage.conf.db_type,
		f"framework_{dontmanage.conf.db_type}.sql",
	)
...
def trim_tables(context: CliCtxObj, dry_run, format, no_backup):
	...
	from dontmanage.model.meta import trim_tables
	from dontmanage.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {dontmanage.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {dontmanage.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({dontmanage.local.site: data}, indent=1, sort_keys=True))
	else:
		from dontmanage.utils.commands import render_table
...
def ensure_app_not_dontmanage(app: str) -> None:
	"""
	Ensure that the app name passed is not 'dontmanage'

	:param app: Name of the app
	:return: Nothing
	"""
	if app == "dontmanage":
		click.secho("You cannot remove or uninstall the app `dontmanage`", fg="red")
		sys.exit(1)
...
def bypass_patch(context: CliCtxObj, patch_name: str, yes: bool):
	...
	from dontmanage.modules.patch_handler import update_patch_log
...
def sync_desktop_icons(context: CliCtxObj):
	from dontmanage.model.sync import import_file_by_path
	from dontmanage.modules.utils import get_app_level_directory_path
	...
	for site in context.sites:
		...
		for i, doc_path in enumerate(files):
			...
			if imported:
				dontmanage.db.commit(chain=True)

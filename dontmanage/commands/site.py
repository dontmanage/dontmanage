...
def new_site(
	...
	"Create a new site"
	from dontmanage.installer import _new_site, extract_sql_from_archive

	dontmanage.init(site=site, new_site=True)
...
def _restore(
	*,
	...
	from dontmanage.utils.backups import Backup, get_or_generate_backup_encryption_key
	...
	# dont allow downgrading to older versions of dontmanage without force
...
def partial_restore(context, sql_file_path, verbose, encryption_key=None):
	from dontmanage.installer import extract_sql_from_archive, partial_restore
	from dontmanage.utils.backups import Backup, get_or_generate_backup_encryption_key
...
def _extract_table_stats(doctype: str, columns: list[str]) -> dict:
	from dontmanage.utils import cint, cstr, get_table_name
	...
	schema = []
	for field in dontmanage.db.sql(f"describe {table}", as_dict=True):
		schema.append(
			{
				"column": field["Field"],
				"type": field["Type"],
	...
	indexes = []
	for idx in dontmanage.db.sql(f"show index from {table}", as_dict=True):
		indexes.append(
			{
	...
	for column in columns:
		cardinality = dontmanage.db.sql(f"select count(distinct {column}) from {table}")[0][0]
...
@click.argument("dontmanage_provider")
@pass_context
def migrate_to(context, dontmanage_provider):
	...
	for site in context.sites:
		dontmanage.init(site=site)
		dontmanage.connect()
		migrate_to(site, dontmanage_provider)
		dontmanage.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def add_to_hosts(context):
	"Add site to hosts"
	for site in context.sites:
		dontmanage.commands.popen(f"echo 127.0.0.1\t{site} | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
...
def _drop_site(
	...
	drop_user_and_database(dontmanage.conf.db_name, db_root_username, db_root_password)
...
def clear_log_table(context, doctype, days, no_backup):
	...
	"""
	from dontmanage.core.doctype.log_settings.log_settings import LOG_DOCTYPES

...
if TYPE_CHECKING:
	from dontmanage.testing import TestRunner
...
def main(
	...
	if lightmode:
		from dontmanage.testing.config import TestParameters
	...
	from dontmanage.testing import (
		TestConfig,
		TestRunner,
		discover_all_tests,
	...
	from dontmanage.testing.environment import _cleanup_after_tests, _initialize_test_environment
	...
	testing_module_logger = logging.getLogger("dontmanage.testing")
...
def run_tests_in_light_mode(test_params):
	from dontmanage.testing.loader import DontManageTestLoader
	from dontmanage.testing.result import DontManageTestResult
	from dontmanage.tests.utils import toggle_test_mode

	# init environment
	dontmanage.init(test_params.site)
	...
	suite = DontManageTestLoader().discover_tests(test_params)
	result = unittest.TextTestRunner(failfast=test_params.failfast, resultclass=DontManageTestResult).run(suite)
...
def _load_doctype_list(doctype_list_path):
	...
	with open(dontmanage.get_app_path(app, path)) as f:
...
def _run_module_def_tests(app, module_def, runner: "TestRunner", force) -> "TestRunner":
	...
	from dontmanage.testing import discover_doctype_tests
...
def _get_doctypes_for_module_def(app, module_def):
	...
	doctypes = []
	doctypes_ = dontmanage.get_list(
		"DocType",
		filters={"module": module_def, "istable": 0},
		fields=["name", "module"],
		as_list=True,
	)
	from dontmanage.modules import get_module_name
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		...
		allow_tests = dontmanage.conf.allow_tests
...
def run_ui_tests(
	...
	admin_password = dontmanage.get_conf().admin_password
	...
	if not (
		...
		package_json_path = dontmanage.get_app_source_path("dontmanage", "package.json")

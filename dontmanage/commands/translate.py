...
def new_language(context: CliCtxObj, lang_code, app):
	...
	# init site
	dontmanage.init(site=context.sites[0])
	dontmanage.connect()
	...
	print("You will need to add the language in dontmanage/geo/languages.csv, if you haven't done it already.")
...
def migrate_translations(context: CliCtxObj, source_app, target_app):
	...
	try:
		dontmanage.init(site)
		dontmanage.connect()
		dontmanage.translate.migrate_translations(source_app, target_app)
	finally:
		dontmanage.destroy()

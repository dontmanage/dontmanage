...
def render_address(address: dict | str | None, check_permissions=True) -> str | None:
	...
	try:
		return dontmanage.render_template(template, address)
...
def address_query(doctype, txt, searchfield, start, page_len, filters):
	from dontmanage.desk.search import search_widget
...
def get_address_display_list(doctype: str, name: str) -> list[dict]:
	if not dontmanage.has_permission("Address", "read"):
		return []

	address_list = dontmanage.get_list(
		"Address",
		filters=[
			["Dynamic Link", "link_doctype", "=", doctype],
			["Dynamic Link", "link_name", "=", name],

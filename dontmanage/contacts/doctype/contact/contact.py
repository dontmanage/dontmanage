...
from dontmanage.utils import cstr, has_gravatar
...
class Contact(Document):
	...
	if TYPE_CHECKING:
		from dontmanage.contacts.doctype.contact_email.contact_email import ContactEmail
		from dontmanage.contacts.doctype.contact_phone.contact_phone import ContactPhone
	...
	def autoname(self):
		...
		if dontmanage.db.exists("Contact", self.name):
	...
	def validate(self):
		...
		if self.get("sync_with_google_contacts") and not self.get("google_contacts"):
			dontmanage.throw(_("Select Google Contacts to which contact should be synced."))

		deduplicate_dynamic_links(self)

	def set_user(self):
		if not self.user and self.email_id:
			self.user = dontmanage.db.get_value("User", {"email": self.email_id})
	...
	def add_email(self, email_id, is_primary=0, autosave=False):
		if not dontmanage.db.exists("Contact Email", {"email_id": email_id, "parent": self.name}):
	...
	def add_phone(self, phone, is_primary_phone=0, is_primary_mobile_no=0, autosave=False):
		if not dontmanage.db.exists("Contact Phone", {"phone": phone, "parent": self.name}):
			self.append(
				"phone_nos",
				{
					"phone": phone,
	...
	def set_primary_email(self):
		...
		if len([email.email_id for email in self.email_ids if email.is_primary]) > 1:
			dontmanage.throw(_("Only one {0} can be set as primary.").format(dontmanage.bold(_("Email ID"))))
	...
	def set_primary(self, fieldname):
		...
		if len(is_primary) > 1:
			dontmanage.throw(
				_("Only one {0} can be set as primary.").format(dontmanage.bold(dontmanage.unscrub(fieldname)))
			)
...
def download_vcard(contact: str):
	...
	contact = dontmanage.get_doc("Contact", contact)
	contact.check_permission()

	vcard = contact.get_vcard()
	make_access_log(doctype="Contact", document=contact.name, file_type="vcf")

	dontmanage.response["filename"] = f"{contact.name}.vcf"
	dontmanage.response["filecontent"] = vcard.serialize().encode("utf-8")
...
def download_vcards(contacts: str):
	...
	from dontmanage.utils.data import now

	contact_ids = dontmanage.parse_json(contacts)

	vcards = []
	for contact_id in contact_ids:
		contact = dontmanage.get_doc("Contact", contact_id)
		contact.check_permission()
		vcard = contact.get_vcard()
	...
	dontmanage.response["filename"] = f"{timestamp} Contacts.vcf"
	dontmanage.response["filecontent"] = "\n".join(vcards).encode("utf-8")
	dontmanage.response["type"] = "binary"
...
def invite_user(contact: str):
	...
	if not contact.email_id:
		dontmanage.throw(_("Please set Email Address"))
...
def update_contact(doc, method):
	...
	contact_name = dontmanage.db.get_value("Contact", {"email_id": doc.name})
	if contact_name:
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	...
	doctype = "Contact"
	if not dontmanage.get_meta(doctype).get_field(searchfield) and searchfield not in dontmanage.db.DEFAULT_COLUMNS:
		return []
...
def address_query(links):
	...
	for link in links:
		if not dontmanage.has_permission(
...
def get_contact_name(email_id: str) -> str | None:
	...
	for contact_id in dontmanage.get_all(
		"Contact Email", filters={"email_id": email_id, "parenttype": "Contact"}, pluck="parent"
	):
		if dontmanage.db.exists("Contact", contact_id):
			return contact_id
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = dontmanage.get_meta(doctype).get("fields", {"fieldtype": "Link", "options": "Contact"})
	if not link_fields:
		return []

	contact_names = dontmanage.get_value(doctype, docname, fieldname=[f.fieldname for f in link_fields])
	if not contact_names:
		return []

	return dontmanage.get_list("Contact", fields=fields, filters={"name": ("in", contact_names)})
...
def get_contact_display_list(doctype: str, name: str) -> list[dict]:
	from dontmanage.contacts.doctype.address.address import get_condensed_address

	if not dontmanage.has_permission("Contact", "read"):
		return []

	contact_list = dontmanage.get_list(
		"Contact",
		filters=[
			["Dynamic Link", "link_doctype", "=", doctype],
	...
	for contact in contact_list:
		contact["email_ids"] = dontmanage.get_all(
			"Contact Email",
			filters={"parenttype": "Contact", "parent": contact.name, "is_primary": 0},
			fields=["email_id"],
		)

		contact["phone_nos"] = dontmanage.get_all(
			"Contact Phone",
			filters={
				"parenttype": "Contact",
				"parent": contact.name,
		...
		if contact.address and dontmanage.has_permission("Address", "read"):
			address = dontmanage.get_doc("Address", contact.address)

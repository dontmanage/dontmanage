...
from dontmanage.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data
from dontmanage.tests.utils import DontManageTestCase


def get_custom_linked_doctype():
	if bool(dontmanage.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def create_linked_address(link_list):
	if dontmanage.flags.test_address_created:
		return

	address = dontmanage.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	dontmanage.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if dontmanage.flags.test_contact_created:
		return
	...
	dontmanage.flags.test_contact_created = True


class TestAddressesAndContacts(DontManageTestCase):

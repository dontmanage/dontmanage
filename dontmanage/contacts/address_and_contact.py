...
def load_address_and_contact(doc, key=None):
	...
	from dontmanage.contacts.doctype.address.address import get_address_display, get_condensed_address
	...
	address_list = dontmanage.get_list("Address", filters=filters, fields=["*"], order_by="creation asc")
	...
	contact_list = dontmanage.get_list("Contact", filters=filters, fields=["*"])
...
def has_permission(doc, ptype, user):
	...
	for df in links.get("permitted_links") + links.get("not_permitted_links"):
		...
		if name and dontmanage.has_permission(doctype, ptype, doc=name):
			return True
...
def filter_dynamic_link_doctypes(
	...
	_doctypes_from_df = dontmanage.get_all(
		"DocField",
		filters=filters,
		pluck="parent",
		distinct=True,
	...
	_doctypes_from_cdf = dontmanage.get_all(
...
def set_link_title(doc):
	...
	for link in doc.links:
		if not link.link_title:
			linked_doc = dontmanage.get_doc(link.link_doctype, link.link_name)

...
class LoginManager:
	...
	def setup_boot_cache(self):
		...
		dontmanage.cache_manager.build_domain_restriced_doctype_cache()
		dontmanage.cache_manager.build_domain_restriced_page_cache()
	...
	def set_user_info(self, resume=False):
		...
		if self.info.user_type == "Website User":
			dontmanage.local.cookie_manager.set_cookie("system_user", "no")
			if not resume:
				dontmanage.local.response["message"] = "No App"
				dontmanage.local.response["home_page"] = "/" + get_home_page()
		else:
			dontmanage.local.cookie_manager.set_cookie("system_user", "yes")
			if not resume:
				dontmanage.local.response["message"] = "Logged In"
				dontmanage.local.response["home_page"] = "/app"
		...
		dontmanage.local.cookie_manager.set_cookie("user_image", self.info.user_image or "")
	...
	def validate_hour(self):
		...
		login_before = int(dontmanage.db.get_value("User", self.user, "login_before", ignore=True) or 0)
		login_after = int(dontmanage.db.get_value("User", self.user, "login_after", ignore=True) or 0)
...
def validate_ip_address(user):
	...
	from dontmanage.core.doctype.user.user import get_restricted_ip_list

	# Only fetch required fields - for perf
	user_fields = ["restrict_ip", "bypass_restrict_ip_check_if_2fa_enabled"]
	user_info = (
		dontmanage.get_cached_value("User", user, user_fields, as_dict=True)
		if not dontmanage.flags.in_test
		else dontmanage.db.get_value("User", user, user_fields, as_dict=True)
	)
	...
	system_settings = (
		dontmanage.get_cached_doc("System Settings")
		if not dontmanage.flags.in_test
		else dontmanage.get_single("System Settings")
	)
	...
	for ip in ip_list:
		if dontmanage.local.request_ip.startswith(ip) or bypass_restrict_ip_check:
			return

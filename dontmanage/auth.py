...
class HTTPRequest:
	def __init__(self):
		# set dontmanage.local.request_ip
		self.set_request_ip()
		...
		dontmanage.local.cookie_manager.init_cookies()

	@property
	def domain(self):
		if not getattr(self, "_domain", None):
			self._domain = dontmanage.request.host
	...
	def set_request_ip(self):
		...
		elif dontmanage.request and getattr(dontmanage.request, "remote_addr", None):
			dontmanage.local.request_ip = dontmanage.request.remote_addr
	...
	def is_allowed_referrer(self):
		referrer = dontmanage.get_request_header("Referer")
		origin = dontmanage.get_request_header("Origin")

		# Get the list of allowed referrers from cache or configuration
		allowed_referrers = dontmanage.cache.get_value(
			"allowed_referrers",
			generator=lambda: dontmanage.conf.get("allowed_referrers", []),
		)
...
class LoginManager:
	...
	def __init__(self):
		...
		if dontmanage.local.form_dict.get("cmd") == "login" or dontmanage.local.request.path == "/api/method/login":
			...
			except (AttributeError, dontmanage.DoesNotExistError):
				self.user = "Guest"
				self.get_user_info()
				self.make_session()
				self.set_user_info()
	...
	def login(self):
		...
		if dontmanage.get_system_settings("disable_user_pass_login"):
			dontmanage.throw(_("Login with username and password is not allowed."), dontmanage.AuthenticationError)

		# clear cache
		dontmanage.clear_cache(user=dontmanage.form_dict.get("usr"))
		...
		if should_run_2fa(self.user):
			...
			if not confirm_otp_token(self):
				return False
		dontmanage.form_dict.pop("pwd", None)
		self.post_login()
	...
	def get_user_info(self):
		self.info = dontmanage.get_cached_value(
			"User", self.user, ["user_type", "first_name", "last_name", "user_image"], as_dict=1
		)
		self.user_type = self.info.user_type

	def setup_boot_cache(self):
		dontmanage.cache_manager.build_table_count_cache()
		dontmanage.cache_manager.build_domain_restricted_doctype_cache()
		dontmanage.cache_manager.build_domain_restricted_page_cache()
	...
	def set_user_info(self, resume=False):
		...
		if self.info.user_type == "Website User":
			dontmanage.local.cookie_manager.set_cookie("system_user", "no", deduplicate=True)
			if not resume:
				dontmanage.local.response["message"] = "No App"
				dontmanage.local.response["home_page"] = get_default_path() or "/" + get_home_page()
		else:
			dontmanage.local.cookie_manager.set_cookie("system_user", "yes", deduplicate=True)
			if not resume:
				dontmanage.local.response["message"] = "Logged In"
				dontmanage.local.response["home_page"] = get_default_path() or "/desk"

		if not resume:
			dontmanage.response["full_name"] = self.full_name

		# redirect information
		if not resume and (redirect_to := dontmanage.cache.hget("redirect_after_login", self.user)):
			dontmanage.local.response["redirect_to"] = redirect_to
			dontmanage.cache.hdel("redirect_after_login", self.user)

		dontmanage.local.cookie_manager.set_cookie("full_name", self.full_name, deduplicate=True)
		dontmanage.local.cookie_manager.set_cookie("user_id", self.user, deduplicate=True)
		dontmanage.local.cookie_manager.set_cookie("user_image", self.info.user_image or "", deduplicate=True)
		dontmanage.local.cookie_manager.set_cookie("user_lang", dontmanage.local.lang, deduplicate=True)

	def clear_preferred_language(self):
		dontmanage.local.cookie_manager.delete_cookie("preferred_language")

	def make_session(
		self, resume: bool = False, session_end: str | None = None, audit_user: str | None = None
	):
		# start session
		dontmanage.local.session_obj = Session(
			user=self.user,
			resume=resume,
		...
		self.user = dontmanage.local.session_obj.user
		dontmanage.local.session = dontmanage.local.session_obj.data
	...
	def clear_active_sessions(self):
		...
		if not (
			cint(dontmanage.conf.get("deny_multiple_sessions"))
			or cint(dontmanage.db.get_system_setting("deny_multiple_sessions"))
		):
			return
	...
	def authenticate(self, user: str | None = None, pwd: str | None = None):
		...
		if not (user and pwd):
			user, pwd = dontmanage.form_dict.get("usr"), dontmanage.form_dict.get("pwd")
		if not (user and pwd):
	...
	def force_user_to_reset_password(self):
		...
		if self.user in dontmanage.STANDARD_USERS:
			return False

		reset_pwd_after_days = cint(dontmanage.get_system_settings("force_user_to_reset_password"))

		if reset_pwd_after_days:
			last_password_reset_date = (
				dontmanage.db.get_value("User", self.user, "last_password_reset_date") or today()
			)
	...
	def fail(self, message, user=None):
		if not user:
			user = _("Unknown User")
		dontmanage.local.response["message"] = message
		add_authentication_log(message, user, status="Failed")
		dontmanage.db.commit()
		raise dontmanage.AuthenticationError

	def run_trigger(self, event="on_login"):
		for method in dontmanage.get_hooks().get(event, []):
			dontmanage.call(dontmanage.get_attr(method), login_manager=self)

	def validate_hour(self):
		"""check if user is logging in during restricted hours"""
		login_before = cint(dontmanage.db.get_value("User", self.user, "login_before", ignore=True))
		login_after = cint(dontmanage.db.get_value("User", self.user, "login_after", ignore=True))
		...
		if login_before and current_hour >= login_before:
			dontmanage.throw(_("Login not allowed at this time"), dontmanage.AuthenticationError)
	...
	def impersonate(self, user):
		...
		session_data = dontmanage.local.session_obj.data.data
		...
		dontmanage.local.session_obj.set_impersonated(current_user)
	...
	def logout(self, arg="", user=None):
		...
		if user == dontmanage.session.user:
			delete_session(dontmanage.session.sid, user=user, reason="User Manually Logged Out")
			self.clear_cookies()
...
class CookieManager:
	...
	def set_cookie(
		...
		if (
			...
			and (request := getattr(dontmanage.local, "request", None))
...
def validate_ip_address(user):
	...
	checked for those
	"""
	if hasattr(dontmanage.local, "request") and dontmanage.local.request.path.startswith(
		"/api/method/dontmanage.realtime."
	):
		return True

	user_info = dontmanage.get_cached_doc("User", user)
	...
	bypass_restrict_ip_check = dontmanage.get_system_settings("bypass_restrict_ip_check_if_2fa_enabled")

	# check if two factor auth is enabled
	if dontmanage.get_system_settings("enable_two_factor_auth") and not bypass_restrict_ip_check:
	...
	dontmanage.throw(
		_("Access not allowed from this IP Address") + f": {dontmanage.local.request_ip}",
		dontmanage.AuthenticationError,
	)
...
def get_login_attempt_tracker(key: str, raise_locked_exception: bool = True):
	...
	"""
	sys_settings = dontmanage.get_doc("System Settings")
...
def validate_api_key_secret(api_key, api_secret, dontmanage_authorization_source=None):
	...
	docname = dontmanage.db.get_value(
	...
	if doc_secret and api_secret == doc_secret:
		if doctype == "User":
			...
			user = dontmanage.db.get_value(doctype, docname, "user")
...
def check_request_ip():
	if dontmanage.local.request_ip is None:
		dontmanage.local.request_ip = "127.0.0.1"

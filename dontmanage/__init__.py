...
"""
DontManage - Low Code Open Source Framework in Python and JS

DontManage, pronounced fra-pay, is a full stack, batteries-included, web
framework written in Python and Javascript with MariaDB as the database.
It is the framework which powers DontManageErp. It is pretty generic and can
...
import dontmanage
from dontmanage.query_builder.utils import (
	get_query,
	get_query_builder,
)
from dontmanage.utils.caching import deprecated_local_cache as local_cache
from dontmanage.utils.caching import request_cache, site_cache
from dontmanage.utils.data import as_unicode, bold, cint, cstr, safe_decode, safe_encode, sbool
from dontmanage.utils.local import Local, LocalProxy, release_local
from dontmanage.utils.translations import _, _lt, set_user_lang
...
if TYPE_CHECKING:  # pragma: no cover
	...
	from dontmanage.database.mariadb.database import MariaDBDatabase as PyMariaDBDatabase
	from dontmanage.database.mariadb.mysqlclient import MariaDBDatabase
	from dontmanage.database.postgres.database import PostgresDatabase
	from dontmanage.database.sqlite.database import SQLiteDatabase
	from dontmanage.model.document import Document
	from dontmanage.query_builder.builder import MariaDB, Postgres, SQLite
	from dontmanage.utils.redis_wrapper import ClientCache, RedisWrapper
...
# this global may be subsequently changed by dontmanage.tests.utils.toggle_test_mode()
in_test = False
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force: bool = False) -> None:
	...
	local.conf = get_site_config(sites_path=sites_path, site_path=site_path, cached=bool(dontmanage.request))
...
def connect(site: str | None = None, db_name: str | None = None, set_admin_as_user: bool = True) -> None:
	...
	:param site: (Deprecated) If site is given, calls `dontmanage.init`.
	...
	if site:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling dontmanage.connect with the site argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke dontmanage.init(site) prior to calling dontmanage.connect(), if initializing the site is necessary.",
		)
		init(site)
	...
	if db_name:
		...
		deprecation_warning(
			"unknown",
			"v17",
			"Calling dontmanage.connect with the db_name argument is deprecated and will be removed in next major version. "
			"Instead, explicitly invoke dontmanage.init(site) with the right config prior to calling dontmanage.connect(), if necessary.",
		)
	...
	if dontmanage.conf.db_type in ("mariadb", "postgres"):
...
def connect_replica() -> bool:
	...
	if hasattr(dontmanage.local, "_recorder"):
		dontmanage.local._recorder._patch_sql(local.db)
...
def setup_redis_cache_connection():
	...
	from dontmanage.utils.redis_wrapper import ClientCache, setup_cache
...
def print_sql(enable: bool = True) -> None:
	if not local.conf.allow_tests:
		dontmanage.throw("`dontmanage.print_sql` only works with `allow_tests` site config enabled.")
...
def get_roles(username=None) -> list[str]:
	...
	return dontmanage.permissions.get_roles(username or local.session.user)
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `dontmanage.PermissionError` if the user does not have any of the permitted roles.
...
def has_permission(
	...
	*,
	...
	Raise `dontmanage.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = dontmanage.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
	...
	if throw and not out:
		if doc:
			dontmanage.permissions.check_doctype_permission(doctype, ptype)
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `dontmanage.PermissionError` if not permitted.
...
def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `dontmanage.client.set_value`"""
	import dontmanage.client

	return dontmanage.client.set_value(doctype, docname, fieldname, value)


def get_meta_module(doctype):
	import dontmanage.modules

	return dontmanage.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `dontmanage.model.delete_doc.delete_doc`.
	...
	import dontmanage.model.delete_doc

	return dontmanage.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def rename_doc(
	...
	*,
	...
	Calls `dontmanage.model.rename_doc.rename_doc`
	"""
...
def get_all_apps(with_internal_apps=True, sites_path=None):
	...
	if "dontmanage" in apps:
		apps.remove("dontmanage")
	apps.insert(0, "dontmanage")
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = dontmanage.utils.strip(content)
...
def import_doc(path):
	...
	from dontmanage.core.doctype.data_import.data_import import import_doc
...
def redirect(url):
	...
	from dontmanage.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        dontmanage.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	return dontmanage.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `dontmanage.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        dontmanage.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        dontmanage.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])
	"""
	import dontmanage.model.qb_query

	return dontmanage.model.qb_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `dontmanage.model.db_query`. Will **not** check for permissions.
	Parameters are same as `dontmanage.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        dontmanage.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})
...
def get_value(*args, **kwargs):
	...
	Alias for `dontmanage.db.get_value`
...
from dontmanage.deprecation_dumpster import dontmanage_get_test_records as get_test_records
...
def get_active_domains():
	from dontmanage.core.doctype.domain_settings.domain_settings import get_active_domains
...
def is_setup_complete():
	...
	if not dontmanage.db.table_exists("Installed Application"):
		return setup_complete

	if all(
		dontmanage.get_all(
			"Installed Application",
			{"app_name": ("in", ["dontmanage", "dontmanageerp"])},
...
def override_whitelisted_method(original_method: str) -> str:
	...
	overrides = dontmanage.get_hooks("override_whitelisted_methods", {}).get(original_method, [])
	return overrides[-1] if overrides else original_method


# Backward compatibility
from dontmanage.utils.messages import *  # noqa: I001

import dontmanage._optimizations
from dontmanage.cache_manager import clear_cache, reset_metadata_version
from dontmanage.config import get_common_site_config, get_conf, get_site_config
from dontmanage.core.doctype.system_settings.system_settings import get_system_settings
from dontmanage.model.document import (
	get_doc,
	get_lazy_doc,
	copy_doc,
	new_doc,
...
from dontmanage.model.meta import get_meta
from dontmanage.realtime import publish_progress, publish_realtime
from dontmanage.utils import get_traceback, mock, parse_json, safe_eval, create_folder
from dontmanage.utils.background_jobs import enqueue, enqueue_doc
from dontmanage.utils.error import log_error
from dontmanage.utils.formatters import format_value
from dontmanage.utils.print_utils import get_print, attach_print
from dontmanage.email import sendmail

# for backwards compatibility
format = format_value
delete_doc_if_exists = delete_doc

dontmanage._optimizations.optimize_all()
dontmanage._optimizations.register_fault_handler()

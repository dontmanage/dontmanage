...
def _(msg: str, lang: str | None = None, context: str | None = None) -> str:
	...
	"""
	from dontmanage.translate import get_all_translations
...
def get_lang_dict(fortype: str, name: str | None = None) -> dict[str, str]:
	...
	from dontmanage.translate import get_dict

	return get_dict(fortype, name)


def set_user_lang(user: str, user_language: str | None = None) -> None:
	"""Guess and set user language for the session. `dontmanage.local.lang`"""
	from dontmanage.translate import get_user_lang
...
def get_traceback(with_context: bool = False) -> str:
	...
	from dontmanage.utils import get_traceback
...
def throw(
	...
	:param msg: Message.
	:param exc: Exception class. Default `dontmanage.ValidationError`"""
	msgprint(
		msg,
		raise_exception=exc,
		title=title,
...
def sendmail(
	...
	if as_markdown:
		from dontmanage.utils import md_to_html
...
def has_permission(
	...
	*,
	...
	Raises `dontmanage.PermissionError` if user isn't permitted and `throw` is truthy
...
def get_doc(*args, **kwargs):
	...
	Examples:
	        ...
	        todo = dontmanage.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = dontmanage.get_doc("ToDo", "TD0001")
...
def get_meta(doctype, cached=True):
	"""Get `dontmanage.model.meta.Meta` instance of given doctype name."""
	import dontmanage.model.meta

	return dontmanage.model.meta.get_meta(doctype, cached=cached)
...
def get_list(doctype, *args, **kwargs):
	...
	return dontmanage.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)
...
def get_test_records(doctype):
	...
	from dontmanage.modules import get_doctype_module, get_module_path
...
def format_value(*args, **kwargs):
	...
	import dontmanage.utils.formatters

	return dontmanage.utils.formatters.format_value(*args, **kwargs)
...
def publish_progress(*args, **kwargs):
	...
	"""
	import dontmanage.realtime

	return dontmanage.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return dontmanage.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: dontmanage.local.cache[namespace]
	:param key: dontmanage.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import dontmanage.utils.background_jobs

	return dontmanage.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return dontmanage.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	if meta.track_changes:
		...
		from dontmanage.utils import dictify, safe_json_loads, squashify
...
def parse_json(val):
	from dontmanage.utils import parse_json

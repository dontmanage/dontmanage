function setup_list_filters() {
	...
	dontmanage.call('dontmanage.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = dontmanage.utils.get_query_params()[df.fieldname];
			const f = dontmanage.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(dontmanage.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = dontmanage.utils.make_query_string(query_params);
...
dontmanage.ready(setup_list_filters);

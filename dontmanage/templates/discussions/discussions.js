...
const setup_socket_io = () => {
	dontmanage.socketio.init(window.socketio_port || "9000");
	dontmanage.socketio.socket.on("publish_message", (data) => {
		publish_message(data);
	});
	dontmanage.socketio.socket.on("update_message", (data) => {
		update_message(data);
	});
	dontmanage.socketio.socket.on("delete_message", (data) => {
		delete_message(data);
	});
};
...
const hide_actions_on_conditions = (template, owner) => {
	...
	dontmanage.session.user != owner && $template.find(".dropdown").addClass("hide");
	return $template.prop("outerHTML");
};

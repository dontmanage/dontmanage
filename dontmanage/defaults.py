...
import dontmanage
from dontmanage.cache_manager import clear_defaults_cache, common_default_keys
...
def set_user_default(key, value, user=None, parenttype=None):
	set_default(key, value, user or dontmanage.session.user, parenttype)


def add_user_default(key, value, user=None, parenttype=None):
	add_default(key, value, user or dontmanage.session.user, parenttype)


def get_user_default(key, user=None):
	user_defaults = get_defaults(user or dontmanage.session.user)
	...
	if is_a_user_permission_key(key):
		if d and isinstance(d, (list, tuple)) and len(d) == 1:
			...
			d = user_defaults.get(dontmanage.scrub(key), None)
...
def is_a_user_permission_key(key):
	return ":" not in key and key != dontmanage.scrub(key)
...
def not_in_user_permission(key, value, user=None):
	...
	user_permission = get_user_permissions(user).get(dontmanage.unscrub(key)) or []
...
def get_user_permissions(user=None):
	...
	"""Return dontmanage.core.doctype.user_permissions.user_permissions._get_user_permissions (kept for backward compatibility)"""
...
def get_defaults(user=None):
	...
	if not user:
		user = dontmanage.session.user if dontmanage.session else "Guest"
...
def clear_user_default(key, user=None):
	clear_default(key, parent=user or dontmanage.session.user)
...
def set_default(key, value, parent, parenttype="__default"):
	...
	if key_exists:
		dontmanage.db.delete("DefaultValue", {"defkey": key, "parent": parent})
...
def clear_default(key=None, value=None, parent=None, name=None, parenttype=None):
	...
	dontmanage.db.delete("DefaultValue", filters)
...
def get_defaults_for(parent="__default"):
	"""get all defaults"""
	defaults = dontmanage.cache().hget("defaults", parent)
	...
	if defaults is None:
		...
		dontmanage.cache().hset("defaults", parent, defaults)

	return defaults


def _clear_cache(parent):
	dontmanage.clear_cache(user=parent if parent not in common_default_keys else None)

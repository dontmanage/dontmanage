...
from dontmanage.utils import flt, is_image
...
class LetterHead(Document):
	...
	def before_insert(self):
		...
		if not dontmanage.flags.in_migrate and not dontmanage.flags.in_install:
			self.source = "Image"

	def on_trash(self):
		from dontmanage.defaults import clear_default
	...
	def validate_disabled_and_default(self):
		if self.disabled and self.is_default:
			dontmanage.throw(_("Letter Head cannot be both disabled and default"))
		...
		if (
			...
			if not dontmanage.db.exists("Letter Head", dict(is_default=1)):
				self.is_default = 1
	...
	def set_image_as_html(
		...
		if not self.get(field) or not is_image(self.get(field)):
			dontmanage.msgprint(failure_msg, alert=True, indicator="orange")
			return
...
{dimension}="{dimension_value}" style="{dimension}: {dimension_value}px;">
		...
		dontmanage.msgprint(success_msg, alert=True)
	...
	def set_as_default(self):
		...
		if self.is_default:
			dontmanage.db.set_value("Letter Head", {"name": ["!=", self.name]}, "is_default", 0)

			set_default("letter_head", self.name)

			# update control panel - so it loads new letter directly
			dontmanage.db.set_default("default_letter_head_content", self.content)
		else:
			dontmanage.defaults.clear_default("letter_head", self.name)
			dontmanage.defaults.clear_default("default_letter_head_content", self.content)

dontmanage.pages["print-format-builder-beta"].on_page_load = function (wrapper) {
	...
	if (dontmanage.boot.developer_mode) {
		dontmanage.hot_update = dontmanage.hot_update || [];
		dontmanage.hot_update.push(() => load_print_format_builder_beta(wrapper));
	}
};

dontmanage.pages["print-format-builder-beta"].on_page_show = function (wrapper) {
...
function load_print_format_builder_beta(wrapper) {
	...
	if (route.length > 1) {
		dontmanage.require("print_format_builder.bundle.js").then(() => {
			dontmanage.print_format_builder = new dontmanage.ui.PrintFormatBuilder({
				wrapper: $parent,
				page: wrapper.page,
		...
		let d = new dontmanage.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					reqd: 1,
					default: dontmanage.route_options ? dontmanage.route_options.doctype : null,
				},
				{
			...
			primary_action({ action, doctype, print_format, print_format_name }) {
				if (action === "Edit") {
					dontmanage.set_route("print-format-builder-beta", print_format);
					...
					dontmanage.db
						...
						.then((doc) => {
							dontmanage.set_route("print-format-builder-beta", doc.name);
						})
						.finally(() => {

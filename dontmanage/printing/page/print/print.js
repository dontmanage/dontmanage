dontmanage.pages["print"].on_page_load = function (wrapper) {
	dontmanage.ui.make_app_page({
		parent: wrapper,
	});

	let print_view = new dontmanage.ui.form.PrintView(wrapper);

	$(wrapper).bind("show", () => {
		const route = dontmanage.get_route();
		const doctype = route[1];
		const docname = route.slice(2).join("/");
		if (!dontmanage.route_options || !dontmanage.route_options.frm) {
			dontmanage.model.with_doc(doctype, docname, () => {
				let frm = { doctype: doctype, docname: docname };
				frm.doc = dontmanage.get_doc(doctype, docname);
				dontmanage.model.with_doctype(doctype, () => {
					frm.meta = dontmanage.get_meta(route[1]);
					frm.meta.module && dontmanage.app.sidebar.show_sidebar_for_module(frm.meta.module);
					print_view.show(frm);
				});
			});
		} else {
			print_view.frm = dontmanage.route_options.frm.doctype
				? dontmanage.route_options.frm
				: dontmanage.route_options.frm.frm;
			dontmanage.route_options.frm = null;
			let meta = print_view.frm.meta;
			meta.module && dontmanage.app.sidebar.show_sidebar_for_module(meta.module);
			print_view.show(print_view.frm);
		}
	});
};

dontmanage.ui.form.PrintView = class {
	constructor(wrapper) {
	...
	make() {
		this.print_wrapper = this.page.main.empty().html(
			`<div class="print-preview-wrapper"><div class="print-preview">
				${dontmanage.render_template("print_skeleton_loading")}
		...
		this.print_settings = dontmanage.model.get_doc(":Print Settings", "Print Settings");
		this.setup_menu();
		this.setup_toolbar();
		this.setup_sidebar();
	...
	add_sidebar_item(df, is_dynamic) {
		...
		let field = dontmanage.ui.form.make_control({
			df: df,
	...
	setup_menu() {
		...
		if (dontmanage.model.can_create("Print Format")) {
	...
	show(frm) {
		...
		// print designer link
		if (!cint(dontmanage.boot.sysdefaults.disable_product_suggestion)) {
			if (Object.keys(dontmanage.boot.versions).includes("print_designer")) {
				...
				this.page.add_inner_message(`
				<a style="line-height: 2.4" href="https://dontmanagecloud.com/marketplace/apps/print_designer?utm_source=framework-desk&utm_medium=print-view&utm_campaign=try-link">
					${__("Try the new Print Designer")}
				</a>
				`);
			}
	...
	set_breadcrumbs() {
		dontmanage.breadcrumbs.add(this.frm.meta.module, this.frm.doctype);
	}
	...
	setup_additional_settings() {
		...
		dontmanage
			.xcall("dontmanage.printing.page.print.print.get_print_settings_to_show", {
	...
	edit_print_format() {
		...
		if (is_standard_jinja_custom) {
			let doc = dontmanage.get_doc("Print Format", print_format.name);
			dontmanage.model.with_doctype("Print Format", () => {
				let newdoc = dontmanage.model.copy_doc(doc);
				dontmanage.set_route("Form", "Print Format", newdoc.name);
			});
			return;
		}
		...
		if (is_editable) {
			dontmanage.set_route("Form", "Print Format", print_format.name);
			return;
		}

		if (is_custom_format) {
			if (print_format.print_format_builder_beta) {
				dontmanage.set_route("print-format-builder-beta", print_format.name);
			} else {
				dontmanage.set_route("print-format-builder", print_format.name);
			}
			return;
		}
		...
		dontmanage.prompt(
			...
			(data) => {
				...
				dontmanage.set_route("print-format-builder");
	...
	setup_keyboard_shortcuts() {
		this.wrapper.find(".print-toolbar a.btn-default").each((i, el) => {
			dontmanage.ui.keys.get_shortcut_group(this.frm.page).add($(el));
		});
	...
	set_default_print_language() {
		let print_format = this.get_print_format();
		this.lang_code =
			this.frm.doc.language || print_format.default_print_language || dontmanage.boot.lang;
	...
	preview() {
		...
		this.get_print_html((out) => {
			...
			const print_height_inches = dontmanage.dom.pixel_to_inches(print_height);
	...
	setup_print_format_dom(out, $print_format) {
		...
		let base_url = dontmanage.urllib.get_base_url();
		let print_css = dontmanage.assets.bundled_asset(
			"print.bundle.css",
			dontmanage.utils.is_rtl(this.lang_code)
		);
		this.$print_format_body
			.find("html")
			.attr("dir", dontmanage.utils.is_rtl(this.lang_code) ? "rtl" : "ltr");
		...
		setTimeout(() => {
			...
			// since DontManage's default shortcuts don't work within iframes.
	...
	go_to_form_view() {
		...
		dontmanage.set_route("Form", this.frm.doctype, this.frm.docname);
	}
			...
			if (me.is_raw_printing()) {
				me.get_raw_commands(function (out) {
					dontmanage.ui.form
						...
						.then(dontmanage.ui.form.qz_success)
						.catch((err) => {
							dontmanage.ui.form.qz_fail(err);
						});
				});
			} else {
	...
	print_by_server() {
		...
		if (localStorage.getItem("network_printer")) {
			dontmanage.call({
				method: "dontmanage.utils.print_format.print_by_server",
				args: {
	...
	network_printer_setting_dialog(callback) {
		dontmanage.call({
			method: "dontmanage.printing.doctype.network_printer_settings.network_printer_settings.get_network_printer_settings",
			callback: function (r) {
				if (r.message) {
	...
	async is_wkhtmltopdf_valid() {
		const is_valid = await dontmanage.xcall("dontmanage.utils.pdf.is_wkhtmltopdf_valid");
	...
	render_pdf() {
		...
		if (print_format.print_format_builder_beta) {
			...
			let w = window.open(`/api/method/dontmanage.utils.weasyprint.download_pdf?${params}`);
			if (!w) {
			...
			this.render_page(
				"/api/method/dontmanage.utils.print_format.download_pdf?",
				false,
	...
	get_print_html(callback) {
		...
		this._req = dontmanage.call({
			method: "dontmanage.www.printview.get_html_and_style",
			args: {
				doc: this.frm.doc,
	...
	get_raw_commands(callback) {
		...
		dontmanage.call({
			method: "dontmanage.www.printview.get_rendered_raw_commands",
			args: {
				doc: this.frm.doc,
				print_format: this.selected_format(),
				_lang: this.lang_code,
	...
	set_style(style) {
		dontmanage.dom.set_style(style || dontmanage.boot.print_css, "print-style");
	}
	...
	printer_setting_dialog() {
		...
		this.printer_list = [];
		dontmanage.ui.form.qz_get_printer_list().then((data) => {
						...
						fields: [
							{
								...
								fieldname: "print_format",
								default: 0,
								options: dontmanage.meta.get_print_formats(this.frm.doctype),
								read_only: 0,
								in_list_view: 1,
								label: __("Print Format"),
							},
			...
			if (!(this.printer_list && this.printer_list.length)) {
				dontmanage.throw(__("No Printer is Available."));
			}
		});
	}
};

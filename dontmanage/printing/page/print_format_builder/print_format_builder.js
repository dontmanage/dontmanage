dontmanage.pages["print-format-builder"].on_page_load = function (wrapper) {
	dontmanage.print_format_builder = new dontmanage.PrintFormatBuilder(wrapper);
	dontmanage.breadcrumbs.add("Setup", "Print Format");
};

dontmanage.pages["print-format-builder"].on_page_show = function (wrapper) {
	var route = dontmanage.get_route();
	if (route.length > 1) {
		dontmanage.model.with_doc("Print Format", route[1], function () {
			dontmanage.print_format_builder.print_format = dontmanage.get_doc("Print Format", route[1]);
			dontmanage.print_format_builder.refresh();
		});
	} else if (dontmanage.route_options) {
		if (dontmanage.route_options.make_new) {
			let { doctype, name, based_on, beta } = dontmanage.route_options;
			dontmanage.route_options = null;
			dontmanage.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			dontmanage.print_format_builder.print_format = dontmanage.route_options.doc;
			dontmanage.route_options = null;
			dontmanage.print_format_builder.refresh();
		}
	}
...
dontmanage.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = dontmanage.ui.make_app_page({
			parent: this.parent,
		...
		this.page.main = $(
			'<div class="col-md-12 border print-format-builder-main dontmanage-card"></div>'
	...
	show_start() {
		this.page.main.html(dontmanage.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = dontmanage.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function () {
			...
			if (!name) return;
			dontmanage.model.with_doc("Print Format", name, function (doc) {
				dontmanage.set_route("print-format-builder", name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = dontmanage.ui.form.make_control({
		...
		this.name_input = dontmanage.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function () {
			...
			if (!(doctype && name)) {
				dontmanage.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		dontmanage.call({
			method: "dontmanage.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
	...
	setup_print_format() {
		var me = this;
		dontmanage.model.with_doctype(this.print_format.doc_type, function (doctype) {
			me.meta = dontmanage.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function () {
				dontmanage.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(dontmanage.render_template("print_format_builder_sidebar", { fields: fields }))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(
			dontmanage.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function (i, f) {
			...
			if (!f.name && f.fieldname) {
				...
				if (f.fieldname === "_custom_html") {
					...
					f = $.extend(
						dontmanage.meta.get_docfield(me.print_format.doc_type, f.fieldname) || {},
						f
					);
				}
			}
	...
	setup_add_section() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function () {
			...
			var $section = $(
				dontmanage.render_template("print_format_builder_section", {
					section: section,
					me: me,
				})
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			var doc_fields = dontmanage.get_meta(doctype).fields;
			...
			// render checkboxes
			$(
				dontmanage.render_template("print_format_builder_column_selector", {
					fields: fields,
	...
	save_print_format() {
		...
		// save format_data
		dontmanage.call({
			method: "dontmanage.client.set_value",
			args: {
			...
			callback: function (r) {
				...
				dontmanage.show_alert({ message: __("Saved"), indicator: "green" });
			},
		});
	}
};

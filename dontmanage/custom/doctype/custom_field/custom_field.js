...
dontmanage.ui.form.on("Custom Field", {
	setup: function (frm) {
	...
	dt: function (frm) {
		...
		return dontmanage.call({
			method: "dontmanage.custom.doctype.custom_field.custom_field.get_fields_label",
	...
	label: function (frm) {
		if (frm.doc.label && dontmanage.utils.has_special_chars(frm.doc.label)) {
	...
	add_rename_field(frm) {
		frm.add_custom_button(__("Rename Fieldname"), () => {
			dontmanage.prompt(
				...
				function (data) {
					dontmanage.call({
						method: "dontmanage.custom.doctype.custom_field.custom_field.rename_fieldname",
						args: {
...
dontmanage.utils.has_special_chars = function (t) {
	var iChars = "!@#$%^&*()+=-[]\\';,./{}|\":<>?";

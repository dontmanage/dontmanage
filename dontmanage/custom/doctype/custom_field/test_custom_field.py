...
class TestCustomField(IntegrationTestCase):
	def test_create_custom_fields(self):
		...
		self.assertTrue(dontmanage.db.exists("Custom Field", "Address-_test_custom_field_1"))
		self.assertTrue(dontmanage.db.exists("Custom Field", "Address-_test_custom_field_2"))
		self.assertTrue(dontmanage.db.exists("Custom Field", "Contact-_test_custom_field_2"))
	...
	def test_custom_field_sorting(self):
		try:
			...
			fields = dontmanage.get_meta("ToDo", cached=False).fields
	...
	def test_custom_section_and_column_breaks_ordering(self):
		...
		custom_section = dontmanage.get_doc(
			{
		...
		custom_column = dontmanage.get_doc(
			{
				"doctype": "Custom Field",
				"dt": "Test Custom Breaks Ordering",
		...
		updated_meta = dontmanage.get_meta("Test Custom Breaks Ordering", cached=False)

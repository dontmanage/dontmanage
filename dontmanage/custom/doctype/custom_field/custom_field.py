			...
			for doctype in doctypes:
				...
				for df in fields:
					...
					if not field:
						...
						custom_field = dontmanage.get_doc({"doctype": "Custom Field", **field})
...
def get_existing_custom_fields(custom_fields):
	...
	existing_fields = dontmanage.get_all("Custom Field", filters={"dt": ("in", doctypes_to_fetch)}, fields="*")
...
def rename_fieldname(custom_field: str, fieldname: str):
	...
	dontmanage.msgprint(_("Custom field renamed to {0} successfully.").format(fieldname), alert=True)

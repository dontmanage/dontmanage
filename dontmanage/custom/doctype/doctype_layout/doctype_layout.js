...
dontmanage.ui.form.on("DocType Layout", {
	...
	add_buttons(frm) {
		if (!frm.is_new()) {
			frm.add_custom_button(__("Go to {0} List", [frm.doc.name]), () => {
				window.open(`/desk/${dontmanage.router.slug(frm.doc.name)}`);
			});

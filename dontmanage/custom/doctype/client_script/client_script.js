...
dontmanage.ui.form.on("Client Script", {
	setup(frm) {
		const sample_field = frm.get_field("sample");
		sample_field.html(SAMPLE_HTML);
		dontmanage.utils.highlight_pre(sample_field.$wrapper);
	},
	refresh(frm) {
		if (frm.doc.dt && frm.doc.script) {
			frm.add_custom_button(__("Go to {0}", [frm.doc.dt]), () => {
				if (dontmanage.model.is_single(frm.doc.dt)) {
					dontmanage.set_route("Form", frm.doc.dt);
				} else {
					dontmanage.set_route("List", frm.doc.dt);
				}
			});
		}
		...
		if (frm.doc.view == "Form") {
			frm.add_custom_button(__("Add script for Child Table"), () => {
				dontmanage.model.with_doctype(frm.doc.dt, () => {
					const child_tables = dontmanage.meta
			...
			if (!frm.is_new()) {
				frm.add_custom_button(__("Compare Versions"), () => {
					new dontmanage.ui.DiffView("Client Script", "script", frm.doc.name);
				});
			}
		}
	...
	dt(frm) {
		frm.toggle_display("view", !dontmanage.boot.single_types.includes(frm.doc.dt));
	...
	view(frm) {
		let has_form_boilerplate = frm.doc.script.includes("dontmanage.ui.form.on");
...
dontmanage.ui.form.on("Task", "validate", function(frm) {

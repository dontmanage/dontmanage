...
dontmanage.ui.form.on("Customize Form", {
	...
	async trim_table(frm) {
		let dropped_columns = await dontmanage.xcall(
			"dontmanage.custom.doctype.customize_form.customize_form.get_orphaned_columns",
			{ doctype: frm.doc.doc_type }
		);

		if (!dropped_columns?.length) {
			dontmanage.toast(__("This doctype has no orphan fields to trim"));
			return;
		}
		let msg = __(

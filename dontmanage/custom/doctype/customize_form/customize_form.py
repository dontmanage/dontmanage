...
from dontmanage.model import core_doctypes_list, no_value_fields
...
class CustomizeForm(Document):
	...
	def validate_doctype(self, meta):
		...
		if self.doc_type in core_doctypes_list:
			dontmanage.throw(_("Core DocTypes cannot be customized."))

		if meta.issingle:
			dontmanage.throw(_("Single DocTypes cannot be customized."))

		if meta.custom:
			dontmanage.throw(_("Only standard DocTypes are allowed to be customized from Customize Form."))
	...
	def get_name_translation(self):
		...
		return dontmanage.get_value(
			"Translation",
			{"source_text": self.doc_type, "language": dontmanage.local.lang or "en"},
	...
	def set_name_translation(self):
		...
		if not self.label:
			if current:
				# clear translation
				dontmanage.delete_doc("Translation", current.name)
			return
			...
			dontmanage.get_doc(
				{
					...
					"language_code": dontmanage.local.lang or "en",
				}
			).insert()
			return

		if self.label != current.translated_text:
			dontmanage.db.set_value("Translation", current.name, "translated_text", self.label)
	...
	def allow_property_change(self, prop, meta_df, df):
		if prop == "fieldtype":
			...
			(
				dontmanage.db.get_value("DocField", {"parent": self.doc_type, "fieldname": df.fieldname}, "reqd")
				== 1
			)
			and (df.get(prop) == 0)
		):
			...
			dontmanage.msgprint(_("You cannot unset 'Read Only' for field {0}").format(df.label))
			return False

		elif prop == "options" and df.get("fieldtype") not in ALLOWED_OPTIONS_CHANGE:
			dontmanage.msgprint(_("You can't set 'Options' for field {0}").format(df.label))
			return False

		elif prop == "translatable" and not supports_translation(df.get("fieldtype")):
			dontmanage.msgprint(_("You can't set 'Translatable' for field {0}").format(df.label))
			return False
	...
	def set_property_setters_for_actions_and_links(self, meta):
		...
		for doctype, fieldname, field_map in (
			...
			for i, d in enumerate(self.get(fieldname) or []):
				d.idx = i
				if dontmanage.db.exists(doctype, d.name) and not d.custom:
	...
	def update_order_property_setter(self, has_custom, fieldname):
		...
		if has_custom:
			...
			dontmanage.db.delete("Property Setter", dict(property=property_name, doc_type=self.doc_type))
	...
	def clear_removed_items(self, doctype, items):
		...
		"""
		if items:
			dontmanage.db.delete(doctype, dict(parent=self.doc_type, custom=1, name=("not in", items)))
		else:
			dontmanage.db.delete(doctype, dict(parent=self.doc_type, custom=1))
	...
	def add_custom_field(self, df, i):
		d = dontmanage.new_doc("Custom Field")
	...
	def update_in_custom_field(self, df, i):
		...
		custom_field = dontmanage.get_doc("Custom Field", meta_df[0].name)
		changed = False
		for prop in docfield_properties:
	...
	def get_existing_property_value(self, property_name, fieldname=None):
		...
		if fieldname:
			...
			if dontmanage.db.has_column("DocType", property_name):
				property_value = dontmanage.db.get_value("DocType", self.doc_type, property_name)
			else:
				property_value = None
	...
	def validate_fieldtype_change(self, df, old_value, new_value):
		...
		if allowed:
			old_value_length = cint(dontmanage.db.type_map.get(old_value)[1])
			new_value_length = cint(dontmanage.db.type_map.get(new_value)[1])
		...
		for field in self.check_length_for_fieldtypes:
			...
			if docs:
				dontmanage.throw(
					...
					).format(dontmanage.bold(label), links_str, dontmanage.bold(max_length)),
...
def reset_customization(doctype):
	...
	for setter in setters:
		dontmanage.delete_doc("Property Setter", setter)
	...
	for field in custom_fields:
		dontmanage.delete_doc("Custom Field", field)

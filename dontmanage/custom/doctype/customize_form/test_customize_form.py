...
import dontmanage
from dontmanage.core.doctype.doctype.doctype import InvalidFieldNameError
...
class TestCustomizeForm(DontManageTestCase):
		...
		self.field = dontmanage.get_doc(
			{
				...
				"options": "\nCustom 1\nCustom 2\nCustom 3",
				"default": "Custom 3",
				"insert_after": dontmanage.get_meta("Event").fields[-1].fieldname,
			}
		).insert()

	def setUp(self):
		self.insert_custom_field()
		dontmanage.db.delete("Property Setter", dict(doc_type="Event"))
	...
	def get_customize_form(self, doctype=None):
		d = dontmanage.get_doc("Customize Form")
		if doctype:
			d.doc_type = doctype
	...
	def test_save_customization_remove_field(self):
		...
		self.assertEqual(dontmanage.db.get_value("Custom Field", custom_field.name), None)

		dontmanage.local.test_objects["Custom Field"] = []
	...
	def test_reset_to_defaults(self):
		...
		dontmanage.local.test_objects["Property Setter"] = []
	...
	def test_core_doctype_customization(self):
		self.assertRaises(dontmanage.ValidationError, self.get_customize_form, "User")
	...
	def test_custom_link(self):
		try:
			...
			event = dontmanage.get_meta("Event")
	...
	def test_custom_internal_links(self):
		...
		dontmanage.clear_cache()
		user_group = dontmanage.get_meta("User Group")

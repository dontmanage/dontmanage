"""
This module handles the setup and teardown of the test environment for DontManage applications.
...
from dontmanage.tests.utils import make_test_records, toggle_test_mode
...
def _initialize_test_environment(site, config):
	...
	dontmanage.flags.print_messages = logger.getEffectiveLevel() < logging.INFO
	dontmanage.flags.tests_verbose = logger.getEffectiveLevel() < logging.INFO
...
def _decorate_all_methods_and_functions_with_type_checker():
	...
	def _decorate_callable(obj, parent_module):
		# whitelisted methods are already checked, see dontmanage.whitelist
		if getattr(obj, "__func__", obj) in dontmanage.whitelisted:
			return obj
		# Check if the function is already decorated
			...
			config = _get_config_from_pyproject(dontmanage.get_app_source_path(app))
	...
	for app in (apps := dontmanage.get_installed_apps()):
		...
		for _, submodule_name, _ in pkgutil.walk_packages(path=pkg.__path__, prefix=pkg.__name__ + "."):
			...
			submodule = dontmanage.get_module(submodule_name)
...
class IntegrationTestPreparation:
	...
	def _run_before_test_hooks(app: str, category: str):
		...
		for hook_function in dontmanage.get_hooks("before_tests", app_name=app):
			logger.info(f'Running "before_tests" hook function {hook_function}')
			dontmanage.get_attr(hook_function)()
	...
	def _create_global_test_record_dependencies(app: str, category: str):
		...
		try:
			test_module = dontmanage.get_module(f"{app}.tests")

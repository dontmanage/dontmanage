...
import dontmanage
from dontmanage.modules.utils import get_module_name
from dontmanage.testing.config import TestParameters


class DontManageTestLoader(unittest.TestLoader):
	...
	def discover_tests(self, params: TestParameters) -> unittest.TestSuite:
		...
		if self.params.tests:
			...
			files = self.get_files(dontmanage.get_installed_apps())
			...
			module = dontmanage.get_cached_value("DocType", self.params.doctype, "module")
			app = dontmanage.get_cached_value("Module Def", module, "app_name")

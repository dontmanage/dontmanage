"""
DontManage Testing Module

This module provides a comprehensive framework for running tests in DontManage applications.
...
- TestRunner: Main class for running test suites with additional DontManage-specific functionality
- TestResult: Custom test result class for improved output formatting and logging
- discover_all_tests: Function to discover all tests in specified DontManage apps
...
- Integration with DontManage's hooks and test record creation system

Usage:
This module is typically used by DontManage's CLI commands for running tests, but can also
be used programmatically for custom test execution scenarios.

Example:
    from dontmanage.testing import TestConfig, TestRunner, discover_all_tests
...
from dontmanage.utils.logger import create_handler as createDontManageFileHandler
...
LOGGING_CONFIG = {
	...
	"loggers": {
		f"{__name__}": {
			"handlers": [],  # only log to the dontmanage handler
			"propagate": False,
		},
	},
}
...
handlers = createDontManageFileHandler(__name__)

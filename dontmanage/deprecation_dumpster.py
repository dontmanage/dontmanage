...
This file is the final resting place (or should we say, "retirement home"?) for all the deprecated functions and methods of the DontManage framework. It's like a code nursing home, but with more monkey-patching and less bingo.
...
2. The DontManage version at the beginning of which it becomes an error and at the end of which it will be removed (its "graduation" to the great codebase in the sky)
...
# see also dontmanage.__init__ enabling them when a dev_server
class DontManageDeprecationError(Warning):
...
class DontManageDeprecationWarning(Warning):
	"""Deprecated feature in next version"""


class PendingDontManageDeprecationWarning(DontManageDeprecationWarning):
...
warnings.simplefilter("error", DontManageDeprecationError)
warnings.simplefilter("ignore", PendingDontManageDeprecationWarning)


class V15DontManageDeprecationWarning(DontManageDeprecationError):
	pass


class V16DontManageDeprecationWarning(DontManageDeprecationWarning):
	pass


class V17DontManageDeprecationWarning(PendingDontManageDeprecationWarning):
	pass
...
def __get_deprecation_class(graduation: str | None = None, class_name: str | None = None) -> type:
	if graduation:
		...
		class_name = f"{cleaned_graduation}DontManageDeprecationWarning"
		current_module = sys.modules[__name__]
	try:
		return getattr(current_module, class_name)
	except AttributeError:
		return PendingDontManageDeprecationWarning
...
for warning_filter in pythonwarnings.split(","):
	...
	if len(parts) >= 3 and (
		parts[2] in ("DontManageDeprecationError", "DontManageDeprecationWarning", "PendingDontManageDeprecationWarning")
		or parts[2].endswith("DontManageDeprecationWarning")
	):
		try:
...
try:
	...
	def _deprecated(message: str, category=DontManageDeprecationWarning, stacklevel=1) -> Callable[[T], T]:
...
def deprecated(original: str, marked: str, graduation: str, msg: str, stacklevel: int = 1):
	...
	Arguments:
	        - original: dontmanage.utils.make_esc  (fully qualified)
	...
	def decorator(func):
		...
		if os.path.basename(caller_filepath) != "deprecation_dumpster.py":
			raise RuntimeError(
				...
				+ colorize("dontmanage/deprecation_dumpster.py\n", Color.CYAN)
				+ colorize("Move the entire function there and import it back via adding\n ", Color.YELLOW)
				+ colorize(f"from dontmanage.deprecation_dumpster import {func.__name__}\n", Color.CYAN)
				+ colorize("to file\n ", Color.YELLOW)
...
def _old_deprecated(func):
	return deprecated(
		"dontmanage.deprecations.deprecated",
		"2024-09-13",
		"v17",
		"Make use of the dontmanage/deprecation_dumpster.py file, instead. 🎉🗑️",
	)(_deprecated("")(func))


def _old_deprecation_warning(msg):
	@deprecated(
		"dontmanage.deprecations.deprecation_warning",
		"2024-09-13",
		"v17",
		"Use dontmanage.deprecation_dumpster.deprecation_warning, instead. 🎉🗑️",
	)
...
@deprecated("dontmanage.utils.make_esc", "unknown", "v17", "Not used anymore.")
...
@deprecated(
	"dontmanage.db.is_column_missing",
	"unknown",
	"v17",
	"Renamed to dontmanage.db.is_missing_column.",
)
def is_column_missing(e):
	import dontmanage

	return dontmanage.db.is_missing_column(e)


@deprecated(
	"dontmanage.desk.doctype.bulk_update.bulk_update",
	"unknown",
	"v17",
	"Unknown.",
)
...
@deprecated(
	"dontmanage.utils.print_format.read_multi_pdf",
	"unknown",
	"v17",
	"Unknown.",
)
...
@deprecated("dontmanage.gzip_compress", "unknown", "v17", "Use py3 methods directly (this was compat for py2).")
...
@deprecated("dontmanage.gzip_decompress", "unknown", "v17", "Use py3 methods directly (this was compat for py2).")
...
@deprecated(
	"dontmanage.email.doctype.email_queue.email_queue.send_mail",
	"unknown",
	"v17",
	"Unknown.",
)
...
@deprecated(
	"dontmanage.geo.country_info.get_translated_dict",
	"unknown",
	"v17",
	"Use dontmanage.geo.country_info.get_translated_countries, instead.",
)
def get_translated_dict():
	from dontmanage.geo.country_info import get_translated_countries
...
@deprecated("dontmanage.tests_runner.get_modules", "2024-20-08", "v17", "use dontmanage.tests.utils.get_modules")
def test_runner_get_modules(doctype):
	from dontmanage.tests.utils import get_modules

	return get_modules(doctype)


@deprecated(
	"dontmanage.tests_runner.make_test_records", "2024-20-08", "v17", "use dontmanage.tests.utils.make_test_records"
)
def test_runner_make_test_records(*args, **kwargs):
	from dontmanage.tests.utils import make_test_records

	return make_test_records(*args, **kwargs)


@deprecated(
	"dontmanage.tests_runner.make_test_objects", "2024-20-08", "v17", "use dontmanage.tests.utils.make_test_objects"
)
def test_runner_make_test_objects(*args, **kwargs):
	from dontmanage.tests.utils import make_test_objects

	return make_test_objects(*args, **kwargs)


@deprecated(
	"dontmanage.tests_runner.make_test_records_for_doctype",
	"2024-20-08",
	"v17",
	"use dontmanage.tests.utils.make_test_records_for_doctype",
)
...
@deprecated(
	"dontmanage.tests_runner.print_mandatory_fields",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_print_mandatory_fields(*args, **kwargs):
	from dontmanage.tests.utils.generators import print_mandatory_fields

	return print_mandatory_fields(*args, **kwargs)


@deprecated(
	"dontmanage.tests_runner.get_test_record_log",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_get_test_record_log(doctype):
	from dontmanage.tests.utils.generators import TestRecordManager

	return TestRecordManager().get(doctype)


@deprecated(
	"dontmanage.tests_runner.add_to_test_record_log",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
...
@deprecated(
	"dontmanage.tests_runner.main",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
def test_runner_main(*args, **kwargs):
	from dontmanage.commands.testing import main

	return main(*args, **kwargs)


@deprecated(
	"dontmanage.tests_runner.xmlrunner_wrapper",
	"2024-20-08",
	"v17",
	"no public api anymore",
)
...
@deprecated(
	"dontmanage.tests.upate_system_settings",
	"2024-20-08",
	"v17",
...
@deprecated(
	"dontmanage.tests.get_system_setting",
	"2024-20-08",
	"v17",
	"use `dontmanage.db.get_single_value('System Settings', key)`",
)
...
@deprecated(
	"dontmanage.tests.utils.change_settings",
	"2024-20-08",
	"v17",
	"use `dontmanage.tests.change_settings` or the cls.change_settings",
)
def tests_change_settings(*args, **kwargs):
	from dontmanage.tests.classes.context_managers import change_settings

	return change_settings(*args, **kwargs)


@deprecated(
	"dontmanage.tests.utils.patch_hooks",
	"2024-20-08",
	"v17",
	"use `dontmanage.tests.patch_hooks` or the cls.patch_hooks",
)
def tests_patch_hooks(*args, **kwargs):
	from dontmanage.tests.classes.context_managers import patch_hooks

	return patch_hooks(*args, **kwargs)


@deprecated(
	"dontmanage.tests.utils.debug_on",
	"2024-20-08",
	"v17",
	"use `dontmanage.tests.debug_on` or the cls.debug_on",
)
def tests_debug_on(*args, **kwargs):
	from dontmanage.tests.classes.context_managers import debug_on

	return debug_on(*args, **kwargs)


@deprecated(
	"dontmanage.tests.utils.timeout",
	"2024-20-08",
	"v17",
	"use `dontmanage.tests.timeout` or the cls.timeout",
)
def tests_timeout(*args, **kwargs):
	from dontmanage.tests.classes.context_managers import timeout

	return timeout(*args, **kwargs)


def get_tests_CompatDontManageTestCase():
	...
	def _rollback_db():
		dontmanage.db.value_cache.clear()
		dontmanage.db.rollback()

	def _restore_thread_locals(flags):
		dontmanage.local.flags = flags
	...
	class DontManageTestCase(unittest.TestCase):
		"""Base test class for DontManage tests.
		...
		@deprecated(
			"dontmanage.tests.utils.DontManageTestCase",
			"2024-20-08",
			"v17",
			"Import `dontmanage.tests.UnitTestCase` or `dontmanage.tests.IntegrationTestCase` respectively instead of `dontmanage.tests.utils.DontManageTestCase` - also see wiki for more info: https://github.com/dontmanage/dontmanage/wiki#testing-guide",
		)
		...
		def setUpClass(cls) -> None:
			cls.TEST_SITE = getattr(dontmanage.local, "site", None) or cls.TEST_SITE
			dontmanage.init(cls.TEST_SITE)
			cls.ADMIN_PASSWORD = dontmanage.get_conf(cls.TEST_SITE).admin_password
			cls._primary_connection = dontmanage.local.db
		...
		def _apply_debug_decorator(self, exceptions=()):
			from dontmanage.tests.utils import debug_on
		...
		# --- DontManage Framework specific assertions
		...
		def primary_connection(self):
			...
			try:
				current_conn = dontmanage.local.db
				dontmanage.local.db = self._primary_connection
				yield
			finally:
				dontmanage.local.db = current_conn

		@contextmanager
		def secondary_connection(self):
			"""Switch to secondary DB connection."""
			if self._secondary_connection is None:
				dontmanage.connect()  # get second connection
				self._secondary_connection = dontmanage.local.db

			try:
				current_conn = dontmanage.local.db
				dontmanage.local.db = self._secondary_connection
				yield
			finally:
				dontmanage.local.db = current_conn
		...
		def assertQueryCount(self, count):
			...
			try:
				orig_sql = dontmanage.db.__class__.sql
				dontmanage.db.__class__.sql = _sql_with_count
				yield
				self.assertLessEqual(len(queries), count, msg="Queries executed: \n" + "\n\n".join(queries))
			finally:
				dontmanage.db.__class__.sql = orig_sql
		...
		def assertRedisCallCounts(self, count):
			...
			try:
				orig_execute = dontmanage.cache.execute_command
				dontmanage.cache.execute_command = execute_command_and_count
				yield
				self.assertLessEqual(
					len(commands), count, msg="commands executed: \n" + "\n".join(str(c) for c in commands)
				)
			finally:
				dontmanage.cache.execute_command = orig_execute
	...
	return DontManageTestCase


# remove alongside get_tests_CompatDontManageTestCase
def get_compat_dontmanage_test_case_preparation(cfg):
	import unittest

	import dontmanage
	from dontmanage.testing.environment import IntegrationTestPreparation

	class DontManageTestCasePreparation(IntegrationTestPreparation):
		def __call__(self, suite: unittest.TestSuite, app: str, category: str) -> None:
			...
			for path, folders, files in os.walk(dontmanage.get_app_path(app)):
				...
				for filename in files:
					if filename.startswith("test_") and filename.endswith(".py"):
						...
						module = dontmanage.get_module(module_name)
						candidates.append((module, path, filename))
			compat_preload_test_records_upfront(candidates)

	return DontManageTestCasePreparation(cfg)


@deprecated(
	"dontmanage.model.trace.traced_field_context",
	"2024-20-08",
	"v17",
	"use `cls.trace_fields`",
)
def model_trace_traced_field_context(*args, **kwargs):
	from dontmanage.tests.classes.context_managers import trace_fields

	return trace_fields(*args, **kwargs)


@deprecated(
	"dontmanage.tests.utils.get_dependencies",
	"2024-20-09",
	"v17",
	"refactor to use dontmanage.tests.utils.get_missing_records_doctypes",
)
def tests_utils_get_dependencies(doctype):
	"""Get the dependencies for the specified doctype"""
	import dontmanage
	from dontmanage.tests.utils.generators import get_modules
	...
	for df in meta.get_table_fields():
		link_fields.extend(dontmanage.get_meta(df.options).get_link_fields())
...
@deprecated(
	"dontmanage.tests_runner.get_dependencies",
	"2024-20-08",
	"v17",
	"refactor to use dontmanage.tests.utils.get_missing_record_doctypes",
)
...
@deprecated(
	"dontmanage.get_test_records",
	"2024-20-09",
	"v17",
	"""Please access the global test records pool via cls.globalTestRecords['Some Doc'] -> list.
If not an IntegrationTestCase, use dontmanage.tests.utils.load_test_records_for (check return type).
""",
)
def dontmanage_get_test_records(doctype):
	import dontmanage
	from dontmanage.tests.utils.generators import load_test_records_for
...
@deprecated(
	"dontmanage.utils.data.get_number_format_info",
	"unknown",
	"v16",
	"Use `NumberFormat.from_string()` from `dontmanage.utils.number_format` instead",
)
def get_number_format_info(format: str) -> tuple[str, str, int]:
	"""DEPRECATED: use `NumberFormat.from_string()` from `dontmanage.utils.number_format` instead.
	...
	"""
	from dontmanage.utils.number_format import NUMBER_FORMAT_MAP
...
@deprecated(
	...
	"2024-11-12",
	"yet unknown",
	"""It has been added for compatibility in addition to the new .dontmanage sentinel file inside the module. This is for your info: you don't have to do anything.
""",
)
def boilerplate_modules_txt(dest, app_name, app_title):
	...
	with open(os.path.join(dest, app_name, app_name, "modules.txt"), "w") as f:
		f.write(dontmanage.as_unicode(app_title))

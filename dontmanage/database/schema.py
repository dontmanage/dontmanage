...
class DBTable:
	def __init__(self, doctype, meta=None):
		...
		self.meta = meta or dontmanage.get_meta(doctype, False)
	...
	def sync(self):
		...
		if self.is_new():
			...
			dontmanage.client_cache.delete_value(f"table_columns::{self.table_name}")
			self.alter()
	...
	def get_column_definitions(self):
		column_list = [*dontmanage.db.DEFAULT_COLUMNS]
		ret = []
	...
	def get_columns_from_docfields(self):
		...
		if not self.meta.get("istable"):
			for fieldname in dontmanage.db.OPTIONAL_COLUMNS:
	...
	def validate(self):
		...
		columns = [
			dontmanage._dict({"fieldname": f, "fieldtype": "Data"}) for f in dontmanage.db.STANDARD_VARCHAR_COLUMNS
		]
		if self.meta.get("istable"):
			columns += [
				dontmanage._dict({"fieldname": f, "fieldtype": "Data"}) for f in dontmanage.db.CHILD_TABLE_COLUMNS
			]
		...
		for col in columns:
			if len(col.fieldname) >= 64:
				dontmanage.throw(
					_("Fieldname is limited to 64 characters ({0})").format(dontmanage.bold(col.fieldname))
				)

			if "varchar" in dontmanage.db.type_map.get(col.fieldtype, ()):
				# validate length range
				new_length = cint(col.length) or cint(dontmanage.db.VARCHAR_LEN)
				if not (1 <= new_length <= 1000):
					dontmanage.throw(_("Length of {0} should be between 1 and 1000").format(col.fieldname))
				...
				if cint(current_length) != cint(new_length):
					...
					if max_length and max_length[0][0] and max_length[0][0] > new_length:
						...
						dontmanage.msgprint(info_message)

	def is_new(self):
		return self.table_name not in dontmanage.db.get_tables()

	def setup_table_columns(self):
		# TODO: figure out a way to get key data
		for c in dontmanage.db.get_table_columns_description(self.table_name):
...
class DbColumn:
	...
	def get_definition(self, for_modification=False):
		...
		if self.fieldtype in ("Check", "Int"):
			...
			self.default
			and (self.default not in dontmanage.db.DEFAULT_SHORTCUTS)
			and not cstr(self.default).startswith(":")
		):
			default = dontmanage.db.escape(self.default)

		if self.not_nullable and null:
			if default is None:
				default = get_not_null_defaults(self.fieldtype)
				if isinstance(default, str):
					default = dontmanage.db.escape(default)
			null = False
	...
	def build_for_alter_table(self, current_def):
		...
		if current_def["type"] != column_type and not (
			...
			current_def["type"] == "longtext" and column_type == "json" and dontmanage.db.db_type == "mariadb"
		):
	...
	def default_changed(self, current_def):
		if "decimal" in current_def["type"]:
			...
			db_field_type = dontmanage.db.type_map.get(fieldtype)
	...
	if special_characters := SPECIAL_CHAR_PATTERN.findall(n):
		...
		dontmanage.throw(
			_("Fieldname {0} cannot have special characters like {1}").format(
				dontmanage.bold(cstr(n)), special_characters
			),
			dontmanage.db.InvalidColumnName,
		)
	return n


def validate_column_length(fieldname):
	if len(fieldname) > dontmanage.db.MAX_COLUMN_LENGTH:
		dontmanage.throw(_("Fieldname is limited to 64 characters ({0})").format(fieldname))


def get_definition(fieldtype, precision=None, length=None, *, options=None):
	d = dontmanage.db.type_map.get(fieldtype)
	...
	if (
		...
		and dontmanage.db.exists("DocType", options)
		and dontmanage.get_meta(options).autoname == "UUID"
	):
		d = ("uuid", None)
	...
	if fieldtype == "Int" and length and length > 11:
		...
		d = dontmanage.db.type_map.get("Long Int")
	...
	if size:
		...
		if length:
			if coltype == "varchar":
				...
				# NOTE: this will only be applicable for mariadb as dontmanage implements int
...
def add_column(doctype, column_name, fieldtype, precision=None, length=None, default=None, not_null=False):
	...
	if dontmanage.db.db_type == "sqlite":
		if column_name in dontmanage.db.get_table_columns(doctype):
			return

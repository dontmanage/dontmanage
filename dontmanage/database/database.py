...
class Database:
	...
	def sql(
		...
		*,
		...
		if isinstance(query, QueryBuilder):
			dontmanage.log("Use run method to execute SQL queries generated by Query Builder")
		...
		try:
			...
			if self.is_syntax_error(e):
				dontmanage.log(f"Syntax error in query:\n{query} {values or ''}")
			...
			# TODO: added temporarily
				...
				dontmanage.log(f"Error in query:\n{e}")
				raise
		...
		if debug:
			time_end = time()
			dontmanage.log(f"Execution time: {(time_end - time_start) * 1000:.3f} ms")
	...
	def explain_query(self, query, values=EmptyQueryValues):
		...
		dontmanage.log("--- query explain ---")
		try:
			results = self.sql(f"EXPLAIN {query}", values, as_dict=1)
		except Exception as e:
			dontmanage.log(f"error in query explain: {e}")
		else:
			dontmanage.log(json.dumps(results, indent=1))
			dontmanage.log("--- query explain end ---")
	...
	def clear_db_table_cache(query_type: str):
		if query_type in CREATE_OR_DROP:
			dontmanage.client_cache.delete_value("db_tables")
	...
	def get_single_value(
		...
		*,
		...
		df = dontmanage.get_meta(doctype).get_field(fieldname) or get_default_df(fieldname)
	...
	def set_value(
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		if isinstance(dn, FilterValue):
			dontmanage.clear_document_cache(dt, convert_to_value(dn))
		else:
	...
	def _build_and_run_bulk_update_query(
		...
		update_query = dontmanage.qb.update(dt)
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		columns = dontmanage.client_cache.get_value(key)
		if columns is None:
			...
			if columns:
				dontmanage.cache.set_value(key, columns)
	...
	from dontmanage.deprecation_dumpster import is_column_missing as _is_column_missing
	...
	def log_touched_tables(self, query, query_type):
		...
		touched_tables = dontmanage.local.flags.touched_tables

		if touched_tables is None:
			touched_tables = set()
			dontmanage.local.flags.touched_tables = touched_tables
	...
	def bulk_insert(
		...
		*,
		...
		if ignore_duplicates:
			...
			if dontmanage.conf.db_type in ("mariadb", "sqlite"):
				query = query.ignore()
	...
	def get_routines(self):
		information_schema = dontmanage.qb.Schema("information_schema")
		return (
			dontmanage.qb.from_(information_schema.routines)
			.select(information_schema.routines.routine_name)
			.where(
				(information_schema.routines.routine_type.isin(["FUNCTION", "PROCEDURE"]))
				& (information_schema.routines.routine_schema.eq(dontmanage.conf.db_name))
			)
...
def get_print_sql_flag() -> bool:
	flag_value = dontmanage.client_cache.get_value("flag_print_sql")
	if flag_value is None:
		flag_value = False
		dontmanage.client_cache.set_value("flag_print_sql", flag_value)

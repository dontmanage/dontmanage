...
from dontmanage.model.db_query import get_timespan_date_range
from dontmanage.query_builder import Criterion, Field, Order, Table, functions
from dontmanage.query_builder.functions import Function, SqlFunctions
...
def like(key: Field, value: str) -> dontmanage.qb:
	...
	Returns:
	        dontmanage.qb: `dontmanage.qb object with `LIKE`
	"""
	return key.like(value)


def func_in(key: Field, value: list | tuple) -> dontmanage.qb:
	...
	Returns:
	        dontmanage.qb: `dontmanage.qb object with `IN`
	"""
	return key.isin(value)


def not_like(key: Field, value: str) -> dontmanage.qb:
	...
	Returns:
	        dontmanage.qb: `dontmanage.qb object with `NOT LIKE`
	"""
...
def func_not_in(key: Field, value: list | tuple):
	...
	Returns:
	        dontmanage.qb: `dontmanage.qb object with `NOT IN`
	"""
	return key.notin(value)


def func_regex(key: Field, value: str) -> dontmanage.qb:
	...
	Returns:
	        dontmanage.qb: `dontmanage.qb object with `REGEX`
	"""
	return key.regex(value)


def func_between(key: Field, value: list | tuple) -> dontmanage.qb:
	...
	Returns:
	        dontmanage.qb: `dontmanage.qb object with `BETWEEN`
	"""
...
def func_timespan(key: Field, value: str) -> dontmanage.qb:
	...
	Returns:
	        dontmanage.qb: `dontmanage.qb object with `TIMESPAN`
	"""
...
def make_function(key: Any, value: int | str):
	...
	Returns:
	        dontmanage.qb: dontmanage.qb object
	"""
...
class Engine:
	...
	def OPERATOR_MAP(self):
		...
		if dontmanage.get_hooks("filters_config"):
			from dontmanage.utils.commands import warn

			warn(
				"The 'filters_config' hook used to add custom operators is not yet implemented"
				" in dontmanage.db.query engine. Use db_query (dontmanage.get_list) instead."
			)

		return all_operators

	def get_condition(self, table: str | Table, **kwargs) -> dontmanage.qb:
		...
		Returns:
		        dontmanage.qb: DocType with initial condition
		"""
		table_object = self.get_table(table)
		if kwargs.get("update"):
			return dontmanage.qb.update(table_object)
		if kwargs.get("into"):
			return dontmanage.qb.into(table_object)
		return dontmanage.qb.from_(table_object)
	...
	def get_table(self, table_name: str | Table) -> Table:
		...
		if table_name not in self.tables:
			self.tables[table_name] = dontmanage.qb.DocType(table_name)
		return self.tables[table_name]

	def criterion_query(self, table: str, criterion: Criterion, **kwargs) -> dontmanage.qb:
		...
		Returns:
		        dontmanage.qb: condition object
		"""
		condition = self.add_conditions(self.get_condition(table, **kwargs), **kwargs)
		return condition.where(criterion)

	def add_conditions(self, conditions: dontmanage.qb, **kwargs):
		"""Adding additional conditions

		Args:
		        conditions (dontmanage.qb): built conditions

		Returns:
		        conditions (dontmanage.qb): dontmanage.qb object
		"""
	...
	def dict_query(self, table: str, filters: dict[str, str | int] = None, **kwargs) -> dontmanage.qb:
		...
		Returns:
		        dontmanage.qb: conditions object
		"""
		conditions = self.get_condition(table, **kwargs)
		if not filters:
	...
	def build_conditions(
		...
		Returns:
		        dontmanage.qb: dontmanage.qb conditions object
		"""
...
class Permission:
	...
	def check_permissions(cls, query, **kwargs):
		...
		for dt in doctype:
			...
			) and not dontmanage.has_permission(
				dt,
				"read",
				user=kwargs.get("user"),
				parent_doctype=kwargs.get("parent_doctype"),
			):
				dontmanage.throw(_("Insufficient Permission for {0}").format(dontmanage.bold(dt)))

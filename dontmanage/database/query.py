...
from dontmanage.database.operator_map import OPERATOR_MAP
from dontmanage.database.schema import SPECIAL_CHAR_PATTERN
from dontmanage.database.utils import DefaultOrderBy, get_doctype_name
...
from dontmanage.query_builder.utils import PseudoColumnMapper
from dontmanage.utils.data import MARIADB_SPECIFIC_COMMENT
...
class Engine:
	def get_query(
		...
		*,
		...
		self.is_mariadb = dontmanage.db.db_type == "mariadb"
		self.is_postgres = dontmanage.db.db_type == "postgres"
		...
		if isinstance(table, Table):
			...
			self.table = dontmanage.qb.DocType(table)

		if update:
			self.query = dontmanage.qb.update(self.table)
		elif into:
			self.query = dontmanage.qb.into(self.table)
		elif delete:
			self.query = dontmanage.qb.from_(self.table).delete()
		else:
			self.query = dontmanage.qb.from_(self.table)
	...
	def _apply_filter(
		...
		if not isinstance(_field, str):
			...
			dontmanage.throw(_("Invalid filter: {0}").format(_field))
		elif not doctype or doctype == self.doctype:
			_field = self.table[field]
		elif doctype:
			_field = dontmanage.qb.DocType(doctype)[field]
		...
		if _operator in OPERATOR_MAP["nested_set"]:
			...
			_df = dontmanage.get_meta(self.doctype).get_field(field)
	...
	def parse(field: str, doctype: str):
		if "." in field:
			...
			if field.startswith("`tab") or field.startswith('"tab'):
				...
				linked_field = dontmanage.get_meta(doctype).get_field(linked_fieldname)

...
from dontmanage.boot import get_additional_filters_from_hooks
from dontmanage.database.operator_map import NESTED_SET_OPERATORS, OPERATOR_MAP
...
from dontmanage.model import OPTIONAL_FIELDS, get_permitted_fields
from dontmanage.model.base_document import DOCTYPES_FOR_DOCTYPE
from dontmanage.model.document import Document
from dontmanage.query_builder import Criterion, Field, Order, functions
from dontmanage.query_builder.custom import Month, MonthName, Quarter
...
def _apply_datetime_field_filter_conversion(between_values: tuple | list, doctype: str, field) -> tuple:
	...
	"""
	from dontmanage.model.db_query import _convert_type_for_between_filters
...
# less restrictive version of dontmanage.core.doctype.doctype.doctype.START_WITH_LETTERS_PATTERN
...
class Engine:
	def get_query(
		...
		*,
		...
		qb = dontmanage.local.qb
		db_type = dontmanage.local.db.db_type

		self.is_mariadb = db_type == "mariadb"
		self.is_postgres = db_type == "postgres"
		self.is_sqlite = db_type == "sqlite"
		self.user = user or dontmanage.session.user
		...
		if limit:
			if not isinstance(limit, int) or limit < 0:
				dontmanage.throw(_("Limit must be a non-negative integer"), TypeError)
			self.query = self.query.limit(limit)

		if offset:
			if not isinstance(offset, int) or offset < 0:
				dontmanage.throw(_("Offset must be a non-negative integer"), TypeError)
	...
	def validate_doctype(self):
		if not TABLE_NAME_PATTERN.match(self.doctype):
			dontmanage.throw(_("Invalid DocType: {0}").format(self.doctype))
		...
		if isinstance(filters, list | tuple):
			...
			if is_nested_structure:
				...
				try:
					...
					dontmanage.throw(_("Error parsing nested filters: {0}. {1}").format(filters, e), exc=e)
	...
	def _build_criterion_for_simple_filter(
		...
		if operator.lower() in additional_filters_config:
			f = dontmanage._dict(doctype=doctype or self.doctype, fieldname=field, operator=operator, value=value)
			from dontmanage.model.db_query import get_additional_filter_field
		...
		if isinstance(value, Document):
			dontmanage.throw(_("Document cannot be used as a filter value"))
		_operator = operator

		if _operator.lower() in ("timespan", "previous", "next"):
			from dontmanage.model.db_query import get_date_range
		...
		if _operator.lower() == "between":
			if isinstance(_value, list | tuple) and len(_value) == 2:
				...
				from dontmanage.model.db_query import get_between_date_filter

				target_meta = dontmanage.get_meta(doctype or self.doctype)
		...
		if _operator in NESTED_SET_OPERATORS:
			...
			main_meta = dontmanage.get_meta(self.doctype)
	...
	def _parse_nested_filters(self, nested_list: list | tuple) -> "Criterion | None":
		...
		if not isinstance(nested_list, list | tuple):
			dontmanage.throw(_("Nested filters must be provided as a list or tuple."))
		...
		while idx < len(nested_list):
			...
			if idx >= len(nested_list):
				dontmanage.throw(_("Filter condition missing after operator: {0}").format(operator_str))
	...
	def _condition_to_criterion(self, condition: list | tuple) -> "Criterion":
		...
		if not isinstance(condition, list | tuple):
			dontmanage.throw(_("Invalid condition type in nested filters: {0}").format(type(condition)))
		...
		if is_nested:
			...
			if (
				...
				dontmanage.throw(_("Invalid simple filter format: {0}").format(condition))
	...
	def _validate_and_prepare_filter_field(self, field: str | Field, doctype: str | None = None) -> Field:
		...
		if "`" in field:
			if parsed := self._parse_backtick_field_notation(field):
				...
				return dontmanage.qb.DocType(table_name)[field_name]
		...
		if "." in field:
			...
			if doctype and doctype != self.doctype:
				...
				parent_meta = dontmanage.get_meta(self.doctype)
				...
				if (
					...
					for df in meta.get_table_fields(include_computed=True):
						try:
							child_meta = dontmanage.get_meta(df.options)
						except dontmanage.DoesNotExistError:
							continue
				...
				return dontmanage.qb.DocType(target_doctype)[target_fieldname]
		...
		if parent_doctype and permission_type == "select":
			dontmanage.throw(
				_("You do not have permission to access child table field: {0}").format(
					dontmanage.bold(f"{doctype}.{fieldname}")
				),
	...
	def parse_string_field(self, field: str):
		...
		if not match:
			dontmanage.throw(_("Could not parse field: {0}").format(field))
		...
		if table_name:
			...
			if not TABLE_NAME_PATTERN.match(table_name.lstrip("tab")):
				dontmanage.throw(_("Invalid characters in table name: {0}").format(table_name))

			doctype_name = table_name[3:] if table_name.startswith("tab") else table_name
			table_obj = dontmanage.qb.DocType(doctype_name)
	...
	def parse_fields(
		...
		if isinstance(fields, str):
			...
			dontmanage.throw(_("Fields must be a string, list, tuple, pypika Field, or pypika Function"))
	...
	def _parse_single_field_item(
		...
		if not isinstance(field, str):
			dontmanage.throw(_("Invalid field type: {0}").format(type(field)))
	...
	def _validate_group_by(self, group_by: str) -> list[Field]:
		...
		if not isinstance(group_by, str):
			dontmanage.throw(_("Group By must be a string"), TypeError)
	...
	def _validate_order_by(self, order_by: str) -> list[tuple[Field | str, Order]]:
		...
		if not isinstance(order_by, str):
			dontmanage.throw(_("Order By must be a string"), TypeError)
	...
	def check_read_permission(self):
		...
		def has_permission(ptype):
			return dontmanage.has_permission(
				self.doctype,
				ptype,
				user=self.user,
	...
	def _raise_permission_error(self, doctype=None):
		dontmanage.throw(
			_("Insufficient Permission for {0}").format(dontmanage.bold(doctype or self.doctype)),
			dontmanage.PermissionError,
		)
		...
		for field in self.fields:
			if isinstance(field, ChildTableField):
				...
				if field.link_fieldname in permitted_fields_set:
					...
					has_target_perm = dontmanage.has_permission(
						target_doctype, "select", user=self.user
					) or dontmanage.has_permission(target_doctype, "read", user=self.user)
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				...
				if docs:
					...
					strict_user_permissions = dontmanage.get_system_settings("apply_strict_user_permissions")
	...
	def add_permission_conditions(self):
		...
		if self.permission_doctype != self.doctype:
			parent_meta = dontmanage.get_meta(self.permission_doctype)
	...
	def get_permission_conditions(self, doctype: str, table: Table) -> Criterion | None:
		role_permissions = dontmanage.permissions.get_role_permissions(doctype, user=self.user)
		...
		if not has_role_permission:
			...
			shared_docs = dontmanage.share.get_shared(doctype, self.user)
			if not shared_docs:
	...
	def get_permission_query_conditions(self, doctype: str | None = None) -> list["RawCriterion"]:
		...
		doctype = doctype or self.permission_doctype
		conditions = []
		hooks = dontmanage.get_hooks("permission_query_conditions", {})
		condition_methods = hooks.get(doctype, []) + hooks.get("*", [])

		for method in condition_methods:
			if c := dontmanage.call(dontmanage.get_attr(method), self.user, doctype=doctype):
		...
		if permission_script_name := get_server_script_map().get("permission_query", {}).get(doctype):
			script = dontmanage.get_doc("Server Script", permission_script_name)
	...
	def get_permission_type(
		...
		if dontmanage.only_has_select_perm(doctype, user=self.user):
			return "select"
		return "read"
	...
	def _is_field_nullable(self, doctype: str, fieldname: str) -> bool:
		...
		try:
			...
			if (meta := dontmanage.client_cache.get_value(f"doctype_meta::{doctype}")) is None:
				return True
	...
	def _get_ifnull_fallback(self, doctype: str, fieldname: str) -> str:
		...
		try:
			...
			if dontmanage.db.db_type == "postgres":
				...
				target_table = dontmanage.utils.get_table_name(doctype)
				info_schema = dontmanage.qb.Schema("information_schema")
				columns = info_schema.columns
				current_schema = dontmanage.conf.get("db_schema", "public")
				res = (
					dontmanage.qb.from_(columns)
		...
		if df is None:
			...
			from dontmanage.model.meta import get_default_df
		...
		try:
			db_type_info = dontmanage.db.type_map.get(fieldtype, ("varchar",))
			if db_type_info:
	...
	def parse(field: str, doctype: str, allow_tab_notation: bool = True):
		if "." in field:
			...
			if child_match:
				...
				try:
					meta = dontmanage.get_meta(doctype)  # Get meta of the *parent* doctype
				...
				if linked_field:
					...
					elif linked_field.fieldtype in dontmanage.model.table_fields:
...
class ChildQuery:
	def __init__(
		...
		field = dontmanage.get_meta(parent_doctype).get_field(fieldname)
		if field.fieldtype not in dontmanage.model.table_fields:
			return
		self.fieldname = fieldname
		self.fields = fields
		self.parent_doctype = parent_doctype
...
def get_nested_set_hierarchy_result(doctype: str, name: str, hierarchy: str) -> list[str]:
	...
	try:
		lft, rgt = dontmanage.qb.from_(table).select("lft", "rgt").where(table.name == name).run()[0]
	except IndexError:
		lft, rgt = None, None
...
class RawCriterion(Term):
	...
	Allows using raw SQL strings in pypika queries:
		dontmanage.qb.from_("DocType").where(RawCriterion("name like 'a%'"))
	"""
...
class SQLFunctionParser:
	...
	def _extract_dict_components(self, d: dict, valid_keys: dict, error_msg: str) -> tuple:
		...
		if not name:
			dontmanage.throw(_("Invalid {0} dictionary format").format(error_msg), dontmanage.ValidationError)

		if name not in valid_keys:
			dontmanage.throw(_("Unsupported {0}: {1}").format(error_msg, name), dontmanage.ValidationError)
	...
	def _validate_string_argument(self, arg: str, *, function_name: str | None = None):
		...
		if not arg:
			dontmanage.throw(_("Empty string arguments are not allowed"), dontmanage.ValidationError)
	...
	def _validate_alias(self, alias: str):
		...
		if not isinstance(alias, str):
			dontmanage.throw(_("Alias must be a string"), dontmanage.ValidationError)

		alias = alias.strip()
		if not alias:
			dontmanage.throw(_("Empty alias is not allowed"), dontmanage.ValidationError)

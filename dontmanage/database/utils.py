...
class LazyMogrify(LazyString):
	...
	def _setup(self) -> str:
		return dontmanage.db.mogrify(self.query, self.values)
...
def dangerously_reconnect_on_connection_abort(func):
	...
	def wrapper(*args, **kwargs):
		try:
			...
			if dontmanage.db.is_interface_error(e) or isinstance(e, dontmanage.db.OperationalError):
	...
	def run(self):
		...
		while self._functions:
			...
			try:
				...
				dontmanage.log_error(title="Error executing commit_after_response callback")
...
def commit_after_response(func):
	...
	request = getattr(dontmanage.local, "request", False)
	if not request or dontmanage.in_test:
		func()
		return
...
def drop_index_if_exists(table: str, index: str):
	...
	try:
		if dontmanage.db.db_type == "postgres":
			...
			dontmanage.db.sql_ddl(f'DROP INDEX IF EXISTS "{safe_index}"')
		else:

...
import dontmanage
from dontmanage.database.database import (
	TRANSACTION_DISABLED_MSG,
	Database,
	ImplicitCommitError,
)
from dontmanage.database.sqlite.schema import SQLiteTable
...
class SQLiteDatabase(SQLiteExceptionUtil, Database):
	...
	def get_db_path(self):
		return Path(dontmanage.get_site_path()) / "db" / f"{self.cur_db_name}.db"
	...
	def change_column_type(
		...
		if not column_exists:
			raise dontmanage.InvalidColumnName(f"Column {column} does not exist in table {table_name}")
	...
	def rename_column(self, doctype: str, old_column_name: str, new_column_name: str):
		...
		if not column_exists:
			raise dontmanage.InvalidColumnName(f"Column {old_column_name} does not exist in table {table_name}")
	...
	def add_index(self, doctype: str, fields: list, index_name: str | None = None):
		...
		from dontmanage.custom.doctype.property_setter.property_setter import (
	...
	def begin(self, *, read_only=False):
		if read_only or dontmanage.flags.read_only:
			if self._conn:
				self._conn.close()

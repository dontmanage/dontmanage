...
from dontmanage.database.schema import DBTable
...
class SQLiteTable(DBTable):
	def create(self):
		...
		dontmanage.db.sql_ddl(create_table_query)

		# Create indexes separately
		for index_query in index_defs:
			dontmanage.db.sql_ddl(index_query)
	...
	def alter(self):
		...
		existing_columns = []
		for column in dontmanage.db.sql(f"PRAGMA table_info(`{self.table_name}`)", as_dict=1):
		...
		dontmanage.db.sql_ddl(create_table)

		# Copy data
		existing_columns = [col.split()[0] for col in existing_columns]
		column_list = ", ".join(existing_columns)
		dontmanage.db.sql_ddl(f"INSERT INTO `{temp_table}` SELECT {column_list} FROM `{self.table_name}`")

		# Drop old table
		dontmanage.db.sql_ddl(f"DROP TABLE `{self.table_name}`")

		# Rename new table
		dontmanage.db.sql_ddl(f"ALTER TABLE `{temp_table}` RENAME TO `{self.table_name}`")
		...
		if self.add_index:
			index_queries.extend(
				...
				if not dontmanage.db.get_column_index(self.table_name, col.fieldname, unique=False)
			)
		if self.meta.sort_field == "modified" and not dontmanage.db.get_column_index(
		...
		for query in index_queries:
			dontmanage.db.sql_ddl(query)

	def alter_primary_key(self) -> str | None:
		# If there are no values in table allow migrating to UUID from TEXT
		autoname = self.meta.autoname
		if autoname == "UUID" and dontmanage.db.get_column_type(self.doctype, "name") != "TEXT":
			if not dontmanage.db.get_value(self.doctype, {}, order_by=None):

...
from dontmanage.database.mariadb.schema import MariaDBTable
from dontmanage.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if dontmanage.conf.local_infile:
			conn_settings["local_infile"] = dontmanage.conf.local_infile

		# Configure SSL settings
		if dontmanage.conf.db_ssl_ca:
			ssl_config = {
				"ca": dontmanage.conf.db_ssl_ca,
				"check_hostname": dontmanage.conf.db_ssl_check_hostname,
			}

			# Add client certificates for mutual SSL if available
			if dontmanage.conf.db_ssl_cert and dontmanage.conf.db_ssl_key:
				ssl_config.update({"cert": dontmanage.conf.db_ssl_cert, "key": dontmanage.conf.db_ssl_key})
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		# Also, given we're promoting use of the PyPika builder via dontmanage.qb, the use
		# of this method should be limited.

		# pymysql expects unicode argument to escape_string with Python 3
		s = dontmanage.as_unicode(escape_string(dontmanage.as_unicode(s)), "utf-8").replace("`", "\\`")
	...
	def get_table_columns_description(self, table_name):
		...
		return self.sql(
			...
			where table_name = '{table_name}'
   			and table_schema = '{dontmanage.db.cur_db_name}' """,
			as_dict=1,
		)
	...
	def add_index(self, doctype: str, fields: list, index_name: str | None = None):
		...
		if not self.has_index(table_name, index_name):
			...
			# via code or console.
			if len(fields) == 1 and not (dontmanage.flags.in_install or dontmanage.flags.in_migrate):
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = dontmanage.client_cache.get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			tables = (
				...
				.where(information_schema.tables.table_schema == dontmanage.db.cur_db_name)
				.run(pluck=True)
			)
			dontmanage.client_cache.set_value("db_tables", tables)
	...
	def estimate_count(self, doctype: str):
		...
		from dontmanage.utils.data import cint
		...
		count = self.sql(
			...
			(table, dontmanage.db.cur_db_name),
		)

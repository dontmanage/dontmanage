...
class MariaDBTable(DBTable):
	...
	def alter(self):
		...
		for col in {*self.drop_index, *self.drop_unique}:
			...
			if unique_constraint_changed and not col.unique:
				if unique_index := dontmanage.db.get_column_index(self.table_name, col.fieldname, unique=True):
			...
			if index_constraint_changed and not col.set_index:
				if index_record := dontmanage.db.get_column_index(self.table_name, col.fieldname, unique=False):
		...
		for col in self.change_nullability:
			if col.not_nullable:
				try:
					...
					dontmanage.qb.update(table).set(
	...
	def alter_primary_key(self) -> str | None:
		...
		if autoname == "UUID" and dontmanage.db.get_column_type(self.doctype, "name") != "uuid":
		...
		if autoname != "UUID" and dontmanage.db.get_column_type(self.doctype, "name") == "uuid":
			return f"modify name varchar({dontmanage.db.VARCHAR_LEN})"

...
from dontmanage.model import child_table_fields, default_fields
...
class DocumentTypeMapping(Document):
	...
	def validate_inner_mapping(self):
		meta = dontmanage.get_meta(self.local_doctype)
		for field_map in self.field_mapping:
			if field_map.local_fieldname not in (default_fields + child_table_fields):
				...
				if not field:
					dontmanage.throw(_("Row #{0}: Invalid Local Fieldname").format(field_map.idx))
			...
			if fieldtype in ["Link", "Dynamic Link", "Table"]:
				if not field_map.mapping and not field_map.default_value:
					...
					).format(field_map.idx, dontmanage.bold(field_map.local_fieldname))
					dontmanage.throw(msg, title="Inner Mapping Missing")
			...
			if field_map.mapping_type == "Document" and not field_map.remote_value_filters:
				...
				).format(field_map.idx, dontmanage.bold(field_map.remote_fieldname))
				dontmanage.throw(msg, title="Remote Value Filters Missing")
	...
	def get_mapping(self, doc, producer_site, update_type):
		...
		mapping = {"doc": dontmanage.as_json(doc)}
		if len(dependencies):
			mapping["dependencies"] = dependencies
		return mapping

	def get_mapped_update(self, update, producer_site):
		update_diff = dontmanage._dict(json.loads(update.data))
		mapping = update_diff
		dependencies = []
		...
		update = {"doc": dontmanage.as_json(mapping)}
		if len(dependencies):
			update["dependencies"] = dependencies
		return update

	def get_mapped_dependency(self, mapping, producer_site, doc):
		inner_mapping = dontmanage.get_doc("Document Type Mapping", mapping.mapping)
		filters = json.loads(mapping.remote_value_filters)
		for key, value in filters.items():
			if value.startswith("eval:"):
				val = dontmanage.safe_eval(value[5:], None, dict(doc=doc))
				filters[key] = val
			if doc.get(value):
	...
	def map_rows_removed(self, update_diff, mapping):
		...
		for key, value in update_diff.removed.copy().items():
			local_table_name = dontmanage.db.get_value(
	...
	def map_rows(self, update_diff, mapping, producer_site, operation):
		...
		for tablename, entries in update_diff.get(operation).copy().items():
			...
			table_map = dontmanage.db.get_value(
				"Document Type Field Mapping",
				{"local_fieldname": local_table_name, "parent": self.name},
				"mapping",
			)
			table_map = dontmanage.get_doc("Document Type Mapping", table_map)
			docs = []
...
def get_mapped_child_table_docs(child_map, table_entries, producer_site):
	...
	child_map = dontmanage.get_doc("Document Type Mapping", child_map)
	mapped_entries = []
	remote_fields = []

...
import dontmanage
from dontmanage.model import no_value_fields, table_fields
...
class EventUpdateLog(Document):
	def after_insert(self):
		...
		enqueued_method = (
			"dontmanage.event_streaming.doctype.event_consumer.event_consumer.notify_event_consumers"
		)
		jobs = get_jobs()
...
def notify_consumers(doc, event):
	...
	if dontmanage.flags.in_install or dontmanage.flags.in_migrate:
		return
...
def check_doctype_has_consumers(doctype):
	...
	return dontmanage.cache_manager.get_doctype_map(
...
def get_update(old, new, for_child=False):
	...
	out = dontmanage._dict(changed={}, added={}, removed={}, row_changed={})
...
def make_event_update_log(doc, update_type):
	...
	if update_type != "Delete":
		...
		data = dontmanage.as_json(doc) if not doc.get("diff") else dontmanage.as_json(doc.diff)
	else:
		data = None
...
def is_consumer_uptodate(update_log, consumer):
	...
	prev_log_consumers = dontmanage.get_all(
...
def mark_consumer_read(update_log_name, consumer_name):
	...
	"""
	update_log = dontmanage.get_doc("Event Update Log", update_log_name)
...
def get_update_logs_for_consumer(event_consumer, doctypes, last_update):
	...
	if isinstance(doctypes, str):
		doctypes = dontmanage.parse_json(doctypes)

	from dontmanage.event_streaming.doctype.event_consumer.event_consumer import has_consumer_access

	consumer = dontmanage.get_doc("Event Consumer", event_consumer)
	docs = dontmanage.get_list(
		doctype="Event Update Log",

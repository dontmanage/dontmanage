...
from dontmanage.utils.data import get_link_to_form, get_url
...
class EventProducer(Document):
	...
	def validate_event_subscriber(self):
		if not dontmanage.db.get_value("User", self.user, "api_key"):
			dontmanage.throw(
				_("Please generate keys for the Event Subscriber User {0} first.").format(
					dontmanage.bold(get_link_to_form("User", self.user))
				)
			)

	def on_update(self):
		if not self.incoming_change:
			if dontmanage.db.exists("Event Producer", self.name):
				if not self.api_key or not self.api_secret:
					dontmanage.throw(_("Please set API Key and Secret on the producer and consumer sites first."))
				else:
	...
	def on_trash(self):
		last_update = dontmanage.db.get_value("Event Producer Last Update", dict(event_producer=self.name))
		if last_update:
			dontmanage.delete_doc("Event Producer Last Update", last_update)

	def check_url(self):
		valid_url_schemes = ("http", "https")
		dontmanage.utils.validate_url(self.producer_url, throw=True, valid_schemes=valid_url_schemes)
	...
	def create_event_consumer(self):
		...
		if self.is_producer_online():
			...
			response = producer_site.post_api(
				"dontmanage.event_streaming.doctype.event_consumer.event_consumer.register_consumer",
	...
	def set_last_update(self, last_update):
		last_update_doc_name = dontmanage.db.get_value(
	...
	def get_request_data(self):
		...
		for entry in self.producer_doctypes:
			if entry.has_mapping:
				...
				dt = dontmanage.db.get_value("Document Type Mapping", entry.mapping, "remote_doctype")
			else:
				dt = entry.ref_doctype
			consumer_doctypes.append({"doctype": dt, "condition": entry.condition})

		user_key = dontmanage.db.get_value("User", self.user, "api_key")
	...
	def update_event_consumer(self):
		if self.is_producer_online():
			...
			event_consumer = dontmanage._dict(event_consumer)
			if event_consumer:
				...
				for entry in self.producer_doctypes:
					if entry.has_mapping:
						# if mapping, subscribe to remote doctype on consumer's site
						ref_doctype = dontmanage.db.get_value("Document Type Mapping", entry.mapping, "remote_doctype")
					else:
	...
	def is_producer_online(self):
		...
		while retry > 0:
			...
			retry -= 1
			time.sleep(5)
		dontmanage.throw(_("Failed to connect to the Event Producer site. Retry after some time."))


def get_producer_site(producer_url):
	"""create a DontManageClient object for event producer site"""
...
def pull_producer_data():
	...
	if response.status_code == 200:
		for event_producer in dontmanage.get_all("Event Producer"):
...
def pull_from_node(event_producer):
	...
	event_producer = dontmanage.get_doc("Event Producer", event_producer)
...
def get_config(event_config):
	...
	for entry in event_config:
		if entry.status == "Approved":
			if entry.has_mapping:
				(mapped_doctype, mapping) = dontmanage.db.get_value(
...
def sync(update, producer_site, event_producer, in_retry=False):
	...
	try:
		...
		if in_retry:
			...
			return "Failed"
		log_event_sync(update, event_producer.name, "Failed", dontmanage.get_traceback())
...
def set_insert(update, producer_site, event_producer):
	...
	if dontmanage.db.get_value(update.ref_doctype, update.docname):
		# doc already created
		return
	doc = dontmanage.get_doc(update.data)
...
def set_update(update, producer_site):
	...
	if local_doc:
		data = dontmanage._dict(update.data)
...
def update_row_added(local_doc, added):
	...
	for tablename, rows in added.items():
		...
		for child in rows:
			child_doc = dontmanage.get_doc(child)
...
def get_updates(producer_site, last_update, doctypes):
	...
	docs = producer_site.post_request(
		{
			"cmd": "dontmanage.event_streaming.doctype.event_update_log.event_update_log.get_update_logs_for_consumer",
			"event_consumer": get_url(),
			"doctypes": dontmanage.as_json(doctypes),
			"last_update": last_update,
		}
	)
	return [dontmanage._dict(d) for d in (docs or [])]
...
def get_local_doc(update):
	...
	try:
		if not update.use_same_name:
			return dontmanage.get_doc(update.ref_doctype, {"remote_docname": update.docname})
		return dontmanage.get_doc(update.ref_doctype, update.docname)
...
def sync_dependencies(document, producer_site):
	...
	def sync_child_table_dependencies(doc, table_fields, producer_site):
		for df in table_fields:
			...
			for entry in child_table:
				...
				if child_doc:
					child_doc = dontmanage._dict(child_doc)
					set_dependencies(child_doc, dontmanage.get_meta(entry.doctype).get_link_fields(), producer_site)
	...
	def sync_dynamic_link_dependencies(doc, dl_fields, producer_site):
		for df in dl_fields:
			...
			if docname and not check_dependency_fulfilled(linked_doctype, docname):
				...
				dontmanage.get_doc(master_doc).insert(set_name=docname)
	...
	def set_dependencies(doc, link_fields, producer_site):
		for df in link_fields:
			...
			if docname and not check_dependency_fulfilled(linked_doctype, docname):
				master_doc = producer_site.get_doc(linked_doctype, docname)
				try:
					master_doc = dontmanage.get_doc(master_doc)
	...
	def check_dependency_fulfilled(linked_doctype, docname):
		return dontmanage.db.exists(linked_doctype, docname)
...
def sync_mapped_dependencies(dependencies, producer_site):
	...
	for entry in dependencies:
		doc = dontmanage._dict(json.loads(entry[1]))
		docname = dontmanage.db.exists(doc.doctype, doc.name)
		if not docname:
			doc = dontmanage.get_doc(doc).insert(set_child_names=False)
...
def log_event_sync(update, event_producer, sync_status, error=None):
	...
	doc = dontmanage.new_doc("Event Sync Log")
	...
	doc.data = dontmanage.as_json(update.data)
	doc.use_same_name = update.use_same_name
	doc.mapping = update.mapping if update.mapping else None
	if update.use_same_name:
		doc.docname = update.docname
	else:
		doc.docname = dontmanage.db.get_value(update.ref_doctype, {"remote_docname": update.docname}, "name")
	if error:
		doc.error = error
	doc.insert()
...
def get_mapped_update(update, producer_site):
	...
	mapping = dontmanage.get_doc("Document Type Mapping", update.mapping)
	if update.update_type == "Create":
		doc = dontmanage._dict(json.loads(update.data))
...
def new_event_notification(producer_url):
	...
	enqueued_method = "dontmanage.event_streaming.doctype.event_producer.event_producer.pull_from_node"
	jobs = get_jobs()
	if not jobs or enqueued_method not in jobs[dontmanage.local.site]:
		dontmanage.enqueue(enqueued_method, queue="default", **{"event_producer": producer_url})


@dontmanage.whitelist()
def resync(update):
	"""Retry syncing update if failed"""
	update = dontmanage._dict(json.loads(update))
	producer_site = get_producer_site(update.event_producer)
	event_producer = dontmanage.get_doc("Event Producer", update.event_producer)
	if update.mapping:
		update = get_mapped_update(update, producer_site)
		update.data = json.loads(update.data)

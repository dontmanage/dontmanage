...
from dontmanage.utils import set_request, update_progress_bar
...
class WebsiteSearch(FullTextSearch):
	...
	def get_document_to_index(self, route: str) -> dontmanage._dict | None:
		...
		try:
			...
			return dontmanage._dict(title=title, content=text_content, path=route)
		except Exception:
			pass
		finally:
...
def slugs_with_web_view(_items_to_index):
	...
	doctype_with_web_views = dontmanage.get_all("DocType", filters=filters, fields=fields)
		...
		if doctype.is_published_field:
			...
			if doctype.website_search_field:
				...
				for doc in docs:
					...
					_items_to_index += [dontmanage._dict(title=doc.title, content=text_content, path=doc.route)]
			else:
				docs = dontmanage.get_all(doctype.name, filters=filters, fields=fields)

...
class TestFullTextSearch(DontManageTestCase):
	def setUp(self):
		index = get_index()
		index.build()
		self.index = index
	...
	def test_search_term(self):
		...
		self.assertEqual(res[0], "sw/dontmanageerp")
	...
	def test_update_index(self):
		...
		self.index.update_index({"name": "sw/dontmanageerp", "content": """AwesomeDontManageErp"""})

		res = self.index.search("CommonSearchTerm")
		self.assertTrue("sw/dontmanageerp" not in res)

		res = self.index.search("AwesomeDontManageErp")
		self.assertEqual(res[0], "sw/dontmanageerp")

		# Update new doc
		self.index.update_index({"name": "sw/dontmanagebooks", "content": """DesktopAccounting"""})

		res = self.index.search("DesktopAccounting")
		self.assertEqual(res[0], "sw/dontmanagebooks")
...
def get_index():
	return TestWrapper("test_dontmanage_index")
...
def get_documents():
	...
	docs.append(
		{
			"name": "sw/dontmanageerp",
			"content": """DontManageErp is a free and open-source integrated Enterprise Resource Planning software developed by
			DontManage and is built on MariaDB database system using a Python based server-side framework.
			DontManageErp is a generic ERP software used by manufacturers, distributors and services companies. CommonSearchTerm""",
		}
	)
	...
	docs.append(
		{
			"name": "sw/dontmanage",
			"content": """DontManage Framework is a full-stack web framework, that includes everything you need to build and

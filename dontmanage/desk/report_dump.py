...
def get_data(doctypes, last_modified):
	data_map = {}
	for dump_report_map in dontmanage.get_hooks().dump_report_map:
		data_map.update(dontmanage.get_attr(dump_report_map))
	...
	for d in doctypes:
		...
		if d in last_modified:
			...
			out[dt]["modified_names"] = dontmanage.db.sql_list(
				"""select %sname from %s
				where %smodified > %s"""

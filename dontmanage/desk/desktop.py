...
class Workspace:
	def __init__(self, page, minimal=False):
		...
		self.restricted_doctypes = (
			dontmanage.cache.get_value("domain_restricted_doctypes") or build_domain_restricted_doctype_cache()
		)
		self.restricted_pages = (
			dontmanage.cache.get_value("domain_restricted_pages") or build_domain_restricted_page_cache()
		)
...
def get_workspace_sidebar_items():
	...
	for page in all_pages:
		try:
			...
			if not page["app"] and page["module"]:
				page["app"] = dontmanage.db.get_value("Module Def", page["module"], "app_name") or get_module_app(
					page["module"]
				)
	...
	return {
		...
		"has_create_access": dontmanage.has_permission(doctype="Workspace", ptype="create"),
	}
...
def get_installed_apps():
	return dontmanage.get_installed_apps()

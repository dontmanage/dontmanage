...
from dontmanage.modules.utils import create_directory_on_app_path
...
class WorkspaceSidebar(Document):
	...
	if TYPE_CHECKING:
		from dontmanage.desk.doctype.workspace_sidebar_item.workspace_sidebar_item import WorkspaceSidebarItem
	...
	def __init__(self, *args, **kwargs):
		...
		self.restricted_doctypes = dontmanage.cache.get_value("domain_restricted_doctypes")
		self.restricted_pages = dontmanage.cache.get_value("domain_restricted_pages")
	...
	def export_sidebar(self):
		allow_export = self.app and not dontmanage.flags.in_import and dontmanage.conf.developer_mode
		if allow_export:
			folder_path = create_directory_on_app_path("workspace_sidebar", self.app)
			file_path = os.path.join(folder_path, f"{dontmanage.scrub(self.title)}.json")
			...
			with open(file_path, "w+") as doc_file:
				doc_file.write(dontmanage.as_json(doc_export) + "\n")

	def on_trash(self):
		if is_workspace_manager():
			if dontmanage.conf.developer_mode and self.app:
				delete_file(self.app, self.title)
		else:
			dontmanage.throw(_("You need to be Workspace Manager to delete a public workspace."))
	...
	def is_item_allowed(self, name, item_type, allowed_workspaces):
		...
		if item_type == "doctype":
			return (
				...
				and dontmanage.has_permission(name)
			)
		if item_type == "page":
	...
	def get_module_from_items(self):
		...
		for item in self.items:
			if item.type != "Section Break" and item.type != "Sidebar Item Group" and item.link_type != "URL":
				try:
					all_modules_in_sidebars.append(dontmanage.get_doc(item.link_type, item.link_to).module)
				except dontmanage.DoesNotExistError as e:
					dontmanage.logger().error(e)
...
def delete_file(app, title):
	...
	file_path = os.path.join(folder_path, f"{dontmanage.scrub(title)}.json")
	if os.path.exists(file_path):
		os.remove(file_path)


def is_workspace_manager():
	return "Workspace Manager" in dontmanage.get_roles()
...
def create_workspace_sidebar_for_workspaces():
	...
	existing_sidebars = dontmanage.get_all("Workspace Sidebar", pluck="title")
	for workspace in all_workspaces:
		if workspace not in existing_sidebars:
			workspace_doc = dontmanage.get_doc("Workspace", workspace)
			sidebar = dontmanage.new_doc("Workspace Sidebar")
			sidebar.title = workspace
			sidebar.header_icon = dontmanage.db.get_value("Workspace", workspace, "icon")
			...
			workspace_sidebar_item = dontmanage.new_doc("Workspace Sidebar Item")
			...
			try:
				...
				dontmanage.log_error(title="Failed To Create Sidebar", message=e)


@dontmanage.whitelist()
def add_sidebar_items(sidebar_title, sidebar_items):
	sidebar_items = loads(sidebar_items)
	title = f"{sidebar_title}-{dontmanage.session.user}"
	w = dontmanage.get_doc("Workspace Sidebar", sidebar_title)
	if not dontmanage.conf.developer_mode:
		try:
			w = dontmanage.get_doc("Workspace Sidebar", title)
		except dontmanage.DoesNotExistError:
			dontmanage.clear_messages()
			w = dontmanage.copy_doc(w, ignore_no_copy=False)
			w.title = title
			w.for_user = dontmanage.session.user
	items = []
	current_idx = 1
	for item in sidebar_items:
		si = dontmanage.new_doc("Workspace Sidebar Item")
		si.update(item)
		si.idx = current_idx
		items.append(si)
		current_idx += 1
		...
		if nested_items:
			for nested_item in nested_items:
				new_nested_item = dontmanage.new_doc("Workspace Sidebar Item")
...
def add_to_my_workspace(workspace):
	try:
		...
		existing_sidebar = dontmanage.db.exists("Workspace Sidebar", sidebar_name)

		if existing_sidebar:
			private_sidebar = dontmanage.get_doc("Workspace Sidebar", existing_sidebar)
		else:
			# clone sidebar
			base_sidebar = dontmanage.get_doc("Workspace Sidebar", "My Workspaces")
			private_sidebar = dontmanage.copy_doc(base_sidebar)
		...
		dontmanage.log_error(title="Error in Adding Private Workspaces", message=e)
...
def auto_generate_sidebar_from_module():
	...
	sidebars = []
	for module in dontmanage.get_all("Module Def", pluck="name"):
		if not (
			dontmanage.db.exists("Workspace Sidebar", {"module": module})
			or dontmanage.db.exists("Workspace Sidebar", {"name": module})
		):
			...
			sidebar.app = dontmanage.local.module_app.get(dontmanage.scrub(module), None)
...
def get_module_info(module_name):
	...
	for entity in entities:
		...
		if entity.lower() == "page":
			...
			pluck = None
		module_info[entity] = dontmanage.get_all(
	...
	if len(doctype_names) > doctype_limit:
		try:
			...
			for doctype in doctype_names:
				if not is_single_doctype(doctype) and not dontmanage.get_meta(doctype).is_virtual:
					doctype_count_map[doctype] = dontmanage.db.count(doctype)
			top_doctypes = [
				name
			...
			return top_doctypes
		except dontmanage.db.ProgrammingError:
...
def create_sidebar_items(module_info):
	...
	for entity, items in module_info.items():
		...
		for item in items:
			...
			sidebar_item = dontmanage.new_doc("Workspace Sidebar Item")
...
def add_section_breaks(label, idx):
	section_break = dontmanage.new_doc("Workspace Sidebar Item")

...
dontmanage.ui.form.on("Number Card", {
	...
	create_add_to_dashboard_button: function (frm) {
		frm.add_custom_button(__("Add Card to Dashboard"), () => {
			const dialog = dontmanage.dashboard_utils.get_add_to_dashboard_dialog(
				frm.doc.name,
				"Number Card",
				"dontmanage.desk.doctype.number_card.number_card.add_card_to_dashboard"
			);

			if (!frm.doc.name) {
				dontmanage.msgprint(__("Please create Card first"));
			} else {
				dialog.show();
			}
		});
	...
	filters_config: function (frm) {
		frm.filters = eval(frm.doc.filters_config);
		const filter_values = dontmanage.report_utils.get_filter_values(frm.filters);
		...
		if (doctype) {
			dontmanage.model.with_doctype(doctype, () => {
				dontmanage.get_meta(doctype).fields.map((df) => {
					if (dontmanage.model.numeric_fieldtypes.includes(df.fieldtype)) {
	...
	set_report_filters: function (frm) {
		...
		if (report_name) {
			dontmanage.report_utils.get_report_filters(report_name).then((filters) => {
				if (filters) {
					frm.filters = filters;
					const filter_values = dontmanage.report_utils.get_filter_values(filters);
	...
	render_filters_table: function (frm) {
		...
		table.on("click", () => {
			...
			if (!dontmanage.boot.developer_mode && frm.doc.is_standard) {
				dontmanage.throw(__("Cannot edit filters for standard number cards"));
			}
	...
	set_parent_document_type: async function (frm) {
		...
		let doc_is_table =
			...
			(await new Promise((resolve) => {
				dontmanage.model.with_doctype(document_type, () => {
					resolve(dontmanage.get_meta(document_type).istable);
				});
			}));

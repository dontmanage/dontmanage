...
class Dashboard(Document):
	...
	def validate(self):
		if not dontmanage.conf.developer_mode and self.is_standard:
			dontmanage.throw(_("Cannot edit Standard Dashboards"))
		...
		if self.is_standard:
			...
			if non_standard_docs_map["Dashboard Chart"] or non_standard_docs_map["Number Card"]:
				...
				dontmanage.throw(message, title=_("Standard Not Set"), is_minimizable=True)
...
def get_permission_query_conditions(user):
	...
	if user == "Administrator" or "System Manager" in dontmanage.get_roles(user):
		return
...
def get_permitted_charts(dashboard_name):
	...
	for chart in dashboard.charts:
		if dontmanage.has_permission("Dashboard Chart", doc=chart.chart):
			chart_dict = dontmanage._dict()
...
def get_non_standard_cards_in_dashboard(dashboard):
	non_standard_cards = [doc.name for doc in dontmanage.get_list("Number Card", {"is_standard": 0})]

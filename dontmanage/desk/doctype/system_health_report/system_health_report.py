...
import dontmanage
from dontmanage.core.doctype.scheduled_job_type.scheduled_job_type import ScheduledJobType
from dontmanage.model.document import Document
from dontmanage.utils.background_jobs import get_queue, get_queue_list, get_redis_conn
		...
		def wrapper(*args, **kwargs):
			try:
				...
				if dontmanage.in_test:
					raise
				dontmanage.log(dontmanage.get_traceback())
				# nosemgrep
				dontmanage.msgprint(
					f"System Health check step {dontmanage.bold(step)} failed: {e}", alert=True, indicator="red"
				)
...
class SystemHealthReport(Document):
	...
	if TYPE_CHECKING:
		from dontmanage.desk.doctype.system_health_report_errors.system_health_report_errors import (
			SystemHealthReportErrors,
		)
		from dontmanage.desk.doctype.system_health_report_failing_jobs.system_health_report_failing_jobs import (
			SystemHealthReportFailingJobs,
		)
		from dontmanage.desk.doctype.system_health_report_queue.system_health_report_queue import (
			SystemHealthReportQueue,
		)
		from dontmanage.desk.doctype.system_health_report_tables.system_health_report_tables import (
			SystemHealthReportTables,
		)
		from dontmanage.desk.doctype.system_health_report_workers.system_health_report_workers import (
	...
	def fetch_background_jobs(self):
		...
		self.test_job_id = dontmanage.enqueue("dontmanage.ping", at_front=True).id
		self.background_jobs_check = "queued"
		workers = dontmanage.get_all("RQ Worker")
	...
	def fetch_scheduler(self):
		...
		failing_jobs = dontmanage.db.multisql(
			{
		...
		for job_type in dontmanage.get_all(
			"Scheduled Job Type",
			filters={"stopped": 0, "last_execution": ("<", threshold)},
			fields="*",
			order_by="last_execution asc",
		):
			job_type: ScheduledJobType = dontmanage.get_doc(doctype="Scheduled Job Type", **job_type)
	...
	def fetch_email_stats(self):
		...
		self.total_outgoing_emails = dontmanage.db.count("Email Queue", filters)
		self.pending_emails = dontmanage.db.count("Email Queue", {"status": "Not Sent", **filters})
		self.failed_emails = dontmanage.db.count("Email Queue", {"status": "Error", **filters})
		self.unhandled_emails = dontmanage.db.count("Unhandled Email", filters)
		self.handled_emails = dontmanage.db.count(
			"Communication",
	...
	def fetch_errors(self):
		...
		filters = {"creation": (">", threshold), "modified": (">", threshold)}
		self.total_errors = dontmanage.db.count("Error Log", filters)

		top_errors = dontmanage.db.sql(
	...
	def fetch_database_details(self):
		...
		self.database = dontmanage.db.db_type
		...
		if dontmanage.db.db_type == "sqlite":
			self.database_version = dontmanage.db.sql("select sqlite_version()")[0][0]
		else:
			self.database_version = dontmanage.db.sql("select version()")[0][0]

		if dontmanage.db.db_type == "mariadb":
			self.bufferpool_size = dontmanage.db.sql("show variables like 'innodb_buffer_pool_size'")[0][1]
			self.binary_logging = dontmanage.db.sql("show variables like 'log_bin'")[0][1]

	@health_check("Cache")
	def fetch_cache_details(self):
		self.cache_keys = len(dontmanage.cache.get_keys(""))
		self.cache_memory_usage = dontmanage.cache.execute_command("INFO", "MEMORY").get("used_memory_human")

	@health_check("Storage")
	def fetch_storage_details(self):
		from dontmanage.desk.page.backups.backups import get_context
		...
		self.onsite_backups = len(get_context(dontmanage._dict()).get("files", []))

	@health_check("Users")
	def fetch_user_stats(self):
		threshold = add_to_date(None, days=-30, as_datetime=True)
		self.total_users = dontmanage.db.count("User", {"enabled": 1})
		self.new_users = dontmanage.db.count("User", {"enabled": 1, "creation": (">", threshold)})
		self.failed_logins = dontmanage.db.count(
			"Activity Log",
			{
				"operation": "login",
				"status": "Failed",
		...
		self.active_sessions = dontmanage.db.count("Sessions")
...
def get_job_status(job_id: str | None = None):
	dontmanage.only_for("System Manager")
	try:
		return dontmanage.get_doc("RQ Job", job_id).status
	except Exception:
...
def _get_directory_size(*path):
	folder = os.path.abspath(dontmanage.get_site_path(*path))

...
class ToDo(Document):
	...
	def validate(self):
		...
		if self.is_new():
			if self.assigned_by == self.allocated_to:
				...
				assignment_message = dontmanage._("{0} assigned {1}: {2}").format(
			...
			if self.get_db_value("status") != self.status:
				if self.allocated_to == dontmanage.session.user:
					removal_message = dontmanage._("{0} removed their assignment.").format(
						get_fullname(dontmanage.session.user)
					)
				else:
					removal_message = dontmanage._("Assignment of {0} removed by {1}").format(
						get_fullname(self.allocated_to), get_fullname(dontmanage.session.user)
					)
	...
	def add_assign_comment(self, text, comment_type):
		...
		dontmanage.get_doc(self.reference_type, self.reference_name).add_comment(comment_type, text)
	...
	def update_in_reference(self):
		...
		try:
			assignments = dontmanage.db.get_values(
				"ToDo",
				{
			...
			elif dontmanage.db.is_missing_column(e):
	...
	def get_owners(cls, filters=None):
		...
		rows = dontmanage.get_all(cls.DocType, filters=filters or {}, fields=["allocated_to"])
...
def on_doctype_update():
	dontmanage.db.add_index("ToDo", ["reference_type", "reference_name"])

...
def after_doctype_insert():
	dontmanage.db.add_unique("Desktop Icon", ("module_name", "owner", "standard"))
...
def get_desktop_icons(user=None):
	...
	if not user_icons:
		...
		blocked_doctypes = dontmanage.get_all(
			"DocType",
		...
		standard_icons = dontmanage.get_all("Desktop Icon", fields=fields, filters={"standard": 1})
		...
		user_icons = dontmanage.get_all("Desktop Icon", fields=fields, filters={"standard": 0, "owner": user})
...
def add_user_icon(_doctype, _report=None, label=None, link=None, type="link", standard=0):
	...
	icon_name = dontmanage.db.exists(
		"Desktop Icon", {"standard": standard, "link": link, "owner": dontmanage.session.user}
	)

	if icon_name:
		if dontmanage.db.get_value("Desktop Icon", icon_name, "hidden"):
			# if it is hidden, unhide it
			dontmanage.db.set_value("Desktop Icon", icon_name, "hidden", 0)
			clear_desktop_icons_cache()

	else:
		idx = (
			dontmanage.db.sql("select max(idx) from `tabDesktop Icon` where owner=%s", dontmanage.session.user)[0][0]
			or dontmanage.db.sql("select count(*) from `tabDesktop Icon` where standard=1")[0][0]
		)

		if not dontmanage.db.get_value("Report", _report):
			_report = None
			userdefined_icon = dontmanage.db.get_value(
		...
		module_icon = dontmanage.get_value(
			"Desktop Icon",
		...
		try:
			...
			dontmanage.throw(_("Desktop Icon already exists"))
	...
	for i, module_name in enumerate(new_order):
		if module_name not in ("Explore",):
			if user:
				...
				name = dontmanage.db.get_value("Desktop Icon", {"standard": 1, "module_name": module_name})
				if name:
					icon = dontmanage.get_doc("Desktop Icon", name)
				else:
...
def set_desktop_icons(visible_list, ignore_duplicate=True):
	...
	if not int(dontmanage.defaults.get_defaults().setup_complete or 0):
		dontmanage.db.delete("Desktop Icon", {"standard": 0})

	# set standard as blocked and hidden if setting first active domain
	if not dontmanage.flags.keep_desktop_icons:
		dontmanage.db.sql("update `tabDesktop Icon` set blocked=0, hidden=1 where standard=1")

	# set as visible if present, or add icon
	for module_name in list(visible_list):
		name = dontmanage.db.get_value("Desktop Icon", {"module_name": module_name})
		if name:
			dontmanage.db.set_value("Desktop Icon", name, "hidden", 0)
		else:
			if dontmanage.db.exists("DocType", module_name):
				try:
					add_user_icon(module_name, standard=1)
				except dontmanage.UniqueValidationError as e:
...
def set_hidden(module_name, user=None, hidden=1):
	...
	if user:
		...
		if hidden and icon.custom:
			dontmanage.delete_doc(icon.doctype, icon.name, ignore_permissions=True)
			return

		# hidden by user
		icon.db_set("hidden", hidden)
	else:
		icon = dontmanage.get_doc("Desktop Icon", {"standard": 1, "module_name": module_name})
...
def get_all_icons():
	return [
		d.module_name for d in dontmanage.get_all("Desktop Icon", filters={"standard": 1}, fields=["module_name"])
	]
...
def get_user_copy(module_name, user=None):
	...
	desktop_icon_name = dontmanage.db.get_value(
		"Desktop Icon", {"module_name": module_name, "owner": user, "standard": 0}
	)

	if desktop_icon_name:
		return dontmanage.get_doc("Desktop Icon", desktop_icon_name)
	else:
...
def make_user_copy(module_name, user):
	...
	standard_name = dontmanage.db.get_value("Desktop Icon", {"module_name": module_name, "standard": 1})

	if not standard_name:
		dontmanage.throw(_("{0} not found").format(module_name), dontmanage.DoesNotExistError)

	original = dontmanage.get_doc("Desktop Icon", standard_name)

	desktop_icon = dontmanage.get_doc(
...
def sync_from_app(app):
	...
	try:
		modules = dontmanage.get_attr(app + ".config.desktop.get_data")() or {}
	except ImportError:
		return []
	...
	for i, m in enumerate(modules_list):
		...
		if desktop_icon_name:
			desktop_icon = dontmanage.get_doc("Desktop Icon", desktop_icon_name)
		else:
			# new icon
			desktop_icon = dontmanage.get_doc(
		...
		try:
			desktop_icon.save()
		except dontmanage.exceptions.UniqueValidationError:
			pass
...
def update_icons(hidden_list, user=None):
	...
	dontmanage.msgprint(dontmanage._("Updated"), indicator="green", title=_("Success"), alert=True)


def get_context(context):
	context.icons = get_user_icons(dontmanage.session.user)
	context.user = dontmanage.session.user
...
def get_module_icons(user=None):
	...
	if not user:
		icons = dontmanage.get_all("Desktop Icon", fields="*", filters={"standard": 1}, order_by="idx")
	else:
		dontmanage.cache.hdel("desktop_icons", user)
		icons = get_user_icons(user)

	for icon in icons:
		icon.value = dontmanage.db.escape(_(icon.label or icon.module_name))
...
def get_user_icons(user):
	...
	from dontmanage.boot import get_allowed_pages

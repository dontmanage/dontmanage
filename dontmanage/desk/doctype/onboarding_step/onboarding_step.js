...
dontmanage.ui.form.on("Onboarding Step", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		dontmanage.boot.developer_mode &&
			frm.set_intro(
				__(
...
function setup_fields(frm) {
	if (frm.doc.reference_document && frm.doc.action == "Update Settings") {
		dontmanage.model.with_doctype(frm.doc.reference_document, () => {
			let fields = dontmanage

...
from dontmanage.desk.desktop import save_new_widget
...
def save_page(title, public, new_widgets, blocks):
	...
	if not public:
		filters = {"for_user": dontmanage.session.user, "label": title + "-" + dontmanage.session.user}
...
def hide_unhide_page(page_name: str, is_hidden: bool):
	page = dontmanage.get_doc("Workspace", page_name)
...
def duplicate_page(page_name, new_page):
	...
	old_doc = dontmanage.get_doc("Workspace", page_name)
	...
	doc.module = ""
	if not doc.public:
		doc.for_user = doc.for_user or dontmanage.session.user
...
def sort_pages(sb_public_items, sb_private_items):
	...
	workspace_private_pages = get_page_list(["name", "title"], {"for_user": dontmanage.session.user})
	...
	for seq, d in enumerate(pages):
		for page in workspace_pages:
			if page.title == d.get("title"):
				doc = dontmanage.get_doc("Workspace", page.name)

...
dontmanage.ui.form.on("Workspace", {
	setup: function () {
		dontmanage.meta.get_field("Workspace Link", "only_for").no_default = true;
	},
	...
	refresh: function (frm) {
		...
		let url = `/desk/${
			frm.doc.public
				? dontmanage.router.slug(frm.doc.title)
				: "private/" + dontmanage.router.slug(frm.doc.title)
		}`;
		frm.sidebar
		...
		if (
			(frm.doc.for_user && frm.doc.for_user !== dontmanage.session.user) ||
			(frm.doc.public && !dontmanage.user.has_role("Workspace Manager"))
		) {

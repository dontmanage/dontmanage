...
import dontmanage
from dontmanage.desk.doctype.dashboard_chart.dashboard_chart import get
from dontmanage.tests.utils import DontManageTestCase
from dontmanage.utils import formatdate, get_last_day, getdate
from dontmanage.utils.dateutils import get_period, get_period_ending


class TestDashboardChart(DontManageTestCase):
	...
	def test_dashboard_chart(self):
		if dontmanage.db.exists("Dashboard Chart", "Test Dashboard Chart"):
			dontmanage.delete_doc("Dashboard Chart", "Test Dashboard Chart")
	...
	def test_empty_dashboard_chart(self):
		if dontmanage.db.exists("Dashboard Chart", "Test Empty Dashboard Chart"):
			dontmanage.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart")
	...
	def test_chart_wih_one_value(self):
		if dontmanage.db.exists("Dashboard Chart", "Test Empty Dashboard Chart 2"):
			dontmanage.delete_doc("Dashboard Chart", "Test Empty Dashboard Chart 2")

		dontmanage.db.delete("Error Log")

		# create one data point
		dontmanage.get_doc(dict(doctype="Error Log", creation="2018-06-01 00:00:00")).insert()
	...
	def test_group_by_chart_type(self):
		if dontmanage.db.exists("Dashboard Chart", "Test Group By Dashboard Chart"):
			dontmanage.delete_doc("Dashboard Chart", "Test Group By Dashboard Chart")

		dontmanage.get_doc({"doctype": "ToDo", "description": "test"}).insert()
		...
		todo_status_count = dontmanage.db.count("ToDo", {"status": result.get("labels")[0]})
	...
	def test_daily_dashboard_chart(self):
		...
		if dontmanage.db.exists("Dashboard Chart", "Test Daily Dashboard Chart"):
			dontmanage.delete_doc("Dashboard Chart", "Test Daily Dashboard Chart")
	...
	def test_weekly_dashboard_chart(self):
		...
		if dontmanage.db.exists("Dashboard Chart", "Test Weekly Dashboard Chart"):
			dontmanage.delete_doc("Dashboard Chart", "Test Weekly Dashboard Chart")
	...
	def test_avg_dashboard_chart(self):
		...
		if dontmanage.db.exists("Dashboard Chart", "Test Average Dashboard Chart"):
			dontmanage.delete_doc("Dashboard Chart", "Test Average Dashboard Chart")
	...
	def test_user_date_label_dashboard_chart(self):
		dontmanage.delete_doc_if_exists("Dashboard Chart", "Test Dashboard Chart Date Label")
		...
		with patch.object(dontmanage.utils.data, "get_user_date_format", return_value="dd.mm.yyyy"):
			result = get(chart_name="Test Dashboard Chart Date Label")
			self.assertEqual(
				sorted(result.get("labels")), sorted(["05.01.2019", "12.01.2019", "19.01.2019"])
			)

		with patch.object(dontmanage.utils.data, "get_user_date_format", return_value="mm-dd-yyyy"):
			result = get(chart_name="Test Dashboard Chart Date Label")
			self.assertEqual(
...
def create_new_communication(subject, date, rating):
	...
	comm = dontmanage.get_doc(communication)
	if not dontmanage.db.exists("Communication", {"subject": comm.subject}):
		comm.insert()

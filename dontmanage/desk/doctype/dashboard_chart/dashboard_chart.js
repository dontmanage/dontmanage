...
dontmanage.ui.form.on("Dashboard Chart", {
	setup: function (frm) {
	...
	before_save: function (frm) {
		...
		static_filters = dontmanage.dashboard_utils.remove_common_static_filter_values(
			static_filters,
			dynamic_filters
		);
	...
	refresh: function (frm) {
		...
		frm.is_disabled = !dontmanage.boot.developer_mode && frm.doc.is_standard;
		...
		if (!frm.is_new()) {
			frm.add_custom_button("Add Chart to Dashboard", () => {
				const dialog = dontmanage.dashboard_utils.get_add_to_dashboard_dialog(
					frm.doc.name,
					"Dashboard Chart",
					"dontmanage.desk.doctype.dashboard_chart.dashboard_chart.add_chart_to_dashboard"
				);

				if (!frm.doc.chart_name) {
					dontmanage.msgprint(__("Please create chart first"));
				} else {
					dialog.show();
				}
			});
	...
	set_heatmap_year_options: function (frm) {
		if (frm.doc.type == "Heatmap") {
			dontmanage.db.get_doc("System Settings").then((doc) => {
				const creation_date = doc.creation;
				frm.set_df_property(
					"heatmap_year",
					"options",
					dontmanage.dashboard_utils.get_years_since_creation(creation_date)
				);
			});
		}
	},
	...
	set_chart_report_filters: function (frm) {
		...
		if (report_name) {
			if (frm.doc.filters_json.length > 2) {
				...
				dontmanage.report_utils.get_report_filters(report_name).then((filters) => {
					if (filters) {
						frm.chart_filters = filters;
						let filter_values = dontmanage.report_utils.get_filter_values(filters);
	...
	set_chart_field_options: function (frm) {
		...
		if (frm.doc.dynamic_filters_json && frm.doc.dynamic_filters_json.length > 2) {
			filters = dontmanage.dashboard_utils.get_all_filters(frm.doc);
		}
		dontmanage
			.xcall("dontmanage.desk.query_report.run", {
			...
			.then((data) => {
				...
				if (!frm.doc.use_report_chart) {
					if (data.result.length) {
						frm.field_options = dontmanage.report_utils.get_field_options_from_report(
							data.columns,
							data
						);
						frm.set_df_property(
						...
						if (!frm.field_options.numeric_fields.length) {
							...
							let y_field_df = dontmanage.meta.get_docfield(
	...
	update_options: function (frm) {
		...
		if (doctype) {
			dontmanage.model.with_doctype(doctype, () => {
				...
				dontmanage.get_meta(doctype).fields.map((df) => {
	...
	show_filters: function (frm) {
		frm.chart_filters = [];
		dontmanage.dashboard_utils.get_filters_for_chart_type(frm.doc).then((filters) => {
			if (filters) {
				frm.chart_filters = filters;
			}
	...
	render_filters_table: function (frm) {
		...
		table.on("click", () => {
			frm.is_disabled && dontmanage.throw(__("Cannot edit filters for standard charts"));
			...
			dontmanage.dashboards.filters_dialog = dialog;
			...
			if (frm.doc.chart_type == "Report") {
				...
				dontmanage.query_reports[frm.doc.report_name] &&
					dontmanage.query_reports[frm.doc.report_name].onload &&
					dontmanage.query_reports[frm.doc.report_name].onload(dontmanage.query_report);
			}
	...
	render_dynamic_filters_table(frm) {
		...
		let fields = dontmanage.dashboard_utils.get_fields_for_dynamic_filter_dialog(
			is_document_type,
			filters,
			frm.dynamic_filters
		);
	...
	set_parent_document_type: async function (frm) {
		...
		dontmanage.model.with_doctype(document_type, async () => {
			let doc_is_table = dontmanage.get_meta(document_type).istable;
			frm.set_df_property("parent_document_type", "hidden", !doc_is_table);

			if (doc_is_table) {
				let parents = await dontmanage.xcall(
					"dontmanage.desk.doctype.dashboard_chart.dashboard_chart.get_parent_doctypes",

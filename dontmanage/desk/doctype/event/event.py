...
from dontmanage.model.utils.user_settings import get_user_settings, sync_user_settings, update_user_settings
...
from dontmanage.utils.caching import http_cache
from dontmanage.utils.user import get_enabled_system_users
...
if TYPE_CHECKING:
	from dontmanage.core.doctype.communication.communication import Communication
...
class Event(Document):
	...
	if TYPE_CHECKING:
		from dontmanage.core.doctype.dynamic_link.dynamic_link import DynamicLink
		from dontmanage.desk.doctype.event_notifications.event_notifications import EventNotifications
	...
	def validate(self):
		...
		if self.sync_with_google_calendar and not self.google_calendar:
			dontmanage.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		...
		for communication in communications:
			dontmanage.delete_doc("Communication", communication, force=True)
	...
	def sync_communication(self):
		...
		for participant in self.event_participants:
			if communications := dontmanage.get_all(
				"Communication",
				filters=[
				...
				for comm in communications:
					communication = dontmanage.get_doc("Communication", comm)
					self.update_communication(participant, communication)
			else:
				meta = dontmanage.get_meta(participant.reference_doctype)
	...
	def set_participants_email(self):
		for participant in self.event_participants:
			...
			participant.email = (
				dontmanage.get_value("Contact", participant_contact, "email_id") if participant_contact else None
			)
...
def update_attending_status(event_name, attendee, status):
	event_doc = dontmanage.get_doc("Event", event_name)

	if event_doc.owner == attendee == dontmanage.session.user:
		dontmanage.db.set_value("Event", event_name, "attending", status)
		return

	for participant in event_doc.event_participants:
		if participant.email == attendee:
			dontmanage.db.set_value("Event Participants", participant.name, "attending", status)
			return

	if not has_permission(event_doc, user=attendee):
		dontmanage.throw(_("You are not allowed to update the status of this event."))
...
def delete_communication(event, reference_doctype, reference_docname):
	...
	deleted_participant = dontmanage.get_doc(reference_doctype, reference_docname)

	comms = dontmanage.get_list(
		"Communication",
		filters=[
	...
	for comm in comms:
		dontmanage.delete_doc("Communication", comm)


def get_permission_query_conditions(user):
	if not user:
		user = dontmanage.session.user
	query = f"""(`tabEvent`.`event_type`='Public' or `tabEvent`.`owner`={dontmanage.db.escape(user)})"""
	if shared_events := dontmanage.share.get_shared("Event", user=user):
		query += f" or `tabEvent`.`name` in ({', '.join([dontmanage.db.escape(e) for e in shared_events])})"

	query += f" or exists (select 'x' from `tabEvent Participants` ep where ep.parent=`tabEvent`.name and ep.email={dontmanage.db.escape(user)})"
...
def send_event_digest():
	...
	for user in users:
		...
		if events:
			dontmanage.set_user_lang(user.name, user.language)
			...
			dontmanage.sendmail(
				...
				subject=dontmanage._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[dontmanage._("Events in Today's Calendar"), "blue"],
			)
...
def get_events(
	...
	type EventLikeDict = Event | dontmanage._dict
	...
	event_candidates: list[EventLikeDict] = dontmanage.db.sql(
		"""
...
def delete_events(ref_type, ref_name, delete_event=False):
	participations = dontmanage.get_all(
		"Event Participants",
	...
	if participations:
		for participation in participations:
			if delete_event:
				dontmanage.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = dontmanage.get_all(
					"Event Participants", filters={"parenttype": "Event", "parent": participation.parent}
				)

				if len(total_participants) <= 1:
					dontmanage.db.delete("Event", {"name": participation.parent})
					dontmanage.db.delete("Event Participants", {"name": participation.name})

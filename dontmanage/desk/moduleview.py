...
from dontmanage.desk.doctype.desktop_icon.desktop_icon import clear_desktop_icons_cache, set_hidden
...
def hide_module(module):
	set_hidden(module, dontmanage.session.user, 1)
		...
		def doctype_contains_a_record(name):
			...
			if not exists:
				if not dontmanage.db.get_value("DocType", name, "issingle"):
					exists = dontmanage.db.count(name)
				else:
					exists = True
...
def build_config_from_file(module):
	...
	data = []
	module = dontmanage.scrub(module)
...
def filter_by_restrict_to_domain(data):
	...
	pages = dontmanage.cache().get_value("domain_restricted_pages") or build_domain_restriced_page_cache()
...
def build_standard_config(module, doctype_info):
	...
	if not dontmanage.db.get_value("Module Def", module):
		dontmanage.throw(_("Module Not Found"))
...
def get_doctype_info(module):
	...
	doctype_info = dontmanage.get_all(
		"DocType",
...
def get_disabled_reports():
	if not hasattr(dontmanage.local, "disabled_reports"):
		dontmanage.local.disabled_reports = {r.name for r in dontmanage.get_all("Report", {"disabled": 1})}
	return dontmanage.local.disabled_reports


def get_config(app, module):
	"""Load module info from `[app].config.[module]`."""
	config = dontmanage.get_module(f"{app}.config.{module}")
...
def config_exists(app, module):
	try:
		dontmanage.get_module(f"{app}.config.{module}")
		return True
	except ImportError:
		return False
...
def get_links(app, module):
	try:
		sections = get_config(app, dontmanage.scrub(module))
	except ImportError:
		return []
...
def get_desktop_settings():
	...
	def apply_user_saved_links(module):
		module = dontmanage._dict(module)
		...
		if module.module_name in user_saved_links_by_module:
			user_links = dontmanage.parse_json(user_saved_links_by_module[module.module_name])
...
def update_hidden_modules(category_map):
	category_map = dontmanage.parse_json(category_map)
	home_settings = get_home_settings()

	saved_hidden_modules = home_settings.hidden_modules or []

	for category in category_map:
		config = dontmanage._dict(category_map[category])
...
def update_global_hidden_modules(modules):
	modules = dontmanage.parse_json(modules)
...
def update_links_for_module(module_name, links):
	links = dontmanage.parse_json(links)
	home_settings = get_home_settings()
...
def get_options_for_global_modules():
	...
	blocked_modules = dontmanage.get_doc("User", "Administrator").get_blocked_modules()
...
def set_home_settings(home_settings):
	dontmanage.cache().hset("home_settings", dontmanage.session.user, home_settings)
	dontmanage.db.set_value("User", dontmanage.session.user, "home_settings", json.dumps(home_settings))


@dontmanage.whitelist()
def get_home_settings():
	def get_from_db():
		settings = dontmanage.db.get_value("User", dontmanage.session.user, "home_settings")
		return dontmanage.parse_json(settings or "{}")

	home_settings = dontmanage.cache().hget("home_settings", dontmanage.session.user, get_from_db)
...
def get_last_modified(doctype):
	def _get():
		try:
			last_modified = dontmanage.get_all(
	...
	last_modified = dontmanage.cache().hget("last_modified", doctype, _get)

...
def get_report_doc(report_name):
	...
	if doc.disabled:
		dontmanage.throw(_("Report {0} is disabled").format(report_name))
...
def get_script(report_name):
	...
	if not script:
		script = "dontmanage.query_reports['%s']={}" % report_name
...
def save_report(reference_report, report_name, columns):
	...
	if docname:
		...
		dontmanage.msgprint(_("{0} saved successfully").format(new_report.name))
...
def get_filtered_data(ref_doctype, columns, data, user):
	...
	role_permissions = get_role_permissions(dontmanage.get_meta(ref_doctype), user)

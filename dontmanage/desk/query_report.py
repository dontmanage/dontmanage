...
from dontmanage.permissions import get_role_permissions, get_roles, has_permission
...
def get_report_doc(report_name):
	...
	if not dontmanage.has_permission(doc.ref_doctype, "report"):
		dontmanage.throw(
	...
	if doc.disabled:
		dontmanage.throw(_("Report {0} is disabled").format(_(report_name)))
...
def get_script(report_name):
	...
	module = report.module or dontmanage.db.get_value("DocType", report.ref_doctype, "module")
	...
	if not script:
		script = "dontmanage.query_reports['{}']={{}}".format(report_name)
...
def run(
	...
	report = get_report_doc(report_name)
	if not dontmanage.has_permission(report.ref_doctype, "report"):
		dontmanage.msgprint(
	...
	try:
		...
		dontmanage.log_error("Report Error")
		raise
...
def export_query():
	...
	if export_in_background:
		...
		dontmanage.enqueue(
			"dontmanage.desk.query_report.run_export_query_job",
...
def format_fields(data: dontmanage._dict) -> None:
...
def get_data_for_custom_field(doctype, field, names=None):
	if not dontmanage.has_permission(doctype, "read"):
		dontmanage.throw(_("Not Permitted to read {0}").format(_(doctype)), dontmanage.PermissionError)
...
def save_report(reference_report, report_name, columns, filters):
	...
	if docname:
		...
		dontmanage.msgprint(_("{0} saved successfully").format(_(new_report.name)))
...
def get_filtered_data(ref_doctype, columns, data, user):
	...
	shared = dontmanage.share.get_shared(ref_doctype, user)
	columns_dict = get_columns_dict(columns)

	ref_doctype_meta = dontmanage.get_meta(ref_doctype)
			...
			for match_filters in filter_list:
				...
				for dt, idx in linked_doctypes.items():
					...
					if (
						...
						and dontmanage.db.exists(dt, cell_value)
					):
						match = False
						break

					if match:
						match = has_unrestricted_read_access(doctype=ref_doctype, user=dontmanage.session.user)
...
def has_unrestricted_read_access(doctype, user):
	...
	standard_perm_exists = dontmanage.db.exists(
		"DocPerm",
		permission_filters,
	)

	custom_perm_exists = dontmanage.db.exists(
		"Custom DocPerm",
		permission_filters,
	)
...
def get_column_as_dict(col):
	...
	if isinstance(col, str):
		...
		col_dict["fieldname"] = dontmanage.scrub(col[0])

	# dict
	else:
		col_dict.update(col)
		if "fieldname" not in col_dict:
			col_dict["fieldname"] = dontmanage.scrub(col_dict["label"])
...
def get_user_match_filters(doctypes, user):
	...
	for dt in doctypes:
		filter_list = dontmanage.desk.reportview.build_match_conditions(dt, user, False)
		if filter_list:

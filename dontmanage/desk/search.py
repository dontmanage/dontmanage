...
from dontmanage import _, bold, is_whitelisted, validate_and_sanitize_search_inputs
...
from dontmanage.utils import cint, cstr, escape_html, unique
...
def validate_ignore_user_permissions(form_doctype, link_fieldname, link_doctype):
	def _throw(message):
		dontmanage.throw(message, title=_('Error validating "Ignore User Permissions"'))

	meta = dontmanage.get_meta(form_doctype)
	...
	if link_field.fieldtype == "Table MultiSelect":
		child_meta = dontmanage.get_meta(link_field.options)
...
def get_names_for_mentions(search_term):
	...
	for mention_data in users_for_mentions + user_groups:
		...
		mention_data["link"] = dontmanage.utils.get_url_to_form(
...
def get_link_title(doctype, docname):
	...
	if meta.show_title_field_in_link:
		return dontmanage.db.get_value(doctype, docname, meta.title_field)

...
from dontmanage.permissions import check_doctype_permission, get_doc_permissions, has_permission
...
def getdoc(doctype, name):
	...
	if (key := dontmanage.can_cache_doc((doctype, name))) and dontmanage.cache.exists(key):
		dontmanage._set_document_in_cache(key, doc)
...
def get_meta_bundle(doctype):
	...
	bundle.extend(
		dontmanage.desk.form.meta.get_meta(df.options)
...
def get_docinfo(doc=None, doctype=None, name=None):
	from dontmanage.share import _get_users as get_docshares

	if not doc:
		doc = dontmanage.get_lazy_doc(doctype, name, check_permission=True)
...
def get_communications(doctype, name, start=0, limit=20):
	...
	dontmanage.get_lazy_doc(doctype, name).check_permission()

	return _get_communications(doctype, name, cint(start), cint(limit))


def get_comments(doctype: str, name: str, comment_type: str | list[str] = "Comment") -> list[dontmanage._dict]:
...
def get_communication_data(
	...
	return dontmanage.db.multisql(
		...
		dict(
			doctype=doctype,
			name=str(name),
			start=dontmanage.utils.cint(start),
			limit=limit,
...
def get_tags(doctype: str, name: str) -> str:
	from dontmanage.desk.doctype.tag_link.tag_link import has_tags

	if not has_tags(doctype):
		return ""

	tags = dontmanage.get_all(
		"Tag Link",
...
def get_document_email(doctype, name):
	from dontmanage.email.doctype.email_account.email_account import get_automatic_email_link
...
def get_title_values_for_link_and_dynamic_link_fields(doc, link_fields=None):
	...
	for field in link_fields:
		...
		meta = dontmanage.get_meta(doctype) if doctype else None
		if not meta or not meta.title_field or not meta.show_title_field_in_link:
			continue

		link_title = dontmanage.db.get_value(doctype, doc_fieldvalue, meta.title_field, cache=True, order_by=None)
...
def update_user_info(docinfo, doc=None):
	...
	dontmanage.utils.add_user_info(users, docinfo.user_info)

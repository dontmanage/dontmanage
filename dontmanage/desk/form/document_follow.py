...
def update_follow(doctype: str, doc_name: str, following: bool):
	if following:
		return (follow_document(doctype, doc_name, dontmanage.session.user) and True) or False
	else:
		return unfollow_document(doctype, doc_name, dontmanage.session.user)
...
def follow_document(doctype, doc_name, user):
	...
	if not dontmanage.get_meta(doctype).track_changes:
		dontmanage.toast(_("Can't follow since changes are not tracked."))
		return False

	if user == "Administrator":
		dontmanage.toast(_("Administrator can't follow"))
		return False

	if not dontmanage.db.get_value("User", user, "document_follow_notify", ignore=True, cache=True):
		dontmanage.toast(_("Document follow is not enabled for this user."))
		return False

	if not is_document_followed(doctype, doc_name, user):
		doc = dontmanage.new_doc("Document Follow")
		doc.update({"ref_doctype": doctype, "ref_docname": doc_name, "user": user})
		doc.save()
		dontmanage.toast(_("Following document {0}").format(doc_name))
		return doc
...
def unfollow_document(doctype, doc_name, user):
	...
	if doc:
		dontmanage.delete_doc("Document Follow", doc[0].name, force=True)
		dontmanage.toast(_("Un-following document {0}").format(doc_name))
		return False
	return False
...
def get_user_list(frequency):
	...
	User = DocType("User")
	return (
		dontmanage.qb.from_(DocumentFollow)
		.join(User)
...
def get_version(doctype, doc_name, frequency, user):
	...
	if version:
		for v in version:
			change = dontmanage.parse_json(v.data)
			time = dontmanage.utils.format_datetime(v.modified, "hh:mm a")
			timeline_items = []
			if change.changed:
...
def get_comments(doctype, doc_name, frequency, user):
	...
	for comment in comments:
		...
		time = dontmanage.utils.format_datetime(comment.modified, "hh:mm a")
		timeline.append(
			{
...
def _get_filters(frequency, user):
	...
	if frequency == "Weekly":
		filters += [
			["modified", ">", dontmanage.utils.add_days(dontmanage.utils.nowdate(), -7)],
			["modified", "<", dontmanage.utils.nowdate()],
		]

	elif frequency == "Daily":
		filters += [
			["modified", ">", dontmanage.utils.add_days(dontmanage.utils.nowdate(), -1)],
			["modified", "<", dontmanage.utils.nowdate()],
		]

	elif frequency == "Hourly":
		filters += [
			["modified", ">", dontmanage.utils.add_to_date(dontmanage.utils.now_datetime(), hours=-1)],

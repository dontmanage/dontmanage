...
def update_follow(doctype, doc_name, following):
	if following:
		return follow_document(doctype, doc_name, dontmanage.session.user)
	else:
...
def follow_document(doctype, doc_name, user):
	...
	if (not dontmanage.get_meta(doctype).track_changes) or user == "Administrator":
		return
...
def unfollow_document(doctype, doc_name, user):
	...
	if doc:
		dontmanage.delete_doc("Document Follow", doc[0].name)
		return 1
	return 0

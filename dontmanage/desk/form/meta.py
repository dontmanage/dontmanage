...
from dontmanage.build import scrub_html_template
...
def get_meta(doctype, cached=True) -> "FormMeta":
	...
	if cached:
		meta = dontmanage.client_cache.get_value(key)
		if not meta:
		...
		meta = FormMeta(doctype, cached=dontmanage.conf.developer_mode)
...
class FormMeta(Meta):
	...
	def add_code(self):
		...
		system_country = dontmanage.get_system_settings("country")
...
"""
	...
	def _show_missing_doctype_msg(self, df):
		...
		msg = _("Field {0} is referring to non-existing doctype {1}.").format(
			dontmanage.bold(df.fieldname), dontmanage.bold(df.options)
		)
		...
		dontmanage.throw(msg, title=_("Missing DocType"))

	def load_print_formats(self):
		print_formats = dontmanage.db.sql(
	...
	def load_workflows(self):
		...
		if workflow_name and dontmanage.db.exists("Workflow", workflow_name):
		...
		if not self.custom:
			...
			if hasattr(module, "form_grid_templates"):
				for key, path in module.form_grid_templates.items():
					templates[key] = get_html_format(dontmanage.get_app_path(app, path))
	...
	def load_workspaces(self):
		Shortcut = dontmanage.qb.DocType("Workspace Shortcut")
		Workspace = dontmanage.qb.DocType("Workspace")
		shortcut = (
			dontmanage.qb.from_(Shortcut)
		...
		if shortcut:
			...
			Link = dontmanage.qb.DocType("Workspace Link")
			link = (
				dontmanage.qb.from_(Link)
				.select(Link.parent)
				.inner_join(Workspace)
...
def get_code_files_via_hooks(hook, name):
	...
	for app_name in dontmanage.get_installed_apps():
		code_hook = dontmanage.get_hooks(hook, default={}, app_name=app_name)
		if not code_hook:
			continue
		...
		for file in files:
			path = dontmanage.get_app_path(app_name, *file.strip("/").split("/"))

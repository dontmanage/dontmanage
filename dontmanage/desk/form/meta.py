...
def get_meta(doctype, cached=True):
	...
	if dontmanage.local.lang != "en":
		meta.set_translations(dontmanage.local.lang)
...
"""
	...
	def add_search_fields(self):
		...
		for df in self.get("fields", {"fieldtype": "Link", "options": ["!=", "[Select]"]}):
			if df.options:
				try:
					search_fields = dontmanage.get_meta(df.options).search_fields
	...
	def add_linked_document_type(self):
		for df in self.get("fields", {"fieldtype": "Link"}):
			if df.options:
				try:
					df.linked_document_type = dontmanage.get_meta(df.options).document_type
				except dontmanage.DoesNotExistError:
					self._show_missing_doctype_msg(df)

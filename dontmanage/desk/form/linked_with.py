...
import dontmanage
import dontmanage.desk.form.load
import dontmanage.desk.form.meta
from dontmanage import _
from dontmanage.model.meta import is_single
...
def get_submitted_linked_docs(doctype: str, name: str, ignore_doctypes_on_cancel_all=None) -> list[tuple]:
	...
	* User should be able to cancel the linked documents along with the one user trying to cancel.
	...
	dontmanage.has_permission(doctype, doc=name, throw=True)
...
class SubmittableDocumentTree:
	def __init__(self, doctype: str, name: str):
		...
		* Node is referenced(linked) by many other documents and those are the child nodes.
		...
		NOTE: child document is a property of child node (not same as DontManage child docs of a table field).
		"""
	...
	def get_submittable_doctypes(self) -> list[str]:
		...
		if not self._submittable_doctypes:
			self._submittable_doctypes = dontmanage.get_all(
...
def get_references_across_doctypes_by_dynamic_link_field(
	...
	for doctype, fieldname, doctype_fieldname in links:
		try:
			...
			for linked_to in dontmanage.get_all(
				doctype,
...
def cancel_all_linked_docs(docs, ignore_doctypes_on_cancel_all=None):
	...
	for i, doc in enumerate(docs, 1):
		if validate_linked_doc(doc, ignore_doctypes_on_cancel_all):
			linked_doc = dontmanage.get_doc(doc.get("doctype"), doc.get("name"))
			linked_doc.cancel()
		dontmanage.publish_progress(percent=i / len(docs) * 100, title=_("Cancelling documents"))
...
def validate_linked_doc(docinfo, ignore_doctypes_on_cancel_all=None):
	...
	if not dontmanage.get_meta(docinfo.get("doctype")).is_submittable:
		return False
...
def get_linked_docs(doctype: str, name: str, linkinfo: dict | None = None) -> dict[str, list]:
	...
	is_target_doctype_table = dontmanage.get_meta(doctype).istable

	for linked_doctype, link_context in linkinfo.items():
		# Don't try to fetch linked documents if the user can't read the doctype
		if not dontmanage.has_permission(linked_doctype):
			continue

		linked_doctype_meta = dontmanage.get_meta(linked_doctype)
		...
		if filters_ctx := link_context.get("filters"):
			ret = dontmanage.get_list(doctype=linked_doctype, fields=fields, filters=filters_ctx, order_by=None)
			...
			if linked_doctype_meta.istable:
				if not (
					dontmanage.db.exists("DocField", {"options": linked_doctype})
					or dontmanage.db.exists(linked_doctype, {"parenttype": doctype, "parent": name})
				):
					continue
			ret = dontmanage.get_list(
...
def get(doctype, docname):
	dontmanage.has_permission(doctype, doc=docname, throw=True)
...
def _get_linked_doctypes(doctype, without_ignore_user_permissions_enabled=False):
	...
	links = dontmanage.get_all("DocField", fields=["parent as dt"], filters=filters)
	links += dontmanage.get_all("Custom Field", fields=["dt"], filters=filters)
...
def get_linked_fields(doctype, without_ignore_user_permissions_enabled=False):
	...
	table_doctypes = dontmanage.get_all(
	...
	for parent, options in dontmanage.get_all(
...
def get_dynamic_linked_fields(doctype, without_ignore_user_permissions_enabled=False):
	...
	for df in links:
		...
		meta = dontmanage.get_meta(df.doctype)
		if meta.is_virtual:
			continue

		is_child = meta.istable
		possible_link = dontmanage.get_all(
			df.doctype,

...
from dontmanage.model.qb_query import DatabaseQuery
from dontmanage.model.utils import is_virtual_doctype
from dontmanage.utils import add_user_info, cint, format_duration
...
def get():
	...
	if is_virtual_doctype(args.doctype):
		...
		data = compress(dontmanage.call(controller.get_list, args=args, **args))
	else:
...
def get_list():
	...
	if is_virtual_doctype(args.doctype):
		controller = get_controller(args.doctype)
		data = dontmanage.call(controller.get_list, args=args, **args)
	else:
		# uncompressed (refactored from dontmanage.model.db_query.get_list)
		data = execute(**args)
...
def get_count() -> int | None:
	...
	if is_virtual_doctype(args.doctype):
		controller = get_controller(args.doctype)
		return dontmanage.call(controller.get_count, args=args, **args)
	...
	if not args.limit:
		...
		return dontmanage.db.sql(f"select count(*) from ( {partial_query} ) p")[0][0]
	...
	timeout_clause = "SET STATEMENT max_statement_time=1 FOR" if dontmanage.db.db_type == "mariadb" else ""

	try:
		count = dontmanage.db.sql(f"{timeout_clause} select count(*) from ( {partial_query} ) p")[0][0]
	except Exception as e:
		if dontmanage.db.is_statement_timeout(e):  # Skip fetching accurate count
			count = None
		else:
			raise

	if count == args.limit or count is None:
		dontmanage.local.response_headers.set("Cache-Control", "private,max-age=600,stale-while-revalidate=10800")
	...
	if isinstance(filters, list):
		...
		for condition in filters:
			if len(condition) == 3:
				...
				if is_standard(fieldname):
					continue
				meta = dontmanage.get_meta(condition[0])
...
def setup_group_by(data):
	...
	if data.group_by and data.aggregate_function:
		if data.aggregate_function.lower() not in ("count", "sum", "avg"):
			dontmanage.throw(_("Invalid aggregate function"))

		if dontmanage.db.has_column(data.aggregate_on_doctype, data.aggregate_on_field):
...
def raise_invalid_field(fieldname):
	dontmanage.throw(_("Field not permitted in query") + f": {fieldname}", dontmanage.DataError)
...
def get_parenttype_and_fieldname(field, data):
	if "." in field:
		...
		parenttype = parts[0]
		fieldname = parts[1]
		df = dontmanage.get_meta(data.doctype).get_field(parenttype)
...
def compress(data, args=None):
	...
	from dontmanage.desk.query_report import add_total_row
	...
	if args.get("add_total_row"):
		meta = dontmanage.get_meta(args.doctype)
...
def save_report(name, doctype, report_settings):
	...
	if dontmanage.db.exists("Report", name):
		report = dontmanage.get_doc("Report", name)
		if report.is_standard == "Yes":
			dontmanage.throw(_("Standard Reports cannot be edited"))

		if report.report_type != "Report Builder":
			dontmanage.throw(_("Only reports of type Report Builder can be edited"))

		if report.owner != dontmanage.session.user and not report.has_permission("write"):
			dontmanage.throw(_("Insufficient Permissions for editing Report"), dontmanage.PermissionError)
	else:
	...
	dontmanage.msgprint(
		_("Report {0} saved").format(dontmanage.bold(report.name)),
		indicator="green",
		alert=True,
	)
	return report.name
...
@dontmanage.whitelist(methods=["POST", "DELETE"])
def delete_report(name):
	"""Delete reports of type Report Builder from Report View"""

	report = dontmanage.get_doc("Report", name)
	if report.is_standard == "Yes":
		dontmanage.throw(_("Standard Reports cannot be deleted"))

	if report.report_type != "Report Builder":
		dontmanage.throw(_("Only reports of type Report Builder can be deleted"))

	if report.owner != dontmanage.session.user and not report.has_permission("delete"):
		dontmanage.throw(_("Insufficient Permissions for deleting Report"), dontmanage.PermissionError)

	report.delete(ignore_permissions=True)
	dontmanage.msgprint(
		_("Report {0} deleted").format(dontmanage.bold(report.name)),
		indicator="green",
		alert=True,
	)
...
def export_query():
	...
	from dontmanage.desk.utils import pop_csv_params
	...
	if export_in_background:
		...
		dontmanage.enqueue(
			"dontmanage.desk.reportview.run_report_view_export_job",
...
def run_report_view_export_job(user_email, form_params, csv_params):
	from dontmanage.desk.utils import send_report_email
...
def _export_query(form_params, csv_params, populate_response=True):
	from dontmanage.desk.utils import get_csv_bytes, provide_binary_file
	from dontmanage.utils.xlsxutils import handle_html, make_xlsx
	...
	if not dontmanage.permissions.can_export(doctype):
		if dontmanage.permissions.can_export(doctype, is_owner=True):
			for row in ret:
				if row[-1] != dontmanage.session.user:
	...
	if dontmanage.local.lang == "en" or not translate_values:
...
def get_field_info(fields, doctype):
	...
	for key in fields:
		...
		if parenttype == doctype and fieldname == "name":
			...
			elif df and df.fieldtype == "Link" and dontmanage.get_meta(df.options).translated_doctype:
				name = df.name
				label = _(df.label)
				fieldtype = df.fieldtype
				translatable = True
...
def delete_items():
	...
	items = sorted(json.loads(dontmanage.form_dict.get("items")), reverse=True)
	doctype = dontmanage.form_dict.get("doctype")

	if len(items) > 10:
		dontmanage.enqueue("dontmanage.desk.reportview.delete_bulk", doctype=doctype, items=items)
	else:
...
def delete_bulk(doctype, items):
	...
	for i, d in enumerate(items):
		try:
			dontmanage.flags.in_bulk_delete = True
...
def get_sidebar_stats(stats, doctype, filters=None):
	...
	if is_virtual_doctype(doctype):
		...
		data = dontmanage.call(controller.get_stats, args=args, **args)
	else:
...
def get_filter_dashboard_data(stats, doctype, filters=None):
	...
	for tag in tags:
		...
		if tag["type"] not in ["Date", "Datetime"]:
			from dontmanage.query_builder import Field, functions

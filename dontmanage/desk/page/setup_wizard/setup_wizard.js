...
dontmanage.pages["setup-wizard"].on_page_load = function (wrapper) {
	if (dontmanage.boot.setup_complete) {
		window.location.href = dontmanage.boot.apps_data.default_path || "/desk";
	}
...
dontmanage.setup.on("before_load", function () {
	if (
		dontmanage.boot.setup_wizard_completed_apps?.length &&
		dontmanage.boot.setup_wizard_completed_apps.includes("dontmanage")
	) {
		return;
	}
...
dontmanage.setup.SetupWizard = class SetupWizard extends dontmanage.ui.Slides {
	...
	post_setup_success() {
		...
		setTimeout(function () {
			...
			window.location.href = current_route || dontmanage.boot.apps_data.default_path || "/desk";
		}, 2000);
	}
...
dontmanage.setup.slides_settings = [
	{
		...
		onload: function (slide) {
			dontmanage.setup.utils.load_prefilled_data(slide, this.initialize_fields);
		},
		...
		initialize_fields: function (slide) {
			...
			if (dontmanage.setup.data.regional_data) {
				...
				dontmanage.setup.utils.load_regional_data(slide, setup_fields);
			}
	...
	},
	{
		...
		fields: [
			...
			},
			{
				...
				fieldtype: "Password",
				length: 512,
				depends_on: "eval:!dontmanage.boot.is_fc_site",
			},
		],
		...
		onload: function (slide) {
			if (dontmanage.session.user !== "Administrator") {
				...
				slide.form.fields_dict.email.refresh();
				if (!dontmanage.boot.is_fc_site) slide.form.fields_dict.password.df.reqd = 1;
...
dontmanage.setup.utils = {
	load_prefilled_data: function (slide, callback) {
		dontmanage.db
			...
			.then((r) => {
				if (r.message) {
					dontmanage.wizard.values.currency = r.message.currency;
					dontmanage.wizard.values.country = r.message.country;
					dontmanage.wizard.values.timezone = r.message.time_zone;
					dontmanage.wizard.values.language = r.message.language;
					...
					dontmanage.db.get_value(
						...
						(r) => {
							if (r) {
								dontmanage.wizard.values.full_name = r.full_name;
								dontmanage.wizard.values.email = r.email;
							}
						}
					);
				}

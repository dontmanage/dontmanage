...
from dontmanage.core.doctype.installed_applications.installed_applications import get_setup_wizard_completed_apps
from dontmanage.geo.country_info import get_country_info
from dontmanage.permissions import AUTOMATIC_ROLES
from dontmanage.translate import send_translations, set_default_language
...
def setup_complete(args):
	...
	if dontmanage.is_setup_complete():
		return {"status": "ok"}
	...
	try:
		...
		for idx, stage in enumerate(stages):
			...
			for task in stage.get("tasks"):
				...
				if "dontmanage" in setup_wizard_completed_apps:
				...
				if task.get("app_name"):
					...
					enable_setup_wizard_complete("dontmanage")
	except Exception:
...
def enable_setup_wizard_complete(app_name):
	dontmanage.db.set_value("Installed Application", {"app_name": app_name}, "is_setup_complete", 1)
...
def get_stages_hooks(args):  # nosemgrep
	...
	installed_apps = dontmanage.get_installed_apps(_ensure_on_bench=True)
	for app_name in installed_apps:
		setup_wizard_stages = dontmanage.get_hooks(app_name=app_name).get("setup_wizard_stages")
		if not setup_wizard_stages:
			continue

		for method in setup_wizard_stages:
			_stages = dontmanage.get_attr(method)(args)
	...
	system_settings.update(
		{
			...
			"enable_scheduler": 1 if not dontmanage.in_test else 0,

...
from dontmanage.utils import get_site_path, get_url
from dontmanage.utils.data import convert_utc_to_system_timezone
...
def get_context(context):
	...
	backup_limit = dontmanage.get_system_settings("backup_limit")
...
def delete_downloadable_backups():
	...
	for x in path.iterdir():
		...
		# Based on the naming convention of the backup files defined in dontmanage.utils.backups
		backup_name = x.name.rsplit("-" + dontmanage.local.site.replace(".", "_"), maxsplit=1)[0]
...
def schedule_files_backup(user_email: str):
	...
	if method not in queued_jobs[dontmanage.local.site]:
		enqueue(
			"dontmanage.desk.page.backups.backups.backup_files_and_notify_user",
			queue="long",
...
def backup_files_and_notify_user(user_email=None):
	from dontmanage.utils.backups import backup

dontmanage.pages["translation-tool"].on_page_load = function (wrapper) {
	...
	dontmanage.translation_tool = new TranslationTool(page);
};
...
class TranslationTool {
	constructor(page) {
		...
		this.wrapper.append(dontmanage.render_template("translation_tool"));
	...
	setup_language_filter() {
		let languages = Object.keys(dontmanage.boot.lang_dict).map((language_label) => {
			let value = dontmanage.boot.lang_dict[language_label];
			return {
		...
		if (translation_language || dontmanage.boot.lang !== "en") {
			language_selector.set_value(translation_language || dontmanage.boot.lang);
		} else {
			dontmanage.prompt(
				{
	...
	fetch_messages() {
		dontmanage.dom.freeze(__("Fetching..."));
		return dontmanage
			.xcall("dontmanage.translate.get_messages", {
			...
			<div
				...
				<div class="bold ellipsis">
					<span class="indicator ${this.get_indicator_color(message)}">
						<span>${dontmanage.utils.escape_html(message.source_text)}</span>
					</span>
				</div>
			</div>
		`;
	...
	get_additional_info(source_id) {
		dontmanage.dom.freeze("Fetching...");
		return dontmanage
			.xcall("dontmanage.translate.get_source_additional_info", {
				source: source_id,
				language: this.page.fields_dict["language"].get_value(),
			})
			.finally(dontmanage.dom.unfreeze);
	}

	make_edit_form(translation, { contributions, positions }) {
		if (!this.form) {
			this.form = new dontmanage.ui.FieldGroup({
				fields: [
					{
						fieldtype: "HTML",
						fieldname: "header",
	...
	show_confirmation_dialog() {
		this.confirmation_dialog = new dontmanage.ui.Dialog({
			fields: [
				{
					label: __("Language"),
					fieldname: "language",
	...
	create_translations() {
		dontmanage.dom.freeze(__("Submitting..."));
		return dontmanage
			.xcall("dontmanage.core.doctype.translation.translation.create_translations", {
	...
	setup_local_contributions() {
		// TODO: Refactor
		dontmanage
			.xcall("dontmanage.translate.get_contributions", {
				language: this.language,
			})
			.then((messages) => {
				let template = (message) => `
	...
	show_translation_status_modal(e, $el) {
		...
		dontmanage.xcall("dontmanage.translate.get_contribution_status", { message_id }).then((doc) => {
	...
	get_code_url(path, line_no, app) {
		...
		return `https://github.com/dontmanage/${app}/blob/develop/${code_path}#L${line_no}`;
	}
}

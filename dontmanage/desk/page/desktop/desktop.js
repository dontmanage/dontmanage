dontmanage.desktop_utils = {};
dontmanage.desktop_grids = [];
dontmanage.desktop_icons_objects = [];
dontmanage.new_icons = [];
$.extend(dontmanage.desktop_utils, {
	modal: null,
	modal_stack: [],
...
dontmanage.pages["desktop"].on_page_load = function (wrapper) {
	...
	dontmanage.pages["desktop"].desktop_page = desktop_page;
	// setup();
};

function get_workspaces_from_app_name(app_name) {
	const app = dontmanage.boot.app_data.filter((a) => {
...
function get_route(desktop_icon) {
	...
	if (desktop_icon.link_type == "External" && desktop_icon.link) {
		...
		let sidebar = dontmanage.boot.workspace_sidebar_item[desktop_icon.label.toLowerCase()];
		if (desktop_icon.link_type == "Workspace Sidebar" && sidebar) {
			...
			if (first_link) {
				if (first_link.link_type === "Report") {
					...
					if (first_link.report || !dontmanage.app.sidebar.editor.edit_mode) {
					...
					route = dontmanage.utils.generate_route(args);
				} else if (first_link.link_type == "Workspace") {
					let workspaces = dontmanage.workspaces[dontmanage.router.slug(first_link.link_to)];
					if (workspaces) {
						if (workspaces.public) {
							route = "/desk/" + dontmanage.router.slug(first_link.link_to);
						} else {
							route = "/desk/private/" + dontmanage.router.slug(workspaces.title);
						}
					}
					...
					route = dontmanage.utils.generate_route({
						type: first_link.link_type,
						name: first_link.link_to,
...
function get_desktop_icon_by_label(title, filters, force) {
	...
	let icons = dontmanage.desktop_icons;
	if (!force && dontmanage.pages["desktop"].desktop_page.edit_mode) {
		icons = dontmanage.new_desktop_icons;
	}
	if (!filters) {
...
function get_desktop_icon_by_idx(idx, parent_icon) {
	return dontmanage.boot.desktop_icons.find((f) => f.idx == idx && f.parent_icon == parent_icon);
}

function save_desktop(icons) {
	// saving in localStorage;
	dontmanage.pages["desktop"].desktop_page.save_layout(icons, dontmanage.new_icons);
}

function reset_to_default() {
	dontmanage.db.delete_doc("Desktop Layout", dontmanage.session.user).then(() => {
...
dontmanage.desktop_utils.get_folder_icons = function (folder_name) {
	let icons_in_folder = [];
	let icons = dontmanage.desktop_icons;
	if (dontmanage.pages["desktop"].desktop_page.edit_mode) {
		icons = dontmanage.new_desktop_icons;
	}
	icons.forEach((icon) => {
...
function add_icons_to_folder(folder_name, items) {
	...
	dontmanage.pages["desktop"].desktop_page.update();
}
...
class DesktopPage {
	...
	prepare() {
		...
		this.folders = [];
		const icon_map = {};
		let icons = this.edit_mode ? dontmanage.new_desktop_icons : dontmanage.desktop_icons;
	...
	get_saved_layout() {
		...
		if (keywords.includes(localStorage.getItem(`${dontmanage.session.user}:desktop`))) {
			return null;
		}
		return JSON.parse(localStorage.getItem(`${dontmanage.session.user}:desktop`));
	}
	sync_layout() {
		const me = this;
		let saved_layout = JSON.parse(localStorage.getItem(`${dontmanage.session.user}:desktop`));
		if (!this.data && saved_layout) {
			this.save_layout(saved_layout);
		} else if (Object.keys(this.data).length != 0) {
			dontmanage.desktop_icons = this.data;
		} else {
			dontmanage.desktop_icons = dontmanage.boot.desktop_icons;
		}
	}
	save_layout(layout, new_icons) {
		const me = this;
		dontmanage.call({
			method: "dontmanage.desk.doctype.desktop_layout.desktop_layout.save_layout",
			args: {
	...
	make() {
		...
		$(dontmanage.render_template("desktop")).appendTo(this.page.body);
		if (!this.data) {
	...
	setup_edit_button() {
		if (this.edit_mode || dontmanage.is_mobile()) return;
		const me = this;
		$(".desktop-edit").remove();
		...
		this.$desktop_edit_button.html(
			dontmanage.utils.icon("square-pen", "md", "", "", "", "", "white")
		);
		this.$desktop_edit_button.on("click", () => {
			dontmanage.new_desktop_icons = JSON.parse(JSON.stringify(dontmanage.desktop_icons));
	...
	stop_editing_layout(action) {
		...
		if (action === "cancel") {
			dontmanage.new_desktop_icons = null;
			this.update();
			return;
		}
		// submit
		save_desktop(dontmanage.new_desktop_icons);
	}
	...
	start_editing_layout() {
		...
		$(".desktop-edit").remove();
		dontmanage.desktop_icons_objects.forEach((icon) => {
			icon.edit_mode = true;
		});
		dontmanage.desktop_grids.forEach((desktop_grid) => {
	...
	add_new_icons_to_grid() {
		...
		this.add_new_icon = `<div class="desktop-icon desktop-edit-mode add-new-icon" title="Add New Icon">
		 ${dontmanage.utils.icon("plus", "lg")}
		 New Icon
		 </div>`;
		grid.append(this.add_new_icon);
		$(".add-new-icon").on("click", function () {
			dontmanage.ui.form.make_quick_entry(
				"Desktop Icon",
				function (icon) {
					dontmanage.new_desktop_icons.push(icon);
					dontmanage.new_icons.push(icon);
					dontmanage.pages["desktop"].desktop_page.update();
				},
				"",
				"",
	...
	setup_avatar() {
		$(".desktop-avatar").html(dontmanage.avatar(dontmanage.session.user, "avatar-medium"));
		let is_dark = document.documentElement.getAttribute("data-theme") === "dark";
		let menu_items = [
			{
				icon: "edit",
				label: "Edit Profile",
				url: `/desk/user/${dontmanage.session.user}`,
			},
			{
				icon: is_dark ? "sun" : "moon",
				label: "Toggle Theme",
				onClick: function () {
					new dontmanage.ui.ThemeSwitcher().show();
				},
			},
			{
				icon: "info",
				label: "About",
				onClick: function () {
					return dontmanage.ui.toolbar.show_about();
				},
			},
			{
				icon: "support",
				label: "DontManage Support",
				onClick: function () {
					window.open("https://support.dontmanage.io/help", "_blank");
				},
			},
			{
				icon: "rotate-ccw",
		...
		dontmanage.ui.create_menu({
			...
			open_on_left: !dontmanage.utils.is_rtl(),
		});
	}
	add_menu_item(item) {
	...
	handle_route_change() {
		const me = this;
		dontmanage.router.on("change", function () {
			if (dontmanage.get_route()[0] == "desktop" || dontmanage.get_route()[0] == "")
				me.setup_navbar();
			else {
				$(".navbar").show();
				dontmanage.desktop_utils.close_desktop_modal();
...
class DesktopIconGrid {
	...
	init() {
		...
		this.prepare();
		this.make();
		dontmanage.desktop_grids.push(this);
	}
	add_folder() {
		DesktopIconGrid.folder_count++;
		let icon = dontmanage.model.get_new_doc("Desktop Icon");
	...
	add_arrows(element) {
		...
		this.left_arrow = $(
			dontmanage.utils.icon("chevron-left", "lg", "", "", "left-page-arrow", "", stroke_color)
		);
		this.right_arrow = $(
			dontmanage.utils.icon("chevron-right", "lg", "", "", "right-page-arrow", "", stroke_color)
		);
			...
			this.sortable = new Sortable($(grid).get(0), {
				...
				onAdd(evt) {
					if (Sortable.get(evt.from).option("group").name == "hidden-icons-grid") {
						...
						icon.hidden = 0;
						dontmanage.new_desktop_icons.push(icon);
						let hidden_icons = dontmanage.pages.desktop.desktop_page.hidden_icons;
						let added_icon_index = hidden_icons.findIndex((d) => d.label == icon_name);
						hidden_icons.splice(added_icon_index, 1);
					}
				},
				onStart(evt) {
					dontmanage.desktop_utils.dragged_item = evt.item;
				},
				...
				onEnd: function (evt) {
					if (dontmanage.desktop_utils.in_folder_creation) return;
	...
	reorder_icons(reordered_icons, filters) {
		...
		dontmanage.desktop_icons.sort((a, b) => a.idx - b.idx);
	}
...
class DesktopIcon {
	constructor(icon, in_folder, grid_obj) {
		...
		if (render) {
			this.icon = $(
				dontmanage.render_template("desktop_icon", {
					icon: this.icon_data,
					in_folder: in_folder,
				})
			);
			...
			Object.defineProperty(this, "in_pane", {
				...
				set: function (value) {
					this._in_pane = value;
					if (value) {
						this.icon.find(".hide-button").html(dontmanage.utils.icon("plus"));
						this.icon.find(".hide-button").attr("data-mode", "add");
						this.setup_hide_button();
					} else {
						this.icon.find(".hide-button").html(dontmanage.utils.icon("x"));
						this.icon.find(".hide-button").attr("data-mode", "hide");
					}
				},
			});
			dontmanage.desktop_icons_objects.push(this);
		}
	...
	validate_icon() {
		...
		if (this.icon_data.label == "My Workspaces") {
			if (dontmanage.boot.workspace_sidebar_item["my workspaces"].items.length == 0)
				return false;
		}
	...
	setup_edit_menu() {
		const me = dontmanage.pages["desktop"].desktop_page;
			...
			menu_items: [
				{
					...
					onClick: function () {
						dontmanage.ui.form.make_quick_entry(
							"Desktop Icon",
							function (icon) {
								let old_index = dontmanage.new_desktop_icons.findIndex(
									(d_icon) => d_icon.label == icon.label
								);
								if (old_index !== -1) {
									dontmanage.new_desktop_icons.splice(old_index, 1);
								}
								dontmanage.new_desktop_icons.push(icon);
								dontmanage.new_icons.push(icon.name);
								dontmanage.pages["desktop"].desktop_page.update();
							},
							function (dialog) {
	...
	setup_click() {
		...
		if (this.child_icons?.length && (this.icon_type == "App" || this.icon_type == "Folder")) {
			$(this.icon).on("click", () => {
				let modal = dontmanage.desktop_utils.create_desktop_modal(me);
				...
				let title = new InlineEditor($title, this.icon_data.label, function (
					...
					let folder_icons = dontmanage.desktop_utils.get_folder_icons(old_value);
					if (icon) {
	...
	setup_dragging() {
		if (!dontmanage.pages["desktop"].desktop_page.edit_mode) return;
		this.icon.on("drag", (event) => {
			const mouse_x = event.clientX;
			const mouse_y = event.clientY;
			if (dontmanage.desktop_utils.modal) {
				let modal = dontmanage.desktop_utils.modal.modal
					.find(".modal-content")
					.get(0)
...
class DesktopModal {
	...
	setup(icon_title, child_icons_data, grid_row_size) {
		...
		const is_edit_mode = dontmanage.pages["desktop"].desktop_page.edit_mode;
		...
		this.modal.on("hidden.bs.modal", function () {
			me.modal.remove();
			dontmanage.desktop_utils.modal = null;
			dontmanage.desktop_utils.modal_stack = [];
		});
	}
	make_modal(icon_title) {
		if ($(".desktop-modal").length == 0) {
			this.modal = new dontmanage.get_modal(__(icon_title), "");
			...
			if (dontmanage.desktop_utils.modal_stack.length == 1) {
	...
	add_back_button() {
		...
		$(this.title_section).prepend(
			dontmanage.utils.icon("chevron-left", "md", "", "", "", "", "white")
		);
		$(this.title_section)
			.find(".icon")
			.on("click", function () {
				const [prev] = dontmanage.desktop_utils.modal_stack.splice(-1, 1);
				let icon =
					dontmanage.desktop_utils.modal_stack[dontmanage.desktop_utils.modal_stack.length - 1];
				if (icon) {
...
class IconsPane {
	...
	show() {
		...
		if (this.grid) {
			this.grid.icons_data = dontmanage.pages.desktop.desktop_page.hidden_icons;
		...
		this.grid = new DesktopIconGrid({
			...
			wrapper: this.wrapper,
			icons_data: dontmanage.pages.desktop.desktop_page.hidden_icons,
			row_size: 6,
			edit_mode: true,
			compact: true,
			is_pane: true,

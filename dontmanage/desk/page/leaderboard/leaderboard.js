dontmanage.pages["leaderboard"].on_page_load = (wrapper) => {
	dontmanage.leaderboard = new Leaderboard(wrapper);

	$(wrapper).bind("show", () => {
		// Get which leaderboard to show
		let doctype = dontmanage.get_route()[1];
		dontmanage.leaderboard.show_leaderboard(doctype);
	});
};
...
class Leaderboard {
	...
	get_leaderboard_config() {
		...
		dontmanage
			.xcall("dontmanage.desk.page.leaderboard.leaderboard.get_leaderboard_config")
			.then((config) => {
				...
				const _initial_doctype = dontmanage.get_route()[1] || this.doctypes[0];
	...
	setup_leaderboard_fields() {
		...
		this.type_select = this.page.add_select(
			...
			this.options.selected_filter.map((d) => {
				return { label: __(dontmanage.model.unscrub(d)), value: d };
			})
		);
	...
	create_date_range_field() {
		let timespan_field = $(this.parent).find(
			`.dontmanage-control[data-original-title="${__("Timespan")}"]`
		);
		this.date_range_field = $(`<div class="from-date-field"></div>`)
			.insertAfter(timespan_field)
			.hide();

		let date_field = dontmanage.ui.form.make_control({
			df: {
				fieldtype: "DateRange",
				fieldname: "selected_date_range",
				placeholder: __("Date Range"),
				default: [dontmanage.datetime.month_start(), dontmanage.datetime.now_date()],
	...
	render_selected_doctype() {
		this.$sidebar_list.on("click", "li", (e) => {
			...
			this.company_select.set_value(
				dontmanage.defaults.get_default("company") || this.company_select.get_value()
			);
			...
			dontmanage.set_route("leaderboard", this.options.selected_doctype);
			this.make_request();
		});
	}
	...
	make_request() {
		dontmanage.model.with_doctype(this.options.selected_doctype, () => {
	...
	get_leaderboard(notify) {
		...
		if (!company && !this.leaderboard_config[this.options.selected_doctype].company_disabled) {
			notify(this, null);
			dontmanage.show_alert(__("Please select Company"));
			return;
		}
		dontmanage
			...
			.then((r) => {
				...
				dontmanage.utils.make_chart(".leaderboard-graph", custom_options);
	...
	get_leaderboard_data(me, res) {
		if (res && res.message.length) {
			...
			dontmanage.utils.setup_search($(me.parent), ".list-item-container", ".list-id");
		} else {
			me.$graph_area.hide();
	...
	render_list_header() {
		const _selected_filter = this.options.selected_filter.map((i) => dontmanage.model.unscrub(i));
		const fields = ["rank", "name", this.options.selected_filter_item];
		const filters = fields
			.map((filter) => {
				const col = __(dontmanage.model.unscrub(filter));
	...
	render_message() {
		...
		return `<div class="leaderboard-empty-state ${display_class}">
  			<div class="no-result text-center">
  				<img src="/assets/dontmanage/images/ui-states/search-empty-state.svg"
  					alt="Empty State"
  					class="null-state"
  				>
	...
	get_item_html(item, index) {
		...
		const link = `/app/${dontmanage.router.slug(this.options.selected_doctype)}/${item.name}`;
	...
	get_sidebar_item(item, icon) {
		let icon_html = icon ? dontmanage.utils.icon(icon, "md") : "";
	...
	get_date_range() {
		...
		let current_date = dontmanage.datetime.now_date();
		let date_range_map = {
			"this week": [dontmanage.datetime.week_start(), dontmanage.datetime.week_end()],
			"this month": [dontmanage.datetime.month_start(), dontmanage.datetime.month_end()],
			"this quarter": [dontmanage.datetime.quarter_start(), dontmanage.datetime.quarter_end()],
			"this year": [dontmanage.datetime.year_start(), dontmanage.datetime.year_end()],
			"last week": [dontmanage.datetime.add_days(current_date, -7), current_date],
			"last month": [dontmanage.datetime.add_months(current_date, -1), current_date],
			"last quarter": [dontmanage.datetime.add_months(current_date, -3), current_date],
			"last year": [dontmanage.datetime.add_months(current_date, -12), current_date],
			"all time": null,
			"select date range": this.selected_date_range || [
				dontmanage.datetime.month_start(),
				current_date,
			],
		};

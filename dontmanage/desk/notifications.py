...
def get_notifications():
	...
	if dontmanage.flags.in_install or not dontmanage.is_setup_complete():
		return out
...
def clear_notifications(user=None):
	...
	if user:
		dontmanage.cache.hdel_names([f"notification_count:{name}" for name in groups], user)
	else:
		dontmanage.cache.delete_value([f"notification_count:{name}" for name in groups])
...
def get_open_count(doctype: str, name: str, items=None):
	...
	dontmanage.db.set_execution_timeout(1)

	try:
		return _get_linked_document_counts(doctype, name, items)
	except Exception as e:
		if dontmanage.db.is_statement_timeout(e):
			return {"count": []}
		raise
...
def get_doc_count(doctype, filters) -> int | Literal["?"]:
	try:
		...
		if dontmanage.db.is_statement_timeout(e):  # Skip fetching correct count if it's too slow
			return "?"
		raise

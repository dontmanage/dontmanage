...
import dontmanage
from dontmanage import sbool
from dontmanage.utils.data import cstr
from dontmanage.utils.response import Response

from .dontmanageclient import DontManageClient
...
class PushNotification:
	...
	def send_notification_to_user(
		...
		if strip_html:
			body = dontmanage.utils.strip_html(body)
	...
	def is_enabled(self) -> bool:
		...
		"""
		return sbool(
			dontmanage.db.get_single_value("Push Notification Settings", "enable_push_notification_relay")
		)
	...
	def _get_credential(self) -> tuple[str, str]:
		...
		"""
		notification_settings = dontmanage.get_doc("Push Notification Settings")
		...
		token = dontmanage.generate_hash(length=48)
		secret = dontmanage.generate_hash(length=32)

		# store the token in the redis cache
		dontmanage.cache.set_value(f"push_relay_registration_token:{secret}", token, expires_in_sec=600)
		body = {
			"endpoint": self._site_name,
			"protocol": self._site_protocol,
			"port": self._site_port,
			"token": token,
			"webhook_route": f"/api/method/dontmanage.push_notification.auth_webhook?secret={secret}",
		}
	...
	def _send_post_request(self, method: str, params: dict, use_authentication: bool = True):
		...
		relay_server_endpoint = dontmanage.conf.get("push_relay_server_url")
		if use_authentication:
			api_key, api_secret = self._get_credential()
			client = DontManageClient(relay_server_endpoint, api_key=api_key, api_secret=api_secret)
		else:
			client = DontManageClient(relay_server_endpoint)
	...
	def _site_name(self) -> str:
		return urlparse(dontmanage.utils.get_url()).hostname

	@property
	def _site_protocol(self) -> str:
		return urlparse(dontmanage.utils.get_url()).scheme

	@property
	def _site_port(self) -> str:
		site_uri = urlparse(dontmanage.utils.get_url())
...
def auth_webhook(secret: str):
	...
	if token := dontmanage.cache.get_value(f"push_relay_registration_token:{secret}"):
		response.data = token
...
def subscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).add_token(dontmanage.session.user, fcm_token)
	return {"success": success, "message": message}


@dontmanage.whitelist(methods=["GET"])
def unsubscribe(fcm_token: str, project_name: str) -> dict:
	success, message = PushNotification(project_name).remove_token(dontmanage.session.user, fcm_token)
	return {"success": success, "message": message}

...
from dontmanage.desk.page.user_profile.user_profile import get_energy_points_heatmap_data
...
class TestEnergyPointLog(DontManageTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def setUp(self):
		dontmanage.cache.delete_value("energy_point_rule_map")

	def tearDown(self):
		dontmanage.set_user("Administrator")
		dontmanage.db.delete("Energy Point Log")
		dontmanage.db.delete("Energy Point Rule")
		dontmanage.cache.delete_value("energy_point_rule_map")
	...
	def test_revert_points_on_cancelled_doc(self):
		...
		energy_point_logs = dontmanage.get_all("Energy Point Log")
		...
		energy_point_logs = dontmanage.get_all("Energy Point Log", fields=["reference_name", "type", "reverted"])
	...
	def test_allow_creation_of_new_log_if_the_previous_log_was_reverted(self):
		...
		log_name = dontmanage.db.exists("Energy Point Log", {"reference_name": created_todo.name})
		dontmanage.get_doc("Energy Point Log", log_name).revert("Just for test")
...
def create_energy_point_rule_for_todo(
	...
	name = "ToDo Closed"
	point_rule_exists = dontmanage.db.exists("Energy Point Rule", name)

	if point_rule_exists:
		return dontmanage.get_doc("Energy Point Rule", name)

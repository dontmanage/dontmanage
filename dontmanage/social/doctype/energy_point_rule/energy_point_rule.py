...
from dontmanage.core.doctype.user.user import get_enabled_users
from dontmanage.model import log_types
from dontmanage.model.document import Document
from dontmanage.social.doctype.energy_point_log.energy_point_log import create_energy_points_log
...
class EnergyPointRule(Document):
	def on_update(self):
		dontmanage.cache_manager.clear_doctype_map("Energy Point Rule", self.reference_doctype)
	...
	def eval_condition(self, doc):
		return self.condition and dontmanage.safe_eval(self.condition, None, {"doc": doc.as_dict()})
...
def process_energy_points(doc, state):
	...
	for d in dontmanage.cache_manager.get_doctype_map(
		"Energy Point Rule", doc.doctype, dict(reference_doctype=doc.doctype, enabled=1)
	):
		dontmanage.get_doc("Energy Point Rule", d.get("name")).apply(doc)
...
def revert_points_for_cancelled_doc(doc):
	...
	for log in energy_point_logs:
		reference_log = dontmanage.get_doc("Energy Point Log", log.name)
...
def get_energy_point_doctypes():
	return [
		d.reference_doctype
		for d in dontmanage.get_all("Energy Point Rule", ["reference_doctype"], {"enabled": 1})
	]

...
from dontmanage.modules.utils import get_doctype_app_map
from dontmanage.monitor import add_data_to_monitor
from dontmanage.utils.response import build_response
from dontmanage.utils.telemetry.pulse.app_heartbeat_event import capture_app_heartbeat
...
def handle(request: Request):
	...
	if dontmanage.get_system_settings("log_api_requests"):
	...
	if data is not None:
		dontmanage.response["data"] = data
	data = build_response("json")

	with suppress(Exception):
		method = arguments.get("method") or dontmanage.form_dict.get("method")
		doctype = arguments.get("doctype") or dontmanage.form_dict.get("doctype")
...
from dontmanage.api.v1 import url_rules as v1_rules
from dontmanage.api.v2 import url_rules as v2_rules
...
def get_api_version() -> ApiVersion | None:
	...
	if dontmanage.request.path.startswith(f"/api/{ApiVersion.V2.value}"):

...
from dontmanage import _, cint, cstr, get_newargs, is_whitelisted
...
def handle_rpc_call(method: str, doctype: str | None = None):
	...
	method = dontmanage.override_whitelisted_method(method)
...
def document_list(doctype: str) -> list[dict[str, Any]]:
	...
	Query Parameters (accessible via dontmanage.form_dict):
	...
	Response:
		dontmanage.response["data"]: List of document records as dicts
		dontmanage.response["has_next_page"]: Indicates if more pages are available
	...
	Controller Customization:
		...
		Example:
			class Project(Document):
				@staticmethod
				def get_list(query):
					Project = dontmanage.qb.DocType("Project")
					if user_has_role("Project Owner"):
						query = query.where(Project.owner == dontmanage.session.user)
					else:
	...
	fields: list | None = dontmanage.parse_json(args.get("fields", None))
	filters: dict | None = dontmanage.parse_json(args.get("filters", None))
	...
	if hasattr(controller, "get_list"):
		try:
			...
			dontmanage.throw(_("Error in {0}.get_list: {1}").format(doctype, str(e)))

	data = query.run(as_dict=as_dict, debug=debug)
	dontmanage.response["has_next_page"] = len(data) > limit
	return data[:limit]
...
def create_doc(doctype: str):
	...
	doc = dontmanage.new_doc(doctype, **data)
...
def copy_doc(doctype: str, name: str, ignore_no_copy: bool = True):
	...
	copy = dontmanage.copy_doc(doc, ignore_no_copy=ignore_no_copy)
...
def get_meta(doctype: str):
	dontmanage.only_for("All")
	return dontmanage.get_meta(doctype)
...
def execute_doc_method(doctype: str, name: str, method: str | None = None):
	...
	result = doc.run_method(method, **dontmanage.form_dict)
	dontmanage.response.docs.append(doc.as_dict())
	return result
...
def run_doc_method(method: str, document: dict[str, Any] | str, kwargs=None):
	...
	doc = dontmanage.get_doc(document, check_permission=PERMISSION_MAP[dontmanage.request.method])

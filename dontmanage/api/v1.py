...
from dontmanage.utils import attach_expanded_links
...
def document_list(doctype: str):
	...
	if dontmanage.form_dict.get("expand"):
		dontmanage.form_dict["expand"] = json.loads(dontmanage.form_dict["expand"])
...
def read_doc(doctype: str, name: str):
	...
	if sbool(dontmanage.form_dict.get("expand_links")):
...
def get_values_for_link_and_dynamic_link_fields(doc_dict):
	meta = dontmanage.get_meta(doc_dict.doctype)
	...
	for field in link_fields:
		...
		link_doc = dontmanage.get_doc(doctype, doc_fieldvalue)
...
def get_values_for_table_and_multiselect_fields(doc_dict):
	...
	for field in table_fields:
		table_link_fieldnames = [f.fieldname for f in dontmanage.get_meta(field.options).get_link_fields()]

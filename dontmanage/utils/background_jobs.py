...
from dontmanage.utils import CallbackManager, cint, get_bench_id, get_sites
...
MAX_QUEUED_JOBS = 500  # dontmanage.enqueue will start failing when these many jobs exist in queue.
...
def get_queues_timeout() -> dict[str, int]:
	...
	"""
	common_site_config = dontmanage.get_conf()
	...
	**kwargs,
	...
	if deduplicate:
		...
		if job and job.get_status(refresh=False) in (JobStatus.QUEUED, JobStatus.STARTED):
			dontmanage.logger().error(f"Not queueing job {job.id} because it is in queue already")
			return
		elif job:
	...
	if not is_async and not dontmanage.in_test:
		from dontmanage.deprecation_dumpster import deprecation_warning
	...
	call_directly = now or (not is_async and not dontmanage.in_test)
	if call_directly:
...
def run_doc_method(doctype, name, doc_method, **kwargs):
	getattr(dontmanage.get_doc(doctype, name), doc_method)(**kwargs)
...
def execute_job(site, method, event, job_name, kwargs, user=None, is_async=True, retry=0):
	...
	try:
	...
	except (dontmanage.db.InternalError, dontmanage.RetryBackgroundJobError) as e:
		dontmanage.db.rollback(chain=True)

		if retry < 5 and (
			isinstance(e, dontmanage.RetryBackgroundJobError)
			or (dontmanage.db.is_deadlocked(e) or dontmanage.db.is_timedout(e))
		):
			# retry the job if
			# 1213 = deadlock
			# 1205 = lock wait timeout
			# or RetryBackgroundJobError is explicitly raised
			dontmanage.job.after_job.reset()
			dontmanage.destroy()
			time.sleep(retry + 1)
			...
			dontmanage.log_error(title=method_name)
			raise

	except Exception as e:
		dontmanage.db.rollback(chain=True)
		dontmanage.log_error(title=method_name)
		dontmanage.monitor.add_data_to_monitor(exception=e.__class__.__name__)
		...
		if not hasattr(dontmanage.local, "site"):
			dontmanage.init(site, force=True)
			dontmanage.connect()
...
class DontManageWorker(Worker):
	def work(self, *args, **kwargs):
		self.start_dontmanage_scheduler()
	...
	def start_dontmanage_scheduler(self):
...
class DontManageWorkerNoFork(DontManageWorker):
...
def start_worker_pool(
	...
	import dontmanage.database.query  # sqlparse and indirect imports
	import dontmanage.query_builder  # pypika
	import dontmanage.utils  # common utils
	...
	if sbool(os.environ.get("DONTMANAGE_BACKGROUND_WORKERS_NOFORK", False)):
		worker_klass = DontManageWorkerNoFork
	else:
		import multiprocessing

		multiprocessing.set_start_method("fork", force=True)
		worker_klass = DontManageWorker
...
def validate_queue(queue: str, default_queue_list: list | None = None) -> None:
	...
	if queue not in default_queue_list:
		dontmanage.throw(_("Queue should be one of {0}").format(", ".join(default_queue_list)))
...
def get_redis_conn(username=None, password=None):
	if not hasattr(dontmanage.local, "conf"):
		raise Exception("You need to call dontmanage.init")
...
def enqueue_test_job():
	enqueue("dontmanage.utils.background_jobs.test_job", s=100)
...
def _check_queue_size(q: Queue):
	max_jobs = cint(dontmanage.conf.max_queued_jobs) or MAX_QUEUED_JOBS
	...
	if cint(q.count) >= max_jobs:
		...
		dontmanage.throw(
			...
			exc=dontmanage.QueueOverloaded,
			primary_action=primary_action if dontmanage.has_permission("System Health Report") else None,
		)

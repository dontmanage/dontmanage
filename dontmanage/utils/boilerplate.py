...
import dontmanage
from dontmanage.utils.change_log import get_app_branch
...
def make_boilerplate(dest, app_name, no_git=False):
	...
	app_name = dontmanage.scrub(app_name)
...
def _get_user_inputs(app_name):
	...
	new_app_config = {
		...
		"branch_name": {"prompt": "Branch Name", "default": get_app_branch("dontmanage")},
	}
...
def copy_from_dontmanage(rel_path: str, new_app_path: str):
	"""Copy files from dontmanage app to new app."""
	src = Path(dontmanage.get_app_path("dontmanage", "..")) / rel_path
...
def _create_app_boilerplate(dest, hooks, no_git=False):
	dontmanage.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, dontmanage.scrub(hooks.app_title)),
		with_init=True,
	)
	dontmanage.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True)
	dontmanage.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	dontmanage.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "pages"), with_init=True
	)
	dontmanage.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "includes"))
	dontmanage.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	dontmanage.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "css"))
	dontmanage.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "js"))
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "__init__.py"), "w") as f:
		f.write(dontmanage.as_unicode(init_template))

	with open(os.path.join(dest, hooks.app_name, "pyproject.toml"), "w") as f:
		f.write(dontmanage.as_unicode(pyproject_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, ".pre-commit-config.yaml"), "w") as f:
		f.write(dontmanage.as_unicode(precommit_template.format(**hooks)))

	license_body = get_license_text(license_name=hooks.app_license)
	with open(os.path.join(dest, hooks.app_name, "license.txt"), "w") as f:
		f.write(dontmanage.as_unicode(license_body))

	with open(
		os.path.join(dest, hooks.app_name, hooks.app_name, dontmanage.scrub(hooks.app_title), ".dontmanage"), "w"
	) as f:
		f.write("")

	from dontmanage.deprecation_dumpster import boilerplate_modules_txt
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "hooks.py"), "w") as f:
		f.write(dontmanage.as_unicode(hooks_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "patches.txt"), "w") as f:
		f.write(dontmanage.as_unicode(patches_template.format(**hooks)))

	app_directory = os.path.join(dest, hooks.app_name)

	copy_from_dontmanage(".editorconfig", app_directory)
	copy_from_dontmanage(".eslintrc", app_directory)
	...
	with open(os.path.join(dest, hooks.app_name, "README.md"), "w") as f:
		f.write(dontmanage.as_unicode(readme_template.format(**hooks)))

	if not no_git:
		with open(os.path.join(dest, hooks.app_name, ".gitignore"), "w") as f:
			f.write(dontmanage.as_unicode(gitignore_template.format(app_name=hooks.app_name)))
...
class PatchCreator:
	def __init__(self):
		self.all_apps = dontmanage.get_all_apps(sites_path=".", with_internal_apps=False)
	...
	def _ask_app_name(self):
		...
		self.app_dir = pathlib.Path(dontmanage.get_app_path(self.app))
	...
	def _ask_patch_meta_info(self):
		...
		default_filename = dontmanage.scrub(self.docstring) + ".py"
...
dynamic = ["version"]
dependencies = [
    # "dontmanage~=16.0.0" # Installed and managed by bench.
]
...
typing-modules = ["dontmanage.types.DF"]
...
# 	"dontmanage.desk.doctype.event.event.get_events": "{app_name}.event.get_events"
# }}
#
    ...
    steps:
      ...
      - name: Setup
        run: |
          ...
          bench init --skip-redis-config-generation --skip-assets --python "$(which python)" ~/dontmanage-bench

      - name: Install
        working-directory: /home/runner/dontmanage-bench
        run: |
...
# Read docs to understand patches: https://dontmanageframework.com/docs/v14/user/en/database-migrations
...
You can install this app using the [bench](https://github.com/dontmanage/bench) CLI:
...
- Linters: Runs [DontManage Semgrep Rules](https://github.com/dontmanage/semgrep-rules) and [pip-audit](https://pypi.org/project/pip-audit/) on every pull request.

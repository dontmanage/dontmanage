...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = dontmanage.utils.escape_html(e)
				dontmanage.throw(
					dontmanage._("{0} is not a valid Email Address").format(invalid_email),
...
def has_gravatar(email: str) -> str:
	...
	if dontmanage.flags.in_import or dontmanage.flags.in_install or dontmanage.flags.in_test:
...
def get_test_client(use_cookies=True) -> Client:
	"""Returns an test instance of the DontManage WSGI"""
...
def get_site_info():
	...
	return json.loads(dontmanage.as_json(site_info))
...
def get_db_count(*args):
	...
	return json.loads(dontmanage.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	"""
	return json.loads(dontmanage.as_json(dontmanage.call(fn, *args, **kwargs)))
...
def get_assets_json():
	...
	if not hasattr(dontmanage.local, "assets_json"):
		if not dontmanage.conf.developer_mode:
			dontmanage.local.assets_json = dontmanage.cache.get_value(
				"assets_json",
				_get_assets,
				shared=True,
			)
			...
			dontmanage.local.assets_json = _get_assets()

	return dontmanage.local.assets_json

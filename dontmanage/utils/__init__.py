...
from dontmanage.deprecation_dumpster import gzip_compress, gzip_decompress, make_esc
...
def get_fullname(user=None):
	...
	if not hasattr(dontmanage.local, "fullnames"):
		dontmanage.local.fullnames = {}

	if not dontmanage.local.fullnames.get(user):
		p = dontmanage.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			dontmanage.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			dontmanage.local.fullnames[user] = user

	return dontmanage.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return dontmanage.db.get_value("User", user, "email")
...
def validate_phone_number_with_country_code(phone_number: str, fieldname: str) -> None:
	...
	try:
		...
		if not valid_number:
			dontmanage.throw(
				error_message.format(dontmanage.bold(phone_number), dontmanage.bold(fieldname)),
				title=error_title,
				exc=dontmanage.InvalidPhoneNumberError,
			)
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		dontmanage.throw(
			dontmanage._("{0} is not a valid Phone Number").format(phone_number), dontmanage.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	* anything else is considered invalid
	...
	Note: "Name" here is name of a person, not the primary key in DontManage doctypes.
	"""
	if not name:
		return False
	...
	if not match and throw:
		dontmanage.throw(dontmanage._("{0} is not a valid Name").format(name), dontmanage.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	for name, addr in addresses:
		if not addr:
			if throw:
				dontmanage.throw(
					dontmanage._("{0} is not a valid Email Address").format(
						dontmanage.utils.escape_html(name or email_str)
					),
		...
		if not match:
			if throw:
				dontmanage.throw(
					dontmanage._("{0} is not a valid Email Address").format(dontmanage.utils.escape_html(addr)),
					dontmanage.InvalidEmailAddressError,
				)
			continue
...
def validate_url(
	...
	if not is_valid and throw:
		dontmanage.throw(dontmanage._("'{0}' is not a valid URL").format(dontmanage.bold(txt)))
...
def validate_iban(iban: str, throw: bool = False) -> bool:
	...
	if not valid and throw:
		dontmanage.throw(dontmanage._("'{0}' is not a valid IBAN").format(dontmanage.bold(iban)))
...
def has_gravatar(email: str) -> str:
	...
	if dontmanage.flags.in_import or dontmanage.flags.in_install or dontmanage.in_test:
...
def get_gravatar_url(email: str, default: Literal["mm", "404"] = "mm") -> str:
	...
	"""
	hexdigest = hashlib.md5(dontmanage.as_unicode(email).encode("utf-8"), usedforsecurity=False).hexdigest()
	...
	return Format(
		custom_var_printers=[
			...
			*[(variable_name, lambda *a, **kw: placeholder) for variable_name in blocklist],
			...
			(["_secret", dontmanage._dict, lambda *a, **kw: False], dict_printer),
		],
	)


def log(event, details):
	dontmanage.logger(event).info(details)
...
def get_defaults(key=None):
	...
	"""
	return dontmanage.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return dontmanage.db.set_default(key, val)
...
def execute_in_shell(cmd, verbose=False, low_priority=False, check_exit_code=False):
	...
	if failed:
		raise dontmanage.CommandFailedError(
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = dontmanage.local.site_path
	return os.path.join(base, *path)


def get_site_base_path():
	return dontmanage.local.site_path
...
def get_bench_path():
	return os.environ.get("DONTMANAGE_BENCH_ROOT") or os.path.realpath(
		os.path.join(os.path.dirname(dontmanage.__file__), "..", "..", "..")
	)


def get_bench_id():
	return dontmanage.get_conf().get("bench_id", get_bench_path().strip("/").replace("/", "-"))


def get_site_id(site=None):
	return f"{site or dontmanage.local.site}@{get_bench_id()}"
...
def get_test_client(use_cookies=True) -> Client:
	"""Return an test instance of the DontManage WSGI."""
	from dontmanage.app import application

	return Client(application, use_cookies=use_cookies)


def get_hook_method(hook_name, fallback=None):
	method = dontmanage.get_hooks().get(hook_name)
	if method:
		method = dontmanage.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in dontmanage.get_hooks(hook):
		out = out or dontmanage.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l, absolute=False):
	...
	if not getattr(dontmanage.local, "request", None) or is_cli():  # pragma: no cover
		lt = len(txt)
		try:
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in INCLUDE_DIRECTIVE_PATTERN.findall(html_format):
			for app_name in dontmanage.get_installed_apps():
				include_path = dontmanage.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(dontmanage.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else dontmanage.utils.md_to_html(text)
...
def get_site_info():
	from dontmanage.email.queue import get_emails_sent_this_month
	from dontmanage.utils.user import get_system_managers

	# only get system users
	users = dontmanage.get_all(
		"User",
		filters={"user_type": "System User", "name": ("not in", dontmanage.STANDARD_USERS)},
	...
	system_settings = dontmanage.db.get_singles_dict("System Settings")
	space_usage = dontmanage._dict((dontmanage.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": dontmanage.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in dontmanage.get_hooks("get_site_info"):
		site_info.update(dontmanage.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return orjson.loads(dontmanage.as_json(site_info))
...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                bench --site dontmanageerp.local execute dontmanage.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
		db_count[doctype] = dontmanage.db.count(doctype)

	return orjson.loads(dontmanage.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Pass a doctype or a series of doctypes to get the count of docs in them
	Parameters:
	        fn: dontmanage function to be called
	...
	Example:
	        via terminal:
	                bench --site dontmanageerp.local execute dontmanage.utils.call --args '''["dontmanage.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
	return orjson.loads(dontmanage.as_json(dontmanage.call(fn, *args, **kwargs)))
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, int | float):
			filters = dontmanage.as_unicode(filters)
...
def set_request(**kwargs):
	...
	dontmanage.local.request = Request(builder.get_environ())
...
def get_html_for_route(route):
	...
	return dontmanage.safe_decode(response.get_data())
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(dontmanage.local.sites_path, "assets/assets.json")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return dontmanage.utils.random_string(8)


def get_assets_json():
	def _get_assets():
		# get merged assets.json and assets-rtl.json
		assets = dontmanage.parse_json(dontmanage.read_file("assets/assets.json"))

		if assets_rtl := dontmanage.read_file("assets/assets-rtl.json"):
			assets.update(dontmanage.parse_json(assets_rtl))
...
def add_user_info(user: str | list[str] | set[str], user_info: dict[str, _UserInfo]) -> None:
	...
	missing_info = dontmanage.get_all(
		"User",
	...
	for info in missing_info:
		user_info.setdefault(info.name, dontmanage._dict()).update(
...
class CallbackManager:
	...
	Example usage: dontmanage.db.after_commit
	"""
...
def create_folder(path, with_init=False):
	...
	from dontmanage.utils import touch_file
...
def get_dontmanage_version() -> str:
	return getattr(dontmanage, "__version__", "unknown")


def get_app_version(app_name: str) -> str:
	try:
		return dontmanage.get_attr(app_name + ".__version__")
	except Exception:
		return "0.0.1"

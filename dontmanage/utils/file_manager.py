...
import dontmanage
from dontmanage import _, conf
from dontmanage.query_builder.utils import DocType
from dontmanage.utils import call_hook_method, cint, cstr, encode, get_files_path, get_hook_method
...
def get_file_url(file_data_name):
	data = dontmanage.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = dontmanage.form_dict.doctype
	dn = dontmanage.form_dict.docname
	file_url = dontmanage.form_dict.file_url
	filename = dontmanage.form_dict.filename
	dontmanage.form_dict.is_private = cint(dontmanage.form_dict.is_private)

	if not filename and not file_url:
		dontmanage.msgprint(_("Please select a file or url"), raise_exception=True)
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	# 	dontmanage.msgprint("URL must start with 'http://' or 'https://'")
	# 	return None, None

	file_url = unquote(file_url)
	file_size = dontmanage.form_dict.file_size
	...
	try:
		...
		return dontmanage.get_doc("File", f.duplicate_entry)
	return f


def get_uploaded_content():
	# should not be unicode when reading a file, hence using dontmanage.form
	if "filedata" in dontmanage.form_dict:
		if "," in dontmanage.form_dict.filedata:
			dontmanage.form_dict.filedata = dontmanage.form_dict.filedata.rsplit(",", 1)[1]
		dontmanage.uploaded_content = safe_b64decode(dontmanage.form_dict.filedata)
		dontmanage.uploaded_filename = dontmanage.form_dict.filename
		return dontmanage.uploaded_filename, dontmanage.uploaded_content
	else:
		dontmanage.msgprint(_("No file attached"))
		return None, None
...
def save_file(fname, content, dt, dn, folder=None, decode=False, is_private=0, df=None):
	...
	f = dontmanage.get_doc(file_data)
	f.flags.ignore_permissions = True
	try:
		f.insert()
	except dontmanage.DuplicateEntryError:
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in dontmanage.get_all(
		"File", {"content_hash": content_hash, "is_private": is_private}, pluck="name"
	):
		b = dontmanage.get_doc("File", name)
		return {k: b.get(k) for k in dontmanage.get_hooks()["write_file_keys"]}
	return False
...
def write_file(content, fname, is_private=0):
	...
	dontmanage.create_folder(file_path)
	# write the file
	if isinstance(content, str):
...
def remove_file(
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = dontmanage.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = doc.has_permission("write") or False
		if dontmanage.flags.in_web_form:
...
def delete_file(path):
	...
	if path:
		...
		if parts[0] == "files":
			path = dontmanage.utils.get_site_path("public", "files", parts[-1])

		else:
			path = dontmanage.utils.get_site_path("private", "files", parts[-1])
...
def get_file_name(fname, optional_suffix):
	...
	n_records = dontmanage.get_all("File", {"file_name": fname}, pluck="name")
...
def download_file(file_url):
	...
	Method : GET
	Endpoint : dontmanage.utils.file_manager.download_file
	...
	dontmanage.local.response.filecontent = filedata
...
def add_attachments(doctype, name, attachments):
	...
	for a in attachments:
		if isinstance(a, str):
			attach = dontmanage.db.get_value(
...
def is_safe_path(path: str) -> bool:
	...
	basedir = dontmanage.get_site_path()

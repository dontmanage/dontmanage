...
from dontmanage.database.database import Database, EmptyQueryValues


class DontManageIntegration(Integration):
	identifier = "dontmanage"
...
def set_scope(scope):
	if job := rq.get_current_job():
		...
		context = dontmanage._dict({"scheduled": False, "wait": 0})
		...
		if dontmanage.form_dict.cmd:
			path = f"/api/method/{dontmanage.form_dict.cmd}"
		else:
	...
	scope.set_user({"id": dontmanage.local.site})
	user = getattr(dontmanage.session, "user", "Unidentified")
	scope.set_tag("dontmanage_user", user)
	# Extract `X-DontManage-Request-ID` to store as a separate field if its present
	if trace_id := dontmanage.monitor.get_trace_id():
		scope.set_tag("dontmanage_trace_id", trace_id)
	...
	try:
		...
		with hub.configure_scope() as scope:
			...
			if dontmanage.request:
				evt_processor = _make_wsgi_event_processor(dontmanage.request.environ, False)
				scope.add_event_processor(evt_processor)
				if dontmanage.request.is_json:
					scope.set_context("JSON Body", dontmanage.request.json)
				elif dontmanage.request.form:
					scope.set_context("Form Data", dontmanage.request.form)

		if client := hub.client:
			exc_info = sys.exc_info()
			if any(exc_info):
				# Don't report errors which we can't "fix" in code
				if isinstance(exc_info[1], dontmanage.ValidationError | dontmanage.PermissionError):
					return
		...
		dontmanage.logger().error("Failed to capture exception", exc_info=True)

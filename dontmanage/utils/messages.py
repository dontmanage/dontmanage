...
from dontmanage.utils.data import safe_decode
...
def msgprint(
	...
	*,
	...
	msg = safe_decode(msg)
	out = dontmanage._dict(message=msg)
	...
	if dontmanage.flags.print_messages and out.message:
		print(f"Message: {msg}")
	...
	if raise_exception:
		...
		out.__dontmanage_exc_id = dontmanage.generate_hash()
	...
	if realtime:
		dontmanage.publish_realtime(event="msgprint", message=out)
	else:
		dontmanage.message_log.append(out)
	_raise_exception()
...
def get_message_log() -> list[dict]:
	return [msg_out for msg_out in dontmanage.local.message_log]


def clear_last_message():
	if len(dontmanage.local.message_log) > 0:
		dontmanage.local.message_log = dontmanage.local.message_log[:-1]


def throw(
	msg: str | Sequence[str],
	exc: type[Exception] | Exception = dontmanage.ValidationError,
	title: str | None = None,
	is_minimizable: bool = False,
	wide: bool = False,
	as_list: bool = False,
...
def throw_permission_error():
	throw(_("Not permitted"), dontmanage.PermissionError)

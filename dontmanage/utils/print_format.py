...
def download_multi_pdf_async(
	...
	dontmanage.local.response["http_status_code"] = http.HTTPStatus.CREATED
...
def _download_multi_pdf(
	...
	if not isinstance(doctype, dict):
		...
		for idx, ss in enumerate(result):
			try:
				...
				if task_id:
					dontmanage.publish_realtime(task_id=task_id, message={"message": "Failed"})
		...
		for doctype_name in doctype:
			...
			for doc_name in doctype[doctype_name]:
				try:
					...
					if task_id:
						dontmanage.publish_realtime(task_id=task_id, message="Failed")
	...
	with BytesIO() as merged_pdf:
		...
		if task_id:
			...
			_file.save()
			dontmanage.publish_realtime(f"task_complete:{task_id}", message={"file_url": _file.unique_url})
		else:
			dontmanage.local.response.filecontent = merged_pdf.getvalue()
			dontmanage.local.response.type = "pdf"


from dontmanage.deprecation_dumpster import read_multi_pdf
...
def render_letterhead_for_print(letterhead: str | None = None, doc: dict | str | None = None) -> dict:
	...
	if not dontmanage.has_permission("Letter Head", "read"):
		return {}
	...
	context_doc = dontmanage._dict(doc or {})
	rendered = {}

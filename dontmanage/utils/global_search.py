...
from dontmanage.utils import cint, strip_html_tags
...
def reset():
	...
	:return:
	"""
	dontmanage.db.delete("__global_search")
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in dontmanage.get_all("DocType", fields=["name", "module"], filters=filters):
			meta = dontmanage.get_meta(d.name)
			if len(meta.get_global_search_fields()) > 0:
				global_search_doctypes.append(d)

		installed_apps = dontmanage.get_installed_apps()
		module_app = dontmanage.local.module_app
...
def rebuild_for_doctype(doctype):
	...
	def _get_filters():
		filters = dontmanage._dict({"docstatus": ["!=", 2]})
	...
	if cint(meta.istable) == 1:
		parent_doctypes = dontmanage.get_all(
			"DocField",
			fields="parent",
			filters={"fieldtype": ["in", dontmanage.model.table_fields], "options": doctype},
		)
	...
	all_records = dontmanage.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = dontmanage.get_doc(doctype, doc.name)
			...
			all_contents.append(
				{
					"doctype": dontmanage.db.escape(doctype),
					"name": dontmanage.db.escape(doc.name),
					"content": dontmanage.db.escape(" ||| ".join(content or "")),
					"published": published,
					"title": dontmanage.db.escape((title or "")[: int(dontmanage.db.VARCHAR_LEN)]),
					"route": dontmanage.db.escape((route or "")[: int(dontmanage.db.VARCHAR_LEN)]),
				}
			)
	if all_contents:
...
def delete_global_search_records_for_doctype(doctype):
	dontmanage.db.delete("__global_search", {"doctype": doctype})
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = dontmanage._dict()
	child_search_fields = dontmanage._dict()

	for child in meta.get_table_fields():
		child_meta = dontmanage.get_meta(child.options)
		...
		if search_fields:
			...
			for record in child_records:
				all_children.setdefault(record.parent, dontmanage._dict()).setdefault(child.options, []).append(
					record
				)
...
def update_global_search(doc):
	...
	if content:
		...
		title = (cstr(doc.get_title()) or "")[: int(dontmanage.db.VARCHAR_LEN)]
...
def update_global_search_for_all_web_pages():
	if dontmanage.conf.get("disable_global_search"):
		return
...
def get_routes_to_index():
	...
	routes_to_index = []
	for app in apps:
		base = dontmanage.get_app_path(app, "www")
		path_to_index = dontmanage.get_app_path(app, "www")
...
def add_route_to_global_search(route):
	...
	dontmanage.local.no_cache = True
...
def get_formatted_value(value, field):
	...
	if getattr(field, "fieldtype", None) in ["Text", "Text Editor"]:
		value = unescape_html(dontmanage.safe_decode(value))
...
def sync_value_in_queue(value):
	try:
		...
		assert not dontmanage.flags.in_test, "Should not fail silently in tests"
		sync_value(value)
...
def delete_for_document(doc):
	...
	"""
	dontmanage.db.delete("__global_search", {"doctype": doc.doctype, "name": doc.name})
...
def search(text, start=0, limit=20, doctype=""):
	...
	from dontmanage.query_builder.functions import Match
		...
		for r in results:
			if r.doctype == doctype and r.rank > 0.0:
				try:
					...
					if meta.image_field:
						r.image = dontmanage.db.get_value(r.doctype, r.name, meta.image_field)
					if meta.title_field:
						r.title = dontmanage.db.get_value(r.doctype, r.name, meta.title_field)
				except Exception:

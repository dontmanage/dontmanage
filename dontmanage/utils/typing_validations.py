...
import dontmanage
from dontmanage.exceptions import DontManageTypeError
...
DontManagePydanticConfig = ConfigDict(arbitrary_types_allowed=True)
...
def TypeAdapter(type_):
	try:
		return PydanticTypeAdapter(type_, config=DontManagePydanticConfig)
...
def transform_parameter_types(func: Callable, args: tuple, kwargs: dict):
	...
	func_params = dontmanage._get_cached_signature_params(func)[0]
	...
	for current_arg, current_arg_type in annotations.items():
		...
		# allow slack for DontManage types

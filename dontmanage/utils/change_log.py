...
def get_change_log(user=None):
	...
	last_known_versions = dontmanage._dict(
		json.loads(dontmanage.db.get_value("User", user, "last_known_versions") or "{}")
	)
	...
	if "dontmanage" in current_versions:
		set_in_change_log("dontmanage", current_versions["dontmanage"], change_log)

	return change_log


def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(dontmanage.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = dontmanage.read_file(file_path)
...
def get_versions():
	...
	Example:
	        ...
	        {
	                "dontmanage": {
	                        "title": "DontManage Framework",
	                        "version": "5.0.0"
	                }
	        }"""
	versions = {}
	for app in dontmanage.get_installed_apps(_ensure_on_bench=True):
		app_hooks = dontmanage.get_hooks(app_name=app)
		versions[app] = {
		...
		try:
			versions[app]["version"] = dontmanage.get_attr(app + ".__version__")
...
def check_for_update():
	if dontmanage.get_system_settings("disable_system_update_notification"):
		return

	updates = dontmanage._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def has_app_update_notifications() -> bool:
	return bool(dontmanage.cache.sismember("changelog-update-user-set", dontmanage.session.user))
		...
		for vuln in advisory["vulnerabilities"]:
			with suppress(Exception):
				...
				if (
					...
					# XXX: this is not 100% correct, but works for dontmanage
...
def add_message_to_redis(update_json):
	...
	dontmanage.cache.set_value("changelog-update-info", json.dumps(update_json))
	user_list = [x.name for x in dontmanage.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in dontmanage.get_roles(user)]
	dontmanage.cache.sadd("changelog-update-user-set", *system_managers)
...
def show_update_popup():
	...
	update_info = dontmanage.cache.get_value("changelog-update-info")
	if not update_info:
		return
	...
	update_message = ""
	if dontmanage.cache.sismember("changelog-update-user-set", user):
		for update_type in updates:
			release_links = ""
			for app in updates[update_type]:
				app = dontmanage._dict(app)
				security_msg = ""
				if app.security_issues:
					...
					security_msg = security_msg.format(dontmanage.bold(app.security_issues))
	...
	primary_action = None
	if on_dontmanagecloud():
		primary_action = {
			"label": _("Update from DontManage Cloud"),
			"client_action": "window.open",
			"args": f"https://dontmanagecloud.com/dashboard/sites/{dontmanage.local.site}",
		}
	...
	if update_message:
		...
		dontmanage.cache.srem("changelog-update-user-set", user)


def get_pyproject(app: str) -> dict | None:
	from tomllib import load

	pyproject_path = dontmanage.get_app_path(app, "..", "pyproject.toml")

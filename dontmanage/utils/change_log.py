...
def check_release_on_github(app: str):
	"""
	Check the latest release for a given DontManage application hosted on Github.

	Args:
	        app (str): The name of the DontManage application.
...
def add_message_to_redis(update_json):
	...
	dontmanage.cache.set_value("update-info", json.dumps(update_json))
	...
	dontmanage.cache.sadd("update-user-set", *system_managers)
...
def show_update_popup():
	...
	update_info = dontmanage.cache.get_value("update-info")
	if not update_info:
		return
	...
	update_message = ""
	if dontmanage.cache.sismember("update-user-set", user):
		for update_type in updates:
			release_links = ""
	...
	if update_message:
		dontmanage.msgprint(update_message, title=_("New updates are available"), indicator="green")
		dontmanage.cache.srem("update-user-set", user)

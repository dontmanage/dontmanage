...
def set_encrypted_password(doctype, name, pwd, fieldname="password"):
	query = dontmanage.qb.into(Auth).columns(
...
def decrypt(txt, encryption_key=None, key: str | None = None):
	...
	try:
		...
		dontmanage.throw(
			...
			+ "<br><br>"
			+ _(
				"Please visit https://dontmanagecloud.com/docs/sites/migrate-an-existing-site#encryption-key for more information."
			),
		)
...
def get_password_reset_limit():
	return dontmanage.get_system_settings("password_reset_limit") or 3

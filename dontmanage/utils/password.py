...
from dontmanage.query_builder import Table
from dontmanage.utils import cstr, encode

Auth = Table("__Auth")


class LegacyPassword(pbkdf2_sha256):
	name = "dontmanage_legacy"
	ident = "$dontmanagel$"
...
def set_encrypted_password(doctype, name, pwd, fieldname="password"):
	...
	except dontmanage.db.DataError as e:
		if dontmanage.db.is_data_too_long(e):
			dontmanage.throw(_("Most probably your password is too long."), exc=e)
		raise e


def remove_encrypted_password(doctype, name, fieldname="password"):
	dontmanage.db.delete("__Auth", {"doctype": doctype, "name": name, "fieldname": fieldname})


def check_password(user, pwd, doctype="User", fieldname="password", delete_tracker_cache=True):
	"""Checks if user and password are correct, else raises dontmanage.AuthenticationError"""
	...
	if not result or not passlibctx.verify(pwd, result[0].password):
		raise dontmanage.AuthenticationError(_("Incorrect User or Password"))
...
def delete_login_failed_cache(user):
	dontmanage.cache().hdel("last_login_tried", user)
	dontmanage.cache().hdel("login_failed_count", user)
	dontmanage.cache().hdel("locked_account_time", user)
...
def delete_all_passwords_for(doctype, name):
	try:
		dontmanage.db.delete("__Auth", {"doctype": doctype, "name": name})
	except Exception as e:
		if not dontmanage.db.is_missing_column(e):
			raise


def rename_password(doctype, old_name, new_name):
	# NOTE: fieldname is not considered, since the document is renamed
	dontmanage.qb.update(Auth).set(Auth.name, new_name).where(
...
def encrypt(txt, encryption_key=None):
	...
	try:
		...
		dontmanage.throw(_("Encryption key is in invalid format!"))
...
def decrypt(txt, encryption_key=None):
	...
	try:
		...
		dontmanage.throw(_("Encryption key is invalid! Please check site_config.json"))


def get_encryption_key():
	if "encryption_key" not in dontmanage.local.conf:
		from dontmanage.installer import update_site_config

		encryption_key = Fernet.generate_key().decode()
		update_site_config("encryption_key", encryption_key)
		dontmanage.local.conf.encryption_key = encryption_key

	return dontmanage.local.conf.encryption_key


def get_password_reset_limit():
	return dontmanage.db.get_single_value("System Settings", "password_reset_limit") or 0

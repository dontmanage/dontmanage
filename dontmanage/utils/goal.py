...
from dontmanage.query_builder.functions import DateFormat, Function
from dontmanage.query_builder.utils import DocType
from dontmanage.utils.data import add_to_date, cstr, flt, now_datetime
...
def get_monthly_results(
	...
	if aggregation.lower() not in {"sum", "avg", "count", "min", "max"}:
		dontmanage.throw(f"Invalid aggregation type: {aggregation}")

	# Check that the goal and date fields exist on the chosen doctype
	valid_fields = dontmanage.get_meta(goal_doctype).get_valid_fields()
	if goal_field not in valid_fields:
		dontmanage.throw(f"Invalid goal field: {goal_field}")
	if date_col not in valid_fields:
		dontmanage.throw(f"Invalid date field: {date_col}")

	Table = DocType(goal_doctype)
	date_format = "%m-%Y" if dontmanage.db.db_type != "postgres" else "MM-YYYY"
...
def get_monthly_goal_graph_data(
	...
	if history and "{" in cstr(history):
		with suppress(ValueError):
			month_to_value_dict = dontmanage.parse_json(history)

	if month_to_value_dict is None:  # nosemgrep
		doc_filter = {}
		with suppress(ValueError):
			doc_filter = dontmanage.parse_json(filters or "{}")

...
from dontmanage.core.doctype.file.file import FILE_ENCODING_OPTIONS
from dontmanage.utils import cint, comma_or, cstr, flt


def read_csv_content_from_attached_file(doc):
	fileid = dontmanage.get_all(
		"File",
		fields=["name"],
	...
	try:
		_file = dontmanage.get_doc("File", fileid)
...
def read_csv_content(fcontent, use_sniffer: bool = False):
	...
	content = [dontmanage.safe_decode(line) for line in fcontent.splitlines(True)]
	...
	if use_sniffer:
		...
		try:
			...
			dialect = sniffer.sniff(
				sample="\n".join(sample_content), delimiters=dontmanage.flags.delimiter_options
			)
		except csv.Error:
	...
	try:
		...
		dontmanage.msgprint(_("Not a valid Comma Separated Value (CSV File)"))
		raise
...
def send_csv_to_client(args):
	...
	dontmanage.response["result"] = cstr(to_csv(args.data))
	dontmanage.response["doctype"] = args.filename
...
def build_csv_response(data, filename):
	dontmanage.response["result"] = cstr(to_csv(data))
	dontmanage.response["doctype"] = filename
	dontmanage.response["type"] = "csv"
...
def check_record(d):
	...
	from dontmanage.utils.dateutils import parse_date
	...
	for key in d:
		...
		if docfield:
			if docfield.reqd and (val == "" or val is None):
				dontmanage.msgprint(_("{0} is required").format(docfield.label), raise_exception=1)
...
def import_doc(d, doctype, overwrite, row_idx, submit=False, ignore_links=False):
	...
	if d.get("name") and dontmanage.db.exists(doctype, d["name"]):
		if overwrite:

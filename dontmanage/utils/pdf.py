...
from dontmanage.utils import cstr, scrub_urls
...
cssutils.log.setLog(dontmanage.logger("cssutils"))
...
def get_pdf(html, options=None, output: PdfWriter | None = None):
	...
	try:
		...
		if any([error in str(e) for error in PDF_CONTENT_ERRORS]):
			if not filedata:
				print(html, options)
				dontmanage.throw(_("PDF generation failed because of broken image links"))
...
def get_chrome_pdf(print_format, html, options, output, pdf_generator=None):
	from dontmanage.utils.pdf_generator.browser import Browser
	from dontmanage.utils.pdf_generator.chrome_pdf_generator import ChromePDFGenerator
	from dontmanage.utils.pdf_generator.pdf_merge import PDFTransformer
...
def prepare_options(html, options):
	...
	# page size
	pdf_page_size = (
		options.get("page-size") or dontmanage.db.get_single_value("Print Settings", "pdf_page_size") or "A4"
	)

	if pdf_page_size == "Custom":
		options["page-height"] = options.get("page-height") or dontmanage.db.get_single_value(
			"Print Settings", "pdf_page_height"
		)
		options["page-width"] = options.get("page-width") or dontmanage.db.get_single_value(
...
def get_cookie_options():
	options = {}
	if dontmanage.session and dontmanage.session.sid and hasattr(dontmanage.local, "request"):
		# Use wkhtmltopdf's cookie-jar feature to set cookies and restrict them to host domain
		cookiejar = f"/tmp/{dontmanage.generate_hash()}.jar"

		# Remove port from request.host
		# https://werkzeug.palletsprojects.com/en/0.16.x/wrappers/#werkzeug.wrappers.BaseRequest.host
		domain = dontmanage.utils.get_host_name().split(":", 1)[0]
		with open(cookiejar, "w") as f:
			f.write(f"sid={dontmanage.session.sid}; Domain={domain};\n")
...
def prepare_header_footer(soup: BeautifulSoup):
	...
	css = dontmanage.read_file(os.path.join(dontmanage.local.sites_path, print_css))
	...
	for html_id in ("header-html", "footer-html"):
		if content := soup.find(id=html_id):
			...
			# create temp file
			fname = os.path.join("/tmp", f"dontmanage-pdf-{dontmanage.generate_hash()}.html")
			with open(fname, "wb") as f:
				f.write(html.encode("utf-8"))

			# {"header-html": "/tmp/dontmanage-pdf-random.html"}
...
def get_host_url():
	if dontmanage.request:
		return dontmanage.request.host_url
	else:
		return get_url() + "/"

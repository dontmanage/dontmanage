...
def get_secret():
	from dontmanage.utils.password import get_encryption_key

	return dontmanage.local.conf.get("secret") or get_encryption_key()


def verify_request():
	"""Verify if the incoming signed request if it is correct."""
	query_string = dontmanage.safe_decode(
		dontmanage.local.flags.signed_query_string or getattr(dontmanage.request, "query_string", None)
	)
	...
	if signature_string in query_string:
		...
		valid_method = dontmanage.request.method == "GET"
		valid_request_data = not (dontmanage.request.form or dontmanage.request.data)
...
def get_url(cmd, params, nonce=None, secret=None):
	...
	return dontmanage.utils.get_url("".join(["api/method/", cmd, "?", urlencode(params)]))


def get_signature(params, nonce, secret=None):
	params = "".join(dontmanage.utils.cstr(p) for p in params.values())
	if not secret:
		secret = dontmanage.local.conf.get("secret") or "secret"

...
from dontmanage.utils import cint, get_file_size, get_url, now, now_datetime
...
class BackupGenerator:
	...
	def __init__(
		...
		self.db_port = self.db_port or dontmanage.db.default_port

		site = dontmanage.local.site or dontmanage.generate_hash(length=8)
	...
	def setup_backup_tables(self):
		...
		existing_tables = dontmanage.db.get_tables()
		...
		specified_tables = get_tables(dontmanage.conf.get("backup", {}).get("includes", []))
		include_tables = (specified_tables + base_tables) if specified_tables else []

		conf_tables = {
			"include": include_tables,
			"exclude": get_tables(dontmanage.conf.get("backup", {}).get("excludes", [])),
		}
	...
	def get_backup(self, older_than=24, ignore_files=False, force=False):
		...
		if not (last_db and last_file and last_private_file and site_config_backup_path):
			...
			if dontmanage.get_system_settings("encrypt_backup"):
	...
	def set_backup_file_name(self):
		...
		enc = "-enc" if dontmanage.get_system_settings("encrypt_backup") else ""
		...
		for path in paths:
			if os.path.exists(path):
				...
				try:
					...
					dontmanage.utils.execute_in_shell(command)
	...
	def get_recent_backup(self, older_than, partial=False):
		...
		if not dontmanage.get_system_settings("encrypt_backup"):
			file_type_slugs = {
	...
	def print_summary(self):
		...
		print(f"Backup Summary for {dontmanage.local.site} at {now()}")
	...
	def backup_files(self):
		for folder in ("public", "private"):
			files_path = dontmanage.get_site_path(folder, "files")
			...
			dontmanage.utils.execute_in_shell(
	...
	def copy_site_config(self):
		...
		site_config_path = os.path.join(dontmanage.get_site_path(), "site_config.json")
	...
	def take_dump(self):
		...
		if not (gzip_exc and db_exc[1]):
			...
			dontmanage.throw(
				f"{_exc} not found in PATH! This is required to take a backup.", exc=dontmanage.ExecutableNotFound
			)
		db_exc = db_exc[0]

		database_header_content = [
			f"Backup generated by DontManage {dontmanage.__version__} on branch {get_app_branch('dontmanage') or 'N/A'}",
			"",
		]

		# escape reserved characters
		args = dontmanage._dict(
			[item[0], dontmanage.utils.esc(str(item[1]), "$ ")] for item in self.__dict__.copy().items()
		)
		...
		if self.partial:
			...
			database_header_content.extend(
				[
					f"Partial Backup of DontManage Site {dontmanage.local.site}",
		...
		if self.db_type == "postgres":
			...
			if self.backup_includes:
				...
				args["exclude"] = " ".join(
					[f"--ignore-table='{dontmanage.conf.db_name}.{table}'" for table in self.backup_excludes]
				)
		...
		dontmanage.utils.execute_in_shell(command, low_priority=True, check_exit_code=True)

	def send_email(self):
		"""
		Sends the link to backup file located at dontmanageerp/backups
		"""
		from dontmanage.email import get_system_managers
...
download only after 24 hours.""".format(
		...
		dontmanage.sendmail(recipients=recipient_list, message=msg, subject=subject)
...
def delete_temp_backups(older_than=24):
	...
	"""
	older_than = cint(dontmanage.conf.keep_backups_for_hours) or older_than
	backup_path = get_backup_path()
...
def get_backup_path():
	backup_path = dontmanage.utils.get_site_path(conf.get("backup_path", "private/backups"))
	return backup_path
...
def get_backup_encryption_key():
	dontmanage.only_for("System Manager")
	return dontmanage.conf.get(BACKUP_ENCRYPTION_CONFIG_KEY)


def get_or_generate_backup_encryption_key():
	from dontmanage.installer import update_site_config

	key = dontmanage.conf.get(BACKUP_ENCRYPTION_CONFIG_KEY)
	if key:
		return key
...
if __name__ == "__main__":
	...
	warn(
		...
		"Use the backup command instead. This script will be removed in DontManage v15.",

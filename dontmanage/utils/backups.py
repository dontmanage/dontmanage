...
class BackupGenerator:
	...
	def _set_existing_tables(self):
		...
		if not hasattr(self, "_existing_tables"):
			self._existing_tables = dontmanage.db.get_tables()
	...
	def set_backup_tables_from_config(self):
		...
		backup_conf = dontmanage.conf.get("backup", {})
		self._set_existing_tables()
	...
	def backup_files(self):
		for folder in ("public", "private"):
			...
			except dontmanage.CommandFailedError as e:
	...
	def take_dump(self):
		if self.db_type == "sqlite":
			...
			db_path = Path(dontmanage.get_site_path()) / "db" / f"{self.db_name}.db"
		...
		if not gzip_exc:
			dontmanage.throw(
				_("gzip not found in PATH! This is required to take a backup."), exc=dontmanage.ExecutableNotFound
			)
		...
		if self.old_backup_metadata:
			...
			database_header_content = [
				"begin dontmanage metadata",
				"[dontmanage]",
				f"version = {dontmanage.__version__}",
				f"branch = {get_app_branch('dontmanage') or 'N/A'}",
				"end dontmanage metadata",
				"",
			]
...
def get_backup_path():
	return dontmanage.utils.get_site_path(conf.get("backup_path", "private/backups"))

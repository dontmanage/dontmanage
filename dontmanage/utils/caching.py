...
def request_cache(func: Callable) -> Callable:
	...
	Cache is stored in `dontmanage.local.request_cache`.
	...
	def wrapper(*args, **kwargs):
		_cache = getattr(dontmanage.local, "request_cache", None)
		if _cache is None:
...
def site_cache(ttl: int | None = None, maxsize: int | None = None) -> Callable:
	...
	The cache is stored in `dontmanage.utils.caching._SITE_CACHE`.
	...
	def time_cache_wrapper(func: Callable | None = None) -> Callable:
		...
		def site_cache_wrapper(*args, **kwargs):
			site = getattr(dontmanage.local, "site", None)
			if not site:
				return func(*args, **kwargs)
...
def redis_cache(ttl: int | None = 3600, user: str | bool | None = None, shared: bool = False) -> Callable:
	...
	def wrapper(func: Callable | None = None) -> Callable:
		...
		def redis_cache_wrapper(*args, **kwargs):
			...
			cached_val = dontmanage.cache.get_value(func_call_key, user=user, shared=shared)
			...
			if dontmanage.cache.exists(func_call_key, user=user, shared=shared):
				return None

			val = func(*args, **kwargs)
			ttl = getattr(func, "ttl", 3600)
			dontmanage.cache.set_value(func_call_key, val, expires_in_sec=ttl, user=user, shared=shared)
			return val
	...
	*,
	...
	def outer(func: Callable) -> Callable:
		...
		def inner(*args, **kwargs):
			...
			if dontmanage.request and dontmanage.request.method == "GET" and qualified_name in dontmanage.request.path:
				dontmanage.local.response_headers.set("Cache-Control", cache_headers)
			return ret
...
def deprecated_local_cache(namespace, key, generator, regenerate_if_none=False):
	if namespace not in dontmanage.local.cache:
		dontmanage.local.cache[namespace] = {}

	if key not in dontmanage.local.cache[namespace]:
		dontmanage.local.cache[namespace][key] = generator()

	elif dontmanage.local.cache[namespace][key] is None and regenerate_if_none:
		# if key exists but the previous result was None
		dontmanage.local.cache[namespace][key] = generator()

	return dontmanage.local.cache[namespace][key]

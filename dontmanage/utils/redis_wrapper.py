...
class RedisWrapper(redis.Redis):
	...
	def make_key(self, key, user=None, shared=False):
		...
		return f"{dontmanage.conf.db_name}|{key}".encode()
	...
	def get_value(self, key, generator=None, user=None, expires=False, shared=False):
		...
		if key in dontmanage.local.cache:
			val = dontmanage.local.cache[key]
	...
	def delete_value(self, keys, user=None, make_keys=True, shared=False):
		...
		for key in keys:
			dontmanage.local.cache.pop(key, None)
	...
	def hget(self, name, key, generator=None, shared=False):
		_name = self.make_key(name, shared=shared)
		if _name not in dontmanage.local.cache:
			dontmanage.local.cache[_name] = {}

		if not key:
			return None

		if key in dontmanage.local.cache[_name]:
			return dontmanage.local.cache[_name][key]
		...
		if value is not None:
			...
			dontmanage.local.cache[_name][key] = value
		elif generator:
			value = generator()
	...
	def hdel(self, name, key, shared=False):
		...
		if _name in dontmanage.local.cache:
			if key in dontmanage.local.cache[_name]:

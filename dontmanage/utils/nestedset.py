...
from dontmanage.query_builder.functions import Coalesce, Max
from dontmanage.query_builder.terms import SubQuery
from dontmanage.query_builder.utils import DocType


class NestedSetRecursionError(dontmanage.ValidationError):
	pass


class NestedSetMultipleRootsError(dontmanage.ValidationError):
	pass


class NestedSetChildExistsError(dontmanage.ValidationError):
	pass


class NestedSetInvalidMergeError(dontmanage.ValidationError):
	pass
...
def update_nsm(doc):
	...
	parent_field = "parent_" + dontmanage.scrub(doc.doctype)
	...
	dontmanage.db.set_value(doc.doctype, doc.name, old_parent_field, parent or "", update_modified=False)
	dontmanage.clear_document_cache(doc.doctype)
...
def update_add_node(doc, parent, parent_field):
	...
	if parent:
		left, right = dontmanage.db.get_value(doctype, {"name": parent}, ["lft", "rgt"], for_update=True)
...
def update_move_node(doc: Document, parent_field: str):
	...
	# move to dark side
	dontmanage.qb.update(Table).set(Table.lft, -Table.lft).set(Table.rgt, -Table.rgt).where(
		(Table.lft >= doc.lft) & (Table.rgt <= doc.rgt)
	).run()

	# shift left
	diff = doc.rgt - doc.lft + 1
	dontmanage.qb.update(Table).set(Table.lft, Table.lft - diff).set(Table.rgt, Table.rgt - diff).where(
		Table.lft > doc.rgt
	).run()

	# shift left rgts of ancestors whose only rgts must shift
	dontmanage.qb.update(Table).set(Table.rgt, Table.rgt - diff).where(
		(Table.lft < doc.lft) & (Table.rgt > doc.rgt)
	).run()
	...
	if parent:
		...
		# set parent lft, rgt
		dontmanage.qb.update(Table).set(Table.rgt, Table.rgt + diff).where(Table.name == parent).run()
		...
		# new root
		max_rgt = dontmanage.qb.from_(Table).select(Max(Table.rgt)).run(pluck=True)[0]
...
def rebuild_tree(doctype: str) -> None:
	...
	if dontmanage.request and dontmanage.local.form_dict.cmd == "rebuild_tree":
	...
	dontmanage.db.auto_commit_on_many_writes = 1
...
def rebuild_node(doctype, parent, left, parent_field):
	...
	result = (dontmanage.qb.from_(table).where(column == parent).select(table.name)).run()
...
def remove_subtree(doctype: str, name: str, throw=True):
	...
	dontmanage.qb.update(table).set(table.lft, table.lft - width).where(table.lft > rgt).run()
	dontmanage.qb.update(table).set(table.rgt, table.rgt - width).where(table.rgt > rgt).run()

	dontmanage.clear_document_cache(doctype)
...
class NestedSet(Document):
	...
	def after_insert(self):
		...
		if dontmanage.db.exists("User Permission", {"user": dontmanage.session.user, "allow": self.doctype}):
			dontmanage.cache.hdel("user_permissions", dontmanage.session.user)
	...
	def validate_if_child_exists(self):
		has_children = dontmanage.db.count(self.doctype, filters={self.nsm_parent_field: self.name})
		if has_children:
			dontmanage.throw(
	...
	def before_rename(self, olddn, newdn, merge=False, group_fname="is_group"):
		if merge and hasattr(self, group_fname):
			is_group = dontmanage.db.get_value(self.doctype, newdn, group_fname)
	...
	def validate_one_root(self):
		if not self.get(self.nsm_parent_field):
			if self.get_root_node_count() > 1:
				dontmanage.throw(_("""Multiple root nodes not allowed."""), NestedSetMultipleRootsError)

	def get_root_node_count(self):
		return dontmanage.db.count(self.doctype, {self.nsm_parent_field: ""})
	...
	def validate_ledger(self, group_identifier="is_group"):
		if hasattr(self, group_identifier) and not bool(self.get(group_identifier)):
			if dontmanage.get_all(self.doctype, {self.nsm_parent_field: self.name, "docstatus": ("!=", 2)}):
				dontmanage.throw(
	...
	def get_parent(self) -> "NestedSet":
		...
		if parent_name:
			return dontmanage.get_doc(self.doctype, parent_name)

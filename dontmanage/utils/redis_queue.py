...
import dontmanage
from dontmanage.utils import get_bench_id, random_string
...
class RedisQueue:
	...
	def add_user(self, username, password=None):
		...
		return dontmanage._dict(user_settings) if is_created else {}

	@classmethod
	def get_connection(cls, username=None, password=None):
		conf = dontmanage.get_site_config()
		if conf.redis_queue_sentinel_enabled:
			from dontmanage.utils.redis_wrapper import get_sentinel_connection

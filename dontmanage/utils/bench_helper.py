...
def DontManageClickWrapper(cls, handler):
	class Cls(cls):
...
def main():
	...
	DontManageClickWrapper(click.Group, handle_exception)(commands=commands)(prog_name="bench")
...
def get_app_groups() -> dict[str, click.Group | click.Command]:
	...
	return dict(
		dontmanage=click.group(
			name="dontmanage", commands=commands, cls=DontManageClickWrapper(click.Group, handle_exception)
		)(app_group)
	)
...
def get_app_group(app: str) -> click.Group:
	if app_commands := get_app_commands(app):
		return click.group(
			name=app, commands=app_commands, cls=DontManageClickWrapper(click.Group, handle_exception)
		)(app_group)
...
def get_sites(site_arg: str) -> list[str]:
	if site_arg == "all":
		...
		return [site_arg]
	elif env_site := os.environ.get("DONTMANAGE_SITE"):
		return [env_site]

...
def get_modules_from_all_apps_for_user(user: str | None = None) -> list[dict]:
	...
	global_blocked_modules = dontmanage.get_cached_doc("User", "Administrator").get_blocked_modules()
	user_blocked_modules = dontmanage.get_cached_doc("User", user).get_blocked_modules()

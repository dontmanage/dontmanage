...
def get_print(
	...
	from dontmanage.website.serve import get_response_without_exception_handling

	local = dontmanage.local
	if "pdf_generator" not in local.form_dict:
		# if arg is passed, use that, else get setting from print format
		if pdf_generator is None:
			pdf_generator = (
				dontmanage.get_cached_value("Print Format", print_format, "pdf_generator") or "wkhtmltopdf"
			)
	...
	if local.form_dict.pdf_generator != "wkhtmltopdf":
		hook_func = dontmanage.get_hooks("pdf_generator")
	...
	for hook in dontmanage.get_hooks("on_print_pdf"):
		dontmanage.call(hook, doctype=doctype, name=name, print_format=print_format)
...
def attach_print(
	...
	dontmanage.local.flags.ignore_print_permissions = True

	with print_language(lang or dontmanage.local.lang):
		content = ""
	...
	dontmanage.local.flags.ignore_print_permissions = False
...
def find_or_download_chromium_executable():
	...
	if chromium_path := shutil.which(dontmanage.get_common_site_config().get("chromium_path", "")):
...
def get_chromium_download_url():
	...
	common_config = dontmanage.get_common_site_config()

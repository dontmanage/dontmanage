...
class PrintFormatGenerator:
	...
	def __init__(self, print_format, doc, letterhead=None):
		...
		"""
		self.base_url = dontmanage.utils.get_url()
		self.print_format = dontmanage.get_doc("Print Format", print_format)
		self.doc = doc

		if letterhead == _("No Letterhead"):
			letterhead = None
		self.letterhead = dontmanage.get_doc("Letter Head", letterhead) if letterhead else None
	...
	def build_context(self):
		self.print_settings = dontmanage.get_doc("Print Settings")
		...
		print_style = (
			dontmanage.get_doc("Print Style", self.print_settings.print_style)
	...
	def get_header_footer_html(self):
		...
		if self.letterhead:
			header_html = dontmanage.render_template("templates/print_format/print_header.html", self.context)
		if self.letterhead:
			footer_html = dontmanage.render_template("templates/print_format/print_footer.html", self.context)
	...
	def get_layout(self, print_format):
		layout = dontmanage.parse_json(print_format.format_data)
	...
	def process_margin_texts(self, layout):
		...
		for key in margin_texts:
			...
			if text and "{{" in text:
				layout["text_" + key] = dontmanage.render_template(text, self.context)

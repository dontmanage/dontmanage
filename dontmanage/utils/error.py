...
def make_error_snapshot(exception):
	if dontmanage.conf.disable_error_snapshot:
		return
	...
	try:
		error_id = "{timestamp:s}-{ip:s}-{hash:s}".format(
			...
			ip=dontmanage.local.request_ip or "127.0.0.1",
			hash=dontmanage.generate_hash(length=3),
		)
		snapshot_folder = get_error_snapshot_path()
		dontmanage.create_folder(snapshot_folder)
		...
		with open(encode(snapshot_file_path), "wb") as error_file:
			error_file.write(encode(dontmanage.as_json(snapshot)))
	...
	try:
		...
		for fname in os.listdir(path):
			...
			for field in ["locals", "exception", "frames"]:
				data[field] = dontmanage.as_json(data[field])

			doc = dontmanage.new_doc("Error Snapshot")
			doc.update(data)
			doc.save()
...
def clear_old_snapshots():
	...
	dontmanage.db.delete(ErrorSnapshot, filters=(ErrorSnapshot.creation < (Now() - Interval(months=1))))
...
def get_error_snapshot_path():
	return dontmanage.get_site_path("error-snapshots")

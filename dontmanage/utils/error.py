...
EXCLUDE_EXCEPTIONS = (
	...
	dontmanage.CSRFTokenError,  # CSRF covers OAuth too
	dontmanage.SecurityException,
	dontmanage.InReadOnlyMode,
)
...
def log_error(title=None, message=None, reference_doctype=None, reference_name=None, *, defer_insert=False):
	...
	from dontmanage.utils.sentry import capture_exception
	...
	traceback = dontmanage.as_unicode(traceback or dontmanage.get_traceback(with_context=True))
	...
	if dontmanage.flags.read_only or defer_insert:
...
def get_error_metadata() -> str:
	...
	try:
		if job := rq.get_current_job():
			...
			metadata["job_name"] = dontmanage.cstr(job.kwargs.get("method"))
			...
			for key in ("method", "path", "referrer"):
				metadata[key] = getattr(dontmanage.local.request, key)
			metadata["form_dict"] = sanitized_dict(dontmanage.form_dict)

		metadata["user"] = getattr(dontmanage.session, "user", "Unidentified")
	except Exception:
		# We don't want to bother with exception handling *while* gathering some error's metadata
		pass
	return dontmanage.as_json(metadata)
...
def log_error_snapshot(exception: Exception):
	...
	logger = dontmanage.logger(with_more_info=True)
	...
	def decorator_raise_error_on_no_output(func):
		...
		def wrapper_raise_error_on_no_output(*args, **kwargs):
			...
			if (not response) and raise_error:
				dontmanage.throw(error_message, error_type or Exception)
			return response
...
def guess_exception_source(exception: str) -> str | None:
	...
	- For dontmanage.throws the exception source is possibly present after skipping dontmanage.throw frames
	...
	with suppress(Exception):
		...
		if (probably_source := apps.most_common(1)) and probably_source[0][0] != "dontmanage":

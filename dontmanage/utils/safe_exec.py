...
from dontmanage.locale import get_date_format, get_number_format, get_time_format
...
from dontmanage.utils.inplacevar import protected_inplacevar
...
class DontManagePrintCollector(PrintCollector):
	"""Collect written text, and return it when called."""

	def _call_print(self, *objects, **kwargs):
		output = io.StringIO()
		print(*objects, file=output, **kwargs)
		dontmanage.log(output.getvalue().strip())
		output.close()


def is_safe_exec_enabled() -> bool:
	# server scripts can only be enabled via common_site_config.json
	return bool(dontmanage.get_common_site_config(cached=True).get(SAFE_EXEC_CONFIG_KEY))
...
def safe_exec(
	...
	*,
	...
	if not is_safe_exec_enabled():
		...
		msg += f"<br><a href='https://dontmanageframework.com/docs/user/en/desk/scripting/server-script' target='_blank' rel='noopener noreferrer'>{docs_cta}</a>"
...
def _compile_code(script: str, filename: str, mode: str = "exec"):
	return compile_restricted(script, filename=filename, policy=DontManageTransformer, mode=mode)
...
@contextmanager
def safe_exec_flags():
	if dontmanage.flags.in_safe_exec is None:
...
def get_safe_globals():
	...
	out = NamespaceDict(
		...
		dontmanage=NamespaceDict(
			...
			if getattr(dontmanage.local, "session", None) and getattr(dontmanage.local.session, "data", None)
			else "Guest",
			...
			website=NamespaceDict(
				abs_url=dontmanage.website.utils.abs_url,
				extract_title=dontmanage.website.utils.extract_title,
				get_boot_data=dontmanage.website.utils.get_boot_data,
				get_home_page=dontmanage.website.utils.get_home_page,
				get_html_content_based_on_type=dontmanage.website.utils.get_html_content_based_on_type,
			),
		...
		html2text=html2text,
		dev_server=dontmanage._dev_server,
		run_script=run_script,
		is_job_queued=is_job_queued,
		get_visible_columns=get_visible_columns,
	)

	out.dontmanage.update(SAFE_EXCEPTIONS)
	...
	out._print_ = DontManagePrintCollector
...
def get_hooks(hook: str | None = None, default=None, app_name: str | None = None) -> dontmanage._dict:
...
SAFE_DATA_UTILS = {key: dontmanage.utils.data.__dict__[key] for key in VALID_UTILS}
...
SAFE_EXCEPTIONS = get_module_properties(
	dontmanage.exceptions, lambda obj: inspect.isclass(obj) and issubclass(obj, Exception)
)

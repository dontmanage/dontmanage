...
from dontmanage.locale import get_date_format, get_first_day_of_the_week, get_number_format, get_time_format
from dontmanage.types.filter import Filters, FilterSignature, FilterTuple
from dontmanage.utils.deprecations import deprecated
from dontmanage.utils.number_format import NUMBER_FORMAT_MAP, NumberFormat
...
# these options are necessary to use orjson with dontmanage
#
...
# dontmanage converts datetime objects differently (__str__) from orjson (RFC 3339)
#
...
def getdate(
	...
	try:
		...
		try:
			...
			dontmanage.throw(
				dontmanage._("{} is not a valid date string.").format(dontmanage.bold(string_date)),
				title=dontmanage._("Invalid Date"),
			)
...
def add_to_date(
	...
	if isinstance(date, str):
		...
		try:
			...
			dontmanage.throw(dontmanage._("Please select a valid date filter"), title=dontmanage._("Invalid Date"))
...
def now() -> str:
	...
	if dontmanage.flags.current_date:
		return (
			getdate(dontmanage.flags.current_date).strftime(DATE_FORMAT)
			+ " "
...
def get_user_date_format() -> str:
	...
	if getattr(dontmanage.local, "user_date_format", None) is None:
		dontmanage.local.user_date_format = get_date_format()

	return dontmanage.local.user_date_format
...
def get_user_time_format() -> str:
	...
	if getattr(dontmanage.local, "user_time_format", None) is None:
		dontmanage.local.user_time_format = get_time_format()

	return dontmanage.local.user_time_format
	...
	* dd/mm/yyyy
	...
	try:
		formatted_date = babel.dates.format_date(
			date, format_string, locale=(dontmanage.local.lang or "").replace("-", "_")
		)
	...
	* HH:mm
	...
	try:
		formatted_time = babel.dates.format_time(
			time_, format_string, locale=(dontmanage.local.lang or "").replace("-", "_")
		)
	except (UnknownLocaleError, ValueError):
	...
	* mm-dd-yyyy HH:mm
	...
	try:
		formatted_datetime = babel.dates.format_datetime(
			datetime, format_string, locale=(dontmanage.local.lang or "").replace("-", "_")
		)
	except (UnknownLocaleError, ValueError):
...
def cast_fieldtype(fieldtype, value, show_warning=True):
	if show_warning:
		message = (
			"Function `dontmanage.utils.data.cast_fieldtype` has been deprecated in favour"
			" of `dontmanage.utils.data.cast`. Use the newer util for safer type casting."
		)
...
def cast(fieldtype, value=None):
	"""Cast the value to the Python native object of the DontManage fieldtype provided.
	...
	Mapping of Python types => DontManage types:
...
def flt(
	...
	try:
		...
		if isinstance(e, dontmanage.InvalidRoundingMethod):
			raise
		num = 0.0
...
def cstr(s, encoding="utf-8") -> str:
	...
	return dontmanage.as_unicode(s, encoding)
...
def rounded(num, precision=0, rounding_method=None):
	...
	rounding_method = (
		rounding_method or dontmanage.get_system_settings("rounding_method") or "Banker's Rounding (legacy)"
	)
	...
	if rounding_method == "Banker's Rounding":
		...
		dontmanage.throw(
			dontmanage._("Unknown Rounding Method: {}").format(rounding_method),
			exc=dontmanage.InvalidRoundingMethod,
		)
...
def round_based_on_smallest_currency_fraction(value, currency, precision=2):
	smallest_currency_fraction_value = flt(
		dontmanage.db.get_value("Currency", currency, "smallest_currency_fraction_value", cache=True)
	)
...
def fmt_money(
	...
	if precision is None:
		precision = cint(dontmanage.db.get_default("currency_precision")) or None
	...
	if number_format.decimal_separator:
		...
		if precision > 2:
			if len(decimals) < 3:
				if currency:
					fraction = dontmanage.db.get_value("Currency", currency, "fraction_units", cache=True) or 100
	...
	if currency and dontmanage.defaults.get_global_default("hide_currency_symbol") != "Yes":
		symbol = dontmanage.db.get_value("Currency", currency, "symbol", cache=True) or currency
		symbol_on_right = dontmanage.db.get_value("Currency", currency, "symbol_on_right", cache=True)

		if symbol_on_right:
			amount = f"{amount} {dontmanage._(symbol)}"
		else:
			amount = f"{dontmanage._(symbol)} {amount}"

	return amount


from dontmanage.deprecation_dumpster import get_number_format_info


def __getattr__(name):
	if name == "number_format_info":
		from dontmanage.deprecation_dumpster import deprecation_warning
		from dontmanage.utils.number_format import NUMBER_FORMAT_MAP

		deprecation_warning(
			"unkown",
			"v16",
			"use dontmanage.utils.number_format.NUMBER_FORMAT_MAP instead of dontmanage.utils.data.number_format_info",
		)
	else:
		try:
			globals()[name]
...
def money_in_words(
	...
	from dontmanage.utils import get_defaults
	...
	if not fraction_currency:
		fraction_currency = dontmanage.db.get_value("Currency", main_currency, "fraction", cache=True)

	number_format = get_number_format()

	fraction_units = dontmanage.db.get_value("Currency", main_currency, "fraction_units", cache=True)
	if fraction_units:
...
def in_words(integer: int, in_million=True) -> str:
	...
	locale = "en_IN" if not in_million else dontmanage.local.lang
	integer = int(integer)
	try:
...
def get_thumbnail_base64_for_image(src: str) -> dict[str, str] | None:
	...
	from dontmanage import cache, safe_decode
	from dontmanage.core.doctype.file.utils import get_local_image

	if not src:
		dontmanage.throw(f"Invalid source for image: {src}")
	...
	def _get_base64():
		file_path = dontmanage.get_site_path("public", src.lstrip("/"))
...
def comma_or(some_list: list | tuple, add_quotes=True) -> str:
	...
	try:
		locale = Locale.parse(dontmanage.local.lang, sep="-")
		pattern = locale.list_patterns["or"]["end"]
	except Exception:
		pattern = dontmanage._("{0} or {1}")
...
def comma_sep(some_list: list | tuple, pattern: str, add_quotes=True) -> str:
	...
	if isinstance(some_list, list | tuple):
		...
		if not some_list:
			...
			return pattern.format(", ".join(dontmanage._(s) for s in some_list[:-1]), some_list[-1])
	else:
		return some_list
...
def get_url(
	...
	host_name = dontmanage.local.conf.host_name or dontmanage.local.conf.hostname
	...
	if not host_name:
		...
		elif dontmanage.local.site:
			protocol = "http://"

			if dontmanage.local.conf.ssl_certificate:
				protocol = "https://"

			elif dontmanage.local.conf.wildcard:
				domain = dontmanage.local.conf.wildcard.get("domain")
				if (
					domain
					and dontmanage.local.site.endswith(domain)
					and dontmanage.local.conf.wildcard.get("ssl_certificate")
				):
					protocol = "https://"

			host_name = protocol + dontmanage.local.site
	...
	if not uri and full_address:
		uri = dontmanage.get_request_header("REQUEST_URI", "")

	port = dontmanage.conf.http_port
	if not port and dontmanage.conf.developer_mode:
		port = dontmanage.conf.webserver_port

	if (
		# XXX: This config is used as proxy for "is production mode enabled?"
		not dontmanage.conf.restart_supervisor_on_update
		and not dontmanage.conf.restart_systemd_on_update
		and host_name
...
def get_host_name() -> str:
	...
	e.g. If site is 'https://cloud.dontmanage.io', returns 'cloud.dontmanage.io'.
	"""
...
def get_link_to_form(doctype: str, name: str | None = None, label: str | None = None) -> str:
	...
	e.g. get_link_to_form("Sales Invoice", "INV-0001", "Link Label") returns:
	    '<a href="https://dontmanage.io/desk/sales-invoice/INV-0001">Link Label</a>'.
	"""
...
def get_link_to_report(
	...
	e.g. get_link_to_report("Revenue Report", "Link Label") returns:
	        '<a href="https://dontmanage.io/desk/query-report/Revenue%20Report">Link Label</a>'.
	"""
	from dontmanage import _
...
def get_url_to_form(doctype: str, name: str | None = None) -> str:
	...
	e.g. when doctype="Sales Invoice" and your site URL is "https://dontmanage.io",
	         returns 'https://dontmanage.io/desk/sales-invoice/INV-00001'
	"""
	if not name:
...
def get_url_to_list(doctype: str) -> str:
	...
	e.g. when doctype="Sales Invoice" and your site URL is "https://dontmanage.io",
	         returns 'https://dontmanage.io/desk/sales-invoice'
	"""
...
def get_url_to_report(name, report_type: str | None = None, doctype: str | None = None) -> str:
	...
	e.g. when name="Sales Register" and your site URL is "https://dontmanage.io",
	         returns 'https://dontmanage.io/desk/query-report/Sales%20Register'

	You can optionally pass `report_type` and `doctype` to get the URL for a Report Builder report.

	get_url_to_report("Revenue", "Report Builder", "Sales Invoice") -> 'https://dontmanage.io/desk/sales-invoice/view/report/Revenue'
	"""
...
def filter_operator_is(value: str | None, pattern: str) -> bool:
	...
	if pattern == "set":
		...
		dontmanage.throw(dontmanage._(f"Invalid argument for operator 'IS': {pattern}"))
...
def get_filter(doctype: str, filters: FilterSignature, filters_config=None) -> "dontmanage._dict":
	"""Return a `_dict` like:
	...
	f = dontmanage._dict(doctype=ft[0], fieldname=ft[1], operator=ft[2], value=ft[3])
	...
	if f.operator.lower() not in valid_operators:
		dontmanage.throw(dontmanage._("Operator must be one of {0}").format(", ".join(valid_operators)))
...
def sanitize_column(column_name: str) -> str:
	return _sanitize_column(column_name, (dontmanage.db and dontmanage.db.db_type) or None)
...
def _sanitize_column(column_name: str, db_type: str) -> str:
	...
	def _raise_exception():
		dontmanage.throw(_("Invalid field name {0}").format(column_name), dontmanage.DataError)
...
def scrub_urls(html: str) -> str:
	...
	e.g. If HTML is '<a href="/files/abc.jpeg">View Image</a>' and site URL is 'https://dontmanage.io',
	        returns '<a href="https://dontmanage.io/files/abc.jpeg">View Image</a>'.
	"""
...
def quoted(url: str) -> str:
	...
	e.g. 'https://dontmanage.io/files/my Image file.jpeg' -> 'https://dontmanage.io/files/my%20Image%20file.jpeg'
	"""
...
def get_user_info_for_avatar(user_id: str) -> _UserInfo:
	...
	e.g. {
	        "email": "faris@dontmanage.io",
	        "image": "/assets/dontmanage/images/ui/avatar.png",
	        "name": "Faris Ansari"
	}
	"""
	try:
		user = dontmanage.get_cached_doc("User", user_id)
...
def validate_python_code(string: str, fieldname: str | None = None, is_expression: bool = True) -> None:
	...
	try:
		...
		msg = dontmanage._("{} Invalid python code on line {}").format(
			fieldname + ":" if fieldname else "", line_no + 1
		)
		msg += f"<br><pre>{error_line}</pre>"
		msg += f"<pre>{' ' * offset}^</pre>"

		dontmanage.throw(msg, title=dontmanage._("Syntax Error"))
	except Exception as e:
		dontmanage.msgprint(
			dontmanage._("{} Possibly invalid python code. <br>{}").format(fieldname + ": " or "", str(e)),
			indicator="orange",
		)
...
def format_timedelta(o: datetime.timedelta | str) -> str:
	...
	# but DontManage doesn't - I think via babel : only allows 0..23 range for hour
...
def mock(type, size=1, locale="en"):
	...
	from dontmanage.utils import squashify
...
def add_trackers_to_url(
	...
	def get_utm_values():
		_source = dontmanage.db.get_value("UTM Source", source, "slug") or source
		_campaign = dontmanage.db.get_value("UTM Campaign", campaign, "slug") or campaign
		_medium = dontmanage.db.get_value("UTM Medium", medium, "slug") or medium
	...
	source, medium, campaign = dontmanage.cache.get_value(
...
def map_trackers(url_trackers: dict, create: bool = False):
	...
	if url_source := url_trackers.get("utm_source", url_trackers.get("source")):
		source = dontmanage.db.get_value("UTM Source", {"slug": slug(url_source)}, "name") or url_source
		if create and source == url_source and not dontmanage.db.exists("UTM Source", source):
			source = dontmanage.new_doc("UTM Source")
			source.name = url_source
			source.description = f"Autogenerated from {url_trackers}"
			source.save(ignore_permissions=True)
		dontmanage_trackers["utm_source"] = source

	if url_medium := url_trackers.get("utm_medium", url_trackers.get("medium")):
		medium = dontmanage.db.get_value("UTM Medium", {"slug": slug(url_medium)}, "name") or url_medium
		if create and medium == url_medium and not dontmanage.db.exists("UTM Medium", medium):
			medium = dontmanage.new_doc("UTM Medium")
			medium.name = url_medium
			medium.description = f"Autogenerated from {url_trackers}"
			medium.save(ignore_permissions=True)
		dontmanage_trackers["utm_medium"] = medium

	if url_campaign := url_trackers.get("utm_campaign", url_trackers.get("campaign")):
		campaign = dontmanage.db.get_value("UTM Campaign", {"slug": slug(url_campaign)}, "name") or url_campaign
		if create and campaign == url_campaign and not dontmanage.db.exists("UTM Campaign", campaign):
			campaign = dontmanage.new_doc("UTM Campaign")
			campaign.name = url_campaign
			campaign.campaign_description = f"Autogenerated from {url_trackers}"
			campaign.save(ignore_permissions=True)
		dontmanage_trackers["utm_campaign"] = campaign

	if url_content := url_trackers.get("utm_content", url_trackers.get("content")):
		dontmanage_trackers["utm_content"] = url_content

	return dontmanage_trackers

...
def get_first_day_of_the_week() -> str:
	return dontmanage.get_system_settings("first_day_of_the_week") or "Sunday"
...
def get_user_date_format() -> str:
	...
	if getattr(dontmanage.local, "user_date_format", None) is None:
		dontmanage.local.user_date_format = dontmanage.db.get_default("date_format")

	return dontmanage.local.user_date_format or "yyyy-mm-dd"
...
def get_user_time_format() -> str:
	...
	if getattr(dontmanage.local, "user_time_format", None) is None:
		dontmanage.local.user_time_format = dontmanage.db.get_default("time_format")
...
def fmt_money(
	...
	"""
	number_format = format or dontmanage.db.get_default("number_format") or "#,###.##"
...
def money_in_words(
	...
	if not fraction_currency:
		fraction_currency = dontmanage.db.get_value("Currency", main_currency, "fraction", cache=True) or _(
			"Cent"
		)

	number_format = (
		dontmanage.db.get_value("Currency", main_currency, "number_format", cache=True)
		or dontmanage.db.get_default("number_format")
		or "#,###.##"
	)
...
def comma_or(some_list, add_quotes=True):
	return comma_sep(some_list, dontmanage._("{0} or {1}"), add_quotes)


def comma_and(some_list, add_quotes=True):
	return comma_sep(some_list, dontmanage._("{0} and {1}"), add_quotes)
...
def get_url(uri: str | None = None, full_address: bool = False) -> str:
	...
	port = dontmanage.conf.http_port or dontmanage.conf.webserver_port

	if (
		not (dontmanage.conf.restart_supervisor_on_update or dontmanage.conf.restart_systemd_on_update)
		and host_name
...
def get_filter(doctype: str, f: dict | list | tuple, filters_config=None) -> "dontmanage._dict":
	...
	if len(f) == 3:
		...
		dontmanage.throw(
			dontmanage._("Filter must have 4 values (doctype, fieldname, operator, value): {0}").format(str(f))
		)

	f = dontmanage._dict(doctype=f[0], fieldname=f[1], operator=f[2], value=f[3])
...
def sanitize_column(column_name: str) -> None:
	...
	if dontmanage.db and dontmanage.db.db_type == "mariadb":
...
def format_timedelta(o: datetime.timedelta) -> str:
	...
	# but dontmanage doesnt - i think via babel : only allows 0..23 range for hour

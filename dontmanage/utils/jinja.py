...
def get_jenv():
	...
	if jenv := getattr(dontmanage.local, "jenv", None):
		return jenv
...
def _get_jenv():
	...
	from dontmanage.utils.safe_exec import UNSAFE_ATTRIBUTES
	...
	jenv = DontManageSandboxedEnvironment(loader=get_jloader(), undefined=DebugUndefined, cache_size=32)
	set_filters(jenv)
...
def validate_template(html):
	...
	try:
		...
		dontmanage.throw(f"Syntax error in template as line {e.lineno}: {e.message}")
...
def get_jloader():
	...
	dontmanage.local.jloader = jloader  # backward compat
	return jloader
...
def _get_jloader():
	...
	if not apps:
		apps = list(reversed(dontmanage.get_installed_apps(_ensure_on_bench=True)))

...
import dontmanage
from dontmanage.model.meta import get_field_currency, get_field_precision
...
def format_value(value, df=None, doc=None, currency=None, translated=False, format=None):
	...
	if isinstance(df, str):
		df = dontmanage._dict(fieldtype=df)
	...
	if not df:
		...
		default_currency = dontmanage.db.get_default("currency")
		...
		if not BLOCK_TAGS_PATTERN.search(value):
			return dontmanage.safe_decode(value).replace("\n", "<br>")

	elif df.get("fieldtype") == "Markdown Editor":
		return dontmanage.utils.markdown(value)

	elif df.get("fieldtype") == "Table MultiSelect":
		values = []
		meta = dontmanage.get_meta(df.options)
		link_field = next(df for df in meta.fields if df.fieldtype == "Link")
		for v in value:
			v.update({"__link_titles": doc.get("__link_titles")})
			formatted_value = dontmanage.format_value(v.get(link_field.fieldname, ""), link_field, v)
		...
		if isinstance(value, str):
			return dontmanage._(value, context=df.parent or "")

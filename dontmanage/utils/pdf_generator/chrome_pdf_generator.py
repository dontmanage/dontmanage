...
from dontmanage.utils.print_utils import find_or_download_chromium_executable
...
class ChromePDFGenerator:
	...
	def _initialize_chromium(self):
		...
		site_config = dontmanage.get_common_site_config()
		...
		if self.CHROMIUM_WEBSOCKET_URL:
			dontmanage.warn("Using external chromium websocket url. Make sure it is accessible.")
	...
	def _verify_chromium_installation(self):
		...
		if not os.access(self._chromium_path, os.X_OK):
			dontmanage.throw(f"Chromium not executable at {self._chromium_path}")
		return True
	...
	def start_chromium_process(self, debug=False):
		...
		try:
			if debug:
				command_args = [
					...
					"--user-data-dir=/tmp/chromium-{}-user-data".format(
						dontmanage.local.site + dontmanage.utils.random_string(10)
					),
					"--disable-gpu",
					"--no-sandbox",
					"--no-first-run",
			...
			dontmanage.log_error(f"Error starting Chromium: {e}")
			dontmanage.throw(_("Could not start Chromium. Check logs for details."))
	...
	def _close_browser(self):
		...
		"""
		if self._browsers:
			dontmanage.log("Cannot close Chromium as there are active browser instances.")
			return
		if self._chromium_process:
			self._chromium_process.terminate()
		...
		self._chromium_process = None
		self._devtools_url = None
		dontmanage.log("Headless Chromium closed successfully.")
	...
	def fetch_devtools_url(self, port):
		...
		try:
			...
			dontmanage.log_error(f"An error occurred: {e}")
		return None

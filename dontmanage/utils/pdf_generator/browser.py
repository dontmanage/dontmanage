...
import dontmanage
from dontmanage.utils.pdf import get_host_url
from dontmanage.utils.print_utils import convert_uom, parse_float_and_unit


class Browser:
	def __init__(self, generator, print_format, html, options):
		self.is_print_designer = dontmanage.get_cached_value("Print Format", print_format, "print_designer")
		self.browserID = dontmanage.utils.random_string(10)
	...
	def open(self, generator):
		from dontmanage.utils.pdf_generator.cdp_connection import CDPSocketClient
	...
	def create_browser_context(self):
		...
		if error:
			dontmanage.log_error(title="Error creating browser context:", message=f"{error}")
	...
	def new_page(self, page_type):
		...
		from dontmanage.utils.pdf_generator.page import Page
	...
	def _parse_pdf_options_from_html(self):
		from dontmanage.utils.pdf import get_print_format_styles
	...
	def prepare_options_for_pdf(self):
		...
		if not options.get("page-height") or not options.get("page-width"):
			if not (page_size := self.options.get("page-size")):
				raise dontmanage.ValidationError("Page size is required")
			if page_size == "CUSTOM":
				raise dontmanage.ValidationError("Custom page size requires page-height and page-width")
			size = PageSize.get(page_size)
			if not size:
				raise dontmanage.ValidationError("Invalid page size")
	...
	def get_rendered_header_footer(self, content, type, head, styles, css):
		from dontmanage.utils.pdf import toggle_visible_pdf

		html_id = f"{type}-html"
		content = content.extract()
		toggle_visible_pdf(content)
		id_map = {"header": "pdf_header_html", "footer": "pdf_footer_html"}
		hook_func = dontmanage.get_hooks(id_map.get(type))

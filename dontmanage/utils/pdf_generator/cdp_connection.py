...
class CDPSocketClient:
	...
	async def _connect(self):
		try:
			...
			dontmanage.log_error(title="Failed to connect to WebSocket:", message=f"{dontmanage.get_traceback()}")
			raise

	async def _listen(self):
		try:
			async for message in self.connection:
				self._handle_message(dontmanage.json.loads(message))
		except Exception:
			dontmanage.log_error(title="WebSocket listening error:", message=f"{dontmanage.get_traceback()}")
	...
	def disconnect(self):
		try:
			...
			dontmanage.log_error(title="Error while disconnecting:", message=f"{dontmanage.get_traceback()}")
			raise
	...
	async def _disconnect(self):
		try:
			...
			dontmanage.log_error(
				title="Error during WebSocket disconnection:", message=f"{dontmanage.get_traceback()}"
			)
	...
	async def _send(self, method, params=None, session_id=None, wait_future_fulfill=True):
		...
		await self.connection.send(dontmanage.json.dumps(message))
	...
	def wait_for_event(self, event, timeout=3):
		...
		try:
			...
			dontmanage.log_error(title="Timeout waiting for event", message=f"{dontmanage.get_traceback()}")

...
class Page:
	def __init__(self, session, browser_context_id, page_type):
		...
		if error:
			dontmanage.log_error(title="Error creating new page:", message=f"{error}")
	...
	def get_frame_id_on_demand(self):
		...
		try:
			...
			dontmanage.log_error(title="Error fetching frameId:", message=f"{dontmanage.get_traceback()}")
			raise
				...
				if url.startswith(get_host_url()):
					...
					if path.startswith("assets/") or path.startswith("files/"):
						...
						if path.startswith("files/"):
							path = dontmanage.utils.get_site_path("public", path)
						content = dontmanage.read_file(path, as_base64=True)
						response_headers = []

...
def is_enabled() -> bool:
	return bool(
		not dontmanage.conf.get("developer_mode", 0)
		and dontmanage.conf.get("pulse_api_key")
		and on_dontmanagecloud()
		...
		eq.add(
			{
				...
				"site": site or dontmanage.local.site,
				"properties": properties,
			},
			interval=interval,
		)
	except Exception as e:
		dontmanage.logger("pulse").error(f"pulse-client - capture failed: {e!s}")
...
def bulk_capture(events):
	...
	if isinstance(events, str):
		events = dontmanage.parse_json(events)
	...
	for event in events:
		capture(
			...
			site=event.get("site"),
			app=event.get("app"),
			user=event.get("user") or dontmanage.session.user,
...
def post(events):
	...
	url = _get_ingest_url()
	data = dontmanage.as_json({"events": events})
	resp = session.post(url, data=data, timeout=15)
	if not (200 <= resp.status_code < 300):
		msg = f"pulse-client - post failed: {resp.status_code} {resp.text}"
		dontmanage.logger("pulse").error(msg)
		raise Exception(msg)
	return resp


def _create_session():
	api_key = dontmanage.conf.get("pulse_api_key")
...
def _get_ingest_url():
	host = dontmanage.conf.get("pulse_host") or "https://pulse.m.dontmanage.cloud"
	host = ensure_http(host)
	host = host.rstrip("/")

	endpoint = dontmanage.conf.get("pulse_ingest_endpoint") or "/api/method/pulse.api.bulk_ingest"
...
class EventQueue:
	...
	@property
	def length(self):
		return dontmanage.cache.llen(self.queue)
	...
	def _is_ratelimited(self, event, interval):
		...
		last_sent = dontmanage.cache.get_value(last_sent_key)
	...
	def _update_ratelimit(self, event, interval):
		...
		event_key = self._get_event_key(event)
		last_sent_key = f"{self.ratelimit_prefix}{event_key}"
		dontmanage.cache.set_value(last_sent_key, time.monotonic())

	def _queue_event(self, event):
		dontmanage.cache.lpush(self.queue, dontmanage.as_json(event))
		dontmanage.cache.ltrim(self.queue, 0, self.queue_size - 1)
		...
		for _ in range(max_batches):
			...
			try:
				...
				if retry_attempts > max_retries:
					...
					dontmanage.logger("pulse").error(f"pulse-client - max retries reached: {e!s}")
					self._requeue_events(events)
					break

				pending_events = events
				time.sleep(backoff_seconds * (2 ** (retry_attempts - 1)))
				dontmanage.logger("pulse").error(f"pulse-client - retrying batch due to error: {e!s}")

	def collect(self, batch_size=100):
		events = []
		for _ in range(batch_size):
			event_json = dontmanage.cache.rpop(self.queue)
			if not event_json:
				break
	...
	def _requeue_events(self, events):
		...
		for event in reversed(events):
			dontmanage.cache.rpush(self.queue, dontmanage.as_json(event))
		dontmanage.cache.ltrim(self.queue, 0, self.queue_size - 1)

	def _decode_event(self, event_json):
		event_json = event_json.decode()
		with suppress(JSONDecodeError):
			return dontmanage.parse_json(event_json)

	def get_events(self, limit=20):
		events = []
		for _ in range(limit):
			event_json = dontmanage.cache.lindex(self.queue, _)
			if not event_json:
				break
			data = self._decode_event(event_json)
			if data:
	...
	def get_last_sent_events(self, limit=20):
		events = []
		keys = dontmanage.cache.get_keys(f"{self.ratelimit_prefix}*")[:limit]
		for key in keys:
			last_sent = dontmanage.cache.get_value(key)

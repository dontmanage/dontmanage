...
from dontmanage.utils.telemetry.pulse.client import EventQueue, capture, is_enabled
from dontmanage.utils.telemetry.pulse.utils import anonymize_user, parse_interval
...
class TestPulseClient(IntegrationTestCase):
	def setUp(self):
		...
		dontmanage.cache.delete_keys("pulse-client:")
...
class TestCapture(TestPulseClient):
	@patch("dontmanage.utils.telemetry.pulse.client.is_enabled")
...
class TestUtils(TestPulseClient):
	...
	def test_anonymize_user(self):
		...
		for standard_user in dontmanage.STANDARD_USERS:
...
class TestEventQueueDecoding(TestPulseClient):
	def test_decode_valid_event(self):
		...
		# Add and retrieve
		eq.add(event)
		event_json = dontmanage.cache.rpop(eq.queue)

...
from dontmanage.permissions import AUTOMATIC_ROLES, get_rights, get_roles, get_valid_perms
...
class UserPermissions:
	"""
	A user permission object can be accessed as `dontmanage.get_user()`
	"""

	def __init__(self, name=""):
		self.defaults = None
		self.name = name or dontmanage.session.get("user")
		self.roles = []
	...
	def setup_user(self):
		def get_user_doc():
			user = None
			try:
				user = dontmanage.get_doc("User", self.name).as_dict()
			except dontmanage.DoesNotExistError:
				pass
			except Exception as e:
				# install boo-boo
				if not dontmanage.db.is_table_missing(e):
					raise

			return user

		if not dontmanage.flags.in_install_db and not dontmanage.in_test:
	...
	def build_doctype_map(self):
		...
		all_doctypes = dontmanage.get_all(
			"DocType",
			fields=[
				"name",
				"in_create",
	...
	def build_permissions(self):
		...
		self.build_perm_map()
		user_shared = dontmanage.share.get_shared_doctypes()
		no_list_view_link = []
		...
		self.shared = dontmanage.get_all(
			"DocShare",
		...
		if "System Manager" in self.get_roles():
			self.can_import += dontmanage.get_all("DocType", {"allow_import": 1}, pluck="name")
			self.can_import += dontmanage.get_all(
				"Property Setter",
				pluck="doc_type",
	...
	def get_defaults(self):
		...
		self.defaults = dontmanage.defaults.get_defaults(self.name)
	...
	def load_user(self):
		...
		if d.get("default_workspace"):
			workspace = dontmanage.get_cached_doc("Workspace", d.default_workspace)
			d.default_workspace = {
				"name": workspace.name,
...
def get_fullname_and_avatar(user: str) -> _dict:
	first_name, last_name, avatar, name = dontmanage.get_cached_value(
...
def get_system_managers(only_name: bool = False) -> list[str]:
	...
	system_managers = (
		...
		.where(
			...
			& (User.docstatus < 2)
			& (User.name.notin(dontmanage.STANDARD_USERS))
		)
		.select(*fields)
...
def add_role(user: str, role: str) -> None:
	dontmanage.get_doc("User", user).add_roles(role)
...
def is_website_user(username: str | None = None) -> str | None:
	return dontmanage.get_cached_value("User", username or dontmanage.session.user, "user_type") == "Website User"
...
def is_system_user(username: str | None = None) -> str | None:
	...
	return dontmanage.db.get_value(
		"User",
		{
			"email": username or dontmanage.session.user,
			"enabled": 1,
			"user_type": "System User",
		},
		cache=True,
...
def is_portal_user():
	...
	if user_type == "Website User" and has_common(dontmanage.get_roles(), roles):
		return True


def get_portal_roles():
	roles = []
	for menu_item in dontmanage.get_single("Portal Settings").menu:

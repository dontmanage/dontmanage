...
def inspect(var, render=True):
	from dontmanage.utils.jinja import get_jenv
...
def web_blocks(blocks):
	import dontmanage
	from dontmanage import _, _dict, throw
	from dontmanage.website.doctype.web_page.web_page import get_web_blocks_html
	...
	if not dontmanage.flags.web_block_scripts:
		dontmanage.flags.web_block_scripts = {}
...
def include_script(path, preload=True):
	...
	if preload:
		...
		dontmanage.local.preload_assets["script"].append(path)
...
def include_icons(path, preload=True):
	...
	if preload:
		...
		dontmanage.local.preload_assets["icons"].append(path)
...
def include_style(path, rtl=None, preload=True):
	...
	if preload:
		...
		dontmanage.local.preload_assets["style"].append(path)

	return f'<link type="text/css" rel="stylesheet" href="{path}">'


def bundled_asset(path, rtl=None):
	from dontmanage.utils import get_assets_json
	from dontmanage.website.utils import abs_url

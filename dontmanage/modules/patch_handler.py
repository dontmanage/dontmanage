...
def run_all(skip_failing: bool = False, patch_type: PatchType | None = None) -> None:
	...
	for patch in dontmanage.flags.final_patches:
...
def reload_doc(args):
	...
	run_single(method=dontmanage.modules.reload_doc, methodargs=args)
...
def execute_patch(patchmodule: str, method=None, methodargs=None):
	...
	if patchmodule.startswith("execute:"):
		...
		_patch = dontmanage.get_attr(patch)
	...
	try:
		if patchmodule:
			if patchmodule.startswith("finally:"):
				...
				dontmanage.flags.final_patches.append(patchmodule)
			else:
				if has_patch_file:
					_patch()
				else:

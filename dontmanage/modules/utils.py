...
def export_customizations(
	...
	custom = {
		...
		"custom_perms": [],
		"links": dontmanage.get_all("DocType Link", fields="*", filters={"parent": doctype}, order_by="name"),
		"doctype": doctype,
...
def sync_customizations(app=None):
	...
	for app_name in apps:
		for module_name in dontmanage.local.app_modules.get(app_name) or []:
			folder = dontmanage.get_app_path(app_name, module_name, "custom")
			if os.path.exists(folder):
				for fname in os.listdir(folder):
					if fname.endswith(".json"):
						...
						elif dontmanage.flags.in_install and app:
							sync_customizations_for_doctype(data, folder, fname)
		...
		def sync_single_doctype(doc_type):
			def _insert(data):
				if data.get(doctype_fieldname) == doc_type:
					...
					doc = dontmanage.get_doc(data)
					doc.db_insert()

			match custom_doctype:
				case "Custom Field":
					for d in data[key]:
						field = dontmanage.db.get_value(
				...
				case "DocType Link":
					for d in data[key]:
						...
						if not link:
							...
							doc_link = dontmanage.get_doc("DocType Link", link)
				...
				case "Custom DocPerm":
					...
					dontmanage.db.delete("Custom DocPerm", {"parent": doc_type})
...
def get_module_app(module: str) -> str:
	app = dontmanage.local.module_app.get(scrub(module))
	if app is None:
		dontmanage.throw(_("Module {} not found").format(module), exc=dontmanage.DoesNotExistError)
	return app
...
def make_boilerplate(template: str, doc: "Document" | "dontmanage._dict", opts: dict | "dontmanage._dict" = None):
...
def create_directory_on_app_path(folder_name, app_name):
	app_path = dontmanage.get_app_path(app_name)

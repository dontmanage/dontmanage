...
from dontmanage.modules import get_module_path, scrub, scrub_dt_dn
...
def export_to_files(record_list=None, record_module=None, verbose=0, create_init=None):
	...
	if record_list:
		for record in record_list:
			...
			write_document_file(
				dontmanage.get_doc(record[0], record[1]),
				record_module,
...
def write_document_file(doc, record_module=None, create_init=True, folder_name=None):
	...
	is_custom_module = dontmanage.db.get_value("Module Def", module, "custom")
	...
	if is_custom_module and not Path(path).resolve().is_relative_to(Path(dontmanage.get_site_path()).resolve()):
		dontmanage.throw("Invalid export path: " + Path(path).as_posix())
	with open(path, "w+") as txtfile:
		txtfile.write(dontmanage.as_json(doc_export) + "\n")
...
def strip_default_fields(doc, doc_export):
	...
	for df in doc.meta.get_table_fields():
		for d in doc_export.get(df.fieldname):
			for fieldname in dontmanage.model.default_fields + dontmanage.model.child_table_fields:
				if fieldname in d:
					del d[fieldname]
	...
	if hasattr(doc, "get_code_fields"):
		for key, extn in doc.get_code_fields().items():
			if doc.get(key):
				...
				if not Path(path).resolve().is_relative_to(Path(dontmanage.get_site_path()).resolve()):
					dontmanage.throw("Invalid export path: " + Path(path).as_posix())
				with open(path, "w+") as txtfile:
...
def get_module_name(doc):
	if doc.doctype == "Module Def":
		...
		module = dontmanage.db.get_value("DocType", doc.document_type, "module")
	elif hasattr(doc, "module"):
		module = doc.module
	else:
		module = dontmanage.db.get_value("DocType", doc.doctype, "module")

	return module


def delete_folder(module, dt, dn):
	if dontmanage.db.get_value("Module Def", module, "custom"):
...
def create_folder(module, dt, dn, create_init, is_custom_module):
	...
	dontmanage.create_folder(folder)
...
def get_custom_module_path(module):
	package = dontmanage.db.get_value("Module Def", module, "package")
	if not package:
		dontmanage.throw(f"Package must be set for custom Module <b>{module}</b>")

	path = os.path.join(get_package_path(package), scrub(module))
	if not Path(path).resolve().is_relative_to(Path(dontmanage.get_site_path()).resolve()):
		dontmanage.throw("Invalid module path: " + Path(path).as_posix())
...
def get_package_path(package):
	path = os.path.join(
		dontmanage.get_site_path("packages"), dontmanage.db.get_value("Package", package, "package_name")
	)
	if not os.path.exists(path):
		os.makedirs(path)
	return path

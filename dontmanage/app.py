...
from dontmanage.auth import SAFE_HTTP_METHODS, UNSAFE_HTTP_METHODS, HTTPRequest, check_request_ip, validate_auth
from dontmanage.integrations.oauth2 import get_resource_url, handle_wellknown, is_oauth_metadata_enabled
from dontmanage.middlewares import StaticDataMiddleware
from dontmanage.permissions import handle_does_not_exist_error
from dontmanage.utils import CallbackManager, cint, get_site_name
from dontmanage.utils.data import escape_html
from dontmanage.utils.error import log_error, log_error_snapshot
...
import dontmanage.database.mariadb.mysqlclient  # Load database related utils
...
# this is necessary because dontmanage desk sends most requests as form data
...
def application(request: Request):
	...
	try:
		...
		if db := getattr(dontmanage.local, "db", None):
...
def init_request(request):
	...
	dontmanage.init(site, sites_path=_sites_path, force=True)
...
def process_response(response: Response):
	...
	if hasattr(dontmanage.local, "rate_limiter"):
		response.headers.update(dontmanage.local.rate_limiter.headers())

	if trace_id := dontmanage.monitor.get_trace_id():
		response.headers.update({"X-DontManage-Request-Id": trace_id})
	...
	response.headers.update(dontmanage.local.response_headers)
	...
	if hasattr(dontmanage.local, "cookie_manager") and not public_cache:
...
def set_cors_headers(response):
	...
	if hasattr(dontmanage.local, "allow_cors"):
		allowed_origins = dontmanage.local.allow_cors

	if not (
		allowed_origins and (request := dontmanage.local.request) and (origin := request.headers.get("Origin"))
	):
		return
...
def make_form_dict(request: Request):
	...
	if request_data and request.is_json:
		try:
			...
			dontmanage.throw(_("Invalid request body"), dontmanage.DataError)
	else:
		args = {}
...
def handle_exception(e):
	...
	respond_as_json = (
		dontmanage.get_request_header("Accept") and (dontmanage.local.is_ajax or "application/json" in accept_header)
	...
	if http_status_code >= 500 or dontmanage.conf.developer_mode:
...
def sync_database():
	db = getattr(dontmanage.local, "db", None)
	if not db:
		# db isn't initialized, can't commit or rollback
		return

	# if HTTP method would change server state, commit if necessary
	if dontmanage.local.request.method in UNSAFE_HTTP_METHODS or dontmanage.local.flags.commit:
	...
	if session := getattr(dontmanage.local, "session_obj", None):
		dontmanage.request.after_response.add(session.update)

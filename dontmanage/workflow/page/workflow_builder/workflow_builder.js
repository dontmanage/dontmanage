dontmanage.pages["workflow-builder"].on_page_load = function (wrapper) {
	...
	if (dontmanage.boot.developer_mode) {
		...
		dontmanage.hot_update.push(() => load_workflow_builder(wrapper));
	}
};

dontmanage.pages["workflow-builder"].on_page_show = function (wrapper) {
...
function load_workflow_builder(wrapper) {
	...
	if (route.length > 1) {
		dontmanage.require("workflow_builder.bundle.js").then(() => {
			dontmanage.workflow_builder = new dontmanage.ui.WorkflowBuilder({
				wrapper: $parent,
				page: wrapper.page,
				workflow: route[1],
			});
		...
		let d = new dontmanage.ui.Dialog({
			...
			primary_action({ action, doctype, workflow, workflow_name }) {
				if (action === "Edit") {
					dontmanage.set_route("workflow-builder", workflow);

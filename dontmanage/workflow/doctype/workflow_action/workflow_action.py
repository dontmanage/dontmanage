...
from dontmanage.desk.form.utils import get_pdf_link
from dontmanage.desk.notifications import clear_doctype_notifications
...
from dontmanage.utils import get_datetime, get_url
...
class WorkflowAction(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanage.workflow.doctype.workflow_action_permitted_role.workflow_action_permitted_role import (
...
def on_doctype_update():
	...
	dontmanage.db.add_index("Workflow Action", ["reference_name", "reference_doctype", "status"])
...
def get_permission_query_conditions(user):
	...
	permitted_workflow_actions = (
		dontmanage.qb.from_(WorkflowAction)
	...
	return f"""(`tabWorkflow Action`.`name` in ({permitted_workflow_actions})
		or `tabWorkflow Action`.`user`={dontmanage.db.escape(user)})
...
def has_permission(doc, user):
	...
	return not permitted_roles.isdisjoint(dontmanage.get_roles(user))
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = dontmanage.session.user
	...
	if logged_in_user == "Guest":
		dontmanage.set_user(logged_in_user)
...
def return_success_page(doc):
	dontmanage.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), dontmanage.bold(doc.get("name")), dontmanage.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	dontmanage.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			dontmanage.bold(doc.get("name")),
			dontmanage.bold(doc_workflow_state),
			dontmanage.bold(dontmanage.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def get_allowed_roles(user, workflow, workflow_state):
	user = user if user else dontmanage.session.user

	allowed_roles = dontmanage.get_all(
		"Workflow Transition",
		fields="allowed",
	...
	user_roles = set(dontmanage.get_roles(user))
...
def update_completed_workflow_actions_using_role(user=None, workflow_action=None):
	...
	(
		dontmanage.qb.update(WorkflowAction)
...
def get_next_possible_transitions(workflow_name, state, doc=None):
	transitions = dontmanage.get_all(
		"Workflow Transition",
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = dontmanage._dict(
					{
						"possible_actions": [],
						"email": dontmanage.db.get_value("User", user, "email"),
					}
				)
...
def create_workflow_actions_for_roles(roles, doc):
	if not roles:
		return
	workflow_action = dontmanage.get_doc(
		{
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = "/api/method/dontmanage.workflow.doctype.workflow_action.workflow_action.apply_action"
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/dontmanage.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def get_common_email_args(doc):
	...
	if email_template:
		subject = dontmanage.render_template(email_template.subject, vars(doc))
		response = dontmanage.render_template(email_template.response, vars(doc))
	else:
	...
	return {
		...
		"attachments": [dontmanage.attach_print(doctype, docname, file_name=docname, doc=doc)],
		"subject": subject,
		"message": response,
	}
...
def get_email_template(doc):
	...
	template_name = dontmanage.db.get_value(
	...
	if not template_name:
		return
	return dontmanage.get_doc("Email Template", template_name)

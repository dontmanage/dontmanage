...
from dontmanage.model.workflow import DEFAULT_WORKFLOW_TASKS
...
class Workflow(Document):
	...
	def validate_docstatus(self):
		def get_state(state):
			...
			dontmanage.throw(dontmanage._("{0} not a valid State").format(state))
		...
		for t in self.transitions:
			...
			if state.doc_status == "2":
				dontmanage.throw(
					dontmanage._("Cannot change state of Cancelled Document. Transition row {0}").format(t.idx)
				)
			...
			if state.doc_status == "0" and next_state.doc_status == "2":
				dontmanage.throw(dontmanage._("Cannot cancel before submitting. See Transition {0}").format(t.idx))
...
def get_workflow_methods():
	return [i["name"] for i in dontmanage.get_hooks("workflow_methods")] + DEFAULT_WORKFLOW_TASKS

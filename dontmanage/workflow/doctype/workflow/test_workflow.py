...
class TestWorkflow(IntegrationTestCase):
	...
	def test_dynamic_update_value_expression(self):
		...
		self.workflow.states[1].update_value = "dontmanage.session.user"
		...
		self.assertEqual(todo.assigned_by, dontmanage.session.user)
	...
	def test_static_value_when_expression_disabled(self):
		...
		self.assertEqual(todo.description, "dontmanage.session.user")
	...
	def test_sync_tasks(self, doc=None):
		...
		domain = dontmanage.new_doc("Domain")
		...
		with self.patch_hooks(
			{
				"workflow_methods": [
					{
						"name": "Create Note",
						"method": "dontmanage.workflow.doctype.workflow.test_workflow.create_new_note",
					}
				]
			}
		):
		...
		self.assertTrue(
			dontmanage.db.exists("Note", {"title": "workflow - " + domain.name, "content": "workflow test"})
		)
		self.assertTrue(dontmanage.db.exists("Domain", {"name": "workflow - " + domain.name}))
		self.assertTrue(dontmanage.db.exists("Webhook Request Log", {"url": "https://workflowtasks.org/post"}))
...
def create_todo_workflow():
	from dontmanage.tests.ui_test_helpers import UI_TEST_USER

	if dontmanage.db.exists("Workflow", "Test ToDo"):
		dontmanage.delete_doc("Workflow", "Test ToDo")

	TEST_ROLE = "Test Approver"

	if not dontmanage.db.exists("Role", TEST_ROLE):
		...
		if dontmanage.db.exists("User", UI_TEST_USER):
			dontmanage.get_doc("User", UI_TEST_USER).add_roles(TEST_ROLE)

	workflow = dontmanage.new_doc("Workflow")
...
def create_domain_workflow():
	...
	if dontmanage.db.exists("Workflow", "Test Domain"):
		dontmanage.delete_doc("Workflow", "Test Domain")
	...
	pending_to_approved_transition = dontmanage.new_doc("Workflow Transition Tasks")
...
def create_new_todo():
	return dontmanage.get_doc(doctype="ToDo", description="workflow " + random_string(10)).insert()
...
def create_new_server_script():
	server_script = dontmanage.new_doc("Server Script")

...
class TestWorkflow(DontManageTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def test_if_workflow_actions_were_processed_using_user(self):
		...
		dontmanage.qb.update(WorkflowAction).set(WorkflowAction.user, "test2@example.com").run()
		dontmanage.qb.update(WorkflowActionPermittedRole).set(WorkflowActionPermittedRole.role, "").run()
...
def create_todo_workflow():
	...
	if not dontmanage.db.exists("Role", TEST_ROLE):
		dontmanage.get_doc(dict(doctype="Role", role_name=TEST_ROLE)).insert(ignore_if_duplicate=True)
...
def create_new_todo():
	return dontmanage.get_doc(dict(doctype="ToDo", description="workflow " + random_string(10))).insert()

...
from dontmanage.tests.test_api import DontManageAPITestCase, suppress_stdout
from dontmanage.tests.utils import toggle_test_mode, whitelist_for_tests
...
class TestResourceAPIV2(DontManageAPITestCase):
	version = "v2"
	DOCTYPE = "ToDo"
	...
	def test_copy_document(self):
		doc = dontmanage.get_doc(self.DOCTYPE, self.GENERATED_DOCUMENTS[0])
	...
	def test_execute_doc_method(self):
		...
		self.assertEqual(response.json["data"][0]["name"], "dontmanage")
...
class TestMethodAPIV2(DontManageAPITestCase):
	version = "v2"
	...
	def test_shorthand_controller_methods(self):
		...
		expanded_response = self.get(
			self.method("dontmanage.core.doctype.user.user.get_all_roles"), {"sid": self.sid}
		)
	...
	def test_add_comment(self):
		comment_txt = dontmanage.generate_hash()
		response = self.post(
...
class TestDocTypeAPIV2(DontManageAPITestCase):
	version = "v2"

...
execute:dontmanage.db.updatedb("Item")
execute:dontmanage.function(arg="1")
...
class TestPatches(DontManageTestCase):
	def test_patch_module_names(self):
		dontmanage.flags.final_patches = []
		dontmanage.flags.in_install = True
		for patchmodule in patch_handler.get_all_patches():
			if patchmodule.startswith("execute:"):
				...
				self.assertTrue(dontmanage.get_attr(patchmodule.split(maxsplit=1)[0] + ".execute"))

		dontmanage.flags.in_install = False

	def test_get_patch_list(self):
		pre = patch_handler.get_patches_from_app("dontmanage", patch_handler.PatchType.pre_model_sync)
		post = patch_handler.get_patches_from_app("dontmanage", patch_handler.PatchType.post_model_sync)
		all_patches = patch_handler.get_patches_from_app("dontmanage")
	...
	def test_all_patches_are_marked_completed(self):
		all_patches = patch_handler.get_patches_from_app("dontmanage")
		finished_patches = dontmanage.db.count("Patch Log")

		self.assertGreaterEqual(finished_patches, len(all_patches))


class TestPatchReader(DontManageTestCase):
	def get_patches(self):
		return (
			patch_handler.get_patches_from_app("dontmanage"),
			patch_handler.get_patches_from_app("dontmanage", patch_handler.PatchType.pre_model_sync),
			patch_handler.get_patches_from_app("dontmanage", patch_handler.PatchType.post_model_sync),
		)
...
def check_patch_files(app):
	...
	patch_dir = Path(dontmanage.get_app_path(app)) / "patches"
	...
	for file in patch_dir.glob("**/*.py"):
		...
		try:
			patch_module = dontmanage.get_module(module)

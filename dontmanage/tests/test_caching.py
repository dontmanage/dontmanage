...
@site_cache
def ping() -> str:
	register_with_external_service(dontmanage.local.site)
	return "pong"
...
class TestSiteCache(DontManageAPITestCase):
...
class TestRedisCache(DontManageAPITestCase):
	...
	def test_user_cache(self):
		...
		def calculate_area(radius: float) -> float:
			nonlocal function_call_count
			PI_APPROX = ENGINEERING_PI if dontmanage.session.user == "Engineer" else PI
			function_call_count += 1
...
class TestRedisWrapper(DontManageAPITestCase):
	...
	def test_user_cache_clear(self):
		from dontmanage.cache_manager import user_cache_keys

		# Set some keys that a user's cache would usually have
		user1 = dontmanage.utils.random_string(10)
		user2 = dontmanage.utils.random_string(10)
		for key in user_cache_keys:
			dontmanage.cache.hset(key, user1, key)
			dontmanage.cache.hset(key, user2, key)

		dontmanage.clear_cache(user=user1)

		# Check that the keys for user1 are gone
		for key in set(user_cache_keys) - {"home_page"}:
			self.assertFalse(dontmanage.cache.hexists(key, user1))
			self.assertTrue(dontmanage.cache.hexists(key, user2))

	def test_doctype_cache_clear(self):
		from dontmanage.cache_manager import doctype_cache_keys

		# Set some keys that a user's cache would usually have
		doctype1 = new_doctype(dontmanage.utils.random_string(10))
		doctype2 = new_doctype(dontmanage.utils.random_string(10))
		for key in doctype_cache_keys:
			dontmanage.cache.hset(key, doctype1.name, key)
			dontmanage.cache.hset(key, doctype2.name, key)

		dontmanage.clear_cache(doctype=doctype1.name)

		# Check that the keys for doctype1 are gone
		for key in doctype_cache_keys:
			self.assertFalse(dontmanage.cache.hexists(key, doctype1.name))
			self.assertTrue(dontmanage.cache.hexists(key, doctype2.name))

	def test_backward_compat_cache(self):
		self.assertEqual(dontmanage.cache, dontmanage.cache())


class TestHttpCache(DontManageAPITestCase):
	def test_http_headers(self):
		resp = self.get(
			self.method("dontmanage.client.is_document_amended"),

...
class TestPerformance(IntegrationTestCase):
	def reset_request_specific_caches(self):
		...
		dontmanage.destroy()  # releases everything on dontmanage.local
		dontmanage.init(self.TEST_SITE)
		dontmanage.connect()
		dontmanage.clear_cache()

	def setUp(self) -> None:
		self.HOST = dontmanage.utils.get_site_url(dontmanage.local.site)
	...
	def test_set_value_query_count(self):
		...
		with self.assertQueryCount(1):
			dontmanage.db.set_value("User", {"user_type": "System User"}, "interest", "Nothing")
	...
	def test_req_per_seconds_basic(self):
		...
		req_count = 1000
		client = DontManageClient(self.HOST, "Administrator", self.ADMIN_PASSWORD)
	...
	def test_no_ifnull_checks(self):
		query = dontmanage.get_all("DocType", {"autoname": ("is", "set")}, run=0).get_sql().lower()
	...
	def test_get_doc_cache_calls(self):
		dontmanage.get_doc("User", "Administrator")
		with self.assertRedisCallCounts(0):
			dontmanage.get_doc("User", "Administrator")

	def test_local_caching(self):
		dontmanage.get_cached_doc("User", "Administrator")
		with self.assertRedisCallCounts(0):
			dontmanage.get_cached_doc("User", "Administrator")
	...
	def test_idle_cpu_utilization_redis_pubsub(self):
		pid = dontmanage.client_cache.invalidator_thread.native_id
		process = psutil.Process(pid)
		self.assertLess(process.cpu_percent(interval=1.0), 2)

	def test_cpu_allocation(self):
		from dontmanage._optimizations import assign_core
...
@run_only_if(db_type_is.MARIADB)
class TestOverheadCalls(DontManageAPITestCase):
	...
	def test_get_doc_overheads(self):
		sid = self.sid
		tables = len(dontmanage.get_meta("User").get_table_fields())

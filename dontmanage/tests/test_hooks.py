...
class TestHooks(IntegrationTestCase):
	...
	def test_has_permission(self):
		...
		wildcard_has_permission_hook.append("dontmanage.tests.test_hooks.custom_has_permission")
		...
		self.assertTrue(dontmanage.has_permission("Note", doc=note, user=username))
		...
		self.assertFalse(dontmanage.has_permission("Note", doc=note, user=username))
	...
	def test_fixture_prefix(self):
		...
		app = "dontmanage"
		if os.path.isdir(dontmanage.get_app_path(app, "fixtures")):
			shutil.rmtree(dontmanage.get_app_path(app, "fixtures"))
		...
		# every call to dontmanage.get_hooks loads the hooks module into cache
		...
		if dontmanage._load_app_hooks in dontmanage.local.request_cache.keys():
			del dontmanage.local.request_cache[dontmanage._load_app_hooks]
		self.assertEqual([False], dontmanage.get_hooks("fixture_auto_order", app_name=app))
		self.assertEqual(
			[
				{"dt": "User"},
				{"dt": "Contact"},
			...
			dontmanage.get_hooks("fixtures", app_name=app),
		)
		...
		self.assertCountEqual(
			["user.json", "contact.json", "role.json"], os.listdir(dontmanage.get_app_path(app, "fixtures"))
		)

		hooks.fixture_auto_order = True
		del dontmanage.local.request_cache[dontmanage._load_app_hooks]
		self.assertEqual([True], dontmanage.get_hooks("fixture_auto_order", app_name=app))

		shutil.rmtree(dontmanage.get_app_path(app, "fixtures"))
		export_fixtures(app)
		self.assertCountEqual(
			["1_user.json", "2_contact.json", "3_role.json"],
			os.listdir(dontmanage.get_app_path(app, "fixtures")),
		)
...
class TestAPIHooks(DontManageAPITestCase):
	def test_auth_hook(self):
		with self.patch_hooks({"auth_hooks": ["dontmanage.tests.test_hooks.custom_auth"]}):
...
def custom_auth():
	_auth_type, token = dontmanage.get_request_header("Authorization", "Bearer ").split(" ")

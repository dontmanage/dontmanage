...
class TestDashboardConnections(IntegrationTestCase):
	...
	def test_external_doctype_link_with_dashboard_override(self):
		...
		# create a test doc
		todo_doc = dontmanage.get_doc(doctype="ToDo", description="test").insert()
		dontmanage.get_doc(doctype="Test Doctype D", title="d-001", doclink=todo_doc.name).insert()
		dontmanage.get_doc(doctype="Test Doctype E", title="e-001", todo=todo_doc.name).insert()

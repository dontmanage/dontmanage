...
from dontmanage.custom.doctype.customize_form.test_customize_form import TestCustomizeForm
from dontmanage.desk.notifications import get_open_count
from dontmanage.tests.utils import DontManageTestCase, patch_hooks


class TestDashboardConnections(DontManageTestCase):
	...
	def test_external_doctype_link_with_dashboard_override(self):
		...
		# create a test doc
		todo_doc = dontmanage.get_doc(dict(doctype="ToDo", description="test")).insert()
		dontmanage.get_doc(dict(doctype="Test Doctype D", title="d-001", doclink=todo_doc.name)).insert()
		dontmanage.get_doc(dict(doctype="Test Doctype E", title="e-001", todo=todo_doc.name)).insert()
		...
		with patch_hooks(
			{
				"override_doctype_dashboards": {
					"ToDo": ["dontmanage.tests.test_dashboard_connections.get_dashboard_for_todo"]
				}
			}
		):
			connections = get_open_count("ToDo", todo_doc.name)["count"]
...
def add_links_in_child_tables():
	test_child_table_with_link_to_doctype_a = dontmanage.get_doc(
	...
	test_child_table_with_link_to_doctype_b = dontmanage.get_doc(

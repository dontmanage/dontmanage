# DontManage Test Framework

This README provides an overview of the test case framework available in DontManage. These utilities are designed to facilitate efficient and effective testing of DontManage applications.

This is different from the `dontmanage.testing` module which houses the discovery and runner infrastructure for CLI and CI.
...
5. Specific test modules for various DontManage components

### Test Case Classes

#### UnitTestCase ([`classes/unit_test_case.py`](./classes/unit_test_case.py))

###### Import convention: `from dontmanage.tests import UnitTestCase`

This class extends `unittest.TestCase` and provides additional utilities specific to the DontManage framework. It's designed for testing individual components or functions in isolation.

Key features include:
- Custom assertions for DontManage-specific comparisons
...
###### Import convention: `from dontmanage.tests import IntegrationTestCase`
...
Various test modules (e.g., test_api.py, test_document.py) contain tests for specific DontManage core components and functionalities.

Note that Document tests are collocated alongside each Document module.

## Usage

To use these test utilities in your DontManage application tests, you can inherit from the appropriate test case class:

```python
from dontmanage.tests import UnitTestCase

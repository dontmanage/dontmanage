...
import dontmanage
from dontmanage.integrations.oauth2 import encode_params
from dontmanage.tests import IntegrationTestCase
from dontmanage.tests.test_api import get_test_client, make_request, suppress_stdout
from dontmanage.tests.utils import make_test_records
from dontmanage.utils.oauth import build_oauth_url

if TYPE_CHECKING:
	from dontmanage.integrations.doctype.social_login_key.social_login_key import SocialLoginKey


class DontManageRequestTestCase(IntegrationTestCase):
	@property
	def sid(self) -> str:
		if not getattr(self, "_sid", None):
			...
			dontmanage.local.login_manager.login_as("test@example.com")
...
class TestOAuth20(DontManageRequestTestCase):
	...
	def setUpClass(cls):
		...
		# Set DontManage server URL reqired for id_token generation
		dontmanage_login_key: SocialLoginKey = dontmanage.new_doc("Social Login Key")
		dontmanage_login_key.get_social_login_provider("DontManage", initialize=True)
		dontmanage_login_key.base_url = dontmanage.utils.get_url()
		dontmanage_login_key.enable_social_login = 0
		dontmanage_login_key.insert(ignore_if_duplicate=True)
		dontmanage.db.commit()

	def setUp(self):
		self.TEST_CLIENT = get_test_client()
		self.oauth_client = dontmanage.new_doc("OAuth Client")
	...
	def test_login_using_authorization_code(self):
		...
		resp = self.get(
			"/api/method/dontmanage.integrations.oauth2.authorize",
			{
		...
		token_response = self.post(
			"/api/method/dontmanage.integrations.oauth2.get_token",
	...
	def test_revoke_token(self):
		client = dontmanage.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = self.post(
			"/api/method/dontmanage.integrations.oauth2.revoke_token",
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = dontmanage.get_doc("OAuth Client", self.client_id)
		...
		try:
			session.get(
				get_full_url("/api/method/dontmanage.integrations.oauth2.authorize"),
	...
	def test_openid_code_id_token(self):
		update_client_for_auth_code_grant(self.client_id)
		nonce = dontmanage.generate_hash()
...
def check_valid_openid_response(access_token=None, client: "DontManageRequestTestCase" = None):
	"""Return True for valid response."""
	# Use token in header
	headers = {}
	URL = "/api/method/dontmanage.integrations.oauth2.openid_profile"
...
def get_full_url(endpoint):
	...
	return urljoin(dontmanage.utils.get_url(), endpoint)

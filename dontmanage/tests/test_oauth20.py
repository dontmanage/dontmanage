...
class TestOAuth20(unittest.TestCase):
	def setUp(self):
		...
		client = dontmanage.get_all("OAuth Client", fields=["*"])[0]
		...
		try:
			dontmanage_login_key = dontmanage.get_doc("Social Login Key", "dontmanage")
		except dontmanage.DoesNotExistError:
			dontmanage_login_key = dontmanage.new_doc("Social Login Key")
	...
	def test_login_using_authorization_code(self):
		...
		token_response = requests.post(
			get_full_url("/api/method/dontmanage.integrations.oauth2.get_token"),
	...
	def test_revoke_token(self):
		...
		revoke_token_response = requests.post(
			get_full_url("/api/method/dontmanage.integrations.oauth2.revoke_token"),
			headers=self.form_header,
...
def check_valid_openid_response(access_token=None):
	...
	openid_response = requests.get(
		get_full_url("/api/method/dontmanage.integrations.oauth2.openid_profile"), headers=headers
	)

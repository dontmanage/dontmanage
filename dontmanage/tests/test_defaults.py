...
class TestDefaults(IntegrationTestCase):
	...
	def test_user_permission_on_defaults(self):
		...
		perm_doc = dontmanage.get_doc(
			doctype="User Permission", user=dontmanage.session.user, allow="Language", for_value="en-GB"
	...
	def test_user_permission_defaults(self):
		...
		perm_doc = dontmanage.get_doc(
			doctype="User Permission", user=dontmanage.session.user, allow="Country", for_value="India"
		).insert(ignore_permissions=True)
		...
		perm_doc = dontmanage.get_doc(
			doctype="User Permission", user=dontmanage.session.user, allow="Country", for_value="United States"
		).insert(ignore_permissions=True)

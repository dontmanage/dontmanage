...
from dontmanage.desk.query_report import build_xlsx_data, export_query, run
...
class TestQueryReport(IntegrationTestCase):
	...
	def test_csv(self):
		...
		if not dontmanage.db.exists("Report", REPORT_NAME):
			...
			report.query = dontmanage.qb.from_(REF_DOCTYPE).select(*REPORT_COLUMNS).limit(10).get_sql()
		...
		dontmanage.delete_doc("Report", REPORT_NAME, delete_permanently=True)
	...
	def test_report_for_duplicate_column_names(self):
		...
		try:
			...
			for i in range(1, 3):
				dontmanage.get_doc({"doctype": "Doc A", "first_name": f"John{i}", "last_name": "Doe"}).insert()
				dontmanage.get_doc({"doctype": "Doc B", "last_name": f"Doe{i}", "first_name": "John"}).insert()

			if not dontmanage.db.exists("Report", "Doc A Report"):
				...
				report = dontmanage.get_doc("Report", "Doc A Report")
...
data = columns, result
	...
	def test_export_report_via_email(self):
		...
		jobs = dontmanage.get_all("RQ Job")
		email_queue = dontmanage.get_all("Email Queue")

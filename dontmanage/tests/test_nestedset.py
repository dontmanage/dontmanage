...
class NestedSetTestUtil:
	...
	def move_it_back(self):
		parent_1 = dontmanage.get_doc("Test Tree DocType", "Parent 1")
		parent_1.parent_test_tree_doctype = "Root Node"
		parent_1.save()

	def get_no_of_children(self, record_name: str) -> int:
		if not record_name:
			return dontmanage.db.count("Test Tree DocType")
...
class TestNestedSet(DontManageTestCase):
	...
	def test_basic_tree(self):
		...
		min_lft = 1
		max_rgt = dontmanage.qb.from_("Test Tree DocType").select(Max(Field("rgt"))).run(pluck=True)[0]
	...
	def test_recursion(self):
		leaf_node = dontmanage.get_doc("Test Tree DocType", {"some_fieldname": "Parent 2"})
	...
	def test_move_group_into_another(self):
		old_lft, old_rgt = dontmanage.db.get_value("Test Tree DocType", "Parent 2", ["lft", "rgt"])
		...
		# after move
		new_lft, new_rgt = dontmanage.db.get_value("Test Tree DocType", "Parent 2", ["lft", "rgt"])
	...
	def test_move_leaf_into_another_group(self):
		child_2 = dontmanage.get_doc("Test Tree DocType", "Child 2")

		# assert that child 2 is not already under parent 1
		parent_lft_old, parent_rgt_old = dontmanage.db.get_value(
		...
		parent_lft_new, parent_rgt_new = dontmanage.db.get_value(
			"Test Tree DocType", "Parent 2", ["lft", "rgt"]
		)
	...
	def test_delete_leaf(self):
		...
		child_1 = dontmanage.get_doc("Test Tree DocType", "Child 1")
		child_1.delete()
		records.remove(el)

		self.test_basic_tree()

		n = dontmanage.new_doc("Test Tree DocType")
		n.update(el)
		n.insert()
		records.append(el)
	...
	def test_delete_group(self):
		...
		with self.assertRaises(NestedSetChildExistsError):
			dontmanage.delete_doc("Test Tree DocType", "Parent 1")
	...
	def test_merge_groups(self):
		...
		dontmanage.rename_doc("Test Tree DocType", "Parent 2", "Parent 1", merge=True)
		records.remove(el)
		self.test_basic_tree()
	...
	def test_merge_leaf_into_group(self):
		with self.assertRaises(NestedSetInvalidMergeError):
			dontmanage.rename_doc("Test Tree DocType", "Child 1", "Parent 1", merge=True)

	def test_merge_group_into_leaf(self):
		with self.assertRaises(NestedSetInvalidMergeError):
			dontmanage.rename_doc("Test Tree DocType", "Parent 1", "Child 1", merge=True)

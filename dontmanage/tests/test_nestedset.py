...
class TestNestedSet(DontManageTestCase):
	@classmethod
	...
	def test_desc_filters(self):
		...
		# db_query
		self.assertNotIn(record, dontmanage.get_all(TEST_DOCTYPE, exclusive_filter, run=0))
		self.assertIn(record, dontmanage.get_all(TEST_DOCTYPE, inclusive_filter, run=0))
		self.assertNotIn(record, dontmanage.get_all(linked_doctype, exclusive_link, run=0))
		self.assertIn(record, dontmanage.get_all(linked_doctype, inclusive_link, run=0))

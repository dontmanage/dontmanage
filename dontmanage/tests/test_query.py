...
class TestQuery(DontManageTestCase):
	...
	def test_functions_fields(self):
		self.assertEqual(
			dontmanage.qb.get_query("User", fields="Count(name)", filters={}).get_sql(),
			dontmanage.qb.from_("User").select(Count(Field("name"))).get_sql(),
		)

		self.assertEqual(
			dontmanage.qb.get_query("User", fields=["Count(name)", "Max(name)"], filters={}).get_sql(),
			dontmanage.qb.from_("User").select(Count(Field("name")), Max(Field("name"))).get_sql(),
		)

		self.assertEqual(
			dontmanage.qb.get_query("User", fields=["abs(name-email)", "Count(name)"], filters={}).get_sql(),
			dontmanage.qb.from_("User")
		...
		self.assertEqual(
			dontmanage.qb.get_query("User", fields=[Count("*")], filters={}).get_sql(),
			dontmanage.qb.from_("User").select(Count("*")).get_sql(),
		)

		self.assertEqual(
			dontmanage.qb.get_query("User", fields="timestamp(creation, modified)", filters={}).get_sql(),
			dontmanage.qb.from_("User").select(Timestamp(Field("creation"), Field("modified"))).get_sql(),
		)
	...
	def test_filters(self):
		self.assertEqual(
			...
			"SELECT `tabDocType`.`name` FROM `tabDocType` LEFT JOIN `tabModule Def` ON `tabModule Def`.`name`=`tabDocType`.`module` WHERE `tabModule Def`.`app_name`='dontmanage'".replace(
				"`", '"' if dontmanage.db.db_type == "postgres" else "`"
			),
		)
		...
		self.assertRaisesRegex(
			...
			"Invalid filter",
			lambda: dontmanage.qb.get_query(
				"DocType",
				fields=["name"],
				filters={"permissions.role": "System Manager"},
		...
		self.assertEqual(
			...
			"SELECT `name` FROM `tabDocType`".replace("`", '"' if dontmanage.db.db_type == "postgres" else "`"),
		)

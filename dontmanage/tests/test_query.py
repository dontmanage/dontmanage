...
from dontmanage.query_builder.functions import Abs, Count, Ifnull, Max, Now, Timestamp
from dontmanage.tests import IntegrationTestCase
from dontmanage.tests.classes.context_managers import enable_safe_exec
from dontmanage.tests.test_db_query import (
...
from dontmanage.utils.nestedset import get_ancestors_of, get_descendants_of
...
def create_tree_docs():
	...
	for record in records:
		d = dontmanage.new_doc("Test Tree DocType")
		d.update(record)
		d.insert()
...
class TestQuery(IntegrationTestCase):
	...
	def test_multiple_tables_in_filters(self):
		...
		query = query.replace("LIKE", "ILIKE" if dontmanage.db.db_type == "postgres" else "LIKE")
		self.assertQueryEqual(
			dontmanage.qb.get_query(
				"DocType",
				["*"],
				[
	...
	def test_string_fields(self):
		self.assertEqual(
			dontmanage.qb.get_query("User", fields="name, email", filters={"name": "Administrator"}).get_sql(),
		...
		self.assertEqual(
			dontmanage.qb.get_query("User", fields=[Count("*")]).get_sql(),
			dontmanage.qb.from_("User").select(Count("*")).get_sql(),
		)
	...
	def test_qb_fields(self):
		...
		self.assertEqual(
			...
			dontmanage.qb.from_(user_doctype).select(user_doctype.name, user_doctype.email).get_sql(),
		)

		self.assertEqual(
			dontmanage.qb.get_query(user_doctype, fields=user_doctype.email, filters={}).get_sql(),
			dontmanage.qb.from_(user_doctype).select(user_doctype.email).get_sql(),
		)
	...
	def test_field_validation_select(self):
		...
		for field in valid_fields:
			try:
				dontmanage.qb.get_query("User", fields=field).get_sql()
		...
		for field in invalid_fields:
			with self.assertRaises(
				(dontmanage.PermissionError, dontmanage.ValidationError),
	...
	def test_field_validation_filters(self):
		...
		for field in valid_fields:
			try:
				# Test in dict filter
				dontmanage.qb.get_query("User", filters={field: "value"}).get_sql()
				# Test in list filter
				dontmanage.qb.get_query("User", filters=[[field, "=", "value"]]).get_sql()
				# Test in list filter with doctype
				dontmanage.qb.get_query("User", filters=[["User", field, "=", "value"]]).get_sql()
			except Exception as e:
				self.fail(f"Valid filter field '{field}' failed validation: {e}")

		for field in invalid_fields:
			with self.assertRaises(
				dontmanage.ValidationError, msg=f"Invalid filter field '{{{field}: val}}' passed validation"
			):
				dontmanage.qb.get_query("User", filters={field: "value"}).get_sql()
	...
	def test_field_validation_group_by(self):
		...
		for group_by_str in valid_fields:
			try:
				dontmanage.qb.get_query("User", group_by=group_by_str).get_sql()
			except Exception as e:
	...
	def test_field_validation_order_by(self):
		...
		for order_by_str in valid_fields:
			try:
				dontmanage.qb.get_query("User", order_by=order_by_str).get_sql()
			except Exception as e:
				self.fail(f"Valid ORDER BY string '{order_by_str}' failed validation: {e}")

		for order_by_str in invalid_fields:
			with self.assertRaises(
				(dontmanage.PermissionError, ValueError, dontmanage.ValidationError),
	...
	def test_aliasing(self):
		user_doctype = dontmanage.qb.DocType("User")
		self.assertEqual(
			dontmanage.qb.get_query("User", fields=["name as owner", "email as id"], filters={}).get_sql(),
		...
		self.assertEqual(
			dontmanage.qb.get_query(user_doctype, fields="name as owner, email as id", filters={}).get_sql(),
			dontmanage.qb.from_(user_doctype)
			.select(user_doctype.name.as_("owner"), user_doctype.email.as_("id"))
			.get_sql(),
		)
	...
	def test_filters(self):
		self.assertQueryEqual(
			dontmanage.qb.get_query(
				"DocType",
				fields=["name"],
				filters={"module.app_name": "dontmanage"},
			).get_sql(),
			"SELECT `tabDocType`.`name` FROM `tabDocType` LEFT JOIN `tabModule Def` ON `tabModule Def`.`name`=`tabDocType`.`module` WHERE `tabModule Def`.`app_name`='dontmanage'",
		)
	...
	def test_or_filters(self):
		...
		query = query = query.replace("LIKE", "ILIKE" if dontmanage.db.db_type == "postgres" else "LIKE")
		self.assertQueryEqual(
			dontmanage.qb.get_query(
				"DocType",
				fields=["name"],
	...
	def test_nested_filters(self):
		...
		self.assertEqual(dontmanage.qb.get_query("User", filters=filters_and).get_sql(), expected_sql_and)
		...
		self.assertEqual(dontmanage.qb.get_query("User", filters=filters_or).get_sql(), expected_sql_or)
		...
		self.assertEqual(dontmanage.qb.get_query("User", filters=filters_mixed).get_sql(), expected_sql_mixed)
		...
		self.assertEqual(dontmanage.qb.get_query("User", filters=filters_nested).get_sql(), expected_sql_nested)
		...
		self.assertEqual(
			dontmanage.qb.get_query("User", filters=filters_single_group).get_sql(), expected_sql_single_group
		)
		...
		self.assertEqual(dontmanage.qb.get_query("User", filters=filters_complex).get_sql(), expected_sql_complex)

	def test_invalid_nested_filters(self):
		"""Test invalid formats for nested filters."""
		# Invalid operator
		with self.assertRaises(dontmanage.ValidationError) as cm:
			dontmanage.qb.get_query("User", filters=[["email", "=", "a"], "xor", ["email", "=", "b"]]).get_sql()
		self.assertIn("Expected 'and' or 'or' operator", str(cm.exception))

		# Missing condition after operator
		with self.assertRaises(dontmanage.ValidationError) as cm:
			dontmanage.qb.get_query("User", filters=[["email", "=", "a"], "and"]).get_sql()
		self.assertIn("Filter condition missing after operator", str(cm.exception))

		# Starting with operator
		with self.assertRaises(dontmanage.ValidationError) as cm:
			dontmanage.qb.get_query("User", filters=["and", ["email", "=", "a"]]).get_sql()
		self.assertIn("Invalid start for filter condition", str(cm.exception))

		# Invalid condition type (string instead of list/tuple)
		with self.assertRaises(dontmanage.ValidationError) as cm:
			dontmanage.qb.get_query("User", filters=[["email", "=", "a"], "and", "enabled = 1"]).get_sql()
		...
		with self.assertRaises(dontmanage.ValidationError) as cm:
			dontmanage.qb.get_query("User", filters=["email", "and", ["enabled", "=", 1]]).get_sql()
		self.assertIn("Invalid start for filter condition", str(cm.exception))
	...
	# 	self.assertNotIn(
	# 		"email", dontmanage.qb.get_query("User", fields=["name", "#email"], filters={}).get_sql()
	# 	)

	def test_nestedset(self):
		dontmanage.db.sql("delete from `tabDocType` where `name` = 'Test Tree DocType'")
		dontmanage.db.sql_ddl("drop table if exists `tabTest Tree DocType`")
		create_tree_docs()
		descendants_result = dontmanage.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
		...
		ancestors_result = dontmanage.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
		...
		not_descendants_result = dontmanage.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
		...
		not_ancestors_result = dontmanage.qb.get_query(
			"Test Tree DocType",
			fields=["name"],
	...
	def test_child_field_syntax(self):
		note1 = dontmanage.get_doc(doctype="Note", title="Note 1", seen_by=[{"user": "Administrator"}]).insert()
		note2 = dontmanage.get_doc(
			doctype="Note", title="Note 2", seen_by=[{"user": "Administrator"}, {"user": "Guest"}]
		).insert()

		result = dontmanage.qb.get_query(
			"Note",
	...
	def test_build_match_conditions(self):
		...
		query = dontmanage.qb.get_query("Test Blog Post", ignore_permissions=False)
		...
		query = str(dontmanage.qb.get_query("Test Blog Post", ignore_permissions=False))
	...
	def test_ignore_permissions_for_query(self):
		...
		with self.assertRaises(dontmanage.PermissionError):
			dontmanage.qb.get_query("DocType", filters={"istable": 1}, ignore_permissions=False)

		result = dontmanage.qb.get_query("DocType", filters={"istable": 1}, ignore_permissions=True).run()
	...
	def test_child_table_access_with_select_permission(self):
		...
		# Cleanup
		dontmanage.set_user("Administrator")
		test_user = dontmanage.get_doc("User", test_user_email)
		test_user.remove_roles(test_role)
		dontmanage.delete_doc("Role", test_role, ignore_missing=True, force=True)
		dontmanage.delete_doc("Note", {"title": test_note_title}, ignore_missing=True, force=True)

		# Setup Role with 'select' on Note and 'read' on Note Seen By
		dontmanage.get_doc({"doctype": "Role", "role_name": test_role}).insert(ignore_if_duplicate=True)
		...
		dontmanage.set_user(test_user_email)
		...
		test_user.remove_roles(test_role)
		dontmanage.delete_doc("Role", test_role, force=True)
	...
	def test_nested_permission(self):
		...
		with self.set_user("test2@example.com"):
			data = dontmanage.qb.get_query("Nested DocType", ignore_permissions=False).run(as_dict=1)
	...
	def test_is_set_is_not_set(self):
		...
		result = dontmanage.qb.get_query("DocType", filters={"autoname": ["is", "not set"]}).run(as_dict=1)
		...
		result = dontmanage.qb.get_query("DocType", filters={"autoname": ["is", "set"]}).run(as_dict=1)
	...
	def test_permission_query_condition(self):
		...
		original_hooks = dontmanage.get_hooks("permission_query_conditions") or {}
		...
		query = dontmanage.qb.get_query("Dashboard Settings", user=self.user, ignore_permissions=False)
		self.assertIn("`tabDashboard Settings`.name = ", str(query))

		# Create a server script for permission query
		script = dontmanage.new_doc(
		...
		dontmanage.hooks.permission_query_conditions = {}  # Clear hooks to test server script alone
		...
		script.delete()
		dontmanage.clear_cache()
		dontmanage.hooks.permission_query_conditions = original_hooks
	...
	def test_link_field_target_permission(self):
		...
		dontmanage.set_user("Administrator")
		dontmanage.delete_doc("DocType", target_dt_name, ignore_missing=True, force=True)
		dontmanage.delete_doc("DocType", source_dt_name, ignore_missing=True, force=True)
		dontmanage.delete_doc("Role", test_role, ignore_missing=True, force=True)
		test_user_doc = dontmanage.get_doc("User", test_user)
		...
		# Create Records
		target_doc = dontmanage.get_doc(
			doctype=target_dt_name, target_field="Secret Data", other_target_field="Public Data"
		).insert(ignore_permissions=True)
		source_doc = dontmanage.get_doc(doctype=source_dt_name, link_field=target_doc.name).insert(
		...
		result_restricted = dontmanage.qb.get_query(
			source_dt_name,
		...
		dontmanage.set_user("Administrator")
		result_admin = dontmanage.qb.get_query(
			source_dt_name,
			filters={"name": source_doc.name},
			fields=[
				"name",
	...
	def test_filter_direct_field_permission(self):
		...
		with setup_patched_blog_post(), setup_test_user(set_user=True) as user:
			...
			with self.assertRaises(dontmanage.PermissionError) as cm:
				dontmanage.qb.get_query(
					"Test Blog Post",
					filters={"published": 1},
					ignore_permissions=False,
	...
	def test_filter_linked_field_permission(self):
		...
		with setup_test_user(set_user=True) as user:
			...
			dontmanage.delete_doc("DocType", source_dt_name, ignore_missing=True, force=True)
			dontmanage.delete_doc("Role", test_role, ignore_missing=True, force=True)
			test_user_doc = dontmanage.get_doc("User", user.name)
			test_user_doc.remove_roles(test_role)
			...
			with self.assertRaises(dontmanage.PermissionError) as cm_link:
				dontmanage.qb.get_query(
					source_dt_name,
	...
	def test_child_table_group_by_order_by_permissions(self):
		...
		dontmanage.set_user("Administrator")
		dontmanage.delete_doc("DocType", child_dt_name, ignore_missing=True, force=True)
		dontmanage.delete_doc("DocType", parent_dt_name, ignore_missing=True, force=True)
		dontmanage.delete_doc("Role", test_role, ignore_missing=True, force=True)

		test_user_doc = dontmanage.get_doc("User", test_user_email)
		test_user_doc.remove_roles(test_role)
	...
	def test_group_by_order_by_validation_errors(self):
		...
		for field in invalid_group_by_fields:
			with self.assertRaises(
				dontmanage.ValidationError, msg=f"Invalid GROUP BY field '{field}' passed validation"
			):
				dontmanage.qb.get_query("User", group_by=field).get_sql()
		...
		for field in invalid_order_by_fields:
			with self.assertRaises(
				(dontmanage.ValidationError, ValueError),
				msg=f"Invalid ORDER BY field '{field}' passed validation",
			):
				dontmanage.qb.get_query("User", order_by=field).get_sql()
	...
	def test_backtick_rejection_group_order(self):
		...
		with self.assertRaises(dontmanage.ValidationError) as cm:
			dontmanage.qb.get_query("User", group_by="`name`").get_sql()
		self.assertIn("invalid backtick notation", str(cm.exception))

		# Test single backtick with direction (invalid notation)
		with self.assertRaises(dontmanage.ValidationError) as cm:
			dontmanage.qb.get_query("User", order_by="`name` ASC").get_sql()
		self.assertIn("invalid backtick notation", str(cm.exception))

		# Test multiple single backticks (invalid notation)
		with self.assertRaises(dontmanage.ValidationError) as cm:
			dontmanage.qb.get_query("User", group_by="`name`, `email`").get_sql()
		self.assertIn("invalid backtick notation", str(cm.exception))

		# Valid backtick notation should work
		dontmanage.qb.get_query("User", group_by="`tabUser`.`name`").get_sql()
		dontmanage.qb.get_query("User", order_by="`tabUser`.`name` ASC").get_sql()
	...
	def test_sql_functions_in_fields(self):
		...
		query = dontmanage.qb.get_query("User", fields=["user_type", {"COUNT": "name"}], group_by="user_type")
		sql = query.get_sql()
		...
		# Test ABS function
		query = dontmanage.qb.get_query("User", fields=[{"ABS": "enabled", "as": "abs_enabled"}])
		sql = query.get_sql()
		...
		query = dontmanage.qb.get_query("User", fields=[{"TIMESTAMP": "creation", "as": "ts"}])
		sql = query.get_sql()
		...
		query = dontmanage.qb.get_query("User", fields=[{"NOW": None, "as": "current_time"}])
		sql = query.get_sql()
		...
		with self.assertRaises(dontmanage.ValidationError) as cm:
			dontmanage.qb.get_query("User", fields=[{"UNSUPPORTED_FUNC": "name"}]).get_sql()
		...
		with self.assertRaises(dontmanage.ValidationError) as cm:
			dontmanage.qb.get_query("User", fields=[{"UPPER": ["first_name"]}]).get_sql()
		self.assertIn("Unsupported function or operator: UPPER", str(cm.exception))

		# Test SQL injection attempt
		with self.assertRaises(dontmanage.ValidationError) as cm:
			dontmanage.qb.get_query("User", fields=[{"DROP": "TABLE users"}]).get_sql()
	...
	def test_arithmetic_operators_in_fields(self):
		...
		query = dontmanage.qb.get_query("User", fields=[{"ADD": [1, 2], "as": "sum_result"}])
		sql = query.get_sql()
		self.assertIn(self.normalize_sql("1+2 `sum_result`"), sql)

		# Test simple subtraction
		query = dontmanage.qb.get_query("User", fields=[{"SUB": [10, 5], "as": "diff_result"}])
		sql = query.get_sql()
		self.assertIn(self.normalize_sql("10-5 `diff_result`"), sql)

		# Test simple multiplication
		query = dontmanage.qb.get_query("User", fields=[{"MUL": [3, 4], "as": "prod_result"}])
		sql = query.get_sql()
		self.assertIn(self.normalize_sql("3*4 `prod_result`"), sql)

		# Test simple division
		query = dontmanage.qb.get_query("User", fields=[{"DIV": [10, 2], "as": "div_result"}])
		sql = query.get_sql()
		self.assertIn(self.normalize_sql("10/2 `div_result`"), sql)

		# Test operator with field names
		query = dontmanage.qb.get_query("User", fields=[{"ADD": ["enabled", "login_after"], "as": "field_sum"}])
		sql = query.get_sql()
		self.assertIn(self.normalize_sql("`enabled`+`login_after` `field_sum`"), sql)

		# Test nested operators
		query = dontmanage.qb.get_query("User", fields=[{"ADD": [{"MUL": [2, 3]}, 4], "as": "nested_result"}])
		sql = query.get_sql()
		...
		elif dontmanage.db.db_type == "sqlite":
			self.assertIn(
	...
	def test_field_alias_in_group_by(self):
		...
		if (
			dontmanage.db.db_type != "postgres"
	...
	def test_share_only_access(self):
		...
		self.addCleanup(lambda: dontmanage.set_user("Administrator"))

		# Verify user can't access without share
		dontmanage.set_user(test_user)
		result = dontmanage.qb.get_query("Event", filters={"name": event.name}, ignore_permissions=False).run()
		self.assertEqual(len(result), 0, "User should not see event without share")

		# Share the document
		dontmanage.set_user("Administrator")
		dontmanage.share.add("Event", event.name, test_user)
	...
	def test_if_owner_constraint_with_shared_docs(self):
		...
		# Share the document
		dontmanage.set_user("Administrator")
		dontmanage.share.add("Test Blog Post", blog_post.name, test_user)
	...
	def test_user_permission_with_shared_docs(self):
		...
		dontmanage.set_user("Administrator")
		dontmanage.share.add("Test Blog Post", blog_post2.name, test_user)
	...
	def test_role_permission_without_restrictions(self):
		...
		from dontmanage.core.page.permission_manager.permission_manager import update
		from dontmanage.permissions import clear_user_permissions_for_doctype
	...
	def test_child_table_filters_orphaned_rows(self):
		...
		self.addCleanup(
			lambda: dontmanage.db.sql("DELETE FROM `tabDefaultValue` WHERE name = '_test_orphan_row'")
		)
		self.addCleanup(lambda: dontmanage.set_user("Administrator"))
	...
	def test_combined_raw_criterion_precedence(self):
		...
		"""
		from dontmanage.database.query import CombinedRawCriterion, RawCriterion
	...
	def test_ifnull_fallback_postgres(self):
		...
		from dontmanage.database.query import Engine

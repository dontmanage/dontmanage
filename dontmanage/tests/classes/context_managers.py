...
if TYPE_CHECKING:
	from dontmanage.model import Document
...
def freeze_time(time_to_freeze: Any, is_utc: bool = False, *args: Any, **kwargs: Any) -> None:
	...
	from dontmanage.utils.data import get_datetime, get_system_timezone
...
def enable_safe_exec() -> None:
	...
	conf = os.path.join(dontmanage.local.sites_path, "common_site_config.json")
...
def trace_fields(
	...
	**field_configs: dict[str, list | Callable | None],
	...
	Yields:
	    Document class
	"""
	from dontmanage.model.trace import traced_field
...
# NOTE: declare those who should also be made available directly dontmanage.tests.* namespace

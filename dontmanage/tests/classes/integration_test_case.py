...
import dontmanage
from dontmanage.database.utils import get_query_type
...
class IntegrationTestCase(UnitTestCase):
	"""Integration test class for DontManage tests.
	...
	def setUpClass(cls) -> None:
		...
		if cls.doctype and cls.doctype not in dontmanage.local.test_objects:
		...
		cls.globalTestRecords = MappingProxyType(dontmanage.local.test_objects)
		if cls.SHOW_TRANSACTION_COMMIT_WARNINGS:
			dontmanage.db.before_commit.add(_commit_watcher)

		# enqueue teardown actions (executed in LIFO order)
		cls.addClassCleanup(_restore_ctx_locals, copy.deepcopy(dontmanage.local.flags))
...
def _restore_ctx_locals(flags):
	...
	dontmanage.local.response = dontmanage._dict({"docs": []})

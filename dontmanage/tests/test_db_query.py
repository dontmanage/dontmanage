...
class TestDBQuery(IntegrationTestCase):
	...
	def test_string_as_field(self):
		self.assertEqual(
			dontmanage.get_all("DocType", as_list=True), dontmanage.get_all("DocType", fields="name", as_list=True)
		)
	...
	def test_query_fields_sanitizer(self):
		...
		from dontmanage.desk.search import search_link
	...
	def test_virtual_doctype(self):
		...
		class VirtualDocType:
			...
			def get_list(args=None, limit_page_length=0, doctype=None):
				from dontmanage.types.filter import FilterTuple
	...
	def test_coalesce_with_in_ops(self):
		self.assertNotIn("IF", dontmanage.get_all("User", {"first_name": ("in", ["a", "b"])}, run=0).get_sql())
		self.assertIn("IFNULL", dontmanage.get_all("User", {"first_name": ("in", ["a", None])}, run=0).get_sql())
		self.assertIn("IFNULL", dontmanage.get_all("User", {"first_name": ("in", ["a", ""])}, run=0).get_sql())
		self.assertIn("IFNULL", dontmanage.get_all("User", {"first_name": ("in", [])}, run=0).get_sql())
		self.assertIn("IFNULL", dontmanage.get_all("User", {"first_name": ("not in", ["a"])}, run=0).get_sql())
		self.assertIn("IFNULL", dontmanage.get_all("User", {"first_name": ("not in", [])}, run=0).get_sql())
		self.assertIn("IFNULL", dontmanage.get_all("User", {"first_name": ("not in", [""])}, run=0).get_sql())

		# primary key is never nullable
		self.assertNotIn("IFNULL", dontmanage.get_all("User", {"name": ("in", ["a", None])}, run=0).get_sql())
		self.assertNotIn("IFNULL", dontmanage.get_all("User", {"name": ("in", ["a", ""])}, run=0).get_sql())
		self.assertNotIn("IFNULL", dontmanage.get_all("User", {"name": ("in", (""))}, run=0).get_sql())
		self.assertNotIn("IFNULL", dontmanage.get_all("User", {"name": ("in", ())}, run=0).get_sql())

	def test_coalesce_with_datetime_ops(self):
		self.assertNotIn(
			"IFNULL", dontmanage.get_all("User", {"last_active": (">", "2022-01-01")}, run=0).get_sql()
		)
		self.assertNotIn("IFNULL", dontmanage.get_all("User", {"creation": ("<", "2022-01-01")}, run=0).get_sql())
		self.assertNotIn(
			"IFNULL",
			dontmanage.get_all(
		...
		self.assertIn("IFNULL", dontmanage.get_all("User", {"last_active": ("<", "2022-01-01")}, run=0).get_sql())
	...
	def test_select_star_expansion(self):
		count = dontmanage.get_list("Language", [{"SUM": 1}, {"COUNT": "*"}], as_list=1, order_by=None)[0]
		self.assertEqual(count[0], dontmanage.db.count("Language"))
		self.assertEqual(count[1], dontmanage.db.count("Language"))

	def test_ifnull_none(self):
		query = dontmanage.get_all("DocField", {"fieldname": None}, run=0).get_sql()
		self.assertIn("IS NULL", query)
		self.assertNotIn("\\'", query)
		self.assertNotIn("ifnull", query)
		self.assertFalse(dontmanage.get_all("DocField", {"name": None}))
		self.assertFalse(dontmanage.get_all("DocField", {"parent": None}))
		self.assertNotIn("0", dontmanage.get_all("DocField", {"parent": None}, run=0).get_sql())
...
class TestReportView(IntegrationTestCase):
	def test_get_count(self):
		...
		count = execute_cmd("dontmanage.desk.reportview.get_count")
		dontmanage.local.form_dict = dontmanage._dict(
			{
				"doctype": "DocType",
	...
	def test_reportview_child_table_alias(self):
		from dontmanage.desk import reportview
	...
	def test_reportview_get_permlevel_system_users(self):
		with setup_patched_blog_post(), setup_test_user(set_user=True):
			...
			data = dontmanage.get_list("Test Blog Post", "published")
			self.assertTrue(len(data) > 0)
	...
	def test_db_filter_not_set(self):
		...
		"""
		dontmanage.get_doc({"doctype": "ToDo", "description": "filter test"}).insert()
		dontmanage.get_doc({"doctype": "ToDo", "description": "filter test", "reference_name": ""}).insert()
		...
		self.assertEqual(
			len(dontmanage.get_all("ToDo", filters={"reference_name": ["is", "not set"]})),
			dontmanage.db.count("ToDo", {"reference_name": ["is", "not set"]}),
		)

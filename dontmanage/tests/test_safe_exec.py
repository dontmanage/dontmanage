...
class TestSafeExec(IntegrationTestCase):
	...
	def test_print(self):
		test_str = dontmanage.generate_hash()
		safe_exec(f"print('{test_str}')")
		self.assertEqual(dontmanage.local.debug_log[-1], test_str)
...
class TestJinjaGlobals(IntegrationTestCase):
	def test_jenv_thread_safety(self):
		...
		# thread.
		dontmanage.init(dontmanage.local.site, force=True)
		second = get_jenv()
		self.assertIsNot(first, second)
		self.assertIsNot(first.globals, second.globals)
		self.assertIsNot(first.filters, second.filters)
		self.assertIsNot(first.globals["dontmanage"], second.globals["dontmanage"])
		self.assertIsNot(first.globals["dontmanage"]["form_dict"], second.globals["dontmanage"]["form_dict"])

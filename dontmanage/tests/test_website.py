...
from dontmanage.website.serve import get_response, get_response_content
from dontmanage.website.utils import build_response, clear_website_cache, get_home_page
...
class TestWebsite(DontManageTestCase):
	...
	def test_home_page(self):
		...
		dontmanage.db.set_value("Role", "home-page-test", "home_page", "home-page-test")
		...
		with patch.object(
			...
			"get_hooks",
			patched_get_hooks(
				"get_website_user_home_page", ["dontmanage.www._test._test_home_page.get_website_user_home_page"]
			),
		):
			self.assertEqual(get_home_page(), "_test/_test_folder")

		clear_website_cache()
		with patch.object(dontmanage, "get_hooks", patched_get_hooks("website_user_home_page", ["login"])):
			self.assertEqual(get_home_page(), "login")

		clear_website_cache()
		with patch.object(dontmanage, "get_hooks", patched_get_hooks("home_page", ["about"])):
			self.assertEqual(get_home_page(), "about")

		clear_website_cache()
		with patch.object(
			dontmanage, "get_hooks", patched_get_hooks("role_home_page", {"home-page-test": ["home-page-test"]})
		):
			self.assertEqual(get_home_page(), "home-page-test")
	...
	def test_app(self):
		...
		dontmanage.local.session_obj = None
	...
	def test_redirect(self):
		import dontmanage.hooks

		dontmanage.set_user("Administrator")

		dontmanage.hooks.website_redirects = [
	...
	def test_custom_page_renderer(self):
		...
		def patched_get_hooks(*args, **kwargs):
			...
			if args and args[0] == "page_renderer":
				return_value = ["dontmanage.tests.test_website.CustomPageRenderer"]
			return return_value

		with patch.object(dontmanage, "get_hooks", patched_get_hooks):
	...
	def test_json_sidebar_data(self):
		dontmanage.flags.look_for_sidebar = False
		content = get_response_content("/_test/_test_folder/_test_page")
		self.assertNotIn("Test Sidebar", content)
		clear_website_cache()
		dontmanage.flags.look_for_sidebar = True
		content = get_response_content("/_test/_test_folder/_test_page")
	...
	def test_safe_render(self):
		...
		self.assertNotIn("dontmanage.exceptions.ValidationError: Illegal template", content)

	def test_never_render(self):
		from pathlib import Path
		from random import choices

		WWW = Path(dontmanage.get_app_path("dontmanage")) / "www"
	...
	def test_resolve_class(self):
		from dontmanage.utils.jinja_globals import resolve_class

		context = dontmanage._dict(primary=True)
		...
		self.assertEqual(
			dontmanage.render_template(content, context), '<a class="btn btn-default btn-primary">Test</a>'
		)
	...
	def test_app_include(self):
		...
		with patch.object(dontmanage, "get_hooks", patched_get_hooks):
			dontmanage.set_user("Administrator")
			dontmanage.hooks.app_include_js.append("test_app_include.js")
			dontmanage.hooks.app_include_css.append("test_app_include.css")
			dontmanage.conf.update({"app_include_js": ["test_app_include_via_site_config.js"]})
			dontmanage.conf.update({"app_include_css": ["test_app_include_via_site_config.css"]})

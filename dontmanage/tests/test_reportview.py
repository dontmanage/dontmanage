...
import dontmanage
from dontmanage.desk.reportview import export_query, extract_fieldnames
...
class TestReportview(IntegrationTestCase):
	...
	def test_export_report_via_email(self):
		...
		export_query()
		jobs = dontmanage.get_all(
			"RQ Job",
			filters={"job_name": "dontmanage.desk.query_report.run_report_view_export_job"},
			fields=["name", "status"],
		)

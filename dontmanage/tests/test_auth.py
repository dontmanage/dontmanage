...
class TestAuth(IntegrationTestCase):
	...
	def test_allow_login_using_mobile(self):
		...
		DontManageClient(self.HOST_NAME, self.test_user_mobile, self.test_user_password)
		DontManageClient(self.HOST_NAME, self.test_user_email, self.test_user_password)

		# login by username should fail
		with self.assertRaises(AuthError):
			DontManageClient(self.HOST_NAME, self.test_user_name, self.test_user_password)
	...
	def test_deny_multiple_login(self):
		...
		first_login = DontManageClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		first_login.get_list("ToDo")

		second_login = DontManageClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		second_login.get_list("ToDo")
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")

		third_login = DontManageClient(self.HOST_NAME, self.test_user_email, self.test_user_password)
		with self.assertRaises(Exception):
			first_login.get_list("ToDo")
		with self.assertRaises(Exception):
			second_login.get_list("ToDo")
...
class TestAllowedReferrer(UnitTestCase):
	def test_is_allowed_referrer(self):
		...
		dontmanage.cache.set_value("allowed_referrers", ["https://example.com"])

		# Test with valid referrer
		dontmanage.local.request = create_request({"Referer": "https://example.com/some/path"})
		http_request = dontmanage.auth.HTTPRequest()
		self.assertTrue(http_request.is_allowed_referrer())

		# Test with invalid referrer
		dontmanage.local.request = create_request({"Referer": "https://malicious.com"})
		http_request = dontmanage.auth.HTTPRequest()
		self.assertFalse(http_request.is_allowed_referrer())

		# Test with valid origin
		dontmanage.local.request = create_request({"Origin": "https://example.com"})
		http_request = dontmanage.auth.HTTPRequest()
		self.assertTrue(http_request.is_allowed_referrer())

		# Test with invalid origin
		dontmanage.local.request = create_request({"Origin": "https://malicious.com"})
		http_request = dontmanage.auth.HTTPRequest()
		self.assertFalse(http_request.is_allowed_referrer())

		# Test subdomain bypass prevention
		dontmanage.local.request = create_request({"Referer": "https://example.com.evil.com"})
		http_request = dontmanage.auth.HTTPRequest()
		self.assertFalse(http_request.is_allowed_referrer())

		# Test exact domain match for referrer
		dontmanage.local.request = create_request({"Referer": "https://example.com"})
		http_request = dontmanage.auth.HTTPRequest()
		self.assertTrue(http_request.is_allowed_referrer())

		# Clean up
		dontmanage.cache.delete_value("allowed_referrers")
...
class TestSessionExpiry(DontManageAPITestCase):

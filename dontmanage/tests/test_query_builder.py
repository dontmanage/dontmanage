...
class TestCustomFunctionsMariaDB(DontManageTestCase):
	...
	def test_time(self):
		...
		select_query = dontmanage.qb.from_(note).select(
			CombineDatetime(note.posting_date, note.posting_time)
		)
...
class TestMisc(DontManageTestCase):
	...
	def test_error_on_query_class(self):
		import dontmanage.query_builder.utils

		dontmanage.query_builder.utils.get_type_hints = lambda x: {"return": None}

		with self.assertRaises(dontmanage.query_builder.utils.BuilderIdentificationFailed):
			dontmanage.query_builder.utils.patch_query_execute()

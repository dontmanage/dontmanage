...
from dontmanage.query_builder.builder import Function
from dontmanage.query_builder.custom import ConstantColumn
from dontmanage.query_builder.functions import (
	Cast_,
	Coalesce,
	CombineDatetime,
	Date,
...
def run_only_if(dbtype: db_type_is) -> Callable:
	return unittest.skipIf(db_type_is(dontmanage.conf.db_type) != dbtype, f"Only runs for {dbtype.value}")


@run_only_if(db_type_is.MARIADB)
class TestCustomFunctionsMariaDB(DontManageTestCase):
	...
	def test_constant_column(self):
		query = dontmanage.qb.from_("DocType").select("name", ConstantColumn("John").as_("User"))
	...
	def test_time(self):
		...
		select_query = dontmanage.qb.from_(note).select(CombineDatetime(note.posting_date, note.posting_time))
	...
	def test_cast(self):
		...
		self.assertEqual(
			dontmanage.qb.from_("red").from_(note).select("other", Cast_(note.name, "varchar")).get_sql(),
	...
	def test_round(self):
		...
		query = dontmanage.qb.from_(note).select(Round(note.price))
		self.assertEqual("select round(`price`,0) from `tabnote`", str(query).lower())

		query = dontmanage.qb.from_(note).select(Round(note.price, 3))
		self.assertEqual("select round(`price`,3) from `tabnote`", str(query).lower())

	def test_truncate(self):
		note = dontmanage.qb.DocType("Note")
		query = dontmanage.qb.from_(note).select(Truncate(note.price, 3))
		self.assertEqual("select truncate(`price`,3) from `tabnote`", str(query).lower())


@run_only_if(db_type_is.POSTGRES)
class TestCustomFunctionsPostgres(DontManageTestCase):
	def test_concat(self):
...
class TestBuilderBase:
	def test_adding_tabs(self):
		self.assertEqual("tabNotes", dontmanage.qb.DocType("Notes").get_sql())
		self.assertEqual("__Auth", dontmanage.qb.DocType("__Auth").get_sql())
		self.assertEqual("Notes", dontmanage.qb.Table("Notes").get_sql())

	def test_run_patcher(self):
		query = dontmanage.qb.from_("ToDo").select("*").limit(1)
	...
	def test_agg_funcs(self):
		dontmanage.db.truncate("Communication")
		sample_data = {
		...
		dontmanage.get_doc(sample_data).insert()
		sample_data["rating"] = 3
		dontmanage.get_doc(sample_data).insert()
		sample_data["rating"] = 4
		dontmanage.get_doc(sample_data).insert()
		self.assertEqual(dontmanage.qb.max("Communication", "rating"), 4)
		self.assertEqual(dontmanage.qb.min("Communication", "rating"), 1)
		self.assertAlmostEqual(dontmanage.qb.avg("Communication", "rating"), 2.666, places=2)
		self.assertEqual(dontmanage.qb.sum("Communication", "rating"), 8.0)
		dontmanage.db.rollback()


class TestParameterization(DontManageTestCase):
	def test_where_conditions(self):
		DocType = dontmanage.qb.DocType("DocType")
		query = dontmanage.qb.from_(DocType).select(DocType.name).where(DocType.owner == "Administrator' --")
	...
	def test_set_conditions(self):
		DocType = dontmanage.qb.DocType("DocType")
		query = dontmanage.qb.update(DocType).set(DocType.value, "some_value")
	...
	def test_where_conditions_functions(self):
		DocType = dontmanage.qb.DocType("DocType")
		query = (
			dontmanage.qb.from_(DocType).select(DocType.name).where(Coalesce(DocType.search_fields == "subject"))
		)
	...
	def test_case(self):
		DocType = dontmanage.qb.DocType("DocType")
		query = dontmanage.qb.from_(DocType).select(
			Case()
	...
	def test_case_in_update(self):
		DocType = dontmanage.qb.DocType("DocType")
		query = dontmanage.qb.update(DocType).set(
			"parent",
			Case()
			.when(DocType.search_fields == "value", "other_value")
			.when(Coalesce(DocType.search_fields == "subject_in_function"), "true_value")
	...
	def test_named_parameter_wrapper(self):
		from dontmanage.query_builder.terms import NamedParameterWrapper
...
@run_only_if(db_type_is.MARIADB)
class TestBuilderMaria(DontManageTestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual("SELECT * FROM `tabNotes`", dontmanage.qb.from_("Notes").select("*").get_sql())
		self.assertEqual("SELECT * FROM `__Auth`", dontmanage.qb.from_("__Auth").select("*").get_sql())

	def test_get_qb_type(self):
		from dontmanage.query_builder import get_query_builder

		qb = get_query_builder(dontmanage.db.db_type)
		self.assertEqual("SELECT * FROM `tabDocType`", qb().from_("DocType").select("*").get_sql())


@run_only_if(db_type_is.POSTGRES)
class TestBuilderPostgres(DontManageTestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual('SELECT * FROM "tabNotes"', dontmanage.qb.from_("Notes").select("*").get_sql())
		self.assertEqual('SELECT * FROM "__Auth"', dontmanage.qb.from_("__Auth").select("*").get_sql())
	...
	def test_replace_tables(self):
		...
		self.assertEqual(
			...
			dontmanage.qb.from_(info_schema.tables).select("*").get_sql(),
		)

	def test_replace_fields_post(self):
		self.assertEqual("relname", dontmanage.qb.Field("table_name").get_sql())
...
class TestMisc(DontManageTestCase):
	...
	def test_union(self):
		...
		users = dontmanage.qb.from_(user).select(user.name)
		roles = dontmanage.qb.from_(role).select(role.name)

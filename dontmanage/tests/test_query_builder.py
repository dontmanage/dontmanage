...
def unimplemented_for(*dbtypes: db_type_is) -> Callable:
	current_db_type = db_type_is(dontmanage.conf.db_type)
...
class TestBuilderBase:
	...
	def test_agg_funcs(self):
		...
		dontmanage.db.truncate(self.doctype_name)
		sample_data = {
			"doctype": self.doctype_name,
			"number": 1,
		}
		dontmanage.get_doc(sample_data).insert(ignore_mandatory=True)
		sample_data["number"] = 3
		dontmanage.get_doc(sample_data).insert(ignore_mandatory=True)
		sample_data["number"] = 4
		dontmanage.get_doc(sample_data).insert(ignore_mandatory=True)
		self.assertEqual(dontmanage.qb.max(self.doctype_name, "number"), 4)
		self.assertEqual(dontmanage.qb.min(self.doctype_name, "number"), 1)
		self.assertAlmostEqual(dontmanage.qb.avg(self.doctype_name, "number"), 2.666, places=2)
		self.assertEqual(dontmanage.qb.sum(self.doctype_name, "number"), 8.0)

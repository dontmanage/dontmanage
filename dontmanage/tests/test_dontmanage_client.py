...
from dontmanage.dontmanageclient import AuthError, DontManageClient, DontManageException
from dontmanage.utils.data import get_url


class TestDontManageClient(unittest.TestCase):
	PASSWORD = dontmanage.conf.admin_password or "admin"

	@classmethod
	def setUpClass(cls) -> None:
		site_url = get_url()
		try:
			DontManageClient(site_url, "Administrator", cls.PASSWORD, verify=False)
		except AuthError:
	...
	def test_insert_many(self):
		server = DontManageClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		dontmanage.db.delete("Note", {"title": ("in", ("Sing", "a", "song", "of", "sixpence"))})
		...
		self.assertTrue(dontmanage.db.get_value("Note", {"title": "Sing"}))
		self.assertTrue(dontmanage.db.get_value("Note", {"title": "a"}))
		self.assertTrue(dontmanage.db.get_value("Note", {"title": "song"}))
		self.assertTrue(dontmanage.db.get_value("Note", {"title": "of"}))
		self.assertTrue(dontmanage.db.get_value("Note", {"title": "sixpence"}))

	def test_create_doc(self):
		server = DontManageClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		dontmanage.db.delete("Note", {"title": "test_create"})
		dontmanage.db.commit()

		server.insert({"doctype": "Note", "public": True, "title": "test_create"})

		self.assertTrue(dontmanage.db.get_value("Note", {"title": "test_create"}))
	...
	def test_get_doc(self):
		server = DontManageClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		dontmanage.db.delete("Note", {"title": "get_this"})
		dontmanage.db.commit()
	...
	def test_get_value(self):
		server = DontManageClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		dontmanage.db.delete("Note", {"title": "get_value"})
		dontmanage.db.commit()
	...
	def test_get_single(self):
		...
		dontmanage.db.set_value("Website Settings", None, "title_prefix", "")

	def test_update_doc(self):
		server = DontManageClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		dontmanage.db.delete("Note", {"title": ("in", ("Sing", "sing"))})
		dontmanage.db.commit()
	...
	def test_update_child_doc(self):
		server = DontManageClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		dontmanage.db.delete("Contact", {"first_name": "George", "last_name": "Steevens"})
		dontmanage.db.delete("Contact", {"first_name": "William", "last_name": "Shakespeare"})
		dontmanage.db.delete("Communication", {"reference_doctype": "Event"})
		dontmanage.db.delete("Communication Link", {"link_doctype": "Contact"})
		dontmanage.db.delete("Event", {"subject": "Sing a song of sixpence"})
		dontmanage.db.delete("Event Participants", {"reference_doctype": "Contact"})
		dontmanage.db.commit()
		...
		self.assertTrue(dontmanage.db.exists("Communication Link", {"link_name": "William Shakespeare"}))

	def test_delete_doc(self):
		server = DontManageClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		dontmanage.db.delete("Note", {"title": "delete"})
		dontmanage.db.commit()
		...
		self.assertFalse(dontmanage.db.get_value("Note", {"title": "delete"}))

	def test_auth_via_api_key_secret(self):
		# generate API key and API secret for administrator
		keys = generate_keys("Administrator")
		dontmanage.db.commit()
		generated_secret = dontmanage.utils.password.get_decrypted_password(
			"User", "Administrator", fieldname="api_secret"
		)

		api_key = dontmanage.db.get_value("User", "Administrator", "api_key")
		header = {"Authorization": f"token {api_key}:{generated_secret}"}
		res = requests.post(get_url() + "/api/method/dontmanage.auth.get_logged_user", headers=header)
		...
		header = {
			"Authorization": "Basic {}".format(
				base64.b64encode(dontmanage.safe_encode(f"{api_key}:{generated_secret}")).decode()
			)
		}
		res = requests.post(get_url() + "/api/method/dontmanage.auth.get_logged_user", headers=header)
		self.assertEqual(res.status_code, 200)

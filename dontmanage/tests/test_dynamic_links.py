...
class TestDynamicLinks(DontManageTestCase):
	def setUp(self):
	...
	def test_delete_normal(self):
		...
		self.assertFalse(dontmanage.db.exists("Email Unsubscribe", unsub.name))
	...
	def test_delete_with_comment(self):
		...
		self.assertTrue(
			dontmanage.get_all("Comment", filters={"reference_doctype": "Event", "reference_name": event.name})
		)
		event.delete()
		self.assertFalse(
			dontmanage.get_all("Comment", filters={"reference_doctype": "Event", "reference_name": event.name})
	...
	def test_custom_fields(self):
		...
		self.assertRaises(dontmanage.LinkExistsError, unsub.delete)
		...
		dontmanage.db.commit()  # undo changes done by DDL

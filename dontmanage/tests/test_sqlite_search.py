...
import dontmanage
from dontmanage.search.sqlite_search import SQLiteSearch, SQLiteSearchIndexMissingError
...
class TestSQLiteSearch(SQLiteSearch):
	...
	def get_search_filters(self):
		...
		return {"owner": dontmanage.session.user}
...
class TestSQLiteSearchAPI(IntegrationTestCase):
	...
	def setUp(self):
		...
		for data in note_data:
			note = dontmanage.get_doc({"doctype": "Note", "title": data["title"], "content": data["content"]})
			note.insert()
		...
		for data in todo_data:
			todo = dontmanage.get_doc({"doctype": "ToDo", "description": data["description"], "status": "Open"})
			todo.insert()
	...
	def test_search_filtering_and_permissions(self):
		...
		original_user = dontmanage.session.user
		try:
			# Create a test user and switch to them
			test_user_email = "test_search_user@example.com"
			if not dontmanage.db.exists("User", test_user_email):
				test_user = dontmanage.get_doc(
					{
						"doctype": "User",
			...
			dontmanage.set_user(original_user)
	...
	@patch("dontmanage.enqueue")
	def test_background_operations(self, mock_enqueue):
		"""Test background job integration and module-level functions."""
		from dontmanage.search.sqlite_search import (
			build_index_in_background,
			get_search_classes,
		)

		# Test getting search classes
		with patch("dontmanage.get_hooks") as mock_get_hooks:
			mock_get_hooks.return_value = ["dontmanage.tests.test_sqlite_search.TestSQLiteSearch"]

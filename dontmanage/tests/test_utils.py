...
from dontmanage.tests import IntegrationTestCase, MockedRequestTestCase, UnitTestCase
from dontmanage.tests.utils import toggle_test_mode
...
from dontmanage.utils.change_log import (
	get_source_url,
	parse_github_url,
)
...
class TestValidationUtils(IntegrationTestCase):
	...
	def test_valid_email(self):
		...
		# Invalid with throw
		self.assertRaises(
			dontmanage.InvalidEmailAddressError,
			validate_email_address,
			"someone.com",
			throw=True,
		)
...
class TestImage(IntegrationTestCase):
	def test_strip_exif_data(self):
		...
		original_image_content = open(
			dontmanage.get_app_path("dontmanage", "tests", "data", "exif_sample_image.jpg"),
			mode="rb",
		).read()
...
class TestDateUtils(IntegrationTestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(dontmanage.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				dontmanage.utils.get_first_day_of_week("2020-12-25"),
				dontmanage.utils.getdate("2020-12-21"),
			)
			self.assertEqual(
				dontmanage.utils.get_first_day_of_week("2020-12-20"),
				dontmanage.utils.getdate("2020-12-14"),
			)

		# Sunday as start of the week
		self.assertEqual(
			dontmanage.utils.get_first_day_of_week("2020-12-25"),
			dontmanage.utils.getdate("2020-12-20"),
		)
		self.assertEqual(
			dontmanage.utils.get_first_day_of_week("2020-12-21"),
			dontmanage.utils.getdate("2020-12-20"),
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			dontmanage.utils.get_last_day_of_week("2020-12-24"),
			dontmanage.utils.getdate("2020-12-26"),
		)
		self.assertEqual(
			dontmanage.utils.get_last_day_of_week("2020-12-28"),
			dontmanage.utils.getdate("2021-01-02"),
		)
...
class TestChangeLog(IntegrationTestCase):
	def test_get_remote_url(self):
		self.assertIsInstance(get_source_url("dontmanage"), str)

	def test_parse_github_url(self):
		# using dontmanageerp as repo in order to be different from the owner
		owner, repo = parse_github_url("https://github.com/dontmanage/dontmanageerp.git")
		self.assertEqual(owner, "dontmanage")
		self.assertEqual(repo, "dontmanageerp")

		owner, repo = parse_github_url("https://github.com/dontmanage/dontmanageerp")
		self.assertEqual(owner, "dontmanage")
		self.assertEqual(repo, "dontmanageerp")

		owner, repo = parse_github_url("git@github.com:dontmanage/dontmanageerp.git")
		self.assertEqual(owner, "dontmanage")
		self.assertEqual(repo, "dontmanageerp")
...
class TestURLTrackers(IntegrationTestCase):
	def test_add_trackers_to_url(self):
		...
		with patch("dontmanage.db.get_value") as mock_get_value:
	...
	def test_map_trackers(self):
		...
		expected = {
			"utm_source": dontmanage.get_doc("UTM Source", "test_source"),
			"utm_medium": dontmanage.get_doc("UTM Medium", "test_medium"),
			"utm_campaign": dontmanage.get_doc("UTM Campaign", "test_campaign"),
			"utm_content": "test_content",
		}

...
from dontmanage.tests.test_query_builder import db_type_is, run_only_if, unimplemented_for
...
class TestDB(IntegrationTestCase):
	...
	def test_get_value(self):
		...
		self.assertEqual(
			dontmanage.db.sql("""SELECT name FROM `tabUser` WHERE name > 's' ORDER BY creation DESC""")[0][0],
			dontmanage.db.get_value("User", {"name": [">", "s"]}),
		)

		self.assertEqual(
			dontmanage.db.sql("""SELECT name FROM `tabUser` WHERE name >= 't' ORDER BY creation DESC""")[0][0],
		...
		self.assertEqual(
			dontmanage.db.sql("select email from tabUser where name='Administrator' order by creation DESC"),
	...
	def test_casted_get_value_singles(self):
		telemetry = dontmanage.db.get_value("System Settings", None, "enable_telemetry")
		self.assertEqual(type(telemetry), int)
		telemetry = dontmanage.db.get_value("System Settings", "System Settings", "enable_telemetry")
		self.assertEqual(type(telemetry), int)

		# Edge case in calling get_value
		dt_name = dontmanage.db.get_value("DocType", "DocType", "name")
		self.assertEqual(dt_name, "DocType")

		timestamp = dontmanage.db.get_value("System Settings", None, "modified")
		self.assertEqual(type(timestamp), datetime.datetime)

	def test_singles_get_values_variant(self):
		[[lang, date_format]] = dontmanage.db.get_values("System Settings", fieldname=["language", "date_format"])
		self.assertEqual(lang, dontmanage.db.get_single_value("System Settings", "language"))
		self.assertEqual(date_format, dontmanage.db.get_single_value("System Settings", "date_format"))

	def test_get_value_casts_singles(self):
		doc = dontmanage.get_doc("System Settings")
		results = dontmanage.db.get_value("System Settings", None, ["language", "date_format"], as_dict=True)
		...
		doc = dontmanage.get_doc("System Settings")
		[lang, date_format] = dontmanage.db.get_value("System Settings", None, ["language", "date_format"])
		self.assertEqual(doc.language, lang)
		self.assertEqual(doc.date_format, date_format)

		# single field as dict
		results = dontmanage.db.get_value("System Settings", None, "enable_telemetry", as_dict=True)
	...
	def test_db_keywords_as_fields(self):
		...
		self.assertEqual(
			next(iter(dontmanage.get_all("ToDo", fields=[{"COUNT": random_field}], limit=1, order_by=None)[0])),
	...
	def test_transactions_disabled_during_writes(self):
		...
		with self.patch_hooks(
			...
			note = dontmanage.new_doc("Note", title=dontmanage.generate_hash())
			note.insert()
		self.assertGreater(dontmanage.db.transaction_writes, 0)  # This would've reset for commit/rollback

		self.assertFalse(dontmanage.db._disable_transaction_control)
	...
	def test_estimated_count(self):
		self.assertGreater(dontmanage.db.estimate_count("DocField"), 100)

	def test_datetime_serialization(self):
		dt = now_datetime()
		dt = dt.replace(microsecond=0)
		self.assertEqual(str(dt), str(dontmanage.db.sql("select %s", dt)[0][0]))

		dontmanage.db.exists("User", {"creation": (">", dt)})
		self.assertIn(str(dt), str(dontmanage.db.last_query))

		before = now_datetime()
		note = dontmanage.get_doc(doctype="Note", title=dontmanage.generate_hash(), content="something").insert()
		after = now_datetime()
		self.assertEqual(note.name, dontmanage.db.exists("Note", {"creation": ("between", (before, after))}))
	...
	def test_bulk_update(self):
		...
		dontmanage.db.bulk_update("ToDo", updates)

		priority, status = dontmanage.db.get_value("ToDo", record_names[0], ["priority", "status"])
		...
		priority, status = dontmanage.db.get_value("ToDo", record_names[1], ["priority", "status"])
		self.assertEqual(priority, "Low")
		self.assertEqual(status, "Closed")  # should stay the same

		# cleanup
		dontmanage.db.delete("ToDo", {"name": ("in", record_names)})
	...
	def test_get_list_return_value_data_type(self):
		...
		note_docs = dontmanage.db.sql("select * from `tabNote`")
	...
	def test_db_explain(self):
		dontmanage.db.sql("select 1", debug=1, explain=1)
...
class TestDDLCommandsPost(IntegrationTestCase):
	...
	def test_is(self):
		...
		query_is_set = dontmanage.db.get_values(user, filters={user.name: ("is", "set")}, run=False).lower()

		query_is_not_set = dontmanage.db.get_values(
...
class TestConcurrency(IntegrationTestCase):
	...
	def test_skip_locking(self):
		with self.primary_connection():
			name = dontmanage.db.get_value("User", "Administrator", for_update=True, skip_locked=True)
	...
	def test_no_wait(self):
		with self.primary_connection():
			name = dontmanage.db.get_value("User", "Administrator", for_update=True)
			self.assertEqual(name, "Administrator")

		with self.secondary_connection():
			self.assertRaises(
				dontmanage.QueryTimeoutError,
				lambda: dontmanage.db.get_value("User", "Administrator", for_update=True, wait=False),
			)

	@timeout(5, "Deletion stuck on lock timeout")
	def test_delete_race_condition(self):
		note = dontmanage.new_doc("Note")
		note.title = note.content = dontmanage.generate_hash()
		note.insert()
		dontmanage.db.commit()  # ensure that second connection can see the document

		with self.primary_connection():
			n1 = dontmanage.get_doc(note.doctype, note.name)
			n1.save()

		with self.secondary_connection():
			self.assertRaises(dontmanage.QueryTimeoutError, dontmanage.delete_doc, note.doctype, note.name)
	...
	def test_value_cache_invalidation(self):
		...
		new_title = dontmanage.generate_hash()

		with self.primary_connection():
			note = dontmanage.get_doc(note.doctype, note.name)
			note.title = new_title
			note.save()  # NOT commited yet, secondary connection will still see old value

		with self.secondary_connection():
			rr_value = dontmanage.db.get_value("Note", note.name, "title", cache=True)
		...
		with self.secondary_connection():
			dontmanage.db.rollback()
			new_value = dontmanage.db.get_value("Note", note.name, "title", cache=True)
...
class TestSqlIterator(IntegrationTestCase):
	...
	def test_unbuffered_cursor_postgres(self):
		...
		for query in test_queries:
			with dontmanage.db.unbuffered_cursor():
				iter_query_val = list(dontmanage.db.sql(query, as_dict=True, as_iterator=True))
			query_val = dontmanage.db.sql(query, as_dict=True)
			self.assertEqual(
				query_val,
				iter_query_val,
				msg=f"{query=} results not same as iterator",
			...
			with dontmanage.db.unbuffered_cursor():
				iter_query_val = list(dontmanage.db.sql(query, pluck=True, as_iterator=True))
			query_val = dontmanage.db.sql(query, pluck=True)
			self.assertEqual(
				query_val,
				iter_query_val,
				msg=f"{query=} results not same as iterator",
			...
			with dontmanage.db.unbuffered_cursor():
				iter_query_val = list(dontmanage.db.sql(query, as_list=True, as_iterator=True))
			query_val = dontmanage.db.sql(query, as_list=True)
			self.assertEqual(
				query_val,
				iter_query_val,
				msg=f"{query=} results not same as iterator",
...
class TestPostgresSchemaQueryIndependence(ExtIntegrationTestCase):
	...
	def test_get_tables(self) -> None:
		tables = dontmanage.db.get_tables(cached=False)
	...
	def test_db_table_columns(self) -> None:
		columns = dontmanage.db.get_table_columns(self.test_table_name)

		# should have received the columns of the table from public schema
		self.assertEqual(columns, ["col_a", "col_b"])

		dontmanage.conf["db_schema"] = "alt_schema"
		# remove table columns cache for next try from alt_schema
		dontmanage.client_cache.delete_keys("table_columns::*")

		# should have received the columns of the table from alt_schema
		columns = dontmanage.db.get_table_columns(self.test_table_name)
		self.assertEqual(columns, ["col_c", "col_d"])

		del dontmanage.conf["db_schema"]
		dontmanage.client_cache.delete_keys("table_columns::*")

	def test_describe(self) -> None:
		self.assertSequenceEqual([("col_a",), ("col_b",)], dontmanage.db.describe(self.test_table_name))
	...
	def test_has_index(self) -> None:
		...
		self.assertFalse(dontmanage.db.has_index(f"tab{self.test_table_name}", f"tab{self.test_table_name}_pkey"))

	def test_add_index(self) -> None:
		dontmanage.conf["db_schema"] = "alt_schema"

		# only dummy tabUser table in alt_schema has "col_c" column
		dontmanage.db.add_index("User", ("col_c",))
		...
		with self.assertSqlException():
			dontmanage.db.add_index(doctype="User", fields=("col_c",))

	# TODO: is there some method like remove_index:
	# TODO: apps/dontmanage/dontmanage/patches/v14_0/drop_unused_indexes.py # def drop_index_if_exists()
	# TODO: apps/dontmanage/dontmanage/database/postgres/schema.py # def alter()
	...
	def test_add_unique(self) -> None:
		...
		with self.assertSqlException():
			dontmanage.db.add_unique(f"{self.test_table_name}", ["col_c", "col_d"])
	...
	def test_get_table_columns_description(self):
		...
		columns = dontmanage.db.get_table_columns_description(f"tab{self.test_table_name}")
	...
	def test_get_column_type(self):
		...
		self.assertEqual(dontmanage.db.get_column_type(self.test_table_name, "col_a"), "character varying")

		# should raise an error for the column in the alt_schema
		with self.assertSqlException():
			dontmanage.db.get_column_type(self.test_table_name, "col_c")

	def test_search_path(self):
		# by default the the public schema tables should be addressed by search path
		rows = dontmanage.db.sql(f'select * from "tab{self.test_table_name}"')
		self.assertEqual(
			rows,
			(
				(
...
class TestDbConnectWithEnvCredentials(IntegrationTestCase):
	current_site = dontmanage.local.site

	def tearDown(self):
		dontmanage.init(self.current_site, force=True)
		dontmanage.connect()
	...
	def test_connect_fails_with_wrong_credentials_by_env(self) -> None:
		...
		# with wrong db name
		with set_env_variable("DONTMANAGE_DB_NAME", "dbiq"):
			dontmanage.init(self.current_site, force=True)
			dontmanage.connect()
		...
		# with wrong host
		with set_env_variable("DONTMANAGE_DB_HOST", "iqx.local"):
			dontmanage.init(self.current_site, force=True)
			dontmanage.connect()

			with self.assertRaises(dontmanage.db.OperationalError) as cm:
				dontmanage.db.connect()

		# with wrong user name
		with set_env_variable("DONTMANAGE_DB_USER", "uname"):
			dontmanage.init(self.current_site, force=True)
			dontmanage.connect()
		...
		# with wrong password
		with set_env_variable("DONTMANAGE_DB_PASSWORD", "pass"):
			dontmanage.init(self.current_site, force=True)
			dontmanage.connect()
		...
		# with wrong password
		with set_env_variable("DONTMANAGE_DB_PORT", "1111"):
			dontmanage.init(self.current_site, force=True)
			dontmanage.connect()

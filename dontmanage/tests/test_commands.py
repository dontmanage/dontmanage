...
from dontmanage.utils import add_to_date, get_bench_path, get_bench_relative_path, now
...
class BaseTestCommands(DontManageTestCase):
	@classmethod
	...
	def setup_test_site(cls):
		cmd_config = {
			...
			"root_password": dontmanage.conf.root_password,
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("""bench --site {site} execute dontmanage.bold --kwargs '{{"text": "DocType"}}'""")
		self.assertEqual(self.returncode, 0)
	...
	def test_bench_drop_site_should_archive_site(self):
		...
		self.execute(f"bench drop-site {site} --force --root-password {dontmanage.conf.root_password}")
		self.assertEqual(self.returncode, 0)
...
class TestRemoveApp(DontManageTestCase):
...
class TestBenchBuild(BaseTestCommands):
	def test_build_assets_size_check(self):
		with cli(dontmanage.commands.utils.build, "--force --production") as result:
			self.assertEqual(result.exit_code, 0)
			self.assertEqual(result.exception, None)

...
first_lang, second_lang, third_lang, fourth_lang, fifth_lang = choices(
	...
	dontmanage.get_all("Language", pluck="name", filters=[["name", "not like", "en%"], ["enabled", "=", 1]]),
	k=5,
)
...
class TestTranslate(IntegrationTestCase):
	...
	def test_clear_cache(self):
		...
		self.assertIsNotNone(dontmanage.cache.hget(USER_TRANSLATION_KEY, dontmanage.local.lang))
		self.assertIsNotNone(dontmanage.cache.hget(MERGED_TRANSLATION_KEY, dontmanage.local.lang))

		clear_cache()

		self.assertIsNone(dontmanage.cache.hget(USER_TRANSLATION_KEY, dontmanage.local.lang))
		self.assertIsNone(dontmanage.cache.hget(MERGED_TRANSLATION_KEY, dontmanage.local.lang))
	...
	def test_translation_with_context(self):
		t1 = dontmanage.new_doc("Translation")
		t1.language = "fr"
		t1.source_text = "Change"
		t1.translated_text = "Changement"
		t1.save()

		t2 = dontmanage.new_doc("Translation")
		t2.language = "fr"
	...
	def test_request_language_resolution_with_form_dict(self):
		"""Test for dontmanage.translate.get_language

		Case 1: dontmanage.form_dict._lang is set
		"""

		dontmanage.form_dict._lang = first_lang

		with patch.object(dontmanage.translate, "get_preferred_language_cookie", return_value=second_lang):
	...
	def test_request_language_resolution_with_cookie(self):
		...
		Case 2: dontmanage.form_dict._lang is not set, but preferred_language cookie is
		"""

		with patch.object(dontmanage.translate, "get_preferred_language_cookie", return_value="fr"):
	...
	def test_guest_request_language_resolution_with_cookie(self):
		...
		Case 3: dontmanage.form_dict._lang is not set, but preferred_language cookie is [Guest User]
		"""
	...
	def test_guest_request_language_resolution_with_request_header(self):
		...
		Case 4: dontmanage.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is [Guest User]
		"""
	...
	def test_request_language_resolution_with_request_header(self):
		...
		Case 5: dontmanage.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is
		"""
	...
	def test_python_extractor(self):
		code = textwrap.dedent(
			...
			dontmanage._("attr")
			_("name")
			dontmanage._("attr with", context="attr context")
...
def verify_translation_files(app):
	...
	translations_dir = Path(dontmanage.get_app_path(app)) / "translations"

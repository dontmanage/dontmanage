...
from dontmanage.desk.search import get_names_for_mentions, search_link, search_widget
from dontmanage.permissions import add_user_permission
from dontmanage.tests import IntegrationTestCase
from dontmanage.tests.utils import whitelist_for_tests
...
class TestSearch(IntegrationTestCase):
	...
	def test_only_enabled_in_mention(self):
		...
		dontmanage.delete_doc("User", email)
	...
	def test_validate_and_sanitize_search_inputs(self):
		...
		# should not fail if function is called via dontmanage.call with extra arguments
		args = ("Random DocType", "Random", "email", "2", "10", dict())
		kwargs = {"as_dict": False}
		self.assertListEqual(dontmanage.call("dontmanage.tests.test_search.get_data", *args, **kwargs), [])
	...
	def test_reference_doctype(self):
		...
		results = search_link(
			...
			query="dontmanage.tests.test_search.query_with_reference_doctype",
		)
		self.assertListEqual(results, [])

	def test_search_relevance(self):
		dontmanage.db.set_value("Language", {"name": ("like", "e%")}, "enabled", 1)
		...
		dontmanage.db.set_value("Language", "es", "idx", 10)
	...
	def test_search_link_with_ignore_user_permissions(self):
		...
		for dt in ("Test Search Form", "Test Search Linked"):
			if dontmanage.db.exists("DocType", dt):
				dontmanage.delete_doc("DocType", dt, force=True)
		...
		self.addCleanup(
			lambda: dontmanage.delete_doc("DocType", "Test Search Form", force=True, ignore_missing=True)
		)
		self.addCleanup(lambda: dontmanage.delete_doc("DocType", "Test Search Linked", force=True))

		# Create some test documents
		allowed_doc = dontmanage.get_doc({"doctype": "Test Search Linked", "title": "Allowed Document"}).insert()
		restricted_doc = dontmanage.get_doc(
			{"doctype": "Test Search Linked", "title": "Restricted Document"}
		).insert()
		self.addCleanup(lambda: dontmanage.delete_doc("Test Search Linked", allowed_doc.name, force=True))
		self.addCleanup(lambda: dontmanage.delete_doc("Test Search Linked", restricted_doc.name, force=True))

		# Create a test user with restricted permissions
		test_user = "test_search_user@example.com"
		if not dontmanage.db.exists("User", test_user):
			...
			self.addCleanup(lambda: dontmanage.delete_doc("User", test_user, force=True))
		...
		self.addCleanup(
			lambda: dontmanage.db.delete("User Permission", {"user": test_user, "allow": "Test Search Linked"})
		)
	...
	def test_search_link_ignore_user_permissions_validation(self):
		...
		self.addCleanup(
			lambda: dontmanage.delete_doc("DocType", "Test Search Linked2", force=True, ignore_missing=True)
		)
		...
		self.addCleanup(lambda: dontmanage.delete_doc("DocType", "Test Search Form Wrong Link", force=True))
...
def setup_test_link_field_order(TestCase):
	...
	# Create Tree doctype
	if not dontmanage.db.exists("DocType", TestCase.tree_doctype_name):
		TestCase.tree_doc = dontmanage.get_doc(
			{
				"doctype": "DocType",
		...
		TestCase.tree_doc = dontmanage.get_doc("DocType", TestCase.tree_doctype_name)

	# Create root for the tree doctype
	if not dontmanage.db.exists(TestCase.tree_doctype_name, {"random": TestCase.parent_doctype_name}):
		dontmanage.get_doc(
...
def use_language(language: str):
	original_lang = dontmanage.local.lang
	dontmanage.local.lang = language

	try:
		yield
	finally:
		dontmanage.local.lang = original_lang

...
from dontmanage.utils import cint, get_test_client, get_url

try:
	_site = dontmanage.local.site
except Exception:
	_site = None
...
class ThreadWithReturnValue(Thread):
	...
	def run(self):
		if self._target is not None:
			with patch("dontmanage.app.get_site_name", return_value=self.site):
				header_patch = patch("dontmanage.get_request_header", new=patch_request_header)
...
class DontManageAPITestCase(IntegrationTestCase):
...
class TestResourceAPI(DontManageAPITestCase):
	DOCTYPE = "ToDo"
	...
	def tearDownClass(cls):
		...
		for name in cls.GENERATED_DOCUMENTS:
			dontmanage.delete_doc_if_exists(cls.DOCTYPE, name)
		dontmanage.delete_doc_if_exists("User", cls.TEST_USER)
		dontmanage.db.commit()
	...
	def test_get_list_dict(self):
		...
		json = dontmanage._dict(response.json)
		self.assertEqual(response.status_code, 200)
	...
	def test_create_document(self):
		data = {"description": dontmanage.mock("paragraph"), "sid": self.sid}
	...
	def test_update_document(self):
		generated_desc = dontmanage.mock("paragraph")
	...
	def test_run_doc_method(self):
		...
		if response.status_code == 403:
			...
			self.assertEqual(
				response.json.get("exception"), "dontmanage.exceptions.PermissionError: Not permitted"
			)
...
class TestMethodAPI(DontManageAPITestCase):
	def test_ping(self):
	...
	def test_get_user_info(self):
		# test 3: test for /api/method/dontmanage.realtime.get_user_info
		response = self.get(self.method("dontmanage.realtime.get_user_info"))
		self.assertEqual(response.status_code, 200)
		self.assertIsInstance(response.json, dict)
	...
	def test_auth_cycle(self):
		...
		response = self.get(self.method("dontmanage.auth.get_logged_user"))
	...
	def test_logs(self):
		method = "dontmanage.tests.test_api.test"

		def get_message(resp, msg_type):
			return dontmanage.parse_json(dontmanage.parse_json(dontmanage.parse_json(resp.json)[msg_type])[0])
	...
	def test_array_response(self):
		method = "dontmanage.tests.test_api.test_array"
...
class TestReadOnlyMode(DontManageAPITestCase):
	...
	def test_blocked_writes(self):
		with suppress_stdout():
			response = self.post(
				self.resource("ToDo"), {"description": dontmanage.mock("paragraph"), "sid": self.sid}
			)
		self.assertEqual(response.status_code, 503)
		self.assertEqual(response.json["exc_type"], "InReadOnlyMode")


class TestWSGIApp(DontManageAPITestCase):
	def test_request_hooks(self):
		self.addCleanup(lambda: _test_REQ_HOOK.clear())

		with self.patch_hooks(
			{
				"before_request": ["dontmanage.tests.test_api.before_request"],
				"after_request": ["dontmanage.tests.test_api.after_request"],
			}
		):
...
class TestAPIResponse(DontManageAPITestCase):
	...
	def test_binary_and_csv_response(self):
		def download_template(file_type):
			...
			return self.post(
				"/api/method/dontmanage.core.doctype.data_import.data_import.download_template",
				{
					"sid": self.sid,
					"doctype": "User",
	...
	def test_download_private_file_with_unique_url(self):
		test_content = dontmanage.generate_hash()

...
from dontmanage.utils import get_site_url, get_test_client
...
class ThreadWithReturnValue(Thread):
	...
	def run(self):
		if self._target is not None:
			with patch("dontmanage.app.get_site_name", return_value=_site):
...
class DontManageAPITestCase(DontManageTestCase):
	...
	def sid(self) -> str:
		if not getattr(self, "_sid", None):
			...
			self._sid = dontmanage.session.sid
...
class TestResourceAPI(DontManageAPITestCase):
	...
	def test_update_document(self):
		...
		desc_before_update = dontmanage.db.get_value(self.DOCTYPE, random_doc, "description")
	...
	def test_delete_document(self):
		...
		non_existent_doc = dontmanage.generate_hash(length=12)
...
class TestMethodAPI(DontManageAPITestCase):
	...
	def test_version(self):
		...
		self.assertEqual(Version(json.message), Version(dontmanage.__version__))
	...
	def test_get_user_info(self):
		...
		response = self.get(f"{self.METHOD_PATH}/dontmanage.realtime.get_user_info")
		self.assertEqual(response.status_code, 200)
		self.assertIsInstance(response.json, dict)
	...
	def test_auth_cycle(self):
		...
		response = self.get("/api/method/dontmanage.auth.get_logged_user")
...
class TestReadOnlyMode(DontManageAPITestCase):
	...
	def test_blocked_writes(self):
		response = self.post(self.REQ_PATH, {"description": dontmanage.mock("paragraph"), "sid": self.sid})
...
class TestWSGIApp(DontManageAPITestCase):
	def test_request_hooks(self):
		...
		with patch("dontmanage.get_hooks", patch_request_hooks):

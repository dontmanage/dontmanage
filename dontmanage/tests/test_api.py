...
from dontmanage.utils import cint, get_site_url, get_test_client, get_url
...
class DontManageAPITestCase(DontManageTestCase):
...
class TestResourceAPI(DontManageAPITestCase):
	...
	def setUpClass(cls):
		super().setUpClass()
		for _ in range(20):
			doc = dontmanage.get_doc({"doctype": "ToDo", "description": dontmanage.mock("paragraph")}).insert()
...
class TestMethodAPI(DontManageAPITestCase):
	...
	def test_get_user_info(self):
		...
		response = self.get(self.method_path("dontmanage.realtime.get_user_info"))
		self.assertEqual(response.status_code, 200)
		self.assertIsInstance(response.json, dict)
	...
	def test_auth_cycle(self):
		...
		response = self.get(self.method_path("dontmanage.auth.get_logged_user"))
...
class TestReadOnlyMode(DontManageAPITestCase):
	...
	def test_blocked_writes(self):
		with suppress_stdout():
			response = self.post(
				self.resource_path("ToDo"), {"description": dontmanage.mock("paragraph"), "sid": self.sid}
			)
...
class TestResponse(DontManageAPITestCase):

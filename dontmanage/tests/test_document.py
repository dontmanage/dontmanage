...
from dontmanage.model.document import LazyChildTable
...
class TestDocument(IntegrationTestCase):
	...
	def test_discard_transitions(self):
		...
		d.submit()
		self.assertRaises(dontmanage.ValidationError, d.discard)
		d.reload()
	...
	def test_save_on_discard_throws(self):
		...
		self.assertRaises(dontmanage.ValidationError, d3.discard)
...
class TestDocumentWebView(IntegrationTestCase):
	...
	def test_base_class_set_correctly_on_has_web_view_change(self):
		...
		from dontmanage.modules.utils import get_doc_path, scrub

		dontmanage.flags.allow_doctype_export = True

		dontmanage.delete_doc_if_exists("DocType", "Test WebViewDocType", force=1)
...
class TestLazyDocument(IntegrationTestCase):
	def test_lazy_documents(self):
		# Warmup meta etc
		_ = dontmanage.get_lazy_doc("User", "Guest")
		eager_guest: User = dontmanage.get_doc("User", "Guest")

		# Only one query for parent document
		with self.assertQueryCount(1):
			guest: User = dontmanage.get_lazy_doc("User", "Guest")
		...
		self.assertTrue(dontmanage.get_lazy_doc("User", "Guest").get("roles"))

	def test_lazy_doc_efficient_saves(self):
		# Only touched tables and self should be updated
		guest = dontmanage.get_lazy_doc("User", "Guest")
		with self.assertQueryCount(1):
			guest.db_update_all()
	...
	def test_for_update(self):
		guest = dontmanage.get_lazy_doc("User", "Guest", for_update=True)

...
class TestNaming(IntegrationTestCase):
	...
	def test_child_table_naming(self):
		...
		name = dontmanage.generate_hash(length=10)

		doc = dontmanage.new_doc(dt_with_child_autoname.name)
	...
	def test_format_autoname_for_consecutive_week_number(self):
		...
		todo_doctype = dontmanage.get_doc("DocType", doctype)
		todo_doctype.autoname = "format:TODO-{WW}-{##}"
		todo_doctype.save()

		description = "Format"

		todo = dontmanage.new_doc(doctype)
	...
	def test_revert_series(self):
		...
		dontmanage.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 1)""", (series,))
		revert_series_if_last(key, name)
		current_index = dontmanage.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]

		self.assertEqual(current_index.get("current"), 0)
		dontmanage.db.delete("Series", {"name": series})

		series = f"TEST-{year}-"
		key = "TEST-.YYYY.-.#####"
		name = f"TEST-{year}-00002"
		dontmanage.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 2)""", (series,))
		revert_series_if_last(key, name)
		current_index = dontmanage.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
		...
		name = "TEST-00003"
		dontmanage.db.delete("Series", {"name": series})
		dontmanage.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 3)""", (series,))
		revert_series_if_last(key, name)
		current_index = dontmanage.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
	...
	def test_naming_for_cancelled_and_amended_doc(self):
		submittable_doctype = dontmanage.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"custom": 1,
		...
		doc = dontmanage.new_doc("Submittable Doctype")
		doc.save()
		...
		amended_doc = dontmanage.copy_doc(doc)
	...
	def test_naming_validations(self):
		...
		# set name via prompt
		tag = dontmanage.get_doc({"doctype": "Tag", "__newname": "Tag"})
		self.assertRaises(dontmanage.NameError, tag.insert)
		...
		tag = dontmanage.get_doc({"doctype": "Tag", "__newname": "New Tag"})
		self.assertRaises(dontmanage.NameError, tag.insert)
		...
		tag = dontmanage.get_doc({"doctype": "Tag", "__newname": ""})
		self.assertRaises(dontmanage.ValidationError, tag.insert)

	def test_autoincremented_naming(self):
		from dontmanage.core.doctype.doctype.test_doctype import new_doctype

		doctype = "autoinc_doctype" + dontmanage.generate_hash(length=5)
		dt = new_doctype(doctype, autoname="autoincrement").insert(ignore_permissions=True)

		for i in range(1, 20):
			self.assertEqual(dontmanage.new_doc(doctype).save(ignore_permissions=True).name, i)
	...
	def test_hash_collision(self):
		...
		for _ in range(10):
			...
			dontmanage.new_doc(doctype).update({"name": name}).insert()
		dontmanage.flags.pop("in_import", None)
	...
	def test_uuid_naming(self):
		...
		self.assertEqual("uuid", dontmanage.db.get_column_type(uuid_doctype, "name"))

		# Auto set names
		document = dontmanage.new_doc(uuid_doctype).insert()
		...
		for uid in (uuid.uuid4(), uuid.uuid7()):
			doc = dontmanage.new_doc(uuid_doctype, name=uid).insert()
			self.assertEqual(doc.name, str(uid))

		# Can specify valid UUID strings too
		for uid in (uuid.uuid4(), uuid.uuid7()):
			doc = dontmanage.new_doc(uuid_doctype, name=str(uid)).insert()
			self.assertEqual(doc.name, str(uid))

		self.assertRaises(InvalidUUIDValue, dontmanage.new_doc(uuid_doctype, name="XYZ").insert)
...
def make_invalid_todo():
	dontmanage.get_doc({"doctype": "ToDo", "description": "Test"}).insert(set_name="ToDo")

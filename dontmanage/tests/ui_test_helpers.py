...
from dontmanage.tests.test_helpers import create_test_blog_category
...
def create_doctype_for_attachment():
	...
	doc = dontmanage.get_doc("Test Blog Category", "_Test Blog Category 2")
	return doc


@whitelist_for_tests()
def create_datetime_test_doctype():
	dt = dontmanage.new_doc("DocType")
	dt.module = "Core"
...
def create_datetime_test_record():
	doc = dontmanage.new_doc("Test Datetime Precision")
	doc.datetime = dontmanage.utils.now_datetime()
	doc.insert()
	return doc
...
def insert_translations():
	...
	for doc in translation:
		dontmanage.get_doc(doc).insert(ignore_if_duplicate=True)
...
def setup_inbox():
	...
	doc = dontmanage.new_doc("Email Account")
...
def empty_my_workspaces():
	my_workspaces = dontmanage.get_doc("Workspace Sidebar", "My Workspaces")
	my_workspaces.items = []
	my_workspaces.save()

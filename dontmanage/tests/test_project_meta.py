...
import dontmanage
from dontmanage.tests.classes.integration_test_case import IntegrationTestCase
...
class TestProjectMeta(IntegrationTestCase):
	def test_init_py_tax_paid(self):
		...
		dontmanage/__init__.py has grown crazy big and keeps getting bigger. Plot the LOC over time and
		...
		init_py_size = len(Path(dontmanage.__file__).read_text().splitlines())
		...
		self.assertLessEqual(
			init_py_size,
			baseline,
			"""\nDon't add more code in dontmanage/__init__.py!\nRemember the Zen of Python:\n"""

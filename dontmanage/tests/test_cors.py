...
import dontmanage
from dontmanage.app import process_response
...
class TestCORS(DontManageTestCase):
	def make_request_and_test(self, origin="http://example.com", absent=False):
		...
		dontmanage.utils.set_request(method="OPTIONS", headers=headers)
	...
	def test_valid_origin(self):
		...
		dontmanage.conf.allow_cors = ["http://example.com", "https://example.com"]
		self.make_request_and_test()

	def test_invalid_origin(self):
		dontmanage.conf.allow_cors = "http://example1.com"
		self.make_request_and_test(absent=True)

		dontmanage.conf.allow_cors = ["http://example1.com", "https://example.com"]
		self.make_request_and_test(absent=True)

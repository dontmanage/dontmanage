...
from dontmanage.utils import global_search, now_datetime
...
class TestGlobalSearch(IntegrationTestCase):
	def setUp(self):
		...
		self.assertTrue("__global_search" in dontmanage.db.get_tables())
		doctype = "Event"
		global_search.reset()
	...
	def tearDown(self):
		dontmanage.db.delete("Property Setter", {"doc_type": "Event"})
	...
	def test_update_doc(self):
		...
		event = dontmanage.get_doc("Event", dontmanage.get_all("Event")[0].name)
	...
	def test_delete_doc(self):
		self.insert_test_events()
		event_name = dontmanage.get_all("Event")[0].name
		event = dontmanage.get_doc("Event", event_name)
		...
		dontmanage.delete_doc("Event", event_name)
		global_search.sync_global_search()
		dontmanage.db.commit()
	...
	def test_insert_child_table(self):
		...
		for text in phrases:
			doc = dontmanage.get_doc({"doctype": "Event", "subject": text, "starts_on": now_datetime()})
			doc.insert()

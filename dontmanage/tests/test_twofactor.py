...
import dontmanage
from dontmanage.auth import HTTPRequest, get_login_attempt_tracker, validate_ip_address
...
from dontmanage.utils import cint, set_request

from . import get_system_setting, update_system_settings


class TestTwoFactor(DontManageTestCase):
	...
	def setUp(self):
		...
		self.login_manager = dontmanage.local.login_manager
		self.user = self.login_manager.user
		update_system_settings({"allow_consecutive_login_attempts": 2})

	def tearDown(self):
		dontmanage.local.response["verification"] = None
		dontmanage.local.response["tmp_id"] = None
		disable_2fa()
		dontmanage.clear_cache(user=self.user)
	...
	def test_authenticate_for_2factor(self):
		"""Verification obj and tmp_id should be set in dontmanage.local."""
		authenticate_for_2factor(self.user)
		verification_obj = dontmanage.local.response["verification"]
		tmp_id = dontmanage.local.response["tmp_id"]
	...
	def test_two_factor_is_enabled(self):
		...
		user.restrict_ip = dontmanage.local.request_ip
		user.save()
		self.assertTrue(should_run_2fa(self.user))
	...
	def test_confirm_otp_token(self):
		...
		dontmanage.flags.otp_expiry = 2
		authenticate_for_2factor(self.user)
		tmp_id = dontmanage.local.response["tmp_id"]
		otp = "wrongotp"
		with self.assertRaises(dontmanage.AuthenticationError):
		...
		dontmanage.flags.otp_expiry = None
		if dontmanage.flags.tests_verbose:
	...
	def test_render_string_template(self):
		...
		args = {"issuer_name": "DontManage Technologies"}
		_str = "Verification Code from {{issuer_name}}"
		_str = dontmanage.render_template(_str, args)
		self.assertEqual(_str, "Verification Code from DontManage Technologies")
	...
	def test_otp_attempt_tracker(self):
		...
		with self.assertRaises(dontmanage.SecurityException):
			confirm_otp_token(self.login_manager, otp=otp, tmp_id=tmp_id)
...
def enable_2fa(bypass_two_factor_auth=0, bypass_restrict_ip_check=0):
	...
	system_settings = dontmanage.get_doc("System Settings")

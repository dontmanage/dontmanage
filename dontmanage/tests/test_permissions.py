...
from dontmanage.test_runner import make_test_records_for_doctype
...
class TestPermissions(DontManageTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		dontmanage.clear_cache(doctype="Blog Post")
	...
	def tearDown(self):
		...
		dontmanage.db.set_value("Blogger", "_Test Blogger 1", "user", None)
	...
	def test_basic_permission(self):
		post = dontmanage.get_doc("Blog Post", "-test-blog-post")
	...
	def test_user_permissions_in_doc(self):
		...
		post1 = dontmanage.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_user_permissions_in_report(self):
		...
		dontmanage.set_user("test2@example.com")
		names = [d.name for d in dontmanage.get_list("Blog Post", fields=["name", "blog_category"])]

		self.assertTrue("-test-blog-post-1" in names)
		self.assertFalse("-test-blog-post" in names)

	def test_default_values(self):
		doc = dontmanage.new_doc("Blog Post")
	...
	def test_user_link_match_doc(self):
		blogger = dontmanage.get_doc("Blogger", "_Test Blogger 1")
		blogger.user = "test2@example.com"
		blogger.save()

		dontmanage.set_user("test2@example.com")

		post = dontmanage.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(post.has_permission("read"))
	...
	def test_user_link_match_report(self):
		...
		names = [d.name for d in dontmanage.get_list("Blog Post", fields=["name", "owner"])]
	...
	def test_not_allowed_to_set_user_permissions(self):
		...
		self.assertRaises(
			dontmanage.PermissionError, add_user_permission, "Blog Post", "-test-blog-post", "test2@example.com"
		)
	...
	def test_read_if_explicit_user_permissions_are_set(self):
		...
		doc = dontmanage.get_doc("Blog Post", "-test-blog-post")
		self.assertTrue(doc.has_permission("read"))

		# and not this one
		doc = dontmanage.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_set_only_once(self):
		blog_post = dontmanage.get_meta("Blog Post")
		doc = dontmanage.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_set_only_once_child_table_rows(self):
		...
		doc = dontmanage.get_doc("DocType", "Blog Post")
	...
	def test_user_permission_doctypes(self):
		...
		doc = dontmanage.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(doc.has_permission("read"))
	...
	def test_insert_if_owner_with_user_permissions(self):
		...
		dontmanage.delete_doc("Blog Post", "-test-blog-post-title")
	...
	def test_user_permissions_not_applied_if_user_can_edit_user_permissions(self):
		...
		self.assertTrue(dontmanage.get_doc("Blogger", "_Test Blogger").has_permission("read"))
	...
	def test_contextual_user_permission(self):
		...
		posts = dontmanage.get_all("Blog Post", fields=["name", "blogger"])
		...
		posts = dontmanage.get_list("Blog Post", fields=["name", "blogger"])
	...
	def test_if_owner_permission_overrides_properly(self):
		...
		dontmanage.set_user("test2@example.com")
		self.assertEqual(dontmanage.get_list("Blog Post"), [])
	...
	def test_if_owner_permission_on_getdoc(self):
		...
		dontmanage.set_user("test2@example.com")
		self.assertRaises(dontmanage.PermissionError, getdoc, "Blog Post", doc.name)
	...
	def test_if_owner_permission_on_get_list(self):
		...
		dontmanage.set_user("test2@example.com")
		self.assertIn(doc.name, dontmanage.get_list("Blog Post", pluck="name"))
		...
		dontmanage.set_user("test2@example.com")
		self.assertNotIn(doc.name, dontmanage.get_list("Blog Post", pluck="name"))
	...
	def test_if_owner_permission_on_delete(self):
		...
		self.assertRaises(dontmanage.PermissionError, dontmanage.delete_doc, "Blog Post", doc.name)

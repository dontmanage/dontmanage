...
from dontmanage.utils.password import check_password, decrypt, encrypt, passlibctx, update_password


class TestPassword(DontManageTestCase):
	def setUp(self):
		dontmanage.delete_doc("Email Account", "Test Email Account Password")
		dontmanage.delete_doc("Email Account", "Test Email Account Password-new")
	...
	def make_email_account(self, name="Test Email Account Password"):
		if not dontmanage.db.exists("Email Account", name):
			...
			return dontmanage.get_doc("Email Account", name)
	...
	def test_hashed_password(self, user="test@example.com"):
		...
		self.assertRaises(dontmanage.AuthenticationError, check_password, user, new_password)
	...
	def test_password_on_rename_user(self):
		...
		dontmanage.rename_doc(doc.doctype, old_name, new_name)

		new_doc = dontmanage.get_doc(doc.doctype, new_name)
		self.assertEqual(new_doc.get_password(), password)
		self.assertTrue(not get_password_list(doc))

		dontmanage.rename_doc(doc.doctype, new_name, old_name)

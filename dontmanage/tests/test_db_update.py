...
class TestDBUpdate(IntegrationTestCase):
	...
	def test_bigint_conversion(self):
		...
		with self.assertRaises(dontmanage.CharacterLengthExceededError):
			dontmanage.get_doc(doctype=doctype.name, int_field=2**62 - 1).insert()
	...
	def test_uuid_varchar_migration(self):
		...
		doctype.save()
		self.assertEqual(dontmanage.db.get_column_type(doctype.name, "name"), "uuid")

		doc = dontmanage.new_doc(doctype.name).insert()

		doctype.autoname = "hash"
		doctype.save()
		varchar = "varchar" if dontmanage.db.db_type == "mariadb" else "character varying"
		self.assertIn(varchar, dontmanage.db.get_column_type(doctype.name, "name"))
	...
	def test_uuid_link_field(self):
		...
		self.assertEqual(dontmanage.db.get_column_type(uuid_doctype.name, "name"), "uuid")
		...
		self.assertEqual(dontmanage.db.get_column_type(referring_doctype.name, link), "uuid")

	def test_varchar_length(self):
		from dontmanage.database.schema import add_column

		test_doc = new_doctype().insert()
		col_name = f"col_{dontmanage.generate_hash(length=4)}"
...
class TestDBUpdateSanityChecks(IntegrationTestCase):
	...
	def test_no_unnecessary_migrates(self):
		doctypes = dontmanage.get_all("DocType", {"is_virtual": 0, "custom": 0}, pluck="name")
		...
		for doctype in doctypes:
			with self.subTest(f"Check {doctype}"):
				dontmanage.reload_doctype(doctype, force=True)

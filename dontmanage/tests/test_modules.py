...
class TestUtils(IntegrationTestCase):
	...
	def tearDown(self):
		...
		dontmanage.local.flags.pop("in_import", None)
	...
	def test_export_customizations_with_module_filter(self):
		...
		with note_customizations() as (custom_field, property_setter, _doctype_link):
			...
			other_module = dontmanage.new_doc("Module Def")

			other_module.update({"module_name": "OtherModule", "app_name": "dontmanage"})
			...
			with open(file_path) as f:
				exported = dontmanage.parse_json(f.read())
	...
	def test_export_doc(self):
		...
		note.title = dontmanage.generate_hash(length=10)
		note.save()
		export_doc(doctype="Note", name=note.name)
		exported_doc_path = Path(
			dontmanage.get_app_path("dontmanage", "desk", "note", note.name, f"{note.name}.json")
		)
	...
	def test_make_boilerplate(self):
		with temp_doctype() as doctype:
			scrubbed = dontmanage.scrub(doctype.name)
			path = dontmanage.get_app_path("dontmanage", "core", "doctype", scrubbed, f"{scrubbed}.json")
...
def temp_doctype():
	try:
		...
		dontmanage.db.sql_ddl(f"DROP TABLE `tab{doctype.name}`")
...
def note_customizations():
	try:
		...
		doctype_link = dontmanage.get_doc(
			{

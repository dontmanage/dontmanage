...
from dontmanage.modules import get_doctype_module, get_module_path, load_doctype_module

if TYPE_CHECKING:
	from dontmanage.model.document import Document

logger = logging.getLogger(__name__)
testing_logger = logging.getLogger("dontmanage.testing.generators")
...
def get_missing_records_module_overrides(module) -> [list, list]:
	...
	if hasattr(module, "test_dependencies"):
		...
		deprecation_warning(
			"2024-10-09",
			"v17",
			"""test_dependencies was clarified to EXTRA_TEST_RECORD_DEPENDENCIES; migration script: https://github.com/dontmanage/dontmanage/pull/28060""",
		)
	...
	if hasattr(module, "test_ignore"):
		...
		deprecation_warning(
			"2024-10-09",
			"v17",
			"""test_ignore was clarified to IGNORE_TEST_RECORD_DEPENDENCIES; migration script: https://github.com/dontmanage/dontmanage/pull/28060""",
		)
...
def load_test_records_for(index_doctype) -> dict[str, Any]:
	module_path = get_module_path(get_doctype_module(index_doctype), "doctype", dontmanage.scrub(index_doctype))
...
def _generate_all_records_towards(index_doctype, reset=False, commit=False) -> Generator[tuple[str, int]]:
	...
	visited = set(dontmanage.local.test_objects.keys())
...
def _generate_records_for(
	...
	if hasattr(test_module, "_make_test_records"):
		...
		if not test_records:
			...
			dontmanage.local.test_objects[index_doctype] = []  # avoid noisy retries on multiple invocations
...
def _sync_records(
	...
	def _load(do_create=True):
		...
		for _sub_doctype, records in test_records.items():
			for record in records:
				...
				dontmanage.local.test_objects[index_doctype].append(MappingProxyType(record))
	...
	if index_doctype in test_record_manager_instance.get():
		...
		if reset:
			...
			dontmanage.local.test_objects[index_doctype] = []
			...
			if index_doctype not in dontmanage.local.test_objects:
...
def _try_create(record, reset=False, commit=False) -> tuple["Document", bool]:
	...
	if not reset:
		dontmanage.db.savepoint("creating_test_record")

	d = dontmanage.copy_doc(record)
	...
	if dontmanage.db.exists(d.doctype, d.name) and not reset:
		dontmanage.db.rollback(save_point="creating_test_record")
		# do not create test records, if already exists
		return dontmanage.get_doc(d.doctype, d.name), False
...
class TestRecordManager:
	def __init__(self):
		...
		self.log_file = Path(dontmanage.get_site_path(PERSISTENT_TEST_LOG_FILE))
		self._log = None
	...
	def _append_to_log(self, index_doctype, records: list["Document"]):
		...
		with self.log_file.open("a") as f:
			f.write(dontmanage.as_json(entry, indent=None) + "\n")
		...
		if self.log_file.exists():
			with self.log_file.open() as f:
				for line in f:
					...
					try:
						for r in records:
							log.setdefault(index_doctype, []).append(dontmanage.get_doc(r["doctype"], r["name"]))
...
def _clear_test_log():
	log_file_path = dontmanage.get_site_path(PERSISTENT_TEST_LOG_FILE)

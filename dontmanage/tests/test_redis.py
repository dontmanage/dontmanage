...
from dontmanage.utils import get_bench_id
from dontmanage.utils.background_jobs import get_redis_conn
...
class TestRedisAuth(DontManageTestCase):
	@skip_if_redis_version_lt("6.0")
	@patch.dict(dontmanage.conf, {"bench_id": "test_bench", "use_rq_auth": False})
	...
	def test_rq_namespace(self):
		...
		# Current bench ID
		bench_id = dontmanage.conf.get("bench_id")
		conn = get_redis_conn()
		...
		conn.acl_deluser(username)
		dontmanage.conf.update({"bench_id": tmp_bench_id})
		_ = RedisQueue(conn).add_user(username, password)
		...
		dontmanage.conf.update({"bench_id": bench_id})
		conn.acl_deluser(username)

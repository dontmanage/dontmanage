...
class TestRenameDoc(IntegrationTestCase):
	...
	def test_rename_autoincrement_doc(self):
		if not dontmanage.db.exists("DocType", "Autoincrement DocType"):
			new_doctype(
		...
		if not dontmanage.db.exists("DocType", "Autoincrement Linked DocType"):
			new_doctype(
		...
		# create records
		mary = dontmanage.new_doc("Autoincrement DocType", first_name="Mary").insert()
		marilyn = dontmanage.new_doc("Autoincrement DocType", first_name="Marilyn").insert()
		linked_with_marilyn = dontmanage.new_doc(
		...
		dontmanage.rename_doc("Autoincrement DocType", marilyn.name, mary.name, merge=True)
		linked_with_marilyn.reload()

		self.assertTrue(dontmanage.db.exists("Autoincrement DocType", marilyn.name) is None)
		self.assertEqual(linked_with_marilyn.autoincrement_doctype, dontmanage.utils.cstr(mary.name))

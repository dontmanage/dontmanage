...
from dontmanage.monitor import MONITOR_REDIS_KEY, get_trace_id
...
class TestMonitor(DontManageTestCase):
	...
	def test_no_response(self):
		...
		dontmanage.monitor.start()
		dontmanage.monitor.stop(response=None)
	...
	def test_trace_ids(self):
		...
		response = build_response("json")
		dontmanage.monitor.start()
		dontmanage.db.sql("select 1")
		self.assertIn(get_trace_id(), str(dontmanage.db.last_query))
		dontmanage.monitor.stop(response)

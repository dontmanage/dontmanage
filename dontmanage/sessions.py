...
from dontmanage.cache_manager import clear_user_cache, reset_metadata_version
from dontmanage.query_builder import Order
from dontmanage.utils import cint, cstr, get_assets_json
from dontmanage.utils.change_log import has_app_update_notifications
...
def get_sessions_to_clear(user=None, keep_current=False, force=False):
	...
	offset = 0
	if not force and user == dontmanage.session.user:
...
def get():
	...
	from dontmanage.boot import get_bootinfo
	from dontmanage.desk.doctype.note.note import get_unseen_notes
	...
	bootinfo["metadata_version"] = dontmanage.client_cache.get_value("metadata_version")
	...
	bootinfo["setup_complete"] = dontmanage.is_setup_complete()
	bootinfo["apps_data"] = {
		"apps": get_apps() or [],
	...
	bootinfo["desk_theme"] = dontmanage.get_cached_value("User", dontmanage.session.user, "desk_theme") or "Light"
	bootinfo["user"]["impersonated_by"] = dontmanage.session.data.get("impersonated_by")
	bootinfo["navbar_settings"] = dontmanage.client_cache.get_doc("Navbar Settings")
	bootinfo.has_app_updates = has_app_update_notifications()
	bootinfo.show_external_link_warning = dontmanage.get_system_settings("show_external_link_warning")
...
class Session:
	...
	def __init__(
		...
		self.sid = cstr(
			dontmanage.form_dict.pop("sid", None) or unquote(dontmanage.request.cookies.get("sid", "Guest"))
		)
		self.user = user
	...
	def validate_user(self):
		if not dontmanage.get_cached_value("User", self.user, "enabled"):
			dontmanage.throw(
	...
	def start(self, session_end: str | None = None, audit_user: str | None = None):
		...
		if dontmanage.request:
			self.data.data.user_agent = dontmanage.request.headers.get("User-Agent")
		...
		if self.user != "Guest":
			...
			# update user
			user = dontmanage.get_lazy_doc("User", self.data["user"])
	...
	def insert_session_record(self):
		...
		(
			...
			.insert(
				(
					dontmanage.as_json(self.data["data"], indent=None, separators=(",", ":")),
					self.data["user"],
					now,
					self.data["sid"],
					"Active",
	...
	def resume(self):
		...
		if self.sid != "Guest":
			dontmanage.local.lang = dontmanage.translate.get_user_lang(self.data.user)
	...
	def get_session_data_from_db(self):
		...
		if record:
			data = dontmanage.parse_json(record[0][1] or "{}")
	...
	def update(self, force=False):
		...
		now = dontmanage.utils.now_datetime()
		...
		) and not dontmanage.flags.read_only:
...
def get_expiry_period():
	exp_sec = dontmanage.get_system_settings("session_expiry") or "240:00:00"

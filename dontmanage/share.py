...
def add_docshare(
	...
	if dontmanage.get_cached_value("User", user, "follow_shared_documents"):
...
def set_docshare_permission(doctype, name, user, permission_to, value=1, everyone=0, flags=None):
	...
	if not share_name:
		...
		share = dontmanage.get_doc("DocShare", share_name)
		if flags:
...
def get_shared_doctypes(user=None):
	...
	if not user:
		user = dontmanage.session.user
	table = dontmanage.qb.DocType("DocShare")
	query = (
...
def check_share_permission(doctype, name, permissions=None, custom_perms=None):
	...
	"""
	if not dontmanage.has_permission(doctype, ptype="share", doc=name):
		dontmanage.throw(
			_("No permission to {0} {1} {2}").format("share", _(doctype), name), dontmanage.PermissionError
		)
	...
	is_owner = (doc.get("owner") or "").lower() == dontmanage.session.user.lower()

	user_perms = get_role_permissions(doc.meta, user=dontmanage.session.user, is_owner=is_owner)
...
def notify_assignment(shared_by, doctype, doc_name, everyone, notify=0):
	...
	reference_user = get_fullname(dontmanage.session.user)
	notification_message = _("{0} shared a document {1} {2} with you").format(
		dontmanage.bold(reference_user), dontmanage.bold(_(doctype)), get_title_html(title)
	)

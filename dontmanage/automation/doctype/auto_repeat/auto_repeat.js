...
dontmanage.provide("dontmanage.auto_repeat");

dontmanage.ui.form.on("Auto Repeat", {
	setup: function (frm) {
		frm.fields_dict["reference_doctype"].get_query = function () {
			return {
				query: "dontmanage.automation.doctype.auto_repeat.auto_repeat.get_auto_repeat_doctypes",
			};
		};
	...
	refresh: function (frm) {
		...
		if (!frm.is_dirty()) {
			...
			frm.add_custom_button(label, () =>
				dontmanage.set_route("List", frm.doc.reference_doctype, { auto_repeat: frm.doc.name })
			);
		}

		// auto repeat schedule
		dontmanage.auto_repeat.render_schedule(frm);
	...
	toggle_submit_on_creation: function (frm) {
		...
		if (frm.doc.reference_doctype) {
			dontmanage.model.with_doctype(frm.doc.reference_doctype, () => {
				let meta = dontmanage.get_meta(frm.doc.reference_doctype);
	...
	template: function (frm) {
		if (frm.doc.template) {
			dontmanage.model.with_doc("Email Template", frm.doc.template, () => {
				let email_template = dontmanage.get_doc("Email Template", frm.doc.template);
	...
	preview_message: function (frm) {
		if (frm.is_dirty()) {
			dontmanage.msgprint(__("Please save the form before previewing the message"));
			return;
		}

		if (frm.doc.message) {
			dontmanage.call({
				method: "dontmanage.automation.doctype.auto_repeat.auto_repeat.generate_message_preview",
				type: "POST",
				args: {
					name: frm.doc.name,
				},
				callback: function (r) {
					if (r.message) {
						dontmanage.msgprint(r.message.message, r.message.subject);
					}
				},
			});
		} else {
			dontmanage.msgprint(__("Please setup a message first"), __("Message not setup"));
		}
	},
});

dontmanage.auto_repeat.render_schedule = function (frm) {
	if (!frm.is_dirty() && frm.doc.status !== "Disabled") {
		frm.call("get_auto_repeat_schedule").then((r) => {
			frm.dashboard.reset();
			frm.dashboard.add_section(
				dontmanage.render_template("auto_repeat_schedule", {

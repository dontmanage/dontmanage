...
from dontmanage.automation.doctype.assignment_rule.assignment_rule import get_repeated
from dontmanage.contacts.doctype.contact.contact import (
	get_contacts_linked_from,
	get_contacts_linking_to,
)
from dontmanage.core.doctype.communication.email import make
from dontmanage.desk.form.assign_to import add as assign_to
...
class AutoRepeat(Document):
	...
	if TYPE_CHECKING:
		from dontmanage.automation.doctype.auto_repeat_day.auto_repeat_day import AutoRepeatDay
		from dontmanage.automation.doctype.auto_repeat_user.auto_repeat_user import AutoRepeatUser
	...
	def on_update(self):
		dontmanage.get_doc(self.reference_doctype, self.reference_document).notify_update()

	def on_trash(self):
		dontmanage.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", "")
		dontmanage.get_doc(self.reference_doctype, self.reference_document).notify_update()
	...
	def set_dates(self):
		if self.disabled:
			...
			if self.end_date and getdate(self.end_date) < getdate(self.next_schedule_date):
				dontmanage.throw(_("The Next Scheduled Date cannot be later than the End Date."))
	...
	def validate_reference_doctype(self):
		if dontmanage.in_test or dontmanage.flags.in_patch:
			return
		if not dontmanage.get_meta(self.reference_doctype).allow_auto_repeat:
			dontmanage.throw(
	...
	def validate_submit_on_creation(self):
		if self.submit_on_creation and not dontmanage.get_meta(self.reference_doctype).is_submittable:
			dontmanage.throw(
				_("Cannot enable {0} for a non-submittable doctype").format(
					dontmanage.bold(_("Submit on Creation"))
				)
			)
	...
	def validate_dates(self):
		...
		if self.end_date:
			...
			if end_date == getdate():
				dontmanage.throw(_("End Date cannot be today."))
			if end_date == getdate(self.start_date):
				dontmanage.throw(
					_("{0} should not be same as {1}").format(
						dontmanage.bold(_("End Date")), dontmanage.bold(_("Start Date"))
					)
				)
			...
			if self.recipients:
				...
				for email in email_list:
					if not validate_email_address(email):
						dontmanage.throw(_("{0} is an invalid email address in 'Recipients'").format(email))
			else:
				dontmanage.throw(_("'Recipients' not specified"))
	...
	def update_auto_repeat_id(self):
		...
		auto_repeat = dontmanage.db.get_value(self.reference_doctype, self.reference_document, "auto_repeat")
		if auto_repeat and auto_repeat != self.name and not dontmanage.flags.in_patch:
			dontmanage.throw(
				_("The {0} is already on auto repeat {1}").format(self.reference_document, auto_repeat)
			)
		else:
			dontmanage.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", self.name)
	...
	def create_documents(self):
		try:
			...
			if self.reference_document and not dontmanage.in_test:
	...
	def make_new_document(self, assignee=None):
		reference_doc = dontmanage.get_doc(self.reference_doctype, self.reference_document)
		new_doc = dontmanage.copy_doc(reference_doc, ignore_no_copy=False)
	...
	def update_doc(self, new_doc, reference_doc):
		...
		auto_repeat_doc = dontmanage.get_doc("Auto Repeat", self.name)
	...
	def set_auto_repeat_period(self, new_doc):
		...
		if mcount and new_doc.meta.get_field("from_date") and new_doc.meta.get_field("to_date"):
			last_ref_doc = dontmanage.get_all(
				doctype=self.reference_doctype,
	...
	def send_notification(self, new_doc):
		...
		if not self.subject:
			...
			subject = dontmanage.render_template(self.subject, {"doc": new_doc})
		...
		try:
			...
			).format(dontmanage.bold(_("Note")), dontmanage.bold(_("Allow Print for Draft")))
			attachments = None
		...
		if error_string:
			...
			message = dontmanage.render_template(self.message, {"doc": new_doc})
	...
	def fetch_linked_contacts(self):
		if self.reference_doctype and self.reference_document:
			...
			if not email_ids:
				dontmanage.msgprint(_("No contacts linked to document"), alert=True)
			else:
				self.recipients = ", ".join(email_ids)

	def disable_auto_repeat(self):
		dontmanage.db.set_value("Auto Repeat", self.name, "disabled", 1)
	...
	def notify_error_to_user(self, error_log):
		...
		form_link = dontmanage.utils.get_link_to_form(self.reference_doctype, self.reference_document)
		auto_repeat_failed_for = _("Auto Repeat failed for {0}").format(form_link)

		error_log_link = dontmanage.utils.get_link_to_form("Error Log", error_log.name)
...
def make_auto_repeat_entry():
	enqueued_method = "dontmanage.automation.doctype.auto_repeat.auto_repeat.create_repeated_entries"
	jobs = get_jobs()

	if not jobs or enqueued_method not in jobs[dontmanage.local.site]:
		date = getdate(today())
		data = get_auto_repeat_entries(date)
		dontmanage.enqueue(enqueued_method, data=data, queue="long")


def create_repeated_entries(data):
	for d in data:
		doc = dontmanage.get_doc("Auto Repeat", d.name)
		...
		if schedule_date == current_date and not doc.disabled:
			...
			if schedule_date and not doc.disabled:
				dontmanage.db.set_value("Auto Repeat", doc.name, "next_schedule_date", schedule_date)
...
def get_auto_repeat_entries(date=None):
	...
	auto_repeat = dontmanage.qb.DocType("Auto Repeat")
	query = dontmanage.qb.from_(auto_repeat)
...
def make_auto_repeat(doctype, docname, frequency="Daily", start_date=None, end_date=None):
	...
	doc = dontmanage.new_doc("Auto Repeat")
...
def update_reference(docname: str, reference: str):
	doc = dontmanage.get_doc("Auto Repeat", str(docname))
...
def generate_message_preview(name: str):
	dontmanage.has_permission("Auto Repeat", "write", throw=True)
	auto_repeat = dontmanage.get_doc("Auto Repeat", str(name))
	doc = dontmanage.get_doc(auto_repeat.reference_doctype, auto_repeat.reference_document)
	doc.check_permission()
	subject_preview = _("Please add a subject to your email")
	msg_preview = dontmanage.render_template(auto_repeat.message, {"doc": doc})
	if auto_repeat.subject:
		subject_preview = dontmanage.render_template(auto_repeat.subject, {"doc": doc})

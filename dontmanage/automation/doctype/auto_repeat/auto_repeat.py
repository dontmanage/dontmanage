...
class AutoRepeat(Document):
	...
	def validate_reference_doctype(self):
		if dontmanage.flags.in_test or dontmanage.flags.in_patch:
			return
	...
	def validate_submit_on_creation(self):
		if self.submit_on_creation and not dontmanage.get_meta(self.reference_doctype).is_submittable:
			dontmanage.throw(
				_("Cannot enable {0} for a non-submittable doctype").format(dontmanage.bold("Submit on Creation"))
			)
	...
	def validate_dates(self):
		...
		if self.end_date == self.start_date:
			dontmanage.throw(
				_("{0} should not be same as {1}").format(dontmanage.bold("End Date"), dontmanage.bold("Start Date"))
			)
	...
	def create_documents(self):
		try:
			...
			if self.reference_document and not dontmanage.flags.in_test:
...
def make_auto_repeat_entry():
	...
	if not jobs or enqueued_method not in jobs[dontmanage.local.site]:
		...
		dontmanage.enqueue(enqueued_method, data=data)
...
def set_auto_repeat_as_completed():
	auto_repeat = dontmanage.get_all("Auto Repeat", filters={"status": ["!=", "Disabled"]})
	for entry in auto_repeat:
		doc = dontmanage.get_doc("Auto Repeat", entry.name)
		if doc.is_completed():
...
def generate_message_preview(reference_dt, reference_doc, message=None, subject=None):
	...
	doc = dontmanage.get_doc(reference_dt, reference_doc)
	doc.check_permission()
	subject_preview = _("Please add a subject to your email")
	msg_preview = dontmanage.render_template(message, {"doc": doc})
	if subject:
		subject_preview = dontmanage.render_template(subject, {"doc": doc})

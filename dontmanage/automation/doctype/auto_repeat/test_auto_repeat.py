...
class TestAutoRepeat(IntegrationTestCase):
	...
	def monthly_auto_repeat(self, doctype, docname, start_date, end_date=None):
		...
		doc = dontmanage.get_doc("Auto Repeat", doc.name)
	...
	def test_submit_on_creation(self):
		...
		submittable_doc = dontmanage.get_doc(doctype=doctype, test="test submit on creation").insert()
	...
	def test_auto_repeat_assignee_with_separate_documents(self):
		...
		new_todo_count = dontmanage.db.count("ToDo", {"auto_repeat": doc.name, "name": ("!=", todo.name)}, "name")
	...
	return dontmanage.get_doc(
		{
			...
			"reference_document": args.reference_document or dontmanage.db.get_value("ToDo", "name"),
...
def create_submittable_doctype(doctype, submit_perms=1):
	if dontmanage.db.exists("DocType", doctype):
		return
	else:

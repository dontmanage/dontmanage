...
def has_permission(
	...
	*,
	...
	:param raise_exception:
	        ...
	        If not False, will display a message using dontmanage.msgprint
	...
	if doc:
		if isinstance(doc, str | int):
			doc = dontmanage.get_doc(meta.name, doc)
		...
		if not perm:
			...
			if dontmanage.has_permission(doc.doctype):
...
def has_user_permission(doc, user=None, debug=False):
	...
	def check_user_permission_on_link_fields(d):
		...
		meta = dontmanage.get_meta(d.get("doctype"))
...
def has_controller_permissions(doc, ptype, user=None, debug=False) -> bool:
	...
	methods = dontmanage.get_hooks("has_permission").get(doc.doctype, [])
...
def remove_user_permission(doctype, name, user):
	...
	dontmanage.delete_doc("User Permission", user_permission_name)
...
def clear_user_permissions_for_doctype(doctype, user=None):
	...
	for d in user_permissions_for_doctype:
		dontmanage.delete_doc("User Permission", d.name)
...
def can_export(doctype, raise_exception=False):
	if "System Manager" in dontmanage.get_roles():
		...
		role_permissions = dontmanage.permissions.get_role_permissions(doctype)
...
def update_permission_property(
	...
	dontmanage.qb.update(table).set(ptype, value).where(table.name == name).run()
...
def reset_perms(doctype):
	...
	dontmanage.db.delete("Custom DocPerm", {"parent": doctype})

...
def print_has_permission_check_logs(func):
	...
	def inner(*args, **kwargs):
		...
		self_perm_check = True if not kwargs.get("user") else kwargs.get("user") == dontmanage.session.user

		if print_logs:
			dontmanage.flags["has_permission_check_logs"] = []
		...
		if not result and self_perm_check and print_logs:
			msgprint(("<br>").join(dontmanage.flags.get("has_permission_check_logs", [])))

		if print_logs:
			dontmanage.flags.pop("has_permission_check_logs", None)
		return result
...
def _debug_log(log: str):
	if not hasattr(dontmanage.local, "permission_debug_log"):
		dontmanage.local.permission_debug_log = []
	dontmanage.local.permission_debug_log.append(log)


def _pop_debug_log() -> list[str]:
	if log := getattr(dontmanage.local, "permission_debug_log", None):
		del dontmanage.local.permission_debug_log
		return log
	return []
...
def has_permission(
	...
	*,
	...
	:param print_logs: If True, will display a message using dontmanage.msgprint
	...
	if ptype == "share" and dontmanage.get_system_settings("disable_document_sharing"):
	...
	if doc:
		if isinstance(doc, str | int):
			...
			doc = dontmanage.get_lazy_doc(meta.name, doc)
		...
		if not perm:
			...
			msg = _("User {0} does not have access to this document").format(dontmanage.bold(user))
			if meta.issingle:
				msg += f": {_(doc.doctype)}"
		...
		debug and _debug_log(
			...
			+ dontmanage.as_json(role_permissions, indent=8)
		)
		...
		if not perm:
			push_perm_check_log(
				_("User {0} does not have doctype access via role permission for document {1}").format(
					dontmanage.bold(user), dontmanage.bold(_(doctype))
				),
				debug=debug,
			)
	...
	def false_if_not_shared():
		...
		if doc:
			...
			shared = dontmanage.share.get_shared(
				doctype,
				user,
				rights=rights,
		...
		elif dontmanage.share.get_shared(doctype, user, rights=rights, limit=1):
...
def get_doc_permissions(doc, user=None, ptype=None, debug=False):
	...
	debug and _debug_log(
		"User has following permissions using role permission system: "
		+ dontmanage.as_json(permissions, indent=8)
	)
	...
	if permissions.get("has_if_owner_enabled"):
		...
		debug and _debug_log(
			"User is owner of document, so permissions are updated to: " + dontmanage.as_json(permissions)
		)
...
def get_role_permissions(doctype_meta, user=None, is_owner=None, debug=False):
	...
	if isinstance(doctype_meta, str):
		doctype_meta = dontmanage.get_meta(doctype_meta)  # assuming doctype name was passed
	...
	if not dontmanage.local.role_permissions.get(cache_key) or debug:
		perms = dontmanage._dict(if_owner={})
		...
		dontmanage.local.role_permissions[cache_key] = perms

	return dontmanage.local.role_permissions[cache_key]


def get_user_permissions(user):
	from dontmanage.core.doctype.user_permission.user_permission import get_user_permissions
...
def has_user_permission(doc, user=None, debug=False, *, ptype=None):
	...
	apply_strict_user_permissions = (
		False if doc.meta.issingle else dontmanage.get_system_settings("apply_strict_user_permissions")
	)
	...
	if (
		...
		and (not docname or (docname and not dontmanage.db.exists(doctype, docname, cache=True)))
	):
	...
	def check_user_permission_on_link_fields(d):
		...
		meta = dontmanage.get_meta(d.doctype)
...
def has_controller_permissions(doc, ptype, user=None, debug=False) -> bool:
	...
	hooks = dontmanage.get_hooks("has_permission")
	methods = hooks.get(doc.doctype, []) + hooks.get("*", [])

	for method in reversed(methods):
		controller_permission = dontmanage.call(method, doc=doc, ptype=ptype, user=user, debug=debug)
...
def get_all_perms(role):
	...
	perms = dontmanage.get_all("DocPerm", fields="*", filters=dict(role=role))
	custom_perms = dontmanage.get_all("Custom DocPerm", fields="*", filters=dict(role=role))
	doctypes_with_custom_perms = dontmanage.get_all("Custom DocPerm", pluck="parent", distinct=True)
...
def get_roles(user=None, with_standard=True):
	...
	def get():
		if user == "Administrator":
			return dontmanage.get_all("Role", pluck="name")  # return all available roles
		else:
	...
	roles = dontmanage.cache.hget("roles", user, get)
...
def get_perms_for(roles, perm_doctype="DocPerm"):
	...
	return dontmanage.get_all(perm_doctype, fields=["*"], filters=filters)


def get_doctypes_with_custom_docperms():
	"""Return all the doctypes with Custom Docperms."""

	doctypes = dontmanage.get_all("Custom DocPerm", fields=["parent"], distinct=1)
...
def add_user_permission(
	...
	"""Add user permission"""
	from dontmanage.core.doctype.user_permission.user_permission import user_permission_exists

	if not user_permission_exists(user, doctype, name, applicable_for):
		if not dontmanage.db.exists(doctype, name):
			dontmanage.throw(_("{0} {1} not found").format(_(doctype), name), dontmanage.DoesNotExistError)
...
def remove_user_permission(doctype, name, user):
	user_permission_name = dontmanage.db.get_value(
		"User Permission", dict(user=user, allow=doctype, for_value=name)
	)
	dontmanage.delete_doc("User Permission", user_permission_name, force=True)
...
def clear_user_permissions_for_doctype(doctype, user=None):
	...
	if user:
		filters["user"] = user
	user_permissions_for_doctype = dontmanage.get_all("User Permission", filters=filters)
	for d in user_permissions_for_doctype:
		dontmanage.delete_doc("User Permission", d.name, force=True)


def can_import(doctype, raise_exception=False):
	if not ("System Manager" in dontmanage.get_roles() or has_permission(doctype, "import")):
		if raise_exception:
			raise dontmanage.PermissionError(f"You are not allowed to import: {doctype}")
		else:
			return False
	return True
...
def can_export(doctype, raise_exception=False, is_owner=False):
	if "System Manager" in dontmanage.get_roles():
		...
		role_permissions = dontmanage.permissions.get_role_permissions(doctype, is_owner=is_owner)
		has_access = role_permissions.get("export") or role_permissions.get("if_owner").get("export")
		if not has_access and raise_exception:
			raise dontmanage.PermissionError(_("You are not allowed to export {} doctype").format(doctype))
		return has_access
...
def update_permission_property(
	...
	from dontmanage.core.doctype.custom_docperm.custom_docperm import update_custom_docperm
	from dontmanage.core.doctype.doctype.doctype import validate_permissions_for_doctype

	out = setup_custom_perms(doctype)

	custom_docperm = dontmanage.db.get_value(
...
def setup_custom_perms(parent):
	...
	if not dontmanage.db.exists("Custom DocPerm", dict(parent=parent)):
		copy_perms(parent)
		return True
...
def add_permission(doctype, role, permlevel=0, ptype=None):
	...
	custom_docperm = dontmanage.get_doc(
		{
...
def copy_perms(parent):
	...
	for d in dontmanage.get_all("DocPerm", fields="*", filters=dict(parent=parent)):
		custom_perm = dontmanage.new_doc("Custom DocPerm")
...
def reset_perms(doctype):
	...
	for custom_docperm in dontmanage.get_all("Custom DocPerm", filters={"parent": doctype}, pluck="name"):
		dontmanage.delete_doc("Custom DocPerm", custom_docperm, ignore_permissions=True, force=True)


def get_linked_doctypes(dt: str) -> list:
	meta = dontmanage.get_meta(dt)
	linked_doctypes = [dt] + [
		d.options
		for d in meta.get(
			"fields",
...
def push_perm_check_log(log, debug=False):
	...
	if dontmanage.flags.get("has_permission_check_logs") is None:
		return

	dontmanage.flags.get("has_permission_check_logs").append(log)
...
def has_child_permission(
	...
	*,
	...
	if isinstance(child_doc, str):
		child_doc = dontmanage.db.get_value(
			child_doctype,
			child_doc,
	...
	if not parent_doctype:
		push_perm_check_log(
			_("Please specify a valid parent DocType for {0}").format(dontmanage.bold(child_doctype)),
			debug=debug,
		)
		return False

	parent_meta = dontmanage.get_meta(parent_doctype)
	...
	if parent_meta.istable or not (
		...
		push_perm_check_log(
			_("{0} is not a valid parent DocType for {1}").format(
				dontmanage.bold(parent_doctype), dontmanage.bold(child_doctype)
			),
			debug=debug,
		)
		return False
	...
	if child_doc:
		...
		if not parentfield:
			push_perm_check_log(
				_("Parentfield not specified in {0}: {1}").format(
					dontmanage.bold(child_doctype), dontmanage.bold(child_doc.name)
				),
				debug=debug,
			)
			return False
		...
		if parentfield not in valid_parentfields:
			push_perm_check_log(
				_("{0} is not a valid parentfield for {1}").format(
					dontmanage.bold(parentfield), dontmanage.bold(child_doctype)
				),
				debug=debug,
			)
			return False
		...
		if permlevel > 0 and permlevel not in accessible_permlevels:
			push_perm_check_log(
				_("Insufficient Permission Level for {0}").format(dontmanage.bold(parent_doctype)), debug=debug
			)
			debug and _debug_log(
...
def is_system_user(user: str | None = None) -> bool:
	return dontmanage.get_cached_value("User", user or dontmanage.session.user, "user_type") == "System User"
...
def check_doctype_permission(doctype: str, ptype: str = "read") -> None:
	...
	_message_log = dontmanage.local.message_log
	dontmanage.local.message_log = []
	try:
		dontmanage.has_permission(doctype, ptype, throw=True, ignore_share_permissions=True)
	except dontmanage.PermissionError:
		dontmanage.flags.disable_traceback = True
		raise

	dontmanage.local.message_log = _message_log
...
def handle_does_not_exist_error(fn):
	...
	def wrapper(e, *args, **kwargs):
		if isinstance(e, dontmanage.DoesNotExistError) and (doctype := getattr(e, "doctype", None)):
			try:
				check_doctype_permission(doctype)
			except dontmanage.PermissionError as _e:
...
def _get_parent_and_ancestors(doctype, parent):
	...
	from dontmanage.utils.nestedset import get_ancestors_of

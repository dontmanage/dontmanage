...
class ParallelTestRunner:
	def __init__(self, app, site, build_number=1, total_builds=1, dry_run=False, lightmode=False):
		self.app = app
		self.site = site
		self.build_number = dontmanage.utils.cint(build_number) or 1
		self.total_builds = dontmanage.utils.cint(total_builds)
	...
	def before_test_setup(self):
		...
		for fn in dontmanage.get_hooks("before_tests", app_name=self.app):
			dontmanage.get_attr(fn)()

		test_module = dontmanage.get_module(f"{self.app}.tests")
	...
	def run_tests_for_file(self, file_info):
		...
		if not self.lightmode:
			from dontmanage.deprecation_dumpster import compat_preload_test_records_upfront
	...
	def get_module(self, path, filename):
		app_path = dontmanage.get_app_path(self.app)
		...
		return dontmanage.get_module(module_name)
...
class ParallelTestWithOrchestrator(ParallelTestRunner):
	...
	def __init__(self, app, site):
		...
		if not self.orchestrator_url:
			...
			click.echo("Pass public URL after hosting https://github.com/dontmanage/test-orchestrator")
			sys.exit(1)

		self.ci_build_id = os.environ.get("CI_BUILD_ID")
		self.ci_instance_id = os.environ.get("CI_INSTANCE_ID") or dontmanage.generate_hash(length=10)

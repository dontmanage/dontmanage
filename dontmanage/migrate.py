...
from dontmanage.core.doctype.navbar_settings.navbar_settings import sync_standard_items
...
from dontmanage.utils.data import cint, comma_and
...
class SiteMigration:
	...
	def pre_schema_updates(self):
		...
		for app in dontmanage.get_installed_apps():
			...
			for doctype in dontmanage.get_hooks("override_doctype_class", {}, app_name=app).keys():
	...
	def post_schema_updates(self):
		...
		* Execute `after_migrate` hooks
		...
		dontmanage.model.sync.remove_orphan_entities()
		dontmanage.model.sync.delete_duplicate_icons()
	...
	def lower_lock_timeout(self):
		...
		if dontmanage.db.db_type != "mariadb":
			return
		dontmanage.db.sql("set session lock_wait_timeout = %s", 5 * 60)
	...
	def kill_idle_connections(self, idle_limit=30):
		...
		processes = dontmanage.db.sql("show full processlist", as_dict=1)
		connection_id = dontmanage.db.sql("select connection_id()")[0][0]
		for process in processes:
			...
			if (
				...
				and process.db == dontmanage.conf.db_name
				and sleeping
				and sleeping_since > idle_limit
				and user != "system user"
			):
				try:
					dontmanage.db.sql(f"kill {pid}")
...
class DBQueryProgressMonitor(threading.Thread):
	...
	def __init__(self) -> None:
		super().__init__()
		self.site = dontmanage.local.site
		self.daemon = True
		self._running = threading.Event()
		if dontmanage.db.db_type == "mariadb":
			self.conn_id = dontmanage.db.sql("select connection_id()")[0][0]
			self.start()
	...
	def run(self):
		...
		while self._running.is_set():
			...
			query = dontmanage._dict(queries[0])

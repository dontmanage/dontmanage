"""
DontManageClient is a library that helps you connect with other dontmanage systems
"""
import base64
import json
...
class DontManageClient:
	def __init__(
		...
		api_key=None,
		api_secret=None,
		dontmanage_authorization_source=None,
	):
		import requests
		...
		self.dontmanage_authorization_source = dontmanage_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.dontmanage_authorization_source:
				auth_source = {"DontManage-Authorization-Source": self.dontmanage_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": dontmanage.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return dontmanage._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "dontmanage.client.insert_many", "docs": dontmanage.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": dontmanage.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return dontmanage._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "dontmanage.client.bulk_update", "docs": dontmanage.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "dontmanage.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "dontmanage.client.submit", "doc": dontmanage.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "dontmanage.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": dontmanage.as_json(filters),
			}
		)
	...
	def set_value(self, doctype, docname, fieldname, value):
		...
		return self.post_request(
			{
				"cmd": "dontmanage.client.set_value",
				"doctype": doctype,
				"name": docname,
				"fieldname": fieldname,
				"value": value,
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "dontmanage.client.cancel", "doctype": doctype, "name": name})
	...
	def rename_doc(self, doctype, old_name, new_name):
		...
		params = {
			"cmd": "dontmanage.client.rename_doc",
			"doctype": doctype,
			"old_name": old_name,
			"new_name": new_name,
		}
	...
	def migrate_doctype(self, doctype, filters=None, update=None, verbose=1, exclude=None, preprocess=None):
		...
		for doc in docs:
			...
			if doctype != "User" and not dontmanage.db.exists("User", doc.get("owner")):
				dontmanage.get_doc(
					{
						"doctype": "User",
						"email": doc.get("owner"),
	...
	def migrate_single(self, doctype):
		...
		doc.modified = dontmanage.db.get_single_value(doctype, "modified")
	...
	def post_process(self, response):
		...
		if rjson and ("exc" in rjson) and rjson["exc"]:
			try:
				...
				exc = "DontManageClient Request Failed\n\n" + exc
			except Exception:
				exc = rjson["exc"]

			raise DontManageException(exc)
		if "message" in rjson:
...
class DontManageOAuth2Client(DontManageClient):

...
from dontmanage.desk.search import PAGE_LENGTH_FOR_LINK_VALIDATION, search_widget
from dontmanage.utils import attach_expanded_links, get_safe_filters
...
def get_list(
	...
	validate_args(args)
	_list = dontmanage.get_list(**args)
...
def get_count(doctype, filters=None, debug=False, cache=False):
	...
	dontmanage.form_dict.filters = get_safe_filters(filters)
	dontmanage.form_dict.debug = debug
...
def get_doc_permissions(doctype: str, docname: str):
	...
	"""
	doc = dontmanage.get_lazy_doc(doctype, docname)
...
def get_password(doctype: str, name: str, fieldname: str):
	...
	return dontmanage.get_lazy_doc(doctype, name).get_password(fieldname)


from dontmanage.deprecation_dumpster import get_js as _get_js

get_js = dontmanage.whitelist()(_get_js)
...
def attach_file(
	...
	doc = dontmanage.get_lazy_doc(doctype, docname, check_permission=True)
...
@dontmanage.whitelist(methods=["GET", "POST"])
def validate_link_and_fetch(
	...
	**search_args,
	...
	if not docname:
		dontmanage.throw(_("Document Name must not be empty"))

	meta = dontmanage.get_meta(doctype)
	fields_to_fetch = dontmanage.parse_json(fields_to_fetch)

	# only cache is no fields to fetch and request is GET
	can_cache = not fields_to_fetch and dontmanage.request.method == "GET"
	...
	if is_virtual_dt:
		try:
			...
			doc.check_permission("select" if dontmanage.only_has_select_perm(doctype) else "read")
		...
		values = dontmanage.db.get_value(doctype, docname, columns_to_fetch, as_dict=True)
	...
	if not fields_to_fetch:
		if can_cache:
			dontmanage.local.response_headers.set(

...
def new_catalog(app: str, locale: str | None = None) -> Catalog:
	def get_hook(hook, app):
		return dontmanage.get_hooks(hook, [None], app)[0]
...
def get_po_dir(app: str) -> Path:
	return Path(dontmanage.get_app_path(app)) / PO_DIR
...
def get_method_map(app: str):
	file_path = Path(dontmanage.get_app_path(app)).parent / "babel_extractors.csv"
	if file_path.exists():
...
def generate_pot(target_app: str | None = None):
	...
	apps = [target_app] if target_app else dontmanage.get_all_apps(True)
	default_method_map = get_method_map("dontmanage")
	...
	for app in apps:
		...
		method_map = [] if app == "dontmanage" else get_method_map(app)
...
def _get_ignored_strings(app: str) -> set[tuple[str, str | None]]:
	...
	Example:
	    If [app]/hooks.py contains:
	        ignore_translatable_strings_from = ["dontmanage"]

	    Then this will return a set of tuples (message, context) with all
	    entries from dontmanage's POT file.
	"""
	ignored_strings = set()
	for ignore_app in dontmanage.get_hooks("ignore_translatable_strings_from", [], app_name=app):
		if ignore_app == app:
			raise ValueError(
...
def get_is_gitignored_function_for_app(app: str | None):
	...
	repo = git.Repo(dontmanage.get_app_source_path(app), search_parent_directories=True)
...
def new_po(locale, target_app: str | None = None):
	...
	for app in apps:
		...
		print(
			"You will need to add the language in dontmanage/geo/languages.csv, if you haven't done it already."
		)
...
def migrate(app: str | None = None, locale: str | None = None):
	apps = [app] if app else dontmanage.get_all_apps(True)
...
def csv_to_po(app: str, locale: str):
	csv_file = Path(dontmanage.get_app_path(app)) / "translations" / f"{locale.replace('_', '-')}.csv"
...
def get_translations_from_mo(lang, app):
	...
	translations = {}
	lang = lang.replace("-", "_")  # DontManage uses dash, babel uses underscore.

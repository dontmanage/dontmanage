...
class LoginLimitExceeded(dontmanage.ValidationError):
	pass
...
class EmailServer:
	...
	def connect_pop(self):
		...
		try:
			...
			if self.is_temporary_system_problem(e):
				...
				dontmanage.msgprint(_("Invalid User Name or Support Password. Please rectify and try again."))
				raise
	...
	def check_imap_uidvalidity(self, folder):
		...
		dontmanage.db.set_value("Email Account", self.settings.email_account, "uidnext", uidnext)
		...
		if not uid_validity or uid_validity != current_uid_validity:
			...
			dontmanage.qb.update(Communication).set(Communication.uid, -1).where(
			...
			if self.settings.use_imap:
				...
				EmailAccount = dontmanage.qb.DocType("Email Account")
				dontmanage.qb.update(EmailAccount).set(EmailAccount.uidvalidity, current_uid_validity).set(
	...
	def get_email_seen_status(self, uid, flag_string):
		...
		for flag in imaplib.ParseFlags(flag_string) or []:
			match = WORDS_PATTERN.search(dontmanage.as_unicode(flag))
...
class Email:
	...
	def set_from(self):
		...
		if _reply_to and not dontmanage.db.get_value("Email Account", {"email_id": _reply_to}, "email_id"):
	...
	def save_attachments_in_doc(self, doc):
		...
		for attachment in self.attachments:
			try:
				...
				pass
			except dontmanage.FileAlreadyAttachedException:
				pass
...
class InboundMail(Email):
	...
	def _build_communication_doc(self):
		...
		communication = dontmanage.get_doc(data)
	...
	def is_reply_to_system_sent_mail(self):
		...
		return self.is_reply() and dontmanage.local.site in self.in_reply_to
	...
	def match_record_by_subject_and_sender(self, doctype):
		...
		3. Find by matching subject alone (Special case)
		        ...
		        the system user is replying to via the common email account in DontManage. This fix bypasses
		...
		if not record:
			...
			name = dontmanage.db.get_value(self.email_account.append_to, filters=filters)
			record = self.get_doc(doctype, name, ignore_error=True) if name else None
		return record
	...
	def get_doc(doctype, docname, ignore_error=False):
		try:
			return dontmanage.get_doc(doctype, docname)
	...
	def get_document(self, doctype, name):
		"""Is same as dontmanage.get_doc but suppresses the DoesNotExist error."""
		try:
			return dontmanage.get_doc(doctype, name)
		except dontmanage.DoesNotExistError:
			return None

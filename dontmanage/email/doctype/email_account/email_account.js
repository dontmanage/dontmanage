...
function oauth_access(frm) {
	dontmanage.model.with_doc("Connected App", frm.doc.connected_app, () => {
		const connected_app = dontmanage.get_doc("Connected App", frm.doc.connected_app);
...
function set_default_max_attachment_size(frm) {
	if (frm.doc.__islocal && !frm.doc["attachment_limit"]) {
		dontmanage.call({
			method: "dontmanage.core.api.file.get_max_file_size",
			callback: function (r) {
				if (!r.exc) {
...
function add_helpful_links(frm) {
	...
	if (frm.doc.service === "DontManage Mail") {
		frm.set_df_property(
			"api_secret",
			"description",
			__("To know more click {0}", [
				"<a href='https://github.com/dontmanage/mail' target='_blank'>" + __("here") + "</a>",
			])
		);
	} else {
...
dontmanage.ui.form.on("Email Account", {
	service: function (frm) {
		$.each(dontmanage.email_defaults[frm.doc.service], function (key, value) {
			frm.set_value(key, value);
		});
		if (!frm.doc.use_imap) {
			$.each(dontmanage.email_defaults_pop[frm.doc.service], function (key, value) {
				frm.set_value(key, value);
			});
		}
	...
	onload: function (frm) {
		...
		frm.set_query(
			"append_to",
			"dontmanage.email.doctype.email_account.email_account.get_append_to"
		);
		frm.set_query("append_to", "imap_folder", function () {
			return {
				query: "dontmanage.email.doctype.email_account.email_account.get_append_to",
			};
		});
	...
	refresh: function (frm) {
		...
		if (dontmanage.route_flags.delete_user_from_locals && dontmanage.route_flags.linked_user) {
			delete dontmanage.route_flags.delete_user_from_locals;
			delete locals["User"][dontmanage.route_flags.linked_user];
		}

		if (!frm.is_dirty() && frm.doc.enable_incoming) {
			frm.add_custom_button(__("Pull Emails"), () => {
				dontmanage.dom.freeze(__("Pulling emails..."));
				frm.call({
					method: "pull_emails",
					...
					if (!(r._server_messages && r._server_messages.length)) {
						dontmanage.show_alert({ message: __("Emails Pulled"), indicator: "green" });
					}
				});
			});
		}
	...
	validate_dontmanage_mail_settings: function (frm) {
		if (frm.doc.service == "DontManage Mail") {
			dontmanage.call({
				doc: frm.doc,
				method: "validate_dontmanage_mail_settings",
			});
		}
	},
	...
	show_oauth_authorization_message(frm) {
		if (
			...
			dontmanage.call({
				method: "dontmanage.integrations.doctype.connected_app.connected_app.has_token",
				args: {
	...
	domain: dontmanage.utils.debounce((frm) => {
		if (frm.doc.domain) {
			dontmanage.call({
	...
	email_sync_option: function (frm) {
		...
		if (frm.doc.email_sync_option == "ALL") {
			...
			dontmanage.confirm(msg, null, function () {
	...
	warn_autoreply_on_incoming: function (frm) {
		if (frm.doc.enable_incoming && frm.doc.enable_auto_reply && frm.doc.__islocal) {
			...
			dontmanage.confirm(msg, null, function () {
				...
				dontmanage.show_alert({ message: __("Disabled Auto Reply"), indicator: "blue" });
			});
		}
	},
});

...
class TestEmailAccount(DontManageTestCase):
	...
	def setUp(self):
		...
		dontmanage.db.delete("Unhandled Email")
	...
	def test_incoming_with_attach(self):
		...
		existing_file = dontmanage.get_doc({"doctype": "File", "file_name": "dontmanageerp-conf-14.png"})
		dontmanage.delete_doc("File", existing_file.name)
		...
		self.assertTrue("dontmanageerp-conf-14.png" in [f.file_name for f in attachments])
	...
	def test_outgoing(self):
		...
		mail = email.message_from_string(dontmanage.get_last_doc("Email Queue").message)
	...
	def test_sendmail(self):
		...
		sent_mail = email.message_from_string(dontmanage.safe_decode(dontmanage.flags.sent_mail))
	...
	def test_print_format(self):
		...
		sent_mail = email.message_from_string(dontmanage.get_last_doc("Email Queue").message)
	...
	def test_threading(self):
		...
		sent_name = make(
			...
			doctype="ToDo",
			name=dontmanage.get_last_doc("ToDo").name,
			send_email=True,
		)["name"]
		...
		sent = dontmanage.get_doc("Communication", sent_name)
	...
	def test_threading_by_message_id(self):
		...
		event = dontmanage.get_doc(dict(doctype="Event", subject="test-message")).insert()
		...
		last_mail = dontmanage.get_doc("Email Queue", dict(reference_name=event.name))
	...
	def test_handle_bad_emails(self):
		...
		self.assertTrue(dontmanage.db.get_value("Unhandled Email", {"message_id": message_id}))
	...
	def test_append_to(self):
		...
		self.assertTrue(dontmanage.db.exists(communication.reference_doctype, communication.reference_name))
	...
	def test_append_to_with_imap_folders(self):
		...
		for mail in mails:
			...
			if mail.append_to == "ToDo":
				...
				self.assertTrue(communication.reference_name)
				self.assertTrue(
					dontmanage.db.exists(communication.reference_doctype, communication.reference_name)
				)
			else:
				test_folder_mails += 1
...
class TestInboundMail(DontManageTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		email_account = dontmanage.get_doc("Email Account", "_Test Email Account 1")
	...
	def test_find_parent_communication_from_header(self):
		...
		communication = self.new_communication()
		mail_content = self.get_test_mail(fname="reply-4.raw").replace(
			"{{ message_id }}", f"<{communication.name}@{dontmanage.local.site}>"
		)
...
def cleanup(sender=None):
	...
	names = dontmanage.get_list("Communication", filters=filters, fields=["name"])
	for name in names:
		dontmanage.delete_doc_if_exists("Communication", name.name)
		dontmanage.delete_doc_if_exists("Communication Link", {"parent": name.name})

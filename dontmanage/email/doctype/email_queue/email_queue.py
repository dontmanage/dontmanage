...
import dontmanage
from dontmanage import _, are_emails_muted, safe_encode, task
...
from dontmanage.email.email_body import add_attachment, get_email, get_formatted_html
from dontmanage.email.dontmanagemail import DontManageMail
from dontmanage.email.queue import get_unsubcribed_url, get_unsubscribe_message
...
class EmailQueue(Document):
	...
	if TYPE_CHECKING:
		from dontmanage.email.doctype.email_queue_recipient.email_queue_recipient import EmailQueueRecipient
	...
	def prevent_email_queue_delete(self):
		if dontmanage.session.user != "Administrator":
			dontmanage.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = dontmanage.get_doc(values)
	...
	def new(cls, doc_data, ignore_permissions=False) -> EmailQueue:
		...
		doc = dontmanage.new_doc(cls.DOCTYPE)
		doc.update(data)
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = dontmanage.get_doc("Communication", self.communication)
	...
	def get_email_account(self, raise_error=False):
		if self.email_account:
			return dontmanage.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			dontmanage.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(dontmanage.db.get_default("suspend_email_queue")) == 1
		):
			return False
	...
	def send(
		...
		dontmanage_mail_client: DontManageMail = None,
		...
		with SendMailContext(self, smtp_server_instance, dontmanage_mail_client) as ctx:
			...
			for recipient in self.recipients:
				...
				elif not dontmanage.in_test or dontmanage.flags.testing_email:
					if ctx.email_account_doc.service == "DontManage Mail":
						is_newsletter = self.reference_doctype == "Newsletter"
						ctx.dontmanage_mail_client.send_raw(
							sender=self.sender,
			...
			if dontmanage.in_test and not dontmanage.flags.testing_email:
				dontmanage.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = dontmanage.qb.DocType("Email Queue")
		email_recipient = dontmanage.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			dontmanage.qb.from_(email_queue).delete().where(email_queue.creation < (Now() - Interval(days=days)))
		).run()
		...
		(
			dontmanage.qb.from_(email_recipient)
			.delete()
			.where(email_recipient.creation < (Now() - Interval(days=days)))
		).run()


from dontmanage.deprecation_dumpster import send_mail as _send_mail
...
class SendMailContext:
	def __init__(
		...
		self.dontmanage_mail_client: DontManageMail = dontmanage_mail_client
	...
	def fetch_outgoing_server(self):
		...
		if self.email_account_doc.service == "DontManage Mail":
			if not self.dontmanage_mail_client:
				self.dontmanage_mail_client = self.email_account_doc.get_dontmanage_mail_client()
	...
	def __exit__(self, exc_type, exc_val, exc_tb):
		if exc_type:
			update_fields = {"error": dontmanage.get_traceback()}
	...
	def get_tracker_str(self, recipient_email) -> str:
		...
		if self.queue_doc.get("email_read_tracker_url"):
			...
			tracker_url = f"{get_url()}/api/method/dontmanage.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = dontmanage.get_doc("File", file_filters)
				...
				print_format_file = dontmanage.attach_print(**attachment)
	...
	def _store_file(self, file_name, content):
		if not dontmanage.get_system_settings("store_attached_pdf_document"):
			return

		file_data = dontmanage._dict(file_name=file_name, is_private=1)
		...
		if dontmanage.db.exists("File", file_data):
			return

		file = dontmanage.new_doc("File", **file_data)
...
def retry_sending(queues: str | list[str]):
	if not dontmanage.has_permission("Email Queue", throw=True):
		return
	...
	email_queue = dontmanage.qb.DocType("Email Queue")
	dontmanage.qb.update(email_queue).set(email_queue.status, "Not Sent").set(email_queue.modified, now()).set(
		email_queue.modified_by, dontmanage.session.user
...
def toggle_sending(enable):
	...
	dontmanage.db.set_default("suspend_email_queue", 0 if sbool(enable) else 1)


def on_doctype_update():
	"""Add index in `tabCommunication` for `(reference_doctype, reference_name)`"""
	dontmanage.db.add_index("Email Queue", ("status", "send_after", "priority", "creation"), "index_bulk_flush")

	dontmanage.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(dontmanage.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def __init__(
		...
		:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/dontmanage.email.queue.unsubscribe`.
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/dontmanage.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				dontmanage.qb.from_(EmailUnsubscribe)
			...
			for att in self._attachments:
				if att.get("fid") or att.get("file_url"):
					...
					if not att.get("lang", None):
						att["lang"] = dontmanage.local.lang
	...
	def prepare_email_content(self):
		...
		if self.in_reply_to:
			if message_id := dontmanage.db.get_value("Communication", self.in_reply_to, "message_id"):
	...
	def process(self, send_now=False) -> EmailQueue | None:
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in dontmanage.utils.create_batch(final_recipients, 1000):
				dontmanage.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=dontmanage.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=dontmanage.in_test or send_now,
					queue="long",
				)
	...
	def send_emails(self, queue_data, final_recipients):
		...
		dontmanage_mail_client = None
		smtp_server_instance = None
		for r in final_recipients:
			recipients = list(set([r, *self.final_cc(), *self.final_bcc()]))
			q = EmailQueue.new({**queue_data, **{"recipients": recipients}}, ignore_permissions=True)
			if not dontmanage_mail_client and not smtp_server_instance:
				email_account = q.get_email_account(raise_error=True)

				if email_account.service == "DontManage Mail":
					dontmanage_mail_client = email_account.get_dontmanage_mail_client()
				else:
					smtp_server_instance = email_account.get_smtp_server()

			with suppress(Exception):
				q.send(smtp_server_instance=smtp_server_instance, dontmanage_mail_client=dontmanage_mail_client)
	...
	def as_dict(self, include_recipients=True):
		...
		except dontmanage.InvalidEmailAddressError:

dontmanage.listview_settings["Email Queue"] = {
...
function show_toggle_sending_button(list_view) {
	...
	const sending_disabled = cint(dontmanage.sys_defaults.suspend_email_queue);
	...
	list_view.page.add_inner_button(label, async () => {
		await dontmanage.xcall(
			"dontmanage.email.doctype.email_queue.email_queue.toggle_sending",
		...
		dontmanage.sys_defaults.suspend_email_queue = sending_disabled ? 0 : 1;
...
function add_bulk_retry_button_to_actions(list_view) {
	list_view.page.add_actions_menu_item(__("Retry Sending"), () => {
		...
		dontmanage.call({
			method: "dontmanage.email.doctype.email_queue.email_queue.retry_sending",
			args: {

...
from dontmanage.utils import add_to_date, get_link_to_form, today
from dontmanage.utils.data import is_html

# test_records = dontmanage.get_test_records('Auto Email Report')


class TestAutoEmailReport(DontManageTestCase):
	def test_auto_email(self):
		dontmanage.delete_doc("Auto Email Report", "Permitted Documents For User")
...
def get_auto_email_report():
	if not dontmanage.db.exists("Auto Email Report", "Permitted Documents For User"):
		auto_email_report = dontmanage.get_doc(
			dict(
		...
		auto_email_report = dontmanage.get_doc("Auto Email Report", "Permitted Documents For User")

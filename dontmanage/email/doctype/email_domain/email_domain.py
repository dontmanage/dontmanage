...
class EmailDomain(Document):
	def validate(self):
		...
		if dontmanage.local.flags.in_patch or dontmanage.local.flags.in_test or dontmanage.local.flags.in_install:
			return
	...
	def on_update(self):
		...
		for email_account in dontmanage.get_all("Email Account", filters={"domain": self.name}):
			try:

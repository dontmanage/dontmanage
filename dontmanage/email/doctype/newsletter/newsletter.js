...
dontmanage.ui.form.on("Newsletter", {
	refresh(frm) {
		...
		if (frm.is_new() && !doc.sender_email) {
			let { fullname, email } = dontmanage.user_info(doc.owner);
	...
	send_emails(frm) {
		dontmanage.dom.freeze(__("Queuing emails..."));
		frm.call("send_emails").then(() => {
			frm.refresh();
			dontmanage.dom.unfreeze();
			dontmanage.show_alert(
				__("Queued {0} emails", [dontmanage.utils.shorten_number(frm.doc.total_recipients)])
			);
		});
	},

	schedule_send_dialog(frm) {
		let hours = dontmanage.utils.range(24);
	...
	update_schedule_message(frm) {
		if (!frm.doc.email_sent && frm.doc.schedule_send) {
			let datetime = dontmanage.datetime.global_date_format(frm.doc.schedule_send);

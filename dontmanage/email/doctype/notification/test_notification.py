...
def get_test_notification(config):
	try:
		notification = dontmanage.get_doc(doctype="Notification", **config).insert()
		yield notification
	finally:
		notification.delete()
...
def get_test_doctype_with_attach_field(doctype_name="Test Attach Doctype"):
	...
	try:
		...
		if not dontmanage.db.exists("DocType", doctype_name):
			dontmanage.get_doc(
				{
					"doctype": "DocType",
					"name": doctype_name,
		...
		dontmanage.db.delete(doctype_name)
		dontmanage.delete_doc_if_exists("DocType", doctype_name, force=1)
		dontmanage.db.commit()
...
def create_test_file(file_name="test_attachment.txt", content="Test file content"):
	...
	try:
		...
		dontmanage.delete_doc_if_exists("File", file_doc.name, force=1)
		dontmanage.db.commit()
...
class TestNotification(IntegrationTestCase):
	def setUp(self):
		...
		if not dontmanage.db.exists("Notification", {"name": "ToDo Status Update"}, "name"):
			notification = dontmanage.new_doc("Notification")
		...
		if not dontmanage.db.exists("Notification", {"name": "Contact Status Update"}, "name"):
			notification = dontmanage.new_doc("Notification")
	...
	def test_condition(self):
		...
		event = dontmanage.new_doc("Event")
		event.subject = "test"
	...
	def test_invalid_condition(self):
		...
		recipent = dontmanage.new_doc("Notification Recipient")
		recipent.receiver_by_document_field = "owner"

		notification.recipents = recipent
		notification.condition = "test"

		self.assertRaises(dontmanage.ValidationError, notification.save)
		notification.delete()
	...
	def test_minutes_positive_offset(self):
		...
		with get_test_notification(notification) as n:
			dontmanage.db.delete("Notification Log", {"subject": n.subject})
			# Run the offset alerts
			trigger_notifications(None, "offset")

			# Check if the notification was triggered
			self.assertEqual(1, dontmanage.db.count("Notification Log", {"subject": n.subject}))
	...
	def test_minutes_offset_validation(self):
		...
		self.assertRaises(dontmanage.ValidationError, notification.insert)
	...
	def test_date_changed(self):
		...
		event.starts_on = dontmanage.utils.add_days(dontmanage.utils.nowdate(), 2) + " 12:00:00"
		event.save()
	...
	def test_cc_jinja(self):
		...
		test_user = dontmanage.new_doc("User")
	...
	def test_notification_to_assignee(self):
		...
		email_queue = dontmanage.get_doc(
	...
	def test_notification_value_change_casted_types(self):
		...
		with get_test_notification(notification) as n:
			...
			user.birth_date = dontmanage.utils.add_days(user.birth_date, 1).date()
			user.save()

			user.reload()
			user.birth_date = dontmanage.utils.getdate(user.birth_date)
			user.save()
			self.assertEqual(1, dontmanage.db.count("Notification Log", {"subject": n.subject}))
	...
	def tearDownClass(cls):
		dontmanage.delete_doc_if_exists("Notification", "ToDo Status Update")
		dontmanage.delete_doc_if_exists("Notification", "Contact Status Update")
		...
		notification = dontmanage.get_doc(
			{
				...
				"message": "{% set val = dontmanage.get_doc('ToDo', doc.name) %} ToDo allocated to {{ doc.allocated_to }}",
				"channel": "Email",
	...
	def test_filters_condition(self):
		...
		dontmanage.delete_doc_if_exists("Notification", "Test Filters Condition")
...
❯ bench run-tests --module dontmanage.email.doctype.notification.test_notification --case TestNotificationOffsetRange
...
Updating Dashboard for dontmanage
...
# 		self.event = dontmanage.new_doc("Event")
...
# 		dontmanage.db.delete("Event", {"name": self.event.name})
# 		dontmanage.db.delete("Notification", {"name": self.notification["name"]})
# 		dontmanage.db.delete("Notification Log", {"subject": self.notification["subject"]})
...
# 						self.assertEqual(
# 							1,
# 							dontmanage.db.count("Notification Log", {"subject": n.subject}),

...
dontmanage.notification = {
	setup_fieldname_select: function (frm) {
		...
		dontmanage.model.with_doctype(frm.doc.document_type, function () {
			...
			let get_receiver_fields = function (
				...
				let extract_receiver_field = function (df) {
					...
					if (dontmanage.model.table_fields.includes(df.fieldtype)) {
						let child_fields = dontmanage.get_doc("DocType", df.options).fields;
			...
			let fields = dontmanage.get_doc("DocType", frm.doc.document_type).fields;
			let options = $.map(fields, function (d) {
				return dontmanage.model.no_value_type.includes(d.fieldtype)
					? null
...
• Amount: {{ doc.grand_total }}
		...
		if (template) {
			...
			if (frm.doc.channel === "Email") {
				dontmanage.utils.highlight_pre(message_examples_field.$wrapper);
			}
		}
	},
};

dontmanage.ui.form.on("Notification", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		dontmanage.notification.setup_fieldname_select(frm);
		dontmanage.notification.setup_example_message(frm);
		...
		if (frm.doc.document_type) {
			frm.add_custom_button(__("Preview"), () => {
				...
				let dialog = new dontmanage.views.RenderPreviewer(args);
				return dialog;
			});
		}
	...
	view_properties: function (frm) {
		dontmanage.route_options = { doc_type: frm.doc.document_type };
	...
	event: function (frm) {
		...
		frm.add_custom_button(__("Get Alerts for Today"), function () {
			dontmanage.call({
				method: "dontmanage.email.doctype.notification.notification.get_documents_for_today",
				args: {
					notification: frm.doc.name,
				},
				callback: function (r) {
					if (r.message && r.message.length > 0) {
						dontmanage.msgprint(r.message.toString());
					} else {
						dontmanage.msgprint(__("No alerts for today"));
					}
				},
			});
		});
	...
	set_up_filters_editor(frm) {
		...
		dontmanage.model.with_doctype(frm.doc.document_type, () => {
			const filter_group = new dontmanage.ui.FilterGroup({
				parent: parent,
				doctype: frm.doc.document_type,
				on_change: () => {

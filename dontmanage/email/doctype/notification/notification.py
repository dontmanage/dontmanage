...
from dontmanage.core.doctype.role.role import get_info_based_on_role, get_user_info
from dontmanage.core.doctype.sms_settings.sms_settings import send_sms
from dontmanage.desk.doctype.notification_log.notification_log import enqueue_create_notification
from dontmanage.integrations.doctype.slack_webhook_url.slack_webhook_url import send_slack_message
from dontmanage.model.document import Document
from dontmanage.modules.utils import export_module_json, get_doc_module
from dontmanage.utils import add_to_date, cast, now_datetime, nowdate, validate_email_address
...
class Notification(Document):
	...
	if TYPE_CHECKING:
		from dontmanage.email.doctype.notification_recipient.notification_recipient import NotificationRecipient
	...
	def preview_meets_condition(self, preview_document):
		if not self.condition and not self.filters:
			return _("Yes")
		try:
			doc = dontmanage.get_cached_doc(self.document_type, preview_document)
			if self.condition_type == "Python":
				context = get_context(doc)
				return _("Yes") if dontmanage.safe_eval(self.condition, eval_locals=context) else _("No")
	...
	def preview_message(self, preview_document):
		try:
			...
			msg = dontmanage.render_template(self.message, context)
			if self.channel == "SMS":
				return dontmanage.utils.strip_html_tags(msg)
			return msg
		except Exception as e:
	...
	def preview_subject(self, preview_document):
		try:
			...
			if not self.subject:
				return _("No subject")
			if "{" in self.subject:
				return dontmanage.render_template(self.subject, context)
			return self.subject
		except Exception as e:
	...
	def validate(self):
		...
		if self.event in ("Days Before", "Days After") and not self.date_changed:
			dontmanage.throw(_("Please specify which date field must be checked"))

		if self.event in ("Minutes Before", "Minutes After"):
			if not self.datetime_changed:
				dontmanage.throw(_("Please specify which datetime field must be checked"))
			if not self.minutes_offset:
				dontmanage.throw(_("Please specify the minutes offset"))
		...
		if self.event == "Value Change" and not self.value_changed:
			dontmanage.throw(_("Please specify which value field must be checked"))

		if self.attach_files == "From Field" and not self.from_attach_field:
			dontmanage.throw(_("Please specify the field from which to attach files"))
...
"""
	...
	def validate_standard(self):
		if self.is_standard and self.enabled and not dontmanage.conf.developer_mode:
			dontmanage.throw(
	...
	def validate_filters(self):
		...
		dummy_doc = dontmanage.new_doc(self.document_type)
	...
	def validate_forbidden_document_types(self):
		if self.document_type in FORBIDDEN_DOCUMENT_TYPES or (
			dontmanage.get_meta(self.document_type).istable and self.event not in DATE_BASED_EVENTS
		):
	...
	def get_documents_for_today(self):
		...
		for d in doc_list:
			doc = dontmanage.get_lazy_doc(self.document_type, d.name)

			if (
				self.condition_type == "Python"
				and self.condition
				and not dontmanage.safe_eval(self.condition, None, get_context(doc))
			):
				continue
	...
	def queue_send(self, doc, enqueue_after_commit=True):
		...
		Example:
		              ...
		              ```python
		              notification = dontmanage.get_doc(
		...
		return enqueue(
			"dontmanage.email.doctype.notification.notification.evaluate_alert",
			doc=doc,
			alert=self,
			now=dontmanage.in_test,
		...
		if self.set_property_after_alert:
			...
			try:
				if allow_update and not doc.flags.in_notification_update:
					...
					if doc.meta.get_field(fieldname).fieldtype in dontmanage.model.numeric_fieldtypes:
						value = dontmanage.utils.cint(value)
	...
	def create_system_notification(self, doc, context):
		...
		if "{" in subject:
			subject = dontmanage.render_template(self.subject, context)
		...
		notification_doc = {
			...
			"email_content": dontmanage.render_template(self.message, context),
	...
	def send_an_email(self, doc, context):
		...
		from dontmanage.core.doctype.communication.email import _make as make_communication
		...
		if doc.doctype != "Communication":
			...
			comm = dontmanage.get_lazy_doc("Communication", communication)
	...
	def send_sms(self, doc, context):
		send_sms(
			...
			msg=dontmanage.utils.strip_html_tags(dontmanage.render_template(self.message, context)),
		)
	...
	def get_list_of_recipients(self, doc, context):
		...
		for recipient in self.recipients:
			if recipient.condition:
				if not dontmanage.safe_eval(recipient.condition, None, context):
					continue
	...
	def get_print(self, doc):
		...
		print_settings = dontmanage.get_doc("Print Settings", "Print Settings")
		if (doc.docstatus == 0 and not print_settings.allow_print_for_draft) or (
			...
			return {
				...
				or dontmanage.db.get_value("Print Format", self.print_format, "default_print_language")
	...
	def get_template(self, md_as_html=False):
		...
		path = os.path.join(os.path.dirname(module.__file__), dontmanage.scrub(self.name))
		extension = FORMATS.get(self.message_type, ".md")
		file_path = path + extension
		...
		if extension == ".md":
			return dontmanage.utils.md_to_html(template)
...
def clear_notification_cache():
	dontmanage.client_cache.delete_keys("notifications::")


@dontmanage.whitelist()
def get_documents_for_today(notification):
	notification = dontmanage.get_doc("Notification", notification)
...
def trigger_notifications(doc, method=None):
	if dontmanage.flags.in_import or dontmanage.flags.in_patch:
	...
	if method == "daily":
		...
		for d in doc_list:
			alert = dontmanage.get_doc("Notification", d.name)
...
def evaluate_alert(doc: Document, alert, event=None):
	...
	try:
		if isinstance(alert, str):
			alert = dontmanage.get_doc("Notification", alert)

		context = get_context(doc)

		if alert.condition_type == "Python" and alert.condition:
			if not dontmanage.safe_eval(alert.condition, None, context):
				return
		...
		if event == "Value Change" and not doc.is_new():
			if not dontmanage.db.has_column(doc.doctype, alert.value_changed):
		...
		message = _("Error while evaluating Notification {0}. Please fix your template.").format(
			dontmanage.utils.get_link_to_form("Notification", alert.name)
		)
		dontmanage.throw(message, title=_("Error in Notification"))
	except Exception as e:
		title = str(e)
		message = dontmanage.get_traceback(with_context=True)
		dontmanage.log_error(title=title, message=message)
		msg = f"<details><summary>{title}</summary>{message}</details>"
		dontmanage.throw(msg, title=_("Error in Notification"))


def get_context(doc):
	DontManage = namedtuple("DontManage", ["dontmanage"])
	dontmanage = DontManage(dontmanage=get_safe_globals().get("dontmanage"))
	return {
		"doc": doc,
		"nowdate": nowdate,
		"dontmanage": dontmanage.dontmanage,
	}


def get_assignees(doc):
	assignees = []
	assignees = dontmanage.get_all(
		"ToDo",
...
def get_emails_from_template(template, context):
	...
	emails = dontmanage.render_template(template, context) if "{" in template else template
...
def create_notifications(notifications: list[dict], update: bool = False):
	...
	for notif_dict in notifications:
		...
		existing = dontmanage.db.exists("Notification", name)

		if existing and not update:
			continue

		if existing and update:
			doc = dontmanage.get_doc("Notification", name)
		...
		doc = dontmanage.get_doc(notif_dict)
		doc.flags.ignore_validate = True
...
def get_notification_templates(templates_dir: str) -> list[dict]:
	...
	for folder_name in os.listdir(templates_dir):
		...
		template = dontmanage.get_file_json(json_file) if os.path.exists(json_file) else None
		if not template:
			continue
		...
		if message := dontmanage.read_file(message_file):
...
def install_notification_templates():
	templates_dir = dontmanage.get_module_path("Email", "doctype", "notification", "templates")

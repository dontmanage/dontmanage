...
from dontmanage.utils import add_to_date, cast, is_html, nowdate, validate_email_address
...
class Notification(Document):
	...
	def validate(self):
		...
		dontmanage.cache().hdel("notifications", self.document_type)
...
"""
	...
	def validate_forbidden_types(self):
		...
		if self.document_type in forbidden_document_types or dontmanage.get_meta(self.document_type).istable:
			# currently notifications don't work on child tables as events are not fired for each record of child table

			dontmanage.throw(_("Cannot set Notification on Document Type {0}").format(self.document_type))
	...
	def get_documents_for_today(self):
		...
		for d in doc_list:
			doc = dontmanage.get_doc(self.document_type, d.name)

			if self.condition and not dontmanage.safe_eval(self.condition, None, get_context(doc)):
				continue
	...
	def get_list_of_recipients(self, doc, context):
		...
		for recipient in self.recipients:
			...
			if recipient.cc and "{" in recipient.cc:
				recipient.cc = dontmanage.render_template(recipient.cc, context)
			...
			if recipient.bcc and "{" in recipient.bcc:
				recipient.bcc = dontmanage.render_template(recipient.bcc, context)
			...
			return [
				{
					...
					"lang": dontmanage.db.get_value("Print Format", self.print_format, "default_print_language")
	...
	def get_template(self):
		...
		def load_template(extn):
			template = ""
			template_path = os.path.join(os.path.dirname(module.__file__), dontmanage.scrub(self.name) + extn)
	...
	def load_standard_properties(self, context):
		...
		if not is_html(self.message):
			self.message = dontmanage.utils.md_to_html(self.message)
...
def evaluate_alert(doc: Document, alert, event):
	...
	try:
		...
		title = str(e)
		message = dontmanage.get_traceback()
		dontmanage.log_error(message=message, title=title)

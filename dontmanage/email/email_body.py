...
class EMail:
	...
	def attach_file(self, n):
		...
		_file = dontmanage.get_doc("File", {"file_name": n})
	...
	def validate(self):
		...
		self.recipients = [strip(r) for r in self.recipients if r not in dontmanage.STANDARD_USERS]
		self.cc = [strip(r) for r in self.cc if r not in dontmanage.STANDARD_USERS]
		self.bcc = [strip(r) for r in self.bcc if r not in dontmanage.STANDARD_USERS]
	...
	def add_headers(self, headers):
		...
		if not isinstance(headers, dict):
			dontmanage.throw(_("Headers must be a dictionary"))
	...
	def make(self):
		...
		for hook in dontmanage.get_hooks("make_email_body_message"):
			dontmanage.get_attr(hook)(self)
...
def get_formatted_html(
	...
	email_account=None,
	header=None,
	unsubscribe_link: dontmanage._dict | None = None,
	sender=None,
	...
	if raw_html:
		rendered_email = dontmanage.render_template(message, params)
	else:
		params.update(
			{
		...
		rendered_email = dontmanage.get_template("templates/emails/standard.html").render(params)
...
def get_email_html(template, args, subject, header=None, with_container=False):
	...
	email = dontmanage.utils.jinja.get_email_from_template(template, args)
...
def inline_style_in_html(html, add_css=True):
	...
	if add_css:
		...
		css_files = dontmanage.get_hooks("email_css")
...
def get_message_id():
	...
	return email.utils.make_msgid(domain=dontmanage.local.site)
...
def get_footer(email_account, footer=None):
	...
	sender_address = dontmanage.db.get_default("email_footer_address")

	if sender_address:
		args.update({"sender_address": sender_address})

	if not cint(dontmanage.db.get_default("disable_standard_email_footer")):
		args.update({"default_mail_footer": dontmanage.get_hooks("default_mail_footer")})

	footer += dontmanage.utils.jinja.get_email_from_template("email_footer", args)[0]

	return footer


def replace_filename_with_cid(message):
	"""Replaces <img embed="assets/dontmanage/images/filename.jpg" ...> with
	...
	while True:
		...
		# found match
		img_path = groups[0]
		img_path_escaped = dontmanage.utils.html_utils.unescape_html(img_path)
...
def get_filecontent_from_path(path):
	...
	if path.startswith("assets/"):
		...
		# public file
		full_path = dontmanage.get_site_path("public", path)
	elif path.startswith("private/files/"):
		# private file
		full_path = dontmanage.get_site_path(path)
	else:
		full_path = path
...
def get_header(header=None):
	...
	from dontmanage.utils.jinja import get_email_from_template
	...
	if not title:
		title = dontmanage.get_hooks("app_title")[-1]

...
def create_email_flag_queue(names, action):
	...
	def mark_as_seen_unseen(name, action):
		doc = dontmanage.get_lazy_doc("Communication", name)
		if action == "Read":
			doc.add_seen()
		else:
			_seen = json.loads(doc._seen or "[]")
			_seen = [user for user in _seen if dontmanage.session.user != user]
	...
	for name in json.loads(names or []):
		uid, seen_status, email_account = dontmanage.db.get_value(
		...
		if (action == "Read" and seen_status == 0) or (action == "Unread" and seen_status == 1):
			create_new = True
			email_flag_queue = dontmanage.db.sql(
			...
			for queue in email_flag_queue:
				if queue.action != action:
					dontmanage.delete_doc("Email Flag Queue", queue.name, ignore_permissions=True, force=True)
				elif queue.action == action:
					# Read or Unread request for email is already available
					create_new = False

			if create_new:
				flag_queue = dontmanage.get_doc(
					{
						"uid": uid,
						"action": action,
						"communication": name,
				...
				dontmanage.db.set_value("Communication", name, "seen", seen, update_modified=False)

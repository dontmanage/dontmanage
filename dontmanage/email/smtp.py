...
from dontmanage.utils import cint, cstr, get_traceback


class InvalidEmailCredentials(dontmanage.ValidationError):
	pass
...
class SMTPServer:
	def __init__(
		...
		if not self.server:
			dontmanage.msgprint(
				...
				raise_exception=dontmanage.OutgoingEmailError,
			)
	...
	def session(self):
		...
		try:
			...
			if not _session:
				dontmanage.msgprint(
					_("Could not connect to outgoing email server"), raise_exception=dontmanage.OutgoingEmailError
				)
			...
			if self.use_oauth:
				...
				if res[0] != 235:
					dontmanage.msgprint(res[1], raise_exception=dontmanage.OutgoingEmailError)
	...
	def _enqueue_connection_closure(self):
		if dontmanage.request and hasattr(dontmanage.request, "after_response"):
			dontmanage.request.after_response.add(self.quit)
		elif dontmanage.job:
			dontmanage.job.after_job.add(self.quit)
		elif not dontmanage.in_test:
			# Console?
			import atexit

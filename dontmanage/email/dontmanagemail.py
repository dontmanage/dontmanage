...
from dontmanage.dontmanageclient import DontManageClient, DontManageOAuth2Client
from dontmanage.utils import convert_utc_to_system_timezone, get_datetime, get_system_timezone

CHUNK_SIZE = 5 * 1024 * 1024  # 5MB


class DontManageMail:
	"""Class to interact with the DontManage Mail API."""
	...
	def get_client(
		...
		api_secret: str | None = None,
		access_token: str | None = None,
	) -> DontManageClient | DontManageOAuth2Client:
		"""Returns a DontManageClient or DontManageOAuth2Client instance."""

		if hasattr(dontmanage.local, "dontmanage_mail_clients"):
			if client := dontmanage.local.dontmanage_mail_clients.get(email):
				return client
		else:
			dontmanage.local.dontmanage_mail_clients = {}

		client = (
			DontManageOAuth2Client(url=site, access_token=access_token)
			if access_token
			else DontManageClient(url=site, api_key=api_key, api_secret=api_secret)
		)
		dontmanage.local.dontmanage_mail_clients[email] = client
	...
	def request(
		...
		"""Makes a request to the DontManage Mail API."""
	...
	def send_raw(
		...
		"""Sends an email using the DontManage Mail API."""
	...
	def pull_raw(
		...
		"""Pull emails for the account using the DontManage Mail API."""
		...
		headers = {"X-Site": dontmanage.utils.get_url()}

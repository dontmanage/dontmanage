...
def sendmail_to_system_managers(subject, content):
	dontmanage.sendmail(recipients=get_system_managers(), subject=subject, content=content)
...
def get_contact_list(txt, page_length=20, extra_filters: str | None = None) -> list[dict]:
	...
	if extra_filters:
		extra_filters = dontmanage.parse_json(extra_filters)
...
def get_communication_doctype(doctype, txt, searchfield, start, page_len, filters):
	user_perms = dontmanage.utils.user.UserPermissions(dontmanage.session.user)
...
def sendmail(
	...
	if now and q:
		dontmanage.db.after_commit.add(q.send)
	return q

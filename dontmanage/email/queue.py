...
def get_unsubcribed_url(reference_doctype, reference_name, email, unsubscribe_method, unsubscribe_params):
	...
	if unsubscribe_params:
		params.update(dontmanage.parse_json(unsubscribe_params))
...
def unsubscribe(doctype, name, email):
	...
	if not dontmanage.in_test and not verify_request():
		return
...
def flush():
	...
	for row in email_queue_batch:
		try:
			email_queue: EmailQueue = dontmanage.get_doc("Email Queue", row.name, for_update=True)
			email_queue.send()
		except Exception:
...
def retry_sending_emails():
	emails_in_sending = dontmanage.get_all(
	...
	for e in emails_in_sending:
		if now_datetime() - e["modified"] > timedelta(minutes=15):
			update_fields = {}
			email_queue = dontmanage.get_doc("Email Queue", e["name"])
			sent_to_atleast_one_recipient = any(
				rec.recipient for rec in email_queue.recipients if rec.is_mail_sent()
			)
			if email_queue.retry < cint(dontmanage.db.get_system_setting("email_retry_limit")) or 3:

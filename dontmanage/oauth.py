...
from dontmanage.utils.data import cstr, get_system_timezone, now_datetime
...
class OAuthWebRequestValidator(RequestValidator):
	...
	def validate_client_id(self, client_id, request, *args, **kwargs):
		...
		cli_id = dontmanage.db.get_value("OAuth Client", {"name": client_id})
		if cli_id:
			client = dontmanage.get_doc("OAuth Client", client_id)
	...
	def validate_redirect_uri(self, client_id, redirect_uri, request, *args, **kwargs):
		...
		redirect_uris = (
			cstr(dontmanage.db.get_value("OAuth Client", client_id, "redirect_uris"))
			.strip()
	...
	def authenticate_client(self, request, *args, **kwargs):
		...
		if request.client_id:
			oc = dontmanage.get_doc("OAuth Client", request.client_id)
		else:
			# Extract token, instantiate OAuth Bearer Token and use clientid from there.
			if "refresh_token" in dontmanage.form_dict:
				oc = dontmanage.get_doc(
					"OAuth Client",
					dontmanage.db.get_value(
						"OAuth Bearer Token",
						{"refresh_token": dontmanage.form_dict["refresh_token"]},
						"client",
					),
				)
			elif "token" in dontmanage.form_dict:
				oc = dontmanage.get_doc(
					"OAuth Client",
					dontmanage.db.get_value("OAuth Bearer Token", dontmanage.form_dict["token"], "client"),
				)
			else:
				oc = dontmanage.get_doc(
					"OAuth Client",
					dontmanage.db.get_value(
						"OAuth Bearer Token",
						dontmanage.get_request_header("Authorization").split(" ")[1],
						"client",
					),
				)
		try:
		...
		return dontmanage.session.user == user_id

	def authenticate_client_id(self, client_id, request, *args, **kwargs):
		cli_id = dontmanage.db.get_value("OAuth Client", client_id, "name")
		if not cli_id:
			# Don't allow public (non-authenticated) clients
			return False
		else:
			request["client"] = dontmanage.get_doc("OAuth Client", cli_id)
			return True
	...
	def validate_code(self, client_id, code, client, request, *args, **kwargs):
		...
		if code in [vcode["name"] for vcode in validcodes]:
			...
			request.user = dontmanage.db.get_value("OAuth Authorization Code", code, "user")
			code_challenge_method = dontmanage.db.get_value(
				"OAuth Authorization Code", code, "code_challenge_method"
			)
			code_challenge = dontmanage.db.get_value("OAuth Authorization Code", code, "code_challenge")

			if code_challenge and not request.code_verifier:
				if dontmanage.db.exists("OAuth Authorization Code", code):
					dontmanage.delete_doc("OAuth Authorization Code", code, ignore_permissions=True, force=True)
					dontmanage.db.commit()
				return False
	...
	def confirm_redirect_uri(self, client_id, code, redirect_uri, client, *args, **kwargs):
		saved_redirect_uri = dontmanage.db.get_value("OAuth Client", client_id, "default_redirect_uri")

		redirect_uris = dontmanage.db.get_value("OAuth Client", client_id, "redirect_uris")
	...
	def save_bearer_token(self, token, request, *args, **kwargs):
		...
		otoken = dontmanage.new_doc("OAuth Bearer Token")
		otoken.client = request.client["name"]
		try:
			otoken.user = (
				request.user
				if request.user
				else dontmanage.db.get_value(
					"OAuth Bearer Token",
			...
			otoken.user = dontmanage.session.user
	...
	def invalidate_authorization_code(self, client_id, code, request, *args, **kwargs):
		...
		dontmanage.db.set_value("OAuth Authorization Code", code, "validity", "Invalid")
		dontmanage.db.commit()
	...
	def get_original_scopes(self, refresh_token, request, *args, **kwargs):
		...
		# request.
		obearer_token = dontmanage.get_doc("OAuth Bearer Token", {"refresh_token": refresh_token})
	...
	def revoke_token(self, token, token_type_hint, request, *args, **kwargs):
		...
		if token_type_hint == "access_token":
			dontmanage.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		elif token_type_hint == "refresh_token":
			dontmanage.db.set_value("OAuth Bearer Token", {"refresh_token": token}, "status", "Revoked")
		else:
			dontmanage.db.set_value("OAuth Bearer Token", token, "status", "Revoked")
		dontmanage.db.commit()
	...
	def finalize_id_token(self, id_token, token, token_handler, request):
		...
		# Check whether dontmanage server URL is set
		id_token_header = {"typ": "jwt", "alg": "HS256"}

		user = dontmanage.get_doc("User", request.user)
		...
		return dontmanage.safe_decode(id_token_encoded)

	def get_authorization_code_nonce(self, client_id, code, redirect_uri, request):
		if dontmanage.get_value("OAuth Authorization Code", code, "validity") == "Valid":
			return dontmanage.get_value("OAuth Authorization Code", code, "nonce")

		return None

	def get_authorization_code_scopes(self, client_id, code, redirect_uri, request):
		scope = dontmanage.get_value("OAuth Client", client_id, "scopes")
		if not scope:
			scope = []
		else:
	...
	def validate_id_token(self, token, scopes, request):
		try:
			id_token = dontmanage.get_doc("OAuth Bearer Token", token)
	...
	def validate_jwt_bearer_token(self, token, scopes, request):
		try:
			jwt = dontmanage.get_doc("OAuth Bearer Token", token)
	...
	def validate_silent_login(self, request):
		...
		- OpenIDConnectHybrid
		"""
		if dontmanage.session.user == "Guest" or request.prompt.lower() == "login":
			return False
		else:
			return True
	...
	def validate_user_match(self, id_token_hint, scopes, claims, request):
		...
		if id_token_hint:
			try:
				...
				client_id, client_secret = dontmanage.get_value(
					"OAuth Client",
					payload.get("aud"),
				...
				if payload.get("sub") and client_id and client_secret:
					user = dontmanage.db.get_value(
						"User Social Login",
						{"userid": payload.get("sub"), "provider": "dontmanage"},
						"parent",
					)
					...
					if verified_payload:
						return user.name == dontmanage.session.user

			except Exception:
				return False

		elif dontmanage.session.user != "Guest":
			return True
...
def get_client_scopes(client_id):
	scopes_string = dontmanage.db.get_value("OAuth Client", client_id, "scopes")
	...
	return dontmanage._dict(
		{
			"sub": dontmanage.db.get_value(
				"User Social Login",
				{"parent": user.name, "provider": "dontmanage"},
				"userid",
			),
			...
			"email": user.email,
			"picture": picture,
			"roles": dontmanage.get_roles(user.name),
...
def generate_json_error_response(e):
	...
	dontmanage.local.response["http_status_code"] = getattr(e, "status_code", 500)
	return
...
def get_server_url():
	...
	return dontmanage.get_value("Social Login Key", "dontmanage", "base_url") or request_url

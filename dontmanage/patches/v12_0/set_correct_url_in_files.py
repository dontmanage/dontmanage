...
def execute():
	...
	private_file_path = dontmanage.get_site_path("private", "files")
	...
	for file in files:
		...
		full_path = dontmanage.utils.get_files_path(file_name, is_private=file_is_private)

		if not os.path.exists(full_path):
			if file_is_private:
				public_file_url = os.path.join(public_file_path, file_name)
				if os.path.exists(public_file_url):
					dontmanage.db.set_value("File", file.name, {"file_url": f"/files/{file_name}", "is_private": 0})
			else:

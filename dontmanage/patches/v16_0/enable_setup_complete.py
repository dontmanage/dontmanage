...
def execute():
	...
	is_setup_complete = dontmanage.db.get_single_value("System Settings", "setup_complete")
	if dontmanage.get_all(
		"User", filters={"name": ("not in", ["Guest", "Administrator"])}, pluck="name", limit=1
	):
		is_setup_complete = 1

	apps_details = dontmanage._dict({})
	for details in dontmanage.utils.get_installed_apps_info():
	...
	for app_name in installed_apps:
		...
		if app_name == "dontmanage":
			has_setup_wizard = 1
		elif dontmanage.get_hooks(app_name=app_name).get("setup_wizard_stages"):
			has_setup_wizard = 1

		if has_setup_wizard:
			if not dontmanage.db.exists("Installed Application", {"app_name": app_name}):

	...
	for doctype in possible_log_types:
		if (
			...
			if dontmanage.db.db_type == "mariadb":
				dontmanage.db.sql(f"ALTER TABLE `tab{doctype}` MODIFY name varchar({dontmanage.db.VARCHAR_LEN})")
			else:
				dontmanage.db.sql(
					f"ALTER TABLE `tab{doctype}` ALTER COLUMN name TYPE varchar({dontmanage.db.VARCHAR_LEN}) USING name::varchar"
				)
...
def _is_implicit_int_pk(doctype: str) -> bool:
	...
	if dontmanage.db.db_type == "mariadb":
		...
		values = (dontmanage.db.cur_db_name,)

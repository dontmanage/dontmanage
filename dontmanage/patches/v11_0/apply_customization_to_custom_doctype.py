...
def execute():
	custom_doctypes = dontmanage.get_all("DocType", filters={"custom": 1})
	...
	for doctype in custom_doctypes:
		...
		custom_fields = dontmanage.get_all("Custom Field", filters={"dt": doctype.name}, fields=["*"])

		property_setter_map = {}

		for prop in property_setters:
			property_setter_map[prop.field_name] = prop
			dontmanage.db.delete("Property Setter", {"name": prop.name})

		meta = dontmanage.get_meta(doctype.name)

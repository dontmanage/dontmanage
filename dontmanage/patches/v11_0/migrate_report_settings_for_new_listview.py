...
def execute():
	...
	reports = dontmanage.get_all("Report", {"report_type": "Report Builder"})

	for report_name in reports:
		settings = dontmanage.db.get_value("Report", report_name, "json")
		if not settings:
			continue

		settings = dontmanage._dict(json.loads(settings))
		...
		dontmanage.db.set_value("Report", report_name, "json", json.dumps(settings))

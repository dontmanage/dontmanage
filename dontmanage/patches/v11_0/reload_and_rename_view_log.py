...
def execute():
	if dontmanage.db.table_exists("View log"):
		...
		dontmanage.db.sql("CREATE TABLE `ViewLogTemp` AS SELECT * FROM `tabView log`")

		# deleting old View log table
		dontmanage.db.sql("DROP table `tabView log`")
		dontmanage.delete_doc("DocType", "View log")

		# reloading view log doctype to create `tabView Log` table
		dontmanage.reload_doc("core", "doctype", "view_log")

		# Move the data to newly created `tabView Log` table
		dontmanage.db.sql("INSERT INTO `tabView Log` SELECT * FROM `ViewLogTemp`")
		dontmanage.db.commit()

		# Delete temporary table
		dontmanage.db.sql("DROP table `ViewLogTemp`")
	else:
		dontmanage.reload_doc("core", "doctype", "view_log")

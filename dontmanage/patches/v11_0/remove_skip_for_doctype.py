...
from dontmanage.patches.v11_0.replicate_old_user_permissions import get_doctypes_to_skip
...
def execute():
	...
	has_skip_for_doctype = dontmanage.db.has_column("User Permission", "skip_for_doctype")
	...
	for user_permission in dontmanage.get_all("User Permission", fields=["*"]):
		skip_for_doctype = []
	...
	if new_user_permissions_list:
		dontmanage.qb.into("User Permission").columns(
			"name",
			"user",
			"allow",
			"for_value",

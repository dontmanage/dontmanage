...
import dontmanage
from dontmanage.permissions import get_valid_perms
from dontmanage.utils import cint


def execute():
	dontmanage.reload_doctype("User Permission")
	user_permissions = dontmanage.get_all("User Permission", fields=["allow", "name", "user"])
	...
	for key, doctype_to_skip in doctype_to_skip_map.items():
		if not doctype_to_skip:
			continue
		if not dontmanage.db.has_column("User Permission", "applicable_for") and dontmanage.db.has_column(
...
def get_linked_doctypes(doctype):
	from dontmanage.permissions import get_linked_doctypes

	linked_doctypes = get_linked_doctypes(doctype)
	child_doctypes = [d.options for d in dontmanage.get_meta(doctype).get_table_fields()]

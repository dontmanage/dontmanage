import dontmanage
from dontmanage.core.doctype.log_settings.log_settings import clear_log_table
from dontmanage.utils import add_to_date, today
...
def execute():
	...
	ref: https://github.com/dontmanage/dontmanage/issues/16971
	"""
...
def is_log_cleanup_stuck(doctype: str, retention: int) -> bool:
	...
	return bool(dontmanage.db.exists(doctype, {"modified": ("<", threshold)}))


def get_current_setting(fieldname):
	try:
		return dontmanage.db.get_single_value("Log Settings", fieldname)
	except Exception:

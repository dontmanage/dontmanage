...
def execute():
	...
	for name, content in dontmanage.get_all(
		...
		dontmanage.db.set_value("Comment", name, "content", content, update_modified=False)
	...
	for name, content in dontmanage.get_all(
		...
		if content and content.startswith("Removed "):
			dontmanage.db.set_value("Comment", name, "content", content[8:], update_modified=False)

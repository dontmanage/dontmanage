...
def execute():
	...
	child_tables = dontmanage.get_all(
		"DocField",
		pluck="options",
		filters={"fieldtype": ["in", dontmanage.model.table_fields], "parent": "Workspace"},
	)

	for child_table in child_tables:
		if child_table != "Has Role":
			dontmanage.reload_doc("desk", "doctype", child_table, force=True)

	for seq, workspace in enumerate(dontmanage.get_all("Workspace", order_by="name asc")):
		doc = dontmanage.get_doc("Workspace", workspace.name)

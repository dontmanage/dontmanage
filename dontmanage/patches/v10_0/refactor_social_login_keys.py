...
def execute():
	...
	dontmanage.reload_doc("integrations", "doctype", "social_login_key", force=True)

	if not dontmanage.db.exists("DocType", "Social Login Keys"):
		return

	social_login_keys = dontmanage.get_doc("Social Login Keys", "Social Login Keys")
	if social_login_keys.get("facebook_client_id") or social_login_keys.get("facebook_client_secret"):
		facebook_login_key = dontmanage.new_doc("Social Login Key")
	...
	if social_login_keys.get("dontmanage_server_url"):
		...
		dontmanage_login_key.social_login_provider = "DontManage"
	...
	if social_login_keys.get("github_client_id") or social_login_keys.get("github_client_secret"):
		github_login_key = dontmanage.new_doc("Social Login Key")
	...
	if social_login_keys.get("google_client_id") or social_login_keys.get("google_client_secret"):
		google_login_key = dontmanage.new_doc("Social Login Key")
	...
	dontmanage.delete_doc("DocType", "Social Login Keys")
...
def run_patch():
	...
	for user in users:
		idx = 0
		if user.dontmanage_userid:
			insert_user_social_login(user.name, user.modified_by, "dontmanage", idx, userid=user.dontmanage_userid)
			idx += 1
...
def insert_user_social_login(user, modified_by, provider, idx, userid=None, username=None):
	...
	creation_time = dontmanage.utils.get_datetime_str(dontmanage.utils.get_datetime())
	values = [

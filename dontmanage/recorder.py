...
from dontmanage.database.utils import is_query_type
...
class RecorderConfig:
	...
	def __post_init__(self):
		if not (self.record_jobs or self.record_requests):
			dontmanage.throw("You must record one of jobs or requests")

	def store(self):
		dontmanage.cache.set_value(RECORDER_CONFIG_FLAG, self, expires_in_sec=RECORDER_AUTO_DISABLE)

	@classmethod
	def retrieve(cls):
		return dontmanage.cache.get_value(RECORDER_CONFIG_FLAG) or cls()

	@staticmethod
	def delete():
		dontmanage.cache.delete_value(RECORDER_CONFIG_FLAG)
...
def get_current_stack_frames():
	from dontmanage.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX
...
def post_process():
	...
	dontmanage.db.begin(read_only=True)  # Explicitly start read only transaction

	config = RecorderConfig.retrieve()
	result = list(dontmanage.cache.hgetall(RECORDER_REQUEST_HASH).values())
	...
	for request in result:
		for call in request["calls"]:
			...
			if config.explain and is_query_type(formatted_query, ("select", "update", "delete")):
				...
				try:
					call["explain_result"] = dontmanage.db.sql(f"EXPLAIN {formatted_query}", as_dict=True)
				except Exception:
					pass
		mark_duplicates(request)
		dontmanage.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)
...
def record(force=False):
	flag_value = dontmanage.client_cache.get_value(RECORDER_INTERCEPT_FLAG)
	if flag_value or force:
		dontmanage.local._recorder = Recorder(force=force)
		return dontmanage.local._recorder
	elif flag_value is None:
		# Explicitly set it once so next requests can use client-side cache
		dontmanage.client_cache.set_value(RECORDER_INTERCEPT_FLAG, False)
...
class Recorder:
	def __init__(self, force=False):
		...
		if (
			...
			and self.config.request_filter in dontmanage.request.path
		):
		...
		elif self.config.record_jobs and dontmanage.job and self.config.jobs_filter in dontmanage.job.method:
			self.event_type = "Background Job"
			self.path = dontmanage.job.method
			self.cmd = None
			self.method = None
			self.headers = None
			self.form_dict = None
...
def do_not_record(function):
	...
	def wrapper(*args, **kwargs):
		if hasattr(dontmanage.local, "_recorder"):
			dontmanage.local._recorder.cleanup()
...
def start(
	...
	**kwargs,
	...
	dontmanage.client_cache.set_value(RECORDER_INTERCEPT_FLAG, True)
	dontmanage.cache.expire_key(RECORDER_INTERCEPT_FLAG, RECORDER_AUTO_DISABLE)
...
def stop(*args, **kwargs):
	dontmanage.client_cache.set_value(RECORDER_INTERCEPT_FLAG, False)
	dontmanage.enqueue(post_process, now=dontmanage.in_test)
...
def import_data(file: str) -> None:
	file_doc = dontmanage.get_doc("File", {"file_url": file})
	file_content = json.loads(file_doc.get_content())
	for request in file_content:
		dontmanage.cache.hset(RECORDER_REQUEST_SPARSE_HASH, request["uuid"], request)
		dontmanage.cache.hset(RECORDER_REQUEST_HASH, request["uuid"], request)

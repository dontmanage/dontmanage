...
def two_factor_is_enabled(user=None):
	...
	enabled = cint(dontmanage.get_system_settings("enable_two_factor_auth"))
	if enabled:
		bypass_two_factor_auth = cint(dontmanage.get_system_settings("bypass_2fa_for_retricted_ip_users"))
...
def get_verification_method():
	return dontmanage.get_system_settings("two_factor_method")
...
def get_verification_obj(user, token, otp_secret):
	otp_issuer = dontmanage.get_system_settings("otp_issuer_name")
	verification_method = get_verification_method()
...
def get_email_subject_for_2fa(kwargs_dict):
	...
	subject_template = _("Login Verification Code from {}").format(
		dontmanage.get_system_settings("otp_issuer_name")
	)
...
def get_link_for_qrcode(user, totp_uri):
	...
	key_uri = f"{key}_uri"
	lifespan = int(dontmanage.get_system_settings("lifespan_qrcode_image")) or 240
...
def send_token_via_sms(otpsecret, token=None, phone_no=None):
	...
	send_token_hook_methods = dontmanage.get_hooks("send_token_via_sms")
	if send_token_hook_methods:
		return dontmanage.get_attr(send_token_hook_methods[-1])(otpsecret, token, phone_no)
...
def get_rendered_otp_message(otp: str) -> str:
	...
	custom_template = dontmanage.get_system_settings("otp_sms_template")
	template = custom_template or default_template

	return dontmanage.render_template(template, {"otp": otp})
...
def should_remove_barcode_image(barcode):
	...
	lifespan = dontmanage.get_system_settings("lifespan_qrcode_image") or 240

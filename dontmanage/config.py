...
import dontmanage
from dontmanage import _dict, get_file_json
from dontmanage.exceptions import IncorrectSitePath
...
def get_site_config(
	...
	*,
	...
	sites_path = sites_path or getattr(dontmanage.local, "sites_path", ".")
	site_path = site_path or getattr(dontmanage.local, "site_path", None)
...
def _get_site_config(sites_path: str, site_path: str) -> _dict[str, Any]:
	...
	if site_path:
		...
		if os.path.exists(site_config):
			try:
				...
				click.secho(f"{dontmanage.local.site}/site_config.json is invalid", fg="red")
				print(error)
				raise
		elif dontmanage.local.site and not dontmanage.local.flags.new_site:
			error_msg = f"{dontmanage.local.site} does not exist."
	...
	if config["db_type"] in ("mariadb", "postgres"):
		config["db_socket"] = os.environ.get("DONTMANAGE_DB_SOCKET") or config.get("db_socket")
		...
		config["db_user"] = os.environ.get("DONTMANAGE_DB_USER") or config.get("db_user") or config.get("db_name")

		# read password
		config["db_password"] = os.environ.get("DONTMANAGE_DB_PASSWORD") or config.get("db_password")

	# vice versa for dbname if not defined
	config["db_name"] = os.environ.get("DONTMANAGE_DB_NAME") or config.get("db_name") or config["db_user"]

...
import dontmanage
from dontmanage.website.utils import extract_title, get_frontmatter
...
def get_page_info_from_web_page_with_dynamic_routes(path):
	...
	"""
	from dontmanage.website.doctype.web_page.web_page import get_dynamic_web_pages
...
def get_page_info_from_web_form(path):
	...
	from dontmanage.website.doctype.web_form.web_form import get_published_web_forms
	...
	if end_point:
		if path.endswith("/list"):
			dontmanage.form_dict.is_list = True
		elif path.endswith("/new"):
			dontmanage.form_dict.is_new = True
		elif path.endswith("/edit"):
			dontmanage.form_dict.is_edit = True
		else:
			dontmanage.form_dict.is_read = True
		return page_info[end_point]
...
def evaluate_dynamic_routes(rules, path):
	...
	if hasattr(dontmanage.local, "request") and dontmanage.local.request.environ:
		urls = route_map.bind_to_environ(dontmanage.local.request.environ)
		try:
			...
			if args:
				...
				dontmanage.local.form_dict.update(args)
...
def get_pages(app=None):
	...
	def _build(app):
		...
		if app:
			...
			apps = dontmanage.local.flags.web_pages_apps or dontmanage.get_installed_apps()
	...
	return dontmanage.cache.get_value("website_pages", lambda: _build(app))
...
def get_pages_from_path(start, app, app_path):
	...
	if os.path.exists(start_path):
		for basepath, folders, files in os.walk(start_path):  # noqa: B007
			...
			if "__init__.py" not in files and dontmanage.conf.get("developer_mode"):
				open(os.path.join(basepath, "__init__.py"), "a").close()

			for fname in files:
				fname = dontmanage.utils.cstr(fname)
				if "." not in fname:
					continue
				...
				if extn in ("html", "xml", "js", "css", "md"):
					...
					# print dontmanage.as_json(pages[-1])
...
def setup_source(page_info):
	...
	if page_info.template.endswith((".md", ".html")):
		...
		if page_info.template.endswith(".md"):
			source = dontmanage.utils.md_to_html(source)
...
def get_base_template(path=None):
	...
	"""
	if not path:
		path = dontmanage.local.request.path

	base_template_map = dontmanage.get_hooks("base_template_map") or {}
...
def load_properties_from_controller(page_info):
	...
	module = dontmanage.get_module(page_info.controller)
	if not module:
		return
...
def get_doctypes_with_web_view():
	...
	def _get():
		...
		doctypes = dontmanage.get_hooks("website_generators")
		doctypes_with_web_view = dontmanage.get_all(
			"DocType", fields=["name", "module"], filters=dict(has_web_view=1)
		)
		module_app_map = dontmanage.local.module_app
		doctypes += [
			d.name
			for d in doctypes_with_web_view
			if module_app_map.get(dontmanage.scrub(d.module)) in installed_apps
		]
		return doctypes

	return dontmanage.cache.get_value("doctypes_with_web_view", _get)


def get_start_folders():
	return dontmanage.local.flags.web_pages_folders or ("www", "templates/pages")


def clear_routing_cache():
	from dontmanage.website.doctype.web_form.web_form import get_published_web_forms
	from dontmanage.website.doctype.web_page.web_page import get_dynamic_web_pages
	from dontmanage.website.page_renderers.document_page import _find_matching_document_webview
	from dontmanage.www.sitemap import get_public_pages_from_doctypes

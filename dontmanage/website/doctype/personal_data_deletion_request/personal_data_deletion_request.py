...
from dontmanage.desk.doctype.notification_settings.notification_settings import is_email_notifications_enabled
from dontmanage.model.document import Document
from dontmanage.utils import get_datetime, get_fullname, time_diff_in_hours
...
class PersonalDataDeletionRequest(Document):
	...
	def __init__(self, *args, **kwargs):
		...
		self.user_data_fields = dontmanage.get_hooks("user_data_fields")
	...
	def autoname(self):
		from dontmanage.model.naming import set_name_from_naming_options
		...
		domain = dontmanage.local.site.replace("_", "-")
	...
	def generate_url_for_confirmation(self):
		params = {"email": self.email, "name": self.name, "host_name": dontmanage.local.site}
		api = dontmanage.utils.get_url(
			"/api/method/dontmanage.website.doctype.personal_data_deletion_request"
	...
	def send_verification_mail(self):
		...
		dontmanage.sendmail(
			...
			args={
				"email": self.email,
				"name": self.name,
				"host_name": dontmanage.utils.get_url(),
				"link": url,
			},
	...
	def notify_system_managers(self):
		...
		dontmanage.sendmail(
			...
			args={"user": self.email, "url": dontmanage.utils.get_url(self.get_url())},
	...
	def validate_data_anonymization(self):
		...
		if self.status != "Pending Approval":
			dontmanage.throw(_("This request has not yet been approved by the user."))
	...
	def generate_anonymization_dict(self, ref):
		...
		meta = dontmanage.get_meta(ref["doctype"])
	...
	def redact_doc(self, doc, ref):
		...
		if ref.get("rename") and doc["name"] != self.anon:
			dontmanage.rename_doc(ref["doctype"], doc["name"], self.anon, force=True, show_alert=False)
	...
	def _anonymize_data(self, email=None, anon=None, set_data=True, commit=False):
		...
		dontmanage.rename_doc("User", email, anon, force=True, show_alert=False)
	...
	def __redact_partial_match_data(self, doctype):
		...
		for df in dontmanage.get_meta(doctype["doctype"]).fields:
...
def process_data_deletion_request():
	auto_account_deletion = dontmanage.db.get_single_value("Website Settings", "auto_account_deletion")
	if auto_account_deletion < 1:
		return

	requests = dontmanage.get_all(
		"Personal Data Deletion Request", filters={"status": "Pending Approval"}, pluck="name"
	)

	for request in requests:
		doc = dontmanage.get_doc("Personal Data Deletion Request", request)
...
def confirm_deletion(email, name, host_name):
	...
	doc = dontmanage.get_doc("Personal Data Deletion Request", name)
	host_name = dontmanage.utils.get_url()

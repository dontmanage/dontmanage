...
test_records = dontmanage.get_test_records("Web Page")


class TestWebPage(DontManageTestCase):
	def setUp(self):
		dontmanage.db.delete("Web Page")
		for t in test_records:
			dontmanage.get_doc(t).insert()
	...
	def test_dynamic_route(self):
		web_page = dontmanage.get_doc(
			dict(
				...
				content_type="HTML",
				dynamic_template=1,
				main_section_html="<div>{{ dontmanage.form_dict.doctype }}</div>",
			)
		).insert()
		try:

	...
	def validate_dates(self):
		if self.end_date:
			if self.start_date and get_datetime(self.end_date) < get_datetime(self.start_date):
				dontmanage.throw(_("End Date cannot be before Start Date!"))
			...
			if self.published and now() > self.end_date:
				...
				dontmanage.msgprint(_("Clearing end date, as it cannot be in the past for published pages."))


def check_publish_status():
	# called via daily scheduler
	web_pages = dontmanage.get_all("Web Page", fields=["name", "published", "start_date", "end_date"])
	...
	for page in web_pages:
		...
		if page.published:
			...
			if (start_date and now_date < start_date) or (end_date and now_date > end_date):
				dontmanage.db.set_value("Web Page", page.name, "published", 0)
		else:
			# Publish pages that are inside the set date ranges
			if start_date:
				if not end_date or (end_date and now_date < end_date):
					dontmanage.db.set_value("Web Page", page.name, "published", 1)


def get_web_blocks_html(blocks):
	"""Converts a list of blocks into Raw HTML and extracts out their scripts for deduplication"""

	out = dontmanage._dict(html="", scripts={}, styles={})
	extracted_scripts = {}
	extracted_styles = {}
	for block in blocks:
		web_template = dontmanage.get_cached_doc("Web Template", block.web_template)
		rendered_html = dontmanage.render_template(

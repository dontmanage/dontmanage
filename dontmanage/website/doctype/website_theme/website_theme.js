...
dontmanage.ui.form.on("Website Theme", {
	...
	refresh(frm) {
		...
		if (!frm.doc.custom && !dontmanage.boot.developer_mode) {
			frm.set_read_only();
			frm.disable_save();
		} else {
			frm.enable_save();
	...
	set_default_theme_button_and_indicator(frm) {
		dontmanage.db.get_single_value("Website Settings", "website_theme").then((value) => {
	...
	make_app_theme_selector(frm) {
		...
		frm.events.get_installed_apps(frm).then((apps) => {
			...
			let form = new dontmanage.ui.FieldGroup({
				fields: [
					{

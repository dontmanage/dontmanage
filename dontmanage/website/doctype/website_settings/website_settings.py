...
from dontmanage.utils import cint, encode, get_request_site_address
...
class WebsiteSettings(Document):
	...
	def validate_google_settings(self):
		if self.enable_google_indexing and not dontmanage.get_cached_value("Google Settings", None, "enable"):
		...
		for idx, row in enumerate(self.route_redirects):
			try:
				...
				if not dontmanage.flags.in_migrate:
					dontmanage.throw(_("Invalid redirect regex in row #{}: {}").format(idx, str(e)))
...
def get_website_settings(context=None):
	...
	context = dontmanage._dict(context or {})
	settings: WebsiteSettings = dontmanage.client_cache.get_doc("Website Settings")
	...
	if context.disable_website_theme:
		context.theme = dontmanage._dict()

	else:
		from dontmanage.website.doctype.website_theme.website_theme import get_active_theme

		context.theme = get_active_theme() or dontmanage._dict()

	if not context.get("favicon"):
		context["favicon"] = "/assets/dontmanage/images/dontmanage-favicon.svg"
	...
	context.read_only_mode = dontmanage.flags.read_only
...
def get_auto_account_deletion():
	return dontmanage.db.get_single_value("Website Settings", "auto_account_deletion")

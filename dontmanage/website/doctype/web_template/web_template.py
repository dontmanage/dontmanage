...
from dontmanage.modules.export_file import get_module_path, scrub_dt_dn, write_document_file
...
class WebTemplate(Document):
	...
	def validate(self):
		if self.standard and not dontmanage.conf.developer_mode and not dontmanage.flags.in_patch:
			dontmanage.throw(_("Enable developer mode to create a standard Web Template"))

		for field in self.fields:
			if not field.fieldname:
				field.fieldname = dontmanage.scrub(field.label)
	...
	def on_trash(self):
		if dontmanage.conf.developer_mode and self.standard:
	...
	def get_template_path(self):
		...
		file_name = dontmanage.scrub(self.name) + ".html"

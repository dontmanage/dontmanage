dontmanage.ui.form.on("Web Form", {
	setup: function () {
		dontmanage.meta.docfield_map["Web Form Field"].fieldtype.formatter = (value) => {
			const prefix = {
				"Page Break": "--red-600",
		...
		dontmanage.meta.docfield_map["Web Form Field"].fieldname.formatter = (value) => {
			if (!value) return;
			return dontmanage.unscrub(value);
		};

		dontmanage.meta.docfield_map["Web Form List Column"].fieldname.formatter = (value) => {
			if (!value) return;
			return dontmanage.unscrub(value);
		};
	},
	...
	refresh: function (frm) {
		...
		frm.sidebar
			...
			.attr("href", "#")
			.on("click", () => {
				const url = dontmanage.urllib.get_full_url(frm.doc.route);
				const code = `<iframe src="${url}" style="border: none; width: 100%; height: inherit;"></iframe>`;
				dontmanage.utils.copy_to_clipboard(code, __("Embed code copied"));
			});

		if (frm.doc.is_standard && !dontmanage.boot.developer_mode) {
			frm.disable_form();
	...
	validate: function (frm) {
		...
		if (!frm.doc.web_form_fields) {
			...
			dontmanage.throw(__("At least one field is required in Web Form Fields Table"));
		}
		...
		if (page_break_count >= 10) {
			dontmanage.throw(__("There can be only 9 Page Break fields in a Web Form"));
		}
	},
	...
	add_get_fields_button(frm) {
		frm.add_custom_button(__("Get Fields"), () => {
			let webform_fieldtypes = dontmanage.meta
		...
		let update_options = (fields) => {
			...
			frm.fields_dict.list_columns.grid.update_docfield_property(
				...
				fields
					.filter(
						(df) =>
							!dontmanage.model.no_value_type.includes(df.fieldtype) &&
							df.is_virtual !== 1
					)
...
dontmanage.ui.form.on("Web Form List Column", {
	fieldname: function (frm, doctype, name) {
		let doc = dontmanage.get_doc(doctype, name);
		let df = dontmanage.meta.get_docfield(frm.doc.doc_type, doc.fieldname);
		if (!df) return;
...
dontmanage.ui.form.on("Web Form Field", {
...
function get_fields_for_doctype(doctype) {
	return new Promise((resolve) => dontmanage.model.with_doctype(doctype, resolve)).then(() => {
		return dontmanage.meta.get_docfields(doctype).filter((df) => {
			return (

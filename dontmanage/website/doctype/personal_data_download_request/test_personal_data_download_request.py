...
from dontmanage.website.doctype.personal_data_download_request.personal_data_download_request import (
	get_user_data,
)


class TestRequestPersonalData(DontManageTestCase):
	def setUp(self):
		create_user_if_not_exists(email="test_privacy@example.com")

	def tearDown(self):
		dontmanage.db.delete("Personal Data Download Request")

	def test_user_data_creation(self):
		user_data = json.loads(get_user_data("test_privacy@example.com"))
		contact_name = get_contact_name("test_privacy@example.com")
		expected_data = {"Contact": dontmanage.get_all("Contact", {"name": contact_name}, ["*"])}
	...
	def test_file_and_email_creation(self):
		dontmanage.set_user("test_privacy@example.com")
		download_request = dontmanage.get_doc(
		...
		file_count = dontmanage.db.count(
			"File",
			{
...
def create_user_if_not_exists(email, first_name=None):
	dontmanage.delete_doc_if_exists("User", email)
	...
	user = dontmanage.get_doc(
		{
			...
			"birth_date": dontmanage.utils.now_datetime(),
		}

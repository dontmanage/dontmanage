...
class HelpArticle(WebsiteGenerator):
	...
	def set_route(self):
		...
		if not self.route:
			self.route = "/".join(
				[dontmanage.get_value("Help Category", self.category, "route"), self.scrub(self.title)]
			)
	...
	def update_category(self):
		cnt = dontmanage.db.count("Help Article", filters={"category": self.category, "published": 1})
		cat = dontmanage.get_doc("Help Category", self.category)
	...
	def get_context(self, context):
		...
		context.category = dontmanage.get_doc("Help Category", self.category)
...
def get_list_context(context=None):
	...
	category = dontmanage.db.get_value("Help Category", {"route": dontmanage.local.path})
	...
	list_context = dontmanage._dict(
		...
		hide_filters=True,
		filters=filters,
		category=dontmanage.local.form_dict.category,
		no_breadcrumbs=True,
	)

	if dontmanage.local.form_dict.txt:
		list_context.blog_subtitle = _('Filtered by "{0}"').format(dontmanage.local.form_dict.txt)
	#
	# list_context.update(dontmanage.get_doc("Blog Settings", "Blog Settings").as_dict())
	return list_context

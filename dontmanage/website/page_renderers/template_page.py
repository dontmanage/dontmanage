...
import dontmanage
from dontmanage.website.page_renderers.base_template_page import BaseTemplatePage
from dontmanage.website.router import get_base_template, get_page_info
...
class TemplatePage(BaseTemplatePage):
	...
	def get_index_path_options(search_path):
		return (
			dontmanage.as_unicode(f"{search_path}{d}") for d in ("", ".html", ".md", "/index.html", "/index.md")
		)
	...
	def update_context(self):
		...
		self.context.build_version = dontmanage.utils.get_build_version()

		if self.pymodule_name:
			self.pymodule = dontmanage.get_module(self.pymodule_name)
		...
		self.context.developer_mode = dontmanage.conf.developer_mode
	...
	def render_template(self):
		if self.template_path.endswith("min.js"):
			...
			html = dontmanage.render_template(self.source, self.context, safe_render=safe_render)
	...
	def get_raw_template(self):
		return dontmanage.get_jloader().get_source(dontmanage.get_jenv(), self.context.template)[0]
	...
	def convert_from_markdown(self):
		if self.template_path.endswith(".md"):
			self.source = dontmanage.utils.md_to_html(self.source)
	...
	def set_standard_path(self, path):
		self.app = "dontmanage"
		self.app_path = dontmanage.get_app_path("dontmanage")
		self.path = path
	...
	def set_user_info(self):
		from dontmanage.utils.user import get_fullname_and_avatar

		info = get_fullname_and_avatar(dontmanage.session.user)

...
dontmanage.provide("dontmanage.awesome_bar_path");
window.cur_frm = null;
...
$.extend(dontmanage, {
	...
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (dontmanage._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				dontmanage._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function (opts) {
		...
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					dontmanage.msgprint(__("Not found"));
				},
				403: function () {
					dontmanage.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			dontmanage.process_response(opts, data);
		});
	},
	...
	process_response: function (opts, data) {
		...
		if (data._server_messages) {
			...
			if (opts.error_msg) {
				...
				dontmanage.msgprint(server_messages);
			}
		}
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return dontmanage.call({
			type: "GET",
			method: "dontmanage.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (dontmanage.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", dontmanage.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				dontmanage.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				dontmanage.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				dontmanage.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	freeze: function (msg) {
		...
		dontmanage.freeze_count++;
	},
	unfreeze: function () {
		if (!dontmanage.freeze_count) return; // anything open?
		dontmanage.freeze_count--;
		if (!dontmanage.freeze_count) {
	...
	trigger_ready: function () {
		dontmanage.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = dontmanage.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		dontmanage.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				dontmanage.do_search(val);
				return false;
			}
		});
	},
	do_search: function (val) {
		var path =
			(dontmanage.awesome_bar_path && dontmanage.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		dontmanage.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return dontmanage.get_cookie("user_id") !== "Guest" && dontmanage.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (dontmanage.session.user === "Guest" && window.show_language_picker) {
			dontmanage
				.call("dontmanage.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = dontmanage.get_cookie("preferred_language");
					let language_codes = [];
...
dontmanage.setup_search = function (target, search_scope) {
	...
	$input.on(
		...
		dontmanage.utils.debounce(() => {
			...
			dontmanage
				.call({
					method: "dontmanage.search.web_search",
					args: {
...
$(document).ready(function () {
	window.full_name = dontmanage.get_cookie("full_name");
	var logged_in = dontmanage.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	dontmanage.bind_navbar_search();

	// add apps link
	let apps = dontmanage.boot?.apps_data?.apps;
	let obj = {
		label: __("Apps"),
		route: "/apps",
	};
	if (apps?.length) {
		...
		let is_desk_apps = dontmanage.boot?.apps_data?.is_desk_apps;
		!is_desk_apps && dontmanage.add_apps(obj);
	}

	// switch to app link
	if (dontmanage.get_cookie("system_user") === "yes" && logged_in) {
		dontmanage.add_switch_to_desk();
	}
...
$(document).on("page-change", function () {
	...
	$.extend(dontmanage, dontmanage.get_cookies());
	dontmanage.session = { user: dontmanage.user_id };

	dontmanage.datetime.refresh_when();
	dontmanage.trigger_ready();
	dontmanage.bind_filters();
	dontmanage.highlight_code_blocks();
	dontmanage.add_link_to_headings();
	dontmanage.make_navbar_active();
	// scroll to hash

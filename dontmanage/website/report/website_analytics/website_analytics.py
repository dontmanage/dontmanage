# Copyright (c) 2013, DontManage Technologies and contributors
...
class WebsiteAnalytics:
	def __init__(self, filters=None):
		...
		if not self.filters.from_date:
			self.filters.from_date = dontmanage.utils.add_days(self.filters.to_date, -7)
	...
	def get_data(self):
		WebPageView = dontmanage.qb.DocType("Web Page View")
		count_all = Count("*").as_("count")
		case = dontmanage.qb.terms.Case().when(WebPageView.is_unique == "1", "1")
	...
	def get_chart_data(self):
		current_dialect = dontmanage.db.db_type or "mariadb"
		...
		self.chart_data = dontmanage.db.sql(query, values=values, as_dict=1)
	...
	def prepare_chart_data(self, data):
		...
		if self.filters.range == "Monthly":
			date_range = [dontmanage.utils.add_days(dd, 1) for dd in date_range]

...
import dontmanage
from dontmanage.website.page_renderers.document_page import DocumentPage
from dontmanage.website.page_renderers.list_renderer import ListPage
from dontmanage.website.page_renderers.not_found_page import NotFoundPage
from dontmanage.website.page_renderers.print_page import PrintPage
...
from dontmanage.website.page_renderers.web_form import WebFormPage
from dontmanage.website.router import evaluate_dynamic_routes
from dontmanage.website.utils import can_cache, check_if_webform_exists, get_home_page
...
class PathResolver:
	...
	def resolve(self):
		...
		except dontmanage.Redirect as e:
			return dontmanage.flags.redirect_location, RedirectPage(self.path, e.http_status_code)
		...
		if dontmanage.get_hooks("website_path_resolver"):
			...
			try:
				...
				dontmanage.flags.redirect_location = e.new_url
				return dontmanage.flags.redirect_location, RedirectPage(e.new_url, e.code)
	...
	def get_custom_page_renderers():
		custom_renderers = []
		for renderer_path in dontmanage.get_hooks("page_renderer") or []:
			try:
				renderer = dontmanage.get_attr(renderer_path)
...
def resolve_redirect(path, query_string=None):
	...
	def raise_redirect(redirect_location, status_code=301, forward_query_params=False):
		if forward_query_params and query_string:
			...
			redirect_location += separator + dontmanage.safe_decode(query_string)
		dontmanage.flags.redirect_location = redirect_location
		raise dontmanage.Redirect(status_code)

	redirect_to = dontmanage.cache.hget("website_redirects", path or "/")
	if redirect_to:
	...
	redirects += [
		...
		for r in (dontmanage.get_website_settings("route_redirects") or [])
	]
	...
	dontmanage.cache.hset("website_redirects", path_to_match or "/", False)
...
def get_website_rules():
	...
	def _get():
		rules = dontmanage.get_hooks("website_route_rules")
		for d in dontmanage.get_all("DocType", "name, route", dict(has_web_view=1)):
			if d.route:
...
def validate_path(path: str):
	if not PathResolver(path).is_valid_path():
		dontmanage.throw(dontmanage._("Path {0} it not a valid path").format(dontmanage.bold(path)))

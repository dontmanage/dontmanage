...
from dontmanage.search.website_search import remove_document_from_index, update_index_for_path
from dontmanage.website.utils import cleanup_page_name, clear_cache
...
class WebsiteGenerator(Document):
	...
	def clear_cache(self):
		...
		dontmanage.db.after_commit.add(lambda: clear_cache(self.route))
		dontmanage.db.after_rollback.add(lambda: clear_cache(self.route))
	...
	def on_trash(self):
		...
		if self.allow_website_search_indexing():
			dontmanage.enqueue(remove_document_from_index, path=self.route, enqueue_after_commit=True)
	...
	def send_indexing_request(self, operation_type="URL_UPDATED"):
		...
		if (
			dontmanage.db.get_single_value("Website Settings", "enable_google_indexing")
			and self.is_website_published()
			and self.meta.allow_guest_to_view
		):
			url = dontmanage.utils.get_url(self.route)
			dontmanage.enqueue(
				"dontmanage.website.doctype.website_settings.google_indexing.publish_site",
				url=url,
	...
	def remove_old_route_from_index(self):
		...
		if self.allow_website_search_indexing() or dontmanage.in_test:
			return
	...
	def update_website_search_index(self):
		...
		"""
		if not self.allow_website_search_indexing() or dontmanage.in_test:
			return

		if self.is_website_published():
			dontmanage.enqueue(update_index_for_path, path=self.route, enqueue_after_commit=True)
		elif self.route:

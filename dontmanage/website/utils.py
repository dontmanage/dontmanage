...
import dontmanage
from dontmanage.apps import get_apps, get_default_path, is_desk_apps
...
def delete_page_cache(path=None):
	if path:
		dontmanage.cache.delete_value(f"{WEBSITE_PAGE_CACHE_PREFIX}{path}")
	else:
		dontmanage.cache.delete_keys(WEBSITE_PAGE_CACHE_PREFIX)
...
def can_cache(no_cache=False):
	if dontmanage.flags.force_website_cache:
		return True
	if dontmanage.conf.disable_website_cache or dontmanage.conf.developer_mode:
		return False
	if getattr(dontmanage.local, "no_cache", False):
		return False
	if dontmanage.request and dontmanage.request.query_string:
		return False
	return not no_cache
...
def get_comment_list(doctype, name):
	comments = dontmanage.get_all(
		...
		or_filters=[["owner", "=", dontmanage.session.user], ["published", "=", 1]],
	)
	...
	communications = dontmanage.get_all(
		...
		or_filters=[
			["recipients", "like", f"%{dontmanage.session.user}%"],
			["cc", "like", f"%{dontmanage.session.user}%"],
			["bcc", "like", f"%{dontmanage.session.user}%"],
		],
	)
...
def get_home_page():
	if dontmanage.local.flags.home_page and not dontmanage.in_test:
		return dontmanage.local.flags.home_page
	...
	def _get_home_page():
		...
		if dontmanage.session.user != "Guest":
			# by role
			for role in dontmanage.get_roles():
				home_page = dontmanage.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = dontmanage.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = dontmanage.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if dontmanage.session.user == "Guest" else "me"

		home_page = home_page.strip("/")
		if home_page == "me" and dontmanage.session.data.user_type == "System User":
			home_page = "desk"
		if home_page == "me" and is_portal_user():
			home_page = "portal"
		return home_page

	if dontmanage._dev_server:
...
def get_home_page_via_hooks():
	...
	home_page_method = dontmanage.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = dontmanage.get_attr(home_page_method[-1])(dontmanage.session.user)
	elif dontmanage.get_hooks("website_user_home_page"):
		home_page = dontmanage.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = dontmanage.get_hooks("role_home_page")
		if role_home_page:
			for role in dontmanage.get_roles():
	...
	if not home_page:
		home_page = dontmanage.get_hooks("home_page")
		if home_page:
...
def get_boot_data():
	...
	return {
		...
		"sysdefaults": {
			...
			"currency": dontmanage.get_system_settings("currency"),
		},
		"time_zone": {
			"system": get_system_timezone(),
			"user": dontmanage.get_cached_value("User", dontmanage.session.user, "time_zone")
...
def get_toc(route, url_prefix=None, app=None):
	...
	return dontmanage.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from dontmanage.website.router import get_pages

	if not dontmanage.local.flags.children_map:
	...
	return dontmanage.local.flags.children_map
...
def clear_cache(path=None):
	...
	dontmanage.cache.delete_value(keys)
...
def get_sidebar_items(parent_sidebar, basepath=None):
	...
	hooks = dontmanage.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else dontmanage.flags.look_for_sidebar
...
def get_portal_sidebar_items():
	...
	if sidebar_items is None:
		...
		portal_settings = dontmanage.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = dontmanage.get_hooks("portal_menu_items")
		if items_via_hooks:
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path) as sidebarfile:
		try:
			...
			dontmanage.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		...
		no_cache = dontmanage.request and dontmanage.request.cache_control.no_cache
		if can_cache(no_cache):
			html = None
			page_cache = dontmanage.cache.get_value(cache_key)
			if page_cache and dontmanage.local.lang in page_cache:
				html = page_cache[dontmanage.local.lang]
			if html:
				dontmanage.local.response.from_cache = True
				dontmanage.local.response_headers.update(cache_headers)
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = dontmanage.cache.get_value(cache_key) or {}
			page_cache[dontmanage.local.lang] = html
			dontmanage.cache.set_value(cache_key, page_cache, expires_in_sec=30 * 60)
			dontmanage.local.response_headers.update(cache_headers)
...
def check_if_webform_exists(route):
	return dontmanage.db.exists("Web Form", {"name": route.strip("/")}) or dontmanage.db.exists(

...
def get_all_empty_tables_by_module():
	table_rows = dontmanage.qb.Field("table_rows")
	table_name = dontmanage.qb.Field("table_name")
	information_schema = dontmanage.qb.Schema("information_schema")

	empty_tables = (
		dontmanage.qb.from_(information_schema.tables).select(table_name).where(table_rows == 0)
	).run()

	empty_tables = {r[0] for r in empty_tables}

	results = dontmanage.get_all("DocType", fields=["name", "module"])

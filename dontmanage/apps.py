...
def get_apps():
	...
	for app in apps:
		if (
			...
			and "System Manager" not in dontmanage.get_roles()
		):
			continue

		if app == "dontmanage":
			continue
		app_details = dontmanage.get_hooks("add_to_apps_screen", app_name=app)
		if not len(app_details):
			continue
		for app_detail in app_details:
			try:
				has_permission_path = app_detail.get("has_permission")
				if has_permission_path and not dontmanage.get_attr(has_permission_path)():
					continue
				app_list.append(
					{
						"name": app,
				...
				dontmanage.log_error(f"Failed to call has_permission hook ({has_permission_path}) for {app}")
	return app_list
...
def get_default_path():
	apps = get_apps()
	_apps = [app for app in apps if app.get("name") != "dontmanage"]

	if len(_apps) == 0:
		return None

	system_default_app = dontmanage.get_system_settings("default_app")
	user_default_app = dontmanage.get_cached_value("User", dontmanage.session.user, "default_app")
...
def set_app_as_default(app_name):
	if dontmanage.db.get_value("User", dontmanage.session.user, "default_app") == app_name:
		dontmanage.db.set_value("User", dontmanage.session.user, "default_app", "")
	else:
		dontmanage.db.set_value("User", dontmanage.session.user, "default_app", app_name)

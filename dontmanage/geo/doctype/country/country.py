...
from dontmanage.model.document import Document, bulk_insert
...
def get_countries_and_currencies():
	from dontmanage.geo.country_info import get_all as get_geo_data
	...
	for name, country in data.items():
		country = dontmanage._dict(country)
		countries.append(
			dontmanage.get_doc(
				doctype="Country",
				name=name,
		...
		if country.currency and country.currency not in added_currencies:
			...
			currencies.append(
				dontmanage.get_doc(
					...
					number_format=dontmanage.db.escape(country.number_format)[1:-1],
				)
			)

...
class ImportMapper:
	...
	def __call__(self, *args: Any, **kwds: Any) -> Callable:
		db = db_type_is(dontmanage.conf.db_type)
...
def mask_fields(
	...
	"""
	from dontmanage.database.query import CORE_DOCTYPES
	...
	masked_fields = dontmanage.get_meta(doctype).get_masked_fields()
...
def execute_query(query, *args, **kwargs):
	...
	result = dontmanage.local.db.sql(query, params, *args, **kwargs)  # nosemgrep
...
def prepare_query(query):
	...
	if dontmanage.local.flags.get("in_safe_exec", False):
		from dontmanage.utils.safe_exec import SERVER_SCRIPT_FILE_PREFIX, check_safe_sql_query

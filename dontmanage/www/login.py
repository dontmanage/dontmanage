...
from dontmanage.core.doctype.navbar_settings.navbar_settings import get_app_logo
...
def get_context(context):
	from dontmanage.integrations.dontmanage_providers.dontmanagecloud_billing import get_site_login_url
	...
	context["login_with_dontmanage_cloud_url"] = (
		f"{get_site_login_url()}?site={dontmanage.local.site}"
		if on_dontmanagecloud() and dontmanage.conf.get("fc_communication_secret")
		else None
	)
...
def get_login_with_email_link_ratelimit() -> int:
	return dontmanage.get_system_settings("rate_limit_email_link_login") or 5
...
def sanitize_redirect(redirect: str | None) -> str | None:
	...
	Allowed redirects:
	- Same host e.g. https://dontmanage.localhost/path
	- Just path e.g. /app gets converted to https://dontmanage.localhost/app
	"""
	if not redirect:
		return redirect
	...
	parsed_request_host = urlparse(dontmanage.local.request.url)

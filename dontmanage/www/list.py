...
from dontmanage.model.document import Document, get_controller
from dontmanage.utils import cint, quoted
from dontmanage.website.path_resolver import resolve_path
...
def get_context(context, **dict_params):
	...
	context.txt = dontmanage.local.form_dict.txt
...
def get_list_data(
	...
	if dontmanage.is_table(doctype):
		dontmanage.throw(_("Child DocTypes are not allowed"), title=_("Invalid DocType"))

	if not txt and dontmanage.form_dict.search:
		txt = dontmanage.form_dict.search
		del dontmanage.form_dict["search"]

	controller = get_controller(doctype)
	meta = dontmanage.get_meta(doctype)

	filters = prepare_filters(doctype, controller, kwargs)
	list_context = get_list_context(dontmanage._dict(), doctype, web_form_name)
	...
	dontmanage.flags.list_context = list_context
...
def prepare_filters(doctype, controller, kwargs):
	for key in kwargs.keys():
		try:
			...
			pass
	filters = dontmanage._dict(kwargs)
	meta = dontmanage.get_meta(doctype)
	...
	if filters.pathname:
		...
		for key, val in dontmanage.local.form_dict.items():
...
def get_list_context(context, doctype, web_form_name=None):
	...
	from dontmanage.website.doctype.web_form.web_form import get_web_form_module

	list_context = context or dontmanage._dict()
	meta = dontmanage.get_meta(doctype)
	...
	def update_context_from_module(module, list_context):
		...
		if hasattr(module, "get_list_context"):
			out = dontmanage._dict(module.get_list_context(list_context) or {})
			if out:
				list_context = out
		return list_context
	...
	if meta.custom and web_form_name:
		webform_list_contexts = dontmanage.get_hooks("webform_list_context")
		if webform_list_contexts:
			out = dontmanage._dict(dontmanage.get_attr(webform_list_contexts[0])(meta.module) or {})
			if out:
				list_context = out

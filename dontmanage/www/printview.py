...
import dontmanage
from dontmanage import _, cstr, get_module_path
...
if TYPE_CHECKING:
	...
	from dontmanage.printing.doctype.print_settings.print_settings import PrintSettings
...
""",
	...
	if dontmanage.form_dict.doc:
		doc = dontmanage.form_dict.doc
	else:
		doc = dontmanage.get_lazy_doc(dontmanage.form_dict.doctype, dontmanage.form_dict.name)

	set_link_titles(doc)

	settings = dontmanage.parse_json(dontmanage.form_dict.settings)

	letterhead = dontmanage.form_dict.letterhead or None
	...
	if print_format and print_format.get("print_format_builder_beta"):
		from dontmanage.utils.weasyprint import get_html

		body = get_html(
			doctype=dontmanage.form_dict.doctype, name=dontmanage.form_dict.name, print_format=print_format.name
		)
		...
		body = get_rendered_template(
			...
			meta=meta,
			trigger_print=dontmanage.form_dict.trigger_print,
			no_letterhead=dontmanage.form_dict.no_letterhead,
	...
	make_access_log(
		...
		document=dontmanage.form_dict.name,
		file_type="PDF",
		method="Print",
	...
	return {
		"body": body,
		"print_style": get_print_style(dontmanage.form_dict.style, print_format),
		"comment": dontmanage.session.user,
		"title": dontmanage.utils.strip_html(cstr(doc.get_title() or doc.name)),
		"lang": dontmanage.local.lang,
		"layout_direction": "rtl" if is_rtl() else "ltr",
		"doctype": dontmanage.form_dict.doctype,
		"name": dontmanage.form_dict.name,
		"key": dontmanage.form_dict.get("key"),
		"print_format": print_format_name,
		"letterhead": letterhead,
		"no_letterhead": dontmanage.form_dict.no_letterhead,
		"pdf_generator": dontmanage.form_dict.get("pdf_generator", "wkhtmltopdf"),
	}
...
def get_print_format_doc(print_format_name: str, meta: "Meta") -> "PrintFormat" | None:
	...
	if not print_format_name:
		print_format_name = dontmanage.form_dict.format or meta.default_print_format or "Standard"

	if print_format_name == "Standard":
		return None
	else:
		try:
			return dontmanage.get_doc("Print Format", print_format_name)
...
def get_rendered_template(
	...
	if not dontmanage.flags.ignore_print_permissions:
	...
	if doc.meta.is_submittable:
		if doc.docstatus.is_draft() and not cint(print_settings.allow_print_for_draft):
			dontmanage.throw(_("Not allowed to print draft documents"), dontmanage.PermissionError)

		if doc.docstatus.is_cancelled() and not cint(print_settings.allow_print_for_cancelled):
			dontmanage.throw(_("Not allowed to print cancelled documents"), dontmanage.PermissionError)
	...
	if print_format:
		...
		if hook_func := dontmanage.get_hooks("get_print_format_template"):
			template = dontmanage.call(hook_func[-1], jenv=jenv, print_format=print_format)
...
def set_title_values_for_link_and_dynamic_link_fields(
	...
	for field in meta.get_link_fields() + meta.get_dynamic_link_fields():
		...
		link_title = dontmanage.get_cached_value(doctype, doc.get(field.fieldname), meta.title_field)
		if parent_doc:
...
def set_title_values_for_table_and_multiselect_fields(meta: "Meta", doc: "Document") -> None:
	for field in meta.get_table_fields(include_computed=True):
		...
		_meta = dontmanage.get_meta(field.options)
	...
	for field in doc.meta.fields:
		if field.fieldtype == "Text Editor":
			...
			if value and "<!-- markdown -->" in value:
				doc.set(field.fieldname, dontmanage.utils.md_to_html(value))
...
def get_html_and_style(
	...
	if isinstance(name, str):
		document = dontmanage.get_lazy_doc(doc, name, check_permission=True)
	else:
		document = dontmanage.get_doc(json.loads(doc), check_permission=True)
	...
	try:
		html = get_rendered_template(
			...
			settings=dontmanage.parse_json(settings),
		)
	except dontmanage.TemplateNotFoundError:
...
def get_rendered_raw_commands(doc: str, name: str | None = None, print_format: str | None = None) -> dict:
	...
	if not print_format or (print_format and not print_format.raw_printing):
		dontmanage.throw(
			_("{0} is not a raw printing format.").format(print_format), dontmanage.TemplateNotFoundError
		)
...
def validate_print_permission(doc: "Document") -> None:
	for ptype in ("read", "print"):
		if dontmanage.has_permission(doc.doctype, ptype, doc):
			return

	if dontmanage.has_website_permission(doc):
		return

	if (key := dontmanage.form_dict.key) and isinstance(key, str) and validate_key(key, doc) is not False:
		return
...
def validate_key(key: str, doc: "Document") -> None:
	document_key_expiry = dontmanage.get_cached_value(
		"Document Share Key",
	...
	if document_key_expiry is not None:
		if is_expired(document_key_expiry[0]):
			raise dontmanage.exceptions.LinkExpired
		else:
			return
...
def get_print_format(doctype: str, print_format: "PrintFormat") -> str:
	...
	is_custom_module = dontmanage.get_cached_value("Module Def", module, "custom")

	if not is_custom_module:
		path = os.path.join(
			get_module_path(module, "Print Format", print_format.name),
			dontmanage.scrub(print_format.name) + ".html",
		)
	...
	dontmanage.throw(_("No template found at path: {0}").format(path), dontmanage.TemplateNotFoundError)
...
def make_layout(doc: "Document", meta: "Meta", format_data=None) -> list:
	...
	for df in format_data or meta.fields:
		...
		if df.fieldtype == "Signature" and not doc.get(df.fieldname):
			placeholder_image = "/assets/dontmanage/images/signature-placeholder.png"
...
def get_print_style(
	...
	css = dontmanage.get_template("templates/styles/standard.css").render(context)

	if style and dontmanage.db.exists("Print Style", style):
		css = css + "\n" + dontmanage.db.get_value("Print Style", style, "css")
...
def get_visible_columns(data: list, table_meta: "Meta", df: "DocField") -> list["DocField"]:
	...
	columns = []
	doc = data[0] or dontmanage.new_doc(df.options)

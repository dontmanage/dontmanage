...
def get_context(context):
	...
	if dontmanage.form_dict.doc:
		...
		doc = dontmanage.get_doc(dontmanage.form_dict.doctype, dontmanage.form_dict.name)
	...
	make_access_log(
		doctype=dontmanage.form_dict.doctype, document=dontmanage.form_dict.name, file_type="PDF", method="Print"
	)
	...
	return {
		...
		"title": dontmanage.utils.strip_html(doc.get_title() or doc.name),
...
def validate_print_permission(doc):
	for ptype in ("read", "print"):
		if dontmanage.has_permission(doc.doctype, ptype, doc) or dontmanage.has_website_permission(doc):
			return

	key = dontmanage.form_dict.key
	if key and isinstance(key, str):
		validate_key(key, doc)
	else:
		raise dontmanage.PermissionError(_("You do not have permission to view this document"))
...
def validate_key(key, doc):
	...
	if dontmanage.get_system_settings("allow_older_web_view_links") and key == doc.get_signature():
		return

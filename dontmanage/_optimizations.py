"""This file houses all DontManage specific optimizations and hooks that run on startup or during fork.
...
def optimize_all():
	...
	# Note:
	# - This function is ALWAYS executed as soon as `import dontmanage` ends.
...
def optimize_gc_parameters():
	...
	if not bool(sbool(os.environ.get("DONTMANAGE_TUNE_GC", True))):
		return
...
def optimize_for_gil_contention():
	if not os.environ.get("DONTMANAGE_PERF_PIN_WORKERS"):
		return

	if "gunicorn" not in str(sys.argv[0]):
		return

	if os.environ.get("DONTMANAGE_PERF_PIN_WORKERS_DETERMINISTIC"):

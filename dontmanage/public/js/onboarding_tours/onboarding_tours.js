dontmanage.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new dontmanage.Driver({
			className: "dontmanage-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				dontmanage.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!dontmanage.boot.user.onboarding_status[this.tour.name] &&
								(dontmanage.boot.user.onboarding_status[this.tour.name] = {});
							dontmanage.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								dontmanage.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							dontmanage.call({
								method: "dontmanage.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(dontmanage.boot.user.onboarding_status),
				...
				if ($input) dontmanage.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await dontmanage.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: dontmanage.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
dontmanage.ui.init_onboarding_tour = () => {
	...
	typeof dontmanage.boot.onboarding_tours == "undefined" && dontmanage.boot.onboarding_tours == [];
	typeof dontmanage.boot.user.onboarding_status == "undefined" &&
		dontmanage.boot.user.onboarding_status == {};
	let route = dontmanage.router.current_route;
	if (route?.[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		dontmanage.boot.onboarding_tours &&
			dontmanage.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (dontmanage.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (dontmanage.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(dontmanage.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == dontmanage.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == dontmanage.ui.next_form_tour);
	if (current_tour) {
		...
		dontmanage.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (dontmanage.ui.currentTourInstance?.driver) {
		dontmanage.ui.currentTourInstance.driver_steps = [];
		dontmanage.ui.currentTourInstance.driver.reset(true);
		dontmanage.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (dontmanage.ui.currentTourInstance = new dontmanage.ui.OnboardingTour());
	...
	const wait_for_data = setInterval(() => {
		...
		if (document.body.getAttribute("data-ajax-state") === "complete") {
			dontmanage.utils.sleep(500).then(() => {
				tour.init({
					tour_name,
					start_step,
				});

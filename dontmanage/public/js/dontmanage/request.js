...
dontmanage.provide("dontmanage.request");
dontmanage.provide("dontmanage.request.error_handlers");
dontmanage.request.url = "/";
dontmanage.request.ajax_count = 0;
dontmanage.request.waiting_for_ajax = [];
dontmanage.request.logs = {};

dontmanage.xcall = function (method, params, type, opts = {}) {
...
dontmanage.call = function (opts) {
	if (!dontmanage.is_online()) {
	...
	if (opts.module && opts.page) {
		...
		$.extend(args, {
			cmd: "run_doc_method",
			docs: dontmanage.get_doc(opts.doc.doctype, opts.doc.name),
			method: opts.method,
			args: opts.args,
		});
	} else if (opts.method) {
	...
	// debouce if required
	if (opts.debounce && dontmanage.request.is_fresh(args, opts.debounce)) {
		return Promise.resolve();
	}

	return dontmanage.request.call({
		type: opts.type || "POST",
		args: args,
		success: callback,
		error: opts.error,
...
dontmanage.request.call = function (opts) {
	dontmanage.request.prepare(opts);
	...
	var statusCode = {
		...
		401: function (xhr) {
			if (dontmanage.app.session_expired_dialog && dontmanage.app.session_expired_dialog.display) {
		...
		500: function (xhr) {
			...
			try {
				opts.error_callback && opts.error_callback();
				dontmanage.request.report_error(xhr, opts);
			} catch (e) {
				dontmanage.request.report_error(xhr, opts);
			}
		},
		504: function (xhr) {
			dontmanage.msgprint(__("Request Timed Out"));
			opts.error_callback && opts.error_callback();
		},
		502: function (xhr) {
	...
	var ajax_args = {
		url: opts.url || dontmanage.request.url,
		data: opts.args,
		type: opts.type,
		...
		headers: Object.assign(
			{
				"X-DontManage-CSRF-Token": dontmanage.csrf_token,
				Accept: "application/json",
				"X-DontManage-CMD": (opts.args && opts.args.cmd) || "" || "",
			},
			opts.headers
		),
	...
	if (opts.args && opts.args.doctype) {
		ajax_args.headers["X-DontManage-Doctype"] = encodeURIComponent(opts.args.doctype);
	}

	dontmanage.last_request = ajax_args.data;
		...
		.done(function (data, textStatus, xhr) {
			try {
				...
				if (data.docs || data.docinfo) {
					dontmanage.model.sync(data);
				}

				// sync translated messages
				if (data.__messages) {
					$.extend(dontmanage._messages, data.__messages);
				}
				...
				if (data._link_titles) {
					...
					$.extend(dontmanage._link_titles, data._link_titles);
				}
			...
			try {
				if (
					...
					if (data && data.exception) {
						// dontmanage.exceptions.CustomError: (1024, ...) -> CustomError
...
dontmanage.request.is_fresh = function (args, threshold) {
	// return true if a request with similar args has been sent recently
	if (!dontmanage.request.logs[args.cmd]) {
		dontmanage.request.logs[args.cmd] = [];
	}

	for (let past_request of dontmanage.request.logs[args.cmd]) {
	...
	// log the request
	dontmanage.request.logs[args.cmd].push({ args: args, timestamp: new Date() });
	return false;
};

// call execute serverside request
dontmanage.request.prepare = function (opts) {
	...
	// freeze page
	if (opts.freeze) dontmanage.dom.freeze(opts.freeze_message);
...
dontmanage.request.cleanup = function (opts, r) {
	...
	// un-freeze page
	if (opts.freeze) dontmanage.dom.unfreeze();

	if (r) {
		// session expired? - Guest has no business here!
		if (
			r.session_expired ||
			(dontmanage.session.user === "Guest" && dontmanage.session.logged_in_user !== "Guest")
		) {
			dontmanage.app.handle_session_expired();
			return;
		}
		...
		// error handlers
		let global_handlers = dontmanage.request.error_handlers[r.exc_type] || [];
	...
	dontmanage.last_response = r;
};

dontmanage.after_server_call = () => {
	if (dontmanage.request.ajax_count) {
		return new Promise((resolve) => {
			dontmanage.request.waiting_for_ajax.push(() => {
				resolve();
			});
		});
	} else {
...
dontmanage.after_ajax = function (fn) {
	return new Promise((resolve) => {
		if (dontmanage.request.ajax_count) {
			dontmanage.request.waiting_for_ajax.push(() => {
...
dontmanage.request.report_error = function (xhr, request_opts) {
	...
	const copy_markdown_to_clipboard = () => {
		...
		const traceback_info = [
			...
			"### Route",
			code_block(dontmanage.get_route_str()),
			"### Traceback",
			code_block(exc),
			"### Request Data",
		...
		dontmanage.utils.copy_to_clipboard(traceback_info);
	};
	...
	var show_communication = function () {
		var error_report_message = [
			...
			"<hr>",
			"<h5>App Versions</h5>",
			"<pre>" + JSON.stringify(dontmanage.boot.versions, null, "\t") + "</pre>",
			"<h5>Route</h5>",
			"<pre>" + dontmanage.get_route_str() + "</pre>",
			"<hr>",
			"<h5>Error Report</h5>",
			"<pre>" + exc + "</pre>",
			"<hr>",
		...
		var communication_composer = new dontmanage.views.CommunicationComposer({
			subject: "Error Report [" + dontmanage.datetime.nowdate() + "]",
			recipients: error_report_email,
			message: error_report_message,
			doc: {
				doctype: "User",
				name: dontmanage.session.user,
			},
		});
		communication_composer.dialog.$wrapper.css(
			"z-index",
			cint(dontmanage.msg_dialog.$wrapper.css("z-index")) + 1
		);
	};

	if (exc) {
		var error_report_email = dontmanage.boot.error_report_email;

		request_opts = dontmanage.request.cleanup_request_opts(request_opts);

		// window.msg_dialog = dontmanage.msgprint({message:error_message, indicator:'red', big: true});
...
dontmanage.request.cleanup_request_opts = function (request_opts) {
	let doc = (request_opts.args || {}).doc;
	if (doc) {
		doc = JSON.parse(doc);
		dontmanage.utils.mask_passwords(doc);
		request_opts.args.doc = JSON.stringify(doc);
	}

	if (request_opts.args) {
		dontmanage.utils.mask_passwords(request_opts.args);
	}

	return request_opts;
};

dontmanage.request.on_error = function (error_type, handler) {
	dontmanage.request.error_handlers[error_type] = dontmanage.request.error_handlers[error_type] || [];
	dontmanage.request.error_handlers[error_type].push(handler);
};

$(document).ajaxSend(function () {
	dontmanage.request.ajax_count++;
});

$(document).ajaxComplete(function () {
	dontmanage.request.ajax_count--;
	if (!dontmanage.request.ajax_count) {
		$.each(dontmanage.request.waiting_for_ajax || [], function (i, fn) {
			fn();
		});
		dontmanage.request.waiting_for_ajax = [];
	}

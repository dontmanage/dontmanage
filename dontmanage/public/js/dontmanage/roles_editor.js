dontmanage.RoleEditor = class {
	constructor(wrapper, frm, disable) {
		...
		this.multicheck = dontmanage.ui.form.make_control({
			...
			df: {
				...
				get_data: () => {
					return dontmanage
						.xcall("dontmanage.core.doctype.user.user.get_all_roles")
						.then((roles) => {
	...
	show_permissions(role) {
		...
		return dontmanage
			.xcall("dontmanage.core.doctype.user.user.get_perm_info", { role })
			.then((permissions) => {
							...
							<table class="user-perm">
								<thead>
									<tr>
										...
										${dontmanage.perm.rights
											.map(
												(p) =>
													`<th class="sticky-top ${header_bg_color}">${__(
														dontmanage.unscrub(p)
													)}</th>`
											)
											.join("")}
									</tr>
					...
					permissions.forEach((perm) => {
						$body.find("tbody").append(`
							<tr>
								...
								<td>${perm.if_owner ? dontmanage.utils.icon("check", "xs") : "-"}</td>
								${dontmanage.perm.rights
									.map(
										(p) =>
											`<td class="text-muted bold">${
												perm[p] ? dontmanage.utils.icon("check", "xs") : "-"
											}</td>`
									)
									.join("")}
							</tr>
	...
	make_perm_dialog() {
		this.perm_dialog = new dontmanage.ui.Dialog({
	...
	set_roles_in_table() {
		...
		roles.map((role_doc) => {
			if (!checked_options.includes(role_doc.role)) {
				dontmanage.model.clear_doc(role_doc.doctype, role_doc.name);
			}
		});
		checked_options.map((role) => {
			if (!roles.find((d) => d.role === role)) {
				let role_doc = dontmanage.model.add_child(this.frm.doc, "Has Role", "roles");
				role_doc.role = role;
			}
		});
	}

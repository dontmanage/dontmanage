...
export default class QuickListWidget extends Widget {
	...
	set_actions() {
		...
		if (dontmanage.model.can_create(this.document_type)) {
	...
	setup_quick_list_item(doc) {
		const indicator = dontmanage.get_indicator(doc, this.document_type);
			...
			<div class="quick-list-item">
				<div class="ellipsis left">
					...
					<div class="timestamp text-muted">
						${dontmanage.datetime.prettyDate(doc.modified)}
					</div>
				</div>
			</div>
		`);
		...
		let icon_to_append = `<div class="right-arrow">${dontmanage.utils.icon("right", "xs")}</div>`;
		if (dontmanage.utils.is_rtl(dontmanage.boot.lang)) {
			icon_to_append = `<div class="left-arrow">${dontmanage.utils.icon("left", "xs")}</div>`;
		}
		...
		$quick_list_item.click((e) => {
			...
			dontmanage.set_route(`${dontmanage.utils.get_form_link(this.document_type, doc.name)}`);
		});
	...
	set_body() {
		...
		dontmanage.model.with_doctype(this.document_type, () => {
			let fields = ["name"];

			// get name of title field
			if (!this.title_field_name) {
				let meta = dontmanage.get_meta(this.document_type);
			...
			this.has_status_field = dontmanage.meta.has_field(this.document_type, "status");
			...
			let workflow_fieldname = dontmanage.workflow.get_state_fieldname(this.document_type);
			workflow_fieldname && fields.push(workflow_fieldname);
			fields.push("modified");

			let add_fields = dontmanage.listview_settings?.[this.document_type]?.add_fields;
			if (Array.isArray(add_fields)) {
				for (const fieldname of add_fields) {
					// Only keep fields that exist and are permitted
					if (dontmanage.meta.has_field(this.document_type, fieldname)) {

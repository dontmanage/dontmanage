		...
		this.dialog = new dontmanage.ui.Dialog({
			...
			primary_action: (data) => {
				...
				if (!this.editing && !data.name) {
					data.name = `${this.type}-${this.label}-${dontmanage.utils.get_random(20)}`;
				}
	...
	get_title() {
		if (this.editing) {
			switch (this.type) {
				...
				default:
					return __("Edit {0}", [__(dontmanage.model.unscrub(this.type))]);
			}
		}
		...
		switch (this.type) {
			...
			default:
				return __("New {0}", [__(dontmanage.model.unscrub(this.type))]);
		}
	}
...
class QuickListDialog extends WidgetDialog {
	...
	generate_filter_from_json() {
		if (this.values && this.values.quick_list_filter) {
			this.filters = dontmanage.utils.get_filter_from_json(
...
class NumberCardDialog extends WidgetDialog {
			...
			return [
				{
					...
					get_query: () => {
						return {
							query: "dontmanage.desk.doctype.number_card.number_card.get_cards_for_user",
							filters: {
	...
	set_aggregate_function_fields() {
		...
		if (this.document_type && dontmanage.get_meta(this.document_type)) {
			dontmanage.get_meta(this.document_type).fields.map((df) => {

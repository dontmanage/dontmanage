...
dontmanage.data_import.ImportPreview = class ImportPreview {
	...
	prepare_columns() {
		this.columns = this.preview_data.columns.map((col, i) => {
			...
			if (col.skip_import) {
				...
				return {
					id: dontmanage.utils.get_random(6),
		...
		this.data = this.data.map((row) => {
			return row.map((cell) => {
				...
				if (typeof cell === "string") {
					cell = dontmanage.utils.xss_sanitise(cell);
				}
				return cell;
			});
		});
	...
	setup_styles() {
		...
		this.datatable.style.setStyle(`svg.import-success`, {
			width: "16px",
			fill: dontmanage.ui.color.get_color_shade("green", "dark"),
		});
		...
		this.datatable.style.setStyle(row_classes, {
			pointerEvents: "none",
			backgroundColor: dontmanage.ui.color.get_color_shade("gray", "extra-light"),
			color: dontmanage.ui.color.get_color_shade("gray", "dark"),
		});
	}
	...
	show_column_warning(_, $target) {
		...
		dontmanage.utils.scroll_to($warning, true, 30);
	}
...
function get_fields_as_options(doctype, column_map) {
	let keys = [doctype];
	dontmanage.meta.get_table_fields(doctype).forEach((df) => {
	...
	return [].concat(
		...keys.map((key) => {
			return column_map[key].map((df) => {
				...
				if (doctype !== key) {
					let table_field = dontmanage.meta.get_docfield(doctype, key);

...
dontmanage.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || dontmanage.router.doctype_layout || __(this.doctype);
		this.meta = dontmanage.get_meta(this.doctype);
		this.settings = dontmanage.listview_settings[this.doctype] || {};
		...
		this.can_create = dontmanage.model.can_create(this.doctype);
	...
	get_list_view_settings() {
		return dontmanage
			.call("dontmanage.desk.listview.get_list_settings", {
	...
	async set_fields() {
		let fields = [].concat(dontmanage.model.std_fields_list, this.meta.title_field);
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			dontmanage.model.std_fields_list.includes(fieldname) ||
	...
	set_stats() {
		...
		this.workflow_state_fieldname = dontmanage.workflow.get_state_fieldname(this.doctype);
		if (this.workflow_state_fieldname) {
			if (!dontmanage.workflow.workflows[this.doctype]["override_status"]) {
	...
	fetch_meta() {
		return dontmanage.model.with_doctype(this.doctype);
	}
	...
	setup_view_menu() {
		if (dontmanage.boot.desk_settings.view_switcher && !this.meta.force_re_route_to_default_view) {
			const icon_map = {
				Image: "image",
				List: "list",
				Report: "sheet",
			...
			this.views_list = new dontmanage.views.ListViewSelect({
				doctype: this.doctype,
	...
	set_breadcrumbs() {
		dontmanage.breadcrumbs.add(this.meta.module, this.doctype);
	}
	...
	setup_list_wrapper() {
		this.$dontmanage_list = $('<div class="dontmanage-list">').appendTo(this.page.main);
	}
	 ...
	 * to the `dontmanage_list` container. This container is used to create a scrollable area for the result content.
	 */
	setup_result_container_area() {
		if (this.view == "List") {
			this.$dontmanage_list.append($(`<div class="result-container border rounded">`));
		}
	}

	setup_result_area() {
		this.$result = $(`<div class="result">`);
		let dontmanage_list = this.$dontmanage_list;
		if (this.view == "List") {
			dontmanage_list = this.$dontmanage_list.find(".result-container");
		}
		dontmanage_list.append(this.$result);
	}
	...
	setup_no_result_area() {
		...
		`).hide();
		this.$dontmanage_list.append(this.$no_result);
	}

	setup_freeze_area() {
		this.$freeze = $('<div class="freeze"></div>').hide();
		this.$dontmanage_list.append(this.$freeze);
	}
	...
	setup_paging_area() {
		...
		this.$dontmanage_list.append(this.$paging_area);
	...
	set_result_height() {
		...
		this.$result.parent(".result-container").css({
			height: resultContainerHeight - (dontmanage.is_mobile() ? 100 : 0) + "px",
		});
	...
	get_fields() {
		...
		return this.fields.map((f) => dontmanage.model.get_full_column_name(f[0], f[1]));
	}

	get_group_by() {
		let name_field = this.fields && this.fields.find((f) => f[0] == "name");
		if (name_field) {
			return dontmanage.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	refresh() {
		...
		return dontmanage.call(args).then((r) => {
			// render
			this.prepare_data(r);
	...
	prepare_data(r) {
		...
		Object.assign(dontmanage.boot.user_info, data.user_info);
		delete data.user_info;

		data = !Array.isArray(data) ? dontmanage.utils.dict(data.keys, data.values) : data;
...
class FilterArea {
	constructor(list_view) {
		...
		this.debounced_refresh_list_view = dontmanage.utils.debounce(
			this.refresh_list_view.bind(this),
			300
		);
		this.setup();
		if (dontmanage.is_mobile()) this.setup_mobile(list_view);
	}
	setup_mobile(list_view) {
		...
		$(`<button class="filter-toggle btn btn-default btn-sm filter-button">
					<span class="filter-icon button-icon">
						${dontmanage.utils.icon("funnel-plus")}
					</span>
				</button>
			</div>`)
	...
	setup() {
		...
		this.user_setting_fields =
			dontmanage.get_user_settings(this.list_view.doctype)?.group_by_fields || [];
	...
	render_non_standard_fields_filter() {
		let get_item_html = (fieldname) => {
			...
			return `<div class="group-by-field list-link form-group dontmanage-control input-max-width">
						...
						href="#" onclick="return false;">
							...
							<span>${dontmanage.utils.icon("select", "xs")}</span>
						</a>
	...
	get_dropdown_html(field, fieldtype, applied = false) {
		...
		if (field.name == null) {
			label = __("Not Set");
		} else if (field.name === dontmanage.session.user) {
			label = __("Me");
		...
		let applied_html = applied
			? `<span class="applied"> ${dontmanage.utils.icon("tick", "xs")} </span>`
			: "";
	...
	get_stats($dropdown) {
		...
		dontmanage.call({
			method: "dontmanage.desk.reportview.get_sidebar_stats",
			type: "GET",
			args: {
				stats: ["_user_tags"],
			...
			callback: function (r) {
				...
				dontmanage.utils.setup_search($dropdown, ".stat-link", ".stat-label");
			},
		});
	}
	...
	render_stat(stats, $dropdown) {
		...
		let tag_list = $(dontmanage.render_template("list_sidebar_stat", args)).on(
			"click",
			".stat-link",
			(e) => {
	...
	get_group_by_count(field) {
		...
		return dontmanage.call("dontmanage.desk.listview.get_group_by_count", args).then((r) => {
			let field_counts = r.message || [];
			field_counts = field_counts.filter((f) => f.count !== 0);
			let current_user = field_counts.find((f) => f.name === dontmanage.session.user);
			field_counts = field_counts.filter(
				(f) => !["Guest", "Administrator", dontmanage.session.user].includes(f.name)
			);
			// Set dontmanage.session.user on top of the list
	...
	setup_search($dropdown) {
		dontmanage.utils.setup_search($dropdown, ".group-by-item", ".group-by-value", "data-name");
	}
	...
	clear(refresh = true) {
		...
		return dontmanage.run_serially(promises).then(() => {
			this.trigger_refresh = true;
					...
					(df) =>
						(df.fieldname === title_field ||
							((df.in_standard_filter ||
								...
								dontmanage.model.is_value_type(df.fieldtype))) &&
						dontmanage.perm.has_perm(this.list_view.doctype, df.permlevel)
				)
				.map((df) => {
	...
	filter_field_with_match_type(df) {
		setTimeout(() => {
			...
			const getIcon = (match_type) => {
				if (match_type === "=") {
					return dontmanage.utils.icon("equal");
				} else {
					return dontmanage.utils.icon("equal-approximately");
				}
			};
...
dontmanage.views.is_valid = (view_mode) => dontmanage.views.view_modes.includes(view_mode);

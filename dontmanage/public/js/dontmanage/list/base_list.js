...
dontmanage.views.BaseList = class BaseList {
	...
	setup_page() {
		...
		!this.hide_card_layout && this.page.main.addClass("dontmanage-card");
	...
	setup_side_bar() {
		if (this.hide_sidebar || !dontmanage.boot.desk_settings.list_sidebar) return;
		this.list_sidebar = new dontmanage.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	setup_result_area() {
		...
		this.$dontmanage_list.append(this.$result);
	}
			...
			doctype_fields
				.filter(
					(df) =>
						...
						(df.in_standard_filter && dontmanage.model.is_value_type(df.fieldtype))
				)
				.map((df) => {

...
dontmanage.views.ListView = class ListView extends dontmanage.views.BaseList {
	static load_last_view() {
		const route = dontmanage.get_route();
		const doctype = route[1];

		if (route.length === 2) {
			const user_settings = dontmanage.get_user_settings(doctype);
			const last_view = user_settings.last_view;
			dontmanage.set_route(
				"list",
				dontmanage.router.doctype_layout || doctype,
				dontmanage.views.is_valid(last_view) ? last_view.toLowerCase() : "list"
			);
			return true;
		}
		return false;
	...
	constructor(opts) {
		super(opts);
		this.show();
		const meta = dontmanage.get_meta(this.doctype);
	...
	check_permissions() {
		if (!this.has_permissions()) {
			dontmanage.set_route("");
			dontmanage.throw(__("Not permitted to view {0}", [this.doctype]));
		}
	}
	...
	validate_filters(filters) {
		...
		valid_fields = valid_fields.concat(dontmanage.model.std_fields_list);
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = dontmanage.perm.get_match_rules(this.doctype);
		if (match_rules_list.length) {
			this.restricted_list = $(
				`<button class="btn btn-xs restricted-button flex align-center">
					${dontmanage.utils.icon("restriction", "xs")}
				</button>`
			)
	...
	show_restrictions(match_rules_list = []) {
		dontmanage.msgprint(
			dontmanage.render_template("list_view_permission_restrictions", {
	...
	async set_fields() {
		...
		await Promise.all(
			fields.map((f) => {
				return new Promise((resolve) => {
					const df =
						typeof f === "string" ? dontmanage.meta.get_docfield(this.doctype, f) : f;
					if (
						df &&
						df.fieldtype == "Link" &&
						dontmanage.boot.link_title_doctypes.includes(df.options)
					) {
						dontmanage.model.with_doctype(df.options, () => {
							const meta = dontmanage.get_meta(df.options);
		...
		this.fields.forEach((f) => {
			const df = dontmanage.meta.get_docfield(f[1], f[0]);
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = dontmanage.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise((resolve) => {
			if (this.required_libs) {
				dontmanage.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
	...
	set_primary_action() {
		if (this.can_create && !dontmanage.boot.read_only) {
			const doctype_name = __(dontmanage.router.doctype_layout) || __(this.doctype);
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (allowed_filter_types.includes(f[2]) && dontmanage.model.is_non_std_field(f[1])) {
				const df = dontmanage.meta.get_field(doctype, f[1]);
				if (df && !df.read_only) {
					options[f[1]] = f[3];
				}
			}
		});
		dontmanage.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = dontmanage.meta.get_docfield.bind(null, this.doctype);
		...
		if (dontmanage.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: "Status",
			});
	...
	before_refresh() {
		if (dontmanage.route_options && this.filter_area) {
	...
	before_render() {
		...
		dontmanage.model.user_settings.save(this.doctype, "last_view", this.view_name);
	...
	get_left_html(doc) {
		...
		for (let i = 0; i < this.columns.length; i++) {
			...
			if (dontmanage.is_mobile() && col.type == "Field" && [3, 4].includes(i)) {
	...
	get_column_html(col, doc, show_in_mobile) {
		...
		const format = () => {
			if (df.fieldtype === "Percent") {
				...
				return dontmanage.format(value, df, null, doc);
			}
		};
		...
		const field_html = () => {
			...
			if (strip_html_required) {
				...
				_value =
					typeof value_display === "string"
						? dontmanage.utils.escape_html(value_display)
						: value_display;
			}
			...
			let masked_fields = dontmanage.get_meta(this.doctype).masked_fields || [];
			...
			if (df.fieldtype === "Image") {
				html = df.options
					...
					: `<div class="missing-image small">
						${dontmanage.utils.icon("restriction")}
					</div>`;
			} else if (df.fieldtype === "Select") {
				html = `<span class="${filterable} indicator-pill ${dontmanage.utils.guess_colour(
					_value
				)} ellipsis"
			...
			} else if (dontmanage.model.html_fieldtypes.includes(df.fieldtype)) {
				html = `<span class="ellipsis">
					${_value}
				</span>`;
			} else {
				html = `<a class="${filterable} ellipsis"
					data-filter="${fieldname},=,${dontmanage.utils.escape_html(value)}">
					${format()}
				</a>`;
			}

			return `<span class="ellipsis"
				title="${__(label)}: ${dontmanage.utils.escape_html(_value)}">
				${html}
			</span>`;
		};
		...
		if (dontmanage.is_mobile() && col.type == "Subject") {
			css_class += " bold";
		}
	...
	apply_column_widths() {
		...
		Object.entries(this.column_max_widths).forEach(([fieldname, width]) => {
			$(`.list-view .dontmanage-list .result .level-left .list-row-col.${fieldname}`).css({
				width: width,
				flex: `1 0 ${width}px`,
			});
		});
	...
	update_listview_classes(has_assignto, assign_to_count) {
		...
		// disable scrolling
		if (this.list_view_settings?.disable_scrolling && !dontmanage.is_mobile()) {
	...
	get_tags_html(user_tags, limit, colored = false) {
		let get_tag_html = (tag) => {
			...
			if (tag) {
				if (colored) {
					color = dontmanage.get_palette(tag);
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments d-flex align-items-center">
					${
						dontmanage.avatar_group(assigned_users, this.max_number_of_avatars - 1, {
							filterable: true,
						})[0].outerHTML
					}
				</div>`;
			...
			if (button_actions) {
				dropdown_buttons = `
					<button type="button" class="btn btn-xs btn-default ellipsis" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
						...
						${dontmanage.utils.icon("select", "xs")}
					</button>
	...
	get_seen_class(doc) {
		...
		return seen_by.includes(dontmanage.session.user) ? "" : "bold";
	}

	get_like_html(doc) {
		const liked_by = doc._liked_by ? JSON.parse(doc._liked_by) : [];
		const is_liked = liked_by.includes(dontmanage.session.user);
	...
	get_subject_text(doc, title) {
		...
		if (dontmanage.model.html_fieldtypes.includes(subject_field.fieldtype)) {
			// NOTE: this is very slow, so only do it for HTML fields
			return dontmanage.utils.html2text(value);
		} else {
			return value;
		}
	}
	...
	get_image_url(doc) {
		...
		if (window.cordova && !dontmanage.utils.is_url(url)) {
			url = dontmanage.base_url + url;
		}
		return url || null;
	}
	...
	setup_filterable() {
		...
		this.$result.on("click", ".filterable", (e) => {
			...
			const filters_to_apply = filters.map((f) => {
				f = f.split(",");
				if (f[2] === "Today") {
					f[2] = dontmanage.datetime.get_today();
				} else if (f[2] == "User") {
					f[2] = dontmanage.session.user;
				}
	...
	setup_list_click() {
		this.$result.on("click", ".list-row, .image-view-header, .file-header", (e) => {
			...
			if (link) {
				dontmanage.set_route(link.pathname);
				return false;
			}
		});
	}
	...
	setup_like() {
		this.$result.on("click", ".like-action", (e) => {
			...
			dontmanage.ui.toggle_like($this, doctype, name);
	...
	process_document_refreshes() {
		...
		dontmanage.call(call_args).then(({ message }) => {
			if (!message) return;
			const data = dontmanage.utils.dict(message.keys, message.values);
	...
	save_view_user_settings(obj) {
		return dontmanage.model.user_settings.save(this.doctype, this.view_name, obj);
	}

	on_update() {}

	update_url_with_filters() {
		if (dontmanage.get_route_str() == this.page_name && !this.report_name) {
	...
	get_menu_items() {
		...
		if (dontmanage.model.can_import(doctype, null, this.meta)) {
			items.push({
				label: __("Import", null, "Button in list view menu"),
				action: () =>
					dontmanage.set_route("list", "data-import", {
		...
		if (
			dontmanage.model.can_create("Custom Field") &&
			dontmanage.model.can_create("Property Setter")
		) {
			items.push({
				...
				action: () => {
					if (!this.meta) return;
					if (this.meta.custom) {
						dontmanage.set_route("form", "doctype", doctype);
		...
		if (dontmanage.user.has_role("System Manager") && dontmanage.boot.developer_mode) {
			// edit doctype
			items.push({
				label: __("Edit DocType", null, "Button in list view menu"),
				action: () => dontmanage.set_route("form", "doctype", doctype),
				standard: true,
			});
		}
	...
	make_group_by_fields_modal() {
		...
		d.set_primary_action(__("Save"), ({ group_by_fields }) => {
			dontmanage.model.user_settings.save(
				this.doctype,
				"group_by_fields",
			...
			dontmanage.msgprint(__("Saving Changes..."));
			setTimeout(() => {
				location.reload();
			}, 1500);
		});
		...
		dontmanage.utils.setup_search(d.$body, ".unit-checkbox", ".label-area");
		d.show();
	}

	get_group_by_dropdown_fields() {
		let group_by_fields = [];
		let default_fields = dontmanage.get_user_settings(this.doctype)?.group_by_fields || [];
	...
	show_list_settings() {
		dontmanage.model.with_doctype(this.doctype, () => {
			new ListSettings({
				listview: this,
				doctype: this.doctype,
				settings: this.list_view_settings,
				meta: dontmanage.get_meta(this.doctype),
			});
		});
	}
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return (
				field_doc.fieldname &&
				dontmanage.model.is_value_type(field_doc) &&
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					dontmanage.meta.get_docfields(doctype).forEach((field_doc) => {
						...
						if (field_doc.fieldtype === "Table" && field_doc.options) {
							...
							const child_fields = dontmanage.meta.get_docfields(child_doctype);
		...
		const copy_to_clipboard = () => {
			return {
				...
				action: () => {
						...
						return columns
							.map((col) => {
								...
								if (col.type === "Status" || col.fieldname === "docstatus") {
									...
									const indicator = dontmanage.get_indicator(item, this.doctype);
									...
									if (link_title_fieldname) {
										...
										if (
											...
											let link_title = dontmanage.utils.get_link_title(
												df.options,
												item[col.fieldname]
											);
					...
					dontmanage.utils.copy_to_clipboard(clipboard_data, message);
				},
				standard: true,
			};
		};
		...
		// bulk printing
		if (dontmanage.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// bulk submit
		if (
			dontmanage.model.is_submittable(doctype) &&
			has_submit_permission(doctype) &&
			!dontmanage.model.has_workflow(doctype)
		) {
			actions_menu_items.push(bulk_submit());
		}

		// bulk cancel
		if (dontmanage.model.can_cancel(doctype) && !dontmanage.model.has_workflow(doctype)) {
			actions_menu_items.push(bulk_cancel());
		}

		// bulk delete
		if (dontmanage.model.can_delete(doctype) && is_bulk_edit_allowed(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		if (!params.toString() && dontmanage.route_options) {
			params = new Map(Object.entries(dontmanage.route_options));
		}
		...
		params.forEach((value, field) => {
			...
			if (!doctype) {
				doctype = dontmanage.meta.get_doctype_for_field(this.doctype, field);
			}
...
dontmanage.get_list_view = (doctype) => {
	let route = `List/${doctype}/List`;
	return dontmanage.views.list_view[route];
};

...
dontmanage.views.ListView = class ListView extends dontmanage.views.BaseList {
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (f[2] === "=" && dontmanage.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
	...
	get_seen_class(doc) {
		return JSON.parse(doc._seen || "[]").includes(dontmanage.session.user) ? "" : "bold";
	}
	...
	setup_like() {
		this.$result.on("click", ".like-action", dontmanage.ui.click_toggle_like);
	...
	get_menu_items() {
		...
		if (dontmanage.user.has_role("System Manager") && dontmanage.boot.developer_mode === 1) {
			// edit doctype
			items.push({
	...
	get_actions_menu_items() {
		...
		// bulk delete
		if (dontmanage.model.can_delete(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		for (let field in dontmanage.route_options) {
			let doctype = null;
			let value = dontmanage.route_options[field];

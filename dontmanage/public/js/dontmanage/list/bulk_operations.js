export default class BulkOperations {
	constructor({ doctype }) {
		if (!doctype) dontmanage.throw(__("Doctype required"));
		this.doctype = doctype;
	}

	print(docs) {
		const print_settings = dontmanage.model.get_doc(":Print Settings", "Print Settings");
		...
		if (invalid_docs.length > 0) {
			dontmanage.msgprint(__("You selected Draft or Cancelled documents"));
			return;
		}

		if (valid_docs.length === 0) {
			dontmanage.msgprint(__("Select atleast 1 record for printing"));
			return;
		}
		...
		const dialog = new dontmanage.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					label: __("Page Size"),
					fieldname: "page_size",
					options: dontmanage.meta.get_print_sizes(),
		...
		dialog.set_primary_action(__("Print"), (args) => {
			if (!args) return;
			const default_print_format = dontmanage.get_meta(this.doctype).default_print_format;
			...
			if (args.page_size === "Custom") {
				if (args.page_height === 0 || args.page_width === 0) {
					dontmanage.throw(__("Page height and width cannot be zero"));
				}
			...
			if (args.background_print) {
				dontmanage
					.call("dontmanage.utils.print_format.download_multi_pdf_async", {
					...
					.then((response) => {
						...
						dontmanage.realtime.task_subscribe(task_id);
						dontmanage.realtime.on(`task_complete:${task_id}`, (data) => {
							dontmanage.msgprint({
							...
							dontmanage.realtime.task_unsubscribe(task_id);
							dontmanage.realtime.off(`task_complete:${task_id}`);
						});
					});
			} else {
				const w = window.open(
					"/api/method/dontmanage.utils.print_format.download_multi_pdf?" +
						"doctype=" +
	...
	get_letterhead_options() {
		...
		dontmanage.call({
			method: "dontmanage.client.get_list",
			args: {
				doctype: "Letter Head",
	...
	delete(docnames, done = null) {
		dontmanage
			.call({
				method: "dontmanage.desk.reportview.delete_items",
				freeze: true,
				freeze_message:
					docnames.length <= 10
	...
	assign(docnames, done) {
		if (docnames.length > 0) {
			const assign_to = new dontmanage.ui.form.AssignToDialog({
				obj: this,
				method: "dontmanage.desk.form.assign_to.add_multiple",
				doctype: this.doctype,
				docname: docnames,
				bulk_assign: true,
				re_assign: true,
			...
			dontmanage.msgprint(__("Select records for assignment"));
		}
	}
	...
	clear_assignment(docnames, done) {
		if (docnames.length > 0) {
			dontmanage
				.call({
					method: "dontmanage.desk.form.assign_to.remove_multiple",
					args: {
						doctype: this.doctype,
						names: docnames,
			...
			dontmanage.msgprint(__("Select records for removing assignment"));
		}
	}
	...
	apply_assignment_rule(docnames, done) {
		if (docnames.length > 0) {
			dontmanage
				.call("dontmanage.automation.doctype.assignment_rule.assignment_rule.bulk_apply", {
					doctype: this.doctype,
					docnames: docnames,
				})
				.then(() => done());
	...
	submit_or_cancel(docnames, action = "submit", done = null) {
		...
		dontmanage.realtime.task_subscribe(task_id);
		return dontmanage
			.xcall("dontmanage.desk.doctype.bulk_update.bulk_update.submit_cancel_or_update_docs", {
				doctype: this.doctype,
				action: action,
				docnames: docnames,
				task_id: task_id,
			...
			.then((failed_docnames) => {
				if (failed_docnames?.length) {
					const comma_separated_records = dontmanage.utils.comma_and(failed_docnames);
					switch (action) {
						case "submit":
							dontmanage.throw(__("Cannot submit {0}.", [comma_separated_records]));
							break;
						case "cancel":
							dontmanage.throw(__("Cannot cancel {0}.", [comma_separated_records]));
							break;
						default:
							dontmanage.throw(__("Cannot {0} {1}.", [action, comma_separated_records]));
					}
				}
				if (failed_docnames?.length < docnames.length) {
					dontmanage.utils.play_sound(action);
					if (done) done();
				}
			})
			.finally(() => {
	...
	edit(docnames, field_mappings, done) {
		...
		const dialog = new dontmanage.ui.Dialog({
			...
			primary_action: ({ value }) => {
				...
				dontmanage
					.call({
						method: "dontmanage.desk.doctype.bulk_update.bulk_update.submit_cancel_or_update_docs",
						args: {
							doctype: this.doctype,
							freeze: true,
							docnames: docnames,
					...
					.then((r) => {
						...
						dialog.hide();
						dontmanage.show_alert(__("Updated successfully"));
					});
			},
		...
		const dialog = new dontmanage.ui.Dialog({
			...
			fields: [
				{
					...
					get_data: function (txt) {
						return dontmanage.db.get_link_options("Tag", txt);
					},
				},
			],
			...
			primary_action: () => {
				...
				if (args && args.tags) {
					...
					dontmanage.call({
						method: "dontmanage.desk.doctype.tag.tag.add_tags",
						args: {
							tags: args.tags,
							dt: this.doctype,
							docs: docnames,
	...
	export(doctype, docnames) {
		dontmanage.require("data_import_tools.bundle.js", () => {
			const data_exporter = new dontmanage.data_import.DataExporter(
				doctype,
				"Insert New Records"
			);
...
dontmanage.ui.BulkOperations = BulkOperations;

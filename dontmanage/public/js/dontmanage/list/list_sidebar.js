...
dontmanage.views.ListSidebar = class ListSidebar {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		var sidebar_content = dontmanage.render_template("list_sidebar", { doctype: this.doctype });
	...
	setup_views() {
		...
		if (this.doctype === "Communication" && dontmanage.boot.email_accounts.length) {
			this.sidebar.find('.list-link[data-view="Inbox"]').removeClass("hide");
			show_list_link = true;
		}

		if (dontmanage.treeview_settings[this.doctype] || dontmanage.get_meta(this.doctype).is_tree) {
			this.sidebar.find(".tree-link").removeClass("hide");
		}

		this.current_view = "List";
		var route = dontmanage.get_route();
		if (route.length > 2 && dontmanage.views.view_modes.includes(route[2])) {
	...
	setup_kanban_boards() {
		...
		dontmanage.views.KanbanView.setup_dropdown_in_sidebar(this.doctype, $dropdown);
	}

	setup_keyboard_shortcuts() {
		this.sidebar.find(".list-link > a, .list-link > .btn-group > a").each((i, el) => {
			dontmanage.ui.keys.get_shortcut_group(this.page).add($(el));
		});
	}

	setup_list_group_by() {
		this.list_group_by = new dontmanage.views.ListGroupBy({
			doctype: this.doctype,
			sidebar: this,
			list_view: this.list_view,
			page: this.page,
	...
	get_stats() {
		...
		dontmanage.call({
			...
			callback: function (r) {
				...
				dontmanage.utils.setup_search(stats_dropdown, ".stat-link", ".stat-label");
			},
		});
	}
	...
	add_insights_banner() {
		try {
			...
			const link = "https://dontmanage.io/s/insights";
			const cta = "DontManage Insights";

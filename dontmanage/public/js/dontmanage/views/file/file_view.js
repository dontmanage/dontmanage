...
dontmanage.views.FileView = class FileView extends dontmanage.views.ListView {
	static load_last_view() {
		const route = dontmanage.get_route();
		if (route.length === 2) {
			const view_user_settings = dontmanage.get_user_settings("File", "File");
			dontmanage.set_route(
				"List",
				"File",
				view_user_settings.last_folder || dontmanage.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			dontmanage.views.FileView.grid_view = !dontmanage.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (dontmanage.views.FileView.grid_view) {
			Object.assign(args, {
		...
		return [
			{
				label: __("Home"),
				action: () => {
					dontmanage.set_route("List", "File", "Home");
				},
			},
			{
				label: __("New Folder"),
					...
					dontmanage.prompt(
						...
						(values) => {
							if (values.value.indexOf("/") > -1) {
								dontmanage.throw(__("Folder name should not include '/' (slash)"));
							}
							const data = {
							...
							dontmanage.call({
								method: "dontmanage.core.api.file.create_new_folder",
								args: data,
							});
						},
			...
			},
			{
				...
				action: () => {
					new dontmanage.ui.FileUploader({
						...
						on_success: (file) => {
							dontmanage.show_alert(__("Unzipping files..."));
							dontmanage
								.call("dontmanage.core.api.file.unzip_file", {
									name: file.name,
								})
								.then((r) => {
									if (r.message) {
										dontmanage.show_alert(__("Unzipped {0} files", [r.message]));
									}
								});
						},
					});
	...
	add_file_action_buttons() {
		this.$cut_button = this.page
			.add_button(__("Cut"), () => {
				dontmanage.file_manager.cut(this.get_checked_items(), this.current_folder);
				this.$checks.parents(".file-wrapper").addClass("cut");
			})
			.hide();

		this.$paste_btn = this.page
			.add_button(__("Paste"), () => dontmanage.file_manager.paste(this.current_folder))
			.hide();

		this.page.add_actions_menu_item(__("Export as zip"), () => {
			let docnames = this.get_checked_items(true);
			if (docnames.length) {
				open_url_post("/api/method/dontmanage.core.api.file.zip_files", {
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter((df) => dontmanage.model.is_value_type(df.fieldtype) && !df.hidden)
			.map((df) => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (dontmanage.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
		...
		title = dontmanage.utils.escape_html(title);
		...
		d.subject_html = `
			${dontmanage.utils.icon(icon_class)}
			<span>${title}</span>
	...
	before_render() {
		super.before_render();
		dontmanage.model.user_settings.save("File", "grid_view", dontmanage.views.FileView.grid_view);
	...
	render_grid_view() {
		...
		let html = this.data
			.map((d) => {
				...
				const file_url = dontmanage.utils.escape_html(d.file_url);
	...
	get_header_html() {
		...
		let header_selector_html = !dontmanage.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
					"Select All"
			  )}">`
			: "";

		let header_columns_html = !dontmanage.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === dontmanage.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = dontmanage.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size ? dontmanage.form.formatters.FileSize(file.file_size) : "";
		...
		return `
			<div class="list-row-col ellipsis list-subject level">
				...
				<span class="level-item  ellipsis" title="${dontmanage.utils.escape_html(file.file_name)}">
					<a class="ellipsis" href="${route_url}" title="${dontmanage.utils.escape_html(file.file_name)}">
						${file.subject_html}
					</a>
				</span>
			</div>
		...
		this.$result.on("drop", (e) => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					dontmanage.file_manager.paste(folder_name);
					dontmanage.show_alert(`File ${file_name} moved to ${folder_name}`);
				}
			}
		});
	}
	...
	setup_copy_event() {
		this.$result.on("click", ".copy-file-url", (e) => {
			dontmanage.utils.copy_to_clipboard(e.currentTarget.getAttribute("data-file-url"));
			e.preventDefault();
			e.stopPropagation();
		});
	}
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!dontmanage.file_manager.can_paste ||
			dontmanage.file_manager.old_folder === this.current_folder;
...
dontmanage.views.FileView.grid_view = dontmanage.get_user_settings("File").grid_view || false;

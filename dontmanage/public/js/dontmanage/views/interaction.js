...
dontmanage.provide("dontmanage.interaction_settings");

dontmanage.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		dontmanage.model.with_doctype("Event", () => {
			let categories = dontmanage.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new dontmanage.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = dontmanage.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = dontmanage.session.user;
		}
	...
	add_attachments(doc, attachments) {
		dontmanage.call({
			method: "dontmanage.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,

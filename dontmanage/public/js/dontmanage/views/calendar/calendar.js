...
dontmanage.views.Calendar = class Calendar {
	...
	get_system_datetime(date) {
		date._offset = moment(date).tz(dontmanage.sys_defaults.time_zone)._offset;
		return dontmanage.datetime.convert_to_system_tz(date);
	}
	setup_options(defaults) {
		...
		this.cal_options = {
			...
			eventClick: function (event) {
				...
				if (dontmanage.model.can_read(doctype)) {
					dontmanage.set_route("Form", doctype, event.name);
				}
			},
	...
	update_event(event, revertFunc) {
		var me = this;
		dontmanage.model.remove_from_locals(me.doctype, event.name);
		return dontmanage.call({

...
dontmanage.provide("dontmanage.views.calendar");
dontmanage.provide("dontmanage.views.calendars");

dontmanage.views.CalendarView = class CalendarView extends dontmanage.views.ListView {
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.calendar_settings = dontmanage.views.Calendar[this.doctype] || {};
			this.calendar_name = dontmanage.get_route()[3];
		});
	}
	...
	render() {
		...
		this.load_lib
			...
			.then((options) => {
				this.calendar = new dontmanage.views.Calendar(options);
			});
	}
	...
	get_calendar_preferences() {
		...
		return new Promise((resolve) => {
			if (calendar_name === "default") {
				Object.assign(options, dontmanage.views.calendar[this.doctype]);
				resolve(options);
			} else {
				dontmanage.model.with_doc("Calendar View", calendar_name, () => {
					const doc = dontmanage.get_doc("Calendar View", calendar_name);
					if (!doc) {
						...
						dontmanage.set_route("List", this.doctype, "Calendar", "default");
						return;
					}
...
dontmanage.views.Calendar = class Calendar {
	constructor(options) {
		$.extend(this, options);
	...
	make_page() {
		...
		$.each(dontmanage.boot.calendars, function (i, doctype) {
			if (dontmanage.model.can_read(doctype)) {
				me.page.add_menu_item(__(doctype), function () {
					dontmanage.set_route("List", doctype, "Calendar");
				});
			}
		});
	...
	make() {
		...
		this.footnote_area = dontmanage.utils.set_footnote(
			this.footnote_area,
			this.$wrapper,
		...
		this.fullCalendar = new dontmanage.FullCalendar(this.$cal[0], this.cal_options);
	...
	set_css() {
		...
		this.$wrapper
			...
			.attr("class", "")
			.html(dontmanage.utils.icon("left"));
		this.$wrapper
			.find(`.fc-next-button span`)
			.attr("class", "")
			.html(dontmanage.utils.icon("right"));
		if (this.$wrapper.find(".fc-today-button svg").length == 0)
			this.$wrapper.find(".fc-today-button").prepend(dontmanage.utils.icon("today"));
	...
	get_system_datetime(date) {
		return dontmanage.datetime.convert_to_system_tz(date, true);
	}
	setup_options(defaults) {
		var me = this;
		defaults.meridiem = "false";
		this.cal_options = {
			plugins: dontmanage.FullCalendar.Plugins,
			...
			eventClick: function (info) {
				...
				if (dontmanage.model.can_read(doctype)) {
					dontmanage.set_route("Form", doctype, info.event.id);
				}
			},
			...
			select: function (info) {
				...
				var event = dontmanage.model.get_new_doc(me.doctype);
				...
				dontmanage.set_route("Form", me.doctype, event.name);
			},
	...
	prepare_events(events) {
		...
		return (events || []).map((d) => {
			d.id = d.name;
			d.editable = dontmanage.model.can_write(d.doctype || me.doctype);
			...
			if (d.convertToUserTz) {
				d.start = dontmanage.datetime.convert_to_user_tz(d.start);
				d.end = dontmanage.datetime.convert_to_user_tz(d.end);
			}

			// show event on single day if start or end date is invalid
			if (!dontmanage.datetime.validate(d.start) && d.end) {
				d.start = dontmanage.datetime.add_days(d.end, -1);
			}

			if (d.start && !dontmanage.datetime.validate(d.end)) {
				d.end = dontmanage.datetime.add_days(d.start, 1);
			}

			me.prepare_colors(d);

			d.title = dontmanage.utils.html2text(d.title);
	...
	prepare_colors(d) {
		...
		if (this.get_css_class) {
			...
			if (color_name.startsWith("#")) {
				color_name = dontmanage.ui.color.validate_hex(color_name) ? color_name : "blue";
			}

			d.backgroundColor = dontmanage.ui.color.get(color_name, "extra-light");
			d.textColor = dontmanage.ui.color.get(color_name, "dark");
		} else {
			color = d.color;
			if (!dontmanage.ui.color.validate_hex(color) || !color) {
				color = dontmanage.ui.color.get("blue", "extra-light");
			}
			d.backgroundColor = color;
			d.textColor = dontmanage.ui.color.get_contrast_color(color);
		}
		return d;
	}
	update_event(event, revertFunc) {
		var me = this;
		dontmanage.model.remove_from_locals(me.doctype, event.id);
		return dontmanage.call({
			method: me.update_event_method || "dontmanage.desk.calendar.update_event",
			args: me.get_update_args(event),
			callback: function (r) {
				if (r.exc) {
					dontmanage.show_alert(__("Unable to update event"));
					revertFunc();
				}
			},
			error: function () {

...
dontmanage.views.GanttView = class GanttView extends dontmanage.views.ListView {
	get view_name() {
		return "Gantt";
	}
	...
	prepare_tasks() {
		...
		this.tasks = this.data.map(function (item) {
			...
			if (item.color && dontmanage.ui.color.validate_hex(item.color)) {
	...
	render_gantt() {
		...
		this.gantt = new Gantt(this.$result[0], this.tasks, {
			...
			on_click: (task) => {
				dontmanage.set_route("Form", task.doctype, task.id);
			},
			on_date_change: (task, start, end) => {
				if (!me.can_write) return;
				dontmanage.db.set_value(task.doctype, task.id, {
	...
	set_colors() {
		...
		let style = classes
			.map((c) => {
				...
				const progress_color = dontmanage.ui.color.get_contrast_color(bar_color);
				return `

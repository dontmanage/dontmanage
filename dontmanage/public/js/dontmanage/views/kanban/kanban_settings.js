export default class KanbanSettings {
	constructor({ kanbanview, doctype, meta, settings }) {
		if (!doctype) {
			dontmanage.throw(__("DocType required"));
		}
	...
	make() {
		...
		this.dialog.set_primary_action(__("Save"), () => {
			...
			dontmanage.call({
				method: "dontmanage.desk.doctype.kanban_board.kanban_board.save_settings",
				args: {
	...
	get_docfield(field_name) {
		return (
			dontmanage.meta.get_docfield(this.doctype, field_name) ||
	...
	get_multiselect_fields() {
		...
		const ignore_fieldtypes = [
			...
			"Code",
			"Color",
			...dontmanage.model.no_value_type,
		];

		return dontmanage.model.std_fields

...
dontmanage.views.KanbanView = class KanbanView extends dontmanage.views.ListView {
	...
	static load_last_view() {
		...
		if (route.length === 3) {
			...
			const user_settings = dontmanage.get_user_settings(doctype)["Kanban"] || {};
			if (!user_settings.last_kanban_board) {
				return new dontmanage.views.KanbanView({ doctype: doctype });
			}
	...
	show() {
		dontmanage.views.KanbanView.get_kanbans(this.doctype).then((kanbans) => {
			if (!kanbans.length) {
				return dontmanage.views.KanbanView.show_kanban_dialog(this.doctype, true);
			} else if (kanbans.length && dontmanage.get_route().length !== 4) {
				// Try to use the last board the user used, else default to the first available board
				const last_board = dontmanage.get_user_settings(this.doctype)["Kanban"]
					?.last_kanban_board;
				if (last_board && kanbans.includes(last_board)) {
					dontmanage.set_route("List", this.doctype, "Kanban", last_board);
					return;
				} else {
					const first_board = kanbans[0];
					dontmanage.set_route("List", this.doctype, "Kanban", first_board.name);
					return;
				}
			} else {
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.board_name = dontmanage.get_route()[3] || get_board_name() || null;
	...
	set_board_perms_and_push_menu_items() {
		...
		return dontmanage.call({
			method: "dontmanage.client.get_doc_permissions",
			args: {
	...
	get_board() {
		return dontmanage.db.get_doc("Kanban Board", this.board_name).then((board) => {
			this.board = board;
	...
	save_kanban_board_filters() {
		...
		dontmanage.db.set_value("Kanban Board", this.board_name, "filters", filters).then((r) => {
			if (r.exc) {
	...
	render() {
		...
		if (!this.kanban) {
			this.kanban = new dontmanage.views.KanbanBoard({
	...
	get_card_meta() {
		var meta = dontmanage.get_meta(this.doctype);
		// preserve route options erased by new doc
		let route_options = { ...dontmanage.route_options };
		var doc = dontmanage.model.get_new_doc(this.doctype);
		dontmanage.route_options = route_options;
		var title_field = null;
		var quick_entry = false;

		if (this.meta.title_field) {
			title_field = dontmanage.meta.get_field(this.doctype, this.meta.title_field);
		}
		...
		if (
			mandatory.some((df) => dontmanage.model.table_fields.includes(df.fieldtype)) ||
			mandatory.length > 1
		) {
			quick_entry = true;
		}
		...
		if (!title_field) {
			title_field = dontmanage.meta.get_field(this.doctype, "name");
		}
...
dontmanage.views.KanbanView.get_kanbans = function (doctype) {
	let kanbans = [];

	return get_kanban_boards().then((kanban_boards) => {
		if (kanban_boards) {
			kanban_boards.forEach((board) => {
				let route = `/desk/${dontmanage.router.slug(board.reference_doctype)}/view/kanban/${
					board.name
				}`;
	...
	function get_kanban_boards() {
		return dontmanage
			.call("dontmanage.desk.doctype.kanban_board.kanban_board.get_kanban_boards", { doctype })
			.then((r) => r.message);
	}
};

dontmanage.views.KanbanView.show_kanban_dialog = function (doctype) {
	let dialog = new_kanban_dialog();
	dialog.show();

	function make_kanban_board(board_name, field_name, project) {
		return dontmanage.call({
			method: "dontmanage.desk.doctype.kanban_board.kanban_board.quick_kanban_board",
			args: {
				doctype,
				board_name,
				field_name,
			...
			callback: function (r) {
				var kb = r.message;
				if (kb.filters) {
					dontmanage.provide("dontmanage.kanban_filters");
					dontmanage.kanban_filters[kb.kanban_board_name] = kb.filters;
				}
				dontmanage.set_route("List", doctype, "Kanban", kb.kanban_board_name);
			},
		});
	}
	...
	function new_kanban_dialog() {
		...
		const select_fields = dontmanage.get_meta(doctype).fields.filter((df) => {
		...
		let primary_action = () => {
			if (to_save) {
				...
				make_kanban_board(values.board_name, values.field_name, values.project).then(
					() => dialog.hide(),
					(err) => dontmanage.msgprint(err)
				);
			} else {
				dontmanage.set_route("Form", "Customize Form", { doc_type: doctype });
			}
		};

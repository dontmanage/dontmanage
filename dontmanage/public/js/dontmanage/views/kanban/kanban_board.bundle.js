...
(function () {
	var method_prefix = "dontmanage.desk.doctype.kanban_board.kanban_board.";
	...
	var store = createStore({
		...
		actions: {
			...
			add_column: function (context, col) {
				if (dontmanage.model.can_create("Custom Field")) {
			...
			add_card: function (context, { card_title, column_title }) {
				var state = context.state;
				var doc = dontmanage.model.get_new_doc(state.doctype);
				...
				if (field && !quick_entry) {
					...
					dontmanage.new_doc(state.doctype, doc);
				}
			},
	...
	dontmanage.views.KanbanBoard = function (opts) {
		var self = {};
		...
		function prepare() {
			...
			if (self.$kanban_board.length === 0) {
				self.$kanban_board = $(dontmanage.render_template("kanban_board"));
		...
		function make_columns() {
			...
			columns.filter(is_active_column).map(function (col) {
				dontmanage.views.KanbanBoardColumn(col, self.$kanban_board, self.board_perms);
			});
		}
		...
		function bind_add_column() {
			...
			const is_custom_field = dontmanage.meta.get_docfield(doctype, fieldname)?.is_custom_field;
	...
	dontmanage.views.KanbanBoardColumn = function (column, wrapper, board_perms) {
		var self = {};
		var filtered_cards = [];
		...
		function make_dom() {
			self.$kanban_column = $(
				dontmanage.render_template("kanban_column", {
					title: column.title,
					doctype: store.state.doctype,
					indicator: dontmanage.scrub(column.indicator, "-"),
				})
			).appendTo(wrapper);
			// add task, archive
			...
			if (order) {
				...
				filtered_cards.forEach(function (card) {
					if (order.indexOf(card.name) === -1) {
						dontmanage.views.KanbanBoardCard(card, self.$kanban_cards);
					}
				});
				order.forEach(function (name) {
					if (!filtered_cards_names.includes(name)) return;
					dontmanage.views.KanbanBoardCard(get_card(name), self.$kanban_cards);
				});
			} else {
		...
		function setup_sortable() {
			...
			if (!dontmanage.model.can_write(store.state.doctype)) return;
		...
		function bind_add_card() {
			...
			if (!dontmanage.model.can_create(store.state.doctype)) {
		...
		function bind_options() {
			...
			get_column_indicators(function (indicators) {
				let html = `<li class="button-group">${indicators
					.map((indicator) => {
						let classname = dontmanage.scrub(indicator, "-");
	...
	dontmanage.views.KanbanBoardCard = function (card, wrapper) {
		var self = {};
		...
		function make_dom() {
			var opts = {
				name: card.name,
				title: dontmanage.utils.html2text(card.title),
				disable_click: card._disable_click ? "disable-click" : "",
				creation: card.creation,
				doc_content: get_doc_content(card),
				image_url: cur_list.get_image_url(card),
				form_link: dontmanage.utils.get_form_link(card.doctype, card.name),
			};

			self.$card = $(dontmanage.render_template("kanban_card", opts)).appendTo(wrapper);

			if (!dontmanage.model.can_write(card.doctype)) {
		...
		function render_card_meta() {
			...
			if (card.color && dontmanage.ui.color.validate_hex(card.color)) {
				const $div = $("<div>");
				$("<div></div>")
					.css({
						width: "30px",
		...
		function get_assignees_group() {
			return dontmanage.avatar_group(card.assigned_list, 3, {
		...
		function show_assign_to_dialog(e) {
			e.preventDefault();
			e.stopPropagation();
			self.assign_to = new dontmanage.ui.form.AssignToDialog({
				obj: self,
				method: "dontmanage.desk.form.assign_to.add",
	...
	function fetch_customization(doctype) {
		return new Promise(function (resolve) {
			dontmanage.model.with_doc("Customize Form", "Customize Form", function () {
				var doc = dontmanage.get_doc("Customize Form");
	...
	function insert_doc(doc) {
		return dontmanage.call({
			method: "dontmanage.client.insert",
			args: {
				doc: doc,
			},
			callback: function () {
				dontmanage.model.clear_doc(doc.doctype, doc.name);
				dontmanage.show_alert({ message: __("Saved"), indicator: "green" }, 1);
			},
		});
	}
	...
	function get_column_indicators(callback) {
		dontmanage.model.with_doctype("Kanban Board Column", function () {
			var meta = dontmanage.get_meta("Kanban Board Column");
			var indicators;

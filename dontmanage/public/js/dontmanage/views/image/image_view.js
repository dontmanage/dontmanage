/**
 * dontmanage.views.ImageView
 */
dontmanage.provide("dontmanage.views");

dontmanage.views.ImageView = class ImageView extends dontmanage.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = dontmanage.utils.escape_html(title);
		...
		const expand_button_html = item._image_url
			? `<div class="zoom-view" data-name="${encoded_name}">
				${dontmanage.utils.icon("expand", "xs")}
			</div>`
			: "";
	...
	get_attached_images() {
		return dontmanage
			.call({
				method: "dontmanage.core.api.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new dontmanage.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
	...
	get required_libs() {
		return [
			"assets/dontmanage/node_modules/photoswipe/src/photoswipe.css",
			"photoswipe.bundle.js",
		];
	}
};

dontmanage.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
		me.prepare();
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = dontmanage.render_template("photoswipe_dom");
	...
	_show(docname) {
		...
		// init
		this.pswp = new dontmanage.PhotoSwipe(options);
		this.pswp.init();
	}
};

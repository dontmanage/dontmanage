...
dontmanage.breadcrumbs = {
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		this.append_breadcrumb_element(
			`/app/${dontmanage.router.slug(breadcrumbs.workspace)}`,
	...
	set_workspace(breadcrumbs) {
		...
		if (breadcrumbs.module) {
			...
			if (
				...
				breadcrumbs.workspace = dontmanage.boot.module_wise_workspaces[breadcrumbs.module][0];
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = dontmanage.get_doc("DocType", doctype);
		if (
	...
	set_form_breadcrumb(breadcrumbs, view) {
		...
		let form_route = `/app/${dontmanage.router.slug(doctype)}/${docname_title}`;
		...
		if (view === "form") {
			...
			last_crumb.click((event) => {
				...
				dontmanage.utils.copy_to_clipboard(last_crumb.text());
			});
		}
	},
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		...
		let dashboard_route = `/app/${dontmanage.router.slug(doctype)}/${docname}`;

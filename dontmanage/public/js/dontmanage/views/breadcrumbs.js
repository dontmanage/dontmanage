...
dontmanage.breadcrumbs = {
	...
	add(module, doctype, type) {
		...
		this.all[dontmanage.breadcrumbs.current_page()] = obj;
		this.update();
	},

	current_page() {
		return dontmanage.get_route_str();
	},

	update() {
		var breadcrumbs = this.all[dontmanage.breadcrumbs.current_page()];
		...
		if (breadcrumbs.type === "Custom") {
			...
			// form / print
			let view = dontmanage.get_route()[0];
			...
			if (breadcrumbs.doctype && ["print", "form"].includes(view)) {
				...
				breadcrumbs.label = dontmanage.query_report.page_title;
	...
	get last_route() {
		return dontmanage.route_history.slice(-2)[0];
	},
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (
			...
			(breadcrumbs.module_info.blocked ||
				!dontmanage.visible_modules.includes(breadcrumbs.module_info.module))
		) {
			return;
		}
		if (dontmanage.app.sidebar.sidebar_title) {
			let icon = dontmanage.utils.get_desktop_icon_by_label(dontmanage.app.sidebar.sidebar_title);
			let url = dontmanage.utils.get_route_for_icon(icon);
			if (url) {
	...
	set_workspace(breadcrumbs) {
		...
		if (this.last_route?.[0] == "Workspaces") {
			...
			if (
				breadcrumbs.module &&
				dontmanage.boot.module_wise_workspaces[breadcrumbs.module]?.includes(last_workspace)
			) {
				breadcrumbs.workspace = last_workspace;
			}
		} else {
			// choose from __workspaces
			const doctype_meta = dontmanage.get_meta(breadcrumbs.doctype);
			...
			if (breadcrumbs.module) {
				...
				breadcrumbs.module_info = dontmanage.get_module(breadcrumbs.module);

				// set workspace
				if (
					breadcrumbs.module_info &&
					dontmanage.boot.module_wise_workspaces[breadcrumbs.module]
				) {
					breadcrumbs.workspace =
						dontmanage.boot.module_wise_workspaces[breadcrumbs.module][0];
				}
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = dontmanage.get_meta(doctype);
		if (
			(doctype === "User" && !dontmanage.user.has_role("System Manager")) ||
			doctype_meta?.issingle
		) {
			// no user listview for non-system managers and single doctypes
		} else {
			let route;
			const doctype_route = dontmanage.router.slug(dontmanage.router.doctype_layout || doctype);
			if (doctype_meta?.is_tree) {
				let view = dontmanage.model.user_settings[doctype].last_view || "Tree";
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		let docname = dontmanage.get_route().slice(2).join("/");
		let doc = dontmanage.get_doc(doctype, docname);
		let form_route = `/desk/${dontmanage.router.slug(doctype)}/${encodeURIComponent(docname)}`;
		...
		if (docname.startsWith("new-" + doctype.toLowerCase().replace(/ /g, "-"))) {
			...
			let title = dontmanage.model.get_doc_title(doc);
			docname_title = title || doc.name;
			if (dontmanage.utils.is_html(docname_title)) {
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		const doctype = breadcrumbs.doctype;
		const docname = dontmanage.get_route()[1];
		let dashboard_route = `/desk/${dontmanage.router.slug(doctype)}/${docname}`;
		$(`<li><a href="${dashboard_route}">${__(docname)}</a></li>`).appendTo(this.$breadcrumbs);
	},

	setup_modules() {
		if (!dontmanage.visible_modules) {
			dontmanage.visible_modules = $.map(dontmanage.boot.allowed_workspaces, (m) => {
				return m.module;
			});
		}
	},
	...
	rename(doctype, old_name, new_name) {
		...
		delete dontmanage.breadcrumbs.all[old_route_str];
		this.update();
	},

	clear() {
		this.$breadcrumbs = $(".navbar-breadcrumbs").empty();
		this.append_breadcrumb_element("/desk", dontmanage.utils.icon("monitor"));
	},

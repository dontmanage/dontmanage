...
dontmanage.last_edited_communication = {};
const separator_element = "<div>---</div>";

dontmanage.views.CommunicationComposer = class {
	constructor(opts) {
		$.extend(this, opts);
		if (!this.doc) {
	...
	get_fields() {
		...
		const fields = [
			...
			},
			{
				...
				fieldname: "content",
				onchange: dontmanage.utils.debounce(this.save_as_draft.bind(this), 300),
			...
			{ fieldtype: "Section Break" },
			{
				...
				default: dontmanage.boot.user.send_me_a_copy,
			},
			{
	...
	guess_language() {
		...
		if (print_format != "Standard") {
			print_format_lang = dontmanage.get_doc(
				"Print Format",
				print_format
			)?.default_print_language;
		}

		let lang = document_lang || print_format_lang || dontmanage.boot.lang;
		this.dialog.set_value("print_language", lang);
	}

	async check_email_template_html(email_template) {
		const r = await dontmanage.db.get_value("Email Template", email_template, "use_html");
	...
	setup_multiselect_queries() {
		["recipients", "cc", "bcc"].forEach((field) => {
			this.dialog.fields_dict[field].get_data = () => {
				...
				dontmanage.call({
					method: "dontmanage.email.get_contact_list",
					args: args,
					callback: (r) => {
	...
	setup_email_template() {
		...
		function add_template() {
			...
			dontmanage.call({
				method: "dontmanage.email.doctype.email_template.email_template.get_email_template",
				args: {
	...
	get_last_edited_communication(clear) {
		if (!dontmanage.last_edited_communication[this.doctype]) {
			dontmanage.last_edited_communication[this.doctype] = {};
		}

		if (clear || !dontmanage.last_edited_communication[this.doctype][this.key]) {
			dontmanage.last_edited_communication[this.doctype][this.key] = {};
		}

		return dontmanage.last_edited_communication[this.doctype][this.key];
	}
	...
	setup_print() {
		...
		if (this.frm) {
			const print_formats = dontmanage.meta.get_print_formats(this.frm.meta.name);
	...
	get_attachment_row(attachment, checked) {
		return $(`<p class="checkbox flex">
			<label title="${attachment.file_name}" style="max-width: 100%">
				...
				>
					${dontmanage.utils.icon("link-url", "sm")}
				</a>
			</label>
		</p>`);
	}
	...
	setup_email() {
		...
		$(fields.send_me_a_copy.input).on("click", () => {
			...
			dontmanage.db.set_value("User", dontmanage.session.user, "send_me_a_copy", val);
			dontmanage.boot.user.send_me_a_copy = val;
		});
	}
	...
	send_email(btn, form_values, selected_attachments, print_html, print_format) {
		...
		if (!form_values.recipients && !form_values.cc && !form_values.bcc) {
			dontmanage.msgprint(__("Enter Email Recipient(s) in the To, CC, or BCC fields"));
			return;
		}
		...
		if (this.frm && !dontmanage.model.can_email(this.doc.doctype, this.frm)) {
			dontmanage.msgprint(__("You are not allowed to send emails related to this document"));
			return;
		}

		return dontmanage.call({
			method: "dontmanage.core.doctype.communication.email.make",
			args: {
					...
					if (r.message["emails_not_sent_to"]) {
						dontmanage.msgprint(
							__("Email not sent to {0} (unsubscribed / disabled)", [
								dontmanage.utils.escape_html(r.message["emails_not_sent_to"]),
							])
						);
					}
	...
	is_print_letterhead_checked() {
		if (this.frm && $(this.frm.wrapper).find(".form-print-wrapper").is(":visible")) {
			...
			return (
				dontmanage.model.get_doc(":Print Settings", "Print Settings") || { with_letterhead: 1 }
			).with_letterhead
				? 1
				: 0;
		}
	...
	async get_signature(sender_email) {
		let signature = dontmanage.boot.user.email_signature;
		...
		if (!signature) {
			...
			const email_accounts = await dontmanage.db.get_list("Email Account", {
				filters: filters,
		...
		if (!dontmanage.utils.is_html(signature)) {
	...
	get_earlier_reply() {
		...
		const communication_date = dontmanage.datetime.global_date_format(
	...
	html2text(html) {
		...
		const text = dontmanage.utils.html2text(html);
		return text.replace(/\n{3,}/g, "\n\n");
	}

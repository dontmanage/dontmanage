...
export default class Block {
	...
	make(block, block_name, widget_type = block) {
		let block_data = this.config.page_data[block + "s"].items.find((obj) => {
			return (
				dontmanage.utils.unescape_html(obj.label) == dontmanage.utils.unescape_html(__(block_name))
			);
		});
		if (!block_data) return false;
		this.wrapper.innerHTML = "";
		block_data.in_customize_mode = !this.readOnly;
		this.block_widget = new dontmanage.widget.SingleWidgetGroup({
	...
	new(block, widget_type = block) {
		...
		this.dialog = new dialog_class({
			...
			primary_action: (widget) => {
				...
				this.block_widget = dontmanage.widget.make_widget({
					...widget,
	...
	add_new_block_button() {
		let $new_button = $(`
			<div class="new-block-button">${dontmanage.utils.icon("add-round", "lg")}</div>
		`);
	...
	add_settings_button() {
		...
		this.dropdown_list = [
			...
			},
			{
				label: "Expand",
				title: "Expand Block",
				icon: dontmanage.utils.icon("expand-alt", "sm"),
				action: () => this.increase_width(),
			},
			{
				label: "Shrink",
				title: "Shrink Block",
				icon: dontmanage.utils.icon("shrink", "sm"),
				action: () => this.decrease_width(),
			},
			{
				label: "Move Up",
				title: "Move Up",
				icon: dontmanage.utils.icon("up-arrow", "sm"),
				action: () => this.move_block("up"),
			},
			{
				label: "Move Down",
				title: "Move Down",
				icon: dontmanage.utils.icon("down-arrow", "sm"),
				action: () => this.move_block("down"),
			},
			{
				label: "Duplicate",
				title: "Duplicate",
				icon: dontmanage.utils.icon("copy", "sm"),

...
dontmanage.standard_pages["Workspaces"] = function () {
	var wrapper = dontmanage.container.add_page("Workspaces");
	...
	dontmanage.workspace = new dontmanage.views.Workspace(wrapper);
	$(wrapper).bind("show", function () {
		dontmanage.workspace.show();
	});
};

dontmanage.views.Workspace = class Workspace {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = wrapper.page;
		this.workspaces = dontmanage.boot.workspaces.pages;

		this.blocks = dontmanage.workspace_block.blocks;
		...
		this.sidebar = dontmanage.app.sidebar;
		this.cached_pages = $.extend(true, {}, dontmanage.boot.workspaces);
		this.has_access = dontmanage.boot.workspaces.has_access;
		this.has_create_access = dontmanage.boot.workspaces.has_create_access;
		this.setup();
		this.show();
	...
	get_pages() {
		return dontmanage.xcall("dontmanage.desk.desktop.get_workspace_sidebar_items", null, "GET");
	}
	...
	get_data(page) {
		return dontmanage
			.call({
				method: "dontmanage.desk.desktop.get_desktop_page",
				args: {
			...
			.then((data) => {
				...
				return dontmanage.dashboard_utils.get_dashboard_settings().then((settings) => {
					if (settings) {
	...
	get_page_to_show() {
		...
		if (dontmanage.boot.user.default_workspace) {
			default_page = {
				name: dontmanage.boot.user.default_workspace.name,
				public: dontmanage.boot.user.default_workspace.public,
			};
		} else if (
		...
		if (this.workspaces.length) {
			...
			if (dontmanage.is_mobile()) {
				dontmanage.breadcrumbs.add({
					type: "Custom",
					label: header_dropdown + `${dontmanage.utils.icon("chevron-down")}`,
					route: "#",
			...
			if (!this._page.public) {
				...
				if (!app && this._page.module) {
					app = dontmanage.boot.module_app[dontmanage.router.slug(this._page.module)];
				}
				// this._page.module && this.sidebar.show_sidebar_for_module(this._page.module);
				if (!app) app = "dontmanage";
			}
			...
			this.wrapper
				.find(".workspace-icon")
				.html(dontmanage.utils.icon(this._page.icon || "folder-normal", "md"));
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			await this.editor.readOnly.toggle();
			this.is_read_only = true;
			dontmanage.boot.workspaces = this.cached_pages;
			this.reload();
			dontmanage.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
		});

		if (page.name && this.has_access) {
			this.page.add_inner_button(__("Settings"), () => {
				dontmanage.set_route(`workspace/${page.name}`);
			});
		}
	}
	...
	initialize_new_page() {
		...
		const d = new dontmanage.ui.Dialog({
			...
			primary_action: (values) => {
				...
				let name = values.title + (values.is_public ? "" : "-" + dontmanage.session.user);
				let blocks = [
					{
						type: "header",
				...
				let new_page = {
					...
					is_editable: true,
					selected: true,
					app: dontmanage.current_app?.app_name,
					type: values.type,
					...
					this.editor
						...
						.then(async () => {
							...
							this.create_page(new_page).then(() => {
								...
								let route = pre_url + dontmanage.router.slug(new_page.title);
	...
	create_page(new_page) {
		...
		return new Promise((resolve) => {
			dontmanage.call({
				method: "dontmanage.desk.doctype.workspace.workspace.new_page",
				args: {
					new_page: new_page,
				},
				callback: (r) => {
					if (r.message) {
						...
						if (r.message) {
							dontmanage.boot.workspaces = r.message.workspace_pages;
							me.workspaces = dontmanage.boot.workspaces.pages;
							me.setup_pages(dontmanage.boot.workspaces.pages);
							dontmanage.boot.workspace_sidebar_item = r.message.sidebar_items;
						}

						if (new_page.public === 0) {
							dontmanage.app.sidebar.setup("private");
						}
	...
	setup_pages(all_pages) {
		...
		if (all_pages) {
			...
			dontmanage.workspace_list = [];
			dontmanage.workspace_map = {};
			for (let page of all_pages) {
				...
				if (!page.app && page.module) {
					page.app = dontmanage.boot.module_app[dontmanage.slug(page.module)];
				}
				dontmanage.workspace_map[page.name] = page;
				dontmanage.workspace_list.push(page);
			}
		}
	}
	initialize_editorjs(blocks) {
		this.tools = {
			...
			HeaderSize: dontmanage.workspace_block.tunes["header_size"],
		};
	...
	save_page(page) {
		...
		return this.editor
			...
			.then((outputData) => {
				...
				page.content = JSON.stringify(blocks);
				dontmanage.call({
					method: "dontmanage.desk.doctype.workspace.workspace.save_page",
					args: {
						name: page.name,
					...
					callback: function (res) {
						if (res.message) {
							...
							if (page.public) {
								dontmanage.set_route("desk", page.title.toLowerCase());
							} else {
								dontmanage.set_route("desk", "private", page.title.toLowerCase());
							}
						}
					},
				});
	...
	reload() {
		...
		return this.get_pages().then((r) => {
			dontmanage.boot.workspaces = r;
			this.setup_pages(dontmanage.boot.workspaces.pages);
			this.show();
	...
	create_page_skeleton() {
		...
		this.body.prepend(dontmanage.render_template("workspace_loading_skeleton"));
	...
	register_awesomebar_shortcut() {
		"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter) => {
			...
			dontmanage.ui.keys.add_shortcut({ shortcut: letter, ...default_shortcut });
			dontmanage.ui.keys.add_shortcut({ shortcut: `shift+${letter}`, ...default_shortcut });
		});
	}
};

...
dontmanage.views.Workspace = class Workspace {
	...
	get_pages() {
		return dontmanage.xcall("dontmanage.desk.desktop.get_workspace_sidebar_items");
	}
			...
			>
				<div class="desk-sidebar-item standard-sidebar-item ${item.selected ? "selected" : ""}">
					<a
						href="/app/${
							item.public
								? dontmanage.router.slug(item.title)
								: "private/" + dontmanage.router.slug(item.title)
						}"
	...
	make_sidebar() {
		...
		this.sidebar.find(".selected").length &&
			!dontmanage.dom.is_element_in_viewport(this.sidebar.find(".selected")) &&
	...
	append_item(item, container) {
		let is_current_page =
			dontmanage.router.slug(item.title) == dontmanage.router.slug(this.get_page_to_show().name) &&
	...
	get_data(page) {
		return dontmanage
			.call("dontmanage.desk.desktop.get_desktop_page", {
				page: page,
			})
			.then((data) => {
	...
	edit_page(item) {
		...
		const d = new dontmanage.ui.Dialog({
			...
			primary_action: (values) => {
				...
				dontmanage.call({
					method: "dontmanage.desk.doctype.workspace.workspace.update_page",
					args: {
						name: old_item.name,
						title: values.title,
						icon: values.icon || "",
					...
					callback: function (res) {
						if (res.message) {
							...
							dontmanage.show_alert({ message: message, indicator: "green" });
						}
					},
				});
				...
				if (this.make_page_selected) {
					...
					let route = pre_url + dontmanage.router.slug(values.title);
	...
	update_sidebar(old_item, new_item) {
		...
		if (is_title_changed || is_section_changed) {
			if (new_item.is_public) {
				...
				let user = dontmanage.session.user;
	...
	update_cached_values(old_item, new_item, duplicate, new_page) {
		...
		// update dontmanage.workspaces
		if (dontmanage.workspaces[dontmanage.router.slug(old_item.name)] || new_page) {
			!duplicate && delete dontmanage.workspaces[dontmanage.router.slug(old_item.name)];
			if (new_item) {
				dontmanage.workspaces[dontmanage.router.slug(new_item.name)] = { title: new_item.title };
			}
		}
	...
	add_settings_button(item, sidebar_control) {
		...
		if (this.is_item_deletable(item)) {
			this.dropdown_list.push({
				...
				icon: dontmanage.utils.icon("delete-active", "sm"),
	...
	is_item_deletable(item) {
		...
		if (
			!item.public ||
			(item.public && (!item.module || (item.module && dontmanage.boot.developer_mode)))
		)
			return true;
		return false;
	}
	...
	delete_page(page) {
		dontmanage.confirm(
			...
			() => {
				dontmanage.call({
					method: "dontmanage.desk.doctype.workspace.workspace.delete_page",
					args: { page: page },
					callback: function (res) {
						if (res.message) {
	...
	duplicate_page(page) {
		...
		const d = new dontmanage.ui.Dialog({
			...
			primary_action: (values) => {
				...
				d.hide();
				dontmanage.call({
					method: "dontmanage.desk.doctype.workspace.workspace.duplicate_page",
					args: {
						page_name: page.name,
						new_page: values,
					},
	...
	hide_unhide_workspace(page, event, hide) {
		...
		dontmanage.call({
			method: "dontmanage.desk.doctype.workspace.workspace." + method,
			args: {
				page_name: page.name,
			},
			callback: (r) => {
	...
	update_sorted_sidebar() {
		if (this.sorted_public_items || this.sorted_private_items) {
			dontmanage.call({
				method: "dontmanage.desk.doctype.workspace.workspace.sort_pages",
				args: {
	...
	validate_page(new_page, old_page) {
		...
		if (dontmanage.router.doctype_route_exist(dontmanage.router.slug(new_page.title))) {
	...
	create_sidebar_skeleton() {
		...
		$(dontmanage.render_template("workspace_sidebar_loading_skeleton")).insertBefore(this.sidebar);

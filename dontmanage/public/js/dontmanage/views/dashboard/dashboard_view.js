...
dontmanage.views.DashboardView = class DashboardView extends dontmanage.views.ListView {
	get view_name() {
		return "Dashboard";
	}
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.dashboard_settings =
				dontmanage.get_user_settings(this.doctype)["dashboard_settings"] || null;
		});
	}
	...
	render_empty_state() {
		...
		const empty_state_image = "/assets/dontmanage/images/ui-states/list-empty-state.svg";
	...
	reset_dashboard_customization() {
		dontmanage.confirm(__("Are you sure you want to reset all customizations?"), () => {
			this.dashboard_settings = null;
			dontmanage.model.user_settings
		...
		const dialog = new dontmanage.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					get_query: () => {
						return {
							query: "dontmanage.desk.doctype.dashboard_chart.dashboard_chart.get_charts_for_user",
							filters: {
								document_type: this.doctype,
							},
						};
			...
			primary_action: (values) => {
				...
				if (chart.new_or_existing == "New Chart") {
					...
					dontmanage
						.xcall(
							"dontmanage.desk.doctype.dashboard_chart.dashboard_chart.create_dashboard_chart",
							{ args: chart }
						)
						.then((doc) => {
	...
	get_field_options() {
		...
		dontmanage.get_meta(this.doctype).fields.map((df) => {

...
dontmanage.provide("dontmanage.query_reports");

dontmanage.standard_pages["query-report"] = function () {
	var wrapper = dontmanage.container.add_page("query-report");
	...
	dontmanage.query_report = new dontmanage.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind("show", function () {
		dontmanage.query_report.show();
	});
};

dontmanage.views.QueryReport = class QueryReport extends dontmanage.views.BaseList {
	show() {
	...
	init() {
		...
		this.init_promise = dontmanage.run_serially(tasks);
	...
	update_url_with_filters() {
		if (dontmanage.get_route_str() == this.page_name) {
	...
	setup_events() {
		dontmanage.realtime.on("report_generated", (data) => {
			...
			if (data.report_name) {
				...
				if (data.name == this.prepared_report_doc_name) {
					...
					dontmanage.show_alert({ message: alert_message, indicator: "orange" });
				}
			}
		});
	...
	load() {
		if (dontmanage.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, dontmanage.route_options);

		if (this.report_name !== dontmanage.get_route()[1]) {
			// different report
			this.load_report(route_options);
		} else if (dontmanage.has_route_options()) {
	...
	add_card_to_dashboard() {
		let field_options = dontmanage.report_utils.get_field_options_from_report(
			this.columns,
			this.raw_data
		);
		const dashboard_field = dontmanage.dashboard_utils.get_dashboard_link_field();
		const set_standard = dontmanage.boot.developer_mode;
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			dontmanage.msgprint(__("Please Set Chart"));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			"dontmanage.desk.doctype.number_card.number_card.create_report_number_card",
			args,
			dashboard_name,
			card_name,
			"Number Card"
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			chart_type: "Report",
			report_name: this.report_name,
			type: chart_args.chart_type || dontmanage.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			"dontmanage.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart",
			args,
			dashboard_name,
			chart_name,
			"Dashboard Chart"
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		dontmanage.xcall(method, { args: args }).then(() => {
			...
			dontmanage.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	refresh_report(route_options) {
		...
		return dontmanage.run_serially([
			() => this.setup_filters(),
	...
	get_report_doc() {
		return dontmanage.model
			.with_doc("Report", this.report_name)
			.then((doc) => {
				this.report_doc = doc;
			})
			.then(() => dontmanage.model.with_doctype(this.report_doc?.ref_doctype))
			.then(
				() =>
					this.report_doc.module &&
					dontmanage.app.sidebar.show_sidebar_for_module(this.report_doc.module)
			);
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (dontmanage.query_reports[this.report_name]) {
				this.report_settings = dontmanage.query_reports[this.report_name];
				resolve();
			} else {
				dontmanage
					.xcall("dontmanage.desk.query_report.get_script", {
						report_name: this.report_name,
					})
					.then((settings) => {
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function () {
			seconds_elapsed += 1;
			dontmanage.show_progress(__("Preparing Report"), seconds_elapsed, execution_time);
		}, 1000);
	}
	...
	evaluate_depends_on_value(expression, filter_label) {
		...
		if (doc) {
			if (typeof expression === "boolean") {
				...
				try {
					out = dontmanage.utils.eval(expression.substr(5), { doc });
				} catch (e) {
					dontmanage.throw(
	...
	add_collapse_button() {
		...
		if (this.filters[filter_no]) {
			this.$collapse_button = $(`<div>${dontmanage.utils.icon("chevron-down", "md")}</div>`);
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = dontmanage.route_options;
	...
	refresh(have_filters_changed) {
		...
		return new Promise((resolve) => {
			const js_filters = (dontmanage.query_reports[this.report_name]?.filters || [])
				.filter(
					(filter) => filter.fieldtype === "Link" && filters[filter.fieldname] !== ""
				)
				.map(({ fieldname, fieldtype, options }) => ({ fieldname, fieldtype, options }));

			this.last_ajax = dontmanage.call({
				method: "dontmanage.desk.query_report.run",
				type: "GET",
				args: {
					report_name: this.report_name,
					filters: filters,
			...
			.then((r) => {
				...
				if (data.result && data.result.length) {
					...
					if (this.chart_options) {
						...
						this.$chart.empty();
						if (this.chart_fields) {
							this.chart_options = dontmanage.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	render_summary(data) {
		data.forEach((summary) => {
			dontmanage.utils.build_summary_item(summary).appendTo(this.$summary);
		});
	...
	add_prepared_report_buttons(doc) {
				...
				function () {
					window.open(
						dontmanage.urllib.get_full_url(
							"/api/method/dontmanage.core.doctype.prepared_report.prepared_report.download_attachment?" +
								"dn=" +
		...
		this.primary_action_map = {
			...
			Edit: {
				label: __("Edit"),
				click: () => {
					dontmanage.set_route(dontmanage.get_route());
				},
			},
			Rebuild: {
				label: __("Rebuild"),
	...
	show_warning_or_generate_report() {
		dontmanage
			.xcall(
				"dontmanage.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state",
				{
			...
			.then((reports) => {
				...
				if (reports.length) {
					...
					this.prepared_report_dialog = dontmanage.warn(
					...
					dontmanage.utils.bind_actions_with_object(
	...
	delete_old_queued_reports() {
		...
		dontmanage
			.xcall("dontmanage.core.doctype.prepared_report.prepared_report.delete_prepared_reports", {
		...
		if (!missing_mandatory.length) {
			...
			return new Promise((resolve) =>
				...
				let alert_message =
					...
					`</a>`;
				dontmanage.show_alert({ message: alert_message, indicator: "orange" }, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (data.length > (cint(dontmanage.boot.sysdefaults.max_report_rows) || 100000)) {
			let msg = __(
			...
			this.toggle_message(true, `${dontmanage.utils.icon("solid-warning")} ${msg}`);
			return;
		}
		...
		if (
			...
			let datatable_options = {
				...
				data: data,
				inlineFilters: true,
				language: dontmanage.boot.lang,
				translations: dontmanage.utils.datatable.get_translations(),
				treeView: this.tree_report,
				layout: "fixed",
				cellHeight: 33,
				showTotalRow: this.raw_data.add_total_row && !this.report_settings.tree,
				direction: dontmanage.utils.is_rtl() ? "rtl" : "ltr",
				hooks: {
					columnTotal: dontmanage.utils.report_column_total,
				},
			};
	...
	update_masked_fields_in_columns(columns) {
		const masked_fields = dontmanage.get_meta(this.report_doc?.ref_doctype).masked_fields;
	...
	get_chart_options(data) {
		...
		options.axisOptions = {
			...
			numberFormatter: dontmanage.utils.format_chart_axis_number,
		};
		options.height = 280;
		return options;
	}
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new dontmanage.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = dontmanage.report_utils.make_chart_options(
					me.columns,
					me.raw_data,
					values
				);
				...
				wrapper.empty();
				new dontmanage.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		...
		return columns.map((column) => {
			column = dontmanage.report_utils.prepare_field_from_column(column);
			...
			if (column.fieldtype === "Date") {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = dontmanage.datetime.user_to_obj(keyword);
					const cellValue = dontmanage.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = dontmanage.get_meta(this.report_doc.ref_doctype);
		dontmanage.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log(method, file_format) {
		dontmanage.call("dontmanage.core.doctype.access_log.access_log.make_access_log", {
	...
	async pdf_report(print_settings) {
		const base_url = dontmanage.urllib.get_base_url();
		const print_css = dontmanage.boot.print_css;
		const landscape = print_settings.orientation == "Landscape";
		...
		const content = dontmanage.render_template(template, {
			title: __(this.report_name),
			subtitle: print_settings?.include_filters ? filters_html : null,
		...
		const html = dontmanage.render_template("print_template", {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
	...
	async get_report_print_format(report_name) {
		...
		const r = await dontmanage.db.get_value("Print Format", filters, ["html", "css"]);
		if (r && r.message && r.message.html) {
			const css = r.message.css || "";
			const html = r.message.html || "";
			return `<style>${css}</style>${html}`;
		} else {
			dontmanage.msgprint(__("Print Format not found"));
			return null;
		}
	}
	...
	async render_report_letterhead(print_settings) {
		...
		if (!doc_context.company) {
			doc_context.company = dontmanage.defaults.get_default("company");
		}

		try {
			const r = await dontmanage.call("dontmanage.utils.print_format.render_letterhead_for_print", {
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map((fieldname) => {
				...
				if (dontmanage.utils.is_empty(value) || docfield.hidden_due_to_dependency) {
					return null;
				}
				...
				if (docfield.fieldtype === "Check") {
					...
					display_value = dontmanage.format(value, docfield);
				}
				...
				extra_fields.push(
					...
					},
					{
						...
						description: __("Hidden columns include: <br> {0}", [
							dontmanage.utils.comma_and(Array.from(hidden_fields)),
						]),
					}
				);
			}
	...
	get_applied_filters(filters) {
		...
		for (const [key, value] of Object.entries(filters)) {
			const df = dontmanage.query_report.get_filter(key).df;
	...
	get_menu_items() {
		let items = [
			...
			{
				label: __("Edit"),
				action: () => dontmanage.set_route("Form", "Report", this.report_name),
				condition: () => dontmanage.user.is_report_manager(),
				standard: true,
			},
			{
				label: __("Print"),
				action: () => {
					let dialog = dontmanage.ui.get_print_settings(
						false,
				...
				condition: () => dontmanage.model.can_print(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("PDF"),
			...
			},
			{
				...
				condition: () => dontmanage.model.can_export(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("Setup Auto Email"),
				action: () =>
					dontmanage.set_route("List", "Auto Email Report", { report: this.report_name }),
				standard: true,
			},
			{
				label: __("Add Column"),
				action: () => {
					let d = new dontmanage.ui.Dialog({
						...
						fields: [
							{
								...
								options: this.linked_doctypes?.map((df) => ({
									label: df.doctype + " (" + dontmanage.unscrub(df.fieldname) + ")",
								...
								change: () => {
									...
									dontmanage.model.with_doctype(doctype, () => {
										let options = dontmanage.meta
											.get_docfields(doctype)
											.filter(dontmanage.model.is_value_type)
											.map((df) => ({
												label: df.label,
												value: df.fieldname,
											}));
						...
						primary_action: (values) => {
							...
							let df = dontmanage.meta.get_docfield(values.doctype, values.field);
							...
							dontmanage.call({
								method: "dontmanage.desk.query_report.get_data_for_custom_field",
								args: {
									field: values.field,
									doctype: values.doctype,
									names: Array.from(
		...
		if (dontmanage.user.is_report_manager()) {
			items.push({
				label: __("Save"),
				action: () => {
					let d = new dontmanage.ui.Dialog({
						...
						primary_action: (values) => {
							dontmanage.call({
								method: "dontmanage.desk.query_report.save_report",
								args: {
								...
								callback: function (r) {
									this.show_save = false;
									d.hide();
									dontmanage.set_route("query-report", r.message);
								},
							});
						},
					});

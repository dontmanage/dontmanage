dontmanage.provide("dontmanage.report_utils");

dontmanage.report_utils = {
	make_chart_options: function (
		...
		function get_translated_column_label(fieldname) {
			...
			return column?.label ?? __(dontmanage.model.unscrub(fieldname));
		}
	},
		...
		function get_options(fields) {
			return fields.map((field) => {
				if (field.fieldname) {
					...
					field = dontmanage.report_utils.prepare_field_from_column(field);
					return { label: field.label, value: field.fieldname };
				}
			});
		}
	...
	get_report_filters: function (report_name) {
		if (dontmanage.query_reports[report_name]) {
			let filters = dontmanage.query_reports[report_name].filters;
		...
		return dontmanage
			...
			.then((r) => {
				dontmanage.dom.eval(r.script);
				return dontmanage.after_ajax(() => {
					if (
						dontmanage.query_reports[report_name] &&
						!dontmanage.query_reports[report_name].filters &&
						r.filters
					) {
						return (dontmanage.query_reports[report_name].filters = r.filters);
					}
					return (
						dontmanage.query_reports[report_name] &&
						dontmanage.query_reports[report_name].filters
	...
	get_export_dialog(report_name, extra_fields, callback) {
		...
		function update_csv_preview(dialog) {
			const is_query_report = dontmanage.get_route()[0] === "query-report";
			const report = is_query_report ? dontmanage.query_report : cur_list;
			...
			dialog.set_value(
				"csv_preview",
				dontmanage.report_utils.get_csv_preview(
					PREVIEW_DATA,
	...
	get_csv_preview(data, quoting, delimiter, decimal_sep) {
		...
		if (delimiter.length > 1) {
			dontmanage.throw(__("Delimiter must be a single character"));
		}

		if (decimal_sep.length > 1) {
			dontmanage.throw(__("Decimal Separator must be a single character"));
		}

		if (0 > quoting || quoting > 3) {
			dontmanage.throw(__("Quoting must be between 0 and 3"));
		}

		if (decimal_sep !== "." && quoting === QUOTING.NonNumeric) {
			dontmanage.throw(__("Decimal Separator must be '.' when Quoting is set to Non-numeric"));
		}

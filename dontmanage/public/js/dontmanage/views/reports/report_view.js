...
dontmanage.views.ReportView = class ReportView extends dontmanage.views.ListView {
	get view_name() {
		return "Report";
	}
	...
	setup_sort_selector() {
		this.sort_selector = new dontmanage.ui.SortSelector({
		...
		this.group_by_control = new dontmanage.ui.GroupBy(this);
	...
	before_refresh() {
		if (this.report_doc) {
			// don't parse dontmanage.route_options if this is a Custom Report
	...
	async get_link_title_field_value(doctype, value) {
		return (
			dontmanage.utils.get_link_title(doctype, value) ||
			(await dontmanage.utils.fetch_link_title(doctype, value))
		);
	}
	...
	set_dirty_state_for_custom_report() {
		...
		if (!dontmanage.utils.deep_equal(current_settings, report_settings)) {
	...
	prepare_data(r) {
		let data = r.message || {};
		data = dontmanage.utils.dict(data.keys, data.values);
	...
	on_update(data) {
		if (this.doctype === data.doctype && data.name) {
			...
			const flash_row = data.user !== dontmanage.session.user;
	...
	setup_charts() {
		...
		for (let col of this.columns) {
			...
			// numeric values in y
			if (col.docfield && dontmanage.model.numeric_fieldtypes.includes(col.docfield.fieldtype)) {
				y_fields.push({
					label: col.content,
					fieldname: col.id,
					value: col.id,
			...
			fields: [
				...
				},
				{
					...
					default: defaults.chart_type
						? dontmanage.utils.to_title_case(defaults.chart_type)
						: "Bar",
				},
			],
			primary_action: (data) => {
	...
	make_chart() {
		...
		this.chart = new dontmanage.Chart(this.$charts_wrapper.find(".charts-inner-wrapper")[0], {
			...
			tooltipOptions: {
				formatTooltipY: (value) => {
					return dontmanage.format(value, get_df(this.chart_args.y_axes[0]), {
	...
	render_editing_input(colIndex, value, parent) {
		...
		if (col.docfield.fieldtype === "Text Editor") {
			...
			// make control
			control = dontmanage.ui.form.make_control({
				df: col.docfield,
				parent: parent,
				render_input: true,
			});
	...
	evaluate_read_only_depends_on(expression, data) {
		...
		if (typeof expression === "boolean") {
			...
			try {
				out = dontmanage.utils.eval(expression.substr(5), { doc: data });
				if (parent && parent.istable && expression.includes("is_submittable")) {
					out = true;
				}
			} catch (e) {
				dontmanage.throw(__('Invalid "depends_on" expression'));
			}
	...
	is_editable(df, data) {
		if (
			df &&
			dontmanage.model.can_write(this.doctype) &&
			...
			dontmanage.model.is_non_std_field(df.fieldname)
		) {
	...
	set_default_fields() {
		...
		const fields = this.meta.fields.filter((df) => {
			return (
				...
				dontmanage.perm.has_perm(this.doctype, df.permlevel, "read") &&
	...
	get_fields() {
		let fields = this.fields.map((f) => {
			let column_name = dontmanage.model.get_full_column_name(f[0], f[1]);
			if (f[1] !== this.doctype) {
				// child table field
		...
		const cdt_name_fields = this.get_unique_cdt_in_view().map(
			(cdt) => dontmanage.model.get_full_column_name("name", cdt) + " as " + `'${cdt}:name'`
		);
	...
	add_currency_column(fieldname, doctype, col_index) {
		...
		const df = dontmanage.meta.get_docfield(doctype, fieldname);
		if (
			df &&
			df.fieldtype === "Currency" &&
			df.options &&
			!df.options.includes(":") &&
			dontmanage.meta.has_field(doctype, df.options)
		) {
			...
			const field_label = dontmanage.meta.get_label(doctype, field[0]);
			dontmanage.show_alert(
	...
	remove_column_from_datatable(column) {
		...
		if (field[0] === "name") {
			this.refresh();
			dontmanage.throw(__("Cannot remove ID field"));
		}
	...
	get_columns_for_picker() {
		...
		if (this.data) {
			has_status_values = dontmanage.get_indicator(this.data[0], this.doctype);
		}

		if (!dontmanage.meta.has_field(this.doctype, "status") && has_status_values) {
			doctype_fields = [
				{
					label: __("Status"),
					fieldname: "docstatus",
		...
		const table_fields = dontmanage.meta.get_table_fields(this.doctype);
		...
		for (let f of this.fields) {
			...
			if (f[0] !== "docstatus") {
				...
				if (
					...
					!dontmanage.meta.has_field(this.doctype, "status")
				) {
	...
	build_column(c) {
		...
		let docfield = dontmanage.meta.docfield_map[doctype || this.doctype][fieldname];
		...
		if (!docfield) {
			docfield = dontmanage.model.get_std_field(fieldname, true);
			...
			if (docfield) {
				...
				if (fieldname == "docstatus" && !dontmanage.meta.has_field(this.doctype, "status")) {
		...
		const align = (() => {
			const is_numeric = dontmanage.model.is_numeric_field(docfield);
			if (is_numeric) {
				return "right";
			}
		...
		return {
			...
			format: (value, row, column, data) => {
				...
				if (Array.isArray(row)) {
					doc = row.reduce((acc, curr) => {
						...
						if (
							...
							dontmanage.boot.link_title_doctypes.includes(curr.column.docfield.options)
						) {
				...
				return dontmanage.format(value, column.docfield, { always_show_decimals: true }, doc);
			},
		};
	}
		...
		if (this.add_totals_row) {
			...
			const totals_row = this.columns.map((col, i) => {
				return {
					...
					format: (value) => {
						let formatted_value = dontmanage.format(
							value,
							col.docfield,
							{
								always_show_decimals: true,
	...
	format_total_cell(formatted_value, df) {
		...
		if (dontmanage.model.is_numeric_field(df.docfield)) {
	...
	build_row(d) {
		return this.columns.map((col) => {
			...
			if (col.field === "docstatus" && !dontmanage.meta.has_field(this.doctype, "status")) {
				// get status from docstatus
				let status = dontmanage.get_indicator(d, this.doctype);
				if (status) {
	...
	save_report(save_type) {
		const _save_report = (name) => {
			...
			return dontmanage.call({
				method: "dontmanage.desk.reportview.save_report",
				args: {
					name: name,
					doctype: this.doctype,
				...
				callback: (r) => {
					if (r.exc) {
						dontmanage.msgprint(__("Report was not saved (there were errors)"));
						return;
					}
					if (r.message != this.report_name) {
						// Rerender the reports dropdown,
						// so that this report is included in the dropdown as well.
						dontmanage.boot.user.all_reports[r.message] = {
							ref_doctype: this.doctype,
							report_type: "Report Builder",
							title: r.message,
						};

						dontmanage.set_route("List", this.doctype, "Report", r.message);
						return;
					}
	...
	delete_report() {
		return dontmanage.call({
			method: "dontmanage.desk.reportview.delete_report",
	...
	get_report_doc() {
		return new Promise((resolve) => {
			dontmanage.model.with_doc("Report", this.report_name, () => {
				resolve(dontmanage.get_doc("Report", this.report_name));
			});
		});
	}
	...
	get_columns_totals(data) {
		...
		this.columns.forEach((col, i) => {
			const totals = data.reduce((totals, d) => {
				if (col.id in d && dontmanage.model.is_numeric_field(col.docfield)) {
					totals += flt(d[col.id]);
					return totals;
				}
			}, 0);
	...
	report_menu_items() {
		let items = [
			...
			{
				label: __("Print"),
				condition: () => dontmanage.model.can_print(this.doctype),
				action: () => {
		...
		if (dontmanage.model.can_export(this.doctype)) {
			items.push({
				label: __("Export"),
				action: () => {
		...
		items.push({
			...
			action: () => {
				if (this.report_name) {
					...
					dontmanage.msgprint(__("Please save the report first"));
				}
			},
		});
		...
		const can_edit_or_delete = (action) => {
			...
			return (
				...
				(dontmanage.model[method]("Report") || this.report_doc.owner === dontmanage.session.user)
			);
		};
		...
		if (can_edit_or_delete("delete")) {
			items.push({
				label: __("Delete"),
				action: () =>
					dontmanage.confirm("Are you sure you want to delete this report?", () =>
						this.delete_report()
					),
		...
		if (this.report_name && dontmanage.user.has_role("System Manager")) {
			items.push({
				...
				action: () => {
					...
					dontmanage.set_route("List", "User Permission", args);
				},
			});
		}

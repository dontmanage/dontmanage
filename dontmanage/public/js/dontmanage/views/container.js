...
dontmanage.views.Container = class Container {
	...
	constructor() {
		...
		$(document).on("page-change", function () {
			...
			var route_str = dontmanage.get_route_str();
			$("body").attr("data-route", route_str);
			$("body").attr("data-sidebar", me.has_sidebar() ? 1 : 0);
		});

		$(document).bind("rename", function (event, dt, old_name, new_name) {
			dontmanage.breadcrumbs.rename(dt, old_name, new_name);
		});
	}
	add_page(label) {
	...
	change_to(label) {
		...
		if (label.tagName) {
			...
			var page = dontmanage.pages[label];
		}
		if (!page) {
		...
		this.page._route = dontmanage.router.get_sub_path();
		$(this.page).trigger("show");
		!this.page.disable_scroll_to_top && dontmanage.utils.scroll_to(0);
	...
	has_sidebar() {
		var flag = 0;
		var route_str = dontmanage.get_route_str();
		// check in dontmanage.ui.pages
		flag = dontmanage.ui.pages[route_str] && !dontmanage.ui.pages[route_str].single_column;

		// sometimes dontmanage.ui.pages is updated later,

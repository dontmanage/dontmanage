/**
 * dontmanage.views.InboxView
 */

dontmanage.provide("dontmanage.views");

dontmanage.views.InboxView = class InboxView extends dontmanage.views.ListView {
	static load_last_view() {
		const route = dontmanage.get_route();
		if (!route[3] && dontmanage.boot.email_accounts.length) {
			let email_account;
			if (dontmanage.boot.email_accounts[0].email_id == "All Accounts") {
				email_account = "All Accounts";
			} else {
				email_account = dontmanage.boot.email_accounts[0].email_account;
			}
			dontmanage.set_route("List", "Communication", "Inbox", email_account);
			return true;
		...
		function is_valid(email_account) {
			return dontmanage.boot.email_accounts.find((d) => d.email_account === email_account);
		}
	}
	...
	setup_defaults() {
		...
		this.email_account = dontmanage.get_route()[3];
	...
	get_meta_html(email) {
		...
		if (email.reference_doctype && email.reference_doctype !== this.doctype) {
			link = `<a class="text-muted grey"
				href="${dontmanage.utils.get_form_link(email.reference_doctype, email.reference_name)}"
	...
	get_inbox_filters() {
		...
		if (email_account === "Sent") {
			...
			if (email_account == "All Accounts") {
				op = "in";
				email_account = dontmanage.boot.all_accounts;
			}
	...
	get_no_result_message() {
		...
		const html = dontmanage.model.can_create(args.doctype)
			? `<p>${args.msg}</p>
			<p>
	...
	make_new_doc() {
		if (!this.email_account && !dontmanage.boot.email_accounts.length) {
			dontmanage.route_options = {
				email_id: dontmanage.session.user_email,
			};
			dontmanage.new_doc("Email Account");
		} else {

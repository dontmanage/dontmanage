...
dontmanage.template.compile = function (str, name) {
	...
	if (!dontmanage.template.compiled[key]) {
		if (str.indexOf("'") !== -1) {
			str.replace(/'/g, "\\'");
		...
		function replacer(match, p1, p2, offset, string) {
			var i = dontmanage.utils.get_random(3);
			var len = dontmanage.utils.get_random(3);
			return (
				"{% for (var " +
				i +
				"=0, " +
		...
		dontmanage.template.debug[name] = fn_str;
		try {
			dontmanage.template.compiled[key] = new Function("obj", fn_str);
		} catch (e) {
...
dontmanage.render_template = function (name, data) {
	...
	if (!template) {
		dontmanage.throw(`Template <b>${name}</b> not found.`);
	}
	return dontmanage.render(template, data, name);
};
(dontmanage.render_grid = function (opts) {
	// build context
	if (opts.grid) {
	...
	// render content
	if (!opts.content) {
		opts.content = dontmanage.render_template(opts.template || "print_grid", opts);
	}

	// render HTML wrapper page
	opts.base_url = dontmanage.urllib.get_base_url();
	opts.print_css = dontmanage.boot.print_css;

	(opts.lang = opts.lang || dontmanage.boot.lang),
		(opts.layout_direction = opts.layout_direction || dontmanage.utils.is_rtl() ? "rtl" : "ltr");

	var html = dontmanage.render_template("print_template", opts);

	var w = window.open();

	if (!w) {
		dontmanage.msgprint(__("Please enable pop-ups in your browser"));
	}

	w.document.write(html);
	w.document.close();
}),
	(dontmanage.render_tree = function (opts) {
		opts.base_url = dontmanage.urllib.get_base_url();
		opts.landscape = false;
		opts.print_css = dontmanage.boot.print_css;
		opts.print_format_css_path = dontmanage.assets.bundled_asset("print_format.bundle.css");
		var tree = dontmanage.render_template("print_tree", opts);
		var w = window.open();
...
dontmanage.render_pdf = function (html, opts = {}) {
	...
	xhr.open("POST", "/api/method/dontmanage.utils.print_format.report_to_pdf");
	xhr.setRequestHeader("X-DontManage-CSRF-Token", dontmanage.csrf_token);

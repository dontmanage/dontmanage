...
Object.assign(dontmanage.model, {
	docinfo: {},
	sync: function (r) {
		/* docs:
			extract docs, docinfo (attachments, comments, assignments)
			from incoming request and set in `locals` and `dontmanage.model.docinfo`
		*/
		var isPlain;
		...
		if (r.docs) {
			for (var i = 0, l = r.docs.length; i < l; i++) {
				...
				if (locals[d.doctype] && locals[d.doctype][d.name]) {
					// update values
					dontmanage.model.update_in_locals(d);
				} else {
					dontmanage.model.add_to_locals(d);
				}

				d.__last_sync_on = new Date();

				if (d.doctype === "DocType") {
					dontmanage.meta.sync(d);
				}

				if (d.localname) {
					dontmanage.model.rename_after_save(d, i);
				}
			}
		}

		dontmanage.model.sync_docinfo(r);
		return r.docs;
	},

	rename_after_save: (d, i) => {
		dontmanage.model.new_names[d.localname] = d.name;
		$(document).trigger("rename", [d.doctype, d.localname, d.name]);
		delete locals[d.doctype][d.localname];

		// update docinfo to new dict keys
		if (i === 0) {
			dontmanage.model.docinfo[d.doctype][d.name] = dontmanage.model.docinfo[d.doctype][d.localname];
			dontmanage.model.docinfo[d.doctype][d.localname] = undefined;
		}
	},
	...
	sync_docinfo: (r) => {
		...
		if (r.docinfo) {
			...
			if (!dontmanage.model.docinfo[doctype]) {
				dontmanage.model.docinfo[doctype] = {};
			}
			dontmanage.model.docinfo[doctype][name] = r.docinfo;

			// copy values to dontmanage.boot.user_info
			Object.assign(dontmanage.boot.user_info, r.docinfo.user_info);
		}
	...
	add_to_locals: function (doc) {
		...
		if (!doc.name && doc.__islocal) {
			...
			if (!doc.parentfield) dontmanage.model.clear_doc(doc);

			doc.name = dontmanage.model.get_new_name(doc.doctype);

			if (!doc.parentfield)
				dontmanage.provide("dontmanage.model.docinfo." + doc.doctype + "." + doc.name);
		}
	...
	update_in_locals: function (doc) {
		...
		for (let fieldname in doc) {
			let df = dontmanage.meta.get_field(doc.doctype, fieldname);
			if (df && dontmanage.model.table_fields.includes(df.fieldtype)) {
				// table
				...
				for (let i = 0; i < doc[fieldname].length; i++) {
					...
					if (local_d) {
						...
						if (!d.name) {
							...
							d.name = dontmanage.model.get_new_name(doc.doctype);
						}

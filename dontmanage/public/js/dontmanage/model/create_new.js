...
$.extend(dontmanage.model, {
	...
	get_new_doc: function (doctype, parent_doc, parentfield, with_mandatory_children) {
		...
		var doc = {
			docstatus: 0,
			doctype: doctype,
			name: dontmanage.model.get_new_name(doctype),
			__islocal: 1,
			__unsaved: 1,
			owner: dontmanage.session.user,
		};
		dontmanage.model.set_default_values(doc, parent_doc);
		...
		if (parent_doc) {
			...
			dontmanage.provide("dontmanage.model.docinfo." + doctype + "." + doc.name);
		}

		dontmanage.model.add_to_locals(doc);

		if (with_mandatory_children) {
			dontmanage.model.create_mandatory_children(doc);
		}
		...
		if (dontmanage.route_options && dontmanage.route_options.name_field) {
			...
			delete dontmanage.route_options.name_field;
		}

		// set route options
		if (dontmanage.route_options && !doc.parent) {
			$.each(dontmanage.route_options, function (fieldname, value) {
	...
	make_new_doc_and_get_name: function (doctype, with_mandatory_children) {
		return dontmanage.model.get_new_doc(doctype, null, null, with_mandatory_children).name;
	},
	...
	create_mandatory_children: function (doc) {
		var meta = dontmanage.get_meta(doc.doctype);
		if (meta && meta.istable) return;

		// create empty rows for mandatory table fields
		dontmanage.meta.get_docfields(doc.doctype).forEach(function (df) {
			if (df.fieldtype === "Table" && df.reqd) {
				dontmanage.model.add_child(doc, df.fieldname);
			}
		});
	},
	...
	get_default_value: function (df, doc, parent_doc) {
		var user_default = "";
		var user_permissions = dontmanage.defaults.get_user_permissions();
		let allowed_records = [];
		let default_doc = null;
		let value = null;
		if (user_permissions) {
			({ allowed_records, default_doc } = dontmanage.perm.filter_allowed_docs_for_doctype(
		...
		if (df.fieldtype === "Link" && df.options !== "User") {
			...
			if (has_user_permissions && default_doc) {
				...
				if (!df.ignore_user_permissions) {
					var user_defaults = dontmanage.defaults.get_user_defaults(df.options);
				...
				if (!user_default) {
					...
					user_default = dontmanage.boot.user.last_selected_values[df.options];
				}
		...
		if (!value || df["default"]) {
			...
			if (default_val == "__user" || default_val.toLowerCase() == "user") {
				value = dontmanage.session.user;
			} else if (default_val == "user_fullname") {
				value = dontmanage.session.user_fullname;
			} else if (default_val == "Today") {
				value = dontmanage.datetime.get_today();
				...
				if (df.fieldtype == "Time") {
					...
					value = dontmanage.datetime.system_datetime();
				}
			} else if (default_val[0] === ":") {
				var boot_doc = dontmanage.model.get_default_from_boot_docs(df, doc, parent_doc);
	...
	get_default_from_boot_docs: function (df, doc, parent_doc) {
		...
		if (dontmanage.get_list(df["default"]).length > 0) {
			var ref_fieldname = df["default"].slice(1).toLowerCase().replace(" ", "_");
			var ref_value = parent_doc
				? parent_doc[ref_fieldname]
				: dontmanage.defaults.get_user_default(ref_fieldname);
			var ref_doc = ref_value ? dontmanage.get_doc(df["default"], ref_value) : null;
	...
	add_child: function (parent_doc, doctype, parentfield, idx) {
		...
		if (arguments.length === 2) {
			parentfield = doctype;
			doctype = dontmanage.meta.get_field(parent_doc.doctype, parentfield).options;
		}

		// create row doc
		idx = idx ? idx - 0.1 : (parent_doc[parentfield] || []).length + 1;

		var child = dontmanage.model.get_new_doc(doctype, parent_doc, parentfield);
		child.idx = idx;
	...
	copy_doc: function (doc, from_amend, parent_doc, parentfield) {
		...
		var newdoc = dontmanage.model.get_new_doc(doc.doctype, parent_doc, parentfield);

		for (var key in doc) {
			// dont copy name and blank fields
			var df = dontmanage.meta.get_docfield(doc.doctype, key);
			...
			if (
				...
				if (dontmanage.model.table_fields.includes(df.fieldtype)) {
					for (var i = 0, j = value.length; i < j; i++) {
						var d = value[i];
						dontmanage.model.copy_doc(d, from_amend, newdoc, df.fieldname);
					}
				} else {
					newdoc[key] = doc[key];
				}
		...
		var user = dontmanage.session.user;
	...
	open_mapped_doc: function (opts) {
		...
		return dontmanage.call({
			type: "POST",
			method: "dontmanage.model.mapper.make_mapped_doc",
			args: {
				method: opts.method,
...
dontmanage.create_routes = {};
dontmanage.new_doc = function (doctype, opts, init_callback) {
	if (doctype === "File") {
	...
	return new Promise((resolve) => {
		if (opts && $.isPlainObject(opts)) {
			dontmanage.route_options = opts;
		}
		dontmanage.model.with_doctype(doctype, function () {
			if (dontmanage.create_routes[doctype]) {
				dontmanage.set_route(dontmanage.create_routes[doctype]).then(() => resolve());
			} else {
				dontmanage.ui.form

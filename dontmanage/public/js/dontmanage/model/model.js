...
$.extend(dontmanage.model, {
		...
		if (locals.DocType[doctype]) {
			...
			return dontmanage.call({
				...
				callback: function (r) {
					...
					let meta = r.docs[0];
					dontmanage.model.init_doctype(meta);
	...
	init_doctype: function (meta) {
		if (meta.name === "DocType") {
			...
			dontmanage.meta.__doctype_meta = JSON.parse(JSON.stringify(meta));
		}
	...
	get_doc_title(doc) {
		...
		let meta = dontmanage.get_meta(doc.doctype);
		if (meta.title_field) {
	...
	get_no_copy_list: function (doctype) {
		...
		var docfields = dontmanage.get_meta(doctype).fields || [];

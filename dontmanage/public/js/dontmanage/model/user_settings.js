dontmanage.provide("dontmanage.model.user_settings");

$.extend(dontmanage.model.user_settings, {
	get: function (doctype) {
		return dontmanage
			.call("dontmanage.model.utils.user_settings.get", { doctype })
			.then((r) => JSON.parse(r.message || "{}"));
	},
	save: function (doctype, key, value) {
		if (dontmanage.session.user === "Guest") return Promise.resolve();

		const old_user_settings = dontmanage.model.user_settings[doctype] || {};
	...
	remove: function (doctype, key) {
		var user_settings = dontmanage.model.user_settings[doctype] || {};
	...
	update: function (doctype, user_settings) {
		...
		return dontmanage.call({
			method: "dontmanage.model.utils.user_settings.save",
			args: {
				doctype: doctype,
				user_settings: user_settings,
			},
			callback: function (r) {
				dontmanage.model.user_settings[doctype] = r.message;
			},
		});
	},
});

dontmanage.get_user_settings = function (doctype, key) {
	var settings = dontmanage.model.user_settings[doctype] || {};
	if (key) {

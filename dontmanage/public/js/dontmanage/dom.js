...
dontmanage.provide("dontmanage.dom");
...
dontmanage.dom = {
	...
	get_unique_id: function () {
		const id = "unique-" + dontmanage.dom.id_count;
		dontmanage.dom.id_count++;
		return id;
	},
	set_unique_id: function (ele) {
		...
		var id = "unique-" + dontmanage.dom.id_count;
		$ele.attr("id", id);
		dontmanage.dom.id_count++;
		return id;
	},
	...
	add: function (parent, newtag, className, cs, innerHTML, onclick) {
		if (parent && parent.substr) parent = dontmanage.dom.by_id(parent);
	...
	freeze: function (msg, css_class) {
		...
		dontmanage.dom.freeze_count++;
	},
	unfreeze: function () {
		if (!dontmanage.dom.freeze_count) return; // anything open?
		dontmanage.dom.freeze_count--;
		if (!dontmanage.dom.freeze_count) {
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				dontmanage.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
dontmanage.ellipsis = function (text, max) {
	if (!max) max = 20;
	text = cstr(text);
	if (text.length > max) {
...
dontmanage.run_serially = function (tasks) {
...
dontmanage.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
dontmanage.timeout = (seconds) => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

dontmanage.scrub = function (text, spacer = "_") {
	return text.replace(/ /g, spacer).toLowerCase();
};

dontmanage.unscrub = function (txt) {
	return dontmanage.model.unscrub(txt);
};

dontmanage.get_data_pill = (label, target_id = null, remove_action = null, image = null) => {
	...
	if (remove_action) {
		let remove_btn = $(`
			<span class="remove-btn cursor-pointer">
				${dontmanage.utils.icon("close", "sm")}
			</span>
		`).click(() => {
...
dontmanage.get_modal = function (title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${dontmanage.utils.icon("collapse")}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${dontmanage.utils.icon("close-alt", "sm", "close-alt")}
						</button>
					</div>
				</div>
...
dontmanage.is_online = function () {
	if (dontmanage.boot.developer_mode == 1) {

export class DropdownConsole {
	constructor() {
		this.dialog = new dontmanage.ui.Dialog({
			...
			fields: [
				{
					description: `
					${dontmanage.utils.icon("solid-warning", "xs")}
	...
	async load_contextual_boilerplate() {
		...
		if (cur_frm && !cur_frm.is_new()) {
			default_code = `doc = dontmanage.get_doc("${cur_frm.doc.doctype}", "${cur_frm.doc.name}")\n`;
		} else if (cur_list && dontmanage.get_route()[0] == "List") {
			let args = cur_list.get_args();
			default_code = `docs = dontmanage.get_all("${args.doctype}",
				fields="*",
	...
	async execute_code() {
		...
		const start = dontmanage.datetime.now_datetime(true);
		let { output } = await dontmanage.xcall(
			"dontmanage.desk.doctype.system_console.system_console.execute_code",
			{
				doc: {
		...
		const end = dontmanage.datetime.now_datetime(true);
		...
		output_field.set_description(`Executed in ${time_taken} milliseconds.
			<a target="_blank" href="/desk/console-log?owner=${dontmanage.session.user}" >View Logs</a>`);
	}

	async load_completions() {
		let me = this;
		let items = await dontmanage.xcall(
			"dontmanage.core.doctype.server_script.server_script.get_autocompletion_items",
			null,
			"GET",
			{ cache: true }
		);

dontmanage.ui.TagEditor = class TagEditor {
	constructor(opts) {
		/* docs:
		Arguments
	...
	setup_tags() {
		...
		this.tags = new dontmanage.ui.Tags({
			...
			onTagAdd: (tag) => {
				if (me.initialized && !me.refreshing) {
					return dontmanage.call({
						method: "dontmanage.desk.doctype.tag.tag.add_tag",
			...
			onTagRemove: (tag) => {
				if (!me.refreshing) {
					return dontmanage.call({
						method: "dontmanage.desk.doctype.tag.tag.remove_tag",
						args: me.get_args(tag),
						callback: function (r) {
	...
	setup_awesomplete() {
		...
		$input.on("input", function (e) {
			...
			dontmanage.call({
				method: "dontmanage.desk.doctype.tag.tag.get_tags",
				args: {

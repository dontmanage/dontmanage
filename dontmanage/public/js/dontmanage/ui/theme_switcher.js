...
dontmanage.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
	...
	toggle_theme(theme) {
		...
		dontmanage.show_alert(__("Theme Changed"), 3);

		dontmanage.xcall("dontmanage.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme),
		});
	}
...
dontmanage.ui.add_system_theme_switch_listener = () => {
	dontmanage.ui.dark_theme_media_query.addEventListener("change", () => {
		dontmanage.ui.set_theme();
	});
};

dontmanage.ui.dark_theme_media_query = window.matchMedia("(prefers-color-scheme: dark)");

dontmanage.ui.set_theme = (theme) => {
	const root = document.documentElement;
	let theme_mode = root.getAttribute("data-theme-mode");
	if (!theme) {
		if (theme_mode === "automatic") {
			theme = dontmanage.ui.dark_theme_media_query.matches ? "dark" : "light";
		}
	}
	root.setAttribute("data-theme", theme || theme_mode);
};

dontmanage.ui.get_current_theme = () => {

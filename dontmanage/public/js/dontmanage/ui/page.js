 ...
 *
 * @returns {dontmanage.ui.Page}
 */

/**
 * @typedef {Object} dontmanage.ui.Page
 */

dontmanage.ui.make_app_page = function (opts) {
	opts.parent.page = new dontmanage.ui.Page(opts);
	return opts.parent.page;
};

dontmanage.ui.pages = {};

dontmanage.ui.Page = class Page {
	constructor(opts) {
		$.extend(this, opts);
		...
		dontmanage.ui.pages[dontmanage.get_route_str()] = this;
	}
	...
	setup_mobile_awesomebar() {
		if (dontmanage.boot.desk_settings.search_bar && dontmanage.is_mobile()) {
	...
	setup_scroll_handler() {
		let last_scroll = 0;
		$(".main-section").scroll(
			dontmanage.utils.throttle(() => {
				$(".page-head").toggleClass("drop-shadow", !!document.documentElement.scrollTop);
				let current_scroll = document.documentElement.scrollTop;
				if (
					current_scroll > 0 &&
					last_scroll <= current_scroll &&
					(dontmanage.boot.read_only || dontmanage.boot.user.impersonated_by)
				) {
	...
	load_lib(callback) {
		dontmanage.require(this.required_libs, callback);
	}

	add_main_section() {
		$(dontmanage.render_template("page", {})).appendTo(this.wrapper);
	...
	setup_main_sidebar_toggle() {
		$(".sidebar-toggle-btn.navbar-brand").on("click", (event) => {
			dontmanage.app.sidebar.set_height();
			dontmanage.app.sidebar.toggle_width();
			dontmanage.app.sidebar.prevent_scroll();
		});
	}
	...
	get_icon_label(icon, label) {
		...
		return `${icon ? dontmanage.utils.icon(icon_name, size) : ""} <span class="hidden-xs"> ${__(
			label
		)} </span>`;
	}
	...
	set_action(btn, opts) {
		...
		dontmanage.ui.keys.get_shortcut_group(this).add(btn, text_span.length ? text_span : btn);
	}
	...
	add_dropdown_item({
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${dontmanage.utils.icon(icon)}</span>`;
		}
		...
		if (shortcut) {
			...
			`);
			dontmanage.ui.keys.add_shortcut(shortcut_obj);
		} else {
			$li = $(`
				<li>
					<a class="grey-link dropdown-item" href="#" onClick="return false;">
	...
	set_title(title, icon = null, strip = true, tab_title = "", tooltip_label = "") {
		...
		this.title = title;
		dontmanage.utils.set_title(tab_title || title);
		if (icon) {
			title = `${dontmanage.utils.icon(icon)} ${title}`;
		}
	...
	add_button(label, click, opts) {
		...
		let button = $(`<button
			class="btn ${opts.btn_class || "btn-default"} ${opts.btn_size || "btn-sm"} ellipsis">
				${opts.icon ? dontmanage.utils.icon(opts.icon) : ""}
				${label}
		</button>`);
	...
	add_dropdown_button(parent, label, click, icon) {
		dontmanage.ui.toolbar.add_dropdown_button(parent, label, click, icon);
	}
	...
	add_field(df, parent) {
		...
		var f = dontmanage.ui.form.make_control({
			df: df,

...
dontmanage.ui.toolbar.Toolbar = class {
	constructor() {
		if (
			dontmanage.boot.read_only ||
			dontmanage.boot.user.impersonated_by ||
			(!localStorage.getItem("dismissed_announcement_widget") &&
				strip_html(dontmanage.boot.navbar_settings.announcement_widget) != "") ||
	...
	setup_read_only_mode() {
		if (!dontmanage.boot.read_only) return;
	...
	setup_announcement_widget() {
		let current_announcement = dontmanage.boot.navbar_settings.announcement_widget;

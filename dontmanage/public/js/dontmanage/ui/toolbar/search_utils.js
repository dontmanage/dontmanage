...
dontmanage.search.utils = {
	setup_recent: function () {
		this.recent = JSON.parse(dontmanage.boot.user.recent || "[]") || [];
	},
	results_to_hide: [],
	get_recent_pages: function (keywords) {
		...
		dontmanage.route_history.forEach(function (route, i) {
			if (route[0] === "Form") {
				...
				values.push([dontmanage.route_titles[route.join("/")] || route[0], route]);
			}
		});
		...
		find(values, keywords, function (match) {
			...
			if (route[0] === "Form") {
				...
				out.label = dontmanage.utils.escape_html(match[0]).bold();
				out.value = match[0];
			} else {
	...
	get_frequent_links() {
		let options = [];
		dontmanage.boot.frequently_visited_links.forEach((link) => {
			const label = dontmanage.utils.get_route_label(link.route);
			options.push({
				route: link.route,
				label: label,
				value: label,
	...
	get_search_in_list: function (keywords) {
		...
		if (keywords.split(" ").includes("in") && keywords.slice(-2) !== "in") {
			...
			dontmanage.boot.user.can_read.forEach(function (item) {
				if (dontmanage.boot.user.can_search.includes(item)) {
	...
	get_creatables: function (keywords) {
		...
		if (firstKeyword.toLowerCase() === __("new")) {
			dontmanage.boot.user.can_create.forEach(function (item) {
	...
	get_doctypes: function (keywords) {
		...
		dontmanage.boot.user.can_read.forEach(function (item) {
			...
			if (score) {
				target = item;
				if (dontmanage.boot.single_types.includes(item)) {
					out.push(option("", ["Form", item, item], 0.05));
				} else if (dontmanage.boot.user.can_search.includes(item)) {
					// include 'making new' option
					if (dontmanage.boot.user.can_create.includes(item)) {
						var match = item;
						out.push({
							type: "New",
							label: __("New {0}", [search_result.marked_string || __(item)]),
							...
							match: item,
							onclick: function () {
								dontmanage.new_doc(match, true);
							},
						});
					}

					out.push(option("List", ["List", item], 0.05));
					if (dontmanage.model.can_get_report(item)) {
	...
	get_reports: function (keywords) {
		...
		var out = [];
		var route;
		Object.keys(dontmanage.boot.user.all_reports).forEach(function (item) {
			const search_result = me.fuzzy_search(keywords, item, true);
			var level = search_result.score;
			if (level > 0) {
				var report = dontmanage.boot.user.all_reports[item];
	...
	get_desktop_icons: function (keywords) {
		var me = this;
		var out = [];
		dontmanage.boot.desktop_icons.forEach(function (item) {
	...
	get_dashboards: function (keywords) {
		var me = this;
		var out = [];
		dontmanage.boot.dashboards.forEach(function (item) {
	...
	get_global_results: function (keywords, start, limit, doctype = "") {
		...
		return new Promise(function (resolve, reject) {
			dontmanage.call({
				method: "dontmanage.utils.global_search.search",
				args: {
					text: keywords,
					start: start,
					limit: limit,
	...
	fuzzy_search: function (keywords = "", _item = "", return_marked_string = false) {
		...
		if (score == 0 && dontmanage.boot.lang !== "en" && item != _item) {
			item = _item || "";
	...
	/**
	 * @deprecated Use dontmanage.search.utils.fuzzy_search(subseq, str, true).marked_string instead.
	 */
	...
	get_marketplace_apps: function (keywords) {
		var me = this;
		var out = [];
		dontmanage.boot.marketplace_apps.forEach(function (item) {
			...
			if (search_result.score > 0) {
				var ret = {
					...
					route: [
						`https://dontmanagecloud.com/${item.route}?utm_source=awesomebar`,
						item.name,
					],
				};

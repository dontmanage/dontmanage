...
dontmanage.tags.utils = {
	get_tags: function (txt) {
		...
		dontmanage.tags.tags.forEach((tag) => {
			const search_result = dontmanage.search.utils.fuzzy_search(txt, tag, true);
			if (search_result.score) {
				out.push({
					...
					onclick() {
						...
						dontmanage.searchdialog.search.init_search("#".concat(tag), "tags");
					},
				});
			}
		});
	...
	fetch_tags() {
		dontmanage.call({
			method: "dontmanage.desk.doctype.tag.tag.get_tags_list_for_awesomebar",
			callback: function (r) {
				if (r && r.message) {
					dontmanage.tags.tags = $.extend([], r.message);
				}
			},
		});
	},
	...
	get_tag_results: function (tag) {
		...
		return new Promise(function (resolve) {
			dontmanage.call({
				method: "dontmanage.desk.doctype.tag.tag.get_documents_for_tag",
				args: {
					tag: tag,
				},
				callback: function (r) {

...
dontmanage.search.SearchDialog = class {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		this.search_dialog = new dontmanage.ui.Dialog({
			minimizable: true,
			size: "large",
		});
		this.set_header();
	...
	init_search_objects() {
		this.searches = {
			...
			tags: {
				...
				get_results: (keywords, callback) => {
					var results = dontmanage.search.utils.get_nav_results(keywords);
					dontmanage.tags.utils.get_tag_results(keywords).then(
						(global_results) => {
							results = results.concat(global_results);
							callback(results, keywords);
						},
	...
	render_data(result_sets) {
		let $search_results = $(dontmanage.render_template("search")).addClass("hide");
	...
	render_result(type, result) {
		...
		if (result.image !== undefined) {
			let avatar_html = dontmanage.get_avatar("avatar-medium", result.label, result.image);
	...
	handle_result_click(result, $result) {
		if (result.route_options) {
			dontmanage.route_options = result.route_options;
		}
		$result.on("click", () => {
			...
			if (result.onclick) {
				...
				dontmanage.set_route(result.route);

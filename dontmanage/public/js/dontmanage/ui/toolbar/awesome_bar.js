...
dontmanage.provide("dontmanage.tags");

dontmanage.search.AwesomeBar = class AwesomeBar {
	setup(element) {
	...
	setup_search_modal(element) {
		...
		let search_modal = new dontmanage.get_modal("Search", "");
		...
		let search_modal_body = `<div class="align-baseline flex py-2 px-1 relative navbar-modal-wrapper">
			<div class="modal-search-icon absolute pr-2 pl-2">${dontmanage.utils.icon("search")}</div>
			<input
				id="navbar-search"
				type="text"
		...
		let search_modal_footer = `<div class="awesomebar-modal-footer flex justify-between w-100">
			<div class="help-navigation">
				<span class="help-item-navigate">
					<span class="help-item">${dontmanage.utils.icon("arrow-up")}</span>
					<span class="help-item">${dontmanage.utils.icon("arrow-down")}</span>
					<span>${__("to navigate")}</span>
				</span>
				<span class="help-item-navigate">
					<span class="help-item">${dontmanage.utils.icon("corner-down-left")}</span>
					<span>${__("to select")}</span>
				</span>
				<span class="help-item help-item-esc">${__("esc")}</span>
				<span>${__("to close")}</span>
			</div>
			<div class="pointer">${dontmanage.utils.icon("circle-question-mark")}</div>
		</div>`;
	...
	setup_event_listeners(search_modal) {
		...
		var awesomplete = new Awesomplete(input, {
			...
			item: function (item, term) {
				...
				let target = "#";
				if (d.route) {
					target = dontmanage.router.make_url(
						dontmanage.router.convert_from_standard_route(
							dontmanage.router.get_route_from_arguments(
				...
				if (d.type == "Desktop Icon") {
					target = dontmanage.utils.get_route_for_icon(d.icon_data);
					d.route = target;
					d.route_options = {
		...
		$input.on(
			"input",
			dontmanage.utils.debounce(function (e) {
				...
				if (txt && txt.length > 1) {
					...
					me.options = me.options.concat(
						me.deduplicate(dontmanage.search.utils.get_recent_pages(txt || ""))
					);
					me.options = me.options.concat(dontmanage.search.utils.get_frequent_links());
				}
		...
		$input.on("awesomplete-select", function (e) {
			...
			if (item.route_options) {
				dontmanage.route_options = item.route_options;
			}
	...
	show_help() {
		const txt =
			...
			__("Open in new tab") +
			"</td><td>" +
			(dontmanage.utils.is_mac() ? "⌘ + Enter" : "Ctrl + Enter") +
			"</td></tr>\
			<tr><td>" +
			__("Calculate") +
			"</td><td>" +
		...
		dontmanage.msgprint(txt, __("Search Help"));
	}
	...
	build_options(txt) {
		var options = dontmanage.search.utils
			.get_creatables(txt)
			.concat(
				dontmanage.search.utils.get_search_in_list(txt),
				dontmanage.search.utils.get_doctypes(txt),
				dontmanage.search.utils.get_reports(txt),
				dontmanage.search.utils.get_pages(txt),
				dontmanage.search.utils.get_desktop_icons(txt),
	...
	make_global_search(txt) {
		...
		// 		<span class="ellipsis">Search for ${dontmanage.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	dontmanage.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${__("Search for {0}", [dontmanage.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
			value: __("Search for {0}", [dontmanage.utils.xss_sanitise(txt)]),
			match: txt,
			index: 100,
			default: "Search",
			onclick: function () {
		...
		if (route[0] === "List" && txt.indexOf(" in") === -1) {
			...
			this.options.push({
				label: __("Find {0} in {1}", [
					dontmanage.utils.xss_sanitise(txt).bold(),
					__(route[1]).bold(),
				]),
				value: __("Find {0} in {1}", [dontmanage.utils.xss_sanitise(txt), __(route[1])]),
		...
		if (first == parseInt(first) || first === "(" || first === "=") {
			...
			try {
				...
				var formatted_value = __("{0} = {1}", [
					dontmanage.utils.xss_sanitise(txt),
					(rounded_val + "").bold(),
				]);
				this.options.push({
					label: formatted_value,
					value: __("{0} = {1}", [dontmanage.utils.xss_sanitise(txt), rounded_val]),
					match: rounded_val,
					index: 80,
					default: "Calculator",
					onclick: function () {
						dontmanage.msgprint(formatted_value, __("Result"));
					},
				});
			} catch (e) {
				// pass
	...
	make_random(txt) {
		if (txt.toLowerCase().includes("random")) {
			this.options.push({
				...
				value: dontmanage.utils.get_random(16),
				onclick: function () {
					dontmanage.msgprint(dontmanage.utils.get_random(16), __("Result"));
				},
			});
		}
	}

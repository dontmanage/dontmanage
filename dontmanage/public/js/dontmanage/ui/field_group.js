...
dontmanage.ui.FieldGroup = class FieldGroup extends dontmanage.ui.form.Layout {
	constructor(opts) {
		super(opts);
		this.dirty = false;
		this.fetch_dict = {};
	...
	make() {
		...
		if (this.fields) {
			...
			$.each(this.fields_list, function (i, field) {
				...
				if (
					def_value == null ||
					(!def_value && !dontmanage.model.is_numeric_field(field.df.fieldtype))
				)
					return;

				if (def_value == "Today" && field.df["fieldtype"] == "Date") {
					def_value = dontmanage.datetime.get_today();
				}

...
dontmanage.ui.Tags = class {
	...
	bind() {
		...
		const select_tag = function () {
			const tagValue = dontmanage.utils.xss_sanitise(me.$input.val());
			me.addTag(tagValue);
			me.$input.val("");
		};
	...
	removeTag(label) {
		label = dontmanage.utils.xss_sanitise(label);
	...
	get_tag(label) {
		let $tag = dontmanage.get_data_pill(label, label, (target, pill_wrapper) => {

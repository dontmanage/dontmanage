...
dontmanage.ui.color = {
	get: function (color_name, shade) {
		...
		return dontmanage.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(dontmanage.ui.color_map);
		if (color_names.includes(color_name)) {
			return dontmanage.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(dontmanage.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(dontmanage.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in dontmanage.ui.color_map) {
			const colors = dontmanage.ui.color_map[key];
...
dontmanage.ui.color_map = dontmanage.ui.color.get_color_map();

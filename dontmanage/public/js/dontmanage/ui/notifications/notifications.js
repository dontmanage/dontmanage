...
dontmanage.ui.Notifications = class Notifications {
	constructor(opts) {
		this.tabs = {};
		this.notification_settings = dontmanage.boot.notification_settings;
	...
	make() {
		...
		this.user = dontmanage.session.user;
	...
	setup_headers() {
		...
		$(`<span class="notification-settings" data-action="go_to_settings">
			${dontmanage.utils.icon("setting-gear")}
		</span>`)
			.on("click", (e) => {
				e.stopImmediatePropagation();
				console.log("what");
				dontmanage.set_route("Form", "Notification Settings", dontmanage.session.user);
			})
		...
		$(`<span class="mark-all-read" data-action="mark_all_as_read">
			${dontmanage.utils.icon("mark-as-read")}
		</span>`)
	...
	mark_all_as_read(e) {
		...
		dontmanage.call("dontmanage.desk.doctype.notification_log.notification_log.mark_all_as_read");
	}
...
dontmanage.ui.notifications = {
	get_notification_config() {
		return dontmanage.xcall("dontmanage.desk.notifications.get_notification_info").then((r) => {
			dontmanage.ui.notifications.config = r;
			return r;
		});
	},

	show_open_count_list(doctype) {
		if (!dontmanage.ui.notifications.config) {
	...
	route_to_list_with_filters(doctype) {
		let filters = dontmanage.ui.notifications.config["conditions"][doctype];
		if (filters && $.isPlainObject(filters)) {
			if (!dontmanage.route_options) {
				dontmanage.route_options = {};
			}
			$.extend(dontmanage.route_options, filters);
		}
		dontmanage.set_route("List", doctype);
	},
};
...
class NotificationsView extends BaseNotificationsView {
	make() {
		...
		this.get_notifications_list(this.max_length).then((r) => {
			...
			dontmanage.update_user_info(r.message.user_info);
	...
	mark_as_read(docname, $el) {
		dontmanage
			.call("dontmanage.desk.doctype.notification_log.notification_log.mark_as_read", {
				docname: docname,
			})
			.then(() => {
	...
	get_dropdown_item_html(notification_log) {
		...
		message = title
			? message.replace(title[1], dontmanage.ellipsis(strip_html(title[1]), 100))
			: message;

		let timestamp = dontmanage.datetime.comment_when(notification_log.creation);
		...
		let user_avatar = dontmanage.avatar(user, "avatar-medium user-avatar");
			...
			if (this.dropdown_items.length) {
				...
				this.container.append(
					...
					<div class="text-center">
						<img src="/assets/dontmanage/images/ui-states/notification-empty-state.svg" alt="Generic Empty State" class="null-state">
	...
	get_notifications_list(limit) {
		return dontmanage.call({
			method: "dontmanage.desk.doctype.notification_log.notification_log.get_notification_logs",
			args: { limit: limit },
			type: "GET",
			cache: true,
		});
	...
	get_item_link(notification_doc) {
		...
		return dontmanage.utils.get_form_link(link_doctype, link_docname);
	}
	...
	toggle_seen(flag) {
		dontmanage.call(
			"dontmanage.desk.doctype.notification_settings.notification_settings.set_seen_value",
			{
				value: cint(flag),
	...
	setup_notification_listeners() {
		dontmanage.realtime.on("notification", () => {
			this.toggle_notification_icon(false);
			this.update_dropdown();
		});

		dontmanage.realtime.on("indicator_hide", () => {
		...
		this.parent.on("show.bs.dropdown", () => {
			...
			if (this.notifications_icon.find(".notifications-unseen").is(":visible")) {
				this.toggle_notification_icon(true);
				dontmanage.call(
					"dontmanage.desk.doctype.notification_log.notification_log.trigger_indicator_hide"
				);
			}
		});
	}
...
class EventsView extends BaseNotificationsView {
	make() {
		...
		dontmanage
			.xcall(
				"dontmanage.desk.doctype.event.event.get_events",
				{
					start: today,
					end: today,
				},
	...
	render_events_html(event_list) {
		...
		if (event_list.length) {
			let get_event_html = (event) => {
				let time = __("All Day");
				if (!event.all_day) {
					let start_time = dontmanage.datetime.get_time(event.starts_on);
					let days_diff = dontmanage.datetime.get_day_diff(event.ends_on, event.starts_on);
					let end_time = dontmanage.datetime.get_time(event.ends_on);
					if (days_diff > 1) {
				...
				if (event.particpants) {
					particpants = dontmanage.avatar_group(event.particpants, 3);
				}
			...
			html = `
				<div class="notification-null-state">
					<div class="text-center">
					<img src="/assets/dontmanage/images/ui-states/event-empty-state.svg" alt="Generic Empty State" class="null-state">
...
class ChangelogFeedView extends BaseNotificationsView {
	make() {
		this.render_changelog_feed_html(dontmanage.boot.changelog_feed || []);
	}
	...
	render_changelog_feed_html(changelog_feed) {
		...
		if (changelog_feed.length) {
			...
			const get_changelog_feed_html = (changelog_feed_item) => {
				const timestamp = dontmanage.datetime.prettyDate(

...
dontmanage.provide("dontmanage.ui.keys.handlers");

dontmanage.ui.keys.setup = function () {
	$(window).on("keydown", function (e) {
		var key = dontmanage.ui.keys.get_key(e);
		if (dontmanage.ui.keys.handlers[key]) {
			var out = null;
			for (var i = 0, l = dontmanage.ui.keys.handlers[key].length; i < l; i++) {
				var handler = dontmanage.ui.keys.handlers[key][i];
...
dontmanage.ui.keys.standard_shortcuts = standard_shortcuts;
dontmanage.ui.keys.add_shortcut = ({
	...
	dontmanage.ui.keys.off(shortcut, page);
	// attach new handler
	dontmanage.ui.keys.on(shortcut, handler);
...
dontmanage.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (dontmanage.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map((shortcut) => {
				let shortcut_label = shortcut.shortcut
					.split("+")
					.map(dontmanage.utils.to_title_case)
					.join("+");
	...
	let dialog = new dontmanage.ui.Dialog({
		...
		on_hide() {
			dontmanage.ui.keys.is_dialog_shown = false;
		},
	});
	...
	dialog.show();
	dontmanage.ui.keys.is_dialog_shown = true;
};

dontmanage.ui.keys.get_key = function (e) {
	var keycode = e.keyCode || e.which;
	var key = dontmanage.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
dontmanage.ui.keys.on = function (key, handler) {
	if (!dontmanage.ui.keys.handlers[key]) {
		dontmanage.ui.keys.handlers[key] = [];
	}
	dontmanage.ui.keys.handlers[key].push(handler);
};

dontmanage.ui.keys.off = function (key, page) {
	let handlers = dontmanage.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	dontmanage.ui.keys.handlers[key] = handlers.filter((h) => {
...
dontmanage.ui.keys.add_shortcut({
	...
	action: function (e) {
		...
		dontmanage.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
dontmanage.ui.keys.on("escape", function (e) {
	handle_escape_key();
});

dontmanage.ui.keys.on("esc", function (e) {
	handle_escape_key();
});

dontmanage.ui.keys.on("enter", function (e) {
	if (window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger("click");
	}
});

dontmanage.ui.keys.on("ctrl+down", function (e) {
	const grid_row = dontmanage.ui.form.get_open_grid_form();
...
dontmanage.ui.keys.on("ctrl+up", function (e) {
	const grid_row = dontmanage.ui.form.get_open_grid_form();
...
"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter, i) => {
	dontmanage.ui.keys.key_map[65 + i] = letter;
});

// keyCode map
dontmanage.ui.keyCode = {
	ESCAPE: 27,
	LEFT: 37,
	RIGHT: 39,
	UP: 38,
...
dontmanage.ui.keys.add_shortcut({
	shortcut: "shift+t",
	action: function (e) {
		if (!dontmanage.model.can_write("System Console")) {
			return;
		}

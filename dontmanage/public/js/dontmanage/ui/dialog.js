...
dontmanage.ui.open_dialogs = [];

dontmanage.ui.Dialog = class Dialog extends dontmanage.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		this.$wrapper = dontmanage.get_modal("", "");
		...
		this.$wrapper
			.on("hide.bs.modal", function () {
				...
				if (dontmanage.ui.open_dialogs[dontmanage.ui.open_dialogs.length - 1] === me) {
					...
					if (dontmanage.ui.open_dialogs.length) {
						window.cur_dialog =
							dontmanage.ui.open_dialogs[dontmanage.ui.open_dialogs.length - 1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function () {
				...
				dontmanage.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger("dontmanage.ui.Dialog:shown");
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(dontmanage.utils.icon(icon));

dontmanage.ui.SortSelector = class SortSelector {
	// parent:
	// change:
	// args:
	...
	make() {
		...
		this.wrapper = $(dontmanage.render_template("sort_selector", this.args)).appendTo(this.parent);
		this.bind_events();
	}
	bind_events() {
		var me = this;
		...
		this.wrapper.find(".btn-order").on("click", function () {
			...
			btn.find(".sort-order").html(dontmanage.utils.icon(icon_name, "sm"));
		...
		if (!this.args.options) {
			...
			meta.fields.forEach(function (df) {
				if (
					...
					dontmanage.perm.has_perm(me.doctype, df.permlevel, "read")
				) {
	...
	get_label(fieldname) {
		if (fieldname === "idx") {
			...
			return this.labels[fieldname] || dontmanage.meta.get_label(this.doctype, fieldname);
		}
	}
	get_sql_string() {

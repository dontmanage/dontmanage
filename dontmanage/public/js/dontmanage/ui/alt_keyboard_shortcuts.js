dontmanage.provide("dontmanage.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
dontmanage.ui.keys.shortcut_groups = shortcut_groups;

dontmanage.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new dontmanage.ui.keys.AltShortcutGroup());
	}
...
dontmanage.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (dontmanage.ui.keys.key_map[e.which] || "").toLowerCase();
...
dontmanage.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = dontmanage.ui.keys.standard_shortcuts
			.filter((s) => !s.page)

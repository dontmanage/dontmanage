...
dontmanage.ui.AddressAutocompleteDialog = class AddressAutocompleteDialog {
	constructor(opts) {
	...
	_get_dialog() {
		...
		const dialog = new dontmanage.ui.Dialog({
			...
			fields: [
				{
					...
					reqd: 1,
					get_query:
						"dontmanage.integrations.doctype.geolocation_settings.geolocation_settings.autocomplete",
					onchange: () => {
						// Disable "Create Address" button if mandatory fields are missing
						dontmanage.model.with_doctype("Address", () => {
							const address = this.parse_selected_value();
							const mandatory_fields = dontmanage
								.get_meta("Address")
								.fields.filter(
									(field) =>
										field.reqd &&
			...
			primary_action: () => {
				...
				dontmanage.db.insert(address).then((doc) => {
			...
			secondary_action: () => {
				...
				const address = this.parse_selected_value();
				dontmanage.new_doc("Address", address);
			},
		});

...
dontmanage.ui.menu = class ContextMenu {
	constructor(opts) {
		this.template = $(`<div class="dontmanage-menu context-menu" role="menu"></div>`);
		this.menu_items = opts.menu_items;
		this.name = dontmanage.utils.get_random(5);
	...
	add_menu_item(item) {
		...
		if (item?.is_divider) {
			...
			const iconMarkup = item.icon_url
				...
				: "";
			let chevron_direction = dontmanage.utils.is_rtl() ? "left " : "right";
			item_wrapper = $(`<div class="dropdown-menu-item" onclick="${
				...
				<a>
					...
					<div class="menu-item-icon" style="margin-left:auto">
						${item.items && item.items.length ? dontmanage.utils.icon(`chevron-${chevron_direction}`) : ""}
					</div>
	...
	handle_nested_menu(item_wrapper, item) {
		return dontmanage.ui.create_menu({
			parent: item_wrapper,
	...
	show(event) {
		...
		if (this.opts.nested && this.opts.parent_menu) {
			...
			let dropdown = dontmanage.menu_map[this.opts.parent_menu].template;
		...
		this.visible = true;
		dontmanage.visible_menus.push(this);
	}
...
dontmanage.menu_map = {};
dontmanage.visible_menus = [];

dontmanage.ui.create_menu = function (opts) {
	if (!opts.right_click) $(opts.parent).css("cursor", "pointer");
	let context_menu = new dontmanage.ui.menu(opts);

	dontmanage.menu_map[context_menu.name] = context_menu;

	$(document).on("click", function () {
		if (dontmanage.menu_map[context_menu.name].visible) {
			dontmanage.menu_map[context_menu.name].hide();
			opts.onHide && opts.onHide(opts.parent);
		}
	});

	$(document).on("keydown", function (e) {
		if (e.key === "Escape" && dontmanage.menu_map[context_menu.name].visible) {
			dontmanage.menu_map[context_menu.name].hide();
			opts.onHide && opts.onHide(opts.parent);
		}
	});
...
function close_all_open_menus() {
	dontmanage.visible_menus.forEach((menu) => {
		menu.hide();
	});
	dontmanage.visible_menus = [];
}

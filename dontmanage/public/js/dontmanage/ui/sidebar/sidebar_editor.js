export class SidebarEditor {
	...
	handle_route_change() {
		...
		dontmanage.router.on("change", function () {
			if (dontmanage.get_prev_route().length == 0) return;
			if (dontmanage.get_prev_route().length !== dontmanage.get_route().length && me.edit_mode) {
				me.stop();
			}
		});
	}
	...
	setup_editing_controls() {
		...
		this.save_sidebar_button.on("click", async function (event) {
			...
			me.prepare_data();
			await dontmanage.call({
				type: "POST",
				method: "dontmanage.desk.doctype.workspace_sidebar.workspace_sidebar.add_sidebar_items",
				args: {
					sidebar_title: me.workspace_title || me.sidebar.sidebar_title,
					sidebar_items: me.new_sidebar_items,
				},
				callback: function (r) {
					dontmanage.boot.workspace_sidebar_item[r.message.name.toLowerCase()] = [
	...
	get_item_obj(item_data) {
		return dontmanage.app.sidebar.items.find((item) => {
	...
	make_dialog(opts) {
		...
		let d;
		this.dialog = d = new dontmanage.ui.Dialog({
			title: title,

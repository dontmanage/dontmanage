...
dontmanage.ui.Sidebar = class Sidebar {
	constructor() {
		...
		this.all_sidebar_items = dontmanage.boot.workspace_sidebar_item;
		this.$items = [];
	...
	prepare() {
		try {
			...
			this.sidebar_data = dontmanage.boot.workspace_sidebar_item[this.workspace_title];
	...
	build_sidebar_module_map() {
		for (const [key, value] of Object.entries(dontmanage.boot.workspace_sidebar_item)) {
	...
	choose_app_name() {
		if (dontmanage.boot.app_name_style === "Default") return;

		for (const app of dontmanage.boot.app_data) {
			if (
				app.workspaces.includes(this.sidebar_title) ||
				(dontmanage.boot.workspace_sidebar_item[this.workspace_title] &&
					app.app_name == dontmanage.boot.workspace_sidebar_item[this.workspace_title].app)
			) {
				this.header_subtitle = app.app_title;
				dontmanage.current_app = app;
				this.app_logo_url = app.app_logo_url;
				return;
			}
		}

		const icon = dontmanage.boot.desktop_icons.find((i) => i.label === this.sidebar_title);
		if (icon) {
			this.header_subtitle = icon.parent_icon;
		}

		if (this.sidebar_title == "My Workspaces") {
			this.header_subtitle = dontmanage.session.user;
		}
	}
	...
	setup(workspace_title) {
		...
		this.sidebar_header = new dontmanage.ui.SidebarHeader(this);
		this.make_sidebar();
	}
	...
	setup_events() {
		const me = this;
		dontmanage.router.on("change", function (router) {
			if (dontmanage.route_options.sidebar) {
				dontmanage.app.sidebar.setup(dontmanage.route_options.sidebar);
			} else {
				dontmanage.app.sidebar.set_workspace_sidebar(router);
			}
		});
		$(document).on("page-change", function () {
			dontmanage.app.sidebar.toggle();
		});
	...
	toggle() {
		if (!dontmanage.container.page.page) return;
		if (dontmanage.container.page.page.hide_sidebar) {
			this.wrapper.hide();
		} else {
			this.wrapper.show();
	...
	make_dom() {
		...
		this.wrapper = $(
			dontmanage.render_template("sidebar", {
	...
	make_sidebar() {
		...
		this.wrapper.find(".selected").length &&
			!dontmanage.dom.is_element_in_viewport(this.wrapper.find(".selected")) &&
	...
	create_sidebar(items) {
		...
		if (items && items.length > 0) {
			items.forEach((w) => {
				if (!w.display_depends_on || dontmanage.utils.eval(w.display_depends_on)) {
	...
	get_workspace_for_module(module) {
		for (let i = 0; i < dontmanage.boot.workspaces.pages.length; i++) {
			const workspace = dontmanage.boot.workspaces.pages[i];
	...
	setup_notifications() {
		if (dontmanage.boot.desk_settings.notifications && dontmanage.session.user !== "Guest") {
			this.notifications = new dontmanage.ui.Notifications({ full_height: true });
		}
	}
	...
	make_sidebar_item(opts) {
		let class_name = `Type${dontmanage.utils.to_title_case(opts.item.type).replace(/ /g, "")}`;

		return new dontmanage.ui.sidebar_item[class_name](opts);
	}
	...
	close() {
		...
		this.expand_sidebar();
		if (dontmanage.is_mobile()) dontmanage.app.sidebar.prevent_scroll();
	}
	open() {
		this.sidebar_expanded = true;
		this.expand_sidebar();
		...
		try {
			...
			switch (route.length) {
				...
				case 2:
					...
					if (dontmanage.boot.workspace_sidebar_item[entity_name.toLowerCase()]) {
						dontmanage.app.sidebar.setup(entity_name);
						return;
					}
					break;
				case 3:
			...
			if (module) {
				sidebars = this.filter_sidebars_from_app(
					sidebars,
					dontmanage.boot.module_app[module.toLowerCase()]
				);
			}
			if (sidebars.length == 1) {
				dontmanage.app.sidebar.setup(sidebars[0]);
				...
				if (sidebars.includes(this.get_workspace_for_module(module))) {
					dontmanage.app.sidebar.setup(sidebar);
				}
			} else if (module) {
	...
	filter_sidebars_from_app(sidebars, app) {
		...
		sidebars.forEach((sidebar) => {
			if (
				...
				dontmanage.boot.workspace_sidebar_item[sidebar.toLowerCase()].app === app
			) {
	...
	show_sidebar_for_module(module) {
		...
		if (dontmanage.boot.workspace_sidebar_item[module.toLowerCase()]) {
			dontmanage.app.sidebar.setup(module);
		} else if (
			workspace_name &&
			dontmanage.boot.workspace_sidebar_item[workspace_name.toLowerCase()]
		) {
			dontmanage.app.sidebar.setup(workspace_name);
		} else {
			let sidebars =
	...
	set_sidebar_for_page() {
		...
		if (module_name) {
			dontmanage.app.sidebar.setup(module_name || this.sidebar_title);
		}
	}

dontmanage.provide("dontmanage.ui.sidebar_item");
dontmanage.ui.sidebar_item.TypeLink = class SidebarItem {
	constructor(opts) {
		this.item = opts.item;
		...
		this.workspace_title =
			...
			dontmanage.app.sidebar.sidebar_title;
		this.prepare(opts);
		this.make();
	}
	get_path() {
		...
		if (this.item.type === "Link") {
			if (this.item.link_type === "Report") {
				...
				if (!dontmanage.app.sidebar.editor.edit_mode) {
					if (this.item.report) {
				...
				path = dontmanage.utils.generate_route(args);
			} else if (this.item.link_type == "Workspace") {
				let workspaces = dontmanage.workspaces[dontmanage.router.slug(this.item.link_to)];
				if (workspaces.public) {
					path = "/desk/" + dontmanage.router.slug(this.item.link_to);
				} else {
					path = "/desk/private/" + dontmanage.router.slug(workspaces.title);
				}
				...
				path = dontmanage.utils.generate_route({
					type: this.item.link_type,
					name: this.item.link_to,
				...
				if (this.item.filters) {
					let filters_json = JSON.parse(
						dontmanage.utils.get_filter_as_json(JSON.parse(this.item.filters))
					);
	...
	make() {
		...
		this.wrapper = $(
			dontmanage.render_template("sidebar_item", {
				item: this.item,
				path: this.path,
				edit_mode: dontmanage.app.sidebar.editor.edit_mode,
			})
		);
		...
		let menu_items = [
			{
				...
				icon: "pen",
				onClick: () => {
					dontmanage.app.sidebar.editor.perform_action("edit", me.item);
				},
			},
			{
				...
				icon: "add",
				onClick: () => {
					dontmanage.app.sidebar.editor.perform_action("add_below", me.item);
				},
			},
			{
				label: "Duplicate",
				icon: "copy",
				onClick: () => {
					dontmanage.app.sidebar.editor.perform_action("duplicate", me.item);
				},
			},
			{
				label: "Delete",
				...
				onClick: () => {
					console.log(me.item);
					dontmanage.app.sidebar.editor.perform_action("delete", me.item);
				},
			},
		];
		return menu_items;
...
dontmanage.ui.sidebar_item.TypeSectionBreak = class SectionBreakSidebarItem extends (
	dontmanage.ui.sidebar_item.TypeLink
) {
	prepare(opts) {
	...
	add_items() {
		...
		this.nested_items.forEach((f) => {
			this.items.push(
				dontmanage.app.sidebar.make_sidebar_item({
	...
	enable_collapsible(item, $item_container) {
		...
		if (item.collapsible) {
			...
			this.$drop_icon = $(`<button class="btn-reset drop-icon hidden">`)
				.html(dontmanage.utils.icon(drop_icon, "sm", "", "", "", "", stroke_color))
	...
	setup_event_listner() {
		...
		$(this.wrapper.find(".standard-sidebar-item")[0]).on("click", (e) => {
			...
			if (!dontmanage.app.sidebar.sidebar_expanded) {
				dontmanage.app.sidebar.open();
				this.open();
			}
		});
	}
	...
	get_menu_items() {
		...
		let menu_items = [
			...
			},
			{
				...
				icon: "add",
				onClick: () => {
					dontmanage.app.sidebar.editor.show_new_dialog({
						nested: true,
						parent_item: me.item,
					});
				},
...
dontmanage.ui.sidebar_item.TypeSpacer = class SpacerItem extends dontmanage.ui.sidebar_item.TypeLink {
	constructor(item, items) {
		super(item);
	}
};

dontmanage.ui.sidebar_item.TypeSidebarItemGroup = class SpacerItem extends (
	dontmanage.ui.sidebar_item.TypeLink
) {
	constructor(item, items) {
		super(item);
		this.title = dontmanage.app.sidebar.workspace_title;
		this.setup_click();
	}
	...
	setup_click() {
		...
		this.wrapper.on("click", function () {
			dontmanage.call({
				method: "dontmanage.desk.doctype.sidebar_item_group.sidebar_item_group.get_reports",
				args: { module_name: dontmanage.app.sidebar.workspace_title },
				callback: function (r) {
					if (r.message) {
						let links_html = "";
						...
						r.message.forEach((report) => {
							...
							links_html += `<a href="${encodeURI(
								dontmanage.utils.generate_route(args)
							)}">${report.title}</a><br>`;
						});
...
dontmanage.ui.sidebar_item.TypeButton = class SidebarButton extends dontmanage.ui.sidebar_item.TypeLink {
	constructor(item) {
		super(item);
		this.title = dontmanage.app.sidebar.workspace_title;

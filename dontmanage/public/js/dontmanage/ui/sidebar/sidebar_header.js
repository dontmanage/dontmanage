dontmanage.ui.SidebarHeader = class SidebarHeader {
	constructor(sidebar) {
		...
		this.dropdown_items = [
			...
			},
			{
				...
				icon: "edit",
				condition: function () {
					return dontmanage.boot.developer_mode;
				},
				onClick: function () {
		...
		if (dontmanage.boot.desk_settings.notifications) {
			let is_dark = dontmanage.ui.get_current_theme() === "dark";
			this.dropdown_items.push(
				...
				},
				{
					...
					action: "dontmanage.ui.toolbar.setup_session_defaults()",
					is_standard: 1,
					icon: "sliders-horizontal",
				},
				{
					label: "Reload",
					action: "dontmanage.ui.toolbar.clear_cache()",
					is_standard: 1,
					icon: "rotate-ccw",
				},
				{
	...
	fetch_related_icons() {
		...
		if (dontmanage.current_app) {
			let desktop_icons = [...dontmanage.boot.desktop_icons];
			desktop_icons.splice(
				desktop_icons.indexOf(dontmanage.utils.get_desktop_icon_by_label(this.title)),
				1
			);
			...
			sibling_icons.forEach((icon) => {
				...
				if (!workspaces_not_to_show.includes(icon.label)) {
					let item = {
						...
						label: icon.label,
						url: dontmanage.utils.get_route_for_icon(icon),
					};
					...
					if (
						dontmanage.utils.get_desktop_icon(icon.label, dontmanage.boot.desktop_icon_style)
					) {
						item.icon_url = dontmanage.utils.get_desktop_icon(
							icon.label,
							dontmanage.boot.desktop_icon_style
						);
					} else {
						item.icon_html = dontmanage.utils.desktop_icon(icon.label, "gray", "sm");
					}
	...
	get_icon_for_menu_item(icon, item) {
		if (dontmanage.utils.get_desktop_icon(icon.label, dontmanage.boot.desktop_icon_style)) {
			item.icon_url = dontmanage.utils.get_desktop_icon(
				icon.label,
				dontmanage.boot.desktop_icon_style
			);
	...
	build_folder_map(desktop_icons) {
		const folder_map = {};
		const sibling_icons = [];
		if (!dontmanage.current_app) return;
		this.sort_icons(desktop_icons);
		desktop_icons.forEach((icon) => {
			if (
				icon.link_type != "External" &&
				icon.app == dontmanage.current_app.app_name &&
				!icon.hidden
			) {
				...
				if (icon.parent_icon) {
					icon.url = dontmanage.utils.get_route_for_icon(icon);
	...
	make() {
		...
		$(
			dontmanage.render_template("sidebar_header", {
	...
	set_header_icon() {
		...
		let desktop_icon_url =
			desktop_icon && dontmanage.utils.get_desktop_icon(desktop_icon.label, "solid");
		if (desktop_icon_url) {
			...
			this.header_icon = dontmanage.utils.desktop_icon(this.sidebar.sidebar_title, "gray", "sm");
		} else {
	...
	get_default_icon() {
		return dontmanage.boot.app_data[0].app_logo_url;
	}
	get_desktop_icon_by_label(title, filters) {
		if (!filters) {
			return dontmanage.boot.desktop_icons.find((f) => f.label === title && f.hidden != 1);
		} else {
		...
		$(`<div class="dropdown-menu-item" data-name="${item.name}"
			...
			<a ${item.href ? `href="${item.href}"` : ""}>
				<div class="sidebar-item-icon">
					${
						item.icon
							? dontmanage.utils.icon(item.icon)
							: `<img
							class="logo"
							src="${item.icon_url}"
						>`

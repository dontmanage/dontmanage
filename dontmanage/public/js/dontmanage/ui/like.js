...
dontmanage.ui.is_liked = function (doc) {
	return dontmanage.ui.get_liked_by(doc).includes(dontmanage.session.user);
};
...
dontmanage.ui.toggle_like = function ($btn, doctype, name, callback) {
	...
	dontmanage.call({
		...
		callback: function (r) {
			...
			if (doc) {
				let liked_by = dontmanage.ui.get_liked_by(doc);

				if (add === "Yes" && !liked_by.includes(dontmanage.session.user)) {
					liked_by.push(dontmanage.session.user);
				}

				if (add === "No" && liked_by.includes(dontmanage.session.user)) {
					liked_by = liked_by.filter((user) => user !== dontmanage.session.user);
				}
...
dontmanage.ui.click_toggle_like = function () {
	console.warn("`dontmanage.ui.click_toggle_like` is deprecated and has no effect.");
};

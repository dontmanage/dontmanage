...
export default {
	...
	methods: {
		...
		refresh: function() {
			dontmanage.call("dontmanage.recorder.get").then( r => this.requests = r.message);
		},
		update: function(message) {
			this.requests.push(JSON.parse(message));
		},
		clear: function() {
			dontmanage.call("dontmanage.recorder.delete").then(r => this.refresh());
		},
		start: function() {
			dontmanage.call("dontmanage.recorder.start").then(r => this.fetch_status());
		},
		stop: function() {
			dontmanage.call("dontmanage.recorder.stop").then(r => this.fetch_status());
		},
		fetch_status: function() {
			dontmanage.call("dontmanage.recorder.status").then(r => this.update_status(r.message));
		},
		update_status: function(result) {
			...
			if(this.status.status == "Active") {
				dontmanage.realtime.on("recorder-dump-event", this.update);
			} else {
				dontmanage.realtime.off("recorder-dump-event", this.update);
			}
		...
		export_data: function() {
			...
			dontmanage.call("dontmanage.recorder.export_data")
				.then((r) => {

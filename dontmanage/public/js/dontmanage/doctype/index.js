...
*/
dontmanage.model.DocTypeController = class DocTypeController extends dontmanage.ui.form.Controller {
	setup() {
		// setup formatters for fieldtype
		dontmanage.meta.docfield_map[
	...
	show_db_utilization() {
		...
		dontmanage
			.xcall("dontmanage.core.doctype.doctype.doctype.get_row_size_utilization", {
				doctype,
			})
			.then((r) => {
				if (r < 50.0) return;
	...
	setup_fetch_from_fields(doc, doctype, docname) {
		...
		let row = dontmanage.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			dontmanage.model.with_doctype(link_doctype, () => {
				let fields = dontmanage.meta

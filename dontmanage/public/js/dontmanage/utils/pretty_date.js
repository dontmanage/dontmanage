function prettyDate(date, mini) {
	...
	if (typeof date == "string") {
		date = dontmanage.datetime.convert_to_user_tz(date);
		date = new Date(
			(date || "")
				.replace(/-/g, "/")
				.replace(/[TZ]/g, " ")
	...
	let diff = (new Date(dontmanage.datetime.now_datetime()).getTime() - date.getTime()) / 1000;
...
window.comment_when = function (datetime, mini) {
	var timestamp = dontmanage.datetime.str_to_user ? dontmanage.datetime.str_to_user(datetime) : datetime;
	return (
		'<span class="dontmanage-timestamp ' +
		(mini ? " mini" : "") +
		'" data-timestamp="' +
		datetime +
		'" title="' +
...
dontmanage.datetime.comment_when = comment_when;
dontmanage.datetime.prettyDate = prettyDate;

dontmanage.datetime.refresh_when = function () {
	if (jQuery) {
		$(".dontmanage-timestamp").each(function () {

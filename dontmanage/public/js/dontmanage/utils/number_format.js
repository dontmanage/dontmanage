...
window.format_number = function (v, format, decimals) {
	if (!format) {
		...
		if (decimals == null) decimals = cint(dontmanage.defaults.get_default("float_precision")) || 3;
	}
...
function format_currency(v, currency, decimals) {
	...
	const show_symbol_on_right =
		dontmanage.model.get_value(":Currency", currency, "symbol_on_right") ?? false;

	if (decimals === undefined) {
		decimals = dontmanage.boot.sysdefaults.currency_precision || null;
	}
...
function get_currency_symbol(currency) {
	if (dontmanage.boot) {
		if (dontmanage.boot.sysdefaults && dontmanage.boot.sysdefaults.hide_currency_symbol == "Yes")
			return null;

		if (!currency) currency = dontmanage.boot.sysdefaults.currency;

		return dontmanage.model.get_value(":Currency", currency, "symbol") || currency;
	} else {
		// load in template
		return dontmanage.currency_symbols[currency];
	}
}

function get_number_format(currency) {
	return (
		(dontmanage.boot && dontmanage.boot.sysdefaults && dontmanage.boot.sysdefaults.number_format) ||
		"#,###.##"
	);
}

function get_number_format_info(format) {
	var info = dontmanage.number_format_info[format];
...
function _round(num, precision, rounding_method) {
	rounding_method =
		rounding_method || dontmanage.boot.sysdefaults.rounding_method || "Banker's Rounding (legacy)";
...
function precision(fieldname, doc) {
	if (cur_frm) {
		...
		var df = dontmanage.meta.get_docfield(doc.doctype, fieldname, doc.parent || doc.name);
		if (!df) console.log(fieldname + ": could not find docfield in method precision()");
		return dontmanage.meta.get_field_precision(df, doc);
	} else {
		return dontmanage.boot.sysdefaults.float_precision;
	}
}
...
function round_based_on_smallest_currency_fraction(value, currency, precision) {
	var smallest_currency_fraction_value = flt(
		dontmanage.model.get_value(":Currency", currency, "smallest_currency_fraction_value")
	);

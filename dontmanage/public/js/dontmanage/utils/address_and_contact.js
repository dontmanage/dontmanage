dontmanage.provide("dontmanage.contacts");

$.extend(dontmanage.contacts, {
	...
	render_address_and_contact: function (frm) {
		...
		for (const item of items) {
			...
			if (field_wrapper && frm.doc.__onload && item.data in frm.doc.__onload) {
				$(field_wrapper)
					.html(dontmanage.render_template(item.template, frm.doc.__onload))
					.find(item.btn)
	...
	get_last_doc: function (frm) {
		const reverse_routes = dontmanage.route_history.slice().reverse();
...
function new_record(doctype, frm) {
	...
	if (dontmanage.boot.enable_address_autocompletion === 1 && doctype === "Address") {
		new dontmanage.ui.AddressAutocompleteDialog({

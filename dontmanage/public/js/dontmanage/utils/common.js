...
dontmanage.avatar = function (
	...
	if (user) {
		// desk
		user_info = dontmanage.user_info(user);
	} else {
		// website
		let full_name = title || dontmanage.get_cookie("full_name");
		user_info = {
			image: image_url === null ? dontmanage.get_cookie("user_image") : image_url,
			fullname: full_name,
			abbr: dontmanage.get_abbr(full_name),
			color: dontmanage.get_palette(full_name),
		};
	}
...
dontmanage.get_avatar = function (css_class, title, image_url = null, remove_color, data_attributes) {
	if (!css_class) {
	...
	if (image_url) {
		...
		let style = "";
		if (!remove_color) {
			let color = dontmanage.get_palette(title);
...
dontmanage.avatar_group = function (users, limit = 4, options = {}) {
	...
	let html = display_users
		.map((user) =>
			dontmanage.avatar(user, "avatar-small " + css_class, null, null, false, options.filterable)
		)
		.join("");
	if (extra_users.length === 1) {
		html += dontmanage.avatar(
			extra_users[0],
		...
		html = `
			...
			<span class="avatar avatar-small ${css_class}">
				<div class="avatar-frame standard-image avatar-extra-count"
					title="${extra_users.map((u) => dontmanage.user_info(u).fullname).join(", ")}">
					+${extra_users.length}
				</div>
			</span>
		`;
		...
		avatar_action_html = `
			<span class="avatar avatar-small">
				<div class="avatar-frame avatar-action">
					${dontmanage.utils.icon(options.action_icon, "sm")}
				</div>
			</span>
		`;
	}
...
dontmanage.ui.scroll = function (element, animate, additional_offset) {
...
function process_palette() {
	dontmanage.palette.forEach((color, index) => {
		let color_name = color[0].split("-")[2];
		dontmanage.palette_map[color_name] = index;
	});
}
dontmanage.palette_map = {};
process_palette();
...
dontmanage.get_abbr = function (txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
dontmanage.gravatars = {};
dontmanage.get_gravatar = function (email_id, size = 0) {
	var param = size ? "s=" + size : "d=retro";
	if (!dontmanage.gravatars[email_id]) {
		// TODO: check if gravatar exists
		dontmanage.gravatars[email_id] =
			"https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return dontmanage.gravatars[email_id];
};
...
dontmanage.get_cookie = function getCookie(name) {
	return dontmanage.get_cookies()[name];
};

dontmanage.get_cookies = function getCookies() {
...
dontmanage.utils.sanitise_redirect = (url) => {
	const is_external = (() => {
		return (url) => {
			function domain(url) {
	...
	url = dontmanage.utils.strip_url(url);

	return is_external(url)
		? ""
		: sanitise_javascript(dontmanage.utils.xss_sanitise(url, { strategies: ["js"] }));
};

dontmanage.utils.strip_url = (url) => {
...
dontmanage.utils.new_auto_repeat_prompt = function (frm) {
	const fields = [
		{
			fieldname: "frequency",
			fieldtype: "Select",
	...
	dontmanage.prompt(
		...
		function (values) {
			dontmanage.call({
				method: "dontmanage.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
				args: {
...
dontmanage.utils.get_page_view_count = function (route) {
	return dontmanage.call("dontmanage.website.doctype.web_page_view.web_page_view.get_page_view_count", {
		path: route,
	});
};

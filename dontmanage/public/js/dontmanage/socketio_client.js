...
class RealTimeClient {
	...
	get_host(port = 9000) {
		...
		if (window.dev_server) {
			...
			port = dontmanage.boot.socketio_port || port.toString() || "9000";
			if (parts.length > 2) {
				host = parts[0] + ":" + parts[1];
			}
			host = host + ":" + port;
		}
		return host + `/${dontmanage.boot.sitename}`;
	}
...
dontmanage.realtime = new RealTimeClient();

// backward compatibility
dontmanage.socketio = dontmanage.realtime;

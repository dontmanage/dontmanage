				...
				<div class="mt-2 text-center">
					...
					<button v-if="google_drive_settings.enabled" class="btn btn-file-upload" @click="show_google_drive_picker">
						<svg width="30" height="30">
							<image href="/assets/dontmanage/icons/social/google_drive.svg" width="30" height="30"/>
						</svg>
...
export default {
	...
	created() {
		...
		if (this.restrictions.max_file_size == null) {
			dontmanage.call('dontmanage.core.api.file.get_max_file_size')
				.then(res => {
					this.restrictions.max_file_size = Number(res.message);
				});
		}
		if (this.restrictions.max_number_of_files == null && this.doctype) {
			this.restrictions.max_number_of_files = dontmanage.get_meta(this.doctype)?.max_attachments;
		}
	},
	watch: {
	...
	methods: {
		...
		return_as_dataurl() {
			let promises = this.files.map(file =>
				dontmanage.dom.file_to_base64(file.file_obj)
					.then(dataurl => {
		...
		capture_image() {
			...
			capture.submit(data_urls => {
				data_urls.forEach(data_url => {
					let filename = `capture_${dontmanage.datetime.now_datetime().replaceAll(/[: -]/g, '_')}.png`;

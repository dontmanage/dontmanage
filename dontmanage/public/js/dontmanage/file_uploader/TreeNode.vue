	...
	<div class="tree-node" :class="{ opened: node.open }">
		...
		>
			...
			<a
				...
				v-html="dontmanage.utils.icon('external-link', 'sm')"
				@click.stop
			/>
		</span>
		<div v-if="node.file_url && dontmanage.utils.is_image_file(node.file_url)">
...
let icon = computed(() => {
	let icons = {
		open: dontmanage.utils.icon("folder-open", "md"),
		closed: dontmanage.utils.icon("folder-normal", "md"),
		leaf: dontmanage.utils.icon("primitive-dot", "xs"),
		search: dontmanage.utils.icon("search"),
	};
...
let open_file = (filename) => {
	return dontmanage.utils.get_form_link("File", filename);
};

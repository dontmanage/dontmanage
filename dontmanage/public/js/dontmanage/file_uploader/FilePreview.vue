<template>
	<div class="file-preview">
		<div class="file-icon">
			...
			<div class="fallback" v-else v-html="dontmanage.utils.icon('file', 'md')"></div>
		</div>
		<div>
			<div>
			...
			<div class="flex config-area">
				<label v-if="is_optimizable" class="dontmanage-checkbox"
					><input
						type="checkbox"
						:checked="optimize"
					...
					/>{{ __("Optimize") }}</label
				>
				<label class="dontmanage-checkbox"
					><input
						type="checkbox"
		...
		<div class="file-actions">
			...
			<div class="file-action-buttons">
				<button
					...
					v-html="dontmanage.utils.icon('crop', 'md')"
				></button>
				<button
					v-if="!uploaded && !file.uploading && !file.failed"
					class="btn muted"
					@click="emit('remove')"
					v-html="dontmanage.utils.icon('delete', 'md')"
				></button>
			</div>
		</div>
	</div>
...
let file_size = computed(() => {
	return dontmanage.form.formatters.FileSize(props.file.file_obj.size);
});

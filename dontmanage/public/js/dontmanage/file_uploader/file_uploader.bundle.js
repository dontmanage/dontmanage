...
class FileUploader {
	...
	constructor({
		...
		allow_toggle_private = Boolean(
			allow_toggle_private && dontmanage.utils.can_upload_public_files()
		);
		...
		if (restrictions && !restrictions.allowed_file_types) {
			...
			let allowed_extensions = dontmanage.sys_defaults?.allowed_file_extensions;
	...
	make_dialog(title) {
		...
		this.dialog = new dontmanage.ui.Dialog(dialog_opts);
...
dontmanage.ui.FileUploader = FileUploader;

		...
		<div class="file-browser-list">
			<div class="file-filter">
				<input
					...
					v-model="search_text"
					@input="dontmanage.utils.debounce(search_by_name(), 300)"
				/>
			</div>
			<TreeNode
...
function get_files_in_folder(folder, start) {
	return dontmanage
		.call("dontmanage.core.api.file.get_files_in_folder", {
			folder,
			start,
...
function search_by_name() {
	...
	dontmanage
		.call("dontmanage.core.api.file.get_files_by_search_text", {
...
function make_file_node(file) {
	...
	let label = dontmanage.utils.file_name_ellipsis(filename, 40);
	return {
		label: label,
		filename: filename,

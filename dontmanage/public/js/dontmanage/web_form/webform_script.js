...
dontmanage.ready(function () {
	let web_form_doc = dontmanage.web_form_doc;
	let reference_doc = dontmanage.reference_doc;
	...
	function show_login_prompt() {
		if (dontmanage.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new dontmanage.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(dontmanage.form.formatters.FileSize($(this).text()));
		});
	}

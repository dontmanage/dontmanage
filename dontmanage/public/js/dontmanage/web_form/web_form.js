...
dontmanage.provide("dontmanage.web_form");
import EventEmitterMixin from "../../dontmanage/event_emitter";

export default class WebForm extends dontmanage.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		dontmanage.web_form = this;
		dontmanage.web_form.events = {};
		Object.assign(dontmanage.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		dontmanage.init_client_script && dontmanage.init_client_script();
		this.setup_listeners();
		dontmanage.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = dontmanage.utils.get_query_params();
		delete values.new;
	...
	delete_form() {
		...
		dontmanage.confirm(__("Are you sure you want to delete this record?"), () => {
			dontmanage.call({
				method: "dontmanage.website.doctype.web_form.web_form.delete",
				args: {
					web_form_name: this.name,
					docname: this.doc.name,
				},
				callback: () => {
					dontmanage.msgprint(__("Deleted!"));
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${dontmanage.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _page.find(".dontmanage-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		dontmanage.form_dirty = false;

		dontmanage.call({
			type: "POST",
			method: "dontmanage.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					dontmanage.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						dontmanage.call({
							type: "POST",
							method: "dontmanage.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			dontmanage.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {

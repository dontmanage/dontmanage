...
dontmanage.provide("dontmanage.web_form_list");

export default class WebFormList {
	constructor(opts) {
		Object.assign(this, opts);
		dontmanage.web_form_list = this;
	...
	make_filters() {
		...
		dontmanage
			.call("dontmanage.website.doctype.web_form.web_form.get_web_form_filters", {
			...
			.then((response) => {
				...
				fields.forEach((field) => {
					...
					let input = dontmanage.ui.form.make_control({
						df: {
	...
	fetch_data() {
		if (this.condition_json && JSON.parse(this.condition_json)) {
			let filter = dontmanage.utils.get_filter_from_json(this.condition_json, this.doctype);
			filter = dontmanage.utils.get_filter_as_json(filter);
			this.filters = Object.assign(this.filters, JSON.parse(filter));
		}

		let args = {
			method: "dontmanage.www.list.get_list_data",
			args: {
	...
	more() {
		...
		this.fetch_data().then((res) => {
			if (res.message.length === 0) {
				dontmanage.msgprint(__("No more items to display"));
			}
	...
	make_table_body() {
		if (this.data.length) {
			...
			this.wrapper.empty();
			dontmanage.has_permission(this.doctype, "", "create", () => {
		...
		let empty_state = $(`
			<div class="no-result text-muted flex justify-center align-center">
				<div class="text-center">
					<div>
						<img
							src="/assets/dontmanage/images/ui-states/list-empty-state.svg"
	...
	append_rows(row_data) {
		...
		row_data.forEach((data_item) => {
			...
			let row = new dontmanage.ui.WebFormListRow({
				row: $row_element,
				doc: data_item,
	...
	make_actions() {
		...
		dontmanage.has_permission(this.doctype, "", "delete", () => {
	...
	delete_rows() {
		...
		dontmanage
			.call({
				type: "POST",
				method: "dontmanage.website.doctype.web_form.web_form.delete_multiple",
				args: {
					web_form_name: this.web_form_name,
...
dontmanage.ui.WebFormListRow = class WebFormListRow {
	...
	make_row() {
		...
		this.columns.forEach((field) => {
			let formatter = dontmanage.form.get_formatter(field.fieldtype);
			let value =

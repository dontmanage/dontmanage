...
dontmanage.db = {
	get_list: function (doctype, args) {
		...
		return new Promise((resolve) => {
			dontmanage.call({
				method: "dontmanage.desk.reportview.get_list",
				args: args,
				type: "GET",
				callback: function (r) {
					resolve(r.message);
	...
	exists: function (doctype, nameOrFilters) {
		return new Promise((resolve) => {
			...
			if (typeof nameOrFilters === "string") {
				...
				dontmanage.db.get_value(doctype, { name: nameOrFilters }, "name").then((r) => {
					r.message && r.message.name ? resolve(true) : resolve(false);
				});
			} else if (typeof nameOrFilters === "object") {
				dontmanage.db.count(doctype, { filters: nameOrFilters, limit: 1 }).then((count) => {
					resolve(count > 0);
				});
			}
		});
	...
	get_single_value: (doctype, field) => {
		return new Promise((resolve) => {
			dontmanage
				.call({
					method: "dontmanage.client.get_single_value",
					args: { doctype, field },
					type: "GET",
				})
	...
	insert: function (doc) {
		return dontmanage.xcall("dontmanage.client.insert", { doc });
	},
	delete_doc: function (doctype, name) {
		return new Promise((resolve) => {
			dontmanage.call("dontmanage.client.delete", { doctype, name }, (r) => resolve(r.message));
		});
	},

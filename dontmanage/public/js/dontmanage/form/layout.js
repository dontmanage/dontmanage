...
dontmanage.ui.form.Layout = class Layout {
	constructor(opts) {
		this.views = {};
		this.pages = [];
		this.tabs = [];
	...
	get_doctype_fields() {
		...
		if (this.doctype_layout) {
			...
			fields = fields.concat(
				dontmanage.meta.sort_docfields(dontmanage.meta.docfield_map[this.doctype])
			);
		}
	...
	get_fields_from_layout() {
		...
		for (let f of this.doctype_layout.fields) {
			const docfield = copy_dict(dontmanage.meta.docfield_map[this.doctype][f.fieldname]);
	...
	show_message(html, color, permanent = false) {
		...
		// Prepare Block
		let $html;
		if (!dontmanage.utils.is_html(html)) {
		...
		const close_message = $(`<div class="close-message">${dontmanage.utils.icon("close")}</div>`);
		if (!permanent) {
	...
	make_page(df) {
		...
		this.fold_btn = head.find(".btn-fold").on("click", function () {
			...
			if (page.hasClass("hide")) {
				...
				dontmanage.utils.scroll_to($(this), true, 30);
				me.folded = false;
			} else {
	...
	is_numeric_field_active() {
		const control = $(document.activeElement).closest(".dontmanage-control");
	...
	attach_doc_and_docfields(refresh) {
		...
		for (let i = 0, l = this.fields_list.length; i < l; i++) {
			...
			if (me.doc) {
				...
				fieldobj.df =
					dontmanage.meta.get_docfield(me.doc.doctype, fieldobj.df.fieldname, me.doc.name) ||
					fieldobj.df;
			}
	...
	setup_events() {
		...
		this.tab_link_container.off("click").on("click", ".nav-link", (e) => {
			...
			if (tabs_content.getBoundingClientRect().top < 100) {
				...
				setTimeout(() => {
					if (dontmanage.boot.read_only || dontmanage.boot.user.impersonated_by) {
		...
		for (let i = start_idx + 1, len = fields.length; i < len; i++) {
			...
			if (this.is_visible(field)) {
				if (field.df.fieldtype === "Table") {
					...
					!dontmanage.model.no_value_type.includes(field.df.fieldtype)
				) {

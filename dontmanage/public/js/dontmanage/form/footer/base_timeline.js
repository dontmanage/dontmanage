...
class BaseTimeline {
	...
	add_action_button(label, action, icon = null, btn_class = null) {
		let icon_element = icon ? dontmanage.utils.icon(icon, "xs") : null;
	...
	get_timeline_item(item) {
		...
		if (item.icon) {
			timeline_item.append(`
				<div class="timeline-badge" title='${item.title || dontmanage.utils.to_title_case(item.icon)}'>
					${dontmanage.utils.icon(item.icon, item.icon_size || "md", item.icon_class || "")}
				</div>
			`);
		...
		timeline_item.append(
			`<div class="timeline-content ${item.is_card ? "dontmanage-card" : ""}">`
		);
		...
		if (!item.hide_timestamp && !item.is_card) {
			timeline_content.append(
				`<span> · ${
					cint(dontmanage.boot.user.show_absolute_datetime_in_timeline) ||
					cint(dontmanage.boot.sysdefaults.show_absolute_datetime_in_timeline)
						? dontmanage.datetime.str_to_user(item.creation)
...
dontmanage.ui.BaseTimeline = BaseTimeline;

...
function get_version_timeline_content(version_doc, frm) {
	...
	if (!$.isEmptyObject(updater_reference)) {
		...
		if (doctype && docname) {
			updater_reference_link = dontmanage.utils.get_form_link(doctype, docname, true, label);
		} else {
	...
	if (data.changed && data.changed.length) {
		...
		data.changed.every(function (p) {
			if (p[0] === "docstatus") {
				...
				const df = dontmanage.meta.get_docfield(frm.doctype, p[0], frm.docname);
				if (df && (!df.hidden || df.show_on_timeline)) {
					const field_display_status = dontmanage.perm.get_field_display_status(
						df,
						null,
						frm.perm
					);
	...
	if (data.row_changed && data.row_changed.length) {
		...
		data.row_changed.every(function (row) {
			row[3].every(function (p) {
				...
				if (df && (!df.hidden || df.show_on_timeline)) {
					var field_display_status = dontmanage.perm.get_field_display_status(
						df,
						null,
						frm.perm
					);
	...
	["added", "removed"].forEach(function (key) {
		if (data[key] && data[key].length) {
			let parts = (data[key] || []).map(function (p) {
				var df = dontmanage.meta.get_docfield(frm.doctype, p[0], frm.docname);
				...
				if (df && (!df.hidden || df.show_on_timeline)) {
					...
					if (
						...
						return __(dontmanage.meta.get_label(frm.doctype, p[0]));
					}
				}
			});
			...
			if (parts.length) {
				...
				out.push(get_version_comment(version_doc, dontmanage.utils.comma_and(messages)));
			}
		}
	});
...
function get_version_comment(version_doc, text) {
	...
	if (text.includes("<a")) {
		...
		try {
			...
			Array.from($(text)).forEach((element) => {
				if ($(element).is("a")) {
					version_comment += unlinked_content
						? dontmanage.utils.get_form_link(
								"Version",
								version_doc.name,
								true,
								unlinked_content
			...
			if (unlinked_content) {
				version_comment += dontmanage.utils.get_form_link(
					"Version",
					version_doc.name,
					true,
					unlinked_content
	...
	return dontmanage.utils.get_form_link("Version", version_doc.name, true, text);
}
...
function format_content_for_timeline(content) {
	...
	// escapes HTML
	// and makes it bold
	content = dontmanage.utils.html2text(content);
	content = dontmanage.ellipsis(content, 40) || '""';
	content = dontmanage.utils.escape_html(content);
	return content.bold();
}

function get_user_link(user) {
	const user_display_text = dontmanage.user_info(user).fullname || "";
	return dontmanage.utils.get_form_link(
		"User",
		user,
		true,
		dontmanage.utils.xss_sanitise(user_display_text)
	);
}

function get_user_message(user, message_self, message_other) {
	return dontmanage.utils.is_current_user(user) ? message_self : message_other;
}

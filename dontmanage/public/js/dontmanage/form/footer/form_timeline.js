...
class FormTimeline extends BaseTimeline {
	...
	setup_timeline_actions() {
		if (dontmanage.model.can_email(null, this.frm)) {
		...
		if (this.frm.meta.allow_events_in_timeline) {
			let create_event = () => {
				const args = {
					...
					txt: dontmanage.markdown(this.frm.comment_box.get_value()),
				};
				return new dontmanage.views.InteractionComposer(args);
			};
	...
	render_timeline_items() {
		...
		dontmanage.utils.bind_actions_with_object(this.timeline_items_wrapper, this);
	}

	add_web_page_view_count() {
		if (this.frm.doc.route && cint(dontmanage.boot.website_tracking_enabled)) {
			dontmanage.utils.get_page_view_count(this.frm.doc.route).then((res) => {
	...
	async get_more_communication_timeline_contents() {
		...
		let response = await dontmanage.call({
			method: "dontmanage.desk.form.load.get_communications",
			args: {
	...
	get_communication_timeline_content(doc, allow_reply = true) {
		doc._url = dontmanage.utils.get_form_link("Communication", doc.name);
		...
		doc.content = dontmanage.dom.remove_script_and_style(doc.content);
		let communication_content = $(dontmanage.render_template("timeline_message_box", { doc }));
		if (allow_reply) {
	...
	get_comment_timeline_content(doc) {
		doc.content = dontmanage.dom.remove_script_and_style(doc.content);
		const comment_content = $(dontmanage.render_template("timeline_message_box", { doc }));
	...
	get_milestone_timeline_contents() {
		...
		(this.doc_info.milestones || []).forEach((milestone_log) => {
			const field = dontmanage.meta.get_label(this.frm.doctype, milestone_log.track_field);
		...
		(this.doc_info.additional_timeline_content || []).forEach((custom_item) => {
			if (custom_item.timeline_badge) {
				custom_timeline_contents.push({
					...
					content: dontmanage.utils.eval(custom_item.method, {
				...
				custom_timeline_contents.push({
					...
					content:
						custom_item.content ||
						dontmanage.render_template(custom_item.template, custom_item.template_data),
				});
			}
		});
	...
	setup_reply(communication_box, communication_doc) {
		...
		let reply = $(`<a class="action-btn reply">${dontmanage.utils.icon("reply", "md")}</a>`).click(
			() => {
				this.compose_mail(communication_doc);
			}
		);
		let reply_all = $(
			`<a class="action-btn reply-all">${dontmanage.utils.icon("reply-all", "md")}</a>`
		).click(() => {
	...
	add_dropdown_item(timeline_box, menu_items) {
		...
		menu_items.forEach((m) => {
			...
			m.empty();
			m.append(
				__("{0}", [dontmanage.utils.to_title_case(formatted_action_name.replace("-", " "))])
			);
			m.removeClass();
	...
	compose_mail(communication_doc = null, reply_all = false) {
		const args = {
			...
			frm: this.frm,
			recipients:
				communication_doc && communication_doc.sender != dontmanage.session.user_email
		...
		const email_accounts = dontmanage.boot.email_accounts
			.filter((account) => {
				return (
		...
		if (communication_doc && args.is_a_reply) {
			args.cc = "";
			if (
				email_accounts.includes(dontmanage.session.user_email) &&
				communication_doc.sender != dontmanage.session.user_email
			) {
				...
				args.cc =
					recipients.filter((r) => r != dontmanage.session.user_email).join(", ") + ", ";
			}
			if (reply_all) {
		...
		if (this.frm.doctype === "Communication") {
			...
			const comment_value = dontmanage.markdown(this.frm.comment_box.get_value());
			args.message = strip_html(comment_value) ? comment_value : "";
		}

		new dontmanage.views.CommunicationComposer(args);
	}
	...
	setup_comment_actions(comment_wrapper, doc) {
		...
		if (dontmanage.session.user == doc.owner || dontmanage.user.has_role("System Manager")) {
			if (dontmanage.model.can_delete("Comment")) {
				const delete_option = $(`
		...
		let edit_button = $();
		let current_user = dontmanage.session.user;
	...
	make_editable(container) {
		return dontmanage.ui.form.make_control({
			parent: container,
			df: {
				fieldtype: "Comment",
				fieldname: "comment",
	...
	update_comment(name, content) {
		return dontmanage
			.xcall("dontmanage.desk.form.utils.update_comment", { name, content })
			.then(() => {
	...
	delete_comment(comment_name) {
		dontmanage.confirm(__("Delete comment?"), () => {
			return dontmanage
				.xcall("dontmanage.client.delete", {
					doctype: "Comment",
					name: comment_name,
				})
				.then(() => {
	...
	update_comment_publicity(comment_name, publish) {
		...
		dontmanage.confirm(message, () => {
			return dontmanage
				.xcall("dontmanage.desk.form.utils.update_comment_publicity", {
					name: comment_name,
					publish,
				})
				.then(() => {
	...
	copy_link(ev) {
		let doc_link = dontmanage.urllib.get_full_url(
			dontmanage.utils.get_form_link(this.frm.doctype, this.frm.docname)
		);
		let element_id = $(ev.currentTarget).closest(".timeline-content").attr("id");
		dontmanage.utils.copy_to_clipboard(`${doc_link}#${element_id}`);
	}
}

...
dontmanage.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = dontmanage.boot.points;
	...
	update_points() {
		return dontmanage
			.xcall("dontmanage.social.doctype.energy_point_log.energy_point_log.get_energy_points", {
				user: dontmanage.session.user,
			})
			.then((data) => {
				dontmanage.boot.points = data;
				this.points = data;
			});
	}
	...
	show_review_dialog() {
		...
		const review_dialog = new dontmanage.ui.Dialog({
			title: __("Add Review"),
			fields: [
				{
					fieldname: "to_user",
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return dontmanage.msgprint(__("You do not have enough points"));
				}
				dontmanage
					.xcall("dontmanage.social.doctype.energy_point_log.energy_point_log.review", {
						doc: {
	...
	setup_detail_popover(el, data) {
		let subject = "";
		let fullname = dontmanage.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${dontmanage.datetime.comment_when(
			data.creation
		)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${dontmanage.utils.icon("review")}
							${subject}

							<p class="mt-1">
								<!-- ${dontmanage.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${dontmanage.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;

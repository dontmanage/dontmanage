...
dontmanage.ui.form.Sidebar = class {
	...
	make() {
		var sidebar_content = dontmanage.render_template("form_sidebar", {
			...
			frm: this.frm,
			can_write:
				dontmanage.model.can_write(this.frm.doctype, this.frm.docname) &&
	...
	setup_copy_event() {
		$(this.sidebar)
			...
			.tooltip()
			.on("click", (e) => {
				dontmanage.utils.copy_to_clipboard($(e.currentTarget).attr("data-copy"));
			});
	}
	...
	setup_print() {
		...
		if (
			...
			if (dontmanage.model.can_print(null, this.frm) && !this.frm.meta.issingle) {
	...
	make_like() {
		...
		this.like_icon.on("click", () => {
			dontmanage.ui.toggle_like(this.like_wrapper, this.frm.doctype, this.frm.doc.name, () => {
				this.refresh_like();
			});
		});
	}

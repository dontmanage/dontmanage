...
dontmanage.ui.form.Sidebar = class {
	constructor(opts) {
		$.extend(this, opts);
	}

	make() {
		var sidebar_content = dontmanage.render_template("form_sidebar", {
		...
		dontmanage.ui.form.setup_user_image_event(this.frm);
	...
	bind_events() {
		...
		this.comments.on("click", function () {
			dontmanage.utils.scroll_to(me.frm.footer.wrapper.find(".comment-box"), true);
		});

		this.like_icon.on("click", function () {
			dontmanage.ui.toggle_like(me.like_wrapper, me.frm.doctype, me.frm.doc.name, function () {
				me.refresh_like();
			});
		});
	}
	...
	setup_keyboard_shortcuts() {
		...
		dontmanage.ui.keys.get_shortcut_group(this.page).add(assignment_link);
	}
	...
	refresh() {
		if (this.frm.doc.__islocal) {
			...
			this.sidebar
				...
				.html(
					__(
						"{0} edited this {1}",
						[
							dontmanage.user.full_name(this.frm.doc.modified_by).bold(),
			...
			this.sidebar
				...
				.html(
					__(
						"{0} created this {1}",
						[
							dontmanage.user.full_name(this.frm.doc.owner).bold(),
			...
			dontmanage.ui.form.set_user_image(this.frm);
		}
	}
			...
			dontmanage.call({
				...
				callback: function (res) {
					...
					me.sidebar.find(".auto-repeat-status").on("click", function () {
						dontmanage.set_route("Form", "Auto Repeat", me.frm.doc.auto_repeat);
					});
				},
			});
		}
	...
	make_tags() {
		...
		this.frm.tags = new dontmanage.ui.TagEditor({
			parent: tags_parent,
	...
	make_attachments() {
		var me = this;
		this.frm.attachments = new dontmanage.ui.form.Attachments({
	...
	make_assignments() {
		this.frm.assign_to = new dontmanage.ui.form.AssignTo({
	...
	make_shared() {
		this.frm.shared = new dontmanage.ui.form.Share({
			frm: this.frm,
	...
	make_like() {
		...
		dontmanage.ui.setup_like_popover(this.sidebar.find(".form-stats-likes"), ".like-icon");
	}
	...
	refresh_like() {
		...
		const liked = dontmanage.ui.is_liked(this.frm.doc);
		this.like_wrapper
	...
	make_review() {
		...
		if (dontmanage.boot.energy_points_enabled && !this.frm.is_new()) {
			this.frm.reviews = new dontmanage.ui.form.Review({
	...
	reload_docinfo(callback) {
		dontmanage.call({
			method: "dontmanage.desk.form.load.get_docinfo",
			args: {
				doctype: this.frm.doctype,

...
dontmanage.provide("dontmanage.ui.form.handlers");
...
dontmanage.ui.form.get_event_handler_list = function (doctype, fieldname) {
	if (!dontmanage.ui.form.handlers[doctype]) {
		dontmanage.ui.form.handlers[doctype] = {};
	}
	if (!dontmanage.ui.form.handlers[doctype][fieldname]) {
		dontmanage.ui.form.handlers[doctype][fieldname] = [];
	}
	return dontmanage.ui.form.handlers[doctype][fieldname];
};

dontmanage.ui.form.on = dontmanage.ui.form.on_change = function (doctype, fieldname, handler) {
	var add_handler = function (fieldname, handler) {
		var handler_list = dontmanage.ui.form.get_event_handler_list(doctype, fieldname);
...
dontmanage.ui.form.off = function (doctype, fieldname, handler) {
	var handler_list = dontmanage.ui.form.get_event_handler_list(doctype, fieldname);
...
dontmanage.ui.form.trigger = function (doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
};

dontmanage.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = dontmanage.get_doc(doctype, name);
	...
	has_handler(event_name) {
		...
		return (
			dontmanage.ui.form.handlers[this.frm.doctype] &&
			dontmanage.ui.form.handlers[this.frm.doctype][event_name]
		);
	}
	...
	get_handlers(event_name, doctype) {
		...
		if (dontmanage.ui.form.handlers[doctype] && dontmanage.ui.form.handlers[doctype][event_name]) {
			$.each(dontmanage.ui.form.handlers[doctype][event_name], function (i, fn) {
				handlers.new_style.push(fn);
			});
		}
		if (dontmanage.ui.form.handlers["*"] && dontmanage.ui.form.handlers["*"][event_name]) {
			$.each(dontmanage.ui.form.handlers["*"][event_name], function (i, fn) {
				handlers.new_style.push(fn);
			});
		}
	...
	setup() {
		...
		$.each(this.frm.fields, function (i, field) {
			...
			if (dontmanage.model.table_fields.includes(field.df.fieldtype)) {
				$.each(
					dontmanage.meta.get_docfields(field.df.options, me.frm.docname),
					function (i, df) {
						setup_add_fetch(df);
					}
				);
		...
		// css
		doctype.__css && dontmanage.dom.set_style(doctype.__css);

		this.trigger("setup");
	}

	log_error(caller, e) {
		dontmanage.show_alert({ message: __("Error in Client Script."), indicator: "error" });

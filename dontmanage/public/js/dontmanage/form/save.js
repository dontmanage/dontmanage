...
dontmanage.ui.form.save = function (frm, action, callback, btn) {
	...
	var save = function () {
		...
		if ((action !== "Save" || frm.is_dirty()) && check_mandatory()) {
			_call({
				method: "dontmanage.desk.form.save.savedocs",
			...
			!frm.is_dirty() &&
				dontmanage.show_alert({ message: __("No changes in document"), indicator: "orange" });
	...
	var remove_empty_rows = function () {
		...
		*/
		const docs = dontmanage.model.get_all_docs(frm.doc);

		// we should only worry about table data
		const tables = docs.filter((d) => {
			return dontmanage.model.is_table(d.doctype);
		});

		let modified_table_fields = [];

		tables.map((doc) => {
			const cells = dontmanage.meta.docfield_list[doc.doctype] || [];
			...
			if (is_empty_row(in_list_view_cells)) {
				dontmanage.model.clear_doc(doc.doctype, doc.name);
	...
	var cancel = function () {
		...
		var workflow_state_fieldname = dontmanage.workflow.get_state_fieldname(frm.doctype);
		...
		_call({
			method: "dontmanage.desk.form.save.cancel",
			args: args,
			callback: function (r) {
				$(document).trigger("save", [frm.doc]);
				callback(r);
	...
	var check_mandatory = function () {
		...
		$.each(dontmanage.model.get_all_docs(frm.doc), function (i, doc) {
			var error_fields = [];
			var folded = false;

			$.each(dontmanage.meta.docfield_list[doc.doctype] || [], function (i, docfield) {
				if (docfield.fieldname) {
					const df = dontmanage.meta.get_docfield(doc.doctype, docfield.fieldname, doc.name);
					...
					if (
						...
						!dontmanage.model.has_value(doc.doctype, doc.name, df.fieldname)
					) {
						has_errors = true;
			...
			if (error_fields.length) {
				...
				let message;
				if (meta.istable) {
					const table_field = dontmanage.meta.docfield_map[doc.parenttype][doc.parentfield];

					const table_label = __(
						table_field.label || dontmanage.unscrub(table_field.fieldname)
					).bold();
	...
	let is_docfield_mandatory = function (doc, df) {
		...
		let parent = dontmanage.get_meta(df.parent);
		...
		if (typeof expression === "boolean") {
			...
			try {
				...
				dontmanage.throw(__('Invalid "mandatory_depends_on" expression'));
			}
		} else {
	...
	var _call = function (opts) {
		...
		if (dontmanage.ui.form.is_saving) {
		...
		if (frm.is_new()) {
			dontmanage.ui.form.remove_old_form_route();
		}
		dontmanage.ui.form.is_saving = true;
		...
		return dontmanage.call({
			...
			always: function (r) {
				$(btn).prop("disabled", false);
				dontmanage.ui.form.is_saving = false;

				if (r) {
					var doc = r.docs && r.docs[0];
					if (doc) {
						dontmanage.ui.form.update_calling_link(doc);
					}
				}
			},
		});
...
dontmanage.ui.form.remove_old_form_route = () => {
	let current_route = dontmanage.get_route().join("/");
	dontmanage.route_history = dontmanage.route_history.filter(
		(route) => route.join("/") !== current_route
	);
};

dontmanage.ui.form.update_calling_link = (newdoc) => {
	if (!dontmanage._from_link) return;
	var doc = dontmanage.get_doc(dontmanage._from_link.doctype, dontmanage._from_link.docname);

	let is_valid_doctype = () => {
		if (dontmanage._from_link.df.fieldtype === "Link") {
			return newdoc.doctype === dontmanage._from_link.df.options;
		} else {
			// dynamic link, type is dynamic
			return newdoc.doctype === doc[dontmanage._from_link.df.options];
		}
	};

	if (is_valid_doctype()) {
		dontmanage.model.with_doctype(newdoc.doctype, () => {
			let meta = dontmanage.get_meta(newdoc.doctype);
			// set value
			if (doc && doc.parentfield) {
				//update values for child table
				$.each(
					dontmanage._from_link.frm.fields_dict[doc.parentfield].grid.grid_rows,
					function (index, field) {
						if (field.doc && field.doc.name === dontmanage._from_link.docname) {
							if (meta.title_field && meta.show_title_field_in_link) {
								dontmanage.utils.add_link_title(
									newdoc.doctype,
									newdoc.name,
									newdoc[meta.title_field]
								);
							}
							dontmanage._from_link.set_value(newdoc.name);
						}
					}
				);
			} else {
			...
			// refresh field
			dontmanage._from_link.refresh();

			// if from form, switch
			if (dontmanage._from_link.frm) {
				dontmanage
					.set_route(
						"Form",
						dontmanage._from_link.frm.doctype,
						dontmanage._from_link.frm.docname
					)
					.then(() => {
						dontmanage.utils.scroll_to(dontmanage._from_link_scrollY);
					});
			}

			dontmanage._from_link = null;
		});
	}
};

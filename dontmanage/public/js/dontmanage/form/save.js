...
dontmanage.ui.form.save = function (frm, action, callback, btn) {
	...
	var save = function () {
		...
		if ((action !== "Save" || frm.is_dirty()) && dontmanage.ui.form.check_mandatory(frm)) {
			_call({
...
dontmanage.ui.form.check_mandatory = function (frm) {
...
dontmanage.ui.form.update_calling_link = async (newdoc) => {
	if (!dontmanage._from_link) return;

	const { field_obj, doc, set_route_args, scrollY } = dontmanage._from_link;
	...
	if (field_obj.frm) {
		await dontmanage.set_route(...set_route_args);
		dontmanage.utils.scroll_to(scrollY);
	}

	delete dontmanage._from_link;

	await dontmanage.model.with_doctype(newdoc.doctype);
	const meta = dontmanage.get_meta(newdoc.doctype);

	// update link title cache
	if (meta.title_field && meta.show_title_field_in_link) {
		dontmanage.utils.add_link_title(newdoc.doctype, newdoc.name, newdoc[meta.title_field]);
	}

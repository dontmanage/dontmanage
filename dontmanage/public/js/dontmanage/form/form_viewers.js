dontmanage.ui.form.FormViewers = class FormViewers {
	...
	refresh() {
		...
		let avatar_group = dontmanage.avatar_group(this.active_users, 5, {
			align: "left",
			overlap: true,
		});
	...
	setup_events() {
		let me = this;
		dontmanage.realtime.off("doc_viewers");
	...
	async update_users({ doctype, docname, users = [] }) {
		users = users.filter((u) => u != dontmanage.session.user);
	...
	async fetch_user_info(users) {
		...
		const data = await dontmanage.xcall("dontmanage.desk.form.load.get_user_info_for_viewers", {
			users: unknown_users,
		});

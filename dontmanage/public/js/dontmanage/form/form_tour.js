dontmanage.ui.form.FormTour = class FormTour {
	constructor({ frm }) {
		this.frm = frm;
		this.driver_steps = [];
	}
	...
	init_driver() {
		...
		dontmanage.router.on("change", () => this.driver.reset());
	...
	async init({ tour_name, on_finish }) {
		if (tour_name) {
			...
			const doctype_tour_exists = await dontmanage.db.exists("Form Tour", this.frm.doctype);
			if (doctype_tour_exists) {
				this.tour = await dontmanage.db.get_doc("Form Tour", this.frm.doctype);
			} else {
				this.tour = { steps: dontmanage.tour[this.frm.doctype] };
			}
		}
		...
		this.tour.steps.forEach((step) => {
			const on_next = () => {
				...
				if (field?.tab && !field.tab.is_active()) {
					...
					dontmanage.utils.sleep(200).then(() => {
						this.start(step.idx);
						this.driver.overlay.refresh();
					});
				}
	...
	get_step(step_info, on_next, on_prev) {
		...
		let element = `.dontmanage-control[data-fieldname='${fieldname}']`;
		...
		if (is_table_field) {
			...
			element = `.grid-row-open .dontmanage-control[data-fieldname='${fieldname}']`;
		}

		return {
			element,
			name,
			popover: { title, description, position: dontmanage.router.slug(position || "Bottom") },
			onNext: on_next,
			onPrevious: on_prev,
		};
	}
	...
	add_new_row_step(step) {
		const $add_row = `.dontmanage-control[data-fieldname='${step.fieldname}'] .grid-add-row`;
		const add_row_step = {
			element: $add_row,
	...
	expand_row_and_proceed(step, start_from) {
		...
		dontmanage.utils.sleep(300).then(() => this.driver.start(start_from));
	}
	...
	open_first_row_of(fieldname) {
		...
		$($close_row).one("click", () => {
			...
			dontmanage.utils.scroll_to(next_element, true, 150, null, () => {
				this.driver.moveNext();
				dontmanage.flags.disable_auto_scroll = false;
			});
			dontmanage.flags.disable_auto_scroll = true;
		});
	}

...
export default class GridRow {
	...
	set_docfields(update = false) {
		if (this.doc && this.parent_df.options) {
			dontmanage.meta.make_docfield_copy_for(
				this.parent_df.options,
				this.doc.name,
				this.docfields
			);
			const docfields = dontmanage.meta.get_docfields(this.parent_df.options, this.doc.name);
			if (update) {
			...
			if (this.frm) {
				...
				dontmanage
					.run_serially([
						...
						() => {
							dontmanage.model.clear_doc(this.doc.doctype, this.doc.name);
		...
		dontmanage.prompt(
			...
			function (values) {
				if (me.doc._sortable === false) {
					dontmanage.msgprint(__("Cannot move row"));
					return;
				}
	...
	render_template() {
		...
		this.row_display = $('<div class="row-data sortable-handle template-row"></div>')
			.appendTo(this.row)
			.html(
				dontmanage.render(this.grid.template, {
					doc: this.doc ? dontmanage.get_format_helper(this.doc) : null,
					frm: this.frm,
					row: this,
				})
			);
	...
	render_row(refresh) {
		...
		if (!this.row_index && !this.show_search) {
			...
			dontmanage.utils.only_allow_num_decimal(this.row_index.find("input"));
		} else {
				...
				if (!this.configure_columns) {
					this.open_form_button = $(`
						<div class="btn-open-row">
							<a>${dontmanage.utils.icon("edit", "xs")}</a>
		...
		if (this.configure_columns && this.frm) {
			this.configure_columns_button = $(`
				<div class="col grid-static-col d-flex justify-content-center" style="cursor: pointer;">
					<a>${dontmanage.utils.icon("setting-gear", "sm", "", "filter: opacity(0.5)")}</a>
				</div>
			`)
				.appendTo(this.row)
				.on("click", () => {
	...
	configure_dialog_for_columns_selector() {
		this.grid_settings_dialog = new dontmanage.ui.Dialog({
		...
		d.set_primary_action(__("Add"), () => {
			...
			if (selected_fields) {
				selected_fields.forEach((selected_column) => {
					let docfield = dontmanage.meta.get_docfield(this.grid.doctype, selected_column);
	...
	prepare_columns_for_dialog(selected_fields) {
		...
		const blocked_fields = dontmanage.model.no_value_type;
	...
	render_selected_columns() {
		...
		if (this.selected_columns_for_grid) {
			this.selected_columns_for_grid.forEach((d) => {
				let docfield = dontmanage.meta.get_docfield(this.grid.doctype, d.fieldname);
					...
					<div class='control-input flex align-center form-control fields_order sortable-handle sortable'
						...
						<div class='row'>
							<div class='col-md-1' style='padding-top: 2px'>
								<a style='cursor: grabbing;'>${dontmanage.utils.icon("drag", "xs")}</a>
							</div>
		...
		$(this.fields_html_wrapper)
			...
			.change((event) => {
				if (cint(event.target.value) === 0) {
					...
					dontmanage.throw(__("Column width cannot be zero."));
				}
	...
	validate_columns_width() {
		...
		if (total_column_width && total_column_width > 10) {
			dontmanage.throw(__("The total column width cannot be more than 10."));
		}
	}
		...
		$(this.fields_html_wrapper)
			...
			.click((event) => {
				...
				if (selected_columns_for_grid && selected_columns_for_grid.length === 0) {
					dontmanage.throw(__("At least one column is required to show in the grid."));
				}
	...
	update_user_settings_for_grid() {
		...
		dontmanage.model.user_settings.save(this.frm.doctype, "GridView", value).then((r) => {
			dontmanage.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}

	reset_user_settings_for_grid() {
		dontmanage.model.user_settings.save(this.frm.doctype, "GridView", null).then((r) => {
			dontmanage.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}
	...
	setup_columns() {
		...
		this.grid.visible_columns.forEach((col, ci) => {
			...
			let txt = this.doc
				? dontmanage.format(this.doc[df.fieldname], df, null, this.doc)
				: __(df.label);
	...
	evaluate_depends_on_value(expression) {
		...
		if (typeof expression === "boolean") {
			...
			try {
				out = dontmanage.utils.eval(expression.substr(5), { doc, parent });
	...
	make_search_column(df, colsize) {
		...
		["Currency", "Float", "Int", "Percent", "Rating"].includes(df.fieldtype) &&
			dontmanage.utils.only_allow_num_decimal($search_input);
		...
		var $col = $(
			...
			.on("touchmove", function (event) {
				...
				if (horizontal) {
					...
					grid.style.left = `${dontmanage.utils.is_rtl() ? "" : "-"}${grid_start}px`;
				}
			})
			...
			.on("click", function (event) {
				if (dontmanage.ui.form.editable_row !== me) {
	...
	toggle_editable_row(show) {
		...
		if (
			...
			if (dontmanage.ui.form.editable_row && dontmanage.ui.form.editable_row !== this) {
				dontmanage.ui.form.editable_row.toggle_editable_row(false);
			}
			...
			dontmanage.ui.form.editable_row = this;
			return false;
		} else {
			...
			dontmanage.ui.form.editable_row = null;
		}
	}
	...
	make_control(column) {
		...
		var field = dontmanage.ui.form.make_control({
			df: df,
			parent: parent,
			only_input: true,
			with_link_btn: true,
	...
	set_arrow_keys(field) {
		...
		if (field.$input) {
			field.$input.on("keydown", function (e) {
				var { TAB, UP: UP_ARROW, DOWN: DOWN_ARROW } = dontmanage.ui.keyCode;
	...
	show_form() {
		...
		dontmanage.dom.freeze("", "dark");
		if (cur_frm) cur_frm.cur_grid = this;
		this.wrapper.addClass("grid-row-open");
		if (
			!dontmanage.dom.is_element_in_viewport(this.wrapper) &&
			!dontmanage.dom.is_element_in_modal(this.wrapper)
		) {
			// -15 offset to make form look visually centered
			dontmanage.utils.scroll_to(this.wrapper, true, -15);
		}
	...
	hide_form() {
		...
		if (!dontmanage.dom.is_element_in_modal(this.row)) {
			dontmanage.utils.scroll_to(this.row, true, 15);
		}
		this.refresh();
	...
	refresh_field(fieldname, txt) {
		...
		if (df && this.doc) {
			txt = dontmanage.format(this.doc[fieldname], df, null, this.doc);
		}

		if (!txt && this.frm) {
			txt = dontmanage.format(this.doc[fieldname], df, null, this.frm.doc);
		}
	...
	get_visible_columns(blacklist = []) {
		...
		var visible_columns = $.map(this.docfields, function (df) {
			var visible =
				...
				!in_list(dontmanage.model.layout_fields, df.fieldtype) &&

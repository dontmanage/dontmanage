		...
		if (this.configure_columns && this.frm) {
			this.configure_columns_button = $(`
				<div class="col grid-static-col pointer">
					<a>${dontmanage.utils.icon("settings", "sm", "", "filter: opacity(0.5)")}</a>
				</div>
			`)
				.appendTo(this.row)
				.on("click", () => {
						...
						<div class='row'>
							...
							<div class='col-1' style='padding-top: 3px;'>
								<a class='text-muted remove-field' data-fieldname='${docfield.fieldname}'>
									${dontmanage.utils.icon("trash", "xs")}
								</a>
							</div>
						</div>
					</div>`;
		...
		var $col = $(
			...
			.on("focusin", function (event) {
				...
				if (df.fieldtype === "Link" || df.fieldtype === "Dynamic Link") {
					dontmanage.utils.sleep(300).then(() => {
	...
	set_arrow_keys(field) {
		...
		if (field.$input) {
			field.$input.on("keydown", function (e) {
				var { ESCAPE, TAB, UP: UP_ARROW, DOWN: DOWN_ARROW } = dontmanage.ui.keyCode;

...
dontmanage.provide("dontmanage.model.docinfo");
...
dontmanage.ui.form.Controller = class FormController {
	constructor(opts) {
		$.extend(this, opts);
	}
};

dontmanage.ui.form.Form = class DontManageForm {
	constructor(doctype, parent, in_form, doctype_layout_name) {
		...
		this.sections = [];
		this.grids = [];
		this.cscript = new dontmanage.ui.form.Controller({ frm: this });
		this.events = {};
		this.fetch_dict = {};
		this.parent = parent;
		this.doctype_layout = dontmanage.get_meta(doctype_layout_name);
	...
	setup_meta() {
		this.meta = dontmanage.get_meta(this.doctype);

		if (this.meta.istable) {
			this.meta.in_dialog = 1;
		}

		this.perm = dontmanage.perm.get_perm(this.doctype); // for create
	...
	setup() {
		this.fields = [];
		this.fields_dict = {};
		this.state_fieldname = dontmanage.workflow.get_state_fieldname(this.doctype);
		...
		this.script_manager = new dontmanage.ui.form.ScriptManager({
			frm: this,
		});
		this.script_manager.setup();
		this.watch_model_updates();

		if (!this.meta.hide_toolbar && dontmanage.boot.desk_settings.timeline) {
			// this.footer_tab = new dontmanage.ui.form.Tab(this.layout, {
			// 	label: __("Activity"),
			// 	fieldname: 'timeline'
			// });

			this.footer = new dontmanage.ui.form.Footer({
				frm: this,
	...
	add_form_keyboard_shortcuts() {
		...
		dontmanage.ui.keys.add_shortcut({
			...
			condition: () => dontmanage.model.can_print(this.doctype, this) && !this.meta.issingle,
		});
	...
	setup_std_layout() {
		...
		// layout
		this.layout = new dontmanage.ui.form.Layout({
			parent: this.body,
		...
		this.dashboard = new dontmanage.ui.form.Dashboard(dashboard_parent, this);

		this.tour = new dontmanage.ui.form.FormTour({
			frm: this,
		});

		// workflow state
		this.states = new dontmanage.ui.form.States({
			frm: this,
		});
	}
	...
	watch_model_updates() {
		...
		// on table fields
		var table_fields = dontmanage.get_children("DocType", me.doctype, "fields", {
			fieldtype: ["in", dontmanage.model.table_fields],
		});
	...
	setup_notify_on_rename() {
		...
		dontmanage.realtime.on("doc_rename", (data) => {
			...
			if (data.doctype == this.doctype && data.old == this.docname) {
				...
				dontmanage.set_route("Form", this.doctype, data.new);
			}
		});
	}
	...
	setup_file_drop() {
		...
		this.$wrapper.on("dragenter dragover", false).on("drop", function (e) {
			...
			if (me.doc.__islocal) {
				dontmanage.msgprint(__("Please save before attaching."));
				throw "attach error";
			}
		...
		this.fields.map((field) => {
			if (field.df.fieldtype === "Markdown Editor") {
				this.set_df_property(field.df.fieldname, "autocompletions", () => {
					...
					return attachments
						.filter((file) => dontmanage.utils.is_image_file(file.file_url))
						.map((file) => {
							return {
	...
	refresh(docname) {
		...
		if (this.docname) {
			...
			// set the doc
			this.doc = dontmanage.get_doc(this.doctype, this.docname);

			// check permissions
			this.fetch_permissions();
			if (!this.has_read_permission()) {
				dontmanage.show_not_permitted(__(this.doctype) + " " + __(cstr(this.docname)));
				return;
			}
			...
			// read only (workflow)
			this.read_only = dontmanage.workflow.is_read_only(this.doctype, this.docname);
			if (this.read_only) {
				this.set_read_only();
			}
	...
	setup_doctype_actions() {
		if (this.meta.actions) {
			for (let action of this.meta.actions) {
				dontmanage.ui.form.on(this.doctype, "refresh", () => {
					if (!this.is_new()) {
						if (!action.hidden) {
	...
	execute_action(action) {
		if (typeof action === "string") {
			...
			if (typeof action === "string") {
				dontmanage.throw(`Action ${action} not found`);
			}
		}
		if (action.action_type === "Server Action") {
			return dontmanage.xcall(action.action, { doc: this.doc }).then((doc) => {
				if (doc.doctype) {
					// document is returned by the method,
					// apply the changes locally and refresh
					dontmanage.model.sync(doc);
					this.refresh();
				}
			...
			return dontmanage.set_route(action.action);
		}
	}
	...
	initialize_new_doc() {
		...
		this.script_manager.trigger("before_load", this.doctype, this.docname).then(() => {
			...
			dontmanage.after_ajax(function () {
				me.trigger_link_fields();
			});

			dontmanage.breadcrumbs.add(me.meta.module, me.doctype);
		});
	...
	render_form(switched) {
		if (!this.meta.istable) {
			...
			if (dontmanage.boot.desk_settings.form_sidebar) {
				this.sidebar = new dontmanage.ui.form.Sidebar({
					frm: this,
					page: this.page,
	...
	run_after_load_hook() {
		if (dontmanage.route_hooks.after_load) {
			let route_callback = dontmanage.route_hooks.after_load;
			delete dontmanage.route_hooks.after_load;
	...
	refresh_header(switched) {
		...
		if (!this.meta.in_dialog || this.in_form) {
			dontmanage.utils.set_title(this.meta.issingle ? this.doctype : this.docname);
		}
		...
		var after_save = function (r) {
			...
			if (!r.exc) {
				if (["Save", "Update", "Amend"].indexOf(save_action) !== -1) {
					dontmanage.utils.play_sound("click");
				}

				me.script_manager.trigger("after_save");

				if (dontmanage.route_hooks.after_save) {
					let route_callback = dontmanage.route_hooks.after_save;
					delete dontmanage.route_hooks.after_save;
		...
		if (save_action != "Update") {
			...
			dontmanage
				.run_serially([
					...
					() => {
						if (!dontmanage.validated) {
							fail();
							return;
						}

						dontmanage.ui.form.save(me, save_action, after_save, btn);
					},
				])
				.catch(fail);
		} else {
	...
	discard(btn, callback, on_error) {
		...
		return new Promise((resolve) => {
			dontmanage.confirm(__("Discard {0}", [this.docname]), function () {
						...
						me.save(
							...
							function (r) {
								if (r.exc) {
									...
									dontmanage.utils.play_sound("submit");
									callback && callback();
									me.script_manager
										.trigger("on_submit")
										.then(() => resolve(me))
										.then(() => {
											if (dontmanage.route_hooks.after_submit) {
												let route_callback =
													dontmanage.route_hooks.after_submit;
												delete dontmanage.route_hooks.after_submit;
												route_callback(me);
											}
										});
								}
	...
	savecancel(btn, callback, on_error) {
		...
		dontmanage
			.call({
				method: "dontmanage.desk.form.linked_with.get_submitted_linked_docs",
				args: {
	...
	_cancel_all(r, btn, callback, on_error) {
		...
		for (let doctype of doctypes) {
			if (!me.ignore_doctypes_on_cancel_all.includes(doctype)) {
				let docnames = links
					...
					.map((link) => dontmanage.utils.get_form_link(link.doctype, link.name, true))
					.join(", ");
		...
		let can_cancel = links.every((link) => dontmanage.model.can_cancel(link.doctype));
		if (can_cancel) {
		...
		let d = new dontmanage.ui.Dialog(
			{
				...
				fields: [
					{
						fieldtype: "HTML",
						options: `<p class="dontmanage-confirm-message">${confirm_message}</p>`,
					},
				],
			},
			() => me.handle_save_fail(btn, on_error)
		...
		if (can_cancel) {
			d.set_primary_action(__("Cancel All"), () => {
				d.hide();
				dontmanage.call({
					method: "dontmanage.desk.form.linked_with.cancel_all_linked_docs",
					args: {
						docs: links,
						ignore_doctypes_on_cancel_all: me.ignore_doctypes_on_cancel_all || [],
					},
			...
			me.script_manager.trigger("before_cancel").then(() => {
				...
				var after_cancel = function (r) {
					if (r.exc) {
						...
						dontmanage.utils.play_sound("cancel");
						me.refresh();
						callback && callback();
						me.script_manager.trigger("after_cancel");
					}
				};
				dontmanage.ui.form.save(me, "cancel", after_cancel, btn);
			});
		};
	...
	_discard(btn, on_error, skip_confirm) {
		...
		const discard_doc = () => {
			...
			me.script_manager.trigger("before_discard").then(() => {
				...
				//dontmanage.ui.form.discard(me, after_discard, btn);
				dontmanage.call({
					freeze: true,
					method: "dontmanage.desk.form.save.discard",
					args: {
						doctype: me.doc.doctype,
						name: me.doc.name,
					},
	...
	savetrash() {
		...
		dontmanage.model.delete_doc(this.doctype, this.docname, function () {
			window.history.back();
		});
	}
	...
	amend_doc() {
		if (!this.fields_dict["amended_from"]) {
			dontmanage.msgprint(__('"amended_from" field must be present to do an amendment.'));
			return;
		}

		dontmanage
			.xcall("dontmanage.client.is_document_amended", {
			...
			.then((is_amended) => {
				...
				var fn = function (newdoc) {
					...
					if (me.fields_dict && me.fields_dict["amendment_date"])
						newdoc.amendment_date = dontmanage.datetime.obj_to_str(new Date());
				};
				this.copy_doc(fn, 1);
				dontmanage.utils.play_sound("click");
			});
	...
	validate_form_action(action, resolve) {
		...
		var perms = dontmanage.perm.get_perm(this.doc.doctype)[0];
		...
		if (
			(dontmanage.workflow.is_read_only(this.doctype, this.docname) &&
	...
	show_submit_message() {
		if (
			...
			!this.is_dirty() &&
			!this.is_new() &&
			!dontmanage.model.has_workflow(this.doctype) && // show only if no workflow
	...
	fetch_permissions() {
		...
		this.perm = dontmanage.perm.get_perm(dt, this.doc);
	}
	...
	check_doctype_conflict(docname) {
		if (this.doctype == "DocType" && docname == "DocType") {
			dontmanage.msgprint(__("Allowing DocType, DocType. Be careful!"));
		} else if (this.doctype == "DocType") {
			if (dontmanage.views.formview[docname] || dontmanage.pages["List/" + docname]) {
				window.location.reload();
			}
		} else {
			if (
				dontmanage.views.formview.DocType &&
				dontmanage.views.formview.DocType.frm.opendocs[this.doctype]
			) {
				window.location.reload();
			}
		}
	...
	rename_notify(dt, old, name) {
		...
		// cleanup
		if (this && this.opendocs[old] && dontmanage.meta.docfield_copy[dt]) {
			// delete docfield copy
			dontmanage.meta.docfield_copy[dt][name] = dontmanage.meta.docfield_copy[dt][old];
			delete dontmanage.meta.docfield_copy[dt][old];
		}
		...
		dontmanage.re_route[dontmanage.router.get_sub_path()] = `${encodeURIComponent(
			dontmanage.router.slug(this.doctype)
		)}/${encodeURIComponent(name)}`;
		!dontmanage._from_link && dontmanage.set_route("Form", this.doctype, name);
	}
	...
	print_doc() {
		...
		dontmanage.set_route("print", this.doctype, this.doc.name);
	}

	navigate_records(prev) {
		let filters, sort_field, sort_order;
		let list_view = dontmanage.get_list_view(this.doctype);
		if (list_view) {
			filters = list_view.get_filters_for_args();
			sort_field = list_view.sort_by;
			sort_order = list_view.sort_order;
		} else {
			let list_settings = dontmanage.get_user_settings(this.doctype)["List"];
			if (list_settings) {
		...
		dontmanage
			.call({ method: "dontmanage.desk.form.utils.get_next", args, freeze: true })
			.then((r) => {
				if (r.message) {
					dontmanage.set_route("Form", this.doctype, r.message);
	...
	rename_doc() {
		dontmanage.model.rename_doc(this.doctype, this.docname, () => this.refresh_header());
	}
	...
	email_doc(message) {
		return new dontmanage.views.CommunicationComposer({
			doc: this.doc,
			frm: this,
	...
	copy_doc(onload, from_amend) {
		...
		var newdoc = dontmanage.model.copy_doc(this.doc, from_amend);
	...
	reload_doc() {
		...
		if (!this.doc.__islocal) {
			dontmanage.model.remove_from_locals(this.doctype, this.docname);
			return dontmanage.model.with_doc(this.doctype, this.docname, () => {
				this.refresh();
			});
		}
	}
	...
	has_perm(ptype) {
		return dontmanage.perm.has_perm(this.doctype, 0, ptype, this.doc);
	}
	...
	get_docinfo() {
		return dontmanage.model.docinfo[this.doctype][this.docname];
	}
	...
	set_footnote(txt) {
		this.footnote_area = dontmanage.utils.set_footnote(this.footnote_area, this.body, txt);
	}
	...
	scroll_to_element() {
		if (dontmanage.route_options && dontmanage.route_options.scroll_to) {
			var scroll_to = dontmanage.route_options.scroll_to;
			delete dontmanage.route_options.scroll_to;
	...
	show_success_action() {
		...
		const success_action = new dontmanage.ui.form.SuccessAction(this);
		success_action.show();
	}
	...
	field_map(fnames, fn) {
		...
		for (var i = 0, l = fnames.length; i < l; i++) {
			...
			var field = dontmanage.meta.get_docfield(this.doctype, fieldname, this.docname);
			if (field) {
				fn(field);
	...
	get_docfield(fieldname1, fieldname2) {
		if (fieldname2) {
			...
			return dontmanage.meta.get_docfield(doctype, fieldname2, this.docname);
		} else {
			// for parent
			return dontmanage.meta.get_docfield(this.doctype, fieldname1, this.docname);
		}
	}
	...
	set_df_property(fieldname, property, value, docname, table_field, table_row_name = null) {
		...
		if (!docname || !table_field) {
			...
			const filtered_fields = dontmanage.utils.filter_dict(grid.docfields, {
				fieldname: table_field,
			});
			if (filtered_fields.length) {
				df = dontmanage.meta.get_docfield(
	...
	get_files() {
		return this.attachments
			? dontmanage.utils.sort(this.attachments.get_attachments(), "file_name", "string")
			: [];
	}
	...
	clear_table(fieldname) {
		dontmanage.model.clear_table(this.doc, fieldname);
	}

	add_child(fieldname, values) {
		var doc = dontmanage.model.add_child(
			this.doc,
			dontmanage.meta.get_docfield(this.doctype, fieldname).options,
			fieldname
		);
		if (values) {
	...
	set_value(field, value, if_missing, skip_dirty_trigger = false) {
		...
		var _set = function (f, v) {
			...
			if (fieldobj) {
				if (!if_missing || !dontmanage.model.has_value(me.doctype, me.doc.name, f)) {
					if (
						dontmanage.model.table_fields.includes(fieldobj.df.fieldtype) &&
						$.isArray(v)
					) {
						// set entire child table from specified array as value
						dontmanage.model.clear_table(me.doc, fieldobj.df.fieldname);

						const standard_fields = [
							...dontmanage.model.std_fields_list,
							...dontmanage.model.child_table_field_list,
						];
						v.forEach((d, idx) => {
							let child = dontmanage.model.add_child(
								me.doc,
								fieldobj.df.options,
						...
						return dontmanage.model.set_value(
							me.doctype,
							me.doc.name,
							f,
							v,
				...
				dontmanage.msgprint(__("Field {0} not found.", [f]));
		...
		if (typeof field == "string") {
			...
			return dontmanage.run_serially(tasks);
		}
	}
	...
	call(opts, args, callback) {
		...
		if (!opts.doc) {
			if (opts.method.indexOf(".") === -1)
				opts.method = dontmanage.model.get_server_module_name(me.doctype) + "." + opts.method;
					...
					if (opts.child) {
						...
						if (opts.child) {
							var std_field_list = ["doctype"]
								.concat(dontmanage.model.std_fields_list)
								.concat(dontmanage.model.child_table_field_list);
	...
	set_read_only() {
		const docperms = dontmanage.perm.get_perm(this.doc.doctype);
	...
	get_formatted(fieldname) {
		return dontmanage.format(
			this.doc[fieldname],
			dontmanage.meta.get_docfield(this.doctype, fieldname, this.docname),
			{ no_icon: true },
			this.doc
		);
	}
	...
	open_grid_row() {
		return dontmanage.ui.form.get_open_grid_form();
	}

	get_title() {
		return dontmanage.model.get_doc_title(this.doc);
	}
	...
	get_selected() {
		...
		var selected = {},
			me = this;
		dontmanage.meta.get_table_fields(this.doctype, true).forEach(function (df) {
	...
	set_indicator_formatter(fieldname, get_color, get_text) {
		...
		var doctype;
		if (dontmanage.meta.docfield_map[this.doctype][fieldname]) {
			doctype = this.doctype;
		} else {
			dontmanage.meta.get_table_fields(this.doctype, true).every(function (df) {
				if (dontmanage.meta.docfield_map[df.options][fieldname]) {
					doctype = df.options;
					return false;
				} else {
					return true;
		...
		dontmanage.meta.docfield_map[doctype][fieldname].formatter = function (
			value,
			df,
			options,
			doc
			...
			if (value) {
				...
				if (get_text) {
					label = get_text(doc);
				} else if (dontmanage.form.link_formatters[df.options]) {
					label = dontmanage.form.link_formatters[df.options](value, doc, df);
				} else {
					label = value;
				}
				...
				return `
						<a class="indicator ${get_color(doc || {})}"
							href="/desk/${dontmanage.router.slug(df.options)}/${escaped_name}"
	...
	make_new(doctype) {
		...
		if (this.make_methods && this.make_methods[doctype]) {
			...
			dontmanage.model.with_doctype(doctype, function () {
				let new_doc = dontmanage.model.get_new_doc(doctype, null, null, true);

				// set link fields (if found)
				me.set_link_field(doctype, new_doc);

				dontmanage.ui.form.make_quick_entry(doctype, null, null, new_doc);
				// dontmanage.set_route('Form', doctype, new_doc.name);
			});
		}
	}
	...
	set_link_field(doctype, new_doc) {
		let me = this;
		dontmanage.get_meta(doctype).fields.forEach(function (df) {
	...
	update_in_all_rows(table_fieldname, fieldname, value) {
		...
		dontmanage.model
			...
			.filter((child) => !dontmanage.model.has_value(child.doctype, child.name, fieldname))
			.forEach((child) =>
				dontmanage.model.set_value(child.doctype, child.name, fieldname, value)
			);
	}
	...
	scroll_to_field(fieldname, focus = true) {
		...
		// scroll to input
		dontmanage.utils.scroll_to($el, true, 15);
	...
	setup_docinfo_change_listener() {
		...
		dontmanage.realtime.on("docinfo_update", ({ doc, key, action = "update" }) => {
			...
			let doc_list = dontmanage.model.docinfo[doctype][docname][key] || [];
			let docindex = doc_list.findIndex((old_doc) => {
				return old_doc.name === doc.name;
			});

			if (action === "add") {
				dontmanage.model.docinfo[doctype][docname][key].push(doc);
			}
			if (docindex > -1) {
				if (action === "update") {
					dontmanage.model.docinfo[doctype][docname][key].splice(docindex, 1, doc);
				}
				if (action === "delete") {
					dontmanage.model.docinfo[doctype][docname][key].splice(docindex, 1);
				}
			}
	...
	show_workflow_read_only_banner() {
		...
		if (this.dashboard) {
			...
			dontmanage.after_ajax(_show_read_only_banner);
		}
	}
};

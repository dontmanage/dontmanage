...
dontmanage.ui.form.Form = class DontManageForm {
	...
	setup_meta() {
		this.meta = dontmanage.get_doc("DocType", this.doctype);
	...
	watch_model_updates() {
		...
		$.each(table_fields, function (i, df) {
			dontmanage.model.on(df.options, "*", function (fieldname, value, doc) {
	...
	refresh(docname) {
		...
		if (this.docname) {
			...
			if (this.read_only) {
				...
				dontmanage.show_alert(__("This form is not editable due to a Workflow."));
			}
	...
	check_doctype_conflict(docname) {
		if (this.doctype == "DocType" && docname == "DocType") {
			...
			if (dontmanage.views.formview[docname] || dontmanage.pages["List/" + docname]) {
				...
				//	dontmanage.msgprint(__("Cannot open {0} when its instance is open", ['DocType']))
			...
			if (
				...
				window.location.reload();
				//	dontmanage.msgprint(__("Cannot open instance when its {0} is open", ['DocType']))
				// throw 'doctype open conflict'
			}
		}
	}
	...
	navigate_records(prev) {
		...
		dontmanage.call("dontmanage.desk.form.utils.get_next", args).then((r) => {
			if (r.message) {
	...
	email_doc(message) {
		new dontmanage.views.CommunicationComposer({
			doc: this.doc,
			frm: this,
	...
	get_selected() {
		...
		var selected = {},
			me = this;
		dontmanage.meta.get_table_fields(this.doctype).forEach(function (df) {
	...
	set_indicator_formatter(fieldname, get_color, get_text) {
		...
		if (dontmanage.meta.docfield_map[this.doctype][fieldname]) {
			...
			dontmanage.meta.get_table_fields(this.doctype).every(function (df) {
		...
		dontmanage.meta.docfield_map[doctype][fieldname].formatter = function (
			...
			if (value) {
				...
				if (get_text) {
					...
					label = dontmanage.form.link_formatters[df.options](value, doc);
				} else {
					label = value;
				}
		...
		dontmanage.realtime.on("docinfo_update", ({ doc, key, action = "update" }) => {
			...
			if (
				!(
					...
					doc.doctype === "Comment" &&
					doc.owner === dontmanage.session.user
				)
			) {

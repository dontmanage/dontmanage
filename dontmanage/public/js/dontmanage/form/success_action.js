...
dontmanage.provide("dontmanage.success_action");

dontmanage.ui.form.SuccessAction = class SuccessAction {
	constructor(form) {
		this.form = form;
		this.load_setting();
	}
	...
	load_setting() {
		this.setting = dontmanage.boot.success_action.find(
	...
	show_alert() {
		dontmanage.db.get_list(this.form.doctype, { limit: 2 }).then((result) => {
	...
	get default_actions() {
		return {
			new: {
				label: __("New"),
				action: (frm) => dontmanage.new_doc(frm.doctype),
			},
			print: {
				label: __("Print"),
			...
			list: {
				label: __("View All"),
				action: (frm) => {
					dontmanage.set_route("List", frm.doctype);
				},
			},
		};
	}

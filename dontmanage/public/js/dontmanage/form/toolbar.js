...
dontmanage.ui.form.Toolbar = class Toolbar {
	...
	make_menu_items() {
		...
		const is_submittable = dontmanage.model.is_submittable(this.frm.doc.doctype);
		...
		if (
			...
			if (dontmanage.model.can_print(null, me.frm) && !this.frm.meta.issingle) {
		...
		// email
		if (dontmanage.model.can_email(null, me.frm) && me.frm.doc.docstatus < 2) {
			this.page.add_menu_item(
				__("Email"),
				function () {
					me.frm.email_doc();
		...
		this.page.add_menu_item(
			...
			function () {
				dontmanage.utils.copy_to_clipboard(JSON.stringify(me.frm.doc));
			},
			true
		);
		...
		if (this.can_repeat()) {
			this.page.add_menu_item(
				__("Repeat"),
				function () {
					dontmanage.utils.new_auto_repeat_prompt(me.frm);
				},
				true
			);
		}
		...
		if (p[CREATE] && !this.frm.meta.issingle && !this.frm.meta.in_create) {
			this.page.add_menu_item(
				...
				function () {
					dontmanage.new_doc(me.frm.doctype, true);
				},
				true,
				{
					shortcut: "Ctrl+B",
	...
	make_customize_buttons() {
		...
		if (dontmanage.model.can_create("DocType")) {
			if (dontmanage.boot.developer_mode === 1 && !is_doctype_form) {
				// edit doctype
				this.page.add_menu_item(
					__("Edit DocType"),
					() => {
	...
	has_workflow() {
		if (this._has_workflow === undefined)
			this._has_workflow = dontmanage.get_list("Workflow", {

...
dontmanage.ui.form.Toolbar = class Toolbar {
	...
	set_title() {
		...
		if (this.frm.is_new()) {
			...
			if (
				...
				this.page.$sub_title_area.on("click", (event) => {
					...
					dontmanage.utils.copy_to_clipboard(this.frm.docname);
				});
			}
		} else {
			title = this.frm.docname;
		...
		if (this.frm.meta.title_field) {
			dontmanage.utils.set_title(title + " - " + this.frm.docname);
		}
	...
	set_indicator() {
		var indicator = dontmanage.get_indicator(this.frm.doc);
		if (
	...
	make_menu() {
		...
		if (dontmanage.boot.desk_settings.form_navigation_buttons) {
	...
	add_discard() {
		if (
			dontmanage.model.is_submittable(this.frm.doc.doctype) &&
	...
	add_email() {
		// email
		if (dontmanage.model.can_email(null, this.frm) && this.frm.doc.docstatus < 2) {
			this.page.add_menu_item(
				__("Email"),
				() => {
					this.frm.email_doc();
	...
	add_duplicate() {
		if (dontmanage.boot.user.can_create.includes(this.frm.doctype) && !this.frm.meta.allow_copy) {
			this.page.add_menu_item(
				__("Duplicate"),
				() => {
					this.frm.copy_doc();
	...
	add_delete() {
		...
		if (
			...
			!this.frm.doc.__islocal &&
			!dontmanage.model.is_single(this.frm.doctype) &&
			dontmanage.model.can_delete(this.frm.doctype)
		) {
			this.page.add_menu_item(
				__("Delete"),
				() => {
	...
	add_follow() {
		if (this.frm.meta.track_changes && dontmanage.boot.user.document_follow_notify) {
	...
	add_copy_to_clipboard() {
		...
		this.page.add_menu_item(
			...
			() => {
				dontmanage.utils.copy_to_clipboard(JSON.stringify(this.frm.doc));
			},
			true
		);
	}
	...
	add_auto_repeat() {
		...
		if (this.can_repeat()) {
			this.page.add_menu_item(
				__("Repeat"),
				() => {
					dontmanage.utils.new_auto_repeat_prompt(this.frm);
				},
				true
			);
		}
	...
	add_new() {
		...
		if (p[CREATE] && !this.frm.meta.issingle && !this.frm.meta.in_create) {
			this.page.add_menu_item(
				...
				() => {
					dontmanage.new_doc(this.frm.doctype, true);
				},
				true,
				{
					shortcut: "Ctrl+B",
	...
	add_audit_trail() {
		if (
			...
			dontmanage.model.get_value("DocType", this.frm.doc.doctype, "track_changes")
		) {
			this.page.add_menu_item(
				__("View Audit Trail"),
				() => {
					dontmanage.set_route("audit-trail");
				},
				true
			);
		}
	...
	make_customize_buttons() {
		...
		if (dontmanage.model.can_create("DocType")) {
			if (dontmanage.boot.developer_mode && !is_doctype_form) {
				// edit doctype
				this.page.add_menu_item(
					__("Edit DocType"),
					() => {
						dontmanage.set_route("Form", "DocType", this.frm.doctype);
					},
					true
				);
			}
	...
	has_workflow() {
		if (this._has_workflow === undefined)
			this._has_workflow = dontmanage.model.has_workflow(this.frm.doctype);
	...
	show_linked_with() {
		if (!this.frm.linked_with) {
			this.frm.linked_with = new dontmanage.ui.form.LinkedWith({
				frm: this.frm,
			});
		}
	...
	set_primary_action(dirty) {
		...
		if (status) {
			...
			if (status !== this.current_status && status === "Amend") {
				...
				dontmanage
					.xcall(
						"dontmanage.client.is_document_amended",
						{
							doctype: doc.doctype,
							docname: doc.name,
						},
	...
	set_page_actions(status) {
		...
		if (status === "Edit") {
			...
			if (this.has_workflow()) {
				dontmanage
					.xcall("dontmanage.model.workflow.can_cancel_document", {
	...
	setup_sidebar_toggle(sidebar_wrapper) {
		if (dontmanage.utils.is_xs() || dontmanage.utils.is_sm()) {
	...
	follow() {
		...
		dontmanage
			.call("dontmanage.desk.form.document_follow.update_follow", {

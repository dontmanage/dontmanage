...
dontmanage.provide("dontmanage.form.formatters");

dontmanage.form.link_formatters = {};

dontmanage.form.formatters = {
	...
	_apply_custom_formatter: function (value, df) {
		...
		example:
			dontmanage.meta.docfield_map[df.parent][df.fieldname].formatter = (value) => {
		...
		*/
		...
		if (df) {
			const std_df =
				...
				dontmanage.meta.docfield_map[df.parent][df.fieldname];
	...
	Data: function (value, df) {
		...
		if (df && df.options == "IBAN") {
			if (!value) return "";
			return dontmanage.utils.get_formatted_iban(value);
		}
		value = value == null ? "" : value;

		return dontmanage.form.formatters._apply_custom_formatter(value, df);
	},
	Autocomplete: function (value, df) {
		return __(dontmanage.form.formatters["Data"](value, df));
	},
	...
	Float: function (value, docfield, options, doc) {
		...
		var precision =
			docfield.precision ||
			cint(dontmanage.boot.sysdefaults && dontmanage.boot.sysdefaults.float_precision) ||
			null;
		if (docfield.options && docfield.options.trim()) {
			...
			return dontmanage.form.formatters.Currency(value, docfield, options, doc);
		} else {
			// show 1.000000 as 1
			...
			return dontmanage.form.formatters._right(format_number(value, null, precision), options);
		}
	},
	...
	Currency: function (value, docfield, options, doc) {
		...
		if (precision > 2) {
			...
			if (decimals.length < 3 || decimals.length < precision) {
				const fraction =
					dontmanage.model.get_value(":Currency", currency, "fraction_units") || 100; // if not set, minimum 2.
		...
		if (options && options.only_value) {
			...
			return dontmanage.form.formatters._right(value, options);
		}
	},
	...
	Link: function (value, docfield, options, doc) {
		...
		if (dontmanage.form.link_formatters[doctype]) {
			// don't apply formatters in case of composite (parent field of same type)
			if (doc && doctype !== doc.doctype) {
				value = dontmanage.form.link_formatters[doctype](value, doc, docfield);
			}
		}
	...
	Date: function (value) {
		if (!dontmanage.datetime.str_to_user) {
			return value;
		}
		if (value) {
	...
	DateRange: function (value) {
		if (Array.isArray(value)) {
			return __("{0} to {1}", [
				dontmanage.datetime.str_to_user(value[0]),
				dontmanage.datetime.str_to_user(value[1]),
			]);
		} else {
			return value || "";
		}
	...
	Datetime: function (value) {
		if (value) {
			return moment(dontmanage.datetime.convert_to_user_tz(value)).format(
				dontmanage.boot.sysdefaults.date_format.toUpperCase() +
					" " +
					(dontmanage.boot.sysdefaults.time_format || "HH:mm:ss")
			);
		} else {
			return "";
		}
	...
	Text: function (value, df) {
		if (value) {
			...
			if (!match) {
				value = dontmanage.utils.replace_newlines(value);
			}
		}

		return dontmanage.form.formatters.Data(value, df);
	},
	Time: function (value) {
		if (value) {
			value = dontmanage.datetime.str_to_user(value, true);
		}
	...
	Duration: function (value, docfield) {
		if (value) {
			let duration_options = dontmanage.utils.get_duration_options(docfield);
			value = dontmanage.utils.get_formatted_duration(value, duration_options);
		}
	...
	SmallText: function (value) {
		return dontmanage.form.formatters.Text(value);
	},
	TextEditor: function (value) {
		let formatted_value = dontmanage.form.formatters.Text(value);
	...
	WorkflowState: function (value) {
		var workflow_state = dontmanage.get_doc("Workflow State", value);
		if (workflow_state) {
			return repl(
	...
	Icon: (value) => {
		return value
			...
			<div class="selected-icon">${dontmanage.utils.icon(value, "md")}</div>
...
dontmanage.form.get_formatter = function (fieldtype) {
	...
	return dontmanage.form.formatters[fieldtype.replace(/ /g, "")] || dontmanage.form.formatters.Data;
};

dontmanage.format = function (value, df, options, doc) {
	let mask_readonly = false;
	if (df?.parent) {
		const mask_fields = dontmanage.get_meta(df.parent)?.masked_fields;
	...
	var formatter = df.formatter || dontmanage.form.get_formatter(fieldtype);

	var formatted = formatter(value, df, options, doc);

	if (typeof formatted == "string") formatted = dontmanage.dom.remove_script_and_style(formatted);

	return formatted;
};

dontmanage.get_format_helper = function (doc) {
	var helper = {
		get_formatted: function (fieldname) {
			var df = dontmanage.meta.get_docfield(doc.doctype, fieldname);
			if (!df) {
				console.log("fieldname not found: " + fieldname);
			}
			return dontmanage.format(doc[fieldname], df, { inline: 1 }, doc);
		},
	};
	$.extend(helper, doc);
	return helper;
...
dontmanage.form.link_formatters["User"] = function (value, doc, docfield) {

dontmanage.ui.form.ControlTableMultiSelect = class ControlTableMultiSelect extends (
	dontmanage.ui.form.ControlLink
) {
	...
	make_input() {
		...
		this.$input_area.on("click", ".btn-remove", (e) => {
			...
			const rows = this._get_rows().filter((row) => {
				...
				dontmanage.run_serially([
					...
					() => {
						dontmanage.model.clear_doc(this.df.options, row.name);
		...
		this.$input_area.on("click", ".btn-link-to-form", (e) => {
			...
			const value = decodeURIComponent($value.data().value);
			const link_field = this.get_link_field();
			dontmanage.set_route("Form", link_field.options, value);
		});
		this.$input.on("keydown", (e) => {
			// if backspace key pressed on empty input, delete last value
			if (e.keyCode == dontmanage.ui.keyCode.BACKSPACE && e.target.value === "") {
	...
	parse(value) {
		...
		let new_row;
		if (this.frm) {
			new_row = dontmanage.model.add_child(this.frm.doc, this.df.options, this.df.fieldname);
	...
	async validate(value) {
		...
		if (
			dontmanage.utils.is_empty(link_value) ||
		...
		if (!this.df.ignore_link_validation) {
			...
			if (dontmanage.utils.is_empty(validated_value)) {
				return all_rows_except_last;
			}
	...
	get_pill_html(value) {
		...
		const pill_name = dontmanage.utils.get_link_title(link_field.options, value) || value;

		return `
			<button class="data-pill btn tb-selected-value" data-value="${encoded_value}">
				<span class="btn-link-to-form">${__(dontmanage.utils.escape_html(pill_name))}</span>
				<span class="btn-remove">${dontmanage.utils.icon("close")}</span>
			</button>
		`;
	}
	get_options() {
	...
	get_link_field() {
		if (!this._link_field) {
			const meta = dontmanage.get_meta(this.df.options);

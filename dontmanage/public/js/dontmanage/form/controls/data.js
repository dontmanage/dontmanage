dontmanage.provide("dontmanage.phone_call");

dontmanage.ui.form.ControlData = class ControlData extends dontmanage.ui.form.ControlInput {
	...
	make_input() {
		...
		this.$input.on("paste", (e) => {
			...
			if (maxlength && pasted_data.length > maxlength) {
				...
				if (this.frm && dontmanage.model.can_write(this.frm.doctype)) {
					let doctype_edit_link = null;
					if (this.frm.meta.custom) {
						doctype_edit_link = dontmanage.utils.get_form_link(
							"DocType",
							this.frm.doctype,
							true,
							__("this form")
		...
		if (this.df.with_copy_button) {
			this.$wrapper
				...
				.append(
					...
					${dontmanage.utils.icon("clipboard", "sm")}
				</button>`
				)
				.find(".action-btn")
				.click(() => {
					dontmanage.utils.copy_to_clipboard(this.value);
				});
		}
	}
	...
	setup_barcode_field() {
		this.$wrapper.find(".control-input").append(
			`<span class="link-btn">
				<a class="btn-open no-decoration" title="${__("Scan")}">
					${dontmanage.utils.icon("scan-barcode", "sm")}
				</a>
			</span>`
		);
		...
		const me = this;
		$(document).on("dontmanage.ui.Dialog:shown", function () {
			me.$scan_btn.toggle(true);
		});
		this.$scan_btn.on("click", "a", () => {
			new dontmanage.ui.Scanner({
				dialog: true,
				multiple: false,
				on_scan(data) {
	...
	bind_change_event() {
		...
		if (this.constructor.trigger_change_on_input_event && !this.in_grid()) {
			...
			this.$input.on("input", dontmanage.utils.debounce(change_handler, 500));
		}
	}
	setup_autoname_check() {
		if (!this.df.parent) return;
		this.meta = dontmanage.get_meta(this.df.parent);
		if (
			this.meta &&
			((this.meta.autoname &&
	...
	format_for_input(val) {
		if (this.df.options == "IBAN" && val) {
			return dontmanage.utils.get_formatted_iban(val);
		}
		return val == null ? "" : val;
	}
	parse(value) {
	...
	validate(v) {
		...
		if (this.df.options == "Phone") {
			...
			var email_list = dontmanage.utils.split_emails(v);
			if (!email_list) {
				return "";
			} else {
...
dontmanage.ui.form.ControlReadOnly = dontmanage.ui.form.ControlData;

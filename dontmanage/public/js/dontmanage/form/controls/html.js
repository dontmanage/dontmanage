dontmanage.ui.form.ControlHTML = class ControlHTML extends dontmanage.ui.form.Control {
	make() {
		super.make();
	...
	get_content() {
		...
		content = __(content);
		try {
			return dontmanage.render(content, this);
		} catch (e) {
			return content;
		}
	}
	...
	_set_html(html) {
		...
		if (typeof html === "string" && /<pre[\s>]/i.test(html)) {
			dontmanage.utils.highlight_pre(this.$wrapper);
		}
	}
};

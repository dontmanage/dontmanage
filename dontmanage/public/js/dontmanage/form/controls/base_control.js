dontmanage.ui.form.Control = class BaseControl {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make_wrapper() {
		this.$wrapper = $("<div class='dontmanage-control'></div>").appendTo(this.parent);
	...
	get_status(explain) {
		...
		var status = dontmanage.perm.get_field_display_status(
			this.df,
			dontmanage.model.get_doc(this.doctype, this.docname),
		...
		if (
			...
			!this.only_input &&
			is_null(value) &&
			cint(dontmanage.boot.sysdefaults.hide_empty_read_only_fields) &&
	...
	show_translatable_button(value) {
		...
		if (
			...
			!dontmanage.model.can_write("Translation") ||
			!value
		)
			return;

		// Disable translation in website
		if (!dontmanage.views || !dontmanage.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on("click", () => {
				if (!this.doc.__islocal) {
					new dontmanage.views.TranslationManager({
						df: this.df,

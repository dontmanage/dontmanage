...
dontmanage.ui.form.ControlLink = class ControlLink extends dontmanage.ui.form.ControlData {
		...
		this.$input.on("focus", function () {
			setTimeout(function () {
				if (me.$input.val() && me.get_options()) {
					...
					me.$link.toggle(true);
					me.$link_open.attr("href", dontmanage.utils.get_form_link(doctype, name));
				}
	...
	parse_validate_and_set_in_model(value, e, label) {
		...
		if (label) {
			...
			dontmanage.utils.add_link_title(this.df.options, value, label);
		}
	...
	new_doc() {
		...
		dontmanage._from_link = dontmanage.utils.deep_clone(this);
		dontmanage._from_link_scrollY = $(document).scrollTop();
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let docfield = dontmanage.meta.get_docfield(doctype, fieldname);
			let label = docfield ? docfield.label : dontmanage.model.unscrub(fieldname);
	...
	validate_link_and_fetch(df, options, docname, value) {
		...
		if (value) {
			return dontmanage
				.xcall("dontmanage.client.validate_link", {
					doctype: options,
					docname: value,

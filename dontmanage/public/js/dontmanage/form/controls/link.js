...
dontmanage.ui.form.recent_link_validations = {};

dontmanage.ui.form.ControlLink = class ControlLink extends dontmanage.ui.form.ControlData {
		...
		$(`<div class="link-field ui-front" style="position: relative;">
			...
			<span class="link-btn">
				<a class="btn-open" tabIndex='-1' style="display: inline-block;" title="${__("Open Link")}">
					${dontmanage.utils.icon("arrow-right", "xs")}
				</a>
			</span>
	...
	show_link_and_clear_buttons() {
		if (this.$input.val() && this.get_options()) {
			...
			this.$link_open.attr("href", dontmanage.utils.get_form_link(doctype, name));
		}
	}
	...
	get_reference_doctype() {
		...
		if (this.doctype) return this.doctype;
			...
			return dontmanage.get_route && dontmanage.get_route()[0] === "List"
				? dontmanage.get_route()[1]
				: null;
		}
	}
	setup_buttons() {
	...
	parse_validate_and_set_in_model(value, e, label) {
		...
		if (label) {
			...
			dontmanage.utils.add_link_title(this.get_options(), value, label);
		}
	...
	new_doc() {
		...
		if (df && df.get_route_options_for_new_doc) {
			dontmanage.route_options = df.get_route_options_for_new_doc(this);
		} else {
			dontmanage.route_options = {};
		}

		// partially entered name field
		dontmanage.route_options.name_field = this.get_label_value();

		// reference to calling link
		dontmanage._from_link = {
			field_obj: this,
			doc: this.doc,
		...
		dontmanage.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete() {
		...
		this.awesomplete = new Awesomplete(me.input, {
			...
			item: function (item) {
				...
				let _label = dontmanage.utils.escape_html(me.get_translated(d.label));
				...
				if (
					...
					html +=
						'<br><span class="small">' +
						__(dontmanage.utils.escape_html(d.description)) +
						"</span>";
				}
				return $(`<div role="option">`)
					...
					.html(`<p title="${dontmanage.utils.escape_html(_label)}">${html}</p>`)
					.get(0);
			},
			sort: function () {
				return 0;
		...
		this._debounced_input_handler = dontmanage.utils.debounce(this.on_input.bind(this), 500);
		...
		this.$input.on("awesomplete-select", function (e) {
			...
			if (me.df.remember_last_selected_value) {
				dontmanage.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	on_input(e) {
		...
		dontmanage.call({
			...
			method: "dontmanage.desk.search.search_link",
			no_spinner: true,
			cache: use_get,
			args: args,
			callback: async (r) => {
				...
				if (!this.df.only_select) {
					...
					let custom__link_options =
						dontmanage.ui.form.ControlLink.link_options &&
						dontmanage.ui.form.ControlLink.link_options(this);
				...
				r.message.forEach((item) => {
					dontmanage.utils.add_link_title(doctype, item.value, item.label);
				});
			},
		});
	}
	...
	toggle_href(doctype) {
		if (dontmanage.model.can_select(doctype) && !dontmanage.model.can_read(doctype)) {
	...
	async get_filter_description(filters) {
		...
		function formatValueForDisplay(docfield, val) {
			...
			// Numbers, dates, etc. - not translated, not quoted
			return dontmanage.format(val, docfield || {}, { inline: true });
		}
		...
		async function describe_filter(filter) {
			...
			await dontmanage.model.with_doctype(_doctype, () => {});

			const docfield = dontmanage.meta.get_docfield(_doctype, fieldname);
			const label = docfield ? docfield.label : dontmanage.model.unscrub(fieldname);
		...
		const filter_string = dontmanage.utils.comma_and(descriptions);
	...
	set_custom_query(args) {
		const is_valid_value = (value, key) => {
			...
			if (this.frm) {
				let field = dontmanage.meta.get_docfield(this.frm.doctype, key);
	...
	parse_filters(link_filters) {
		...
		link_filters.forEach((filter) => {
			...
			if (value?.startsWith?.("eval:")) {
				...
				value = dontmanage.utils.eval(value, context);
			}
	...
	validate_link_and_fetch(value) {
		...
		return dontmanage
			.xcall(
				"dontmanage.client.validate_link_and_fetch",
				{
					...args,
					docname: value,

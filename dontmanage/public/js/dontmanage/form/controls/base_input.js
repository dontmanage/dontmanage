dontmanage.ui.form.ControlInput = class ControlInput extends dontmanage.ui.form.Control {
	...
	make_wrapper() {
		if (this.only_input) {
			this.$wrapper = $('<div class="form-group dontmanage-control">').appendTo(this.parent);
		} else {
			this.$wrapper = $(
				`<div class="dontmanage-control">
	...
	refresh_input() {
		...
		if (me.disp_status != "None") {
			// refresh value
			if (me.frm) {
				me.value = dontmanage.model.get_value(me.doctype, me.docname, me.df.fieldname);
			} else if (me.doc) {
				...
				if (is_fetch_from_read_only) {
					$(me.disp_area).attr(
						...
						__(
							...
							me.df.fetch_from.split(".").map((value) => __(dontmanage.unscrub(value)))
						)
					);
				}
			}
	...
	set_disp_area(value) {
		...
		if (
			...
			value = dontmanage.utils.escape_html(value);
		}
		let doc = this.doc || (this.frm && this.frm.doc);
		let display_value = dontmanage.format(value, this.df, { no_icon: true, inline: true }, doc);
	...
	set_doc_url() {
		...
		$help.empty();
		$(`<a
			href="${dontmanage.utils.escape_html(this.df.documentation_url)}"
			target="_blank"
			title="${dontmanage.utils.escape_html(__("Documentation"))}"
		>

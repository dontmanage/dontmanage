...
dontmanage.ui.form.ControlIcon = class ControlIcon extends dontmanage.ui.form.ControlData {
	make_input() {
	...
	make_icon_input() {
		...
		if (!this.selected_icon) {
			this.selected_icon = $(
				`<div class="selected-icon">${dontmanage.utils.icon("folder-normal", "md")}</div>`
			);

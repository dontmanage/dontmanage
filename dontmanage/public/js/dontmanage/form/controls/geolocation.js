...
dontmanage.ui.form.ControlGeolocation = class ControlGeolocation extends dontmanage.ui.form.ControlData {
	static horizontal = false;

	async make() {
		await dontmanage.require(this.required_libs);
		super.make();
	}
	...
	make_wrapper() {
		...
		this.map_id = dontmanage.dom.get_unique_id();
		this.map_area = $(
		...
		if (this.frm) {
			...
			$(document).on("dontmanage.ui.Dialog:shown", () => {
				this.make_map();
			});
		}
	}
	...
	make_map() {
		...
		this.map.setView(dontmanage.utils.map_defaults.center, dontmanage.utils.map_defaults.zoom);
	}
	...
	bind_leaflet_map() {
		...
		L.tileLayer(dontmanage.utils.map_defaults.tiles, dontmanage.utils.map_defaults.options).addTo(
			this.map
		);
	}
	...
	bind_leaflet_draw_control() {
		...
		var options = {
			...
			draw: {
				polyline: {
					shapeOptions: {
						color: dontmanage.ui.color.get("blue"),
						weight: 10,
					},
				},
				polygon: {
					...
					drawError: {
						color: dontmanage.ui.color.get("orange"), // Color the shape will turn when intersects
	...
	get required_libs() {
		return [
			"assets/dontmanage/js/lib/leaflet_easy_button/easy-button.css",
			"assets/dontmanage/js/lib/leaflet_control_locate/L.Control.Locate.css",
			"assets/dontmanage/js/lib/leaflet_draw/leaflet.draw.css",
			"assets/dontmanage/js/lib/leaflet/leaflet.css",
			"assets/dontmanage/js/lib/leaflet/leaflet.js",
			"assets/dontmanage/js/lib/leaflet_easy_button/easy-button.js",
			"assets/dontmanage/js/lib/leaflet_draw/leaflet.draw.js",
			"assets/dontmanage/js/lib/leaflet_control_locate/L.Control.Locate.js",
		];
	}
};

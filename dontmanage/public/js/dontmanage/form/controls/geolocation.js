...
dontmanage.ui.form.ControlGeolocation = class ControlGeolocation extends dontmanage.ui.form.ControlData {
	...
	bind_leaflet_map() {
		...
		this.streetLayer = L.tileLayer(
			dontmanage.utils.map_defaults.tiles.default_tile.url,
			dontmanage.utils.map_defaults.tiles.default_tile.options
		);
		this.satelliteLayer = L.tileLayer(
			dontmanage.utils.map_defaults.tiles.satellite_tile.url,
			dontmanage.utils.map_defaults.tiles.satellite_tile.options
		);
		this.labelsLayer = L.tileLayer(
			dontmanage.utils.map_defaults.tiles.labels_tail.url,
			dontmanage.utils.map_defaults.tiles.labels_tail.options
		);
		this.terrainLayer = L.tileLayer(
			dontmanage.utils.map_defaults.tiles.terrain_lines_tail.url,
			dontmanage.utils.map_defaults.tiles.terrain_lines_tail.options
		);
			...
			if (e.name === "Satellite") {
				...
				Object.values(this.map._layers).forEach((layer) => {
					if (
						...
						(layer._url === dontmanage.utils.map_defaults.tiles.labels_tail.url ||
							layer._url === dontmanage.utils.map_defaults.tiles.terrain_lines_tail.url)
					) {

dontmanage.ui.form.ControlCode = class ControlCode extends dontmanage.ui.form.ControlText {
	make_input() {
	...
	setup_copy_button() {
		...
		this.copy_button.on("click", () => {
			dontmanage.utils.copy_to_clipboard(this.get_model_value() || this.get_value());
		});
	...
	set_language() {
		...
		this.editor.setKeyboardHandler(
			`ace/keyboard/${dontmanage.boot.user.code_editor_type || "vscode"}`
		);
	}
	...
	load_lib() {
		...
		if (dontmanage.boot.developer_mode) {
			this.root_lib_path = "/assets/dontmanage/node_modules/ace-builds/src-noconflict/";
		} else {
			this.root_lib_path = "/assets/dontmanage/node_modules/ace-builds/src-min-noconflict/";
		}

		this.library_loaded = new Promise((resolve) => {
			dontmanage.require(this.root_lib_path + "ace.js", () => {

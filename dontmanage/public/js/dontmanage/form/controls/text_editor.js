...
import ImageResize from "dontmanage-quill-image-resize";
...
dontmanage.ui.form.ControlTextEditor = class ControlTextEditor extends dontmanage.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on(
			"text-change",
			dontmanage.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on("keydown", (e) => {
			const key = dontmanage.ui.keys && dontmanage.ui.keys.get_key(e);
	...
	get_mention_options() {
		...
		return {
			...
			source: dontmanage.utils.debounce(async function (search_term, renderList) {
				let method =
					me.mention_search_method || "dontmanage.desk.search.get_names_for_mentions";
				let values = await dontmanage.xcall(method, {
					search_term,
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? dontmanage.utils.icon("users") : ""}`;
			},
		};
	}
	...
	parse(value) {
		...
		return dontmanage.dom.remove_script_and_style(value);
	}

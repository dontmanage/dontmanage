...
dontmanage.Application = class Application {
	...
	startup() {
		...
		dontmanage.broadcast.emit("boot", {
			csrf_token: dontmanage.csrf_token,
			user: dontmanage.session.user,
		});
	}

	make_sidebar() {
		this.sidebar = new dontmanage.ui.Sidebar({});
	}
	...
	set_route() {
		...
		dontmanage.router.on("change", () => {
			$(".tooltip").hide();
			if (dontmanage.dontmanage_toolbar && dontmanage.is_mobile()) dontmanage.dontmanage_toolbar.show_app_logo();
		});
	}
	...
	load_bootinfo() {
		if (dontmanage.boot) {
			...
			dontmanage.boot.setup_complete = dontmanage.boot.sysdefaults["setup_complete"];
	...
	setup_workspaces() {
		...
		dontmanage.boot.allowed_workspaces = dontmanage.boot.workspaces.pages;
	...
	show_update_available() {
		if (!dontmanage.boot.has_app_updates) return;
		dontmanage.xcall("dontmanage.utils.change_log.show_update_popup");
	}
		...
		if (dontmanage.boot.notes.length) {
			dontmanage.boot.notes.forEach(function (note) {
				if (!note.seen || note.notify_on_every_login) {
					var d = new dontmanage.ui.Dialog({ content: note.content, title: note.title });
					d.keep_open = true;
					...
					d.onhide = function () {
						...
						if (!note.notify_on_every_login) {
							...
							dontmanage.call({
								method: "dontmanage.desk.doctype.note.note.reset_notes",
							});
						}
					};
					d.show();
	...
	setup_broadcast_listeners() {
		...
		dontmanage.broadcast.on("boot", ({ csrf_token, user }) => {
			if (user && user != dontmanage.session.user) {
				dontmanage.msgprint({
					message: __(
			...
			if (csrf_token) {
				...
				dontmanage.csrf_token = csrf_token;
			}
		});
	}

...
dontmanage.re_route = { "#login": "" };
dontmanage.route_titles = {};
dontmanage.route_flags = {};
dontmanage.route_history = [];
dontmanage.view_factory = {};
dontmanage.view_factories = [];
dontmanage.route_options = null;
dontmanage.open_in_new_tab = false;
dontmanage.route_hooks = {};
...
$("body").on("click", "a", function (e) {
	...
	if (dontmanage.router.show_external_link_warning_if_needed(target_element)) {
		e.preventDefault();
...
dontmanage.router = {
	...
	setup() {
		...
		for (let doctype of dontmanage.boot.user.can_read) {
			this.routes[this.slug(doctype)] = { doctype: doctype };
		}
		if (dontmanage.boot.doctype_layouts) {
			for (let doctype_layout of dontmanage.boot.doctype_layouts) {
	...
	async convert_to_standard_route(route) {
		...
		if (dontmanage.workspaces[route[0]]) {
			// public workspace
			route = ["Workspaces", dontmanage.workspaces[route[0]].name];
		} else if (route[0] == "private") {
			// private workspace
			let private_workspace = route[1] && `${route[1]}-${dontmanage.user.name.toLowerCase()}`;
			if (!dontmanage.workspaces[private_workspace]) {
				dontmanage.msgprint(
					__("Workspace <b>{0}</b> does not exist", [
						dontmanage.utils.xss_sanitise(route[1]),
					])
				);
				return ["Workspaces"];
			}
			route = ["Workspaces", "private", dontmanage.workspaces[private_workspace].name];
	...
	set_doctype_route(route) {
		...
		return dontmanage.model.with_doctype(doctype_route.doctype).then(() => {
			// doctype route
			let meta = dontmanage.get_meta(doctype_route.doctype);
			this.meta = meta;
			...
			} else if (dontmanage.model.is_single(doctype_route.doctype)) {
	...
	set_history() {
		dontmanage.route_history.push(this.current_route);
	...
	render() {
		if (this.current_route[0]) {
			...
			// Show home
			dontmanage.views.pageview.show("");
		}
	},
	...
	render_page() {
		...
		const factory = dontmanage.utils.to_title_case(route[0]);
		if (route[1] && dontmanage.views[factory + "Factory"]) {
			route[0] = factory;
			// has a view generator, generate!
			if (!dontmanage.view_factory[factory]) {
				dontmanage.view_factory[factory] = new dontmanage.views[factory + "Factory"]();
			}

			dontmanage.view_factory[factory].show();
		} else {
			// show page
			const route_name = dontmanage.utils.xss_sanitise(route[0]);
			if (dontmanage.views.pageview) {
				dontmanage.views.pageview.show(route_name);
			}
		}
	},

	re_route(sub_path) {
		if (dontmanage.re_route[sub_path] !== undefined) {
			...
			const re_route_val = this.get_sub_path(dontmanage.re_route[sub_path]);
			if (re_route_val === this.current_sub_path) {
				window.history.back();
			} else {
				dontmanage.set_route(re_route_val);
			}
	...
	set_title(sub_path) {
		if (dontmanage.route_titles[sub_path]) {
			dontmanage.utils.set_title(dontmanage.route_titles[sub_path]);
		}
	},

	set_route() {
		// set the route (push state) with given arguments
		// example 1: dontmanage.set_route('a', 'b', 'c');
		// example 2: dontmanage.set_route(['a', 'b', 'c']);
		// example 3: dontmanage.set_route('a/b/c');
		...
		return new Promise((resolve) => {
			...
			if (dontmanage.open_in_new_tab) {
				...
				try {
					const route_options = dontmanage.route_options || {};
		...
		}).finally(() => (dontmanage.route_flags = {}));
	},

	get_route_from_arguments(route) {
		if (route.length === 1 && $.isArray(route[0])) {
			// called as dontmanage.set_route(['a', 'b', 'c']);
			route = route[0];
		}
		...
		if (view === "list") {
			if (route[2] && route[2] !== "list" && !$.isPlainObject(route[2])) {
				...
				if ($.isPlainObject(route[2])) {
					dontmanage.route_options = route[2];
				}
	...
	make_url(params) {
		...
		let private_home = `home-${dontmanage.user.name.toLowerCase()}`;
		let default_workspace = dontmanage.router.slug(dontmanage.boot.user.default_workspace?.name || "");

		let workspace =
			dontmanage.workspaces[default_workspace] ||
			dontmanage.workspaces[private_home] ||
			dontmanage.workspaces["home"] ||
			Object.values(dontmanage.workspace_map).find((w) => w.app === dontmanage.current_app) ||
			Object.values(dontmanage.workspaces)[0];

		if (workspace) {
			return (
				"/desk/" +
				(workspace.public ? "" : "private/") +
				dontmanage.router.slug(workspace.name)
			);
		}
	...
	push_state(path, query_params = "") {
		if (window.location.pathname !== path || window.location.search !== query_params) {
			...
			const method = dontmanage.route_flags.replace_route ? "replaceState" : "pushState";
	...
	set_route_options_from_url() {
		// set query parameters as dontmanage.route_options
		...
		if (localStorage.getItem("route_options")) {
			dontmanage.route_options = JSON.parse(localStorage.getItem("route_options"));
	...
	show_external_link_warning_if_needed(/** @type {HTMLAnchorElement} */ aElement) {
		try {
			...
			const showWarningWhen = dontmanage.boot.show_external_link_warning || "Never";
				...
				fields: [
					...
					},
					{
						...
						label: __("Do not warn me again about {0}", [
							dontmanage.utils.escape_html(hostname).bold(),
						]),
						default: 0,
			...
			const skipToken = dontmanage.utils.get_random(16);
...
dontmanage.get_route = () => dontmanage.router.current_route;
dontmanage.get_route_str = () => dontmanage.router.current_route.join("/");
dontmanage.set_route = function () {
	return dontmanage.router.set_route.apply(dontmanage.router, arguments);
};

dontmanage.get_prev_route = function () {
	if (dontmanage.route_history && dontmanage.route_history.length > 1) {
		return dontmanage.route_history[dontmanage.route_history.length - 2];
	} else {
		return [];
	}
};

dontmanage.set_re_route = function () {
	var tmp = dontmanage.router.get_sub_path();
	dontmanage.set_route.apply(null, arguments);
	dontmanage.re_route[tmp] = dontmanage.router.get_sub_path();
};

dontmanage.has_route_options = function () {
	return Boolean(Object.keys(dontmanage.route_options || {}).length);
};

dontmanage.utils.make_event_emitter(dontmanage.router);

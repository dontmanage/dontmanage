...
export default class GoogleDrivePicker {
	...
	loadGapi() {
		...
		if (!dontmanage.boot.user.google_drive_token) {
	...
	handleAuthResult(authResult) {
		...
		if (authResult && !authResult.error) {
			dontmanage.boot.user.google_drive_token = authResult.access_token;
			this.createPicker();
		} else {
			let error = error_map[authResult.error] || __("Google Authentication Error");
			dontmanage.throw(error);
		}
	}
	...
	createPicker() {
		...
		if (this.pickerApiLoaded && dontmanage.boot.user.google_drive_token) {
			...
			this.picker = new google.picker.PickerBuilder()
				...
				.setOAuthToken(dontmanage.boot.user.google_drive_token)
				.addView(this.view)
				.setLocale(dontmanage.boot.lang)

class PulseProvider {
	...
	is_enabled() {
		return dontmanage.boot.telemetry_provider?.includes("pulse") && dontmanage.boot.enable_telemetry;
	}
	...
	sendEvents(events) {
		...
		return new Promise((resolve, reject) => {
			try {
				dontmanage.call({
					method: "dontmanage.utils.telemetry.pulse.client.bulk_capture",
					args: { events },
					type: "POST",
					no_spinner: true,
					freeze: false,
	...
	sendBeacon(events) {
		try {
			if (navigator.sendBeacon) {
				const url = "/api/method/dontmanage.utils.telemetry.pulse.client.bulk_capture";

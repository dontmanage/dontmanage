...
function toggle_edit_letterhead() {
	...
	if (!control.value) {
		control.value = dontmanage.ui.form.make_control({
			parent: editor.value,
			df: {
...
// mounted
onMounted(() => {
	if (!letterhead.value && dontmanage.boot.sysdefaults.letter_head) {
		set_letterhead(dontmanage.boot.sysdefaults.letter_head);
	}

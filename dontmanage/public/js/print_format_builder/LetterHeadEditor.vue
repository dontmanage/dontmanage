...
export default {
	...
	mounted() {
		if (!this.letterhead && dontmanage.boot.sysdefaults.letter_head) {
			this.set_letterhead(dontmanage.boot.sysdefaults.letter_head);
		}
	...
	methods: {
		toggle_edit_letterhead() {
			...
			if (!this.control) {
				this.control = dontmanage.ui.form.make_control({

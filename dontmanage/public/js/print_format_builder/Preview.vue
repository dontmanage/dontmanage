...
export default {
	...
	mounted() {
		this.doc_select = dontmanage.ui.form.make_control({
		...
		this.preview_type = dontmanage.ui.form.make_control({
	...
	methods: {
		...
		get_default_docname() {
			return dontmanage.db.get_list(this.doctype, { limit: 1 }).then(doc => {
	...
	computed: {
		...
		url() {
			...
			let url =
				this.type == "PDF"
					? `/api/method/dontmanage.utils.weasyprint.download_pdf`
					: "/printpreview";

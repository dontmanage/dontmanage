...
export const useStore = defineStore("workflow-builder-store", () => {
	...
	async function fetch() {
		await dontmanage.model.clear_doc("Workflow", workflow_name.value);
		await dontmanage.model.with_doc("Workflow", workflow_name.value);

		workflow_doc.value = dontmanage.get_doc("Workflow", workflow_name.value);
		await dontmanage.model.with_doctype(workflow_doc.value.document_type);

		if (!workflowfields.value.length) {
			await dontmanage.model.with_doctype("Workflow");
			workflowfields.value = dontmanage.get_meta("Workflow").fields;
		}

		if (!statefields.value.length) {
			await dontmanage.model.with_doctype("Workflow Document State");
			statefields.value = dontmanage.get_meta("Workflow Document State").fields;
		}

		if (!transitionfields.value.length) {
			await dontmanage.model.with_doctype("Workflow Transition");
			transitionfields.value = dontmanage.get_meta("Workflow Transition").fields;
		}

		if (!workflow_doc_fields.value.length) {
			let doc_type = workflow_doc.value.document_type;
			await dontmanage.model.with_doctype(doc_type);
			workflow_doc_fields.value = dontmanage.meta
				.get_docfields(doc_type, null, {
					fieldtype: ["not in", dontmanage.model.no_value_type],
				})
				.sort((a, b) => {
					if (a.label && b.label) {
	...
	async function save_changes() {
		dontmanage.dom.freeze(__("Saving..."));
		...
		try {
			...
			await dontmanage.call("dontmanage.client.save", { doc });
			dontmanage.toast(__("Workflow updated successfully"));
			fetch();
		} catch (e) {
			console.error(e);
		} finally {
	...
	function setup_breadcrumbs() {
		...
		`;
		dontmanage.breadcrumbs.clear();
		dontmanage.breadcrumbs.$breadcrumbs.append(breadcrumbs);
	}

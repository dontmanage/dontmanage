...
function duplicate_field() {
	...
	duplicate_field.df.owner = dontmanage.session.user;
...
function make_dialog(frm) {
	frm.dialog = new dontmanage.ui.Dialog({
	...
	if (frm.doctype === "Customize Form") {
		...
		dontmanage.db.exists("Property Setter", property_setter_id).then((exits) => {
			if (exits) {
				frm.dialog.set_secondary_action_label(__("Reset To Default"));
				frm.dialog.set_secondary_action(() => {
					dontmanage.call({
						method: "dontmanage.custom.doctype.customize_form.customize_form.get_link_filters_from_doc_without_customisations",
						args: {
...
function edit_filters() {
	...
	dontmanage.model.with_doctype(field_doctype, () => {
		frm.dialog.show();
		add_existing_filter(frm, props.field.df);
	});
}
			...
			<template #label>
				<div class="field-label">
					...
					<div
						...
						v-html="dontmanage.utils.icon('help', 'sm')"
					/>
				</div>
			</template>
			<template #actions>
				<div class="field-actions" :hidden="store.read_only">
					<button
						...
						@click="edit_filters"
					>
						<div v-html="dontmanage.utils.icon('filter', 'sm')" />
					</button>
					<AddFieldButton ref="add_field_ref" :column="column" :field="field">
						<div v-html="dontmanage.utils.icon('add', 'sm')" />
					</AddFieldButton>
					<button
					...
					>
						<div v-html="dontmanage.utils.icon('move', 'sm')" />
					</button>
					<button
						class="btn btn-xs btn-icon"
					...
					>
						<div v-html="dontmanage.utils.icon('duplicate', 'sm')" />
					</button>
					<button
					...
					>
						<div v-html="dontmanage.utils.icon('external-link', 'sm')" />
					</button>
					<button
						class="btn btn-xs btn-icon"
					...
					>
						<div v-html="dontmanage.utils.icon('remove', 'sm')" />
					</button>
				</div>
			</template>
		</component>

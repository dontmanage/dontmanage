...
let table_columns = computedAsync(async () => {
	...
	if (!doctype) return [];
	if (!dontmanage.get_meta(doctype)) {
		await load_doctype_model(doctype);
	}
	let child_doctype = dontmanage.get_meta(doctype);
...
function open_new_child_doctype_dialog() {
	...
	dontmanage.model.with_doctype("DocType").then(() => {
		dontmanage.listview_settings["DocType"].new_doctype_dialog({ is_child: 1, is_custom });
	});
}
</script>

...
if (props.df.fieldtype === "Datetime") {
	let time_zone_text = dontmanage.boot.time_zone
		? dontmanage.boot.time_zone.user
		: dontmanage.sys_defaults.time_zone;
...
<template>
	<div class="control dontmanage-control" :class="{ editable: slots.label }">
		<!-- label -->
		...
		<div
			...
			v-html="dontmanage.utils.icon('folder-normal', 'md')"
		/>
		<!-- phone selector icon -->
		<div
			v-if="df.fieldtype == 'Phone'"
			class="selected-phone no-value"
			v-html="dontmanage.utils.icon('down', 'sm')"
		/>
	</div>
</template>

let dontmanageCloudBaseEndpoint = "https://dontmanagecloud.com";
let isFCUser = false;

$(document).ready(function () {
	if (
		dontmanage.boot.is_fc_site &&
		!!dontmanage.boot.setup_complete &&
		!dontmanage.is_mobile() &&
		dontmanage.user.has_role("System Manager")
	) {
		dontmanage.call({
			method: "dontmanage.integrations.dontmanage_providers.dontmanagecloud_billing.current_site_info",
			callback: (r) => {
				if (!r?.message) return;

				const response = r.message;
				const trial_end_date = new Date(response.trial_end_date);
				dontmanageCloudBaseEndpoint = response.base_url;
...
function addManageBillingDropdown() {
	$(document).on("desktop_screen", function (event, data) {
		data.desktop.add_menu_item({
			...
			icon: "receipt-text",
			condition: function () {
				return dontmanage.boot.sysdefaults.demo_company;
			},
			onClick: function () {
				return openDontManageCloudDashboard();
			},
		});
	});
}
function openDontManageCloudDashboard() {
	window.open(`${dontmanageCloudBaseEndpoint}/dashboard/sites/${dontmanage.boot.sitename}`, "_blank");
}

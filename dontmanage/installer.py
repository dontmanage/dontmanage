...
import dontmanage
from dontmanage.defaults import _clear_cache
from dontmanage.utils import cint, is_git_url
...
def _is_scheduler_enabled(site) -> bool:
	...
	try:
		...
		dontmanage.connect()
		enable_scheduler = cint(dontmanage.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
		dontmanage.db.close()
...
def _new_site(
	...
	"""Install a new DontManage site"""

	from dontmanage.utils import scheduler
	...
	if not db_name:
		db_name = f"_{dontmanage.generate_hash(length=16)}"
	...
	if rollback_callback:
		rollback_callback.add(lambda: shutil.rmtree(dontmanage.get_site_path()))
	...
	with filelock("bench_new_site", timeout=1):
		...
		apps_to_install = ["dontmanage"] + (dontmanage.conf.get("install_apps") or []) + (list(install_apps or []))
	...
	scheduler_status = "disabled" if dontmanage.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from dontmanage.database import bootstrap_database, drop_user_and_database, setup_database
	...
	dontmanage.flags.in_install_db = True

	if root_login:
		dontmanage.flags.root_login = root_login

	if root_password:
		dontmanage.flags.root_password = root_password
	...
	dontmanage.conf.admin_password = dontmanage.conf.admin_password or admin_password

	remove_missing_apps()

	dontmanage.db.create_auth_table()
	dontmanage.db.create_global_search_table()
	dontmanage.db.create_user_settings_table()

	dontmanage.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	find_org()
	ref -> https://github.com/dontmanage/bench/blob/develop/bench/utils/__init__.py#L390
	...
	from dontmanage.exceptions import InvalidRemoteException
...
def fetch_details_from_tag(_tag: str) -> tuple[str, str, str]:
	...
	ref -> https://github.com/dontmanage/bench/blob/develop/bench/utils/__init__.py#L403
...
def parse_app_name(name: str) -> str:
	...
	ref -> https://github.com/dontmanage/bench/blob/develop/bench/app.py#L114
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	...
	from dontmanage.model.sync import sync_for
	from dontmanage.modules.utils import sync_customizations
	from dontmanage.utils.fixtures import sync_fixtures

	dontmanage.flags.in_install = name
	dontmanage.flags.ignore_in_install = False

	dontmanage.clear_cache()
	app_hooks = dontmanage.get_hooks(app_name=name)
	installed_apps = dontmanage.get_installed_apps()
	...
	if name not in dontmanage.get_all_apps():
	...
	other_class_overrides = dontmanage.get_hooks("override_doctype_class")
	if (
	...
	for before_install in app_hooks.before_install or []:
		out = dontmanage.get_attr(before_install)()
		if out is False:
			return

	for fn in dontmanage.get_hooks("before_app_install"):
		dontmanage.get_attr(fn)(name)
	...
	dontmanage.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		dontmanage.get_attr(after_install)()

	for fn in dontmanage.get_hooks("after_app_install"):
		dontmanage.get_attr(fn)(name)
	...
	for after_sync in app_hooks.after_sync or []:
		dontmanage.get_attr(after_sync)()  #

	dontmanage.clear_cache()
	dontmanage.client_cache.erase_persistent_caches()
	dontmanage.flags.in_install = False
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		dontmanage.db.set_global("installed_apps", json.dumps(installed_apps))
		dontmanage.db.commit()
...
def remove_from_installed_apps(app_name):
	...
	if app_name in installed_apps:
		...
		dontmanage.local.doc_events_hooks = None
		dontmanage.get_single("Installed Applications").update_versions()
		dontmanage.db.commit()
		if dontmanage.flags.in_install:
			post_install()
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = dontmanage.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in dontmanage.get_installed_apps():
	...
	for app in dontmanage.get_installed_apps():
		if app != app_name:
			hooks = dontmanage.get_hooks(app_name=app)
	...
	if not (dry_run or no_backup):
		from dontmanage.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	dontmanage.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		dontmanage.get_attr(before_uninstall)()

	for fn in dontmanage.get_hooks("before_app_uninstall"):
		dontmanage.get_attr(fn)(app_name)

	modules = dontmanage.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	for after_uninstall in app_hooks.after_uninstall or []:
		dontmanage.get_attr(after_uninstall)()

	for fn in dontmanage.get_hooks("after_app_uninstall"):
		dontmanage.get_attr(fn)(app_name)

	dontmanage.client_cache.erase_persistent_caches()

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	dontmanage.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in dontmanage.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					dontmanage.delete_doc(doctype.name, doctype.name, ignore_on_trash=True, force=True)
					dontmanage.delete_doc("DocType", doctype.name, ignore_on_trash=True, force=True)
				else:
		...
		if not dry_run:
			dontmanage.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(module_name: str, doctype_linkfield_map: dict[str, str], dry_run: bool) -> None:
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in dontmanage.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				dontmanage.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = dontmanage.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if dontmanage.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			dontmanage.delete_doc("DocType", doctype, ignore_on_trash=True, force=True)
			dontmanage.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from dontmanage.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		dontmanage.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	dontmanage.db.commit()


def init_singles():
	singles = dontmanage.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if dontmanage.db.get_singles_dict(single):
			continue

		try:
			doc = dontmanage.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, dontmanage.DoesNotExistError):
...
def make_conf(
	...
	sites_path = dontmanage.local.sites_path
	dontmanage.destroy()
	dontmanage.init(site, sites_path=sites_path)
...
def make_site_config(
	...
	dontmanage.create_folder(os.path.join(dontmanage.local.site_path))
...
def update_site_config(key, value, validate=True, site_config_path=None):
	...
	from dontmanage.config import clear_site_config_cache
	from dontmanage.utils.synchronization import filelock
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(dontmanage.local, "conf"):
		dontmanage.local.conf[key] = value


def get_site_config_path():
	return os.path.join(dontmanage.local.site_path, "site_config.json")
...
def clear_site_locks():
	...
	path = Path(dontmanage.get_site_path("locks"))
...
def make_site_dirs():
	for dir_path in [
		...
		path = dontmanage.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = dontmanage.get_module_list(app)
	for module in modules:
		d = dontmanage.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	apps = ("dontmanage_subscription", "shopping_cart")
	installed_apps = json.loads(dontmanage.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_files(site_name, file_path):
	...
	from dontmanage.utils import get_bench_relative_path

	file_path = get_bench_relative_path(file_path)

	# Need to do dontmanage.init to maintain the site locals
	dontmanage.init(site_name)
	abs_site_path = os.path.abspath(dontmanage.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	...
	current_version = Version(dontmanage.__version__)
	...
	if verbose and is_downgrade:
		print(f"Your site is currently on DontManage {current_version} and your backup is {backup_version}.")

	return is_downgrade


def get_old_backup_version(sql_file_path: str) -> Version | None:
	"""Return the dontmanage version used to create the specified database dump.

	This methods supports older versions of DontManage wich used a different format.
	"""
	header = get_db_dump_header(sql_file_path).split("\n")
	if match := re.search(r"DontManage (\d+\.\d+\.\d+)", header[0]):
...
def get_backup_version(sql_file_path: str) -> Version | None:
	...
	header = get_db_dump_header(sql_file_path).split("\n")
	metadata = ""
	if "begin dontmanage metadata" in header[0]:
		for line in header[1:]:
			if "end dontmanage metadata" in line:
				break
			metadata += line.replace("--", "").strip() + "\n"
		parser = configparser.ConfigParser()
		parser.read_string(metadata)
		return Version(parser["dontmanage"]["version"])
...
def is_partial(sql_file_path: str) -> bool:
	...
	"""
	if dontmanage.conf.db_type == "sqlite":
		return False
...
def partial_restore(sql_file_path, verbose=False):
	if dontmanage.conf.db_type == "mariadb":
		from dontmanage.database.mariadb.setup_db import import_db_from_sql
	elif dontmanage.conf.db_type == "postgres":
		import warnings

		from dontmanage.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path: str, _raise: bool = True) -> None:
	...
	if os.path.getsize(path):
		if (executable := which(executable_name)) is None:
			dontmanage.throw(
				...
				exc=dontmanage.ExecutableNotFound,
			)
		try:
			dontmanage.utils.execute_in_shell(f"{executable} -m1 __Auth {path}", check_exit_code=True)
			return
		except Exception:
	...
	if _raise:
		raise dontmanage.InvalidDatabaseFile

...
from dontmanage.modules import get_module_name, load_doctype_module
...
def main(
	...
	if doctype_list_path:
		...
		with open(dontmanage.get_app_path(app, doctype_list_path)) as f:
	...
	try:
		dontmanage.flags.print_messages = verbose
		...
		scheduler_disabled_by_user = dontmanage.utils.scheduler.is_scheduler_disabled()
		if not scheduler_disabled_by_user:
			dontmanage.utils.scheduler.disable_scheduler()

		set_test_email_config()

		if not dontmanage.flags.skip_before_tests:
			if verbose:
				print('Running "before_tests" hooks')
			for fn in dontmanage.get_hooks("before_tests", app_name=app):
		...
		if doctype:
			...
			doctypes = dontmanage.db.get_list(
...
def run_all_tests(
	...
	for app in apps:
		for path, folders, files in os.walk(dontmanage.get_pymodule_path(app)):
...
def run_tests_for_doctype(
	...
	for doctype in doctypes:
		module = dontmanage.db.get_value("DocType", doctype, "module")
		if not module:
		...
		if force:
			for name in dontmanage.db.sql_list("select name from `tab%s`" % doctype):
				dontmanage.delete_doc(doctype, name, force=True)
...
def _run_unittest(
	...
	dontmanage.flags.tests_verbose = verbose
...
def _add_test(app, path, filename, verbose, test_suite=None, ui_tests=False):
	...
	app_path = dontmanage.get_pymodule_path(app)
...
def make_test_records(doctype, verbose=0, force=False, commit=False):
	...
	if dontmanage.flags.skip_test_records:
		return
	...
	for options in get_dependencies(doctype):
		...
		if options not in dontmanage.local.test_objects:
			dontmanage.local.test_objects[options] = []
...
def make_test_records_for_doctype(doctype, verbose=0, force=False, commit=False):
	...
	if hasattr(test_module, "_make_test_records"):
		dontmanage.local.test_objects[doctype] += test_module._make_test_records(verbose)

	elif hasattr(test_module, "test_records"):
		if doctype in dontmanage.local.test_objects:
			dontmanage.local.test_objects[doctype] += make_test_objects(
		...
		test_records = dontmanage.get_test_records(doctype)
		if test_records:
			dontmanage.local.test_objects[doctype] += make_test_objects(
...
def make_test_objects(doctype, test_records=None, verbose=None, reset=False, commit=False):
	...
	for doc in test_records:
		...
		if dontmanage.db.exists(d.doctype, d.name) and not reset:
...
def add_to_test_record_log(doctype):
	...
	if doctype not in test_record_log:
		dontmanage.flags.test_record_log.append(doctype)
		with open(dontmanage.get_site_path(".test_log"), "w") as f:
			f.write("\n".join(filter(None, dontmanage.flags.test_record_log)))


def get_test_record_log():
	"""Return the list of doctypes for which test records have been created"""
	if "test_record_log" not in dontmanage.flags:
		if os.path.exists(dontmanage.get_site_path(".test_log")):
			with open(dontmanage.get_site_path(".test_log")) as f:
				dontmanage.flags.test_record_log = f.read().splitlines()
		else:
			dontmanage.flags.test_record_log = []

	return dontmanage.flags.test_record_log

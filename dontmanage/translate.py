...
def get_language(lang_list: list | None = None) -> str:
	...
	return dontmanage.get_system_settings("language") or "en"
...
def get_user_lang(user: str | None = None) -> str:
	...
	return (
		dontmanage.get_cached_value("User", user, "language")
		or dontmanage.get_system_settings("language")
...
def get_lang_dict():
	...
	return dict(
		dontmanage.get_all("Language", fields=["language_name", "name"], order_by="creation", as_list=True)
	)
...
def get_messages_for_boot():
	...
	return get_all_translations(dontmanage.local.lang)
...
def get_translations_from_apps(lang, apps=None):
	...
	translations = {}
	from dontmanage.gettext.translate import get_translations_from_mo
...
def get_translations_from_csv(lang, app):
	return get_translation_dict_from_file(
		os.path.join(dontmanage.get_app_path(app, "translations"), lang + ".csv"), lang, app
	)
...
def get_messages_from_doctype(name):
	...
	from dontmanage.gettext.extractors.utils import is_translatable
...
def get_messages_from_page(name):
	"""Return all translatable strings from a :class:`dontmanage.core.doctype.Page`."""
	return _get_messages_from_page_or_report("Page", name)


def get_messages_from_report(name):
	"""Return all translatable strings from a :class:`dontmanage.core.doctype.Report`."""
	from dontmanage.gettext.extractors.utils import is_translatable
...
def get_messages_from_file(path: str) -> list[tuple[str, str, str | None, int]]:
	...
	from dontmanage.gettext.extractors.utils import extract_messages_from_code
...
def extract_messages_from_javascript_code(code: str) -> list[tuple[int, str, str | None]]:
	...
	messages = []
	from dontmanage.gettext.extractors.javascript import extract_javascript
...
def get_all_languages(with_language_name: bool = False) -> list:
	...
	if with_language_name:
		...
		languages = dontmanage.client_cache.get_value("languages")
		if not languages:
			languages = dontmanage.get_all("Language", filters={"enabled": 1}, pluck="name")
			dontmanage.client_cache.set_value("languages", languages)
		return languages

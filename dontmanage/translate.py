...
"""
	dontmanage.translate
	~~~~~~~~~~~~~~~~

	Translation tools for dontmanage
"""
...
import dontmanage
from dontmanage.model.utils import InvalidIncludePath, render_include
from dontmanage.query_builder import DocType, Field
from dontmanage.utils import cstr, get_bench_path, is_html, strip, strip_html_tags, unique
...
def get_language(lang_list: list = None) -> str:
	...
	"""
	is_logged_in = dontmanage.session.user != "Guest"

	# fetch language from form_dict
	if dontmanage.form_dict._lang:
		language = get_lang_code(dontmanage.form_dict._lang or get_parent_language(dontmanage.form_dict._lang))
		if language:
			return language
	...
	accept_language = list(dontmanage.request.accept_languages.values())
	...
	return dontmanage.db.get_default("lang") or "en"
...
def get_user_lang(user: str = None) -> str:
	...
	if not lang:
		...
		lang = (
			dontmanage.db.get_value("User", user, "language")
			or dontmanage.db.get_default("lang")
...
def get_lang_code(lang: str) -> str | None:
	return dontmanage.db.get_value("Language", {"name": lang}) or dontmanage.db.get_value(
...
def set_default_language(lang):
	...
	if dontmanage.db.get_default("lang") != lang:
		dontmanage.db.set_default("lang", lang)
	dontmanage.local.lang = lang


def get_lang_dict():
	"""Returns all languages in dict format, full name is the key e.g. `{"english":"en"}`"""
	return dict(
		dontmanage.get_all("Language", fields=["language_name", "name"], order_by="modified", as_list=True)
	)
...
def get_dict(fortype: str, name: str | None = None) -> dict[str, str]:
	...
	if asset_key not in translation_assets:
		...
		if fortype == "doctype":
			...
			apps = dontmanage.get_all_apps(True)
			for app in apps:
			...
			messages += get_messages_from_include_files()
			messages += (
				dontmanage.qb.from_("Print Format").select(PseudoColumn("'Print Format:'"), "name")
			).run()
			messages += (dontmanage.qb.from_("DocType").select(PseudoColumn("'DocType:'"), "name")).run()
			messages += dontmanage.qb.from_("Role").select(PseudoColumn("'Role:'"), "name").run()
			messages += (dontmanage.qb.from_("Module Def").select(PseudoColumn("'Module:'"), "name")).run()
			messages += (
				dontmanage.qb.from_("Workspace Shortcut")
				.where(Field("format").isnotnull())
				.select(PseudoColumn("''"), "format")
			).run()
			messages += (dontmanage.qb.from_("Onboarding Step").select(PseudoColumn("''"), "title")).run()
	...
	translation_map.update(get_user_translations(dontmanage.local.lang))
...
def get_messages_for_boot():
	...
	messages = get_all_translations(dontmanage.local.lang)
...
def make_dict_from_messages(messages, full_dict=None, load_user_translation=True):
	"""Returns translated messages as a dict in Language specified in `dontmanage.local.lang`
	...
	if full_dict is None:
		if load_user_translation:
			full_dict = get_all_translations(dontmanage.local.lang)
		else:
			full_dict = get_translations_from_apps(dontmanage.local.lang)
...
def get_lang_js(fortype: str, name: str) -> str:
	...
	"""
	return f"\n\n$.extend(dontmanage._messages, {json.dumps(get_dict(fortype, name))})"
...
def get_user_translations(lang):
	...
	def _read_from_db():
		user_translations = {}
		translations = dontmanage.get_all(
...
def get_messages_for_app(app, deduplicate=True):
	...
	messages = []
	modules = [dontmanage.unscrub(m) for m in dontmanage.local.app_modules[app]]
	...
	if modules:
		if isinstance(modules, str):
			modules = [modules]
		filtered_doctypes = (
			dontmanage.qb.from_("DocType").where(Field("module").isin(modules)).select("name").run(pluck=True)
		)
		...
		# pages
		filtered_pages = (
			dontmanage.qb.from_("Page").where(Field("module").isin(modules)).select("name", "title").run()
		)
...
def get_messages_from_navbar():
	...
	labels = dontmanage.get_all("Navbar Item", filters={"item_label": ("is", "set")}, pluck="item_label")
...
def get_messages_from_doctype(name):
	...
	if not meta.custom:
		doctype_file_path = dontmanage.get_module_path(meta.module, "doctype", meta.name, meta.name)
...
def get_messages_from_workflow(doctype=None, app_name=None):
	...
	workflows = []
	if doctype:
		workflows = dontmanage.get_all("Workflow", filters={"document_type": doctype})
	else:
		fixtures = dontmanage.get_hooks("fixtures", app_name=app_name) or []
		for fixture in fixtures:
			if isinstance(fixture, str) and fixture == "Worflow":
				workflows = dontmanage.get_all("Workflow")
				break
			elif isinstance(fixture, dict) and fixture.get("dt", fixture.get("doctype")) == "Workflow":
				workflows.extend(dontmanage.get_all("Workflow", filters=fixture.get("filters")))

	messages = []
	document_state = DocType("Workflow Document State")
	for w in workflows:
		states = dontmanage.db.get_values(
			document_state,
		...
		actions = dontmanage.db.get_values(
			"Workflow Transition",
...
def get_messages_from_custom_fields(app_name):
	...
	for fixture in fixtures:
		if isinstance(fixture, str) and fixture == "Custom Field":
			custom_fields = dontmanage.get_all(
...
def get_messages_from_page(name):
	"""Returns all translatable strings from a :class:`dontmanage.core.doctype.Page`"""
	return _get_messages_from_page_or_report("Page", name)


def get_messages_from_report(name):
	"""Returns all translatable strings from a :class:`dontmanage.core.doctype.Report`"""
	report = dontmanage.get_doc("Report", name)
	messages = _get_messages_from_page_or_report(
		"Report", name, dontmanage.db.get_value("DocType", report.ref_doctype, "module")
	)
...
def _get_messages_from_page_or_report(doctype, name, module=None):
	...
	doc_path = dontmanage.get_module_path(module, doctype, name)

	messages = get_messages_from_file(os.path.join(doc_path, dontmanage.scrub(name) + ".py"))
...
def get_server_messages(app):
	"""Extracts all translatable strings (tagged with :func:`dontmanage._`) from Python modules
	inside an app"""
	messages = []
...
def get_messages_from_include_files(app_name=None):
	...
	from dontmanage.utils.jinja_globals import bundled_asset

	messages = []
	app_include_js = dontmanage.get_hooks("app_include_js", app_name=app_name) or []
	web_include_js = dontmanage.get_hooks("web_include_js", app_name=app_name) or []
	include_js = app_include_js + web_include_js

	for js_path in include_js:
		file_path = bundled_asset(js_path)
		relative_path = os.path.join(dontmanage.local.sites_path, file_path.lstrip("/"))
...
def get_messages_from_file(path: str) -> list[tuple[str, str, str | None, int]]:
	...
	"""
	dontmanage.flags.setdefault("scanned_files", set())
	# TODO: Find better alternative
	# To avoid duplicate scan
	if path in dontmanage.flags.scanned_files:
		return []

	dontmanage.flags.scanned_files.add(path)
...
def extract_messages_from_code(code):
	...
	try:
		code = dontmanage.as_unicode(render_include(code))
...
def import_translations(lang, path):
	...
	for app in dontmanage.get_all_apps(True):
...
def rebuild_all_translation_files():
	...
	for lang in get_all_languages():
		for app in dontmanage.get_all_apps():
...
def send_translations(translation_dict):
	"""Append translated dict in `dontmanage.local.response`"""
	if "__messages" not in dontmanage.local.response:
		dontmanage.local.response["__messages"] = {}

	dontmanage.local.response["__messages"].update(translation_dict)
...
def update_translations_for_source(source=None, translation_dict=None):
	...
	translation_records = dontmanage.db.get_values(
...
def get_translator_url():
	return dontmanage.get_hooks()["translator_url"][0]
...
def get_all_languages(with_language_name=False):
	...
	def get_language_codes():
		return dontmanage.get_all("Language", filters={"enabled": 1}, pluck="name")

	def get_all_language_with_name():
		return dontmanage.get_all("Language", ["language_code", "language_name"], {"enabled": 1})
...
def get_preferred_language_cookie():
	return dontmanage.request.cookies.get("preferred_language")


def get_translated_doctypes():
	dts = dontmanage.get_all("DocType", {"translated_doctype": 1}, pluck="name")
...
def print_language(language: str):
	...
	_lang = dontmanage.local.lang
	_jenv = dontmanage.local.jenv

	# set language, empty any existing lang_full_dict and jenv
	dontmanage.local.lang = language
	dontmanage.local.jenv = None

	yield

	# restore original values
	dontmanage.local.lang = _lang
	dontmanage.local.jenv = _jenv

...
def set_field_property(filters, key, value):
	...
	docs = [
		dontmanage.get_doc("DocType", d.parent)
		for d in dontmanage.get_all("DocField", fields=["parent"], filters=filters)
	]
...
class InvalidIncludePath(dontmanage.ValidationError):
	pass
	...
	for _ in range(5):
		if "{% include" in content:
			...
			for path in paths:
				...
				with open(dontmanage.get_app_path(app, app_path), encoding="utf-8") as f:
					include = f.read()
...
def get_fetch_values(doctype, fieldname, value):
	...
	db_values = dontmanage.db.get_value(
...
def is_virtual_doctype(doctype: str):
	if dontmanage.flags.in_install or dontmanage.flags.in_migrate:
		return (
			dontmanage.db.get_value("DocType", doctype, "is_virtual")
			if dontmanage.db.has_column("DocType", "is_virtual")
			else False
		)

	return getattr(dontmanage.get_meta(doctype), "is_virtual", False)
...
def is_single_doctype(doctype: str) -> bool:
	...
	if dontmanage.flags.in_install or dontmanage.flags.in_migrate:
		...
		return getattr(dontmanage.get_meta(doctype), "issingle", False)

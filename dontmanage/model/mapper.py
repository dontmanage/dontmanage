...
from dontmanage.model import child_table_fields, default_fields, table_fields
...
def make_mapped_doc(method, source_name, selected_children=None, args=None):
	...
	method = dontmanage.get_attr(dontmanage.override_whitelisted_method(method))

	dontmanage.is_whitelisted(method)

	if selected_children:
		selected_children = json.loads(selected_children)

	if args:
		dontmanage.flags.args = dontmanage._dict(json.loads(args))

	dontmanage.flags.selected_children = selected_children or None
	...
	if not target_doc:
		...
		if table_maps[from_doctype].get("on_parent"):
			...
			if isinstance(target_parent, str):
				target_parent = dontmanage.get_doc(json.loads(target_parent))
			target_parentfield = target_parent.get_parentfield_of_doctype(target_doctype)
			target_doc = dontmanage.new_doc(
			...
			target_doc = dontmanage.new_doc(target_doctype)
			ret_doc = target_doc
	elif isinstance(target_doc, str):
		target_doc = dontmanage.get_doc(json.loads(target_doc))
		ret_doc = target_doc
	else:
		ret_doc = target_doc
		...
		for df in source_doc.meta.get_table_fields():
			...
			if table_map:
				...
				for source_d in source_doc.get(df.fieldname):
					...
					if (
						dontmanage.flags.selected_children
						and (df.fieldname in dontmanage.flags.selected_children)
						and source_d.name not in dontmanage.flags.selected_children[df.fieldname]
					):
						continue
...
def map_fetch_fields(target_doc, df, no_copy_fields):
	...
	for fetch_df in target_doc.meta.get("fields", {"fetch_from": f"^{df.fieldname}."}):
		if (
			...
			if not linked_doc:
				try:
					linked_doc = dontmanage.get_doc(df.options, target_doc.get(df.fieldname))
				except Exception:
					return

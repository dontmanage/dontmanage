...
from dontmanage.modules.patch_handler import _patch_mode
...
def sync_for(app_name, force=0, reset_permissions=False):
	...
	if app_name == "dontmanage":
		...
		DONTMANAGE_PATH = dontmanage.get_app_path("dontmanage")
		...
		for core_module in [
			...
			files.append(os.path.join(DONTMANAGE_PATH, "core", "doctype", core_module, f"{core_module}.json"))

		# sync permission type and its dependencies
		for dt in ["user", "docshare", "custom_docperm", "docperm", "permission_type"]:
			files.append(os.path.join(DONTMANAGE_PATH, "core", "doctype", dt, f"{dt}.json"))

		for custom_module in ["custom_field", "property_setter"]:
			files.append(
				os.path.join(DONTMANAGE_PATH, "custom", "doctype", custom_module, f"{custom_module}.json")
			)

		for website_module in ["web_form", "web_template", "web_form_field", "portal_menu_item"]:
			files.append(
				os.path.join(DONTMANAGE_PATH, "website", "doctype", website_module, f"{website_module}.json")
			)
		...
		for desk_module in [
			...
			files.append(os.path.join(DONTMANAGE_PATH, "desk", "doctype", desk_module, f"{desk_module}.json"))

		for module_name, document_type in IMPORTABLE_DOCTYPES:
			file = os.path.join(DONTMANAGE_PATH, module_name, "doctype", document_type, f"{document_type}.json")
			if file not in files:
				files.append(file)

	for module_name in dontmanage.local.app_modules.get(app_name) or []:
		folder = os.path.dirname(dontmanage.get_module(app_name + "." + module_name).__file__)
...
def get_doc_files(files, start_path):
	...
	for _module, doctype in IMPORTABLE_DOCTYPES + [
		(None, dontmanage.scrub(dt)) for dt in dontmanage.get_hooks("importable_doctypes")
	]:
...
def remove_orphan_doctypes():
	...
	doctype_names = dontmanage.get_all("DocType", {"custom": 0}, pluck="name")
	orphan_doctypes = []
	...
	for i, name in enumerate(orphan_doctypes):
		dontmanage.delete_doc("DocType", name, force=True, ignore_missing=True)
...
def remove_orphan_entities():
	...
	for entity in entites:
		...
		all_enitities = dontmanage.get_all(
		...
		for i, w in enumerate(all_enitities):
			try:
				...
				try:
					...
					dontmanage.delete_doc(entity, w.name, force=True, ignore_missing=True)
	...
	for app_entity in app_level_entities:
		...
		for i, w in enumerate(all_enitities):
			if w.app and not check_if_record_exists("app", dontmanage.get_app_path(w.app), app_entity, w.name):
				try:
					print(f"Deleting entity {app_entity} {w.name}")
					dontmanage.delete_doc(app_entity, w.name, force=True, ignore_missing=True)
...
def create_entity_file_map(entities):
	...
	from dontmanage.modules.import_file import read_doc_from_file
...
def check_if_record_exists(type=None, path=None, entity_type=None, name=None, module_name=None):
	scrubbed_name = dontmanage.scrub(name.lower())
	scrubbed_entity_type = dontmanage.scrub(entity_type.lower())
	if scrubbed_entity_type == "dashboard" and module_name:
		scrubbed_entity_type = f"{dontmanage.scrub(module_name.lower())}_dashboard"
...
def delete_duplicate_icons():
	...
	for app in dontmanage.get_installed_apps():
		icons = dontmanage.get_all("Desktop Icon", filters=[{"icon_type": "App"}, {"app": app}], pluck="name")
		...
		if len(icons) > 1:
			for i in icons:
				...
				if not check_if_record_exists(type="app", path=app_path, entity_type="Desktop Icon", name=i):
					...
					dontmanage.delete_doc("Desktop Icon", i)

...
def get_doc(*args, **kwargs):
	"""returns a dontmanage.model.Document object.
...
class Document(BaseDocument):
	...
	def load_from_db(self):
		...
		for df in self._get_table_fields():
			...
			# During dontmanage is installed, the property "is_virtual" is not available in tabDocType, so
			# we need to filter those cases for the access to dontmanage.db.get_value() as it would crash otherwise.
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Document":
		...
		self.flags.ignore_version = dontmanage.flags.in_test if ignore_version is None else ignore_version
	...
	def update_single(self, d):
		...
		if self.doctype in dontmanage.db.value_cache:
			del dontmanage.db.value_cache[self.doctype]
				...
				if fail:
					dontmanage.throw(
						_("Value cannot be changed for {0}").format(
							dontmanage.bold(self.meta.get_label(field.fieldname))
						),
	...
	def run_notifications(self, method):
		...
		if (
			(dontmanage.flags.in_import and dontmanage.flags.mute_emails)
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", dontmanage._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", dontmanage._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		try:
			self.lock()
		except dontmanage.DocumentLockedError:
			dontmanage.throw(

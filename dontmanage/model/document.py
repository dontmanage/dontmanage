...
from dontmanage.database.utils import commit_after_response
...
from dontmanage.model import optional_fields, table_fields
from dontmanage.model.base_document import BaseDocument, D, get_controller
from dontmanage.model.docstatus import DocStatus
from dontmanage.model.naming import set_new_name, validate_name
from dontmanage.model.utils import is_virtual_doctype, simple_singledispatch
from dontmanage.model.workflow import set_workflow_state_on_action, validate_workflow
from dontmanage.types import DF
from dontmanage.types.filter import FilterSignature
from dontmanage.utils import compare, cstr, date_diff, file_lock, flt, get_table_name, now
...
def get_doc(*args, **kwargs) -> "Document":
	"""Return a `dontmanage.model.Document` object.
...
class Document(BaseDocument):
	...
	def load_from_db(self) -> "Self":
		...
		if not is_doctype and self.meta.issingle:
			single_doc = dontmanage.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = dontmanage.new_doc(self.doctype, as_dict=True)
			...
			if not is_doctype and isinstance(self.name, str | int):
				for_update = ""
				if self.flags.for_update and dontmanage.db.db_type != "sqlite":
			...
			if not d:
				dontmanage.throw(
					...
					dontmanage.DoesNotExistError(doctype=self.doctype),
				)
	...
	def mask_fields(self):
		from dontmanage.model.db_query import mask_field_value

		mask_fields = dontmanage.get_meta(self.doctype).get_masked_fields()
	...
	def load_children_from_db(self):
		...
		for fieldname, child_doctype in self._table_fieldnames.items():
			...
			if is_doctype:
				...
				children = dontmanage.db.get_values(
					child_doctype,
	...
	def raise_no_permission_to(self, perm_type):
		...
		dontmanage.flags.error_message = _(
			"You need the '{0}' permission on {1} {2} to perform this action."
		).format(
			_(perm_type),
			dontmanage.bold(_(self.doctype)),
			self.name or "",
		)
		raise dontmanage.PermissionError
	...
	def check_if_locked(self):
		...
		if file_lock.lock_age(self.get_signature()) > DOCUMENT_LOCK_SOFT_EXPIRY:
			primary_action = {
				"label": "Force Unlock",
				"server_action": "dontmanage.model.document.unlock_document",
		...
		dontmanage.throw(
			...
			exc=dontmanage.DocumentLockedError,
		)
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Self":
		...
		self.flags.ignore_version = dontmanage.in_test if ignore_version is None else ignore_version
	...
	def validate_amended_from(self):
		if dontmanage.db.get_value(self.doctype, self.get("amended_from"), "docstatus") != 2:
			message = _(
				"{0} cannot be amended because it is not cancelled. Please cancel the document before creating an amendment."
			).format(dontmanage.utils.get_link_to_form(self.doctype, self.get("amended_from")))
			dontmanage.throw(message, title=_("Amendment Not Allowed"))
	...
	def update_single(self, d):
		...
		dontmanage.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in dontmanage.db.value_cache:
			dontmanage.db.value_cache.pop(self.doctype, None)

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = dontmanage.session.user
		...
		if self.is_new() and not (
			dontmanage.flags.in_install or dontmanage.flags.in_patch or dontmanage.flags.in_migrate
		):
		...
		dontmanage.flags.currently_saving.append((self.doctype, self.name))
				...
				if fail:
					dontmanage.throw(
						_("Value cannot be changed for {0}").format(
							dontmanage.bold(_(self.meta.get_label(field.fieldname)))
						),
						exc=dontmanage.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields(include_computed=True):
			for df in dontmanage.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or dontmanage.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = dontmanage.get_meta(df.options).get_high_permlevel_fields()
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = dontmanage.get_meta(self.parenttype).permissions
		else:
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			dontmanage.msgprint(
				...
				raise_exception=dontmanage.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.DRAFT:
			if self.docstatus.is_draft():
				...
				raise dontmanage.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise dontmanage.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise dontmanage.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		raise dontmanage.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			dontmanage.throw(_("Could not find {0}").format(msg), dontmanage.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			dontmanage.throw(_("Cannot link cancelled document: {0}").format(msg), dontmanage.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			method == "onload"
			or (dontmanage.flags.in_import and dontmanage.flags.mute_emails)
		...
		from dontmanage.email.doctype.notification.notification import evaluate_alert
		...
		notifications = dontmanage.client_cache.get_value(
	...
	def discard(self):
		...
		if not self.docstatus.is_draft():
			raise dontmanage.ValidationError(_("Only draft documents can be discarded"), self.docstatus)
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = dontmanage.get_doc(self.doctype, self.name, for_update=True)
		except dontmanage.DoesNotExistError:
			if raise_exception:
				raise

			return dontmanage.clear_last_message()
	...
	def run_post_save_methods(self):
		...
		if not (dontmanage.flags.in_import and self.is_new()):
			self.clear_cache()
		...
		if (self.doctype, self.name) in dontmanage.flags.currently_saving:
			dontmanage.flags.currently_saving.remove((self.doctype, self.name))
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run dontmanage.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in dontmanage.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", dontmanage.session.user)
	...
	def db_get(self, fieldname):
		...
		return dontmanage.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from dontmanage.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def save_version(self):
		...
		if (
			...
			or dontmanage.flags.in_install
			or (not self._doc_before_save and dontmanage.flags.in_patch)
		):
			return
	...
	def hook(f):
		...
		def compose(fn, *hooks):
			def runner(self, method, *args, **kwargs):
				...
				for f in hooks:
					try:
						dontmanage.db._disable_transaction_control += 1
		...
		def composer(self, *args, **kwargs):
			hooks = []
			method = f.__name__
			doc_events = dontmanage.get_doc_hooks()
			for handler in doc_events.get(self.doctype, {}).get(method, []) + doc_events.get("*", {}).get(
				method, []
			):
				hooks.append(dontmanage.get_attr(handler))
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			dontmanage.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or dontmanage.EmptyTableError
			)
	...
	def round_floats_in(self, doc, fieldnames=None):
		...
		rounding_method = dontmanage.get_system_settings("rounding_method")
		...
		return dontmanage.get_doc(
			{
				...
				"comment_email": comment_email or dontmanage.session.user,
		...
		if self.meta.track_seen and not dontmanage.flags.read_only and not self.meta.issingle:
			...
			if user not in _seen:
				...
				commit_after_response(
					lambda: dontmanage.db.set_value(
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		existing_key = dontmanage.db.exists(
			"Document Share Key",
			{
				"reference_doctype": self.doctype,
		...
		if existing_key:
			doc = dontmanage.get_doc("Document Share Key", existing_key)
		else:
			doc = dontmanage.new_doc("Document Share Key")
	...
	def queue_action(self, action, **kwargs):
		...
		return enqueue(
			"dontmanage.model.document.execute_action",
	...
	def lock(self, timeout=None):
		...
		dontmanage.local.locked_documents.append(self)
	...
	def validate_from_to_dates(self, from_date_field: str, to_date_field: str) -> None:
		...
		if date_diff(to_date, from_date) < 0:
			...
			if self.meta.istable:
				table_row = (
					_("{0} row #{1}:").format(
						_(dontmanage.unscrub(self.parentfield)),
						self.idx,
					)
					+ " "
				)
	...
	def get_assigned_users(self):
		assigned_users = dontmanage.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from dontmanage.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from dontmanage.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = dontmanage.get_doc(__doctype, __name)
	doc.unlock()
	try:
...
def bulk_insert(
	...
	while document_batch := list(itertools.islice(documents, chunk_size)):
		...
		for dt, docs in values_map.items():
			dontmanage.db.bulk_insert(dt, valid_column_map[dt], docs, ignore_duplicates=ignore_duplicates)
...
def unlock_document(doctype: str, name: str):
	dontmanage.get_lazy_doc(doctype, name).unlock()
	dontmanage.msgprint(dontmanage._("Document Unlocked"), alert=True)


def get_lazy_controller(doctype):
	lazy_controllers = dontmanage.lazy_controllers.setdefault(dontmanage.local.site, {})
...
def get_cached_doc(*args: Any, **kwargs: Any) -> "Document":
	"""Identical to `dontmanage.get_doc`, but return from cache if available."""
	if (key := can_cache_doc(args)) and (doc := dontmanage.cache.get_value(key)):
		return doc
...
def _set_document_in_cache(key: str, doc: "Document") -> None:
	dontmanage.cache.set_value(key, doc, expires_in_sec=3600)
...
def clear_document_cache(doctype: str, name: str | None = None) -> None:
	dontmanage.db.value_cache.pop(doctype, None)

	def clear_in_redis():
		if name is not None:
			dontmanage.cache.delete_value(get_document_cache_key(doctype, name))
		else:
			dontmanage.cache.delete_keys(get_document_cache_key(doctype, ""))

	clear_in_redis()
	if hasattr(dontmanage.db, "after_commit"):
		dontmanage.db.after_commit.add(clear_in_redis)
		dontmanage.db.after_rollback.add(clear_in_redis)

	if doctype == "System Settings" and hasattr(dontmanage.local, "system_settings"):
		delattr(dontmanage.local, "system_settings")

	if doctype == "Website Settings" and hasattr(dontmanage.local, "website_settings"):
		delattr(dontmanage.local, "website_settings")
...
def get_cached_value(
	...
	if isinstance(fieldname, str):
		if as_dict:
			dontmanage.throw("Cannot make dict for single fieldname")
		return doc.get(fieldname)

	values = [doc.get(f) for f in fieldname]
	if as_dict:
		return dontmanage._dict(zip(fieldname, values, strict=False))
	return values
...
def get_single_value(setting: str, fieldname: str, /, *, as_dict: bool = False):
	...
	Usage:
		telemetry_enabled = dontmanage.get_single_value("System Settings", "telemetry_enabled")
	"""
...
def get_last_doc(
	...
	*,
	...
	d = dontmanage.get_all(doctype, filters=filters, limit_page_length=1, order_by=order_by, pluck="name")
	if d:
		return get_doc(doctype, d[0], for_update=for_update)
	else:
		raise dontmanage.DoesNotExistError(doctype=doctype)


def get_single(doctype):
	"""Return a `dontmanage.model.document.Document` object of the given Single doctype."""

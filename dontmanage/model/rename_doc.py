...
from dontmanage.model.dynamic_links import get_dynamic_link_map
from dontmanage.model.naming import validate_name
from dontmanage.model.utils.user_settings import sync_user_settings, update_user_settings_data
from dontmanage.query_builder import Field
from dontmanage.utils.data import cint, cstr, sbool
from dontmanage.utils.password import rename_password
...
def rename_doc(
	...
	if not (new_usage_style or old_usage_style):
		raise TypeError(
			"{doctype, old, new} or {doc, new} are required arguments for dontmanage.model.rename_doc"
		)
	...
	if validate:
		old_doc = doc or dontmanage.get_doc(doctype, old)
	...
	if merge:
		dontmanage.delete_doc(doctype, old, ignore_permissions=ignore_permissions)

	new_doc.clear_cache()
	dontmanage.clear_cache()
	if rebuild_search:
		dontmanage.enqueue("dontmanage.utils.global_search.rebuild_for_doctype", doctype=doctype)
...
def update_assignments(old: str, new: str, doctype: str) -> None:
	old_assignments = dontmanage.parse_json(dontmanage.db.get_value(doctype, old, "_assign")) or []
	new_assignments = dontmanage.parse_json(dontmanage.db.get_value(doctype, new, "_assign")) or []
	...
	for user in common_assignments:
		...
		for todo in todos:
			dontmanage.delete_doc("ToDo", todo.name, force=True)

	unique_assignments = list(set(old_assignments + new_assignments))
	dontmanage.db.set_value(doctype, new, "_assign", dontmanage.as_json(unique_assignments, indent=0))
...
def update_user_settings(old: str, new: str, link_fields: list[dict]) -> None:
	...
	UserSettings = dontmanage.qb.Table("__UserSettings")

	user_settings_details = (
		dontmanage.qb.from_(UserSettings)
...
def update_customizations(old: str, new: str) -> None:
	dontmanage.db.set_value("Custom DocPerm", {"parent": old}, "parent", new, update_modified=False)
...
def update_autoname_field(doctype: str, new: str, meta: "Meta") -> None:
	...
	if meta.get("autoname"):
		...
		if field and field[0] == "field":
			dontmanage.qb.update(doctype).set(field[1], new).where(Field("name") == new).run()
...
def validate_rename(
	...
	if not dontmanage.db.exists(doctype, old):
		dontmanage.throw(_("Can't rename {0} to {1} because {0} doesn't exist.").format(old, new))
	...
	if not (ignore_permissions or dontmanage.permissions.has_permission(**kwargs)):
		dontmanage.throw(_("You need write permission on {0} {1} to rename").format(doctype, old))

	if merge:
		kwargs["doc"] = dontmanage.get_doc(doctype, new)
		if not (ignore_permissions or dontmanage.permissions.has_permission(**kwargs)):
			dontmanage.throw(_("You need write permission on {0} {1} to merge").format(doctype, new))

	if not force and not ignore_permissions and not meta.allow_rename:
		dontmanage.throw(_("{0} not allowed to be renamed").format(_(doctype)))

	# validate naming like it's done in doc.py
	new = validate_name(doctype, new)

	if save_point:
		dontmanage.db.rollback(save_point=_SAVE_POINT)
...
def update_child_docs(old: str, new: str, meta: "Meta") -> None:
	...
	for df in meta.get_table_fields():
		(
			dontmanage.qb.update(df.options)
			.set("parent", new)
...
def update_link_field_values(link_fields: list[dict], old: str, new: str, doctype: str) -> None:
	for field in link_fields:
		if field["issingle"]:
			try:
				single_doc = dontmanage.get_doc(field["parent"])
			...
			dontmanage.db.set_value(parent, {docfield: cstr(old)}, docfield, cstr(new), update_modified=False)
...
def get_link_fields(doctype: str) -> list[dict]:
	...
	if not dontmanage.flags.link_fields:
		dontmanage.flags.link_fields = {}

	if doctype not in dontmanage.flags.link_fields:
		dt = dontmanage.qb.DocType("DocType")
		df = dontmanage.qb.DocType("DocField")
		...
		virtual_doctypes = dontmanage.get_all("DocType", {"is_virtual": 1}, pluck="name")

		standard_fields = standard_fields_query.run(as_dict=True)

		cf_issingle = dontmanage.qb.from_(dt).select(dt.issingle).where(dt.name == cf.dt).as_("issingle")
		custom_fields = (
			dontmanage.qb.from_(cf)
		...
		dontmanage.flags.link_fields[doctype] = standard_fields + custom_fields + property_setter_fields

	return dontmanage.flags.link_fields[doctype]


def update_options_for_fieldtype(fieldtype: str, old: str, new: str) -> None:
	CustomField = dontmanage.qb.DocType("Custom Field")
	PropertySetter = dontmanage.qb.DocType("Property Setter")
	...
	dontmanage.qb.update(PropertySetter).set(PropertySetter.value, new).where(
...
def update_select_field_values(old: str, new: str):
	from dontmanage.query_builder.functions import Replace

	DocField = dontmanage.qb.DocType("DocField")
	CustomField = dontmanage.qb.DocType("Custom Field")
	PropertySetter = dontmanage.qb.DocType("Property Setter")

	dontmanage.qb.update(DocField).set(DocField.options, Replace(DocField.options, old, new)).where(
...
def update_parenttype_values(old: str, new: str):
	child_doctypes = dontmanage.get_all(
		"DocField",
		fields=["options", "fieldname"],
		filters={"parent": new, "fieldtype": ["in", dontmanage.model.table_fields]},
	)

	custom_child_doctypes = dontmanage.get_all(
		"Custom Field",
		fields=["options", "fieldname"],
		filters={"dt": new, "fieldtype": ["in", dontmanage.model.table_fields]},
	)

	child_doctypes += custom_child_doctypes
	fields = [d["fieldname"] for d in child_doctypes]

	property_setter_child_doctypes = dontmanage.get_all(
		"Property Setter",
	...
	for doctype in child_doctypes:
		...
		dontmanage.qb.update(table).set(table.parenttype, new).where(table.parenttype == old).run()


def rename_dynamic_links(doctype: str, old: str, new: str):
	Singles = dontmanage.qb.DocType("Singles")
	for df in get_dynamic_link_map().get(doctype, []):
		# dynamic link in single, just one value to check
		meta = dontmanage.get_meta(df.parent)
		if meta.is_virtual:
			continue
		if meta.issingle:
			refdoc = dontmanage.db.get_singles_dict(df.parent)
			if refdoc.get(df.options) == doctype and refdoc.get(df.fieldname) == old:
				dontmanage.qb.update(Singles).set(Singles.value, new).where(
			...
			dontmanage.qb.update(parent).set(df.fieldname, new).where(
...
def bulk_rename(doctype: str, rows: list[list] | None = None, via_console: bool = False) -> list[str] | None:
	...
	if not rows:
		dontmanage.throw(_("Please select a valid csv file with data"))

	if not via_console:
		max_rows = 500
		if len(rows) > max_rows:
			dontmanage.throw(_("Maximum {0} rows allowed").format(max_rows))

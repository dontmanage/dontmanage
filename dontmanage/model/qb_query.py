...
"""Query implementation using dontmanage's query builder"""
...
import dontmanage
from dontmanage.database.utils import DefaultOrderBy, FilterValue
...
class DatabaseQuery:
	...
	def execute(
		...
		*,
		...
		kwargs = {
			...
			"group_by": group_by,
			"order_by": order_by,
			"limit": dontmanage.cint(limit),
			"offset": dontmanage.cint(offset),
			"distinct": distinct,
		...
		query = dontmanage.qb.get_query(**kwargs)
	...
	def _handle_virtual_doctype(
		...
		from dontmanage.types.filter import Filters
		...
		# Use dontmanage.call to filter kwargs and call controller

"""
Traced Fields for DontManage

This module provides utilities for creating traced fields in DontManage documents,
...
- Seamlessly integrate with DontManage's document model

Example of standard usage:
		from dontmanage.model.trace import TracedDocument, traced_field
...
See dontmanage.tests.classes.context_managers for a context manager built into test classes.
"""
...
class TracedValue:
	"""
	A descriptor class for creating traced fields in DontManage documents.

	This class allows for monitoring and validating changes to specific fields
	in a DontManage document. It can enforce forbidden values and apply custom
	validation logic.
	...
	def __set__(self, obj, value):
		...
		if value in self.forbidden_values:
			if dontmanage.in_test:
				dontmanage.throw(f"{self.field_name} cannot be set to {value}", AssertionError)
			else:
				dontmanage.throw(f"{self.field_name} cannot be set to {value}")
		...
		if self.custom_validation:
			try:
				...
				if dontmanage.in_test:
					dontmanage.throw(str(e), AssertionError)
				else:
...
class TracedDocument(Document):
	"""
	A base class for DontManage documents with traced fields.

	This class extends DontManage's Document class to provide support for
	traced fields created with TracedValue.

	Attributes:
	        Inherits all attributes from dontmanage.model.document.Document
	"""
...
def traced_field(*args, **kwargs):
	...
	This function simplifies the creation of traced fields in DontManage documents.
...
from dontmanage.deprecation_dumpster import model_trace_traced_field_context as traced_field_context

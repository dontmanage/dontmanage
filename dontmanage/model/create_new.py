...
def make_new_doc(doctype):
	doc = dontmanage.get_doc(
		{"doctype": doctype, "__islocal": 1, "owner": dontmanage.session.user, "docstatus": 0}
	)
...
def get_default_based_on_another_field(df, user_permissions, parent_doc):
	...
	reference_name = (
		parent_doc.get(ref_fieldname) if parent_doc else dontmanage.db.get_default(ref_fieldname)
	)

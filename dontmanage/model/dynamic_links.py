...
def get_dynamic_link_map(for_delete=False):
	...
	"""
	if getattr(dontmanage.local, "dynamic_link_map", None) is None or dontmanage.flags.in_test:
		# Build from scratch
		dynamic_link_map = {}
		for df in get_dynamic_links():
			...
			if meta.issingle:
				...
				try:
					links = dontmanage.db.sql_list("""select distinct {options} from `tab{parent}`""".format(**df))
					for doctype in links:
						dynamic_link_map.setdefault(doctype, []).append(df)
				except dontmanage.db.TableMissingError:  # noqa: E722
					pass

...
from dontmanage.database.utils import DefaultOrderBy, FallBackDateTimeStr, NestedSetHierarchy
...
from dontmanage.model.utils.mask import mask_field_value
...
from dontmanage.types import Filters, FilterSignature, FilterTuple
...
class DatabaseQuery:
	...
	def get_meta(self, doctype: str):
		if doctype not in self._metas:
			self._metas[doctype] = dontmanage.get_meta(doctype)
	...
	def execute(
		...
		*,
		...
		if as_list and not isinstance(self.fields, (Sequence | str)) and len(self.fields) > 1:
			dontmanage.throw(_("Fields must be a list or tuple when as_list is enabled"))
		...
		if is_virtual_doctype(self.doctype):
			...
			return dontmanage.call(controller.get_list, args=kwargs, **kwargs)
	...
	def mask_fields(self, result):
		...
		from dontmanage.model.utils.mask import mask_dict_results, mask_list_results
...
{limit}""".format(**args)
	...
	def prepare_args(self):
		...
		for child in self.tables[1:]:
			...
			args.tables += f" {self.join} {child} on ({child}.parenttype = {dontmanage.db.escape(self.doctype)} and {child}.parent = {parent_name})"
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			dontmanage.throw(_("Use of sub-query or function is restricted"), dontmanage.DataError)
		...
		for field in self.fields:
			...
			if self.strict:
				if STRICT_FIELD_PATTERN.match(field):
					dontmanage.throw(_("Illegal SQL Query"))
	...
	def apply_fieldlevel_read_permissions(self):
		...
		Note: Does not apply to `dontmanage.model.core_doctype_list`
		...
		"""
		from dontmanage.desk.reportview import extract_fieldnames
		...
		for i, field in fields_to_check:
			...
			# labels / pseudo columns or dontmanage internals
			if column[0] in {"'", '"'}:
				continue
	...
	def prepare_filter_condition(self, ft: FilterTuple) -> str:
		...
		if f.operator.lower() in ("in", "not in"):
			...
			if value is None:
				...
				fallback = "''"
				value = [dontmanage.db.escape((cstr(v) or "").strip(), percent=False) for v in values]
				if len(value):
					value = f"({', '.join(value)})"
				else:
					value = "('')"
			...
			if f.operator in (">", "<", ">=", "<=") and (f.fieldname in ("creation", "modified")):
				...
				df
				and (db_type := cstr(dontmanage.db.type_map.get(df.fieldtype, " ")[0]))
			...
			if isinstance(f.value, Column):
				...
				quote = '"' if dontmanage.conf.db_type == "postgres" else "`"
				value = f"{tname}.{quote}{f.value.name}{quote}"

			# escape value
			elif escape and isinstance(value, str):
				value = f"{dontmanage.db.escape(value, percent=False)}"
		...
		if (
			...
			if f.operator.lower() == "like" and dontmanage.conf.get("db_type") == "postgres":
				f.operator = "ilike"
	...
	def get_share_condition(self):
		return (
			...
			+ f" in ({', '.join(dontmanage.db.escape(s, percent=False) for s in self.shared)})"
		)
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if dontmanage.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					values = ", ".join(dontmanage.db.escape(doc, percent=False) for doc in docs)
	...
	def get_permission_query_conditions(self) -> str:
		...
		for method in condition_methods:
			if c := dontmanage.call(dontmanage.get_attr(method), self.user, doctype=self.doctype):
				conditions.append(c)
	...
	def validate_order_by_and_group_by(self, parameters: str):
		...
		if any(re.search(r"\b" + pattern + r"\b", sanitized) for pattern in subquery_indicators):
			dontmanage.throw(_("Cannot use sub-query here."))
		...
		for field in parameters.split(","):
			...
			if full_field_name:
				...
				if tbl not in self.tables:
					if tbl.startswith("`"):
						tbl = tbl[4:-1]
					dontmanage.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))
			...
			for func in blacklisted_sql_functions:
				if re.search(r"\b" + re.escape(func) + r"\W*\(", field.lower()):
					dontmanage.throw(_("Cannot use {0} in order/group by").format(field))
...
def get_additional_filter_field(additional_filters_config, f, value):
	...
	f = dontmanage._dict(dontmanage.get_attr(additional_filter["get_field"])())
	if f.query_value:
		for option in f.options:
			option = dontmanage._dict(option)

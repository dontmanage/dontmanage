...
from dontmanage.model import get_permitted_fields, optional_fields
...
class DatabaseQuery:
	...
	def doctype_meta(self):
		if not hasattr(self, "_doctype_meta"):
			self._doctype_meta = dontmanage.get_meta(self.doctype)
		...
		for field in self.fields:
			...
			if SUB_QUERY_PATTERN.match(field):
				...
				if function in blacklisted_functions:
					dontmanage.throw(
						_("Use of function {0} in field is restricted").format(function), exc=dontmanage.DataError
					)
		...
		for i, field in enumerate(self.fields):
			...
			# handle child / joined table fields
				...
				if wrap_grave_quotes(table) in self.query_tables:
					...
					raise dontmanage.PermissionError(ch_doctype)
	...
	def get_permission_query_conditions(self):
		conditions = []
		condition_methods = dontmanage.get_hooks("permission_query_conditions", {}).get(self.doctype, [])
		if condition_methods:
			for method in condition_methods:
				c = dontmanage.call(dontmanage.get_attr(method), self.user)
				if c:
					conditions.append(c)

		permision_script_name = get_server_script_map().get("permission_query", {}).get(self.doctype)
		if permision_script_name:
			script = dontmanage.get_doc("Server Script", permision_script_name)
	...
	def validate_order_by_and_group_by(self, parameters: str):
		...
		if "select" in _lower and "from" in _lower:
			dontmanage.throw(_("Cannot use sub-query in order by"))
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if child_doctype and not (
			dontmanage.db.exists("DocField", {"parent": parent, "options": child_doctype})
			or dontmanage.db.exists("Custom Field", {"dt": parent, "options": child_doctype})
		):
			raise dontmanage.PermissionError

		if dontmanage.permissions.has_permission(parent):
			return

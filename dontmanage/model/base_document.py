...
class BaseDocument:
	...
	def get_valid_columns(self) -> list[str]:
		if self.doctype not in dontmanage.local.valid_columns:
			if self.doctype in DOCTYPES_FOR_DOCTYPE:
			...
			dontmanage.local.valid_columns[self.doctype] = valid

		return dontmanage.local.valid_columns[self.doctype]
	...
	def db_insert(self, ignore_if_duplicate=False):
		...
		if ignore_if_duplicate and dontmanage.db.db_type == "postgres":
		...
		try:
			...
			if dontmanage.db.is_primary_key_violation(e):
				if self.meta.autoname == "hash":
					...
					dontmanage.flags.retry_count = (dontmanage.flags.retry_count or 0) + 1
					if dontmanage.flags.retry_count > 5 and not dontmanage.flags.in_test:
						raise
					self.name = None
					self.db_insert()
					return
		...
		for df in self.meta.get_link_fields() + self.meta.get("fields", {"fieldtype": ("=", "Dynamic Link")}):
			...
			if docname:
				if df.fieldtype == "Link":
					...
					if not doctype:
						dontmanage.throw(_("{0} must be set first").format(self.meta.get_label(df.options)))
				...
				if not dontmanage.get_meta(doctype).get("is_virtual"):
				...
				if getattr(dontmanage.get_meta(doctype), "issingle", 0):
					values.name = doctype

				if dontmanage.get_meta(doctype).get("is_virtual"):
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (dontmanage.flags.in_test and value.startswith("_T-")):
	...
	def _validate_data_fields(self):
		...
		for phone_field in self.meta.get_phone_fields():
			...
			dontmanage.utils.validate_phone_number_with_country_code(phone, phone_field.fieldname)
		...
		for data_field in self.meta.get_data_fields():
			...
			if data_field_options == "Email":
				if (self.owner in dontmanage.STANDARD_USERS) and (data in dontmanage.STANDARD_USERS):
					continue
				for email_address in dontmanage.utils.split_emails(data):
					dontmanage.utils.validate_email_address(email_address, throw=True)

			if data_field_options == "Name":
				dontmanage.utils.validate_name(data, throw=True)

			if data_field_options == "Phone":
				dontmanage.utils.validate_phone_number(data, throw=True)

			if data_field_options == "URL":
				if not data:
					continue

				dontmanage.utils.validate_url(data, throw=True)

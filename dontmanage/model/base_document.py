...
from dontmanage.model.dynamic_links import invalidate_distinct_link_doctypes
...
from dontmanage.utils.defaults import get_not_null_defaults
...
def get_controller(doctype):
	...
	For `custom` type, return `dontmanage.model.document.Document`.
...
def import_controller(doctype):
	...
	if doctype not in DOCTYPES_FOR_DOCTYPE:
		doctype_info = dontmanage.db.get_value("DocType", doctype, ("module", "custom", "is_tree"), as_dict=True)
		if doctype_info:
	...
	if class_overrides and class_overrides.get(doctype):
		...
		custom_module = dontmanage.get_module(module_path)
		custom_class_ = getattr(custom_module, custom_classname, None)
		if not issubclass(custom_class_, class_):
			original_class_path = dontmanage.bold(f"{class_.__module__}.{class_.__name__}")
			dontmanage.throw(
				f"{doctype}: {dontmanage.bold(import_path)} must be a subclass of {original_class_path}",
...
def _get_extended_class(base_class, doctype):
	...
	extensions = dontmanage.get_hooks("extend_doctype_class", {}).get(doctype)
	if not extensions:
		return base_class

	# Get extension classes in reverse order using dontmanage.get_attr
	extension_classes = []
	for extension_path in reversed(extensions):
		try:
			extension_class = dontmanage.get_attr(extension_path)
...
class BaseDocument:
	...
	def get_db_value(self, key):
		return dontmanage.db.get_value(self.doctype, self.name, key)
	...
	def _init_child(self, value, key):
		...
		if not __dict.get("name"):
			__dict["__islocal"] = 1
			__dict["__temporary_name"] = dontmanage.generate_hash(length=10)
	...
	def get_valid_columns(self) -> list[str]:
		valid_columns_cache = dontmanage.local.valid_columns

		if self.doctype not in valid_columns_cache:
			if self.doctype in DOCTYPES_FOR_DOCTYPE:
				from dontmanage.model.meta import get_table_columns
	...
	def as_json(self):
		return dontmanage.as_json(self.as_dict())
	...
	def db_insert(self, ignore_if_duplicate=False):
		...
		if (
			...
			and dontmanage.db.db_type == "postgres"
		...
		try:
			...
			if (
				dontmanage.db.db_type == "postgres" and self.meta.autoname == "hash" and not name
			):  # To avoid a transaction block, we regen in try (pg specific)
				return self._handle_hash_conflict()
		except Exception as e:
			if dontmanage.db.is_primary_key_violation(e):
				if self.meta.autoname == "hash":
				...
				if not ignore_if_duplicate:
					...
					raise dontmanage.DuplicateEntryError(self.doctype, self.name, e)

			elif dontmanage.db.is_unique_key_violation(e):
				# unique constraint
	...
	def db_update(self):
		...
		try:
			...
			if dontmanage.db.is_data_too_long(e):
				...
				if column:
					...
					dontmanage.throw(
						...
						).format(dontmanage.bold(label), dontmanage.bold(self.doctype)),
	...
	def show_unique_validation_message(self, e):
		if dontmanage.db.db_type == "mariadb":
			...
			dontmanage.msgprint(_("{0} must be unique").format(label or fieldname))

		# this is used to preserve traceback
		raise dontmanage.UniqueValidationError(self.doctype, self.name, e)
	...
	def get_invalid_links(self, is_submittable=False):
		...
		for df in self.meta.get_link_fields() + self.meta.get("fields", {"fieldtype": ("=", "Dynamic Link")}):
			...
			if df.fieldtype == "Link":
				doctype = df.options
				if not doctype:
					dontmanage.throw(_("Options not set for link field {0}").format(df.fieldname))
			else:
				assert df.fieldtype == "Dynamic Link"
				doctype = self.get(df.options)
				if not doctype:
					dontmanage.throw(_("{0} must be set first").format(_(self.meta.get_label(df.options))))
			...
			check_docstatus = is_submittable and dontmanage.get_meta(doctype).is_submittable
			...
			if not meta.get("is_virtual"):
				values = dontmanage.db.get_value(
				...
				values = dontmanage.get_doc(doctype, docname).as_dict()
	...
	def set_fetch_from_value(self, doctype, df, values):
		...
		if df.fieldtype in ["Small Text", "Text", "Data"]:
			...
			fetch_from_df = get_default_df(fetch_from_fieldname) or dontmanage.get_meta(doctype).get_field(
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (dontmanage.in_test and value.startswith("_T-")):
	...
	def _validate_data_fields(self):
		...
		# data_field options defined in dontmanage.model.data_field_options
		for data_field in self.meta.get_data_fields():
			...
			if data_field_options == "Email":
				if (self.owner in dontmanage.STANDARD_USERS) and (data in dontmanage.STANDARD_USERS):
					continue
	...
	def _validate_constants(self):
		if dontmanage.flags.in_import or self.is_new() or self.flags.ignore_validate_constants:
			return

		constants = [d.fieldname for d in self.meta.get("fields", {"set_only_once": ("=", 1)})]
		if constants:
			values = dontmanage.db.get_value(self.doctype, self.name, constants, as_dict=True)
		...
		for fieldname in constants:
			...
			if self.get(fieldname) != value:
				dontmanage.throw(
					...
					dontmanage.CannotChangeConstantError,
				)
	...
	def _validate_length(self):
		...
		type_map = dontmanage.db.type_map
	...
	def _validate_code_fields(self):
		for field in self.meta.get_code_fields():
			...
			if language == "Python":
				dontmanage.utils.validate_python_code(code_string, fieldname=field.label, is_expression=False)

			elif language == "PythonExpression":
				dontmanage.utils.validate_python_code(code_string, fieldname=field.label)
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		dontmanage.throw(
			_("{0}: '{1}' ({3}) will get truncated, as max characters allowed is {2}").format(
				reference, dontmanage.bold(_(df.label, context=df.parent)), max_length, value
			),
	...
	def _validate_update_after_submit(self):
		...
		db_values = dontmanage.get_doc(self.doctype, self.name).as_dict()
			...
			if df and not df.allow_on_submit and (self.get(key) or db_value):
				...
				if self_value != db_value:
					dontmanage.throw(
						...
						dontmanage.UpdateAfterSubmitError,
	...
	def _sanitize_content(self):
		...
		for fieldname, value in self.get_valid_dict(ignore_virtual=True).items():
			...
			value = dontmanage.as_unicode(value)
	...
	def _save_passwords(self):
		...
		from dontmanage.utils.password import remove_encrypted_password, set_encrypted_password
	...
	def get_formatted(
		...
		if (
			...
			currency = dontmanage.db.get_value("Currency", currency_value, cache=True)
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields, mask_fields=None):
		...
		if not self.is_new() and masked_fieldnames:
			ref_values = dontmanage.db.get_value(self.doctype, self.name, masked_fieldnames, as_dict=True) or {}
	...
	def _extract_images_from_text_editor(self):
		from dontmanage.core.doctype.file.utils import extract_images_from_doc

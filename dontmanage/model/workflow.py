...
if TYPE_CHECKING:
	...
	from dontmanage.workflow.doctype.workflow.workflow import Workflow


DEFAULT_WORKFLOW_TASKS = ["Webhook", "Server Script"]


class WorkflowStateError(dontmanage.ValidationError):
	pass


class WorkflowTransitionError(dontmanage.ValidationError):
	pass


class WorkflowPermissionError(dontmanage.ValidationError):
	pass
...
def get_transitions(
	...
	if not current_state:
		if raise_exception:
			...
			dontmanage.throw(_("Workflow State not set"), WorkflowStateError)
...
def get_workflow_safe_globals():
	# access to dontmanage.db.get_value, dontmanage.db.get_list, and date time utils.
	return dict(
		dontmanage=dontmanage._dict(
			db=dontmanage._dict(get_value=dontmanage.db.get_value, get_list=dontmanage.db.get_list),
			session=dontmanage.session,
			utils=dontmanage._dict(
				now_datetime=dontmanage.utils.now_datetime,
				add_to_date=dontmanage.utils.add_to_date,
				get_datetime=dontmanage.utils.get_datetime,
...
def evaluate_workflow_value(value, evaluate_as_expression, doc):
	...
	if evaluate_as_expression:
		try:
			return dontmanage.safe_eval(value, get_workflow_safe_globals(), dict(doc=doc.as_dict()))
...
def apply_workflow(doc, action):
	...
	if not transition:
		dontmanage.throw(_("Not a valid Workflow Action"), WorkflowTransitionError)

	if not has_approval_access(user, doc, transition):
		dontmanage.throw(_("Self approval is not allowed"))
	...
	if transition.transition_tasks:
		workflow_transitions = dontmanage.db.get_all(
		...
		workflow_methods = [{"name": "Create a customer", "method":
					 		"dontmanage.dotted.path.create_customer"}]
		"""

		tasks = {i["name"]: i["method"] for i in dontmanage.get_hooks("workflow_methods")}
		...
		for workflow_transition in workflow_transitions:
			...
			if workflow_transition.task in DEFAULT_WORKFLOW_TASKS:
				match workflow_transition.task:
					case "Webhook":
						webhook = dontmanage.get_doc("Webhook", workflow_transition.link)
						task_method = webhook.execute_for_doc

					case "Server Script":
						server_script = dontmanage.get_doc("Server Script", workflow_transition.link)
						task_method = server_script.execute_workflow_task

			else:  # normal app-defined tasks
				try:
					task_method = dontmanage.get_attr(tasks[workflow_transition.task])
				except KeyError:
					dontmanage.throw(_('There is no task called "{}"').format(workflow_transition.task))
		...
		for async_task in async_tasks:
			dontmanage.enqueue(async_task, doc=doc, enqueue_after_commit=True)
	...
	if doc.docstatus.is_draft() and new_docstatus.is_draft():
		...
		dontmanage.throw(_("Illegal Document Status for {0}").format(next_state.state))
...
def validate_workflow(doc):
	...
	if current_state != next_state:
		bold_current = dontmanage.bold(current_state)
		bold_next = dontmanage.bold(next_state)
...
def _bulk_workflow_action(docnames, doctype, action):
	...
	for idx, docname in enumerate(docnames, 1):
		...
		try:
			...
			apply_workflow(dontmanage.get_doc(doctype, docname), action)
			dontmanage.db.commit()
		except Exception as e:
			if not dontmanage.message_log:
			...
			if not message_dict:
				if dontmanage.message_log:
					messages = dontmanage.get_message_log()
...
def print_workflow_log(messages, title, doctype, indicator):
	if messages.keys():
		...
		for doc in messages.keys():
			if len(messages[doc]):
				html = f"<details><summary>{dontmanage.utils.get_link_to_form(doctype, doc)}</summary>"
	...
	try:
		for i, doc in enumerate(docs, 1):
			...
			actions = [
				...
				if has_approval_access(dontmanage.session.user, doc, t)
			]
			if not actions:
				return []

...
class InvalidNamingSeriesError(dontmanage.ValidationError):
	pass


class InvalidUUIDValue(dontmanage.ValidationError):
	pass
...
class NamingSeries:
	...
	def validate(self):
		if "." not in self.series:
			dontmanage.throw(
				_("Invalid naming series {}: dot (.) missing").format(dontmanage.bold(self.series)),
		...
		if not NAMING_SERIES_PATTERN.match(self.series):
			dontmanage.throw(
				...
				).format(dontmanage.bold(self.series)),
				exc=InvalidNamingSeriesError,
			)
	...
	def get_prefix(self) -> str:
		...
		if prefix is None:
			dontmanage.throw(_("Invalid Naming Series: {}").format(self.series))
	...
	def update_counter(self, new_count: int) -> None:
		...
		Series = dontmanage.qb.DocType("Series")
	...
	def get_current_value(self) -> int:
		prefix = self.get_prefix()
		return cint(dontmanage.db.get_value("Series", prefix, "current", order_by="name", for_update=True))
...
def set_new_name(doc):
	...
	if autoname.lower() not in ("prompt", "uuid") and not dontmanage.flags.in_import:
		doc.name = None

	if is_autoincremented(doc.doctype, meta):
		doc.name = dontmanage.db.get_next_sequence_val(doc.doctype)
		return
	...
	if meta.autoname == "UUID":
		if not doc.name:
			...
			try:
				...
				dontmanage.throw(_("Invalid value specified for UUID: {}").format(doc.name), InvalidUUIDValue)
		return
...
def set_name_from_naming_options(autoname, doc):
	...
	if _autoname.startswith("field:"):
		...
		if not doc.name:
			...
			dontmanage.throw(_("{0} is required").format(doc.meta.get_label(fieldname)))
...
def set_name_by_naming_series(doc):
	...
	if not doc.naming_series:
		dontmanage.throw(dontmanage._("Naming Series mandatory"))
...
def _generate_random_string(length=10):
	"""Better version of dontmanage.generate_hash for naming.
...
def getseries(key, digits):
	# series created ?
	# Using dontmanage.qb as dontmanage.get_values does not allow order_by=None
	series = DocType("Series")
	current = (dontmanage.qb.from_(series).where(series.name == key).for_update().select("current")).run()

	if current and current[0][0] is not None:
		current = current[0][0]
		# yes, update it
		dontmanage.db.sql("UPDATE `tabSeries` SET `current` = `current` + 1 WHERE `name`=%s", (key,))
		current = cint(current) + 1
	else:
		# no, create it
		dontmanage.db.sql("INSERT INTO `tabSeries` (`name`, `current`) VALUES (%s, 1)", (key,))
		current = 1
...
def get_default_naming_series(doctype: str) -> str | None:
	...
	naming_series_options = dontmanage.get_meta(doctype).get_naming_series_options()
...
def validate_name(doctype: str, name: int | str):
	...
	if isinstance(name, int):
		if is_autoincremented(doctype):
			...
			dontmanage.db.set_next_sequence_val(doctype, name, is_val_used=True)
			return name

		dontmanage.throw(_("Invalid name type (integer) for varchar name column"), dontmanage.NameError)
	...
	if not dontmanage.get_meta(doctype).get("issingle") and (doctype == name) and (name != "DocType"):
		dontmanage.throw(_("Name of {0} cannot be {1}").format(doctype, name), dontmanage.NameError)
...
def append_number_if_name_exists(doctype, value, fieldname="name", separator="-", filters=None):
	...
	exists = dontmanage.db.exists(doctype, filters)
...
def _set_amended_name(doc):
	...
	if not amend_naming_rule:
		amend_naming_rule = dontmanage.get_single_value("Document Naming Settings", "default_amend_naming")

	...
	for name in names or []:
		...
		if doctype == "DocType":
			...
			if (
				...
				and not doc.custom
				and not (
					for_reload or dontmanage.flags.in_migrate or dontmanage.flags.in_install or dontmanage.flags.in_uninstall
				)
			):
				try:
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	for lf in link_fields:
		...
		if not issingle:
			...
			if dontmanage.get_meta(link_dt).istable:
			...
			for item in dontmanage.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
				...
				if linked_doctype in dontmanage.get_hooks("ignore_links_on_delete") or (
			...
			if dontmanage.db.get_value(link_dt, None, link_field) == doc.name:

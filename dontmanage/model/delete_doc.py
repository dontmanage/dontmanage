...
def check_permission_and_not_submitted(doc):
	...
	if (
		...
		and dontmanage.session.user != "Administrator"
		...
		dontmanage.msgprint(
			...
			raise_exception=dontmanage.PermissionError,
		)
	...
	if doc.docstatus.is_submitted():
		dontmanage.msgprint(
			_("{0} {1}: Submitted Record cannot be deleted. You must {2} Cancel {3} it first.").format(
				_(doc.doctype),
				doc.name,
				"<a href='https://docs.dontmanageerp.com//docs/user/manual/en/setting-up/articles/delete-submitted-document' target='_blank'>",
				"</a>",
			),

...
from dontmanage.desk.doctype.tag.tag import delete_tags_for_document
from dontmanage.model.docstatus import DocStatus
from dontmanage.model.dynamic_links import get_dynamic_link_map
from dontmanage.model.naming import revert_series_if_last
from dontmanage.model.utils import is_virtual_doctype
from dontmanage.utils.data import get_link_to_form
from dontmanage.utils.file_manager import remove_all
from dontmanage.utils.global_search import delete_for_document
from dontmanage.utils.password import delete_all_passwords_for
...
def delete_doc(
	...
	Args:
		doctype (str, optional): The document type to delete. If not provided,
			retrieved from dontmanage.form_dict.get("dt"). Defaults to None.
		name (str | int | list, optional): The name/ID of the document(s) to delete.
			...
			retrieved from dontmanage.form_dict.get("dn"). Defaults to None.
		...
		ignore_missing (bool, optional): When True, doesn't raise an error if the
			...
			dontmanage.DoesNotExistError if document is missing. Defaults to True.
	...
	Raises:
		dontmanage.DoesNotExistError: When document doesn't exist and ignore_missing=False.
		dontmanage.LinkExistsError: When document is linked to other records and force=False.
		dontmanage.PermissionError: When user doesn't have delete permissions and ignore_permissions=False.
		dontmanage.ValidationError: When trying to delete a submitted document.
		dontmanage.QueryTimeoutError: When document is locked by another user.
	...
	# get from form
	if not doctype:
		doctype = dontmanage.form_dict.get("dt")
		name = dontmanage.form_dict.get("dn")
	...
	for name in names or []:
		if is_virtual:
			dontmanage.get_doc(doctype, name).delete()
			continue
		...
		if doctype == "DocType":
			if for_reload:
				...
				doc = dontmanage.get_doc(doctype, name)
				if not (doc.custom or dontmanage.conf.developer_mode or dontmanage.flags.in_patch or force):
					dontmanage.throw(_("Standard DocType can not be deleted."))

				update_flags(doc, flags, ignore_permissions)
				check_permission_and_not_submitted(doc)
				# delete custom table fields using this doctype.
				dontmanage.db.delete(
					"Custom Field", {"options": name, "fieldtype": ("in", dontmanage.model.table_fields)}
				)
				dontmanage.db.delete("__global_search", {"doctype": name})
			...
			if (
				...
				and not (
					...
					or dontmanage.flags.in_uninstall
				)
			):
				try:
			...
			dontmanage.clear_cache(doctype=name)

		else:
			# Lock the doc without waiting
			try:
				dontmanage.db.get_value(doctype, name, for_update=True, wait=False)
			except (dontmanage.QueryTimeoutError, dontmanage.QueryDeadlockError):
				dontmanage.throw(
					_(
						"This document can not be deleted right now as it's being modified by another user. Please try again after some time."
					),
					exc=dontmanage.QueryTimeoutError,
				)
			doc = dontmanage.get_doc(doctype, name)
			...
			if not for_reload:
				...
				if not force:
					...
					except dontmanage.LinkExistsError as e:
			...
			if not for_reload:
				...
				dontmanage.enqueue(
					"dontmanage.model.delete_doc.delete_dynamic_links",
					doctype=doc.doctype,
					name=doc.name,
...
def add_to_deleted_document(doc):
	...
	if doc.doctype != "Deleted Document" and dontmanage.flags.in_install != "dontmanage":
		dontmanage.get_doc(
...
def delete_from_table(doctype: str, name: str, ignore_doctypes: list[str], doc):
	if doctype != "DocType" and doctype == name:
		dontmanage.db.delete("Singles", {"doctype": name})
	else:
		dontmanage.db.delete(doctype, {"name": name})
	if doc:
		child_doctypes = [
			d.options for d in doc.meta.get_table_fields() if dontmanage.get_meta(d.options).is_virtual == 0
		]
		...
		child_doctypes = dontmanage.get_all(
			"DocField",
			fields="options",
			filters={"fieldtype": ["in", dontmanage.model.table_fields], "parent": doctype},
			pluck="options",
		)

	child_doctypes_to_delete = set(child_doctypes) - set(ignore_doctypes)
	for child_doctype in child_doctypes_to_delete:
		dontmanage.db.delete(child_doctype, {"parenttype": doctype, "parent": name})
...
def check_permission_and_not_submitted(doc):
	# permission
	if not doc.flags.ignore_permissions and dontmanage.session.user != "Administrator":
		if doc.doctype == "DocType" and not doc.custom:
			dontmanage.throw(_("Only the Administrator can delete a standard DocType."))
		else:
	...
	if doc.meta.is_submittable and doc.docstatus.is_submitted():
		dontmanage.msgprint(
			_("{0} {1}: Submitted Record cannot be deleted. You must {2} Cancel {3} it first.").format(
				_(doc.doctype),
				doc.name,
				"<a href='https://docs.dontmanage.io/dontmanageerp/user/manual/en/delete-submitted-document' target='_blank'>",
				"</a>",
			),
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	"""
	from dontmanage.model.rename_doc import get_link_fields
	...
	if method == "Delete":
		ignored_doctypes.update(dontmanage.get_hooks("ignore_links_on_delete"))
	...
	for lf in link_fields:
		...
		try:
			meta = dontmanage.get_meta(link_dt)
		except dontmanage.DoesNotExistError:
		...
		if issingle:
			if dontmanage.db.get_single_value(link_dt, link_field) == doc.name:
		...
		for item in dontmanage.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
...
def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	"""Raise `dontmanage.LinkExistsError` if the document is dynamically linked"""
	for df in get_dynamic_link_map().get(doc.doctype, []):
		ignore_linked_doctypes = doc.get("ignore_linked_doctypes") or []

		if df.parent in dontmanage.get_hooks("ignore_links_on_delete") or (
		...
		if meta.issingle:
			...
			for refdoc in dontmanage.db.sql(
				...
				if (method == "Delete" and not DocStatus(refdoc.docstatus).is_cancelled()) or (
					...
					if reference_doctype in dontmanage.get_hooks("ignore_links_on_delete") or (
...
def clear_timeline_references(link_doctype, link_name):
	dontmanage.db.delete("Communication Link", {"link_doctype": link_doctype, "link_name": link_name})
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, "doctype", dontmanage.scrub(doctype))

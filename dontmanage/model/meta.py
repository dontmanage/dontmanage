...
from dontmanage.model.base_document import (
...
from dontmanage.utils import cached_property, cast, cint, cstr
from dontmanage.utils.caching import site_cache
from dontmanage.utils.data import add_to_date, get_datetime
...
def get_meta(doctype: "str | DocType", cached: bool = True) -> "_Meta":
	...
	if (
		...
		and (meta := dontmanage.client_cache.get_value(f"doctype_meta::{doctype}"))
	):
		return meta
	...
	dontmanage.client_cache.set_value(key, meta)
	return meta
...
def clear_meta_cache(doctype: str = "*"):
	...
	if doctype == "*":
		dontmanage.client_cache.delete_keys(key)
	else:
		dontmanage.client_cache.delete_value(key)
...
def get_table_columns(doctype):
	return dontmanage.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = dontmanage.scrub(doctype)
	with open(dontmanage.get_app_path("dontmanage", "core", "doctype", fname, fname + ".json")) as f:
...
class Meta(Document):
	...
	def get_masked_fields(self):
		...
		if dontmanage.session.user == "Administrator":
			return []
		cache_key = f"masked_fields::{self.name}::{dontmanage.session.user}"
		masked_fields = dontmanage.cache.get_value(cache_key)
		...
		if masked_fields is None:
			...
			dontmanage.cache.set_value(cache_key, masked_fields)
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(dontmanage._dict(fieldtype="Data", fieldname="name", label="Name"))
	...
	def _valid_fields(self):
		if (dontmanage.flags.in_install or dontmanage.flags.in_migrate) and self.name in self.special_doctypes:
	...
	def add_custom_fields(self):
		if not dontmanage.db.table_exists("Custom Field"):
			return

		custom_fields = dontmanage.db.get_values(
			"Custom Field",
	...
	def check_if_large_table(self):
		...
		if self.istable or not dontmanage.db.table_exists(self.name):  # During install, new migrate
			return

		if dontmanage.db.estimate_count(self.name) > LARGE_TABLE_SIZE_THRESHOLD:
			# Raw SQL to prevent querying meta when already in meta
			recent_change = dontmanage.db.sql(
	...
	def set_custom_permissions(self):
		...
		if dontmanage.flags.in_patch or dontmanage.flags.in_install:
			return

		if not self.istable and self.name not in ("DocType", "DocField", "DocPerm", "Custom DocPerm"):
			custom_perms = dontmanage.get_all(
				"Custom DocPerm",
				fields="*",
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(dontmanage._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permitted_fieldnames(
		...
		*,
		...
		if 0 not in permlevel_access and permission_type in ("read", "select"):
			if dontmanage.share.get_shared(self.name, user, rights=["read"], limit=1):
	...
	def get_permlevel_access(self, permission_type="read", parenttype=None, *, user=None):
		has_access_to = []
		roles = set(dontmanage.get_roles(user))
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = dontmanage.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other DontManage applications via hooks.
		"""
		data = dontmanage._dict()
		if not self.custom:
			try:
				module = load_doctype_module(self.name, suffix="_dashboard")
				if hasattr(module, "get_data"):
					data = dontmanage._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in dontmanage.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = dontmanage._dict(dontmanage.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = dontmanage.scrub(self.module)
		doctype = dontmanage.scrub(self.name)
		template_path = dontmanage.get_module_path(
...
def get_parent_dt(dt):
	if not dontmanage.is_table(dt):
		return ""

	return (
		dontmanage.db.get_value(
			"DocField",
			{"fieldtype": ("in", dontmanage.model.table_fields), "options": dt},
			"parent",
		)
		or ""
	)
...
def set_fieldname(field_id, fieldname):
	dontmanage.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(dontmanage.local, "field_currency", None):
		dontmanage.local.field_currency = dontmanage._dict()

	if not (
		dontmanage.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.get("parent")
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = dontmanage.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if dontmanage.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = dontmanage.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			dontmanage.local.field_currency.setdefault((doc.doctype, ref_docname), dontmanage._dict()).setdefault(
...
def get_field_precision(df, doc=None, currency=None):
	...
	if df.precision:
		...
		precision = cint(dontmanage.db.get_default("currency_precision"))
		if not precision:
			precision = get_precision_from_currency_format(currency or get_field_currency(df, doc))
	else:
		precision = cint(dontmanage.db.get_default("float_precision")) or 3
...
def get_precision_from_currency_format(currency: str) -> int:
	...
	from dontmanage.locale import get_number_format
	from dontmanage.utils.number_format import NumberFormat

	use_format_from_currency = dontmanage.get_system_settings("use_number_format_from_currency")
	number_format = get_number_format()
	if use_format_from_currency:
		currency_format = dontmanage.db.get_value("Currency", currency, "number_format", cache=True)
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return dontmanage._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return dontmanage._dict(fieldname=fieldname, fieldtype="Int")

		elif fieldname in ("owner", "modified_by"):
			return dontmanage._dict(fieldname=fieldname, fieldtype="Link", options="User")

		return dontmanage._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in dontmanage.get_all("DocType", filters=filters, pluck="name"):
		try:
...
def trim_table(doctype, dry_run=True):
	...
	if columns_to_remove and not dry_run:
		...
		dontmanage.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")

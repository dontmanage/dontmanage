...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "dontmanage", "dist", type)
	...
	assets_json = dontmanage.read_file("assets/assets.json")
	if assets_json:
		assets_json = dontmanage.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/dontmanage"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["dontmanage/" + name for name in missing_assets]
			bundle(build_mode, files=files_to_build)
	else:
		# no assets.json, run full build
		bundle(build_mode, apps="dontmanage")
...
def get_assets_link(dontmanage_head) -> str:
	...
	tag = getoutput(
		r"cd ../apps/dontmanage && git show-ref --tags -d | grep {} | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{{}}//'".format(dontmanage_head)
	)
...
def fetch_assets(url, dontmanage_head):
	click.secho("Retrieving assets...", fg="yellow")

	prefix = mkdtemp(prefix="dontmanage-assets-", suffix=dontmanage_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	click.echo(click.style("✔", fg="green") + f" Downloaded DontManage assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./dontmanage-bench/sites", "")
...
def download_dontmanage_assets(verbose=True) -> bool:
	"""Download and set up DontManage assets if they exist based on the current commit HEAD.
	Return True if correctly setup else return False.
	"""
	dontmanage_head = getoutput("cd ../apps/dontmanage && git rev-parse HEAD")

	if not dontmanage_head:
		return False

	try:
		url = get_assets_link(dontmanage_head)
		assets_archive = fetch_assets(url, dontmanage_head)
...
def setup():
	...
	for app in dontmanage.get_all_apps(True):
		try:
			pymodules.append(dontmanage.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(dontmanage.local.sites_path, "assets")
...
def watch(apps=None):
	...
	live_reload = dontmanage.utils.cint(os.environ.get("LIVE_RELOAD", dontmanage.conf.live_reload))
...
def generate_assets_map():
	...
	for app_name in dontmanage.get_all_apps():
		app_doc_path = None

		pymodule = dontmanage.get_module(app_name)
...
def html_to_js_template(path, content):
	"""Return HTML template content as Javascript code, by adding it to `dontmanage.templates`."""
	return """dontmanage.templates["{key}"] = '{content}';\n""".format(

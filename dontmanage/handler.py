...
def execute_cmd(cmd, from_async=False):
	...
	cmd = dontmanage.override_whitelisted_method(cmd)
...
def is_valid_http_method(method):
	...
	if http_method not in dontmanage.allowed_http_methods_for_whitelisted_func[method]:
		dontmanage.throw_permission_error()
...
def upload_file():
	...
	if dontmanage.session.user == "Guest":
		...
		user: User = dontmanage.get_lazy_doc("User", dontmanage.session.user)
	...
	dontmanage.local.uploaded_file_url = file_url
	...
	if content is not None and (dontmanage.session.user == "Guest" or (user and not user.has_desk_access())):
		...
		if filetype not in ALLOWED_MIMETYPES:
			dontmanage.throw(_("You can only upload JPG, PNG, GIF, PDF, TXT, CSV or Microsoft documents."))
...
def check_write_permission(doctype: str | None = None, name: str | None = None):
	...
	try:
		dontmanage.get_lazy_doc(doctype, name, check_permission="write")
		...
		dontmanage.new_doc(doctype).check_permission("write")
		return
...
def run_doc_method(method, docs=None, dt=None, dn=None, arg=None, args=None):
	...
	if dt:  # not called from a doctype (from a page)
		if not dn:
			dn = dt  # single
		doc = dontmanage.get_doc(dt, dn, check_permission=True)

	else:
		docs = dontmanage.parse_json(docs)
		doc = dontmanage.get_doc(docs, check_permission=True)

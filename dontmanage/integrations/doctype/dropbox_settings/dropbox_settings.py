...
from dontmanage.utils import cint, encode, get_backups_path, get_files_path, get_request_site_address
...
class DropboxSettings(Document):
	...
	def validate(self):
		if self.enabled and self.limit_no_of_backups and self.no_of_backups < 1:
			dontmanage.throw(_("Number of DB backups cannot be less than 1"))
...
def take_backup():
	...
	enqueue(
		"dontmanage.integrations.doctype.dropbox_settings.dropbox_settings.take_backup_to_dropbox",
		queue="long",
		timeout=1500,
	)
...
def take_backups_if(freq):
	if dontmanage.db.get_single_value("Dropbox Settings", "backup_frequency") == freq:
...
def take_backup_to_dropbox(retry_count=0, upload_db_backup=True):
	...
	try:
		if cint(dontmanage.db.get_single_value("Dropbox Settings", "enabled")):
			validate_file_size()
			...
			if cint(dontmanage.db.get_single_value("Dropbox Settings", "send_email_for_successful_backup")):
...
def upload_from_folder(
	...
	if is_fresh_upload():
		...
		response = dontmanage._dict({"entries": []})

	path = str(path)

	for f in dontmanage.get_all(
		"File",
		...
		for file_metadata in response.entries:
			try:
				...
				error_log.append(dontmanage.get_traceback())
...
def update_file_dropbox_status(file_name):
	dontmanage.db.set_value("File", file_name, "uploaded_to_dropbox", 1, update_modified=False)


def is_fresh_upload():
	file_name = dontmanage.db.get_value("File", {"uploaded_to_dropbox": 1}, "name")
...
def get_uploaded_files_meta(dropbox_folder, dropbox_client):
	try:
		...
		if isinstance(e.error, dropbox.files.ListFolderError):
			return dontmanage._dict({"entries": []})
		raise
...
def get_dropbox_settings(redirect_uri=False):
	...
	settings = dontmanage.get_doc("Dropbox Settings")
	app_details = {
		"app_key": settings.app_access_key or dontmanage.conf.dropbox_access_key,
		"app_secret": settings.get_password(fieldname="app_secret_key", raise_exception=False)
		if settings.app_secret_key
		else dontmanage.conf.dropbox_secret_key,
		...
		app_details.update(
			{
				...
				+ "/api/method/dontmanage.integrations.doctype.dropbox_settings.dropbox_settings.dropbox_auth_finish"
			}
		)
...
def dropbox_auth_finish():
	...
	if not callback.state or not callback.code:
		dontmanage.respond_as_web_page(
			...
			indicator_color="red",
			http_status_code=dontmanage.AuthenticationError.http_status_code,
		)
		return
	...
	dontmanage.local.response["location"] = "/app/dropbox-settings"
...
def set_dropbox_token(access_token, refresh_token=None):
	...
	dropbox_settings = dontmanage.get_single("Dropbox Settings")

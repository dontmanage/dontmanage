...
class GoogleContacts(Document):
	...
	def get_access_token(self):
		if not self.refresh_token:
			button_label = dontmanage.bold(_("Allow Google Contacts Access"))
...
def get_google_contacts_object(g_contact):
	...
	"""
	account = dontmanage.get_doc("Google Contacts", g_contact)
	oauth_obj = GoogleOAuth("contacts")
...
def sync(g_contact=None):
	...
	google_contacts = dontmanage.get_list("Google Contacts", filters=filters)
...
def sync_contacts_from_google_contacts(g_contact):
	...
	dontmanage.db.set_value("Google Contacts", account.name, "last_sync_on", dontmanage.utils.now_datetime())
...
def insert_contacts_to_google_contacts(doc, method=None):
	...
	"""
	if (
		not dontmanage.db.exists("Google Contacts", {"name": doc.google_contacts})
	...
	try:
		...
		dontmanage.db.set_value("Contact", doc.name, "google_contacts_id", contact.get("resourceName"))
	except HttpError as err:
		dontmanage.msgprint(
...
def update_contacts_to_google_contacts(doc, method=None):
	...
	try:
		...
		dontmanage.msgprint(_("Contact Synced with Google Contacts."))
	except HttpError as err:
		dontmanage.msgprint(

...
dontmanage.ui.form.on("Google Contacts", {
	refresh: function (frm) {
		...
		dontmanage.realtime.on("import_google_contacts", (data) => {
			if (data.progress) {
		...
		if (frm.doc.refresh_token) {
			let sync_button = frm.add_custom_button(__("Sync Contacts"), function () {
				...
				dontmanage
					.call({
						method: "dontmanage.integrations.doctype.google_contacts.google_contacts.sync",
						args: {
	...
	authorize_google_contacts_access: function (frm) {
		dontmanage.call({
			method: "dontmanage.integrations.doctype.google_contacts.google_contacts.authorize_access",
			args: {
				g_contact: frm.doc.name,

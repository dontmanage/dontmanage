...
class S3BackupSettings(Document):
	...
	def validate(self):
		...
		try:
			...
			bucket_name = dontmanage.bold(self.bucket)
			if error_code == "403":
...
def take_backups_if(freq):
	if cint(dontmanage.db.get_single_value("S3 Backup Settings", "enabled")):
		if dontmanage.db.get_single_value("S3 Backup Settings", "frequency") == freq:
			take_backups_s3()
...
def notify():
	error_message = dontmanage.get_traceback()
	send_email(False, "Amazon S3", "S3 Backup Settings", "notify_email", error_message)


def backup_to_s3():
	from dontmanage.utils import get_backups_path
	from dontmanage.utils.backups import new_backup

	doc = dontmanage.get_single("S3 Backup Settings")
	bucket = doc.bucket

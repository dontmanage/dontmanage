...
dontmanage.ui.form.on("S3 Backup Settings", {
	...
	take_backup: function (frm) {
		if (frm.doc.access_key_id && frm.doc.secret_access_key) {
			frm.add_custom_button(__("Take Backup Now"), function () {
				...
				dontmanage.call({
					method: "dontmanage.integrations.doctype.s3_backup_settings.s3_backup_settings.take_backups_s3",
					callback: function (r) {
						if (!r.exc) {
							dontmanage.msgprint(__("S3 Backup complete!"));

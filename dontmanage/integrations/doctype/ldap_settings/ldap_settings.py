...
import dontmanage
from dontmanage import _, safe_encode
from dontmanage.model.document import Document
from dontmanage.twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa
...
class LDAPSettings(Document):
	...
	if TYPE_CHECKING:
		from dontmanage.integrations.doctype.ldap_group_mapping.ldap_group_mapping import LDAPGroupMapping
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			dontmanage.throw(msg, title=_("LDAP Not Installed"))
		except LDAPInvalidCredentialsResult:
			dontmanage.throw(_("Invalid username or password"))
		except Exception as ex:
			dontmanage.throw(_(str(ex)))

	@staticmethod
	def get_ldap_client_settings() -> dict:
		# return the settings to be used on the client side.
		result = {"enabled": False}
		ldap = dontmanage.get_cached_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "dontmanage.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def sync_roles(self, user: "User", additional_groups: list | None = None):
		...
		all_mapped_roles = {r.dontmanageerp_role for r in self.ldap_groups}
		matched_roles = {r.dontmanageerp_role for r in self.ldap_groups if r.ldap_group.lower() in lower_groups}
	...
	def create_or_update_user(self, user_data: dict, groups: list | None = None):
		...
		if dontmanage.db.exists("User", user_data["email"]):
			user = dontmanage.get_doc("User", user_data["email"])
		...
		if self.default_user_type == "System User":
			...
			role = dontmanage.db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		if not self.enabled:
			dontmanage.throw(_("LDAP is not enabled."))
		...
		try:
			...
			dontmanage.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					...
					dontmanage.msgprint(_("Password changed successfully."))
				else:
					dontmanage.throw(_("Failed to change password."))
			else:
				dontmanage.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			dontmanage.throw(_("No LDAP User found for email: {0}").format(user))
	...
	def convert_ldap_entry_to_dict(self, user_entry: Entry):
		...
		if isinstance(email, list):
			...
			for e in email:
				if dontmanage.db.exists("User", e):
					email = e
					break
			else:
...
def login():
	...
	ldap: LDAPSettings = dontmanage.get_doc("LDAP Settings")

	user = ldap.authenticate(dontmanage.as_unicode(args.usr), dontmanage.as_unicode(args.pwd))

	dontmanage.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(dontmanage.local.login_manager):
			return False

	dontmanage.form_dict.pop("pwd", None)
	dontmanage.local.login_manager.post_login()

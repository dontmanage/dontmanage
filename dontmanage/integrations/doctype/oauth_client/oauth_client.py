...
from dontmanage.permissions import SYSTEM_USER_ROLE
...
class OAuthClient(Document):
	...
	if TYPE_CHECKING:
		from dontmanage.integrations.doctype.oauth_client_role.oauth_client_role import OAuthClientRole
	...
	def user_has_allowed_role(self) -> bool:
		...
		return bool(allowed_roles & set(dontmanage.get_roles()))
	...
	def client_id_issued_at(self) -> int:
		...
		try:
			...
			return int(dontmanage.utils.now_datetime().timestamp())

...
from dontmanage.integrations.utils import make_get_request
from dontmanage.model.document import Document

if any((os.getenv("CI"), dontmanage.conf.developer_mode, dontmanage.conf.allow_tests)):
...
class ConnectedApp(Document):
	...
	if TYPE_CHECKING:
		from dontmanage.integrations.doctype.oauth_scope.oauth_scope import OAuthScope
		from dontmanage.integrations.doctype.query_parameters.query_parameters import QueryParameters
	...
	def get_openid_configuration(self):
		if not self.openid_configuration:
			dontmanage.throw(_("Please enter OpenID Configuration URL"))
		return make_get_request(self.openid_configuration)

	def validate(self):
		base_url = dontmanage.utils.get_url()
		callback_path = (
			"/api/method/dontmanage.integrations.doctype.connected_app.connected_app.callback/" + self.name
		)
	...
	def initiate_web_application_flow(self, user=None, success_uri=None):
		...
		if not token_cache:
			token_cache = dontmanage.new_doc("Token Cache")
	...
	def get_user_token(self, user=None, success_uri=None):
		...
		dontmanage.local.response["location"] = redirect
		return redirect
	...
	def get_token_cache(self, user):
		...
		if dontmanage.db.exists("Token Cache", token_cache_name):
			token_cache = dontmanage.get_doc("Token Cache", token_cache_name)
...
@dontmanage.whitelist(methods=["GET"], allow_guest=True)
def callback(code=None, state=None):
	...
	if len(path) != 4 or not path[3]:
		dontmanage.throw(_("Invalid Parameters."))

	connected_app = dontmanage.get_doc("Connected App", path[3])
	token_cache = dontmanage.get_doc("Token Cache", connected_app.name + "-" + dontmanage.session.user)

	if state != token_cache.state:
		dontmanage.throw(_("Invalid token state! Check if the token has been created by the OAuth user."))
	...
	dontmanage.local.response["type"] = "redirect"
	dontmanage.local.response["location"] = token_cache.get("success_uri") or connected_app.get_url()


@dontmanage.whitelist()
def has_token(connected_app, connected_user=None):
	app = dontmanage.get_doc("Connected App", connected_app)
	token_cache = app.get_token_cache(connected_user or dontmanage.session.user)

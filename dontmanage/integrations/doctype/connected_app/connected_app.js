...
dontmanage.ui.form.on("Connected App", {
	refresh: (frm) => {
		frm.add_custom_button(__("Get OpenID Configuration"), async () => {
			if (!frm.doc.openid_configuration) {
				dontmanage.msgprint(__("Please enter OpenID Configuration URL"));
			} else {
				try {
					...
					dontmanage.msgprint(__("Please check OpenID Configuration URL"));
				}
			}
		});

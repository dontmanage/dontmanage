...
from dontmanage.utils.password import set_encrypted_password

if TYPE_CHECKING:
	from dontmanage.desk.doctype.event.event import Event
...
class GoogleCalendar(Document):
	...
	def validate(self):
		google_settings = dontmanage.get_cached_doc("Google Settings")
		...
		if not google_settings.client_id or not google_settings.client_secret:
			dontmanage.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			raise dontmanage.ValidationError(
				_("Click on {0} to generate Refresh Token.").format(dontmanage.bold(allow_google_calendar_label))
			)
		...
		try:
			...
			dontmanage.throw(
				...
				).format(dontmanage.bold(allow_google_calendar_label))
			)
...
def authorize_access(g_calendar: str, reauthorize: bool = False):
	...
	if "refresh_token" in r:
		dontmanage.db.set_value("Google Calendar", google_calendar.name, "refresh_token", r["refresh_token"])
		dontmanage.db.commit()

	dontmanage.local.response["type"] = "redirect"
	dontmanage.local.response["location"] = google_calendar.get_url()

	dontmanage.msgprint(_("Google Calendar has been configured."), indicator="green")
...
def sync(g_calendar: str | None = None):
	...
	for g in dontmanage.get_list("Google Calendar", filters=filters, pluck="name"):
...
def get_google_calendar_object(g_calendar):
	...
	google_settings = dontmanage.get_cached_doc("Google Settings")
	account: GoogleCalendar = dontmanage.get_doc("Google Calendar", g_calendar)
...
def check_google_calendar(account: GoogleCalendar, google_calendar):
	...
	calendar = {
		...
		"timeZone": dontmanage.get_system_settings("time_zone"),
	}
	try:
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				dontmanage.msgprint(msg, title="Invalid Sync Token", indicator="blue")
			else:
	...
	for idx, event in enumerate(results):
		dontmanage.publish_realtime(
			"import_google_calendar", {"progress": idx + 1, "total": len(results)}, user=dontmanage.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			# NOTE: Skip if event is already synced; DontManage doesn't track individual
			# instances of recurring events, so we need to check if the event is already
			# synced in DontManage Calendar
			if event.get("recurringEventId"):
				...
			elif not dontmanage.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
		...
		# If any synced Google Calendar Event is cancelled, then close the Event
			...
			event_name = dontmanage.db.get_value(
				"Event",
				{
...
def insert_event_to_calendar(account, event, recurrence=None):
	"""
	Inserts event in DontManage Calendar during Sync
	"""
	calendar_event = {
		"doctype": "Event",
	...
	e: Event = dontmanage.get_doc(calendar_event)
...
def update_participants_in_event(calendar_event: "Event", google_event: dict):
	...
	in_system_participants = dontmanage.get_all(
...
def update_event_in_calendar(account, event, recurrence=None):
	"""
	Updates Event in DontManage Calendar if any existing Google Calendar Event is updated
	"""
	calendar_event = dontmanage.get_doc("Event", {"google_calendar_event_id": event.get("id")})
...
def insert_event_in_google_calendar(doc, method=None):
	...
	try:
		...
		dontmanage.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		dontmanage.throw(
...
def update_event_in_google_calendar(doc, method=None):
	"""
	Updates Events in Google Calendar if any existing event is modified in DontManage Calendar
	"""
...
def delete_event_from_google_calendar(doc, method=None):
	"""
	Delete Events from Google Calendar if DontManage Event is deleted.
	"""

	if not dontmanage.db.exists("Google Calendar", {"name": doc.google_calendar, "push_to_google_calendar": 1}):
		return
...
def get_attendees(doc):
	...
	owner = dontmanage.db.get_value("Google Calendar", doc.google_calendar, "user")

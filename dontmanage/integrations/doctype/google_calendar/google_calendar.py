...
class GoogleCalendar(Document):
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			button_label = dontmanage.bold(_("Allow Google Calendar Access"))
...
def authorize_access(g_calendar, reauthorize=None):
	...
	redirect_uri = (
		...
		+ "?cmd=dontmanage.integrations.doctype.google_calendar.google_calendar.google_callback"
	)
	...
	if not google_calendar.authorization_code or reauthorize:
		...
		try:
			...
			dontmanage.local.response["location"] = "/app/Form/{}/{}".format(
				quote("Google Calendar"), quote(google_calendar.name)
			)

			dontmanage.msgprint(_("Google Calendar has been configured."))
...
def sync(g_calendar=None):
	...
	google_calendars = dontmanage.get_list("Google Calendar", filters=filters)
...
def get_google_calendar_object(g_calendar):
	...
	"""
	google_settings = dontmanage.get_doc("Google Settings")
	account = dontmanage.get_doc("Google Calendar", g_calendar)
	...
	try:
		if account.google_calendar_id:
			...
			calendar = {
				...
				"timeZone": dontmanage.db.get_single_value("System Settings", "time_zone"),
			}
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	for idx, event in enumerate(results):
		dontmanage.publish_realtime(
			"import_google_calendar", dict(progress=idx + 1, total=len(results)), user=dontmanage.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			if not dontmanage.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
			...
			dontmanage.get_doc(
				{
					...
					"reference_name": dontmanage.db.get_value(
						"Event",
						{
							"google_calendar_id": account.google_calendar_id,
							"google_calendar_event_id": event.get("id"),
...
def delete_event_from_google_calendar(doc, method=None):
	...
	if not dontmanage.db.exists("Google Calendar", {"name": doc.google_calendar}):
		return

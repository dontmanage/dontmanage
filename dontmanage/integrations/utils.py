...
import dontmanage
from dontmanage.integrations.doctype.oauth_client.oauth_client import OAuthClient
...
def make_request(method: str, url: str, auth=None, headers=None, data=None, json=None, params=None):
	...
	try:
		...
		response = dontmanage.flags.integration_request = s.request(
		...
		if dontmanage.flags.integration_request_doc:
			dontmanage.flags.integration_request_doc.log_error()
		else:
...
def create_request_log(
	...
	**kwargs,
	...
	integration_request = dontmanage.get_doc(
		{
...
def get_json(obj):
	return obj if isinstance(obj, str) else dontmanage.as_json(obj, indent=1)
...
def validate_dynamic_client_metadata(client: OAuth2DynamicClientMetadata):
	...
	if not dontmanage.conf.developer_mode and any(c.scheme != "https" for c in client.redirect_uris):
...
def create_new_oauth_client(client: OAuth2DynamicClientMetadata):
	doc = cast(OAuthClient, dontmanage.get_doc({"doctype": "OAuth Client"}))
...
def get_oauth_settings():
	...
	settings: dict[str, Any] = dontmanage._dict({"skip_authorization": None})
	if dontmanage.get_cached_value("OAuth Settings", "OAuth Settings", "skip_authorization"):
		settings["skip_authorization"] = "Auto"  # based on legacy OAuth Provider Settings value

	elif value := dontmanage.get_cached_value(

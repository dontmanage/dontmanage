...
import dontmanage
from dontmanage.utils import convert_utc_to_user_timezone
from dontmanage.utils.background_jobs import get_queues, get_workers
...
def get_info(view=None, queue_timeout=None, job_status=None) -> list[dict]:
	...
	def add_job(job: "Job", queue: str) -> None:
		...
		if job.kwargs.get("site") == dontmanage.local.site:
			job_info = {
	...
	if view == "Jobs":
		...
		for worker in workers:
			...
			if current_job:
				if hasattr(current_job, "kwargs") and current_job.kwargs.get("site") == dontmanage.local.site:

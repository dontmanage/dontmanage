dontmanage.pages["background_jobs"].on_page_load = (wrapper) => {
...
class BackgroundJobs {
	constructor(wrapper) {
		...
		this.page.add_inner_button(__("Remove Failed Jobs"), () => {
			dontmanage.confirm(__("Are you sure you want to remove all failed jobs?"), () => {
				dontmanage
					.call("dontmanage.core.page.background_jobs.background_jobs.remove_failed_jobs")
		...
		this.make_filters();
		this.refresh_jobs = dontmanage.utils.throttle(this.refresh_jobs.bind(this), 1000);
	}
	...
	update_scheduler_status() {
		dontmanage.call({
			method: "dontmanage.core.page.background_jobs.background_jobs.get_scheduler_status",
			callback: (r) => {
	...
	refresh_jobs() {
		...
		dontmanage.call({
			method: "dontmanage.core.page.background_jobs.background_jobs.get_info",
			args,
			callback: (res) => {
				...
				this.$content.html(
					dontmanage.render_template(template, {
						jobs: res.message || [],
					})
				);

				let auto_refresh = this.auto_refresh.get_value();
				if (dontmanage.get_route()[0] === "background_jobs" && auto_refresh) {

...
dontmanage.PermissionEngine = class PermissionEngine {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page
			...
			.change(function () {
				dontmanage.set_route("permission-manager", $(this).val());
			});
	...
	set_from_route() {
		...
		if (dontmanage.get_route()[1]) {
			this.doctype_select.val(dontmanage.get_route()[1]);
		} else if (dontmanage.route_options) {
			if (dontmanage.route_options.doctype) {
				this.doctype_select.val(dontmanage.route_options.doctype);
			}
			if (dontmanage.route_options.role) {
				this.role_select.val(dontmanage.route_options.role);
			}

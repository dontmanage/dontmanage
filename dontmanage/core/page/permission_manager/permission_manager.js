dontmanage.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = dontmanage.ui.make_app_page({
		parent: wrapper,
	...
	$(dontmanage.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new dontmanage.PermissionEngine(wrapper);
};

dontmanage.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

dontmanage.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page.add_field({
			...
			change: function () {
				dontmanage.set_route("permission-manager", this.get_value());
			},
		});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return dontmanage.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (dontmanage.get_route()[1]) {
			this.doctype_select.set_value(dontmanage.get_route()[1]);
		} else if (dontmanage.route_options) {
			if (dontmanage.route_options.doctype) {
				this.doctype_select.set_value(dontmanage.route_options.doctype);
			}
			if (dontmanage.route_options.role) {
				this.role_select.set_value(dontmanage.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = dontmanage.confirm(__("Reset Permissions for {0}?", [__(doctype)]), () => {
			return dontmanage
				.call({
					module: "dontmanage.core",
					page: "permission_manager",
		...
		let $d = $(d.wrapper)
			.find(".dontmanage-confirm-message")
		...
		data.message.forEach((d) => {
			...
			d.rights = this.rights
				...
				.map((r) => {
					return __(toTitle(dontmanage.unscrub(r)));
				})
				.join(", ");
						...
						if (user_count === 0) {
							...
							const user_list = display_users
								.map(
									(user) =>
										`<li class="py-1">
                        					${dontmanage.utils.get_form_link("User", user, true)}
                    					</li>`
								)
								.join("");
							...
							if (user_count > display_count) {
								...
								message_html += `<div class="text-center">
                    								<a href="${route}" class="text-muted">
														${dontmanage.utils.icon("external-link", "sm", "mr-1")}
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${dontmanage.utils.icon(
				"x"
			)}</button>`
		)
			...
			.on("click", () => {
				return dontmanage.call({
					...
					callback: (r) => {
						if (r.exc) {
							dontmanage.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			dontmanage.route_options = { allow: this.get_doctype() || "" };
			...
			() => {
				...
				d.set_primary_action(__("Add"), () => {
					...
					dontmanage.call({
						...
						callback: (r) => {
							if (r.exc) {
								dontmanage.msgprint(__("Did not add"));
							} else {
								this.refresh();
							}
						},
	...
	get_link_fields(doctype) {
		return dontmanage.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",

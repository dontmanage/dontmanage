...
from dontmanage.permissions import get_rights
...
def execute(filters=None):
	...
	data = dontmanage.get_list(doctype, fields=fields, as_list=True, user=user)

	if show_permissions:
		rights = get_rights(doctype)
		columns = columns + [dontmanage.unscrub(right) + ":Check:80" for right in rights]
		data = list(data)
		for i, doc in enumerate(data):
			permission = dontmanage.permissions.get_doc_permissions(dontmanage.get_doc(doctype, doc[0]), user)
...
def get_columns_and_fields(doctype):
	...
	fields = ["name"]
	for df in dontmanage.get_meta(doctype).fields:
...
def query_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	user_perms = dontmanage.utils.user.UserPermissions(user)
	...
	single_doctypes = [d[0] for d in dontmanage.db.get_values("DocType", {"issingle": 1})]

	return [
		[dt]
		for dt in can_read
		if txt.lower().replace("%", "") in dontmanage._(dt).lower()

...
from dontmanage.config import get_modules_from_app
from dontmanage.model.document import Document
from dontmanage.permissions import add_permission, add_user_permission
...
class UserType(Document):
	...
	def on_trash(self):
		if self.is_standard:
			dontmanage.throw(_("Standard user type {0} can not be deleted.").format(dontmanage.bold(self.name)))

	def set_modules(self):
		if not self.user_doctypes:
			return

		modules = dontmanage.get_all(
			"DocType",
	...
	def validate_document_type_limit(self):
		limit = dontmanage.conf.get("user_type_doctype_limit", {}).get(dontmanage.scrub(self.name))

		if not limit and dontmanage.session.user != "Administrator":
			dontmanage.throw(
				_("User does not have permission to create the new {0}").format(dontmanage.bold(_("User Type"))),
	...
	def validate_role(self):
		if not self.role:
			dontmanage.throw(_("The field {0} is mandatory").format(dontmanage.bold(_("Role"))))

		if not dontmanage.db.get_value("Role", self.role, "is_custom"):
			dontmanage.throw(
				_("The role {0} should be a custom role.").format(
					dontmanage.bold(get_link_to_form("Role", self.role))
				)
			)

	def update_users(self):
		for row in dontmanage.get_all("User", filters={"user_type": self.name}):
			user = dontmanage.get_cached_doc("User", row.name)
	...
	def update_modules_in_user(self, user):
		block_modules = dontmanage.get_all(
			"Module Def",
	...
	def add_role_permissions_for_user_doctypes(self):
		...
		for row in self.user_doctypes:
			...
			dontmanage.db.set_value("Custom DocPerm", docperm, values)

	def add_select_perm_doctypes(self):
		if dontmanage.flags.ignore_select_perm:
			return
	...
	def add_role_permissions_for_file(self):
		...
		dontmanage.db.set_value("Custom DocPerm", docperm, {"read": 1, "create": 1, "write": 1})
	...
	def remove_permission_for_deleted_doctypes(self):
		...
		for perm in dontmanage.get_all(
			...
			dontmanage.delete_doc("Custom DocPerm", perm.name)


def add_role_permissions(doctype, role):
	name = dontmanage.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=0))
...
def get_user_linked_doctypes(doctype, txt, searchfield, start, page_len, filters):
	modules = [d.get("module_name") for d in get_modules_from_app("dontmanage")]
	...
	custom_doctypes = dontmanage.get_all(
...
def user_linked_with_permission_on_doctype(doc, user):
	...
	if not doc.user_id_field:
		dontmanage.throw(_("User Id Field is mandatory in the user type {0}").format(dontmanage.bold(doc.name)))

	if dontmanage.db.get_value(doc.apply_user_permission_on, {doc.user_id_field: user}, "name"):
		return True
	else:
		label = dontmanage.get_meta(doc.apply_user_permission_on).get_field(doc.user_id_field).label

		dontmanage.msgprint(
			_(
				"To set the role {0} in the user {1}, kindly set the {2} field as {3} in one of the {4} record."
			).format(
				dontmanage.bold(doc.role),
				dontmanage.bold(user),
				dontmanage.bold(label),
				dontmanage.bold(user),
				dontmanage.bold(doc.apply_user_permission_on),
			)
		)
...
def apply_permissions_for_non_standard_user_type(doc, method=None):
	...
	for user_type, data in user_types.items():
		...
		if dontmanage.get_cached_value("User", doc.get(data[1]), "user_type") != user_type:
			return

		if doc.get(data[1]) and (
			not doc._doc_before_save
			or doc.get(data[1]) != doc._doc_before_save.get(data[1])
			or not dontmanage.db.get_value(
			...
			if not perm_data:
				user_doc = dontmanage.get_cached_doc("User", doc.get(data[1]))
				...
				dontmanage.db.set_value("User Permission", perm_data[0], "user", doc.get(data[1]))

...
class SystemSettings(Document):
	...
	def validate_backup_limit(self):
		if not self.backup_limit or self.backup_limit < 1:
			dontmanage.msgprint(_("Number of backups must be greater than zero."), alert=True)
			self.backup_limit = 1
...
def load():
	...
	if "System Manager" not in dontmanage.get_roles():
...
def get_system_settings(key: str):
	...
	if not (system_settings := getattr(dontmanage.local, "system_settings", None)):
		try:
			system_settings = dontmanage.client_cache.get_doc("System Settings")
			dontmanage.local.system_settings = system_settings
		except dontmanage.DoesNotExistError:  # possible during new install
...
def clear_system_settings_cache():
	dontmanage.client_cache.delete_value(dontmanage.get_document_cache_key("System Settings", "System Settings"))
	dontmanage.cache.delete_value("system_settings")
	dontmanage.cache.delete_value("time_zone")


def sync_system_settings():
	if dontmanage.db.get_single_value("System Settings", "currency") is None:
		dontmanage.db.set_single_value("System Settings", "currency", dontmanage.defaults.get_defaults()["currency"])

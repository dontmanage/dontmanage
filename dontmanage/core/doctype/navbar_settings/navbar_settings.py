...
def get_app_logo():
	app_logo = dontmanage.get_website_settings("app_logo") or dontmanage.get_cached_value(
		"Navbar Settings",
		"Navbar Settings",
		"app_logo",
	)
	...
	if not app_logo:
		logos = dontmanage.get_hooks("app_logo_url")
		app_logo = logos[0]
		if len(logos) == 2:
			app_logo = logos[1]
...
def sync_table(key, hook):
	...
	for item in dontmanage.get_hooks(hook):

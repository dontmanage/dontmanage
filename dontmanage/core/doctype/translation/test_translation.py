...
from dontmanage.translate import clear_cache


class TestTranslation(DontManageTestCase):
	def setUp(self):
		dontmanage.db.delete("Translation")
	...
	def test_doctype(self):
		...
		for key, val in translation_data.items():
			dontmanage.local.lang = key

			translation = create_translation(key, val)
			self.assertEqual(_(val[0]), val[1])

			dontmanage.delete_doc("Translation", translation.name)
...
def create_translation(key, val):
	translation = dontmanage.new_doc("Translation")
...
def clear_translation_cache():
	dontmanage.cache().delete_key("translations_from_apps", shared=True)

...
from dontmanage.utils import cint, cstr, duration_to_seconds, flt, update_progress_bar
from dontmanage.utils.csvutils import get_csv_content_from_google_sheets, read_csv_content
from dontmanage.utils.xlsxutils import (
...
class Importer:
	def __init__(
		...
		if not self.data_import:
			self.data_import = dontmanage.get_doc(doctype="Data Import")
			if import_type:
				self.data_import.import_type = import_type

		self.template_options = dontmanage.parse_json(self.data_import.template_options or "{}")
	...
	def get_data_for_import_preview(self):
		...
		out.import_log = dontmanage.get_all(
			"Data Import Log",
	...
	def before_import(self):
		...
		dontmanage.flags.mute_emails = self.data_import.mute_emails
	...
	def import_data(self):
		...
		if (
			...
			dontmanage.db.delete("Data Import Log", {"success": 0, "data_import": self.data_import.name})

		# get successfully imported rows
		imported_rows = []
		for log in import_log:
			log = dontmanage._dict(log)
		...
		batch_size = dontmanage.conf.data_import_batch_size or 1000

		for batch_index, batched_payloads in enumerate(dontmanage.utils.create_batch(payloads, batch_size)):
			for i, payload in enumerate(batched_payloads):
				...
				try:
					...
					messages = dontmanage.local.message_log
					...
					create_import_log(
						...
						{
							"success": False,
							"exception": dontmanage.get_traceback(),
							"messages": messages,
							"row_indexes": row_indexes,
						},
					)
	...
	def insert_record(self, doc):
		...
		new_doc = dontmanage.new_doc(self.doctype)
		new_doc.update(doc)
	...
	def update_record(self, doc):
		...
		existing_doc = dontmanage.get_doc(self.doctype, doc.get(id_field.fieldname))

		updated_doc = dontmanage.get_doc(self.doctype, doc.get(id_field.fieldname))
		...
		if get_diff(existing_doc, updated_doc):
			...
			# throw if no changes
			dontmanage.throw(_("No changes to update"))
	...
	def export_import_log(self):
		...
		for log in import_log:
			...
			exception = dontmanage.utils.cstr(log.get("exception", ""))
	...
	def print_import_log(self, import_log):
		...
		if failed_records:
			...
			file_name = f"{self.doctype}_import_on_{dontmanage.utils.now()}.txt"
...
class ImportFile:
	def __init__(
		...
		if isinstance(file, str):
			if dontmanage.db.exists("File", {"file_url": file}):
				self.file_doc = dontmanage.get_doc("File", {"file_url": file})
		...
		if not self.file_doc and not self.file_path and not self.google_sheets_url:
			dontmanage.throw(_("Invalid template file for import"))
	...
	def get_data_from_template_file(self):
		...
		if not content:
			dontmanage.throw(_("Invalid or corrupted content for import"))
	...
	def get_data_for_import_preview(self):
		...
		columns = [dontmanage._dict({"header_title": "Sr. No", "skip_import": True})]
	...
	def get_payloads_for_import(self):
		...
		while data:
			...
			payloads.append(dontmanage._dict(doc=doc, rows=rows))
		return payloads
	...
	def read_content(self, content, extension):
		...
		if extension not in ("csv", "xlsx", "xls"):
			dontmanage.throw(_("Import template should be of type .csv, .xlsx or .xls"), title=error_title)
...
class Row:
	...
	def _parse_doc(self, doctype, columns, values, parent_doc=None, table_df=None):
		...
		is_table = dontmanage.get_meta(doctype).istable
		...
		if is_table and is_update:
			...
			if id_value and dontmanage.db.exists(doctype, id_value):
				existing_doc = dontmanage.get_doc(doctype, id_value)
				existing_doc.update(doc)
				doc = existing_doc
			else:
				# for table rows being inserted in update
				# create a new doc with defaults set
				new_doc = dontmanage.new_doc(doctype, as_dict=True)
				new_doc.update(doc)
				doc = new_doc
	...
	def validate_value(self, value, col):
		...
		if df.fieldtype == "Select":
			...
			if not exists:
				msg = _("Value {0} missing for {1}").format(dontmanage.bold(value), dontmanage.bold(df.options))
				self.warnings.append(
					{
						"row": self.row_number,
						"field": df_as_json(df),
				...
				self.warnings.append(
					{
						...
						"message": _("Value {0} must in {1} format").format(
							dontmanage.bold(value), dontmanage.bold(get_user_format(col.date_format))
						),
					}
				)
				return
...
class Header(Row):
	def __init__(self, index, row, doctype, raw_data, column_to_field_map=None):
		...
		self.data = row
		self.doctype = doctype
		column_to_field_map = column_to_field_map or dontmanage._dict()
...
class Column:
	def __init__(self, index, header, doctype, column_values, map_to_field=None, seen=None):
		...
		self.meta = dontmanage.get_meta(doctype)
		self.parse()
	...
	def parse(self):
				...
				self.warnings.append(
					{
						"message": _("Mapping column {0} to field {1}").format(
							dontmanage.bold(header_title or "<i>Untitled Column</i>"), dontmanage.bold(df.label)
						),
						"type": "info",
					}
				)
		...
		if header_title in self.seen:
			self.warnings.append(
				{
					"col": column_number,
					"message": _("Skipping Duplicate Column {0}").format(dontmanage.bold(header_title)),
					"type": "info",
				}
			)
			df = None
			...
			self.warnings.append(
				{
					"col": column_number,
					"message": _("Skipping column {0}").format(dontmanage.bold(header_title)),
					"type": "info",
				}
			)
			...
			self.warnings.append(
				{
					"col": column_number,
					"message": _("Cannot match column {0} with any field").format(dontmanage.bold(header_title)),
					"type": "info",
				}
			)
		...
		def guess_date_format(d):
			if isinstance(d, datetime | date | time):
				...
				if self.df.fieldtype == "Time":
					return "%H:%M:%S"
			if isinstance(d, str):
				return dontmanage.utils.guess_date_format(d)
			...
			self.warnings.append(
				{
					...
					"message": message.format(
						dontmanage.bold(self.header_title),
						len(unique_date_formats),
						dontmanage.bold(user_date_format),
					),
					"type": "info",
				}
			)
	...
	def validate_values(self):
		...
		if self.df.fieldtype == "Link":
			...
			transform = (lambda v: cstr(v).lower()) if dontmanage.db.db_type == "mariadb" else cstr
			values = list({transform(v) for v in self.column_values if v})
			exists = [
				transform(d.name) for d in dontmanage.get_all(self.df.options, filters={"name": ("in", values)})
			]
			...
			if options:
				...
				if invalid:
					valid_values = ", ".join(dontmanage.bold(o) for o in options)
					invalid_values = ", ".join(dontmanage.bold(i) for i in invalid)
...
def build_fields_dict_for_column_matching(parent_doctype):
	...
	for doctype, table_df in doctypes:
		...
		fields = get_standard_fields(doctype) + dontmanage.get_meta(doctype).fields
		for df in fields:
			...
			if parent_doctype == doctype:
				...
				if isinstance(df, dict):
					new_df = dontmanage._dict(df.copy())
				else:
					new_df = df.as_dict()
...
def get_id_field(doctype):
	...
	return dontmanage._dict({"label": "ID", "fieldname": "name", "fieldtype": "Data"})

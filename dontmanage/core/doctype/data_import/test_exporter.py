...
from dontmanage.core.doctype.data_import.test_importer import create_doctype_if_not_exists
from dontmanage.tests.utils import DontManageTestCase

doctype_name = "DocType for Export"


class TestExporter(DontManageTestCase):
	def setUp(self):
		create_doctype_if_not_exists(doctype_name)

	def test_exports_specified_fields(self):
		if not dontmanage.db.exists(doctype_name, "Test"):
			...
			doc = dontmanage.get_doc(doctype_name, "Test")
	...
	def test_export_csv_response(self):
		...
		self.assertEqual(dontmanage.response["doctype"], doctype_name)

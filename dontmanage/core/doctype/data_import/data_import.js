...
dontmanage.ui.form.on("Data Import", {
	setup(frm) {
		dontmanage.realtime.on("data_import_refresh", ({ data_import }) => {
			frm.import_in_progress = false;
			if (data_import !== frm.doc.name) return;
			dontmanage.model.clear_doc("Data Import", frm.doc.name);
			dontmanage.model.with_doc("Data Import", frm.doc.name).then(() => {
				frm.refresh();
			});
		});
		dontmanage.realtime.on("data_import_progress", (data) => {
		...
		frm.set_query("reference_doctype", () => {
			return {
				filters: {
					name: ["in", dontmanage.boot.user.can_import],
				},
			};
		});
	...
	refresh(frm) {
		...
		if (frm.doc.status.includes("Success")) {
			frm.add_custom_button(__("Go to {0} List", [__(frm.doc.reference_doctype)]), () =>
				dontmanage.set_route("List", frm.doc.reference_doctype)
			);
		}
	},
	...
	update_indicators(frm) {
		const indicator = dontmanage.get_indicator(frm.doc);
		if (indicator) {
	...
	show_import_status(frm) {
		dontmanage.call({
			method: "dontmanage.core.doctype.data_import.data_import.get_import_status",
			args: {
	...
	show_cancel_import_btn(frm) {
		frm.add_custom_button(__("Cancel Import"), () => {
			dontmanage.confirm(
				...
				() => {
					dontmanage
						.xcall("dontmanage.core.doctype.data_import.data_import.stop_data_import", {
				...
				.then((result) => {
					if (result.length > 0) {
						frm.add_custom_button("Report Error", () => {
							...
							dontmanage.request.report_error(fake_xhr, {});
						});
					}
				});
		}
	...
	download_template(frm) {
		dontmanage.require("data_import_tools.bundle.js", () => {
			frm.data_exporter = new dontmanage.data_import.DataExporter(
	...
	show_import_preview(frm, preview_data) {
		...
		dontmanage.require("data_import_tools.bundle.js", () => {
			frm.import_preview = new dontmanage.data_import.ImportPreview({
	...
	export_errored_rows(frm) {
		open_url_post(
			"/api/method/dontmanage.core.doctype.data_import.data_import.download_errored_template",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	export_import_log(frm) {
		open_url_post(
			"/api/method/dontmanage.core.doctype.data_import.data_import.download_import_log",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	render_import_log(frm) {
		dontmanage.call({
			method: "dontmanage.core.doctype.data_import.data_import.get_import_logs",
			args: {
				data_import: frm.doc.name,
			},
			callback: function (r) {
						...
						if (log.success) {
							if (frm.doc.import_type === "Insert New Records") {
								html = __("Successfully imported {0}", [
									`<span class="underline">${dontmanage.utils.get_form_link(
										frm.doc.reference_doctype,
										log.docname,
										true
									)}<span>`,
							...
							let messages = JSON.parse(log.messages || "[]")
								...
								.join("");
							let id = dontmanage.dom.get_unique_id();
							html = `${messages}
	...
	show_import_log(frm) {
		...
		dontmanage.call({
			method: "dontmanage.client.get_count",
			args: {

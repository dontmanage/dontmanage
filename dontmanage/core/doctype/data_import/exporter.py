...
from dontmanage.model import table_fields as table_fieldtypes
from dontmanage.utils import flt, format_duration, groupby_metric
from dontmanage.utils.csvutils import build_csv_response
from dontmanage.utils.xlsxutils import build_xlsx_response
...
class Exporter:
	...
	def serialize_exportable_fields(self):
		...
		for key, exportable_fields in self.exportable_fields.items():
			for _df in exportable_fields:
				...
				if isinstance(_df, dontmanage.core.doctype.docfield.docfield.DocField):
					df = _df.as_dict()
				else:
					df = _df.copy()
	...
	def get_data_to_export(self):
		...
		if not dontmanage.permissions.can_export(self.doctype):
			if dontmanage.permissions.can_export(self.doctype, is_owner=True):
				for doc in data:
					if doc.get("owner") != dontmanage.session.user:
		...
		for i, df in enumerate(self.fields):
			if df.parent == doctype:
				...
				if df.fieldtype == "Text Editor" and value:
					value = dontmanage.core.utils.html2text(value)
				row[i] = value
		return rows

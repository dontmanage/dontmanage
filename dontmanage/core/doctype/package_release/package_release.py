...
class PackageRelease(Document):
	...
	def validate(self):
		...
		package_path = Path(dontmanage.get_site_path("packages", package.package_name))
		if not package_path.resolve().is_relative_to(Path(dontmanage.get_site_path()).resolve()):
			dontmanage.throw("Invalid package path: " + package_path.as_posix())

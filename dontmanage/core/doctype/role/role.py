...
from dontmanage.website.path_resolver import validate_path
...
class Role(Document):
	...
	def disable_role(self):
		if self.name in STANDARD_ROLES:
			dontmanage.throw(dontmanage._("Standard roles cannot be disabled"))
		else:
			self.remove_roles()

	def validate_homepage(self):
		if dontmanage.request and self.home_page:
	...
	def remove_roles(self):
		dontmanage.db.delete("Has Role", {"role": self.name})
...
def get_user_info(users, field="email"):
	...
	info_list = []
	for user in users:
		user_info, enabled = dontmanage.db.get_value("User", user.get("user_name"), [field, "enabled"])

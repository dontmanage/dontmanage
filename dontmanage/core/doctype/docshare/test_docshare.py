...
class TestDocShare(IntegrationTestCase):
	...
	def test_share_with_submit_perm(self):
		...
		submittable_doc = dontmanage.get_doc(doctype=doctype, test="test docshare with submit").insert()
	...
	def test_cannot_share_without_permission(self):
		...
		doc = dontmanage.new_doc("Communication", subject="Hello World").save()
		...
		try:
			...
			# Can share read
			dontmanage.share.add("Communication", doc.name, "test1@example.com")
		finally:
			doc.delete()

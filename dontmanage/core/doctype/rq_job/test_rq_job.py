...
import dontmanage
from dontmanage.core.doctype.rq_job.rq_job import RQJob, remove_failed_jobs, stop_job
from dontmanage.installer import update_site_config
from dontmanage.tests import IntegrationTestCase, timeout
from dontmanage.utils import cstr, execute_in_shell
from dontmanage.utils.background_jobs import get_job_status, is_job_enqueued
...
class TestRQJob(IntegrationTestCase):
	BG_JOB = "dontmanage.core.doctype.rq_job.test_rq_job.test_func"

	def setUp(self) -> None:
		# Cleanup all pending jobs
		for job in dontmanage.get_all("RQ Job", {"status": "queued"}):
			dontmanage.get_doc("RQ Job", job.name).cancel()
		return super().setUp()
	...
	def test_serialization(self):
		job = dontmanage.enqueue(method=self.BG_JOB, queue="short")
		rq_job = dontmanage.get_doc("RQ Job", job.id)
	...
	def test_configurable_ttl(self):
		dontmanage.conf.rq_job_failure_ttl = 600
		job = dontmanage.enqueue(method=self.BG_JOB, queue="short")

		self.assertEqual(job.failure_ttl, 600)

	def test_func_obj_serialization(self):
		job = dontmanage.enqueue(method=test_func, queue="short")
		rq_job = dontmanage.get_doc("RQ Job", job.id)
		self.assertEqual(rq_job.job_name, "dontmanage.core.doctype.rq_job.test_rq_job.test_func")

	@timeout
	def test_get_list_filtering(self):
		# Check failed job clearning and filtering
		remove_failed_jobs()
		jobs = dontmanage.get_all("RQ Job", {"status": "failed"})
		...
		# Fail a job
		job = dontmanage.enqueue(method=self.BG_JOB, queue="short", fail=True)
		...
		non_failed_jobs = dontmanage.get_all("RQ Job", {"status": ("!=", "failed")})
		self.assertGreaterEqual(len(non_failed_jobs), 1)

		# Create a slow job and check if it's stuck in "Started"
		job = dontmanage.enqueue(method=self.BG_JOB, queue="short", sleep=10)
		time.sleep(3)
	...
	def test_delete_doc(self):
		job = dontmanage.enqueue(method=self.BG_JOB, queue="short")
		dontmanage.get_doc("RQ Job", job.id).delete()
	...
	def test_multi_queue_burst_consumption(self):
		for _ in range(3):
			for q in ["default", "short"]:
				dontmanage.enqueue(self.BG_JOB, sleep=1, queue=q)
	...
	def test_job_id_manual_dedup(self):
		job_id = "test_dedup"
		job = dontmanage.enqueue(self.BG_JOB, sleep=5, job_id=job_id)
	...
	def test_auto_job_dedup(self):
		job_id = "test_dedup"
		job1 = dontmanage.enqueue(self.BG_JOB, sleep=2, job_id=job_id, deduplicate=True)
		job2 = dontmanage.enqueue(self.BG_JOB, sleep=5, job_id=job_id, deduplicate=True)
		...
		job3 = dontmanage.enqueue(self.BG_JOB, fail=True, job_id=job_id, deduplicate=True)
		self.check_status(job3, "failed")
		job4 = dontmanage.enqueue(self.BG_JOB, sleep=1, job_id=job_id, deduplicate=True)
		self.check_status(job4, "finished")

	@timeout
	def test_enqueue_after_commit(self):
		job_id = dontmanage.generate_hash()

		dontmanage.enqueue(self.BG_JOB, enqueue_after_commit=True, job_id=job_id)
	...
	def test_memory_usage(self):
		if dontmanage.db.db_type != "mariadb":
			return
		job = dontmanage.enqueue("dontmanage.utils.data._get_rss_memory_usage")
		self.check_status(job, "finished")
		...
		# Refer this PR: https://github.com/dontmanage/dontmanage/pull/21467
		LAST_MEASURED_USAGE = 46
		if dontmanage.conf.use_mysqlclient:
	...
	def test_clear_failed_jobs(self):
		...
		jobs = [dontmanage.enqueue(method=self.BG_JOB, queue="short", fail=True) for _ in range(limit * 2)]

...
from dontmanage.utils.background_jobs import get_queues, get_redis_conn
...
def serialize_job(job: Job) -> dontmanage._dict:
	...
	if job_name == "dontmanage.utils.background_jobs.run_doc_method":
	...
	return dontmanage._dict(
		...
		exc_info=exc_info,
		arguments=dontmanage.as_json(job.kwargs),
		timeout=job.timeout,
...
def for_current_site(job: Job) -> bool:
	return job.kwargs.get("site") == dontmanage.local.site
...
@dontmanage.whitelist()
def stop_job(job_id):
	dontmanage.get_doc("RQ Job", job_id).stop_job()


@dontmanage.whitelist()
def get_custom_queues():
	dontmanage.has_permission("RQ Job", throw=True)
	return list((dontmanage.conf.workers or {}).keys())

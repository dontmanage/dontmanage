...
import dontmanage
from dontmanage.core.doctype.log_settings.log_settings import _supports_log_clearing, run_log_clean_up
from dontmanage.tests.utils import DontManageTestCase
from dontmanage.utils import add_to_date, now_datetime


class TestLogSettings(DontManageTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def setUp(self) -> None:
		if self._testMethodName == "test_delete_logs":
			self.datetime = dontmanage._dict()
	...
	def test_delete_logs(self):
		...
		activity_log_count = dontmanage.db.count("Activity Log", {"creation": ("<=", self.datetime.past)})
		error_log_count = dontmanage.db.count("Error Log", {"creation": ("<=", self.datetime.past)})
		email_queue_count = dontmanage.db.count("Email Queue", {"creation": ("<=", self.datetime.past)})
...
def setup_test_logs(past: datetime) -> None:
	activity_log = dontmanage.get_doc(
		{

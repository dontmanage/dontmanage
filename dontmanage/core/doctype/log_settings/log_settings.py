...
def clear_log_table(doctype, days=90):
	...
	try:
		if dontmanage.db.db_type == "postgres":
			dontmanage.db.sql_ddl(f'CREATE TABLE "{temporary}" (LIKE "{original}" INCLUDING ALL)')
			...
			dontmanage.db.sql_ddl(f'ALTER TABLE "{original}" RENAME TO "{backup}"')
			dontmanage.db.sql_ddl(f'ALTER TABLE "{temporary}" RENAME TO "{original}"')

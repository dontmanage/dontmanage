...
from dontmanage.core.doctype.doctype.doctype import validate_series
from dontmanage.model.document import Document
from dontmanage.model.naming import NamingSeries
from dontmanage.permissions import get_doctypes_with_read


class NamingSeriesNotSetError(dontmanage.ValidationError):
	pass
...
class DocumentNamingSettings(Document):
	...
	def _get_transactions(self) -> list[str]:
		...
		custom = dontmanage.get_all("Custom Field", {"fieldname": "naming_series"}, "dt", pluck="dt")
	...
	def _get_prefixes(self, doctypes) -> list[str]:
		...
		for d in doctypes:
			try:
				options = dontmanage.get_meta(d).get_naming_series_options()
				series_templates.update(options)
			except dontmanage.DoesNotExistError:
				dontmanage.msgprint(_("Unable to find DocType {0}").format(d))
				continue

		custom_templates = dontmanage.get_all(
			"DocType",
			fields=["autoname"],
			filters={
	...
	def _evaluate_and_clean_templates(self, series_templates: set[str]) -> list[str]:
		...
		prefixes_from_db = dontmanage.qb.from_(series).select(series.name).run(pluck=True)
		...
		for series_template in series_templates:
			try:
				...
				dontmanage.log_error(f"Invalid naming series {series_template}")
	...
	def validate_set_series(self):
		if self.transaction_type and not self.naming_series_options:
			dontmanage.throw(_("Please set the series to be used."))
	...
	def check_duplicate(self):
		...
		for doctype in all_doctypes_with_naming_series:
			for series in dontmanage.get_meta(doctype).get_naming_series_options():
				existing_series[stripped_series(series)] = doctype

		dt = dontmanage.get_doc("DocType", self.transaction_type)

		options = self.get_options_list(self.naming_series_options)
		for series in options:
			if stripped_series(series) in existing_series:
				dontmanage.throw(_("Series {0} already used in {1}").format(series, existing_series[series]))
	...
	def get_options(self, doctype=None):
		...
		if dontmanage.get_meta(doctype or self.transaction_type).get_field("naming_series"):
			return dontmanage.get_meta(doctype or self.transaction_type).get_field("naming_series").options
	...
	def update_series_start(self):
		...
		if self.prefix is None:
			dontmanage.throw(_("Please select prefix first"))
	...
	def create_version_log_for_change(self, series, old, new):
		...
		version.data = dontmanage.as_json({"changed": [["current", old, new]]})
	...
	def _fetch_last_doc_if_available(self):
		...
		try:
			return dontmanage.get_last_doc(self.transaction_type)
		except Exception:
			return None

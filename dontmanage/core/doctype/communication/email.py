...
def make(
	...
	**kwargs,
	...
	if doctype and name:
		dontmanage.has_permission(doctype, doc=name, ptype="email", throw=True)

	if (
		raw_html
		and email_template
		and not dontmanage.get_cached_value("Email Template", email_template, "use_html")
	):
		warn(
			_(
...
def _make(
	...
	comm.flags.skip_add_signature = not add_signature or (
		raw_html and dontmanage.get_cached_value("Email Template", email_template, "use_html")
	)

dontmanage.ui.form.on("Communication", {
	onload: function (frm) {
		if (frm.doc.content) {
			frm.doc.content = dontmanage.dom.remove_script_and_style(frm.doc.content);
		}
	...
	refresh: function (frm) {
		...
		if (frm.doc.reference_doctype && frm.doc.reference_name) {
			frm.add_custom_button(__(frm.doc.reference_name), function () {
				dontmanage.set_route("Form", frm.doc.reference_doctype, frm.doc.reference_name);
			});
		} else {
			...
			fields: [
				{
					...
					get_query: function () {
						return { query: "dontmanage.email.get_communication_doctype" };
					},
				},
				{
			...
			if (values) {
				dontmanage.confirm(
					...
					function () {
						d.hide();
						dontmanage.call({
							method: "dontmanage.email.relink",
							args: {
								name: frm.doc.name,
	...
	show_move_dialog: function (frm) {
		var d = new dontmanage.ui.Dialog({
			...
			primary_action(values) {
				d.hide();
				dontmanage.call({
					method: "dontmanage.email.inbox.move_email",
					args: {
	...
	mark_as_read_unread: function (frm) {
		...
		return dontmanage.call({
			method: "dontmanage.email.inbox.create_email_flag_queue",
			args: {
				names: [frm.doc.name],
				action: action,
				flag: flag,
	...
	mark_as_closed_open: function (frm) {
		...
		return dontmanage.call({
			method: "dontmanage.email.inbox.mark_as_closed_open",
			args: {
				communication: frm.doc.name,
				status: status,
			},
	...
	get_mail_args: function (frm) {
		var sender_email_id = "";
		$.each(dontmanage.boot.email_accounts, function (idx, account) {
	...
	mark_as_spam: function (frm) {
		dontmanage.call({
			method: "dontmanage.email.inbox.mark_as_spam",
			args: {
				communication: frm.doc.name,
			...
			freeze: true,
			callback: function (r) {
				dontmanage.msgprint(__("Email has been marked as spam"));
			},
		});
	},
	...
	move_to_trash: function (frm) {
		dontmanage.call({
			method: "dontmanage.email.inbox.mark_as_trash",
			args: {
				communication: frm.doc.name,
			},
			freeze: true,
			callback: function (r) {
				dontmanage.msgprint(__("Email has been moved to trash"));
			},
		});
	},
});

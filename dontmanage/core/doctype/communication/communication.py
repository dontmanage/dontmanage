...
from dontmanage.automation.doctype.assignment_rule.assignment_rule import (
	apply as apply_assignment_rule,
)
from dontmanage.contacts.doctype.contact.contact import get_contact_name
from dontmanage.core.doctype.comment.comment import update_comment_in_doc
from dontmanage.core.doctype.communication.email import validate_email
from dontmanage.core.doctype.communication.mixins import CommunicationEmailMixin
...
class Communication(Document, CommunicationEmailMixin):
	...
	if TYPE_CHECKING:
		from dontmanage.core.doctype.communication_link.communication_link import CommunicationLink
	...
	def onload(self):
		...
		if (
			...
			email_flag_queue = dontmanage.db.get_value(
	...
	def validate_reference(self):
		if self.reference_doctype and self.reference_name:
			if not self.reference_owner:
				self.reference_owner = dontmanage.db.get_value(
					self.reference_doctype, self.reference_name, "owner"
				)

			# prevent communication against a child table
			if dontmanage.get_meta(self.reference_doctype).istable:
				dontmanage.throw(
			...
			if self.reference_doctype == "Communication":
				...
				if circular_linking:
					dontmanage.throw(
						...
						dontmanage.CircularLinkingError,
					)
	...
	def after_insert(self):
		...
		if self.reference_doctype == "Communication" and self.sent_or_received == "Sent":
			dontmanage.db.set_value("Communication", self.reference_name, "status", "Replied")

		if self.communication_type == "Communication":
			self.notify_change("add")

		elif self.communication_type in ("Chat", "Notification"):
			if self.reference_name == dontmanage.session.user:
				message = self.as_dict()
				message["broadcast"] = True
				dontmanage.publish_realtime("new_message", message, after_commit=True)
			else:
	...
	def set_signature_in_email_content(self):
		...
		signature = user_email_signature or dontmanage.db.get_value(
			"Email Account",
	...
	def mark_email_as_spam(self):
		if (
			...
			and self.communication_medium == "Email"
			and self.sent_or_received == "Received"
			and dontmanage.db.exists("Email Rule", {"email_id": self.sender, "is_spam": 1})
		):
	...
	def find_one_by_filters(cls, *, order_by=None, **kwargs):
		name = dontmanage.db.get_value(cls.DOCTYPE, kwargs, order_by=order_by)
	...
	def set_sender_full_name(self):
		if not self.sender_full_name and self.sender:
			if self.sender == "Administrator":
				self.sender_full_name = dontmanage.db.get_value("User", "Administrator", "full_name")
				self.sender = dontmanage.db.get_value("User", "Administrator", "email")
				...
				if not self.sender_full_name:
					self.sender_full_name = dontmanage.db.get_value("User", self.sender, "full_name")

				if not self.sender_full_name:
					first_name, last_name = dontmanage.db.get_value(
	...
	def set_delivery_status(self, commit=False):
		...
		status_counts = Counter(
			dontmanage.get_all("Email Queue", pluck="status", filters={"communication": self.name})
		)
	...
	def parse_email_for_timeline_links(self):
		if not dontmanage.db.get_value("Email Account", filters={"enable_automatic_linking": 1}):
			return

		for doctype, docname in parse_email([self.recipients, self.cc, self.bcc]):
			if not dontmanage.db.get_value(doctype, docname, ignore=True):
				continue
	...
	def set_timeline_links(self):
		contacts = []
		create_contact_enabled = self.email_account and dontmanage.db.get_value(
...
def on_doctype_update():
	...
	dontmanage.db.add_index("Communication", ["reference_doctype", "reference_name"])
	dontmanage.db.add_index("Communication", ["status", "communication_type"])
	dontmanage.db.add_index("Communication", ["message_id(140)"])
...
def add_contact_links_to_communication(communication, contact_name):
	contact_links = dontmanage.get_all(
		"Dynamic Link",
...
def get_email_without_link(email):
	...
	"""
	if not dontmanage.get_all("Email Account", filters={"enable_automatic_linking": 1}):
		return email
...
def update_first_response_time(parent, communication):
	if parent.meta.has_field("first_response_time") and not parent.get("first_response_time"):
		if (
			...
			or dontmanage.get_cached_value("User", dontmanage.session.user, "user_type") == "System User"
		):
...
def set_avg_response_time(parent, communication):
	if parent.meta.has_field("avg_response_time") and communication.sent_or_received == "Sent":
		...
		communications = dontmanage.get_list(
			"Communication",

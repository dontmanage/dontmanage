...
class Communication(Document, CommunicationEmailMixin):
	...
	def set_status(self):
		...
		email_rule = dontmanage.db.get_value("Email Rule", {"email_id": self.sender, "is_spam": 1})
		if (
			self.communication_type == "Communication"
			and self.communication_medium == "Email"
...
def has_permission(doc, ptype, user):
	if ptype == "read":
		...
		if doc.reference_doctype and doc.reference_name:
			if dontmanage.has_permission(doc.reference_doctype, ptype="read", doc=doc.reference_name):
				return True
	...
	for email_string in email_strings:
		if email_string:
			for email in email_string.split(","):
				...
				if doctype and docname and dontmanage.db.get_value(doctype, docname, ignore=True):

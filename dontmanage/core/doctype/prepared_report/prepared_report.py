...
from dontmanage.database.utils import dangerously_reconnect_on_connection_abort
...
from dontmanage.utils.background_jobs import enqueue, get_redis_conn
...
class PreparedReport(Document):
	...
	def clear_old_logs(days=30):
		prepared_reports_to_delete = dontmanage.get_all(
			"Prepared Report",
			filters={"creation": ["<", dontmanage.utils.add_days(dontmanage.utils.now(), -days)]},
		)
	...
	def after_insert(self):
		timeout = dontmanage.get_value("Report", self.report_name, "timeout")
		enqueue(
			generate_report,
			queue="long",
...
def generate_report(prepared_report):
	...
	instance: PreparedReport = dontmanage.get_doc("Prepared Report", prepared_report)
	...
	try:
		...
		_save_error(instance, error=dontmanage.get_traceback(with_context=True))
...
def stop_prepared_report(report_name: str):
	...
	prepared_report = dontmanage.get_doc("Prepared Report", report_name)
	prepared_report.check_permission("write")

	job_id = prepared_report.job_id
	if not job_id.startswith(dontmanage.local.site):
		dontmanage.throw(f"Invalid job_id: must start with {dontmanage.local.site}")
	...
	try:
		...
		dontmanage.msgprint(_("Job stopped successfully"), alert=True, indicator="green")
...
def create_json_gz_file(data, dt, dn, report_name):
	...
	json_filename = "{}_{}.json.gz".format(
		dontmanage.scrub(report_name), dontmanage.utils.data.format_datetime(dontmanage.utils.now(), "Y-m-d-H-M")
	)
	encoded_content = dontmanage.safe_encode(dontmanage.as_json(data, indent=None, separators=(",", ":")))
...
def convert_json_to_csv(prepared_report_name):
	...
	doc = dontmanage.get_doc("Prepared Report", prepared_report_name)
	json_content, file_name = doc.get_prepared_data(with_file_name=True)

	if not json_content:
		dontmanage.log_error(f"No JSON content found for {prepared_report_name}", "CSV Conversion")
		return
	...
	if not columns or not result:
		dontmanage.log_error("Columns or result is empty", "CSV Conversion")
		return

...
dontmanage.ui.form.on("Data Export", {
	refresh: (frm) => {
		frm.disable_save();
		...
		frm.set_query("reference_doctype", () => {
			return {
				filters: {
					issingle: 0,
					istable: 0,
					name: ["in", dontmanage.boot.user.can_export],
				},
			};
		});
	},
	reference_doctype: (frm) => {
		...
		if (doctype) {
			dontmanage.model.with_doctype(doctype, () => set_field_options(frm));
		} else {
...
const can_export = (frm) => {
	...
	if (!doctype) {
		dontmanage.msgprint(__("Please select the Document Type."));
	} else if (!parent_multicheck_options.length) {
		dontmanage.msgprint(__("At least one field of Parent Document Type is mandatory"));
	} else {
		is_valid_form = true;
	}
...
const export_data = (frm) => {
	let get_template_url = "/api/method/dontmanage.core.doctype.data_export.exporter.export_data";
	var export_params = () => {
		let columns = {};
...
const set_field_options = (frm) => {
	...
	frm.filter_list = new dontmanage.ui.FilterGroup({
...
const make_multiselect_buttons = (parent_wrapper) => {
	...
	["Select All", "Unselect All"].map((d) => {
		dontmanage.ui.form.make_control({
			parent: $(button_container),
			df: {
				label: __(d),
				fieldname: dontmanage.scrub(d),
				fieldtype: "Button",
				click: () => {
...
const get_doctypes = (parentdt) => {
	return [parentdt].concat(dontmanage.meta.get_table_fields(parentdt).map((df) => df.options));
};

const add_doctype_field_multicheck_control = (doctype, parent_wrapper) => {
	const fields = get_fields(doctype);

	dontmanage.model.std_fields
	...
	const multicheck_control = dontmanage.ui.form.make_control({
...
const filter_fields = (df) => dontmanage.model.is_value_type(df) && !df.hidden;
const get_fields = (dt) => dontmanage.meta.get_docfields(dt).filter(filter_fields);

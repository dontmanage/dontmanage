...
class DataExporter:
	...
	def build_response(self):
		...
		if self.file_type == "Excel":
			...
			dontmanage.response["result"] = cstr(self.writer.getvalue())
			dontmanage.response["type"] = "csv"
			dontmanage.response["doctype"] = self.doctype
	...
	def build_field_columns(self, dt, parentfield=None):
		...
		_column_start_end = dontmanage._dict(start=0)
		...
		if dt == self.doctype:
			...
			_column_start_end = dontmanage._dict(start=len(self.columns))
	...
	def getinforow(docfield):
		...
		if docfield.fieldtype == "Select":
			...
			return cstr(dontmanage.defaults.get_defaults().date_format)
	...
	def add_data(self):
		...
		dontmanage.permissions.can_export(self.parent_doctype, raise_exception=True)

		# sort nested set doctypes by `lft asc`
		order_by = None
		table_columns = dontmanage.db.get_table_columns(self.parent_doctype)
		if "lft" in table_columns and "rgt" in table_columns:
			order_by = f"`tab{self.parent_doctype}`.`lft` asc"
		# get permitted data only
		self.data = dontmanage.get_list(

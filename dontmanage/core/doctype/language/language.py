...
from dontmanage.defaults import clear_default, set_default
...
class Language(Document):
	...
	def on_update(self):
		...
		dontmanage.client_cache.delete_value("languages")
	...
	def update_user_defaults(self):
		...
		should be updated.
		"""
		users = dontmanage.get_all("User", filters={"language": self.name}, pluck="name")
...
def sync_languages():
	"""Create Language records from dontmanage/geo/languages.csv"""
	from csv import DictReader

	with open(dontmanage.get_app_path("dontmanage", "geo", "languages.csv")) as f:
		reader = DictReader(f)
		for row in reader:
			if not dontmanage.db.exists("Language", row["language_code"]):
				doc = dontmanage.new_doc("Language")
				doc.update(row)
				doc.insert()

...
dontmanage.ui.form.on("Document Naming Rule", {
	...
	document_type: (frm) => {
		...
		if (frm.doc.document_type) {
			dontmanage.model.with_doctype(frm.doc.document_type, () => {
				let fieldnames = dontmanage
					.get_meta(frm.doc.document_type)
					.fields.filter((d) => {
						return dontmanage.model.no_value_type.indexOf(d.fieldtype) === -1;
					})
					.map((d) => {
	...
	add_update_counter_button: (frm) => {
		frm.add_custom_button(__("Update Counter"), function () {
			...
			let primary_action = (fields) => {
				dontmanage.call({
					method: "dontmanage.core.doctype.document_naming_rule.document_naming_rule.update_current",
					args: {
						name: frm.doc.name,

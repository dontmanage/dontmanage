...
class TestDocumentNamingRule(IntegrationTestCase):
	def test_naming_rule_by_series(self):
		...
		todo = dontmanage.get_doc(
			doctype="ToDo", description="Is this my name " + dontmanage.generate_hash()
		).insert()
	...
	def test_naming_rule_by_condition(self):
		...
		todo = dontmanage.get_doc(
			doctype="ToDo", priority="High", description="Is this my name " + dontmanage.generate_hash()
		).insert()

		todo_1 = dontmanage.get_doc(
			doctype="ToDo", priority="Medium", description="Is this my name " + dontmanage.generate_hash()
		).insert()

		todo_2 = dontmanage.get_doc(
			doctype="ToDo", priority="Low", description="Is this my name " + dontmanage.generate_hash()
		).insert()

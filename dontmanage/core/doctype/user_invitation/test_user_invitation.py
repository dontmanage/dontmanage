...
from dontmanage.core.api.user_invitation import (
	_accept_invitation,
	cancel_invitation,
	get_pending_invitations,
	invite_by_email,
)
from dontmanage.core.doctype.user_invitation.user_invitation import mark_expired_invitations
...
class IntegrationTestUserInvitation(IntegrationTestCase):
	...
	def tearDownClass(cls):
		...
		for user_email in emails:
			if dontmanage.db.exists("User", user_email):
				dontmanage.delete_doc("User", user_email)
	...
	def delete_all_user_roles(cls):
		...
		dontmanage.db.sql(cls.normalize_sql(query))
	...
	def delete_invitation(cls, name: str):
		...
		dontmanage.db.sql(cls.normalize_sql(query), name)
	...
	def test_insert_invitation(self):
		...
		self.assertEqual(invitation.invited_by, dontmanage.session.user)
	...
	def test_mark_expired_invitations(self):
		...
		invitation.db_set("creation", dontmanage.utils.add_days(dontmanage.utils.now(), -4))
	...
	def test_invite_by_email_api_disabled_user(self):
		...
		dontmanage.delete_doc("User", user.email)
	...
	def test_accept_invitation_api_pass_redirect(self):
		...
		self.assertEqual(len(dontmanage.get_all("User", filters={"email": invitation.email}, pluck="name")), 0)
		self.assertEqual(len(self.get_email_names(False)), 1)
		key = invitation._after_insert()
		self.assertEqual(len(self.get_email_names(False)), 2)
		_accept_invitation(key, True)
		res = dontmanage.local.response
		self.assertEqual(res.type, "redirect")
		pattern = f"^{re.escape(dontmanage.utils.get_url(''))}/update-password\\?key=.+&redirect_to=/abc$"
	...
	def test_accept_invitation_api_direct_redirect(self):
		...
		self.assertEqual(len(dontmanage.get_all("User", filters={"email": invitation.email}, pluck="name")), 0)
		original_disable_user_pass_login = dontmanage.get_system_settings("disable_user_pass_login")
		dontmanage.db.set_single_value("System Settings", "disable_user_pass_login", 1)
		self.assertEqual(len(self.get_email_names(False)), 1)
		key = invitation._after_insert()
		self.assertEqual(len(self.get_email_names(False)), 2)
		_accept_invitation(key, True)
		...
		res = dontmanage.local.response
		self.assertEqual(res.type, "redirect")
		pattern = f"^{re.escape(dontmanage.utils.get_url(''))}/abc$"
		self.assertRegex(res.location, pattern)
		user = dontmanage.get_doc("User", invitation.email)
		IntegrationTestUserInvitation.delete_invitation(invitation.name)
		dontmanage.delete_doc("User", user.name)
	...
	def test_get_pending_invitations_api(self):
		...
		invitation.insert()
		invitation.reload()
		pending_invitations = get_pending_invitations("dontmanage")
	...
	def test_cancel_invitation_api(self):
		...
		self.assertEqual(invitation.status, "Pending")
		self.assertEqual(len(self.get_email_names()), 1)
		res = cancel_invitation(invitation.name, "dontmanage")
		dontmanage.db.commit()
	...
	def get_email_names(self, sent_only=True):
		...
		return dontmanage.db.get_all("Email Queue", filters=filters, fields=["name"])

	def get_email_messages(self, sent_only=True):
		filters = {"status": "Sent"} if sent_only else None
		return dontmanage.db.get_all("Email Queue", filters=filters, fields=["message"])

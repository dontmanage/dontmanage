...
from dontmanage.permissions import get_roles
...
class UserInvitation(Document):
	...
	if TYPE_CHECKING:
		from dontmanage.core.doctype.user_role.user_role import UserRole
	...
	def expire(self):
		...
		self.save()
		email_title = self._get_email_title()
		invited_by_user = dontmanage.get_doc("User", self.invited_by)
		dontmanage.sendmail(
	...
	def _validate_invite(self):
		...
		if dontmanage.db.get_value(
			...
			dontmanage.throw(title=_("Error"), msg=_("Invitation already accepted"))
		if dontmanage.db.get_value(
			"User Invitation", filters={"email": self.email, "status": "Pending", "app_name": self.app_name}
		):
			dontmanage.throw(title=_("Error"), msg=_("Invitation already exists"))
		user_enabled = dontmanage.db.get_value("User", self.email, "enabled")
		if user_enabled is not None and user_enabled == 0:
			dontmanage.throw(title=_("Error"), msg=_("User is disabled"))

	def _after_insert(self):
		key = dontmanage.generate_hash()
		self.db_set("key", dontmanage.utils.sha256_hash(key))
		invite_link = dontmanage.utils.get_url(
			f"/api/method/dontmanage.core.api.user_invitation.accept_invitation?key={key}"
		)
		email_title = self._get_email_title()
		dontmanage.sendmail(
			recipients=self.email,
	...
	def _accept(self):
		...
		if self.status == "Expired":
			dontmanage.throw(title=_("Error"), msg=_("Invitation is expired"))
		if self.status == "Cancelled":
			dontmanage.throw(title=_("Error"), msg=_("Invitation is cancelled"))
	...
	def _upsert_user(self, ignore_permissions: bool = False):
		...
		user_inserted = False
		if dontmanage.db.exists("User", self.user):
	...
	def _run_after_accept_hooks(self, user: Document, user_inserted: bool):
		user_invitation_hook = dontmanage.get_hooks("user_invitation", app_name=self.app_name)
		if not isinstance(user_invitation_hook, dict):
			return
		for dot_path in user_invitation_hook.get("after_accept") or []:
			dontmanage.call(dot_path, invitation=self, user=user, user_inserted=user_inserted)

	def _get_email_title(self):
		return dontmanage.get_hooks("app_title", app_name=self.app_name)[0]
	...
	def _get_allowed_roles(self):
		...
		for role in only_for & set(dontmanage.get_roles()):
			res.update(allowed_roles_mp[role])
		return list(res)

	def _validate_roles(self):
		if self.app_name == "dontmanage":
			return
	...
	def _validate_email(self):
		dontmanage.utils.validate_email_address(self.email, throw=True)
	...
	def validate_app_name(app_name: str):
		if app_name not in dontmanage.get_installed_apps():
			dontmanage.throw(title=_("Invalid app"), msg=_("Application is not installed"))

	@staticmethod
	def validate_role(app_name: str) -> None:
		UserInvitation.validate_app_name(app_name)
		user_invitation_hook = dontmanage.get_hooks("user_invitation", app_name=app_name)
		only_for: list[str] = []
		if isinstance(user_invitation_hook, dict):
			only_for = list((user_invitation_hook.get("allowed_roles") or dict()).keys())
		dontmanage.only_for(only_for)


def mark_expired_invitations() -> None:
	days = 3
	invitations_to_expire = dontmanage.db.get_all(
		"User Invitation",
		filters={"status": "Pending", "creation": ["<", dontmanage.utils.add_days(dontmanage.utils.now(), -days)]},
	)
	for invitation in invitations_to_expire:
		invitation = dontmanage.get_doc("User Invitation", invitation.name)
		invitation.expire()
...
def get_allowed_apps(user: Document | None) -> list[str]:
	...
	for app in dontmanage.get_installed_apps():
		user_invitation_hooks = dontmanage.get_hooks("user_invitation", app_name=app)
...
def get_user(user: Document | None) -> Document:
	return user or dontmanage.session.user

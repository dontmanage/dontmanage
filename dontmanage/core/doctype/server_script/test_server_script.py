...
from dontmanage.core.doctype.server_script.server_script import ServerScript
...
class TestServerScript(IntegrationTestCase):
	...
	def tearDownClass(cls):
		dontmanage.db.commit()
		dontmanage.db.truncate("Server Script")
		dontmanage.client_cache.delete_value("server_script_map")

	def setUp(self):
		dontmanage.client_cache.delete_value("server_script_map")

	def test_doctype_event(self):
		todo = dontmanage.get_doc(doctype="ToDo", description="hello").insert()
		self.assertEqual(todo.status, "Open")

		todo = dontmanage.get_doc(doctype="ToDo", description="test todo").insert()
		self.assertEqual(todo.status, "Closed")

		self.assertRaises(
			dontmanage.ValidationError, dontmanage.get_doc(doctype="ToDo", description="validate me").insert
		)

		role = dontmanage.get_doc(doctype="Role", role_name="_Test Role 9").insert(ignore_if_duplicate=True)
	...
	def test_permission_query(self):
		sql = dontmanage.db.get_list("ToDo", run=False).get_sql()
	...
	def test_commit_in_doctype_event(self):
		...
		self.assertRaises(AttributeError, dontmanage.get_doc(doctype="ToDo", description="test me").insert)
...
"""
	...
	def test_server_script_scheduled(self):
		...
		updated_cron_job_name = dontmanage.db.get_value("Scheduled Job Type", {"server_script": cron_script.name})
		updated_cron_job = dontmanage.get_doc("Scheduled Job Type", updated_cron_job_name)
		self.assertEqual(updated_cron_job.next_execution.day, 2)

	def test_server_script_state_changes(self):
		script: ServerScript = dontmanage.get_doc(
			doctype="Server Script",
		...
		job: ScheduledJobType = dontmanage.get_doc("Scheduled Job Type", {"server_script": script.name})

...
class ServerScript(Document):
	...
	def on_trash(self):
		...
		if self.script_type == "Scheduler Event":
			for job in self.scheduled_jobs:
				scheduled_job_type: ScheduledJobType = dontmanage.get_doc("Scheduled Job Type", job.name)
	...
	def sync_scheduled_job_type(self):
		...
		def get_scheduled_job() -> "ScheduledJobType":
			if scheduled_script := dontmanage.db.get_value("Scheduled Job Type", {"server_script": self.name}):
				return dontmanage.get_doc("Scheduled Job Type", scheduled_script)
			else:
		...
		get_scheduled_job().update(
			{
				"method": dontmanage.scrub(f"{self.name}-{self.event_frequency}"),
		...
		dontmanage.msgprint(_("Scheduled execution for script {0} has updated").format(self.name), alert=True)
	...
	def execute_method(self) -> dict:
		...
		Return:
		        dict: Evaluate self.script with dontmanage.utils.safe_exec.safe_exec and return the flags set in its safe globals.
		"""
...
def get_autocompletion_items():
	...
	e.g., ["dontmanage.utils.cint", "dontmanage.get_all", ...]
	"""

	return dontmanage.cache.get_value(

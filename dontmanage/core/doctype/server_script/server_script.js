...
dontmanage.ui.form.on("Server Script", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Compare Versions"), () => {
				new dontmanage.ui.DiffView("Server Script", "script", frm.doc.name);
			});
		}

		setTimeout(() => {
			dontmanage
				.call("dontmanage.core.doctype.server_script.server_script.get_autocompletion_items")
				.then((r) => r.message)
				.then((items) => {
	...
	check_safe_exec(frm) {
		dontmanage.xcall("dontmanage.core.doctype.server_script.server_script.enabled").then((enabled) => {
			if (enabled === false) {
				let docs_link =
					"https://dontmanageframework.com/docs/user/en/desk/scripting/server-script";
...
<p>Payment processing events have a special state. See the <a href="https://github.com/dontmanage/payments/blob/develop/payments/controllers/payment_controller.py">PaymentController in DontManage Payments</a> for details.</p>
<pre><code class="language-python">
...
if dontmanage.form_dict.message == "ping":
	dontmanage.response['message'] = "pong"
else:
	dontmanage.response['message'] = "ok"
</code></pre>
...
<pre><code class="language-python">
tenant_id = dontmanage.db.get_value(...) # -> 2

dontmanage.listview_settings["DocType"] = {
	primary_action: function () {
		this.new_doctype_dialog();
	},

	new_doctype_dialog() {
		let non_developer = dontmanage.session.user !== "Administrator" || !dontmanage.boot.developer_mode;
		let fields = [
			{
		...
		let new_d = new dontmanage.ui.Dialog({
			...
			primary_action(values) {
				...
				dontmanage.db
					...
					.then((doc) => {
						dontmanage.set_route("Form", "DocType", doc.name);
					});
			},

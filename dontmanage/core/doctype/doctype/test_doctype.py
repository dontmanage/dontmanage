...
import dontmanage
from dontmanage.cache_manager import clear_doctype_cache
...
class TestDocType(IntegrationTestCase):
	...
	def test_validate_name(self):
		self.assertRaises(dontmanage.NameError, new_doctype("_Some DocType").insert)
		self.assertRaises(dontmanage.NameError, new_doctype("8Some DocType").insert)
		self.assertRaises(dontmanage.NameError, new_doctype("Some (DocType)").insert)
		self.assertRaises(
		...
		for name in ("Some DocType", "Some_DocType", "Some-DocType"):
			if dontmanage.db.exists("DocType", name):
				dontmanage.delete_doc("DocType", name)
	...
	def test_making_sequence_on_change(self):
		dontmanage.delete_doc_if_exists("DocType", self._testMethodName)
		...
		self.assertEqual(
			...
			"varchar" if dontmanage.db.db_type == "mariadb" else "character varying",
		)

	def test_doctype_unique_constraint_dropped(self):
		if dontmanage.db.exists("DocType", "With_Unique"):
			dontmanage.delete_doc("DocType", "With_Unique")

		dt = new_doctype("With_Unique", unique=1)
		dt.insert()

		doc1 = dontmanage.new_doc("With_Unique")
		doc2 = dontmanage.new_doc("With_Unique")
		...
		doc1.insert()
		self.assertRaises(dontmanage.UniqueValidationError, doc2.insert)
		dontmanage.db.rollback()
	...
	def test_validate_search_fields(self):
		...
		self.assertRaises(dontmanage.ValidationError, doc.save)
	...
	def test_sync_field_order(self):
		...
		# create test doctype
		test_doctype = dontmanage.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"fields": [
		...
		dontmanage.delete_doc_if_exists("DocType", "Test Field Order DocType")
		if os.path.isfile(path):
			os.remove(path)

		try:
			dontmanage.flags.allow_doctype_export = 1
			test_doctype.save()
			...
			with open(path, "w+") as txtfile:
				txtfile.write(dontmanage.as_json(test_doctype_json))
			...
			dontmanage.reload_doctype(test_doctype.name, force=True)
			test_doctype.reload()
			...
			dontmanage.flags.allow_doctype_export = 0
	...
	def test_cancel_link_doctype(self):
		...
		from dontmanage.desk.form.linked_with import cancel_all_linked_docs, get_submitted_linked_docs
		...
		# create doctype data
		data_link_doc = dontmanage.new_doc("Test Linked Doctype")
		data_link_doc.some_fieldname = "Data1"
		data_link_doc.insert()
		data_link_doc.save()
		data_link_doc.submit()

		data_doc = dontmanage.new_doc("Test Doctype")
	...
	def test_ignore_cancelation_of_linked_doctype_during_cancel(self):
		...
		# create doctype data
		data_link_doc_1 = dontmanage.new_doc("Test Linked Doctype 1")
		data_link_doc_1.some_fieldname = "Data1"
		data_link_doc_1.insert()
		data_link_doc_1.save()
		data_link_doc_1.submit()

		data_doc_2 = dontmanage.new_doc("Test Doctype 1")
		...
		data_doc = dontmanage.new_doc("Test Doctype 2")
		data_doc.some_fieldname = "Data1"
		...
		self.assertRaises(dontmanage.LinkExistsError, data_link_doc_1.cancel)
	...
	def test_create_virtual_doctype(self):
		...
		virtual_doc.reload()
		doc = dontmanage.get_doc("DocType", "Test Virtual Doctype")

		self.assertDictEqual(doc.as_dict(), virtual_doc.as_dict())
		self.assertEqual(doc.is_virtual, 1)
		self.assertFalse(dontmanage.db.table_exists("Test Virtual Doctype"))

	def test_create_virtual_doctype_as_child_table(self):
		"""Test virtual DocType as Child Table below a normal DocType."""
		dontmanage.delete_doc_if_exists("DocType", "Test Parent Virtual DocType", force=1)
		dontmanage.delete_doc_if_exists("DocType", "Test Virtual DocType as Child Table", force=1)
		...
		doc = dontmanage.get_doc("DocType", "Test Virtual DocType as Child Table")

		self.assertEqual(doc.is_virtual, 1)
		self.assertEqual(doc.istable, 1)
		self.assertFalse(dontmanage.db.table_exists("Test Virtual DocType as Child Table"))
	...
	def test_autoincremented_doctype_transition(self):
		dontmanage.delete_doc_if_exists("DocType", "testy_autoinc_dt")
		dt = new_doctype("testy_autoinc_dt", autoname="autoincrement").insert(ignore_permissions=True)
		dt.autoname = "hash"

		dt.save(ignore_permissions=True)

		dt_data = dontmanage.get_doc({"doctype": dt.name, "some_fieldname": "test data"}).insert(
	...
	def test_json_field(self):
		...
		json_doc.insert()
		json_doc.save()
		doc = dontmanage.get_doc("DocType", "Test Json Doctype")
		...
		test_json = dontmanage.get_doc("Test Json Doctype", doc.name)
	...
	def test_delete_doctype_with_customization(self):
		...
		# Create 1 record
		original_doc = dontmanage.get_doc(doctype=doctype, custom_field_name="wat").insert()
		...
		restored_doc = dontmanage.get_last_doc(doctype)
	...
	def test_not_in_list_view_for_not_allowed_mandatory_field(self):
		...
		dontmanage.delete_doc("DocType", doctype.name)
	...
	def test_no_recursive_fetch(self):
		...
		self.assertRaises(dontmanage.ValidationError, recursive_dt.insert)
	...
	def test_meta_serialization(self):
		...
		doc = dontmanage.new_doc(doctype.name, some_fieldname="something").insert()
		doc.save()
		doc.submit()
		dontmanage.get_meta(doctype.name).as_dict()
	...
	def test_row_compression(self):
		...
		def get_format(dt):
			return (
				...
				.select("row_format")
				.where(
					(information_schema.tables.table_schema == dontmanage.conf.db_name)
	...
	def test_decimal_field_configuration(self):
		...
		decimal_field_type = dontmanage.db.get_column_type(doctype.name, "decimal_field")
	...
	def test_decimal_field_precision_exceeds_length(self):
		...
		self.assertRaises(dontmanage.ValidationError, doctype.insert)

	def test_delete_doc_clears_cache(self):
		dt = new_doctype(
			fields=[{"fieldname": "test_fdname", "fieldtype": "Data", "label": "Test Field"}],
		).insert()
		dontmanage.get_meta(dt.name)
		dontmanage.delete_doc("DocType", dt.name, force=1, delete_permanently=False)
		dontmanage.db.commit()
		with self.assertRaises(dontmanage.DoesNotExistError):
			dontmanage.get_meta(dt.name)

...
from dontmanage.model.base_document import RESERVED_KEYWORDS, get_controller
...
from dontmanage.utils import cint, flt, get_datetime, is_a_property, random_string
...
class InvalidFieldNameError(dontmanage.ValidationError):
	pass


class UniqueFieldnameError(dontmanage.ValidationError):
	pass


class IllegalMandatoryError(dontmanage.ValidationError):
	pass


class DoctypeLinkError(dontmanage.ValidationError):
	pass


class WrongOptionsDoctypeLinkError(dontmanage.ValidationError):
	pass


class HiddenAndMandatoryWithoutDefaultError(dontmanage.ValidationError):
	pass


class NonUniqueError(dontmanage.ValidationError):
	pass


class CannotIndexedError(dontmanage.ValidationError):
	pass


class CannotCreateStandardDoctypeError(dontmanage.ValidationError):
	pass
...
class DocType(Document):
	...
	def validate(self):
		...
		if not self.is_new():
			self.before_update = dontmanage.get_doc("DocType", self.name)
		...
		if self.default_print_format and not self.custom:
			dontmanage.throw(_("Standard DocType cannot have default print format, use Customize Form"))
	...
	def check_developer_mode(self):
		...
		if dontmanage.flags.in_patch or dontmanage.in_test:
			return

		if not dontmanage.conf.get("developer_mode") and not self.custom:
			dontmanage.throw(
		...
		if dontmanage.conf.developer_mode and not self.owner:
	...
	def setup_fields_to_fetch(self):
		...
		try:
			old_meta = dontmanage.get_meta(dontmanage.get_doc("DocType", self.name), cached=False)
			old_fields_to_fetch = [df.fieldname for df in old_meta.get_fields_to_fetch()]
		except dontmanage.DoesNotExistError:
			old_fields_to_fetch = []

		new_meta = dontmanage.get_meta(self, cached=False)
	...
	def ensure_minimum_max_attachment_limit(self):
		...
		from dontmanage.model import attachment_fieldtypes
		...
		if total_attach_fields > self.max_attachments:
			...
			field_label = dontmanage.bold(self.meta.get_field("max_attachments").label)
			dontmanage.msgprint(
	...
	def patch_old_naming_expressions(self):
		...
		if self.autoname and self.autoname.startswith("format:"):
			...
			dontmanage.toast(_("Warning: Usage of 'format:' is discouraged."), indicator="yellow")
	...
	def change_modified_of_parent(self):
		...
		if dontmanage.flags.in_import:
			return
		parent_list = dontmanage.get_all(
	...
	def on_update(self):
		...
		try:
			dontmanage.db.updatedb(self.name, Meta(self))
		...
		allow_doctype_export = (
			not self.custom
			and not dontmanage.flags.in_import
			and (dontmanage.conf.developer_mode or dontmanage.flags.allow_doctype_export)
		)
		...
		if not dontmanage.flags.in_install and hasattr(self, "before_update"):
	...
	def setup_autoincrement_and_sequence(self):
		...
		name_type = f"varchar({dontmanage.db.VARCHAR_LEN})"
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			now = (not dontmanage.request) or dontmanage.in_test or dontmanage.flags.in_install
			dontmanage.enqueue("dontmanage.utils.global_search.rebuild_for_doctype", now=now, doctype=self.name)
	...
	def set_base_class_for_controller(self):
		...
		scrubbed_name = dontmanage.scrub(self.name)
		scrubbed_module = dontmanage.scrub(self.module)
		controller_path = dontmanage.get_module_path(
			scrubbed_module, "doctype", scrubbed_name, f"{scrubbed_name}.py"
		)

		document_cls_tag = f"class {despaced_name}(Document)"
		document_import_tag = "from dontmanage.model.document import Document"
		website_generator_cls_tag = f"class {despaced_name}(WebsiteGenerator)"
		website_generator_import_tag = "from dontmanage.website.website_generator import WebsiteGenerator"
	...
	def before_rename(self, old, new, merge=False):
		...
		if not self.custom and dontmanage.session.user != "Administrator":
			dontmanage.throw(_("DocType can only be renamed by Administrator"))

		self.check_developer_mode()
		self.validate_name(new)

		if merge:
			dontmanage.throw(_("DocType can not be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			dontmanage.db.sql("""update tabSingles set doctype=%s where doctype=%s""", (new, old))
			dontmanage.db.sql(
			...
			dontmanage.db.rename_table(old, new)
	...
	def rename_files_and_folders(self, old, new):
		...
		for fname in os.listdir(new_path):
			if dontmanage.scrub(old) in fname:
				old_file_name = os.path.join(new_path, fname)
				new_file_name = os.path.join(new_path, fname.replace(dontmanage.scrub(old), dontmanage.scrub(new)))
				shutil.move(old_file_name, new_file_name)

		self.rename_inside_controller(new, old, new_path)
		dontmanage.msgprint(_("Renamed files and replaced code in controllers, please check!"))

	def rename_inside_controller(self, new, old, new_path):
		for fname in ("{}.js", "{}.py", "{}_list.js", "{}_calendar.js", "test_{}.py", "test_{}.js"):
			fname = os.path.join(new_path, fname.format(dontmanage.scrub(new)))
		...
		doctype_json_path = os.path.join(new_path, f"{dontmanage.scrub(new)}.json")
		current_data = dontmanage.get_file_json(doctype_json_path)
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		if not (
			dontmanage.db.table_exists(self.name)
			and dontmanage.get_all(self.name, fields=["name"], limit=1, as_list=True)
		):
			return

		existing_property_setter = dontmanage.db.get_value(
	...
	def make_controller_template(self):
		...
		if self.has_web_view:
			templates_path = dontmanage.get_module_path(
				dontmanage.scrub(self.module), "doctype", dontmanage.scrub(self.name), "templates"
			)
	...
	def validate_nestedset(self):
		...
		if not self.nsm_parent_field:
			field_label = dontmanage.bold(_("Parent Field (Tree)"))
			dontmanage.throw(_("{0} is a mandatory field").format(field_label), dontmanage.MandatoryError)

		# check if field is valid
		fieldnames = [df.fieldname for df in self.fields]
		if self.nsm_parent_field and self.nsm_parent_field not in fieldnames:
			dontmanage.throw(_("Parent Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def add_nestedset_fields(self):
		...
		parent_field_name = dontmanage.scrub(parent_field_label)
		self.append(
			"fields",
			{
	...
	def get_max_idx(self):
		...
		max_idx = dontmanage.db.sql("""select max(idx) from `tabDocField` where parent = %s""", self.name)
	...
	def validate_name(self, name=None):
		...
		max_length = dontmanage.db.MAX_COLUMN_LENGTH - 3
		...
		if WHITESPACE_PADDING_PATTERN.search(name):
			dontmanage.throw(_("DocType's name should not start or end with whitespace"), dontmanage.NameError)
	...
	def check_pending_migration(self) -> bool:
		...
		file = Path(get_file_path(dontmanage.scrub(self.module), self.doctype, self.name))
...
def validate_series(dt, autoname=None, name=None):
	...
	if not autoname and dt.get("fields", {"fieldname": "naming_series"}):
		...
		if not dt.get("fields", {"fieldname": fieldname}):
			dontmanage.throw(
				_("Fieldname called {0} must exist to enable autonaming").format(dontmanage.bold(fieldname)),
	...
	if autoname and autoname.startswith("field:"):
		...
		if not field or field not in [df.fieldname for df in dt.fields]:
			dontmanage.throw(_("Invalid fieldname '{0}' in autoname").format(field))
		else:
			for df in dt.fields:
	...
	if (
		...
		if used_in:
			dontmanage.throw(_("Series {0} already used in {1}").format(prefix, used_in[0][0]))
...
def validate_empty_name(dt, autoname):
	...
	if not autoname and not (dt.issingle or dt.istable):
		...
		if not controller or (not hasattr(controller, "autoname")):
			dontmanage.toast(_("Warning: Naming is not set"), indicator="yellow")
...
def validate_autoincrement_autoname(dt: DocType | "CustomizeForm") -> bool:
	...
	def get_autoname_before_save(dt: DocType | "CustomizeForm") -> str:
		if dt.doctype == "Customize Form":
			property_value = dontmanage.db.get_value(
			...
			if not property_value:
				return dontmanage.db.get_value("DocType", dt.doc_type, "autoname") or ""
	...
	if not dt.is_new():
		...
		if (is_autoname_autoincrement and autoname_before_save != "autoincrement") or (
			...
			if dontmanage.get_meta(dt.name).issingle:
				return False

			if not dontmanage.get_all(dt.name, limit=1):
...
def change_name_column_type(doctype_name: str, type: str) -> None:
	...
	dontmanage.db.change_column_type(*args)


def validate_links_table_fieldnames(meta):
	"""Validate fieldnames in Links table"""
	if not meta.links or dontmanage.flags.in_patch or dontmanage.flags.in_fixtures or dontmanage.flags.in_migrate:
		return
	...
	for index, link in enumerate(meta.links, 1):
		...
		if not link.parent_doctype:
			...
			dontmanage.throw(message, dontmanage.ValidationError, _("Parent Missing"))

		if not link.table_fieldname:
			message = _("Document Links Row #{0}: Table Fieldname is mandatory for internal links").format(
				index
			)
			dontmanage.throw(message, dontmanage.ValidationError, _("Table Fieldname Missing"))

		if meta.name == link.parent_doctype:
			field_exists = link.table_fieldname in fieldnames
		else:
			field_exists = dontmanage.get_meta(link.parent_doctype).has_field(link.table_fieldname)

		if not field_exists:
			message = _("Document Links Row #{0}: Could not find field {1} in {2} DocType").format(
				index, dontmanage.bold(link.table_fieldname), dontmanage.bold(meta.name)
			)
			dontmanage.throw(message, dontmanage.ValidationError, _("Invalid Table Fieldname"))
...
def _test_connection_query(doctype, field, idx):
	...
	try:
		dontmanage.get_all(doctype, filters=filters, limit=1, distinct=True, ignore_ifnull=True)
	except Exception as e:
		dontmanage.clear_last_message()
		msg = _("Document Links Row #{0}: Invalid doctype or fieldname.").format(idx)
		msg += "<br>" + str(e)
		dontmanage.throw(msg, InvalidFieldNameError)
...
def validate_fields(meta: Meta):
	...
	:param meta: `dontmanage.model.meta.Meta` object to check."""
		...
		if fieldname in RESERVED_KEYWORDS:
			dontmanage.throw(
				_("{0}: fieldname cannot be set to reserved keyword {1}").format(
					dontmanage.bold(docname),
					dontmanage.bold(fieldname),
				),
	...
	def check_link_table_options(docname, d):
		...
		if d.fieldtype in ("Link", *table_fields):
			...
			if d.options != d.parent:
				options = dontmanage.db.get_value("DocType", d.options, "name")
				if not options:
					dontmanage.throw(
	...
	def check_hidden_and_mandatory(docname, d):
		if d.hidden and d.reqd and not d.default and not dontmanage.flags.in_migrate:
			dontmanage.throw(
	...
	def check_width(d):
		if d.fieldtype == "Currency" and cint(d.width) < 100:
			dontmanage.throw(_("Max width for type Currency is 100px in row {0}").format(d.idx))
	...
	def check_precision(d):
		if (
			...
			dontmanage.throw(_("Precision should be between 1 and 6"))
	...
	def check_fold(fields):
		...
		for i, f in enumerate(fields):
			if f.fieldtype == "Fold":
				if fold_exists:
					dontmanage.throw(_("There can be only one Fold in a form"))
				fold_exists = True
				if i < len(fields) - 1:
					nxt = fields[i + 1]
					if nxt.fieldtype != "Section Break":
						dontmanage.throw(_("Fold must come before a Section Break"))
				else:
					dontmanage.throw(_("Fold can not be at the end of the form"))
	...
	def check_search_fields(meta, fields):
		...
		for fieldname in search_fields:
			...
			if (fieldtype_mapper.get(fieldname) in no_value_fields) or (fieldname not in fieldname_list):
				dontmanage.throw(_("Search field {0} is not valid").format(fieldname))
	...
	def check_title_field(meta):
		...
		if meta.title_field not in fieldname_list:
			dontmanage.throw(_("Title field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_image_field(meta):
		...
		if not df:
			dontmanage.throw(_("Image field must be a valid fieldname"), InvalidFieldNameError)
		if df[0].fieldtype != "Attach Image":
			dontmanage.throw(_("Image field must be of type Attach Image"), InvalidFieldNameError)
	...
	def check_is_published_field(meta):
		...
		if meta.is_published_field not in fieldname_list:
			dontmanage.throw(_("Is Published Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_website_search_field(meta):
		...
		if meta.website_search_field not in fieldname_list:
			dontmanage.throw(_("Website Search Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_timeline_field(meta):
		...
		if meta.timeline_field not in fieldname_list:
			dontmanage.throw(_("Timeline field must be a valid fieldname"), InvalidFieldNameError)

		df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
		if df.fieldtype not in ("Link", "Dynamic Link"):
			dontmanage.throw(_("Timeline field must be a Link or Dynamic Link"), InvalidFieldNameError)
	...
	def check_illegal_depends_on_conditions(docfield):
		...
		for field in depends_on_fields:
			...
			if depends_on and ("=" in depends_on) and DEPENDS_ON_PATTERN.match(depends_on):
				dontmanage.throw(_("Invalid {0} condition").format(dontmanage.unscrub(field)), dontmanage.ValidationError)
	...
	def validate_fetch_from(field):
		...
		if source_field == field.fieldname:
			...
			dontmanage.throw(msg, title=_("Recursive Fetch From"))
	...
	def validate_data_field_type(docfield):
		...
		if docfield.fieldtype == "Data" and not (docfield.oldfieldtype and docfield.oldfieldtype != "Data"):
			if docfield.options and (docfield.options not in data_field_options):
				...
				dontmanage.msgprint(text_str + df_options_str, title="Invalid Data Field", alert=True)

	def check_child_table_option(docfield):
		if dontmanage.flags.in_fixtures:
			return
		...
		if not meta.is_virtual and child_doctype_meta.is_virtual and not docfield.is_virtual:
			dontmanage.throw(
				_("Field {0} must be a virtual field to support virtual doctype.").format(
					dontmanage.bold(docfield.fieldname)
				),
	...
	def check_max_height(docfield):
		if getattr(docfield, "max_height", None) and (docfield.max_height[-2:] not in ("px", "em")):
			dontmanage.throw(f"Max for {dontmanage.bold(docfield.fieldname)} height must be in px, em, rem")
			...
			if cint(docfield.precision) > cint(docfield.length):
				dontmanage.throw(
					_("Precision ({0}) for {1} cannot be greater than its length ({2}).").format(
						docfield.precision, dontmanage.bold(docfield.label), docfield.length
					)
				)
	...
	for d in fields:
		...
		if not dontmanage.flags.in_migrate or in_ci:
...
def validate_permissions_for_doctype(doctype, for_remove=False, alert=False):
	...
	doctype = dontmanage.get_doc("DocType", doctype)
...
def validate_permissions(doctype, for_remove=False, alert=False):
	...
	if not permissions and alert:
		dontmanage.msgprint(_("No Permissions Specified"), alert=True, indicator="orange")
	...
	def check_permission_dependency(d):
		if d.cancel and not d.submit:
			dontmanage.throw(_("{0}: Cannot set Cancel without Submit").format(get_txt(d)))

		if (d.submit or d.cancel or d.amend) and not d.write:
			dontmanage.throw(_("{0}: Cannot set Submit, Cancel, Amend without Write").format(get_txt(d)))
		if d.amend and not d.write:
			dontmanage.throw(_("{0}: Cannot set Amend without Cancel").format(get_txt(d)))
		if d.get("import") and not d.create:
			dontmanage.throw(_("{0}: Cannot set Import without Create").format(get_txt(d)))
	...
	def remove_rights_for_single(d):
		...
		if d.report:
			dontmanage.msgprint(_("Report cannot be set for Single types"))
			d.report = 0
			d.set("import", 0)
			d.set("export", 0)
	...
	def check_if_submittable(d):
		if d.submit and not issubmittable:
			...
			dontmanage.throw(_("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d)))

	def check_if_importable(d):
		if d.get("import") and not isimportable:
			dontmanage.throw(_("{0}: Cannot set import as {1} is not importable").format(get_txt(d), doctype))
	...
	def validate_permission_for_all_role(d):
		...
		if doctype.custom:
			...
			roles = [row.name for row in dontmanage.get_all("Role", filters={"is_custom": 1})]
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if (
			...
			and not dontmanage.db.exists("Domain", doc.restrict_to_domain)
		):
			dontmanage.get_doc(doctype="Domain", domain=doc.restrict_to_domain).insert()

		if "tabModule Def" in dontmanage.db.get_tables() and not dontmanage.db.exists("Module Def", doc.module):
			m = dontmanage.get_doc({"doctype": "Module Def", "module_name": doc.module})
			if dontmanage.scrub(doc.module) in dontmanage.local.module_app:
				m.app_name = dontmanage.local.module_app[dontmanage.scrub(doc.module)]
			else:
				m.app_name = "dontmanage"
			m.flags.ignore_mandatory = m.flags.ignore_permissions = True
			if dontmanage.flags.package:
				m.package = dontmanage.flags.package.name
				m.custom = 1
			m.insert()
...
def check_fieldname_conflicts(docfield):
	...
	if docfield.fieldname in method_list + property_list:
		dontmanage.msgprint(msg, raise_exception=not docfield.is_virtual)
...
def check_email_append_to(doc):
	...
	if doc.subject_field and not subject_field:
		dontmanage.throw(_("Select a valid Subject field for creating documents from Email"))
	...
	if subject_field and subject_field.fieldtype not in [
		...
		dontmanage.throw(_("Subject Field type should be Data, Text, Long Text, Small Text, Text Editor"))
	...
	if doc.sender_field and not sender_field:
		dontmanage.throw(_("Select a valid Sender Field for creating documents from Email"))

	if sender_field.options != "Email":
		dontmanage.throw(_("Sender Field should have Email in options"))

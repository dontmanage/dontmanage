...
import dontmanage
from dontmanage import _, msgprint, throw
...
def send_sms(receiver_list, msg, sender_name="", success_msg=True):
	send_sms_hook_methods = dontmanage.get_hooks("send_sms")
	if send_sms_hook_methods:
		return dontmanage.get_attr(send_sms_hook_methods[-1])(receiver_list, msg, sender_name, success_msg)
	...
	arg = {
		"receiver_list": receiver_list,
		"message": dontmanage.safe_decode(msg).encode("utf-8"),
		"success_msg": success_msg,
	}

	if dontmanage.db.get_single_value("SMS Settings", "sms_gateway_url"):
...
def send_via_gateway(arg):
	...
	message = dontmanage.safe_decode(arg.get("message"))
	...
	if len(success_list) > 0:
		...
		if arg.get("success_msg"):
			dontmanage.msgprint(_("SMS sent successfully"))


def get_headers(sms_settings=None):
	if not sms_settings:
		sms_settings = dontmanage.get_doc("SMS Settings", "SMS Settings")

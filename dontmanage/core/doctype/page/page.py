...
import dontmanage
from dontmanage import _, conf, get_module_path, safe_decode
...
from dontmanage.desk.form.meta import get_code_files_via_hooks, get_js
...
class Page(Document):
	...
	def autoname(self):
		...
		if (self.name and self.name.startswith("New Page")) or not self.name:
			...
			if dontmanage.db.exists("Page", self.name):
				cnt = dontmanage.db.sql(
	...
	def validate(self):
		...
		if self.is_new() and not getattr(conf, "developer_mode", 0):
			dontmanage.throw(_("Not in Developer Mode"))

		# setting ignore_permissions via update_setup_wizard_access (setup_wizard.py)
		if dontmanage.session.user != "Administrator" and not self.flags.ignore_permissions:
			dontmanage.throw(_("Only Administrator can edit"))
	...
	def on_update(self):
		...
		from dontmanage.core.doctype.doctype.doctype import make_module_and_roles
		...
		if path:
			...
			if not os.path.exists(path + ".js"):
				with open(path + ".js", "w") as f:
					f.write(
						f"""dontmanage.pages['{self.name}'].on_page_load = function(wrapper) {{
	...
	def on_trash(self):
		if not dontmanage.conf.developer_mode and not dontmanage.flags.in_migrate:
			dontmanage.throw(_("Deletion of this document is only permitted in developer mode."))

		delete_custom_role("page", self.name)
		dontmanage.db.after_commit(self.delete_folder_with_contents)

	def delete_folder_with_contents(self):
		try:
			module_path = get_module_path(self.module)
			dir_path = os.path.join(module_path, "page", dontmanage.scrub(self.name))
			...
			if fname.endswith(".html"):
				with open(os.path.join(path, fname)) as f:
					...
					if "<!-- jinja -->" in template:
						...
						try:
							out = dontmanage.get_attr(
								"{app}.{module}.page.{page}.{page}.get_context".format(
									app=dontmanage.local.module_app[scrub(self.module)],
						...
						template = dontmanage.render_template(template, context)
...
def delete_custom_role(field, docname):
	name = dontmanage.db.get_value("Custom Role", {field: docname}, "name")
	if name:
		dontmanage.delete_doc("Custom Role", name)

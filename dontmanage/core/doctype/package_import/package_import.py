...
class PackageImport(Document):
	...
	def import_package(self):
		...
		if not attachment:
			dontmanage.throw(dontmanage._("Please attach the package"))

		attachment = attachment[0]

		# get package_name from file (package_name-0.0.0.tar.gz)
		package_name = attachment.file_name.split(".", 1)[0].rsplit("-", 1)[0]
		if not os.path.exists(dontmanage.get_site_path("packages")):
			os.makedirs(dontmanage.get_site_path("packages"))

...
import dontmanage
from dontmanage.permissions import update_permission_property
...
class IntegrationTestPermissionType(IntegrationTestCase):
	...
	def test_approve_ptype_on_blog_post(self):
		...
		try:
			...
			dontmanage.delete_doc("User", user.name, force=True)
			dontmanage.delete_doc("Permission Type", ptype_doc.name, force=True)
	...
	def _verify_custom_fields_created(self, ptype_doc, doc_type):
		...
		for target in ["Custom DocPerm", "DocPerm", "DocShare"]:
			custom_field = dontmanage.get_doc("Custom Field", {"dt": target, "fieldname": ptype_doc.perm_type})
	...
	def _verify_user_lacks_permission(self, doc_type, ptype_name, user_name):
		...
		self.assertFalse(dontmanage.has_permission(doc_type, ptype=ptype_name, user=user_name))

	def _verify_user_has_permission(self, doc_type, ptype_name, user_name):
		"""Verify that user has the specified permission type."""
		self.assertTrue(dontmanage.has_permission(doc_type, ptype=ptype_name, user=user_name))

...
from dontmanage.utils import cstr, get_site_url


class TestAccessLog(DontManageTestCase):
	def setUp(self):
		# generate keys for current user to send requests for the following tests
		generate_keys(dontmanage.session.user)
		dontmanage.db.commit()
		generated_secret = dontmanage.utils.password.get_decrypted_password(
			"User", dontmanage.session.user, fieldname="api_secret"
		)
		...
		self.file_name = dontmanage.utils.random_string(10) + ".txt"
		self.test_content = dontmanage.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = dontmanage.get_last_doc("Access Log")
	...
	def test_private_file_download(self):
		...
		new_private_file = dontmanage.get_doc(
			{
				"doctype": self.test_doctype,
		...
		private_file_link = get_site_url(dontmanage.local.site) + new_private_file.file_url

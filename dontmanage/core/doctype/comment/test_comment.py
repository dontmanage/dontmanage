...
class TestComment(IntegrationTestCase):
	...
	def test_comment_creation(self):
		test_doc = dontmanage.get_doc(doctype="ToDo", description="test")
		test_doc.insert()
		...
		# Check comment count
		counts = dontmanage.get_all("ToDo", {"name": test_doc.name}, ["*"], with_comment_count=True)
	...
	def test_public_comment(self):
		...
		dontmanage.db.delete("Comment", {"reference_doctype": "Test Blog Post"})
		add_comment_args = {
	...
	def test_user_not_logged_in(self):
		...
		test_blog = dontmanage.get_doc("Web Page", "test-web-page-1")
		with set_user("Guest"):
			self.assertRaises(

...
from dontmanage.desk.notifications import notify_mentions
from dontmanage.exceptions import ImplicitCommitError
...
class Comment(Document):
	...
	def validate(self):
		if not self.comment_email:
			self.comment_email = dontmanage.session.user
		self.content = dontmanage.utils.sanitize_html(
...
def on_doctype_update():
	dontmanage.db.add_index("Comment", ["reference_doctype", "reference_name"])
...
def get_comments_from_parent(doc):
	...
	try:
		if is_virtual_doctype(doc.reference_doctype):
			...
			_comments = dontmanage.db.get_value(doc.reference_doctype, doc.reference_name, "_comments") or "[]"

	except Exception as e:
		if dontmanage.db.is_missing_table_or_column(e):
			_comments = "[]"
...
def update_comments_in_parent(reference_doctype, reference_name, _comments):
	...
	if (
		...
		or dontmanage.db.get_value("DocType", reference_doctype, "issingle")
	...
	try:
		...
		if dontmanage.db.is_missing_column(e) and getattr(dontmanage.local, "request", None):
			pass

...
class Report(Document):
	...
	def on_trash(self):
		if (
			...
			and not dontmanage.flags.in_patch
		):
	...
	def export_doc(self):
		...
		if self.is_standard == "Yes" and dontmanage.conf.developer_mode:
	...
	def execute_script_report(self, filters):
		...
		if not self.prepared_report:
			prepared_report_watcher = threading.Timer(
				...
				kwargs={"report": self.name, "site": dontmanage.local.site},
			)
...
def is_prepared_report_enabled(report):
	return cint(dontmanage.db.get_value("Report", report, "prepared_report"))

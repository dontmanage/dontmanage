...
class TestReport(DontManageTestCase):
	...
	def test_custom_report(self):
		...
		columns, result = custom_report.run_query_report(user=dontmanage.session.user)
	...
	def test_report_permissions(self):
		...
		if not dontmanage.db.exists("Role", "Test Has Role"):
			dontmanage.get_doc({"doctype": "Role", "role_name": "Test Has Role"}).insert(ignore_permissions=True)

...
class TestUserPermission(DontManageTestCase):
	def setUp(self):
		test_users = (
	...
	def test_default_user_permission_corectness(self):
		...
		dontmanage.delete_doc("Blog Post", test_blog.name)
	...
	def test_default_user_permission(self):
		...
		for category in ["general", "public"]:
			if not dontmanage.db.exists("Blog Category", category):
				dontmanage.get_doc({"doctype": "Blog Category", "title": category}).insert()

...
from dontmanage.core.doctype.user_permission.user_permission import (
...
class TestUserPermission(IntegrationTestCase):
	def setUp(self):
		...
		dontmanage.db.delete("User Permission", {"user": ("in", test_users)})
		dontmanage.delete_doc_if_exists("DocType", "Person")
		dontmanage.db.sql_ddl("DROP TABLE IF EXISTS `tabPerson`")
		dontmanage.delete_doc_if_exists("DocType", "Doc A")
		dontmanage.db.sql_ddl("DROP TABLE IF EXISTS `tabDoc A`")
		setup_for_tests()
	...
	def test_default_user_permission_validation(self):
		...
		self.assertRaises(dontmanage.ValidationError, add_user_permissions, param)
	...
	def test_default_user_permission_corectness(self):
		...
		test_blog = dontmanage.get_doc("Test Blog Post", "_Test Blog Post 1")
		param = get_params(perm_user, "Test Blog Post", test_blog.name, is_default=1, hide_descendants=1)
		add_user_permissions(param)
		dontmanage.db.delete("User Permission", filters={"for_value": test_blog.name})
		dontmanage.delete_doc("Test Blog Post", test_blog.name)
	...
	def test_default_user_permission(self):
		...
		for category in ["general", "public"]:
			if not dontmanage.db.exists("Test Blog Category", category):
				dontmanage.get_doc({"doctype": "Test Blog Category", "title": category}).insert()
		...
		dontmanage.set_user("test_user_perm1@example.com")
	...
	def test_for_applicable_on_update_from_apply_to_all(self):
		...
		removed_apply_to_all = dontmanage.db.exists("User Permission", get_exists_param(user))
		is_created_applicable_first = dontmanage.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		is_created_applicable_second = dontmanage.db.exists(
	...
	def test_for_apply_to_all_on_update_from_applicable(self):
		...
		is_created = add_user_permissions(param)
		is_created_apply_to_all = dontmanage.db.exists("User Permission", get_exists_param(user))
		removed_applicable_first = dontmanage.db.exists(
			"User Permission", get_exists_param(user, applicable="Comment")
		)
		removed_applicable_second = dontmanage.db.exists(
			"User Permission", get_exists_param(user, applicable="Contact")
		)
	...
	def test_user_perm_for_nested_doctype(self):
		...
		if not dontmanage.db.exists("DocType", "Person"):
			doc = new_doctype(
				"Person",
	...
	def test_user_perm_on_new_doc_with_field_default(self):
		...
		if not dontmanage.db.exists("DocType", "Doc A"):
			doc = new_doctype(
				"Doc A",
				fields=[
					{
		...
		dontmanage.set_user("new_doc_test@example.com")

		new_doc = dontmanage.new_doc("Doc A")
	...
	def test_user_perm_on_new_doc_with_user_default(self):
		"""Test User Perm impact on dontmanage.new_doc. with *user* default value"""
		...
		if not dontmanage.db.exists("Session Default", {"ref_doctype": "DocType"}):
...
def create_user(email, *roles):
	...
	if dontmanage.db.exists("User", email):
		return dontmanage.get_doc("User", email)

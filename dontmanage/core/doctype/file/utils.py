...
def setup_folder_path(filename: str, new_parent: str) -> None:
	file: File = dontmanage.get_doc("File", filename)
...
def get_local_image(file_url: str) -> tuple["ImageFile", str, str]:
	...
	file_path = dontmanage.get_site_path(*file_url_path)

	try:
		image = Image.open(file_path)
	except OSError:
		dontmanage.throw(_("Unable to read file format for {0}").format(file_url))
...
def get_web_image(file_url: str) -> tuple["ImageFile", str, str]:
	...
	try:
		...
		if "404" in e.args[0]:
			dontmanage.msgprint(_("File '{0}' not found").format(file_url))
		else:
			dontmanage.msgprint(_("Unable to read file format for {0}").format(file_url))
		raise

	try:
		image = Image.open(BytesIO(r.content))
	except Exception as e:
		dontmanage.msgprint(_("Image link '{0}' is not valid").format(file_url), raise_exception=e)
...
def remove_file_by_url(file_url: str, doctype: str | None = None, name: str | None = None) -> "Document":
	if doctype and name:
		...
		fid = dontmanage.db.get_value("File", {"file_url": file_url})

	if fid:
		from dontmanage.utils.file_manager import remove_file
...
def get_file_name(fname: str, optional_suffix: str | None = None) -> str:
	...
	suffix = optional_suffix or dontmanage.generate_hash(length=6)
...
def extract_images_from_doc(doc: "Document", fieldname: str, is_private=True):
	...
	if dontmanage.flags.has_dataurl:
...
def update_existing_file_docs(doc: "File") -> None:
	...
	file_doctype = dontmanage.qb.DocType("File")
	(
		dontmanage.qb.update(file_doctype)
...
def relink_files(doc, fieldname, temp_doc_name):
	...
	mislinked_file = dontmanage.db.get_value(
		"File",
		{

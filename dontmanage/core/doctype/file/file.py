...
from dontmanage.permissions import SYSTEM_USER_ROLE, get_doctypes_with_read
...
from dontmanage.utils.file_manager import is_safe_path
from dontmanage.utils.html_utils import escape_html
from dontmanage.utils.image import optimize_image, strip_exif_data
from dontmanage.utils.pdf import pdf_contains_js
...
class File(Document):
	...
	def autoname(self):
		...
		if self.is_folder:
			...
			self.name = dontmanage.generate_hash(length=10)
	...
	def before_insert(self):
		...
		if self.is_remote_file:
			...
			dontmanage.db.after_rollback.add(self.on_rollback)
	...
	def validate(self):
		...
		self.file_size = dontmanage.form_dict.file_size or self.file_size
	...
	def validate_attachment_references(self):
		...
		if self.attached_to_field and SPECIAL_CHAR_PATTERN.search(self.attached_to_field):
			dontmanage.throw(_("The fieldname you've specified in Attached To Field is invalid"))

	def enforce_public_file_restrictions(self):
		if not self.is_private and dontmanage.get_system_settings(
			"only_allow_system_managers_to_upload_public_files"
		):
			try:
				dontmanage.only_for("System Manager")
			except PermissionError:
				dontmanage.throw(_("Only System Managers can make this file public."))
	...
	def on_trash(self):
		if self.is_home_folder or self.is_attachments_folder:
			dontmanage.throw(_("Cannot delete Home and Attachments folders"))
	...
	def get_successors(self):
		return dontmanage.get_all("File", filters={"folder": self.name}, pluck="name")
	...
	def handle_is_private_changed(self):
		...
		private_file_path = Path(dontmanage.get_site_path("private", "files", file_name))
		public_file_path = Path(dontmanage.get_site_path("public", "files", file_name))
		...
		if dontmanage.get_meta(self.attached_to_doctype).issingle:
	...
	def fetch_attached_to_field(self, old_file_url):
		...
		reference_dict = dontmanage.get_doc(self.attached_to_doctype, self.attached_to_name).as_dict()
	...
	def validate_attachment_limit(self):
		...
		if self.attached_to_doctype and self.attached_to_name:
			attachment_limit = cint(dontmanage.get_meta(self.attached_to_doctype).max_attachments)
			...
			if current_attachment_count >= attachment_limit:
				dontmanage.throw(
					_("Maximum Attachment Limit of {0} has been reached for {1} {2}.").format(
						dontmanage.bold(attachment_limit),
						self.attached_to_doctype,
						self.attached_to_name,
					),
	...
	def set_folder_name(self):
		...
		if self.attached_to_doctype:
			self.folder = dontmanage.db.get_value("File", {"is_attachments_folder": 1})
	...
	def validate_file_on_disk(self):
		...
		if not os.path.exists(full_path):
			dontmanage.throw(_("File {0} does not exist").format(self.file_url), IOError)

	def validate_file_extension(self):
		# Only validate uploaded files, not generated by code/integrations.
		if not self.file_type or not dontmanage.request:
			return

		allowed_extensions = dontmanage.get_system_settings("allowed_file_extensions")
	...
	def check_content(self):
		if self.file_type == "PDF" and self._content and pdf_contains_js(self._content):
			dontmanage.throw(_("This PDF cannot be uploaded as it contains unsafe content."))
	...
	def validate_duplicate_entry(self):
		if not self.flags.ignore_duplicate_entry_error and not self.is_folder:
			...
			duplicate_file = dontmanage.db.get_value("File", filters, ["name", "file_url"], as_dict=1)

			if duplicate_file:
				duplicate_file_doc = dontmanage.get_cached_doc("File", duplicate_file.name)
	...
	def generate_content_hash(self):
		...
		try:
			...
			dontmanage.throw(_("File {0} does not exist").format(file_path))
	...
	def make_thumbnail(
		...
		path = os.path.abspath(dontmanage.get_site_path("public", thumbnail_url.lstrip("/")))
	...
	def validate_empty_folder(self):
		...
		if self.is_folder and dontmanage.get_all("File", filters={"folder": self.name}, limit=1):
			dontmanage.throw(_("Folder {0} is not empty").format(self.name), FolderNotEmpty)
	...
	def validate_protected_file(self):
		...
		try:
			ref_doc = dontmanage.get_doc(self.attached_to_doctype, self.attached_to_name)
	...
	def _delete_file_on_disk(self):
		...
		on_disk_file_not_shared = self.content_hash and not dontmanage.get_all(
			"File",
			filters={
				"content_hash": self.content_hash,
				"name": ["!=", self.name],
	...
	def unzip(self) -> list["File"]:
		...
		if not self.file_url.endswith(".zip"):
			dontmanage.throw(_("{0} is not a zip file").format(self.file_name))
		...
		with zipfile.ZipFile(zip_path) as z:
			for file in z.filelist:
				...
				try:
					...
					dontmanage.throw(_("{0} is a not a valid zip file").format(self.file_name))
		...
		dontmanage.delete_doc("File", self.name)
		return files
	...
	def get_content(self, encodings=None) -> bytes | str:
		if self.is_folder:
			dontmanage.throw(_("Cannot get file contents of a Folder"))
	...
	def get_full_path(self):
		...
		if file_path.startswith("/private/files/"):
			...
			dontmanage.throw(_("There is some problem with the file url: {0}").format(file_path))

		if not is_safe_path(file_path):
			dontmanage.throw(_("Cannot access file path {0}").format(file_path))

		if os.path.sep in self.file_name:
			dontmanage.throw(_("File name cannot have {0}").format(os.path.sep))
	...
	def save_file(
		...
		if (
			...
			and dontmanage.get_system_settings("strip_exif_metadata_from_uploaded_images")
		):
		...
		if not ignore_existing_file_check:
			duplicate_file = dontmanage.get_value(
				"File",
		...
		if duplicate_file:
			file_doc: File = dontmanage.get_cached_doc("File", duplicate_file.name)
	...
	def check_max_file_size(self):
		...
		if file_size > max_file_size:
			...
			if dontmanage.has_permission("System Settings", "write"):
				msg += ".<br>" + _("You can increase the limit from System Settings.")
			dontmanage.throw(msg, exc=MaxFileSizeReachedError)
	...
	def create_attachment_record(self):
		...
		file_url = quote(dontmanage.safe_encode(self.file_url), safe="/:") if self.file_url else self.file_name
	...
	def add_comment_in_reference_doc(self, comment_type, text):
		if self.attached_to_doctype and self.attached_to_name:
			try:
				doc = dontmanage.get_doc(self.attached_to_doctype, self.attached_to_name)
	...
	def zip_files(files):
		...
		for _file in files:
			if isinstance(_file, str):
				_file = dontmanage.get_doc("File", _file)
...
def on_doctype_update():
	dontmanage.db.add_index("File", ["attached_to_doctype", "attached_to_name"])
	dontmanage.db.add_index("File", ["file_url(100)"])
...
def has_permission(doc, ptype=None, user=None, debug=False):
	...
	if doc.attached_to_doctype and doc.attached_to_name:
		...
		try:
			ref_doc = dontmanage.get_doc(attached_to_doctype, attached_to_name)
...
def get_permission_query_conditions(user: str | None = None) -> str:
	...
	if SYSTEM_USER_ROLE not in dontmanage.get_roles(user):
		return f""" `tabFile`.`owner` = {dontmanage.db.escape(user)} """

	readable_doctypes = ", ".join(repr(dt) for dt in get_doctypes_with_read())
	return f"""
		(`tabFile`.`is_private` = 0)
		OR (`tabFile`.`attached_to_doctype` IS NULL AND `tabFile`.`owner` = {dontmanage.db.escape(user)})
...
from dontmanage.core.api.file import *

...
from dontmanage.permissions import get_doctypes_with_read
from dontmanage.utils import call_hook_method, cint, get_files_path, get_hook_method, get_url
...
class File(Document):
	...
	def validate_attachment_references(self):
		...
		if not self.attached_to_name or not isinstance(self.attached_to_name, str | int):
			dontmanage.throw(_("Attached To Name must be a string or an integer"), dontmanage.ValidationError)
			...
			if current_attachment_count >= attachment_limit:
				dontmanage.throw(
					_("Maximum Attachment Limit of {0} has been reached for {1} {2}.").format(
						dontmanage.bold(attachment_limit), self.attached_to_doctype, self.attached_to_name
					),
	...
	def validate_file_extension(self):
		...
		if self.file_type not in allowed_extensions.splitlines():
			dontmanage.throw(_("File type of {0} is not allowed").format(self.file_type), exc=FileTypeNotAllowed)
	...
	def set_file_name(self):
		if not self.file_name and not self.file_url:
			dontmanage.throw(
				_("Fields `file_name` or `file_url` must be set for File"), exc=dontmanage.MandatoryError
			)
...
def has_permission(doc, ptype=None, user=None, debug=False):
	...
	if ptype == "create":
		return dontmanage.has_permission("File", "create", user=user, debug=debug)

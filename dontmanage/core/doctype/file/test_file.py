...
class TestSimpleFile(DontManageTestCase):
	def setUp(self):
...
class TestSameFileName(DontManageTestCase):
	def test_saved_content(self):
		self.attached_to_doctype, self.attached_to_docname = make_test_doc()
...
class TestSameContent(DontManageTestCase):
	def setUp(self):
...
class TestFile(DontManageTestCase):
	def setUp(self):
...
class TestAttachment(DontManageTestCase):
...
class TestAttachmentsAccess(DontManageTestCase):
	def setUp(self) -> None:
...
class TestFileUtils(DontManageTestCase):
	def test_extract_images_from_doc(self):
		...
		self.assertTrue(dontmanage.db.exists("File", {"attached_to_name": todo.name}))
...
class TestFileOptimization(DontManageTestCase):

...
from dontmanage.core.doctype.file.utils import get_corrupted_image_msg, get_extension
...
class TestBase64File(IntegrationTestCase):
	def setUp(self):
		...
		_file: dontmanage.Document = dontmanage.get_doc(
			{
				"doctype": "File",
	...
	def test_saved_content(self):
		_file: dontmanage.Document = dontmanage.get_doc("File", {"file_url": self.saved_file_url})
		content = _file.get_content()
...
class TestSameFileName(IntegrationTestCase):
	def test_saved_content(self):
		...
		_file = dontmanage.get_doc("File", {"file_url": self.saved_file_url1})
		content1 = _file.get_content()
		self.assertEqual(content1, self.test_content1)
		_file = dontmanage.get_doc("File", {"file_url": self.saved_file_url2})
	...
	def test_saved_content_private(self):
		...
		_file = dontmanage.get_doc("File", {"file_url": _file1.file_url})
		self.assertEqual(_file.get_content(), test_content1)

		_file = dontmanage.get_doc("File", {"file_url": _file2.file_url})
...
class TestSameContent(IntegrationTestCase):
	...
	def test_attachment_limit(self):
		...
		self.assertRaises(dontmanage.exceptions.AttachmentLimitReached, file2.insert)
	...
	def test_utf8_bom_content_decoding(self):
		...
		_file.save()
		saved_file = dontmanage.get_doc("File", _file.name)
...
class TestFile(IntegrationTestCase):
	...
	def tearDown(self):
		try:
			dontmanage.get_doc("File", {"file_name": "file_copy.txt"}).delete()
		except dontmanage.DoesNotExistError:
			pass

	def delete_test_data(self):
		test_file_data = dontmanage.get_all(
			"File",
			pluck="name",
	...
	def tests_after_upload(self):
		...
		file_folder = dontmanage.db.get_value("File", self.saved_name, "folder")
		self.assertEqual(file_folder, _("Home/Test Folder 1"))

	def test_file_copy(self):
		folder = self.get_folder("Test Folder 2", "Home")

		file = dontmanage.get_doc("File", {"file_name": "file_copy.txt"})
	...
	def test_folder_copy(self):
		...
		file = dontmanage.get_doc("File", {"file_name": "folder_copy.txt"})
		file_copy_txt = dontmanage.get_value("File", {"file_name": "file_copy.txt"})
		if file_copy_txt:
			dontmanage.get_doc("File", file_copy_txt).delete()

		self.assertEqual(_("Home/Test Folder 1/Test Folder 3"), file.folder)

	def test_default_folder(self):
		d = dontmanage.get_doc({"doctype": "File", "file_name": _("Test_Folder"), "is_folder": 1})
		d.save()
	...
	def test_on_delete(self):
		...
		self.assertEqual(dontmanage.db.get_value("File", _("Home/Test Folder 1"), "file_size"), 0)
		...
		folder = dontmanage.get_doc("File", "Home/Test Folder 1/Test Folder 3")
	...
	def test_same_file_url_update(self):
		...
		file2 = dontmanage.get_doc("File", file2.name)
	...
	def test_file_url_validation(self):
		test_file: File = dontmanage.new_doc("File")
		test_file.update({"file_name": "logo", "file_url": "https://dontmanage.io/files/dontmanage.png"})
	...
	def test_make_thumbnail(self):
		# test web image
		test_file: File = dontmanage.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": dontmanage.utils.get_url("/_test/assets/image.jpg"),
			}
		...
		test_file = dontmanage.get_doc(
			{
				"doctype": "File",
				"file_name": "logo",
				"file_url": dontmanage.utils.get_url("/_test/assets/image"),
			}
		).insert(ignore_permissions=True)
	...
	def test_file_unzip(self):
		file_path = dontmanage.get_app_path("dontmanage", "www/_test/assets/file.zip")
		public_file_path = dontmanage.get_site_path("public", "files")
		try:
			import shutil
...
class TestAttachment(IntegrationTestCase):
	...
	def tearDownClass(cls):
		dontmanage.db.rollback()
		dontmanage.delete_doc("DocType", cls.test_doctype)

	def test_file_attachment_on_update(self):
		doc = dontmanage.get_doc(doctype=self.test_doctype, title="test for attachment on update").insert()
...
class TestFileUtils(IntegrationTestCase):
	def test_extract_images_from_doc(self):
		is_private = not dontmanage.get_meta("ToDo").make_attachments_public
		...
		self.assertTrue(dontmanage.db.exists("File", {"attached_to_name": todo.name, "is_private": is_private}))
		...
		filename = dontmanage.db.exists("File", {"attached_to_name": todo.name})
		self.assertIn(f'<img src="{dontmanage.get_doc("File", filename).file_url}', todo.description)
	...
	def test_extract_images_from_comment(self):
		...
		"""
		is_private = not dontmanage.get_meta("ToDo").make_attachments_public
		test_doc = dontmanage.get_doc(doctype="ToDo", description="comment test").insert()
		comment = add_comment(
			"ToDo",
			test_doc.name,
		...
		self.assertTrue(
			dontmanage.db.exists("File", {"attached_to_name": test_doc.name, "is_private": is_private})
		)
	...
	def test_extract_images_from_communication(self):
		...
		"""
		is_private = not dontmanage.get_meta("Communication").make_attachments_public
		...
		self.assertTrue(
			dontmanage.db.exists("File", {"attached_to_name": communication.name, "is_private": is_private})
		)
...
class TestFileOptimization(IntegrationTestCase):
	...
	def test_optimize_svg(self):
		file_path = dontmanage.get_app_path("dontmanage", "tests/data/sample_svg.svg")
		with open(file_path, "rb") as f:
			file_content = f.read()
		test_file = dontmanage.get_doc(
	...
	def test_optimize_folder(self):
		test_folder = dontmanage.get_doc("File", "Home/Attachments")
...
class TestGuestFileAndAttachments(IntegrationTestCase):
	...
	def test_private_remains_private_even_if_same_hash(self):
		file_name = "test" + dontmanage.generate_hash()
		content = file_name.encode()

		doc_pub: File = dontmanage.new_doc("File")  # type: ignore
		doc_pub.file_url = f"/files/{file_name}.txt"
		doc_pub.content = content
		doc_pub.save()

		doc_pri: File = dontmanage.new_doc("File")  # type: ignore
...
class TestPublicFileRestriction(IntegrationTestCase):
	...
	def setUpClass(cls):
		...
		if not dontmanage.db.exists("User", "test_restricted@example.com"):
	...
	def test_non_system_manager_cannot_upload_public_file_when_setting_enabled(self):
		...
		dontmanage.set_user("test_restricted@example.com")

		file_doc = dontmanage.get_doc(
			{
				"doctype": "File",
		...
		self.assertRaises(dontmanage.PermissionError, file_doc.insert)

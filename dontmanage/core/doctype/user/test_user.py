...
class TestUser(IntegrationTestCase):
	...
	def test_delete(self):
		...
		user = dontmanage.copy_doc(self.globalTestRecords["User"][1])
		...
		dontmanage.copy_doc(self.globalTestRecords["Role"][1]).insert()
	...
	def test_signup(self):
		...
		self.assertEqual(
			dontmanage.cache.hget("redirect_after_login", random_user), sanitize_redirect("/welcome")
		)
...
class TestImpersonation(DontManageAPITestCase):
	def test_impersonation(self):
		with test_user(roles=["System Manager"], commit=True) as user:
			self.post(
				self.method("dontmanage.core.doctype.user.user.impersonate"),
				{"user": user.name, "reason": "test", "sid": self.sid},
			)
			resp = self.get(self.method("dontmanage.auth.get_logged_user"))
...
def test_user(
	*, first_name: str | None = None, email: str | None = None, roles: list[str], commit=False, **kwargs
	...
	try:
		...
		user.insert()
		yield user
		commit and dontmanage.db.commit()
	finally:

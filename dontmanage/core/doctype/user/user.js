dontmanage.ui.form.on("User", {
	setup: function (frm) {
		frm.set_query("default_workspace", () => {
			return {
				filters: {
					for_user: ["in", [null, dontmanage.session.user]],
		...
		if (
			...
			if (!frm.roles_editor) {
				...
				if (frm.doc.user_type == "System User") {
					...
					frm.module_editor = new dontmanage.ModuleEditor(
						frm,
						module_area,
		...
		if (!frm.is_new()) {
			if (has_access_to_edit_user()) {
				...
				frm.add_custom_button(
					...
					() =>
						dontmanage.set_route("query-report", "User Doctype Permissions", {
							user: frm.doc.name,
						}),
					__("Permissions")
				);
		...
		if (frm.doc.user_emails && dontmanage.model.can_create("Email Account")) {
			...
			if (!found) {
				frm.add_custom_button(__("Create User Email"), function () {
					if (!frm.doc.email) {
						dontmanage.msgprint(__("Email is mandatory to create User Email"));
						return;
					}
...
dontmanage.ui.form.on("User Role Profile", {
...
function show_api_key_dialog(api_key, api_secret) {
	const dialog = new dontmanage.ui.Dialog({
		...
		secondary_action: () => {
			...
			dontmanage.utils.copy_to_clipboard(token);
			dialog.hide();
		},
	});
...
dontmanage.ui.form.on("User Session Display", {
	sign_out(frm, doctype, name) {
		dontmanage
			.xcall("dontmanage.core.doctype.user.user.clear_session", { sid_hash: name })
			.then(() => frm.reload_doc());
	},
});

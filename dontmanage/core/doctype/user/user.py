...
from dontmanage.apps import get_default_path
...
from dontmanage.sessions import clear_sessions
...
from dontmanage.website.utils import get_home_page, is_signup_disabled
from dontmanage.www.login import sanitize_redirect
...
class User(Document):
	...
	if TYPE_CHECKING:
		...
		from dontmanage.core.doctype.user_role_profile.user_role_profile import UserRoleProfile
		from dontmanage.core.doctype.user_session_display.user_session_display import UserSessionDisplay
	...
	def active_sessions(self):
		...
		if self.name != dontmanage.session.user:
		...
		for session in sessions_data:
			data = dontmanage.parse_json(session.sessiondata)
			...
			session_docs.append(
				{
					...
					"is_current": session.sid == dontmanage.session.sid,
	...
	def onload(self):
		from dontmanage.utils.modules import get_modules_from_all_apps
	...
	def populate_role_profile_roles(self):
		...
		for role_profile in self.role_profiles:
			role_profile = dontmanage.get_cached_doc("Role Profile", role_profile.role_profile)
	...
	def validate_allowed_modules(self):
		if self.module_profile:
			module_profile = dontmanage.get_doc("Module Profile", self.module_profile)
	...
	def validate_user_image(self):
		if self.user_image and len(self.user_image) > 2000:
			dontmanage.throw(_("Not a valid User Image."))

	def on_update(self):
		# clear new password
		self.share_with_self()
		clear_notifications(user=self.name)
		dontmanage.clear_cache(user=self.name)
		now = dontmanage.in_test or dontmanage.flags.in_install
		self.send_password_notification(self.__new_password)
		dontmanage.enqueue(
			"dontmanage.core.doctype.user.user.create_contact",
			user=self,
		...
		if self.name not in STANDARD_USERS and not self.user_image:
			dontmanage.enqueue(
				"dontmanage.core.doctype.user.user.update_gravatar",
				name=self.name,
				now=now,
				enqueue_after_commit=True,
			)
		...
		if self.has_value_changed("language"):
			...
			if self.language:
				language = dontmanage.get_doc("Language", self.language)
				for key in locale_keys:
					value = language.get(key)
					if value:
						dontmanage.defaults.set_default(key, value, self.name)
			else:
				for key in locale_keys:
					dontmanage.defaults.clear_default(key, parent=self.name)
	...
	def has_website_permission(self, ptype, user, verbose=False):
		...
		return self.name == dontmanage.session.user
	...
	def check_enable_disable(self):
		...
		if not cint(self.enabled) and self.name in STANDARD_USERS:
			dontmanage.throw(_("User {0} cannot be disabled").format(self.name))

		# clear sessions if disabled
		if not cint(self.enabled) and getattr(dontmanage.local, "login_manager", None):
			dontmanage.local.login_manager.logout(user=self.name)
	...
	def set_system_user(self):
		...
		if self.user_type and not dontmanage.get_cached_value("User Type", self.user_type, "is_standard"):
	...
	def set_roles_and_modules_based_on_user_type(self):
		user_type_doc = dontmanage.get_cached_doc("User Type", self.user_type)
	...
	def validate_share(self, docshare):
		...
		# 		if docshare.share != 1:
		# 			dontmanage.throw(_("Sorry! User should have complete access to their own record."))
		# 	else:
		# 		dontmanage.throw(_("Sorry! Sharing with Website User is prohibited."))
	...
	def send_welcome_mail_to_user(self):
		...
		subject = None
		method = dontmanage.get_hooks("welcome_email")
		if method:
			subject = dontmanage.get_attr(method[-1])()
		if not subject:
			site_name = dontmanage.db.get_default("site_name") or dontmanage.get_conf().get("site_name")
			if site_name:
	...
	def send_login_mail(self, subject, template, add_args, now=None, custom_template=None):
		...
		created_by = get_user_fullname(dontmanage.session["user"])
		if created_by == "Guest":
		...
		sender = (
			dontmanage.session.user not in STANDARD_USERS and get_formatted_email(dontmanage.session.user)
		) or None
	...
	def on_trash(self):
		...
		self.enabled = 0
		if getattr(dontmanage.local, "login_manager", None):
			dontmanage.local.login_manager.logout(user=self.name)

		# delete todos
		dontmanage.db.delete("ToDo", {"allocated_to": self.name})
		todo_table = DocType("ToDo")
		(
			dontmanage.qb.update(todo_table)
			.set(todo_table.assigned_by, None)
			.where(todo_table.assigned_by == self.name)
		).run()

		# delete events
		dontmanage.db.delete("Event", {"owner": self.name, "event_type": "Private"})

		# delete shares
		dontmanage.db.delete("DocShare", {"user": self.name})
		# unlink contact
		table = DocType("Contact")
		dontmanage.qb.update(table).where(table.user == self.name).set(table.user, None).run()
		...
		dontmanage.db.set_value("Workflow Action", {"user": self.name}, "user", None)

		# Delete user's List Filters
		dontmanage.db.delete("List Filter", {"for_user": self.name})

		# Remove user from Note's Seen By table
		seen_notes = dontmanage.get_all("Note", filters=[["Note Seen By", "user", "=", self.name]], pluck="name")
		for note_id in seen_notes:
			note = dontmanage.get_doc("Note", note_id)
		...
		invites = dontmanage.db.get_all("User Invitation", filters={"email": self.name}, pluck="name")
		for invite in invites:
			invite_doc = dontmanage.get_doc("User Invitation", invite)
	...
	def before_rename(self, old_name, new_name, merge=False):
		...
		if merge:
			dontmanage.delete_doc("Notification Settings", old_name, ignore_permissions=True)
	...
	def after_rename(self, old_name, new_name, merge=False):
		...
		if dontmanage.db.exists("Notification Settings", old_name):
	...
	def remove_disabled_roles(self):
		disabled_roles = [d.name for d in dontmanage.get_all("Role", filters={"disabled": 1})]
	...
	def validate_username(self):
		if not self.username and self.is_new() and self.first_name:
			self.username = dontmanage.scrub(self.first_name)
		...
		if self.username_exists():
			if self.user_type == "System User":
				dontmanage.msgprint(_("Username {0} already exists").format(self.username))
	...
	def suggest_username(self):
		...
		# @firstname
		username = _check_suggestion(dontmanage.scrub(self.first_name))
	...
	def validate_user_email_inbox(self):
		...
		if len(email_accounts) != len(set(email_accounts)):
			dontmanage.throw(_("Email Account added multiple times"))
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(dontmanage.get_system_settings("allow_login_using_mobile_number"))
		login_with_username = cint(dontmanage.get_system_settings("allow_login_using_user_name"))
		...
		users = dontmanage.get_all("User", fields=["name", "enabled"], or_filters=or_filters, limit=1)
		if not users:
			return
		...
		if validate_password:
			...
			except dontmanage.AuthenticationError:
...
def get_all_roles():
	...
	roles = (
		...
		.select(Role.name)
		.where(
			(Role.name.notin(dontmanage.permissions.AUTOMATIC_ROLES)) & (Role.disabled == 0) & domain_condition
		)
		.orderby(Role.name)
		.run(as_dict=True)
	)
...
def get_roles(arg=None):
	...
	return dontmanage.get_roles(dontmanage.form_dict.get("uid", dontmanage.session.user))


@dontmanage.whitelist()
def get_perm_info(role):
	"""get permission info"""
	from dontmanage.permissions import get_all_perms
...
def update_password(
	...
	if res.get("message"):
		dontmanage.local.response.http_status_code = 410
		return res["message"]
	else:
		user = res["user"]

	logout_all_sessions = cint(logout_all_sessions) or dontmanage.get_system_settings("logout_on_password_reset")
	...
	dontmanage.db.set_value("User", user, "last_password_reset_date", today())
	dontmanage.db.set_value("User", user, "reset_password_key", "")
...
def test_password_strength(new_password: str, key=None, old_password=None, user_data: tuple | None = None):
	...
	enable_password_policy = dontmanage.get_system_settings("enable_password_policy")

	if not enable_password_policy:
		return {}

	if not user_data:
		user_data = dontmanage.db.get_value(
			"User", dontmanage.session.user, ["first_name", "middle_name", "last_name", "email", "birth_date"]
		)

	if new_password:
		result = _test_password_strength(new_password, user_inputs=user_data)
		password_policy_validation_passed = False
		minimum_password_score = cint(dontmanage.get_system_settings("minimum_password_score"))
...
@dontmanage.whitelist(allow_guest=False)
...
def ask_pass_update():
	...
	from dontmanage.utils import set_default

	password_list = dontmanage.get_all(
		"User Email",
	...
	if key:
		...
		if result.user:
			reset_password_link_expiry = cint(
				dontmanage.get_system_settings("reset_password_link_expiry_duration")
			)
			if (
		...
		# verify old password
		dontmanage.local.login_manager.check_password(dontmanage.session.user, old_password)
...
def sign_up(email: str, full_name: str, redirect_to: str) -> tuple[int, str]:
	...
	user = dontmanage.db.get("User", {"email": email})
	if user:
		if user.enabled:
		...
		max_signups_allowed_per_hour = cint(dontmanage.get_system_settings("max_signups_allowed_per_hour") or 300)
		users_created_past_hour = dontmanage.db.get_creation_count("User", 60)
		...
		default_role = dontmanage.get_single_value("Portal Settings", "default_role")
		if default_role:
			user.add_roles(default_role)

		if redirect_to:
			dontmanage.cache.hset("redirect_after_login", user.name, sanitize_redirect(redirect_to))
...
def reset_password(user: str) -> str:
	try:
		...
		dontmanage.local.response["http_status_code"] = 404
...
def user_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if filters:
		if not (filters.get("ignore_user_type") and dontmanage.session.data.user_type == "System User"):
...
def get_website_users():
	...
	return dontmanage.db.count("User", filters={"enabled": True, "user_type": "Website User"})
...
def get_permission_query_conditions(user):
	if user == "Administrator":
		...
		return """(`tabUser`.name not in ({standard_users}))""".format(
			standard_users=", ".join(dontmanage.db.escape(user) for user in STANDARD_USERS)
		)
...
def update_gravatar(name):
	...
	if gravatar:
		dontmanage.db.set_value("User", name, "user_image", gravatar)
...
def throttle_user_creation():
	...
	if dontmanage.db.get_creation_count("User", 60) > dontmanage.local.conf.get("throttle_user_limit", 60):
		dontmanage.throw(_("Throttled"))
...
def generate_keys(user: str):
	...
	if not user_details.api_key:
		api_key = dontmanage.generate_hash(length=15)
...
def switch_theme(theme):
	if theme in ["Dark", "Light", "Automatic"]:
		dontmanage.db.set_value("User", dontmanage.session.user, "desk_theme", theme)
...
def impersonate(user: str, reason: str):
	dontmanage.has_permission("User", "impersonate")
...
def clear_session(sid_hash: str):
	from dontmanage.sessions import delete_session

	sessions = dontmanage.qb.DocType("Sessions")
	sessions_data = (
		dontmanage.qb.from_(sessions).select(sessions.sid).where(sessions.user == dontmanage.session.user)
	).run(pluck=True)

	for session in sessions_data:
		if sha256_hash(session) == sid_hash:
			delete_session(sid=session, reason="Force Logged out by the user", user=dontmanage.session.user)
			dontmanage.toast(_("Successfully signed out"))
			return

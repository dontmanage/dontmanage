...
from dontmanage.website.utils import is_signup_disabled
...
class User(Document):
	...
	def is_system_manager_disabled(self):
		return dontmanage.db.get_value("Role", {"name": "System Manager"}, ["disabled"])
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(
			dontmanage.db.get_single_value("System Settings", "allow_login_using_mobile_number")
		)
		login_with_username = cint(
			dontmanage.db.get_single_value("System Settings", "allow_login_using_user_name")
		)
...
def get_roles(arg=None):
	...
	return dontmanage.get_roles(dontmanage.form_dict["uid"])
...
def update_password(
	...
	logout_all_sessions = cint(logout_all_sessions) or dontmanage.db.get_single_value(
	...
	if key:
		...
		if result.user:
			reset_password_link_expiry = cint(
				dontmanage.db.get_single_value("System Settings", "reset_password_link_expiry_duration")
			)
			if (
...
def sign_up(email: str, full_name: str, redirect_to: str) -> tuple[int, str]:
	...
	if user:
		...
		if dontmanage.db.get_creation_count("User", 60) > 300:
		...
		default_role = dontmanage.db.get_single_value("Portal Settings", "default_role")
		if default_role:
...
def user_query(doctype, txt, searchfield, start, page_len, filters):
	...
	""".format(
			...
			standard_users=", ".join(dontmanage.db.escape(u) for u in STANDARD_USERS),
			key=searchfield,

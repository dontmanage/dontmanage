...
import dontmanage
from dontmanage.core.doctype.version.version import (
	_as_string,
	_generate_html_diff,
...
class TestVersion(IntegrationTestCase):
	def test_onload_generates_html_diffs_for_multiline(self):
		...
		version = dontmanage.get_doc(
			doctype="Version",
			ref_doctype="ToDo",
			docname="test-doc",
			data=dontmanage.as_json({"changed": [["description", "line1\nline2", "line1\nmodified"]]}),
		)
	...
	def test_onload_generates_html_diffs_for_long_text(self):
		...
		version = dontmanage.get_doc(
			...
			ref_doctype="ToDo",
			docname="test-doc",
			data=dontmanage.as_json({"changed": [["notes", "x" * 81, "y" * 81]]}),
		)
	...
	def test_onload_no_html_diffs_for_simple_changes(self):
		...
		version = dontmanage.get_doc(
			...
			ref_doctype="ToDo",
			docname="test-doc",
			data=dontmanage.as_json({"changed": [["status", "Open", "Closed"]]}),
		)
	...
	def test_onload_handles_empty_data(self):
		...
		version.data = dontmanage.as_json({"changed": []})
		version.onload()
		self.assertIsNone(version.get_onload().get("html_diffs"))
	...
	def test_get_diff(self):
		...
		old_doc = dontmanage.get_doc("Event", test_records[0])
	...
	def test_no_version_on_new_doc(self):
		from dontmanage.desk.form.load import get_versions

		t = dontmanage.get_doc(doctype="ToDo", description="something")
		t.save(ignore_version=False)

		self.assertFalse(get_versions(t))

		t = dontmanage.get_doc(t.doctype, t.name)

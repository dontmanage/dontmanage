...
import dontmanage
from dontmanage.model import datetime_fields, no_value_fields, table_fields
...
class Version(Document):
	...
	def set_impersonator(data):
		if not dontmanage.session:
			return
		if impersonator := dontmanage.session.data.get("impersonated_by"):
			data["impersonated_by"] = impersonator

		if audit_user := dontmanage.session.data.get("audit_user"):
	...
	def set_diff(self, old: Document, new: Document) -> bool:
		...
		if diff:
			...
			self.data = dontmanage.as_json(diff, indent=None, separators=(",", ":"))
			return True
		else:
			return False
	...
	def for_insert(self, doc: Document) -> bool:
		...
		self.data = dontmanage.as_json(data, indent=None, separators=(",", ":"))
		return True
				...
				if doctype:
					...
					if (field_meta := meta.get_field(df.fieldname)) and field_meta.fieldtype == "Link":
						...
						if link_meta.show_title_field_in_link and (
							...
							result = dontmanage.db.get_values(
								field_meta.options,
...
def on_doctype_update():
	dontmanage.db.add_index("Version", ["ref_doctype", "docname"])
...
def _generate_html_diff(old_str: str, new_str: str) -> str | None:
	...
	html_diff = differ.make_table(
		old_lines,
		new_lines,
		fromdesc=dontmanage._("Original"),
		todesc=dontmanage._("New"),
		context=True,
		numlines=3,
	)
	return html_diff

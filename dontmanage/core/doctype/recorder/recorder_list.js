dontmanage.listview_settings["Recorder"] = {
	...
	onload(listview) {
		...
		listview.page.add_button(__("Clear"), () => {
			dontmanage.xcall("dontmanage.recorder.delete").then(listview.refresh);
		});
		...
		listview.page.add_menu_item(__("Import"), () => {
			new dontmanage.ui.FileUploader({
				...
				on_success: (file) => {
					...
					dontmanage.call({
						method: "dontmanage.recorder.import_data",
						args: {
							file: file.file_url,
						},
						callback: function () {
		...
		listview.page.add_menu_item(__("Export"), () => {
			dontmanage.call({
				method: "dontmanage.recorder.export_data",
				callback: function (r) {
	...
	fetch_recorder_status(listview) {
		return dontmanage.xcall("dontmanage.recorder.status").then((status) => {
	...
	stop_recorder(listview) {
		let me = this;
		dontmanage.xcall("dontmanage.recorder.stop", {}).then(() => {
			listview.refresh();
	...
	start_recorder(listview) {
		...
		dontmanage.prompt(
			[
				...
				},
				{
					...
					description: `This will be used for filtering paths which will be recorded.
						...
						e.g. <code>/api/method/dontmanageerp</code>. Leave it empty to record every request.`,
				},
				{
					fieldtype: "Column Break",
			...
			(values) => {
				dontmanage.xcall("dontmanage.recorder.start", values).then(() => {
					listview.refresh();

...
from dontmanage.core.doctype.recorder.db_optimizer import DBOptimizer, DBTable
from dontmanage.custom.doctype.property_setter.property_setter import make_property_setter
from dontmanage.model.document import Document
from dontmanage.recorder import RECORDER_REQUEST_HASH
from dontmanage.recorder import get as get_recorder_data
from dontmanage.utils import cint, cstr, evaluate_filters, get_table_name
...
class Recorder(Document):
	...
	if TYPE_CHECKING:
		from dontmanage.core.doctype.recorder_query.recorder_query import RecorderQuery
		from dontmanage.core.doctype.recorder_suggested_index.recorder_suggested_index import (
...
def serialize_request(request):
	request = dontmanage._dict(request)
	if request.get("calls"):
		for i in request.calls:
			i["stack"] = dontmanage.as_json(i["stack"])
			i["explain_result"] = dontmanage.as_json(i["explain_result"])
	request.update(
		name=request.get("uuid"),
		number_of_queries=request.get("queries"),
		time_in_queries=request.get("time_queries"),
		request_headers=dontmanage.as_json(request.get("headers", {}), indent=4),
		form_dict=dontmanage.as_json(request.get("form_dict", {}), indent=4),
...
def add_indexes(indexes):
	...
	for index in indexes:
		dontmanage.enqueue(_add_index, table=index["table"], column=index["column"])
	dontmanage.msgprint(_("Enqueued creation of indexes"), alert=True)


def _add_index(table, column):
	doctype = get_doctype_name(table)
	dontmanage.db.add_index(doctype, [column])
	dontmanage.msgprint(
...
def optimize(recorder_id: str):
	dontmanage.only_for("Administrator")
	dontmanage.enqueue(_optimize, recorder_id=recorder_id, queue="long")


def _optimize(recorder_id):
	record: Recorder = dontmanage.get_doc("Recorder", recorder_id)
	...
	data = dontmanage.cache.hget(RECORDER_REQUEST_HASH, record.name)
	data["suggested_indexes"] = [{"table": idx[0][0], "column": idx[0][1]} for idx in suggested_indexes]
	dontmanage.cache.hset(RECORDER_REQUEST_HASH, record.name, data)
	dontmanage.publish_realtime("recorder-analysis-complete", user=dontmanage.session.user)
	dontmanage.msgprint(_("Query analysis complete. Check suggested indexes."), realtime=True, alert=True)
...
def _optimize_query(query):
	...
	for table in tables:
		...
		if not stats:
			return
		db_table = DBTable.from_dontmanage_ouput(stats)
...
def _fetch_table_stats(doctype: str, columns: list[str]) -> dict | None:
	...
	if not dontmanage.db.table_exists(doctype):
		return
...
def _get_column_cardinality(table, column):
	return dontmanage.db.sql(f"select count(distinct {column}) from {table}")[0][0]

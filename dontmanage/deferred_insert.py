...
def deferred_insert(doctype: str, records: list[dict | "Document"] | str):
	...
	try:
		dontmanage.cache.rpush(f"{queue_prefix}{doctype}", _records)
...
def save_to_db():
	queue_keys = dontmanage.cache.get_keys(queue_prefix)
	for key in queue_keys:
		record_count = 0
		queue_key = get_key_name(key)
		doctype = get_doctype_name(key)
		while dontmanage.cache.llen(queue_key) > 0 and record_count <= 10000:
			records = dontmanage.cache.lpop(queue_key)
...
def insert_record(record: dict | "Document", doctype: str):
	try:
		...
		dontmanage.get_doc(record).insert()
	except Exception as e:
		dontmanage.logger().error(f"Error while inserting deferred {doctype} record: {e}")
